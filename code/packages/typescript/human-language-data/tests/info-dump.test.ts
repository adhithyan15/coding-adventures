...
describe("the committed corpus", () => {
  ...
  it("pins the first measurement", () => {
    ...
    expect(report.summary.lessons).toBe(1825); // +8: HL-C94 payoff lessons // +4: HL-C98
    ...
    expect(report.summary.ruleStatements).toBe(21); // +1: ES-C02-concordancia states exactly ONE rule, which is the budget
    expect(report.summary.paradigmTables).toBe(95);
    expect(report.summary.fullParadigmGrids).toBe(22);
    expect(report.summary.lessonsWithFindings).toBe(112);
  });
