...
//!
//! Multiply uses fixed partial-product gate networks, divide uses a fixed
//! restoring network, and the segment left-shift (`seg × 16 = seg << 4`) is
//! wiring rather than arithmetic.
//!
//! # Memory model
//!
//! One MiB stored in 8,388,608 D flip-flops. Physical address =
//! (seg×16 + off) & 0xFFFFF.
//!
//! # Interrupts
//!
//! `INT3`, `INT n`, and a taken `INTO` are repository protocol halt boundaries;
//! `IRET` restores IP, CS, and FLAGS from the architectural stack.
//!
//! # Halt
//!
//! `HLT` (0xF4) sets `halted = true`. The checked API returns
//! `Intel8086Error::Halted`; the preserved legacy `step()` asserts instead.

#![allow(clippy::too_many_lines)]

use crate::alu::{
    aaa, aad, aam, aas, add16, add8, and16, and8, daa, das, dec16, dec8, div16, div8, idiv16,
    idiv8, imul16, imul8, inc16, inc8, mul16, mul8, neg16, neg8, not16, not8, or16, or8, rcl, rcr,
    rol, ror, sar, shl, shr, sub16, sub8, xor16, xor8, AluResult8086,
};
use crate::bits::{add_16bit, compute_parity, compute_zero, int_to_bits8, invert_16bit};
use crate::registers::{add_20bit, RegisterFile8086};
use crate::state::{DffMemory, StateRegister};
use intel8086_simulator::{
...
/// Exact persistent-state topology: 8,388,608 memory bits, 208 register
/// bits, nine FLAGS bits, one halt bit, and 4,096 port-latch bits.
pub const FLIP_FLOP_COUNT: usize = DffMemory::DFF_COUNT + 13 * 16 + 9 + 1 + 2 * PORT_SIZE * 8;

// ─── Public CPU state snapshot ────────────────────────────────────────────────

/// Backward-compatible name for the complete shared architectural state.
pub type CpuState = Intel8086State;
...
pub struct Cpu8086 {
    ...
    input_port_state: [StateRegister; PORT_SIZE],
    output_port_state: [StateRegister; PORT_SIZE],
}
...
impl Cpu8086 {
    ...
    pub fn new() -> Self {
        let mut cpu = Cpu8086 {
            ...
            input_port_state: std::array::from_fn(|_| StateRegister::new(8)),
            output_port_state: std::array::from_fn(|_| StateRegister::new(8)),
        };
    ...
    pub fn load(&mut self, program: &[u8], origin: usize) {
        ...
        self.mem.copy_from_slice(origin, &program[..end - origin]);
    }
    ...
    /// Restore a complete shared state atomically.
    pub fn restore(&mut self, state: &Intel8086State) -> Result<(), Intel8086Error> {
        ...
        self.rf.es = state.es;
        self.rf.ip = state.ip;
        self.rf.unpack_flags(state.flags);
        self.halted = state.halted;
        self.input_ports = state.input_ports;
        self.output_ports = state.output_ports;
        self.mem = DffMemory::new();
        self.mem.copy_from_slice(0, &state.memory);
        self.clock_wires_into_state();
        Ok(())
    }

    /// Checked, atomic program load at a physical address.
    pub fn load_checked(&mut self, program: &[u8], origin: usize) -> Result<(), Intel8086Error> {
        let end = origin
        ...
        self.mem.copy_from_slice(origin, program);
        self.rf.ip = 0;
        self.halted = false;
        self.clock_wires_into_state();
        Ok(())
    ...
    pub fn set_input_port(&mut self, port: usize, value: u8) -> Result<(), Intel8086Error> {
        ...
        *latch = value;
        self.input_port_state[port].write(u16::from(value));
        Ok(())
    }
    ...
    /// Execute one validated instruction and return a complete shared trace.
    pub fn step_checked(&mut self) -> Result<StepTrace, Intel8086Error> {
        ...
        let mut oracle = Intel8086Simulator::new(MEM_SIZE);
    ...
    /// Run already-loaded code transactionally.
    pub fn run_loaded_checked(
        &mut self,
        max_steps: usize,
    ) -> Result<ExecutionResult, Intel8086Error> {
        let entry = self.get_state();
    ...
    pub fn run_checked(
        ...
        candidate.load_checked(program, 0)?;
    ...
    fn clock_wires_into_state(&mut self) {
        ...
        for port in 0..PORT_SIZE {
            self.input_port_state[port].write(u16::from(self.input_ports[port]));
            self.output_port_state[port].write(u16::from(self.output_ports[port]));
        }
    }

    /// Read one physical memory byte from the DFF-backed memory array.
    pub fn read_memory(&self, address: usize) -> u8 {
        self.mem.read(address)
    }

    /// Clock one physical memory byte into the DFF-backed memory array.
    pub fn write_memory(&mut self, address: usize, value: u8) {
        self.mem.write(address, value);
    }

    /// Return all architectural memory bytes from the latched Q bus.
    pub fn memory_snapshot(&self) -> Box<[u8]> {
        self.mem.snapshot()
    }
    ...
    fn read_byte(&self, seg: u16, offset: u16) -> u8 {
        self.mem.read(self.phys(seg, offset))
    }
    ...
    fn read_word(&self, seg: u16, offset: u16) -> u16 {
        let lo = self.mem.read(self.phys(seg, offset));
        let hi = self.mem.read(self.phys(seg, offset.wrapping_add(1)));
    ...
    fn write_word(&mut self, seg: u16, offset: u16, value: u16) {
        ...
        self.mem.write(addr_lo, (value & 0xFF) as u8);
        self.mem.write(addr_hi, ((value >> 8) & 0xFF) as u8);
    }
    ...
    fn fetch8(&mut self) -> u8 {
        let ip = self.rf.ip;
        let cs = self.rf.cs;
        let v = self.mem.read(self.phys(cs, ip));
    ...
    fn decode_modrm(&mut self, modrm: u8, _word: bool) -> (u8, u8, u8, u16) {
        ...
        let base: u16 = match rm {
            0 => {
                let (v, _, _) = add_16bit(bx, si, 0);
                v
            }
            1 => {
                let (v, _, _) = add_16bit(bx, di, 0);
                v
            }
            2 => {
                let (v, _, _) = add_16bit(bp, si, 0);
                v
            }
            3 => {
                let (v, _, _) = add_16bit(bp, di, 0);
                v
            }
            4 => si,
            5 => di,
    ...
    fn write_rm(&mut self, mod_: u8, rm: u8, seg: u16, ea: u16, val: u16, word: bool) {
        if mod_ == 3 {
            if word {
                self.rf.write16(rm, val);
            } else {
                self.rf.write8(rm, val as u8);
            }
        } else if word {
    ...
    fn alu_op(&mut self, op: u8, a: u16, b: u16, word: bool) -> (u16, u8) {
        ...
        let result = if word {
            ...
            match op {
                0 => {
                    let r = add8(a as u8, b as u8, 0);
                    self.apply_alu_result(&r);
                    r.result
                }
                1 => {
                    let r = or8(a as u8, b as u8);
                    self.apply_logic_flags(&r);
                    r.result
                }
                2 => {
                    let r = add8(a as u8, b as u8, cf);
                    self.apply_alu_result(&r);
                    r.result
                }
                3 => {
                    let r = sub8(a as u8, b as u8, cf);
                    self.apply_alu_result(&r);
                    r.result
                }
                4 => {
                    let r = and8(a as u8, b as u8);
                    self.apply_logic_flags(&r);
                    r.result
                }
                5 => {
                    let r = sub8(a as u8, b as u8, 0);
                    self.apply_alu_result(&r);
                    r.result
                }
                6 => {
                    let r = xor8(a as u8, b as u8);
                    self.apply_logic_flags(&r);
                    r.result
                }
                _ => {
    ...
    fn exec_cmps(&mut self, word: bool, step: i16, seg_src: u16, rep: Option<u8>) {
        ...
        loop {
            ...
            let di = self.rf.di;
            let a = if word {
                self.read_word(seg_src, si)
            } else {
                self.read_byte(seg_src, si) as u16
            };
            let b = if word {
                self.read_word(es, di)
            } else {
                self.read_byte(es, di) as u16
            };
            let r = if word {
                sub16(a, b, 0)
            } else {
                sub8(a as u8, b as u8, 0)
            };
            self.apply_alu_result(&r);
            let (new_si, _, _) = add_16bit(si, step_u, 0);
            let (new_di, _, _) = add_16bit(di, step_u, 0);
    ...
    fn exec_op(&mut self, op: u8, seg_override: Option<u8>, rep_prefix: Option<u8>) {
        ...
        if matches!(op, 0x88..=0x8B) {
            ...
            if d {
                ...
                if word {
                    self.rf.write16(reg, src);
                } else {
                    self.rf.write8(reg, src as u8);
                }
            } else {
                let src = if word {
                    self.rf.read16(reg)
                } else {
                    self.rf.read8(reg) as u16
                };
        ...
        if op == 0xA2 || op == 0xA3 {
            ...
            let seg = seg_override.map_or(self.rf.ds, |s| self.rf.read_seg(s));
            if word {
                self.write_word(seg, addr, self.rf.ax);
            } else {
                self.write_byte(seg, addr, (self.rf.ax & 0xFF) as u8);
            }
            return;
        }
        ...
        if op == 0x86 || op == 0x87 {
            ...
            if word {
                self.rf.write16(reg, a);
            } else {
                self.rf.write8(reg, a as u8);
            }
            return;
        }
        ...
        // PUSHF / POPF
        if op == 0x9C {
            let f = self.rf.pack_flags();
            self.push16(f);
            return;
        }
        if op == 0x9D {
            let v = self.pop16();
            self.rf.unpack_flags(v);
            return;
        }
            ...
            if let Some(&(_, alu_op, word)) = ACC_IMM.iter().find(|&&(o, _, _)| o == op) {
                ...
                if alu_op != 7 && op != 0xA8 && op != 0xA9 {
                    if word {
                        ...
                        self.rf.ax = (self.rf.ax & 0xFF00) | (result & 0xFF);
                    }
                }
                return;
            }
        ...
        // Standard ALU r/m ↔ reg (00–3B)
        {
            ...
            if let Some(&(_, alu_op, word, d_bit)) = STD_ALU.iter().find(|&&(o, _, _, _)| o == op) {
                ...
                if d_bit {
                    ...
                    if alu_op != 7 {
                        if word {
                            self.rf.write16(reg, result);
                        } else {
                            self.rf.write8(reg, result as u8);
                        }
                    }
                } else {
                    let a = self.read_rm(mod_, rm, seg, ea, word);
            ...
            match ext {
                ...
                4 => {
                    ...
                    if word {
                        ...
                        self.rf.ax = ax;
                        self.rf.dx = dx;
                        self.rf.flag_cf = cf_of;
                        self.rf.flag_of = cf_of;
                    } else {
                ...
                6 => {
                    ...
                    if word {
                        ...
                        match div8(self.rf.ax, a as u8) {
                            ...
                            Some((q, r)) => {
                                self.rf.ax = ((r as u16) << 8) | (q as u16);
                            }
                        }
                    }
                }
        ...
        if matches!(op, 0xD0..=0xD3) {
            let word = (op & 1) != 0;
            let count: u8 = if op < 0xD2 {
                1
            } else {
                (self.rf.cx & 0xFF) as u8
            };
            let modrm = self.fetch8();
            let ext = (modrm >> 3) & 7;
            let rm = modrm & 7;
        ...
        if op == 0xCC {
            // INT 3 — repository protocol halt boundary
            self.halted = true;
            return;
        }
        if op == 0xCD {
        ...
        if op == 0xCE {
            // INTO — repository protocol halt boundary
            self.halted = true;
            return;
        }
        if op == 0xCF {
        ...
        if op == 0xF5 {
            self.rf.flag_cf = 1 - self.rf.flag_cf;
            return;
        } // CMC
        if op == 0xFC {
            self.rf.flag_df = 0;
...
mod tests {
    ...
    #[test]
    fn exact_persistent_topology_and_latched_outputs() {
        assert_eq!(FLIP_FLOP_COUNT, 8_392_922);
        let mut cpu = Cpu8086::new();
        cpu.rf.ax = 0x1234;
        cpu.rf.es = 0xabcd;
        cpu.rf.flag_cf = 1;
        cpu.input_ports[7] = 0x5a;
        cpu.output_ports[9] = 0xa5;
        cpu.halted = true;
        cpu.clock_wires_into_state();
        let (registers, flags) = cpu.rf.latched_values();
        assert_eq!(registers[0], 0x1234);
        assert_eq!(registers[11], 0xabcd);
        assert_eq!(flags[0], 1);
        assert_eq!(cpu.input_port_state[7].read(), 0x5a);
        assert_eq!(cpu.output_port_state[9].read(), 0xa5);
        assert_eq!(cpu.halt_state.read(), 1);
        cpu.write_memory(0xfffff, 0x3c);
        assert_eq!(cpu.read_memory(0xfffff), 0x3c);
    }
    ...
    fn string_movs() {
        ...
        let mut cpu = Cpu8086::new();
        cpu.write_memory(0x200, 0xAA); // source byte
        cpu.write_memory(0, 0xA4); // MOVSB
        cpu.rf.si = 0x200;
        cpu.rf.di = 0x100;
        cpu.step(); // execute MOVSB
        assert_eq!(cpu.read_memory(0x100), 0xAA);
    }
    ...
    fn physical_address_wrapping() {
        ...
        cpu.rf.cs = 0x0100;
        cpu.rf.ip = 0;
        cpu.write_memory(0x1000, 0xF4); // HLT at physical 0x1000
        cpu.halted = false;
        cpu.step();
        assert!(cpu.halted);
    }
