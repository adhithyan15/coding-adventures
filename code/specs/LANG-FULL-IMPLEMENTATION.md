...
- ✅ **AL14** — direct formal procedures. A `procedure p` formal accepts a
  direct declared procedure actual, then the compiler emits a specialised
  sibling with no IIR argument for `p`; calls through `p` resolve to the
  actual's existing checked signature. An active formal can be forwarded into
  a nested direct wrapper, so `dispatch(square, 6)` through `forward(p, x)`
  returns 36 on native/LLVM/WASM/JVM/CLR/VM/JIT. This intentionally excludes
  expression/scalar actuals, standard procedures, `value procedure` formals,
  and dynamic/nonlocal procedure values, which require the descriptor ABI
  described by LANG27.
