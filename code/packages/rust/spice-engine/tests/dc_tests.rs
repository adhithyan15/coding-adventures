...
#[test]
fn mos_model_card_rejects_negative_or_non_finite_bottom_junction_capacitance() {
    for value in [0.0, 2.0e-3, 1.0] {
        let valid = normalize_model_card("Mvalid", "nmos", &[("CJ", value)]).unwrap();
        assert_close(*valid.parameters.get("CJ").unwrap(), value);
    }
    ...
    for invalid_capacitance in [f64::NEG_INFINITY, -0.1, f64::INFINITY, f64::NAN] {
        assert!(matches!(
            ...
            Err(SpiceError::InvalidElement { reason, .. })
                if reason == "MOSFET CJ must be finite and non-negative"
        ));
    }
}
...
#[test]
fn mos_model_card_rejects_negative_or_non_finite_source_resistance() {
    for value in [0.0, 10.0, 1.0e6] {
        let valid = normalize_model_card("Mvalid", "pmos", &[("RS", value)]).unwrap();
        assert_close(*valid.parameters.get("RS").unwrap(), value);
    }
    ...
    for invalid_resistance in [f64::NEG_INFINITY, -0.1, f64::INFINITY, f64::NAN] {
        assert!(matches!(
            ...
            Err(SpiceError::InvalidElement { reason, .. })
                if reason == "MOSFET RS must be finite and non-negative"
        ));
    }
}
...
#[test]
fn mos_model_card_rejects_negative_or_non_finite_flicker_noise_exponent() {
    for value in [0.0, 1.0, 2.0] {
        let valid = normalize_model_card("Mvalid", "nmos", &[("AF", value)]).unwrap();
        assert_close(*valid.parameters.get("AF").unwrap(), value);
    }

    for invalid_exponent in [f64::NEG_INFINITY, -0.1, f64::INFINITY, f64::NAN] {
        assert!(matches!(
            normalize_model_card("Minvalid", "nmos", &[("AF", invalid_exponent)]),
            Err(SpiceError::InvalidElement { reason, .. })
                if reason == "MOSFET AF must be finite and non-negative"
        ));
    }
}
