#!/bin/sh
# Build and run the jit-compiler tests under EVERY available C/C++ compiler
# (pure ISO), via the shared iso-harness (code/packages/c/iso-harness).
set -e
...
iso_build_and_run c jit_compiler-tests tests/jit_compiler_test.c src/jit_compiler.c
