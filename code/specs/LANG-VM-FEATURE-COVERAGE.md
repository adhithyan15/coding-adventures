...
Audit base: `cd73f3ad86` (2026-09-05); corpus counts updated for VM-046b. This is an inventory of the implemented
...
| COBOL-60 | 36 | 252 | Much larger frontend JIT/oracle suite |
...
The normal non-ALGOL capstone therefore declares 184 programs and 1308 cells.
...
| [COBOL lowerer](../packages/rust/cobol-iir-compiler/src/lib.rs) | PICTURE/scaled arithmetic, DISPLAY/MOVE, condition names, IF/EVALUATE, PERFORM/GOTO, COMPUTE/power, size errors and signed/alphanumeric operations occur in the matrix, now expanded to 36 rows with ASCII reference modification: literal/computed bounds, comparisons, MOVE fitting and invalid-bound traps, plus STRING SIZE full-width copying, truncation and untouched tails, STRING delimiters and UNSTRING field fitting/empty fields/exhaustion. The [JIT/oracle suite](../packages/rust/cobol-iir-compiler/tests/jit_e2e.rs) additionally exercises reference modification, STRING, UNSTRING and INSPECT families. | VM-045 adds reference-modification rows; VM-046a/b add STRING SIZE and delimiter/splitting rows; pointer/overflow and INSPECT proofs remain VM-046c and VM-047. Validator acceptance is insufficient. Existing byte/character and category restrictions must remain explicit. |
