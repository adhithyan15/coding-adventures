...
pub fn read_v11_collection_bytes(data: &[u8]) -> Result<Vec<u8>, ApkgError> {
    ...
    let entries = archive_entries(&reader);
    let collection = collection_member(&reader, &entries)?;
    // The collection member gets the same expansion budget the media files
    // already had, and for the stronger reason.
    //
    // A zip entry's uncompressed size is a `u32`, and the deflate reader's only
    // bound is that declared size -- so a small archive may declare a ~4 GiB
    // `collection.anki2` and, with a compressible stream, actually produce it.
    // Both the CRC and the length check are satisfiable by whoever wrote the
    // archive, so neither catches it. The allocation that follows does not fail
    // recoverably: Rust's allocation error handler ABORTS, which neither
    // `catch_json`'s `catch_unwind` nor the C ABI's can intercept, so the host
    // process dies and takes the reader's unsaved collection with it.
    //
    // Checked against the DECLARED size, before the read, because the point is
    // to never make the allocation. The modern branch needs no equivalent --
    // zstd caps its own output.
    //
    // The CEILING alone, deliberately not `media_budget`'s size-proportional
    // ratio. That ratio is calibrated for media, which is already compressed
    // and barely expands; a collection is SQLite full of repetitive text and
    // routinely expands sixty-fold or more. Applying the ratio would refuse
    // ordinary decks -- a 500 KiB archive holding a 30 MiB text-only collection
    // is a real deck, and it would have failed against a 24 MiB budget. The
    // ceiling still turns a 4 GiB allocation into a sentence, which is the
    // whole job here.
    if u64::from(collection.size) > MEDIA_EXPANSION_CEILING {
        return Err(apkg_error(format!(
            "collection member declares {} bytes, past the limit of {MEDIA_EXPANSION_CEILING}",
            collection.size
        )));
    }
    let bytes = reader
...
"#,
  ...
  "3": {"id": 3, "name": "Filtered::Today", "desc": "Filtered cram deck", "dyn": 1, "conf": 1, "terms": [["deck:Spanish", 10, 0]], "resched": true}
    ...
    /// A tiny archive that DECLARES a huge collection is refused before the
    /// allocation, not after.
    ///
    /// Built by hand rather than through the writer, because the writer only
    /// ever declares sizes it really produced -- the whole point is a header
    /// that lies. The refusal has to come from the declared size, since acting
    /// on the real one means having already made the allocation this prevents.
    #[test]
    fn a_collection_member_declaring_more_than_its_budget_is_refused() {
        let archive = zip_declaring_collection_size(u32::MAX);
        let budget = media_budget(archive.len());
        assert!(
            u64::from(u32::MAX) > budget,
            "the fixture must actually exceed the budget, or this proves nothing"
        );
        let error = read_v11_collection_bytes(&archive)
            .expect_err("a collection declaring 4 GiB must be refused");
        assert!(
            error.message.contains("past the limit"),
            "the refusal must say why: {}",
            error.message
        );
    }

    /// A small archive declaring a large-but-ordinary collection is allowed.
    ///
    /// This is the false positive the size-proportional media ratio would have
    /// caused: SQLite full of repetitive text compresses far harder than media
    /// does, so a 500 KiB archive holding a 30 MiB collection is a real deck,
    /// not an attack. Pinned because the tempting fix -- reusing `media_budget`
    /// -- refuses it.
    #[test]
    fn a_dense_text_deck_is_not_mistaken_for_a_bomb() {
        let thirty_megabytes = 30 * 1024 * 1024;
        assert!(
            u64::from(thirty_megabytes) <= MEDIA_EXPANSION_CEILING,
            "an ordinary dense deck must be under the ceiling"
        );
        let archive = zip_declaring_collection_size(thirty_megabytes);
        // Far below what `media_budget` would have allowed for an archive this
        // small, which is exactly why the ratio is not used here.
        assert!(
            u64::from(thirty_megabytes) > media_budget(archive.len()),
            "the ratio-based budget would have refused this deck"
        );
        if let Err(error) = read_v11_collection_bytes(&archive) {
            assert!(
                !error.message.contains("past the limit"),
                "an ordinary deck must not be refused for size: {}",
                error.message
            );
        }
    }

    /// The same archive, declaring a size within budget, is NOT refused by this
    /// check -- so the guard above is a bound rather than a blanket rejection.
    #[test]
    fn a_collection_member_within_budget_passes_the_size_guard() {
        let archive = zip_declaring_collection_size(64);
        let outcome = read_v11_collection_bytes(&archive);
        if let Err(error) = outcome {
            assert!(
                !error.message.contains("past the limit"),
                "a within-budget member must not be refused for size: {}",
                error.message
            );
        }
    }

    /// A single stored `collection.anki2` whose declared uncompressed size is
    /// whatever the caller wants, regardless of the bytes actually present.
    fn zip_declaring_collection_size(declared: u32) -> Vec<u8> {
        const NAME: &[u8] = b"collection.anki2";
        // SQLite's header, so the member is recognised as a collection at all.
        let body: &[u8] = b"SQLite format 3\0";
        let crc = crc32(body);
        let mut out = Vec::new();
        // Local file header.
        ...
        out.extend_from_slice(&declared.to_le_bytes()); // uncompressed: the lie
