...
This gem is a thin wrapper around `coding_adventures_parser`'s `GrammarDrivenParser`. It loads `excel.grammar` and delegates all parsing to the general-purpose engine.
...
ast = CodingAdventures::ExcelParser.parse("let x = 1 + 2;")
puts ast.rule_name  # "program"
```

## Dependencies

- `coding_adventures_excel_lexer` -- tokenizes JavaScript source code
