...
**Status:** implemented on every in-scope backend — mosstyle contract,
XAML, Compose, Qt, and Flutter — see §4. (SwiftUI is tracked separately
in #13206, unbuildable on this dev box; web backends never needed
`elevation`, see §4's react/html/webcomponent row.)
...
| Compose | **implemented** — `part_elevation_tier` reads `elevation` directly inside `compose_box_style`, the single function every styled container/control (`Box`/`Row`/`Column`, `HostButton`, `HostDraggable`'s `Column` delegation, …) already funnels its modifier chain through, so implementing it once covered every call site — unlike XAML, which needed separate wiring for `Row`/`Column`/`HostDraggable`. `raised` → `Modifier.shadow(4.dp)`, `overlay` → `Modifier.shadow(16.dp)` (same numbers as XAML's `ElevationTier::translation_z`), emitted right after `.width`/`.height` and before `.background`/`.border` (shadow-then-background is Compose's documented ordering; reversed order still compiles but the background would occlude the shadow). Verified against the real toolchain — and a real version correction: an initial scratch probe assumed the `org.jetbrains.compose` 1.6.11 pin cited in this crate's own `HostProgressRing`/`Path` CHANGELOG entries, but `mosaic-compile pkg --backend compose --emit-project` against the real `task-app` package showed the generator now pins **1.11.1**/Kotlin **2.3.21** (version drift since those PRs landed). Re-verified against the real pin: `gradle compileKotlin` on the real generated `TaskApp.kt` inside the real generated project scaffold — `BUILD SUCCESSFUL`; `gradle run` launched with no Compose crash or exception. Confirmed all 13 real `elevation`-declaring parts in `TaskApp.light.msl` produce a `.shadow(...)` in the generated output, across `Box`/`Row`/`Column`/`HostButton` call sites, plus `ProjectNav`'s own `elevation` part flowing through correctly when composed as a TaskApp child package. |
| Qt | **implemented** — `part_elevation_tier` wired into five separate call sites (`Box`, `Row`/`Column`/`Stack`, `HostButton`, `HostDraggable`, `HostSurface`) since Qt has no single shared style function like Compose's `compose_box_style`. `raised`/`overlay` → a `QtQuick.Effects.MultiEffect` drop shadow (4px/16px vertical offset, matching XAML/Compose's numbers). Two real bugs found via a real `qml`/`qmlscene` launch of the actual generated `TaskApp`/`ProjectNav` packages (not caught by `qmllint`'s static pass): (1) a shadowed element that's the sole child of an `If`/`Else` conditional broke `Component`'s single-top-level-child rule with a hard "Invalid component body specification" load failure — real `notes-row-on`/`project-on` toggle buttons hit this; (2) a `MultiEffect` anchored inside a `RowLayout`/`ColumnLayout` triggers `qmllint`'s "undefined behavior" layout-positioning warning, confirmed via `grabToImage` to still render correctly on Qt 6.8.1 but worth avoiding outright. Both fixed by `qml_elevation_wrap`, which wraps the shadowed element and its `MultiEffect` in one outer `Item` (the sole `Component` root, never itself layout-managed). Verified against the real toolchain: all four real components build clean via `mosaic-compile pkg --backend qt --profile native-complete`; the real generated `TaskApp.qml` produces a `MultiEffect` for all 13 real `elevation`-declaring parts (16 total instances — matching Compose's identical count against the same source); re-verified clean with `qmllint` (no new warnings) and a real `qmlscene` launch of all four regenerated packages (zero errors); a `grabToImage` screenshot of the real compiled `TaskApp` shows a visible shadow on the brand-mark icon and the task-composer card. |
| Flutter | **implemented** — two separate mechanisms, no shared style function: `Box` → `Container` gets `boxShadow: [BoxShadow(color: 0x40000000, blurRadius: 8/24, offset: Offset(0, 4/16))]` inside `BoxDecoration`; `Row`/`Column`/`Stack` (which have no decoration mechanism of their own) get force-wrapped in a `Container` carrying just the shadow; `HostButton` uses Flutter's own first-class Material `ButtonStyle.elevation` (`4.0`/`16.0`) instead of `BoxShadow` — a materially better fit, verified via a real widget-test golden render showing the shadow deepen monotonically with the value; `HostDraggable` delegates straight to the `Box`/`Container` path with no primitive-specific wiring needed (unlike XAML's real XamlCompiler bug on the same primitive); `HostSurface` stays out of scope (no `part_styles` access in its signature today, no real caller). Verified against the real toolchain: `flutter analyze` clean on a real scratch project (Flutter 3.47.0/Dart 3.13.0); a real `flutter test --update-goldens` render confirmed both `BoxShadow` and `ButtonStyle.elevation` produce a visible shadow — this caught and fixed a real bug (an initial `overlay`-tier `blurRadius: 16` rendered as a flat block, not a shadow; fixed to `24`). All four real components build clean via `mosaic-compile --profile native-complete`; the real generated `TaskApp.dart` produces a shadow for all 13 real `elevation`-declaring parts (16 total instances, matching Compose's and Qt's identical counts against the same source). A full live mount of the complete real `TaskApp` hit a genuine, confirmed-pre-existing (present on the pre-elevation baseline too) Flutter layout crash unrelated to this change — filed as [#13429](https://github.com/adhithyan15/coding-adventures/issues/13429) rather than silently worked around; elevation-specific verification instead used isolated widget-test golden renders plus static analysis and the grep-based coverage check. |
