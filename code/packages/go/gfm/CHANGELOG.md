...
- `ToHtml(markdown string) string` — parse and render Markdown to HTML with raw
  HTML passthrough enabled (full GFM spec compliance).
...
- `TestGFMSpec` — loads the official GFM 0.31.2 `spec.json`
...
- `spec.json` — official GFM 0.31.2 spec test suite (652 examples)
