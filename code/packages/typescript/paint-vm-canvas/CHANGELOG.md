...
- Apply optional exact clip commands through `Path2D` and `ctx.clip(path)`.
...
- Copy pixel bytes into an `ArrayBuffer`-backed clamped view accepted by current
  DOM `ImageData` typings.
