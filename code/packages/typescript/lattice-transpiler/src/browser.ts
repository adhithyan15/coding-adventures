 ...
 *
 * This module is functionally identical to `index.ts` but replaces all
 * Node.js `fs.readFileSync()` calls with embedded grammar strings. The
 * grammar text is stored as string constants at the top of this file.
 *
 * Why a Separate Browser Entry Point?
 * ------------------------------------
 *
 * The standard `lattice-lexer` and `lattice-parser` packages read grammar
 * files from disk at runtime:
 *
 *     readFileSync("code/grammars/lattice.tokens", "utf-8")
 *
 * This works perfectly in Node.js, but browsers do not have a file system.
 * Bundlers (Webpack, esbuild, Vite) would fail to resolve these imports
 * unless the files were explicitly inlined.
 *
 * This module solves the problem by embedding the grammar text as TypeScript
 * string constants. No file system access is needed at runtime.
 *
 * The tradeoff: when the grammar files change, this file must be updated to
 * match. The strings are kept in sync manually. They are clearly labeled
 * with the source file path and the date they were copied.
 *
 * How It Works (Pipeline)
 * -----------------------
 *
 *   Embedded string constants
 *       ↓  parseTokenGrammar(LATTICE_TOKENS_GRAMMAR)
 *   TokenGrammar object
 *       ↓  grammarTokenize(source, tokenGrammar)
 *   Token[]
 *       ↓  parseParserGrammar(LATTICE_PARSER_GRAMMAR)
 *   ParserGrammar object
 *       ↓  new GrammarParser(tokens, parserGrammar).parse()
 *   ASTNode (Lattice AST)
 *       ↓  new LatticeTransformer().transform(ast)
 *   ASTNode (CSS AST)
 *       ↓  new CSSEmitter(indent, minified).emit(cssAst)
 *   CSS text string
 *
 * Usage
 * -----
 ...
 * -------------
 *
 * When bundling for the browser (Vite, Webpack, esbuild, Rollup), use
 * this file as the entry point instead of `index.ts`:
 *
 *     // vite.config.ts
 *     build: {
 *       lib: {
 *         entry: "./src/browser.ts",
 *         name: "LatticeTranspiler",
 *       }
 *     }
 *
 * The bundler will inline all imports and produce a self-contained bundle.
 * No grammar files need to be shipped separately.
 */

import { parseTokenGrammar, parseParserGrammar } from "@coding-adventures/grammar-tools";
...
// =============================================================================
//
// These strings are verbatim copies of the grammar files from:
//   code/grammars/lattice.tokens
//   code/grammars/lattice.grammar
//
// They are embedded here so that no file system access is needed at runtime.
// If you update the grammar files, update these strings to match.

/**
 * The Lattice token grammar.
 *
...
// Re-export TranspileOptions so consumers only need one import
export type { TranspileOptions };

// Re-export error classes for error handling in browser consumers
export {
  LatticeError,
