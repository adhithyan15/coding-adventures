...
non-alphanumeric receiver (a single-character ASCII `DELIMITED BY` delimiter and a
`WITH POINTER p` phrase over a `PIC 9(n)` pointer ARE supported), and an `UNSTRING`
