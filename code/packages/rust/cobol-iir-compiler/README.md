...
field under a delimiter, per-field different delimiters, `WITH POINTER`,
`ON`/`NOT ON OVERFLOW`, a numeric item or figurative as a sending field, or a
non-alphanumeric receiver (a single-character ASCII `DELIMITED BY` delimiter IS
supported), and an `UNSTRING` with a multi-character / `ALL` / `OR`
