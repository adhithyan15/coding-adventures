...
describe("corpus snapshot", () => {
  ...
  it("pins where the corpus actually stands on the ladder", () => {
    ...
    expect(summary.byLevel.A1).toBe(485); // +1: ES-C02-concordancia sits on SPINE-TIME-OF-DAY // +42: HL-C136 wave I. The whole wave lands at A1, not pre-A1, and that is DERIVED rather than chosen: all six chapters realize asking-and-pointing nodes the shared spine declares at A1, so `pre-A1` (1038) does not move at all. The wave is a *pre-A1 lexicon* drive by selection order, which is a statement about which words are worth teaching first, not a claim about which spine node they realize. // HL-C137 wave II: +36 adjective lessons, +6 chapters, all six Indic tracks
    ...
    expect(summary.byLevel.A2).toBe(492); // +39: Spanish chapters 11-18 plus prerequisite closure // +3: HL-C88 slice 8 // +1: HL-C88 slice 9 (falsos amigos) // +3 — the plural rung sits on SPINE-TALK-ABOUT-PAST, which is A2: HL-C113 preterite plural // HL-C113 preterite close // HL-C152: +5 lessons, +1 chapter — Spanish realizes SPINE-NEGATE-AND-ASK, completing A2 at 5/5
