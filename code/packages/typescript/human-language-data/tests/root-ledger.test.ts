...
describe("the committed corpus", () => {
  ...
  it("pins the first ledger, and it is the reason the rule exists", () => {
    ...
    expect(l.summary.neverSpent).toBe(1958); // -6: HL-C94 payoff lessons re-spend roots latin-vos, introduced and not yet re-spent // +1: HL-C98 // +51: vocabulary wave 5 // +6: HL-C88 slices 5-6 // +2: HL-C88 slice 8 // +72: vocabulary wave 6 // +2: HL-C113 (B1 si-condition rung) // HL-C113 preterite close // HL-C113: HL-C113 imperfect subjunctive
  ...
  it("pins Spanish, the pilot track", () => {
    ...
    expect(l.summary).toMatchObject({
      ...
      neverSpent: 218, // +3: HL-C88 slice 4 introduces three roots not yet re-spent // +6: HL-C88 slices 5-6 // +2: HL-C88 slice 8 adds arius-latin and ES-ETYMON-ARIUS // +2: HL-C88 slice 9 adds exitus-latin and ES-ETYMON-EXITUS // +2: B1 si-condition adds si-latin and ES-ETYMON-SI-LATIN // +1: HL-C113 imperfect subjunctive adds only ES-ETYMON-RA-PLUPERFECT; it re-spends fabulari-latin rather than minting a slug for a grammatical category // HL-C113 step 6: +1 -- ES-ETYMON-DIJO-DIXIT, unspent until the reported-speech review revisits it
