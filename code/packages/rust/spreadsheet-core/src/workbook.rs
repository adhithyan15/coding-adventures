...
impl Workbook {
    ...
    fn evaluate_cell(&mut self, sheet: SheetId, addr: CellAddress) {
        ...
        // Snapshot the values needed by `lookup`.
        // We pass `self` indirectly: build a closure that captures a
        // read-only view via a freshly-cloned HashMap of cell values.
        // For a Phase-1 simple engine, cloning is acceptable.
        let snapshot: HashMap<(SheetId, CellAddress), CellValue> = self
            .sheets
            .iter()
            .enumerate()
            .flat_map(|(i, s)| {
                ...
                s.cells
                    .iter()
                    .map(move |(a, c)| ((sid, *a), c.current_value()))
            })
            .collect();
        ...
        let result = match evaluate(&ast, sheet, &lookup) {
            Ok(v) => v,
