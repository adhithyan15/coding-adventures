...
- `generate_keypair(seed)` — derive public/secret key from 32-byte seed
- `sign(message, secret_key)` — deterministic Ed25519 signing
- `verify(message, signature, public_key)` — signature verification
- Full field arithmetic over GF(2^255-19) using native Ruby integers
...
- Tested against RFC 8032 Section 7.1 test vectors
