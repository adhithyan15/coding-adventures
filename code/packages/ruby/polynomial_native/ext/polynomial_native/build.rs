// build.rs — linker flags for polynomial_native Ruby extension
//
// Ruby extension modules (.so/.bundle/.dll) are loaded by the Ruby
// interpreter at runtime. The C API symbols (rb_define_method, rb_raise,
// etc.) are provided by the interpreter process, not by a library we
// link against at compile time.
//
...
fn generate_msvc_lib(dll_name: &str, so_name: &str, out_dir: &str) -> bool {
    // All Ruby C API symbols used by ruby-bridge and polynomial_native.
