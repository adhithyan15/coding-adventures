...
exact upstream commits used for the latest completed audit. Since BR02 P1.4,
`.github/workflows/html-parser-upstream-audit.yml` fetches those commits
(html5lib-tests, and a sparse checkout of WPT's parsing resources) and fails
when a fresh audit differs from the report, so the report is checked in CI,
not only regenerated by hand.
