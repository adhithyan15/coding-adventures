...
use language_parser::create_javascript_parser;
...
pub fn parse(source: &str, version: &str) -> Result<String, JsValue> {
    let mut parser = create_javascript_parser(source, resolve_version(version)).map_err(to_js_error)?;
    let ast = parser
        .parse()
        .map_err(|e| to_js_error(format!("JavaScript parse failed: {e}")))?;
