...
pub fn from_pipeline_with_options(
    ...
    let project = if options.emit_project {
        Some(build_webcomp_project_files(&component.component_name))
    } else {
        None
    };
...
fn build_webcomp_project_files(component_name: &str) -> ProjectFiles {
...
fn build_index_html(component_name: &str, custom_tag: &str) -> String {
    ...
    shell.push_str("  <meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\">\n");
    shell.push_str(&format!("  <title>{component_name}</title>\n"));
    shell.push_str(&format!("  <script type=\"module\" src=\"./{component_name}.js\"></script>\n"));
    shell.push_str("</head>\n");
    shell.push_str("<body>\n");
...
mod tests {
    ...
    fn ui32_every_emitted_side_file_carries_auto_generated_banner() {
        ...
        assert!(
            proj.index_html.contains(
                "<!-- AUTO-GENERATED by mosaic-compile --emit-project"
            ),
    ...
    fn ui32_custom_element_tag_in_shell_matches_emitter_registration() {
        ...
        assert!(
            r.output.contains("customElements.define(\"mos-hello\", MosHello);"),
        ...
        assert!(
            r2.output.contains("customElements.define(\"mos-profile-card\", MosProfileCard);"),
    ...
    fn ui32_project_files_struct_exposes_only_spec_22_webcomp_files() {
        ...
        let proj = from_pipeline_with_options(&m, &l, &s, &opts)
            ...
            .project
            .unwrap();
        let ProjectFiles { index_html, readme } = proj;
