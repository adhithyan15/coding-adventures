...
func (cpu *ARM1) ReadRegister(index int) uint32 {
	return cpu.regs[cpu.physicalReg(index)]
}
...
func (cpu *ARM1) Mode() int {
	return int(cpu.regs[15] & ModeMask)
}
...
func (cpu *ARM1) Step() Trace {
	...
	for i := 0; i < 16; i++ {
		regsBefore[i] = cpu.ReadRegister(i)
	}
	flagsBefore := cpu.Flags()

	// Fetch: read 32-bit instruction at PC
	...
	for i := 0; i < 16; i++ {
		trace.RegsAfter[i] = cpu.ReadRegister(i)
	}
...
func EncodeDataProcessing(cond, opcode, s, rn, rd int, operand2 uint32) uint32 {
	return uint32(cond)<<28 | operand2 |
...
func EncodeMovImm(cond, rd int, imm8 uint32) uint32 {
	return EncodeDataProcessing(cond, OpMOV, 0, 0, rd, (1<<25)|imm8)
}
...
func EncodeALUReg(cond, opcode, s, rd, rn, rm int) uint32 {
	return EncodeDataProcessing(cond, opcode, s, rn, rd, uint32(rm))
}
...
func EncodeBranch(cond int, link bool, offset int32) uint32 {
	...
	// Offset is in bytes, relative to PC+8. We encode (offset/4) in 24 bits.
...
func EncodeHalt() uint32 {
	return uint32(CondAL)<<28 | 0x0F000000 | HaltSWI
}
...
func EncodeLDR(cond, rd, rn int, offset int, preIndex bool) uint32 {
	inst := uint32(cond)<<28 | 0x04100000 // bits 27:26=01, L=1, I=0 (immediate)
	inst |= uint32(rd) << 12
	inst |= uint32(rn) << 16
	if preIndex {
		inst |= 1 << 24 // P bit
...
func EncodeSTR(cond, rd, rn int, offset int, preIndex bool) uint32 {
	inst := uint32(cond)<<28 | 0x04000000 // bits 27:26=01, L=0, I=0 (immediate)
	inst |= uint32(rd) << 12
	inst |= uint32(rn) << 16
	if preIndex {
		inst |= 1 << 24 // P bit
...
func EncodeLDM(cond, rn int, regList uint16, writeBack bool, mode string) uint32 {
	inst := uint32(cond)<<28 | 0x08100000 // bits 27:25=100, L=1
	inst |= uint32(rn) << 16
	inst |= uint32(regList)
	if writeBack {
		inst |= 1 << 21
...
func EncodeSTM(cond, rn int, regList uint16, writeBack bool, mode string) uint32 {
	...
	inst &= ^uint32(1 << 20) // Clear L bit (store, not load)
	return inst
}
