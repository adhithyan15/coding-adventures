...
describe("the chapter-owned real book-generation ledger", () => {
  ...
  it("has the measured stable owner counts", () => {
    ...
    // RATCHET, not a pin: may fall freely, may never grow. Five branches have
    // now collided on this one line, and twice two sides held the SAME stale
    // literal so git merged it clean while the tree had moved. Do not tighten it
    // when your PR retires a chapter.
    expect(
