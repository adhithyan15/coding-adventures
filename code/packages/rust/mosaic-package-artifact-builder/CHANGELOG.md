...
A companion test pins the other half: that the two backends *claiming* to
install really do have an emitter. A backend that claims wrongly would let a
dead handler through for the precise reason the refusal exists.

Mutation-tested: removing the refusal fails the refusal test, and falsely
marking Compose as installing fails both.
