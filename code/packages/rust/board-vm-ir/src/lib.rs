...
impl CapabilitySet {
    ...
    pub const fn with_can(self) -> Self {
        Self { can: true, ..self }
    }
...
fn stack_effect(op: Op) -> (i16, i16) {
    match op {
        ...
        Op::CallU8(CAP_NETWORK_DNS_SET_SERVER_U8) | Op::CallU16(CAP_NETWORK_DNS_SET_SERVER) => {
            (2, 0)
        }
...
mod tests {
    ...
    #[test]
    fn validates_can_open_capability() {
        let module = Module {
            flags: 0,
            max_stack: 2,
            code: &[0x12, 0, 0x40, CAP_CAN_OPEN as u8, 0x50],
            const_pool: &[],
        };

        validate(&module, CapabilitySet::blink_mvp().with_can(), 2).unwrap();
        let mut capabilities = [0u16; 1];
        let count = collect_required_capabilities(&module, &mut capabilities).unwrap();
    ...
    #[test]
    fn validates_can_write_capability() {
        let module = Module {
            flags: FLAG_PROGRAM_REQUESTS_PERSISTENT_HANDLES,
            max_stack: 3,
            code: &[0x20, 0x12, 0xa5, 0x40, CAP_CAN_WRITE as u8],
            const_pool: &[],
        };

        validate(&module, CapabilitySet::blink_mvp().with_can(), 3).unwrap();
        let mut capabilities = [0u16; 1];
        let count = collect_required_capabilities(&module, &mut capabilities).unwrap();
    ...
    #[test]
    fn validates_can_read_capability() {
        let module = Module {
            flags: FLAG_PROGRAM_REQUESTS_PERSISTENT_HANDLES,
            max_stack: 2,
            code: &[0x20, 0x40, CAP_CAN_READ as u8, 0x50],
            const_pool: &[],
        };
    ...
    #[test]
    fn validates_network_dns_set_server_capability() {
        let module = Module {
            flags: 0,
            max_stack: 2,
            code: &[
    ...
    #[test]
    fn validates_network_dns_query_capability() {
        let module = Module {
            flags: 0,
            max_stack: 2,
            code: &[
    ...
    fn validates_network_tcp_connected_capability() {
        let module = Module {
            flags: 0,
            max_stack: 1,
            code: &[0x12, 0x00, 0x40, CAP_NETWORK_TCP_CONNECTED as u8, 0x50],
            const_pool: &[],
        };
    ...
    #[test]
    fn validates_network_dns_udp_exchange_retry_capability() {
        let module = Module {
            flags: 0,
            max_stack: 7,
            code: &[
    ...
    #[test]
    fn rejects_can_open_without_capability() {
        let module = Module {
            flags: 0,
            max_stack: 2,
            code: &[0x12, 0, 0x40, CAP_CAN_OPEN as u8, 0x50],
        ...
        assert_eq!(
            validate(&module, CapabilitySet::blink_mvp(), 2),
            Err(ValidateError::UnsupportedCapability(CAP_CAN_OPEN))
        );
    }

    #[test]
    fn rejects_can_write_without_capability() {
        let module = Module {
            flags: FLAG_PROGRAM_REQUESTS_PERSISTENT_HANDLES,
            max_stack: 3,
            code: &[0x20, 0x12, 0xa5, 0x40, CAP_CAN_WRITE as u8],
        ...
        assert_eq!(
            validate(&module, CapabilitySet::blink_mvp(), 3),
            Err(ValidateError::UnsupportedCapability(CAP_CAN_WRITE))
        );
    }

    #[test]
    fn rejects_can_read_without_capability() {
        let module = Module {
            flags: FLAG_PROGRAM_REQUESTS_PERSISTENT_HANDLES,
            max_stack: 2,
            code: &[0x20, 0x40, CAP_CAN_READ as u8, 0x50],
    ...
    #[test]
    fn rejects_network_dns_set_server_without_capability() {
        let module = Module {
            flags: 0,
            max_stack: 2,
            code: &[
    ...
    #[test]
    fn rejects_network_dns_query_without_capability() {
        let module = Module {
            flags: 0,
            max_stack: 2,
            code: &[
    ...
    #[test]
    fn rejects_network_tcp_connected_without_capability() {
        let module = Module {
            flags: 0,
            max_stack: 1,
            code: &[0x12, 0x00, 0x40, CAP_NETWORK_TCP_CONNECTED as u8, 0x50],
    ...
    #[test]
    fn rejects_network_dns_udp_exchange_retry_without_both_capabilities() {
        let module = Module {
            flags: 0,
            max_stack: 7,
            code: &[
        ...
        assert_eq!(
            validate(&module, CapabilitySet::blink_mvp(), 7),
            Err(ValidateError::UnsupportedCapability(
                CAP_NETWORK_DNS_EXCHANGE_UDP_RETRY
            ))
        );
        assert_eq!(
            validate(&module, CapabilitySet::blink_mvp().with_network_dns(), 7),
            Err(ValidateError::UnsupportedCapability(
                CAP_NETWORK_DNS_EXCHANGE_UDP_RETRY
            ))
        );
