// argon2i.hpp — Argon2i, data-independent memory-hard password hashing (RFC
// 9106), in pure ISO C++17 (header-only), in namespace ca. A faithful port of
// the Rust `argon2i` crate.
// ===========================================================================
//
// Argon2 fills a large memory matrix (memory_cost KiB) with a BLAKE2b-derived
// compression function. The *i* variant picks each reference block from a
// deterministic pseudo-random stream that does NOT depend on the password or
// memory contents — a constant memory-access pattern that defeats side-channel
// observers, at the cost of being the easiest variant to parallelise. Prefer
// Argon2id for password hashing.
//
// The addressing stream (RFC 9106 §3.4.2) yields (J1, J2) pairs by running the
// compression function twice over a counter block, so the reference index never
...
#ifndef CA_ARGON2I_HPP
#define CA_ARGON2I_HPP
...
constexpr std::uint32_t type_i = 1;
...
inline std::vector<std::uint8_t> blake2b_long(std::uint32_t t,
    ...
    if (t == 0) {
        throw std::invalid_argument("argon2i: H' output length must be positive");
    }
...
// Optional inputs for argon2i (key / associated data / version override).
struct Argon2iOptions {
    std::vector<std::uint8_t> key;
    std::vector<std::uint8_t> associated_data;
    std::optional<std::uint32_t> version;
};

constexpr std::uint32_t argon2i_version = 0x13;

// argon2i — compute the Argon2i tag (RFC 9106). Throws std::invalid_argument on
// any invalid parameter.
inline std::vector<std::uint8_t> argon2i(const std::vector<std::uint8_t>& password,
                                         const std::vector<std::uint8_t>& salt,
                                         std::uint32_t time_cost,
                                         std::uint32_t memory_cost,
                                         std::uint32_t parallelism,
                                         std::uint32_t tag_length,
                                         const Argon2iOptions& opts = {}) {
    namespace d = argon2i_detail;
    const std::vector<std::uint8_t>& key = opts.key;
    const std::vector<std::uint8_t>& ad = opts.associated_data;
    std::uint32_t version = opts.version.value_or(argon2i_version);

    if (static_cast<std::uint64_t>(password.size()) > 0xFFFFFFFFull) {
        throw std::invalid_argument("argon2i: password length must fit in 32 bits");
    }
    if (salt.size() < 8) {
        throw std::invalid_argument("argon2i: salt must be at least 8 bytes");
    }
    if (static_cast<std::uint64_t>(salt.size()) > 0xFFFFFFFFull) {
        throw std::invalid_argument("argon2i: salt length must fit in 32 bits");
    }
    if (static_cast<std::uint64_t>(key.size()) > 0xFFFFFFFFull) {
        throw std::invalid_argument("argon2i: key length must fit in 32 bits");
    }
    if (static_cast<std::uint64_t>(ad.size()) > 0xFFFFFFFFull) {
        throw std::invalid_argument("argon2i: associated data must fit in 32 bits");
    }
    if (tag_length < 4) {
        throw std::invalid_argument("argon2i: tag_length must be >= 4");
    }
    if (parallelism < 1 || parallelism > 0xFFFFFF) {
        throw std::invalid_argument("argon2i: parallelism must be in [1, 2^24-1]");
    }
    if (memory_cost < 8 * parallelism) {
        throw std::invalid_argument("argon2i: memory_cost must be >= 8*parallelism");
    }
    if (time_cost < 1) {
        throw std::invalid_argument("argon2i: time_cost must be >= 1");
    }
    if (version != argon2i_version) {
        throw std::invalid_argument("argon2i: only Argon2 v1.3 (0x13) is supported");
    }
...
// argon2i_hex — like argon2i but returns a lowercase hex string.
inline std::string argon2i_hex(const std::vector<std::uint8_t>& password,
                               const std::vector<std::uint8_t>& salt,
                               std::uint32_t time_cost, std::uint32_t memory_cost,
                               std::uint32_t parallelism, std::uint32_t tag_length,
                               const Argon2iOptions& opts = {}) {
    std::vector<std::uint8_t> tag = argon2i(password, salt, time_cost, memory_cost,
