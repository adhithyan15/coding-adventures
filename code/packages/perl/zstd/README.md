...
28 subtests covering empty input, single byte, all 256 byte values,
...
the `zstd` CLI (TC-9 and a high-sequence-count regression test — skipped,
not failed, when `zstd` isn't on `PATH`).
