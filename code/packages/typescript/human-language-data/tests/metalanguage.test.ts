...
describe("the committed corpus", () => {
  it("pins the first measurement", () => {
    ...
    // The ORDER is pinned exactly; the counts are floors. A `toEqual` on the pair
    // made both numbers exact pins on a figure that only grows, so every content
    // tranche had to edit them and only the first to merge could be right.
    expect(r.summary.worstTerms.slice(0, 2).map((t) => t.term)).toEqual(["verb", "noun"]);
    expect(r.summary.worstTerms[0]!.lessons).toBeGreaterThanOrEqual(1014); // FLOOR — content only grows; exact pins serialize parallel tranches
    expect(r.summary.worstTerms[1]!.lessons).toBeGreaterThanOrEqual(531); // FLOOR — content only grows; exact pins serialize parallel tranches
    // Superseded exact pair, kept for the record:
    //   { term: "verb", lessons: 1014 }, // Marathi's explicit review sessions retrieve its verb families at distance.
    //   { term: "noun", lessons: 531 },  // Marathi's family review explicitly contrasts the noun histories.
    /* Historical cumulative context:
      { term: "verb", lessons: 1010 }, // HL-C247: +4 -- a chapter about three verbs necessarily says "verb"; french modal chapter (ch33) // hindi pre-A1 round 2: +35 lessons, +7 chapters (chapters 52-58) // +4: HL-C98 // +2: HL-C99 // +8: vocabulary wave 5 // +3: HL-C88 slices 5-6 // +11: vocabulary wave 6 // +2: B1 si-condition rung // +3: HL-C113 preterite plural // +2: HL-C113 preterite close (strong plurals, review, synthesis) // +2: HL-C113 imperfect subjunctive // +3: HL-C113 unreal condition // HL-C113 step 7: +4 // HL-C128 step 3: +1 // HL-C128 step 5: +2 // HL-C127: +3 // HL-C128 step 7: +2 // HL-C128 step 8: +2 // HL-C128 step 9: +4 // HL-C128 step 10: +3 // HL-C134: +1 each — the carried prose says 'verb' and 'noun' in lessons whose markdown had not, though the book always did // HL-C152: Spanish realizes SPINE-NEGATE-AND-ASK — five lessons, one chapter, A2 complete at 5/5 // kannada pre-A1 tranche: +35 lessons, +7 chapters (chapters 46-52) // hindi pre-A1 tranche: +35 lessons, +7 chapters (chapters 45-51) // latin pre-A1 tranche: +20 lessons, +4 chapters (chapters 44-47) // french questions chapter: +9 lessons, +1 chapter (ch32) -- the interrogation category, 0/5 -> 5/5
      { term: "noun", lessons: 528 }, // hindi pre-A1 round 2: +35 lessons, +7 chapters (chapters 52-58) // +1: ES-C02-concordancia names the noun it agrees with // +4: vocabulary wave 5 // +3: HL-C88 slices 5-6 // +1: HL-C88 slice 7 (ES-C09-ncia) // +1: HL-C88 slice 8 (-ario, review, synthesis) // +14: vocabulary wave 6 // HL-C113 step 6: +1 // HL-C128 step 2: +4 // HL-C128 step 3: +4 // HL-C128 step 8: +1 // HL-C128 step 9: +3 // HL-C128 step 10: +4 // HL-C134: +1 each — the carried prose says 'verb' and 'noun' in lessons whose markdown had not, though the book always did // kannada pre-A1 tranche: +35 lessons, +7 chapters (chapters 46-52) // latin pre-A1 tranche: +20 lessons, +4 chapters (chapters 44-47) // french questions chapter: +9 lessons, +1 chapter (ch32) -- the interrogation category, 0/5 -> 5/5
      */
  });
