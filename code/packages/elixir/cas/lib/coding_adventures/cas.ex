defmodule CodingAdventures.Cas do
  @moduledoc """
  ...
  | `CodingAdventures.Cas.Store`              | CAS logic: hashing, integrity, prefix search  |
  | `CodingAdventures.Cas.LocalDiskStore`     | Filesystem backend with Git 2/38 layout       |
  | `CodingAdventures.Cas.Hex`               | Hex encoding/decoding utilities               |
  | `CodingAdventures.Cas.Error`              | Typed error reason catalogue                  |

  ## Quick Start

      backend = CodingAdventures.Cas.LocalDiskStore.new!("/tmp/objects")
      store   = CodingAdventures.Cas.Store.new(backend)

      {:ok, key} = CodingAdventures.Cas.Store.put(store, "hello, world")
      {:ok, data} = CodingAdventures.Cas.Store.get(store, key)
      # data == "hello, world"

      # Abbreviated lookup (like `git show a3f4b2`)
      short = String.slice(CodingAdventures.Cas.Hex.key_to_hex(key), 0, 8)
      {:ok, full_key} = CodingAdventures.Cas.Store.find_by_prefix(store, short)
      # full_key == key
