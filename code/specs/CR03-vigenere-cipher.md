...
Every established implementation lane also consumes the complete Vigenere
subset through dependency-free native tests generated by
`code/scripts/generate_vigenere_fixture_consumers.py`. Run it with `--check`
to prove the committed consumers match the fixture digest, case roster,
complete expected values, normalized errors, and language denominator.
