# ladder_t7_getter_setter_simple

Rung 7.getter_setter of the CCR-066 differential complexity ladder, at `SIMPLE`.

## What this rung is for

Exercises an object literal accessor pair.
...
java -Duser.language=en -Duser.country=US -Duser.timezone=UTC -Dfile.encoding=UTF-8 \
  ...
  --compilation_level SIMPLE_OPTIMIZATIONS --js tests/diff/ladder_t7_getter_setter_simple/input/a.js
```
...
closurec refuses valid JavaScript that upstream compiles: an object-literal accessor pair fails at the parse stage. This is a front-end capability gap, not an optimizer one. Note it fails only on the typed path — WHITESPACE_ONLY passes, because that path never parses.
