use arm1_gatelevel::{ARM1GateLevel, FLIP_FLOP_COUNT};
use arm1_simulator::{
    encode_halt, encode_mov_imm, Arm1Error, COND_AL, FLAG_F, FLAG_I, MEMORY_SIZE, MODE_FIQ,
    MODE_IRQ, MODE_USR,
};
...
#[test]
fn exact_topology_and_architectural_constructor_cover_every_persistent_bit() {
    let cpu = ARM1GateLevel::architectural();
    assert_eq!(cpu.memory().len(), MEMORY_SIZE);
    assert_eq!(cpu.get_state().regs.len(), 27);
    assert_eq!(FLIP_FLOP_COUNT, MEMORY_SIZE * 8 + 27 * 32 + 1);
}
...
#[test]
fn checked_register_and_memory_accesses_are_typed_and_clocked() {
    let mut cpu = ARM1GateLevel::new(64);
    assert_eq!(
    ...
    cpu.write_word_checked(60, 0x1020_3040).unwrap();
    assert_eq!(cpu.read_word_checked(60).unwrap(), 0x1020_3040);
    assert!(matches!(
...
#[test]
fn deterministic_load_clears_stale_memory_and_tracks_exact_fetch_range() {
    let mut cpu = ARM1GateLevel::new(1024);
...
fn checked_step_rejects_halt_truncation_and_data_bounds_atomically() {
    ...
    assert_eq!(cpu.step().mnemonic, "HALTED");
    assert_eq!(cpu.get_state(), halted);
}

#[test]
fn checked_run_returns_complete_state_and_rolls_back_late_failures() {
    ...
    let mut failing = ARM1GateLevel::new(64);
    let ldr = (COND_AL << 28) | 0x0591_0000;
    failing
        .load_words_checked(&[encode_mov_imm(COND_AL, 0, 7), ldr], 0)
        .unwrap();
    failing.write_register(1, 0x1000);
    let before = failing.get_state();
    assert!(matches!(
        failing.run_loaded_checked(10),
        Err(Arm1Error::MemoryOutOfRange { .. })
    ));
    assert_eq!(failing.get_state(), before);
}

#[test]
fn irq_and_fiq_entry_clock_banked_links_and_respect_masks() {
    let mut cpu = ARM1GateLevel::new(1024);
...
#[test]
fn block_transfer_force_user_targets_unbanked_registers() {
    let mut cpu = ARM1GateLevel::new(1024);
