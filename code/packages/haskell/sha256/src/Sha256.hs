...
sha256Update (Sha256Context state totalBytes buffered) chunk =
    case checkedAdvanceBytes totalBytes (fromIntegral (BS.length chunk)) of
        ...
        Just nextTotalBytes ->
            let (nextState, ownedRemainder) = updateBytes state buffered chunk
             in Sha256Context nextState nextTotalBytes ownedRemainder

updateBytes :: HashState -> ByteString -> ByteString -> (HashState, ByteString)
updateBytes state buffered chunk
    | BS.null buffered = compressChunkRemainder state chunk 0 (BS.length chunk)
    | BS.length chunk < bridgeBytes = (state, BS.append buffered chunk)
    | otherwise =
        let bridge = BS.append buffered (BS.take bridgeBytes chunk)
            bridgedState = compressBlocks state bridge
            remainingLength = BS.length chunk - bridgeBytes
         in bridgedState
                `seq` compressChunkRemainder bridgedState chunk bridgeBytes remainingLength
  where
    bridgeBytes = 64 - BS.length buffered

compressChunkRemainder :: HashState -> ByteString -> Int -> Int -> (HashState, ByteString)
compressChunkRemainder state bytes start byteCount =
    (nextState, ownedRemainder)
  where
    completeLength = byteCount - (byteCount `mod` 64)
    remainderStart = start + completeLength
    remainderLength = byteCount - completeLength
    nextState = compressRegion state bytes start completeLength
    ownedRemainder
        | remainderLength == 0 = BS.empty
        | otherwise = BS.copy (BS.take remainderLength (BS.drop remainderStart bytes))
...
compressBlocks state bytes = compressRegion state bytes 0 (BS.length bytes)

compressRegion :: HashState -> ByteString -> Int -> Int -> HashState
compressRegion state bytes start byteCount
    | byteCount == 0 = state
    | byteCount `mod` 64 /= 0 = error "compressRegion: byte count must contain complete blocks"
    | otherwise = runST $ do
        schedule <- newSchedule
        compressRegionWith schedule state bytes start (start + byteCount)

newSchedule :: ST s (STUArray s Int Word32)
newSchedule = newArray (0, 15) 0

compressRegionWith :: STUArray s Int Word32 -> HashState -> ByteString -> Int -> Int -> ST s HashState
compressRegionWith schedule state bytes offset end
    | offset == end = pure state
    | otherwise = do
        nextState <- compressBlock schedule state bytes offset
        nextState `seq` compressRegionWith schedule nextState bytes (offset + 64) end

compressBlock :: STUArray s Int Word32 -> HashState -> ByteString -> Int -> ST s HashState
compressBlock schedule (HashState a0 b0 c0 d0 e0 f0 g0 h0) bytes offset = do
    loadInitialWords schedule bytes offset 0
    WorkingState aFinal bFinal cFinal dFinal eFinal fFinal gFinal hFinal <-
        runRounds schedule 0 (WorkingState a0 b0 c0 d0 e0 f0 g0 h0)
    pure $
        HashState
            (a0 + aFinal)
            (b0 + bFinal)
...
loadInitialWords :: STUArray s Int Word32 -> ByteString -> Int -> Int -> ST s ()
loadInitialWords schedule bytes offset wordIndex
    | wordIndex == 16 = pure ()
    | otherwise = do
        writeArray schedule wordIndex (word32At bytes (offset + wordIndex * 4))
        loadInitialWords schedule bytes offset (wordIndex + 1)

runRounds :: STUArray s Int Word32 -> Int -> WorkingState -> ST s WorkingState
runRounds schedule roundIndex workingState
    | roundIndex == 64 = pure workingState
    | otherwise = do
        wordValue <- scheduleWord schedule roundIndex
        let nextState = roundStep workingState (roundConstants ! roundIndex, wordValue)
        nextState `seq` runRounds schedule (roundIndex + 1) nextState

scheduleWord :: STUArray s Int Word32 -> Int -> ST s Word32
scheduleWord schedule roundIndex
    | roundIndex < 16 = readArray schedule roundIndex
    | otherwise = do
        wordMinus2 <- readArray schedule ((roundIndex - 2) .&. 15)
        wordMinus7 <- readArray schedule ((roundIndex - 7) .&. 15)
        wordMinus15 <- readArray schedule ((roundIndex - 15) .&. 15)
        wordMinus16 <- readArray schedule (roundIndex .&. 15)
        let nextValue =
                smallSigma1 wordMinus2
                    + wordMinus7
                    + smallSigma0 wordMinus15
                    + wordMinus16
        writeArray schedule (roundIndex .&. 15) nextValue
        pure nextValue
