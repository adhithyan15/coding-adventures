...
| Backend | Status |
|---|---|
| XAML | **implemented** — `circle`/`line`/`curve` lower to real `<Ellipse>`/`<Line>`/`<Path><PathGeometry>...` geometry (`mosaic-emit-xaml::emit_path`). `line` accepts literal numbers, slot bindings, and numeric expressions, including `segment[index]` inside a typed `For` template; numeric indexers become generated `double` row properties so WinUI's typed `x:Bind` compiler can resolve them. `circle` and `curve` remain literal-only: their XAML lowering respectively needs derived margins and `Point` construction, neither of which can be expressed as independent property bindings without a broader native geometry contract. `arc` did not make it into this PR — it hard-errors with a named "not yet supported" message rather than silently rendering nothing; tracked as a follow-up. Verified against the real toolchain: a `dotnet build` probe confirmed the exact syntax before implementation, and a real `mosaic-compile pkg --backend xaml --emit-project` build + launch of the crescent-moon shape (two overlapping `Path` circles) succeeds cleanly. |
| Qt | **implemented** — `circle` lowers to a `Rectangle` with `radius: width / 2` (reuses the existing `background`/`border-color`/`border-width` → `color`/`border.color`/`border.width` mapping directly, no remap needed); `line`/`curve` lower to `QtQuick.Shapes`' `Shape`/`ShapePath`/`PathLine`/`PathQuad`, behind a conditional `import QtQuick.Shapes` that only fires when a `line`/`curve`/`arc` kind is actually present. `arc` is the same named stretch-goal gap as XAML. Fixed a real architectural mismatch along the way: `Stack`'s QML lowering unconditionally injects `anchors.fill: parent` onto every child, which would stretch a `Path`'s explicit geometry-based positioning to fill the whole container — `Path` children now get a narrow, documented exception. Verified against the real toolchain: `qmllint` and a real `qmlscene` launch confirmed the exact syntax before implementation; a real `mosaic-compile pkg --backend qt` build of the crescent-moon shape (and a second build exercising all three implemented kinds together) produces `nativeComplete: true`, passes `qmllint` with zero errors, and launches cleanly via `qmlscene`. |
| Flutter | **implemented** — `circle` lowers to a `Container(decoration: BoxDecoration(shape: BoxShape.circle, ...))` (reuses `background`/`border-color`/`border-width` → `color`/`border` directly, no remap needed, the same reuse Qt's `Rectangle` lowering made); `line`/`curve` lower to `CustomPaint`, backed by two small painter classes (`_MosaicLinePainter`, `_MosaicCurvePainter`) emitted once per file when `Path` is present anywhere in the tree, using absolute canvas coordinates (mirrors Qt's `ShapePath`). `arc` is the same named stretch-goal gap as XAML/Qt. Only `circle` needs explicit `Positioned(left:, top:, ...)` placement, and only when it's an actual direct `Stack` child (`Positioned` panics outside a `Stack`, unlike WinUI's `Margin` or QML's implicit `x`/`y`); `line`/`curve` never need this since Flutter `Stack`'s default un-positioned-child placement already lands them at `(0, 0)`, matching their own absolute drawn coordinates. Verified against the real toolchain: a `mosaic-compile pkg --backend flutter --profile native-complete` build of the crescent-moon shape (two overlapping `Path` circles, a line, and a curve) produces `nativeComplete: true` with zero degradations, and the generated component passes `dart analyze` (via a minimal one-time `pubspec.yaml` scaffold) with zero issues. |
| Compose | **implemented** — `circle` lowers to `Modifier.background(color, CircleShape)` + `Modifier.border(width, color, CircleShape)` (reuses the already-unconditionally-imported `.background`/`.border` modifiers directly, no remap needed, the same reuse Qt's `Rectangle` and Flutter's `BoxDecoration` lowerings made); `line`/`curve` lower to a `Canvas` drawing a Compose `graphics.Path` via `moveTo`/`lineTo`/`quadraticBezierTo`, using absolute canvas coordinates (mirrors Qt's `ShapePath` and Flutter's `CustomPaint`). `arc` is the same named stretch-goal gap as the other backends. Positioning is notably simpler than Flutter's: Compose's `Modifier.offset(x, y)` is legal on any composable regardless of parent type (unlike Flutter's `Positioned`, which only type-checks as a direct `Stack` child), so `circle` always emits `Modifier.offset(...)` unconditionally — no parent-type detection needed. Verified against the real toolchain: a `mosaic-compile pkg --backend compose --profile native-complete` build of the crescent-moon shape (two overlapping circles, a line, and a curve) produces `nativeComplete: true` with zero degradations; the generated Kotlin compiled cleanly via `gradle compileKotlin` in a real JetBrains Compose Multiplatform Desktop project and launched via `gradle run` with no crash. |
