...
use riscv_simulator::{Rv32IError, Rv32ISimulator, RV32I_MEMORY_SIZE};
...
fn restore_validates_every_invariant_before_commit() {
    ...
    valid.csr_mscratch = 0x1234;
    cpu.restore(&valid).unwrap();
    assert_eq!(cpu.get_state(), valid);
}

#[test]
fn register_and_memory_access_are_checked() {
    let mut cpu = Rv32ISimulator::new();
    assert_eq!(
...
#[test]
fn fetch_decode_and_data_faults_are_step_atomic() {
    let cases = [
        (
...
#[test]
fn trap_csrs_and_mret_follow_the_existing_architectural_contract() {
    let mut cpu = Rv32ISimulator::new();
    ...
    let trap = cpu.step_checked().unwrap();
    assert_eq!(trap.pc_after, 0x100);
    ...
    let returned = cpu.step_checked().unwrap();
    assert_eq!(returned.pc_after, 0);
