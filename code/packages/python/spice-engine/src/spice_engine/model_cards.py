...
@dataclass(frozen=True, slots=True)
class ModelCardSupportedParameterCoverageDashboardRow:
    """Per-kind supported-parameter coverage status for dashboards."""
    ...
    expected_canonical_parameter_count: int
    accepted_name_count: int
    expected_accepted_name_count: int
    aliased_parameter_count: int
    expected_aliased_parameter_count: int
    max_alias_count: int
    expected_max_alias_count: int
    issue_count: int
...
def model_card_supported_parameter_coverage_dashboard(
    coverage: Sequence[ModelCardSupportedParameterCoverage] | None = None,
) -> tuple[ModelCardSupportedParameterCoverageDashboardRow, ...]:
    """Return per-kind supported-parameter coverage status rows."""

    rows = model_card_supported_parameter_coverage() if coverage is None else tuple(coverage)
    summaries = {
        row.kind: row for row in _model_card_supported_parameter_coverage_summary_from(rows)
    }
    report = model_card_supported_parameter_coverage_gate(rows)
    global_issue_fields: list[str] = []
    issue_fields_by_kind: dict[str, list[str]] = {
        kind: [] for kind in _MODEL_CARD_SUPPORTED_PARAMETER_KINDS
    }
    for issue in report.issues:
        target = (
            global_issue_fields
            if issue.kind == "catalog"
            else issue_fields_by_kind.get(issue.kind)
        )
        if target is not None and issue.field not in target:
            target.append(issue.field)

    dashboard: list[ModelCardSupportedParameterCoverageDashboardRow] = []
    for kind in _MODEL_CARD_SUPPORTED_PARAMETER_KINDS:
        ...
        (
            ...
            expected_aliased_count,
            expected_max_alias_count,
        ) = _MODEL_CARD_SUPPORTED_PARAMETER_COVERAGE_EXPECTED_SUMMARIES[kind]
        issue_fields = tuple(
            field
            for field in [*global_issue_fields, *issue_fields_by_kind[kind]]
        )
        dashboard.append(
            ModelCardSupportedParameterCoverageDashboardRow(
                kind=kind,
                passed=not issue_fields,
...
def format_model_card_supported_parameter_coverage_dashboard_table(
    coverage: Sequence[ModelCardSupportedParameterCoverage] | None = None,
) -> str:
    """Return supported-parameter coverage dashboard rows as a stable table."""
    ...
    lines.extend(
        f"{row.kind}\t{str(row.passed).lower()}\t"
        f"{row.canonical_parameter_count}\t"
        f"{row.expected_canonical_parameter_count}\t"
        f"{row.accepted_name_count}\t{row.expected_accepted_name_count}\t"
        f"{row.aliased_parameter_count}\t"
        f"{row.expected_aliased_parameter_count}\t{row.max_alias_count}\t"
        f"{row.expected_max_alias_count}\t{row.issue_count}\t"
        f"{'|'.join(row.issue_fields)}"
        for row in model_card_supported_parameter_coverage_dashboard(coverage)
    )
    return "\n".join(lines)


def model_card_supported_parameter_coverage_dashboard_records(
    coverage: Sequence[ModelCardSupportedParameterCoverage] | None = None,
) -> list[dict[str, str]]:
    """Return header-keyed records for supported-parameter coverage dashboard rows."""
...
def format_model_card_supported_parameter_coverage_dashboard_csv(
    coverage: Sequence[ModelCardSupportedParameterCoverage] | None = None,
) -> str:
    """Return supported-parameter coverage dashboard rows as CSV."""
...
def format_model_card_supported_parameter_coverage_dashboard_json(
    coverage: Sequence[ModelCardSupportedParameterCoverage] | None = None,
) -> str:
    """Return compact JSON records for supported-parameter dashboard rows."""
