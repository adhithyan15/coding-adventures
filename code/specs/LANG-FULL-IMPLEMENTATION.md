...
- ◑ **AL4** — literal string `print`/`output` I/O runs on all 7 backends via
  **E4**. Undeclared statement-position `print('HI')`/`output('HI')` calls lower
  ...
  domains must be present. When the base does contain a pure standard-function
  result, a pure conditional exponent may also participate if every runtime
  branch proves the same exact bounded value; its selector remains in emitted
  IIR. Uninitialized or nonlocal values, inexact integer widening,
  fractional or non-finite results, differing branches, effectful selectors,
  calls, nested powers, and conditional standard-free forms remain on
  `f64_pow`.
