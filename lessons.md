...
- **CI runs clippy with `-D warnings`; `cargo test` does NOT.** The build-tool's per-package build denies clippy warnings, so a lint that never shows up in a local `cargo test` (or on a Windows-only local run) fails the ubuntu/macos build jobs while Windows-local looks green. Always run `cargo clippy -p <pkg> --all-features --all-targets -- -D warnings` before pushing. Real case (task-core reducer): `contains_key` then `insert` on a `BTreeMap` tripped `clippy::map_entry` on ubuntu+macos, passed on Windows-local `cargo test` — fix is the `entry()` API. `symbolic-vm`'s `StrictBackend` also *panics* on division by zero, so evaluating untrusted formulas must `catch_unwind` (or the wasm boundary aborts).
