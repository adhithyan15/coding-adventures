"""Argon2id test suite — mirrors RFC 9106 §5 and parameter-edge vectors."""
...
def test_rfc_9106_vector() -> None:
    """The RFC 9106 §5.3 test vector is the gold standard."""
    tag = argon2id(
...
def test_hex_form_matches_bytes_form() -> None:
    tag = argon2id(RFC_PASSWORD, RFC_SALT, **RFC_PARAMS)
    tag_hex = argon2id_hex(RFC_PASSWORD, RFC_SALT, **RFC_PARAMS)
...
def test_single_lane_minimum_memory_deterministic() -> None:
    tag_a = argon2id(b"password", b"somesalt", 1, 8, 1, 32)
    tag_b = argon2id(b"password", b"somesalt", 1, 8, 1, 32)
    assert tag_a == tag_b
    assert len(tag_a) == 32


def test_different_passwords_differ() -> None:
    tag_a = argon2id(b"password1", b"somesalt", 1, 8, 1, 32)
    tag_b = argon2id(b"password2", b"somesalt", 1, 8, 1, 32)
    assert tag_a != tag_b


def test_different_salts_differ() -> None:
    tag_a = argon2id(b"password", b"saltsalt", 1, 8, 1, 32)
    tag_b = argon2id(b"password", b"saltsal2", 1, 8, 1, 32)
    assert tag_a != tag_b


def test_key_binds_output() -> None:
    tag_a = argon2id(b"password", b"saltsalt", 1, 8, 1, 32)
    tag_b = argon2id(b"password", b"saltsalt", 1, 8, 1, 32, key=b"secret!!")
    assert tag_a != tag_b
...
def test_tag_length_variants(T: int) -> None:
    tag = argon2id(b"password", b"saltsalt", 1, 8, 1, T)
    assert len(tag) == T
...
def test_multi_lane_parameters() -> None:
    ...
    tag = argon2id(RFC_PASSWORD, RFC_SALT, 3, 32, 4, 32)
    assert len(tag) == 32


def test_multiple_passes() -> None:
    tag_t1 = argon2id(b"password", b"saltsalt", 1, 8, 1, 32)
    tag_t2 = argon2id(b"password", b"saltsalt", 2, 8, 1, 32)
    tag_t3 = argon2id(b"password", b"saltsalt", 3, 8, 1, 32)
