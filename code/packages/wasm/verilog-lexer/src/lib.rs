...
use language_lexer::{create_verilog_lexer, tokenize_verilog_with_version};
...
pub fn tokenize(source: &str) -> Result<String, JsValue> {
    ...
    let tokens = lexer
        .tokenize()
        .map_err(|e| to_js_error(format!("Verilog tokenization failed: {e}")))?;
...
pub fn tokenize_with_version(source: &str, version: &str) -> Result<String, JsValue> {
    let tokens = tokenize_verilog_with_version(source, version)
        .map_err(to_js_error)?;
    tokens_to_json_string(tokens).map_err(|e| JsValue::from_str(&e.to_string()))
}
...
mod tests {
    ...
    fn wrapper_tokenizes_to_json() {
        let json = tokenize("module top; endmodule").unwrap();
