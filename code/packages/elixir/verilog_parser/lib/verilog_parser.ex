defmodule CodingAdventures.VerilogParser do
  @moduledoc """
  Verilog Parser — Thin wrapper around the grammar-driven parser engine.

  This module combines `VerilogLexer.tokenize/1` with `GrammarParser.parse/2`
  to parse Verilog source code into an AST. It reads `verilog.grammar` from the
  shared grammars directory.

  ## Usage

      {:ok, ast} = CodingAdventures.VerilogParser.parse(~s({"key": [1, 2, 3]}))
  ...
  def parse(source) do
    ...
    case VerilogLexer.tokenize(source) do
  ...
  @doc """
  Parse the verilog.grammar file and return the ParserGrammar.
  """
  @spec create_parser() :: ParserGrammar.t()
  def create_parser do
    grammar_path = Path.join(@grammars_dir, "verilog.grammar")
