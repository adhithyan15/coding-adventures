defmodule CodingAdventures.CommonmarkParser.BlockParser do
  @moduledoc """
  Block-Level Parser — Phase 1 of GFM parsing.
  ...
  @doc """
  Parse a GFM document into a block-level tree (Phase 1).
  ...
  @spec apply_gfm_block_extensions(map(), map()) :: {map(), map()}
  def apply_gfm_block_extensions(document, raw_inline) do
    next_id =
      case Map.keys(raw_inline) do
        [] -> 0
        keys -> Enum.max(keys) + 1
      end

    {document, raw_inline, _next_id} = transform_block(document, raw_inline, next_id)
    {document, raw_inline}
  end

  defp transform_block(%{type: :document, children: children} = node, raw_inline, next_id) do
    {children, raw_inline, next_id} = transform_blocks(children, raw_inline, next_id)
    {%{node | children: children}, raw_inline, next_id}
  end

  defp transform_block(%{type: :blockquote, children: children} = node, raw_inline, next_id) do
    {children, raw_inline, next_id} = transform_blocks(children, raw_inline, next_id)
    {%{node | children: children}, raw_inline, next_id}
  end

  defp transform_block(%{type: :list, children: children} = node, raw_inline, next_id) do
    {children, raw_inline, next_id} = transform_list_children(children, raw_inline, next_id)
    {%{node | children: children}, raw_inline, next_id}
  end

  defp transform_block(%{type: type} = node, raw_inline, next_id) when type in [:list_item, :task_item] do
    transform_list_child(node, raw_inline, next_id)
  end

  defp transform_block(%{type: :paragraph} = node, raw_inline, next_id) do
    case maybe_transform_paragraph_to_table(node, raw_inline, next_id) do
      {nil, raw_inline, next_id} -> {node, raw_inline, next_id}
      result -> result
    end
  end

  defp transform_block(node, raw_inline, next_id), do: {node, raw_inline, next_id}

  defp transform_blocks(children, raw_inline, next_id) do
    Enum.reduce(children, {[], raw_inline, next_id}, fn child, {acc, raw, ctr} ->
      {child, raw, ctr} = transform_block(child, raw, ctr)
      {acc ++ [child], raw, ctr}
    end)
  end

  defp transform_list_children(children, raw_inline, next_id) do
    Enum.reduce(children, {[], raw_inline, next_id}, fn child, {acc, raw, ctr} ->
      {child, raw, ctr} = transform_list_child(child, raw, ctr)
      {acc ++ [child], raw, ctr}
    end)
  end

  defp transform_list_child(%{type: type, children: children} = node, raw_inline, next_id)
       when type in [:list_item, :task_item] do
    {children, raw_inline, next_id} = transform_blocks(children, raw_inline, next_id)
    next_node =
      case type do
        :task_item -> %{type: :task_item, checked: node.checked, children: children}
        :list_item -> %{type: :list_item, children: children}
      end

    maybe_convert_task_item(next_node, raw_inline, next_id)
  end

  defp maybe_convert_task_item(%{type: type, children: [first | _]} = node, raw_inline, next_id)
       when type in [:list_item, :task_item] do
    cond do
      first.type != :paragraph || first[:_raw_id] == nil ->
        {node, raw_inline, next_id}

      true ->
        raw = Map.get(raw_inline, first._raw_id, "")

        case Regex.run(~r/^\[( |x|X)\](?:[ \t]+|$)/, raw) do
          [match, marker] ->
            raw_inline = Map.put(raw_inline, first._raw_id, binary_part(raw, byte_size(match), byte_size(raw) - byte_size(match)))
            {%{type: :task_item, checked: String.downcase(marker) == "x", children: node.children}, raw_inline, next_id}
  ...
  defp maybe_convert_task_item(node, raw_inline, next_id), do: {node, raw_inline, next_id}

  defp maybe_transform_paragraph_to_table(%{type: :paragraph, _raw_id: raw_id}, raw_inline, next_id) do
    raw = Map.get(raw_inline, raw_id)

    case try_parse_table(raw) do
      nil ->
        {nil, raw_inline, next_id}

      %{align: align, header: header, rows: rows} ->
        raw_inline = Map.delete(raw_inline, raw_id)
        {header_row, raw_inline, next_id} = make_table_row(header, true, raw_inline, next_id)

        {body_rows, raw_inline, next_id} =
          Enum.reduce(rows, {[], raw_inline, next_id}, fn row, {acc, raw, ctr} ->
            {row_node, raw, ctr} = make_table_row(row, false, raw, ctr)
            {acc ++ [row_node], raw, ctr}
          end)

        {%{type: :table, align: align, children: [header_row | body_rows]}, raw_inline, next_id}
    end
  end

  defp maybe_transform_paragraph_to_table(_node, raw_inline, next_id), do: {nil, raw_inline, next_id}

  defp make_table_row(cells, is_header, raw_inline, next_id) do
    {cells, raw_inline, next_id} =
      Enum.reduce(cells, {[], raw_inline, next_id}, fn cell_content, {acc, raw, ctr} ->
        node = %{type: :table_cell, children: [], _raw_id: ctr}
        {acc ++ [node], Map.put(raw, ctr, cell_content), ctr + 1}
      end)

    {%{type: :table_row, is_header: is_header, children: cells}, raw_inline, next_id}
  end

  defp try_parse_table(nil), do: nil
  defp try_parse_table(raw) do
    lines = String.split(raw, "\n")

    if length(lines) < 2 do
      nil
    else
      [header_line, delimiter_line | body_lines] = lines
      header_cells = split_table_row(header_line)
      delimiter_cells = split_table_row(delimiter_line)

      cond do
        header_cells == nil || delimiter_cells == nil ->
          nil

        header_cells == [] || length(header_cells) != length(delimiter_cells) ->
          nil

        true ->
          with {:ok, align} <- parse_table_alignments(delimiter_cells),
               {:ok, rows} <- parse_table_rows(body_lines, length(header_cells)) do
            %{align: align, header: normalize_table_row(header_cells, length(delimiter_cells)), rows: rows}
          else
            _ -> nil
          end
      end
  ...
  defp parse_table_alignments(cells) do
    Enum.reduce_while(cells, {:ok, []}, fn cell, {:ok, acc} ->
      ...
      if Regex.match?(~r/^:?-{3,}:?$/, trimmed) do
        align =
          cond do
            String.starts_with?(trimmed, ":") && String.ends_with?(trimmed, ":") -> :center
            String.starts_with?(trimmed, ":") -> :left
            String.ends_with?(trimmed, ":") -> :right
            true -> nil
          end
  ...
  defp parse_table_rows(lines, width) do
    Enum.reduce_while(lines, {:ok, []}, fn line, {:ok, acc} ->
      cond do
        ...
        true ->
          case split_table_row(line) do
            nil -> {:halt, :error}
            cells -> {:cont, {:ok, acc ++ [normalize_table_row(cells, width)]}}
          end
      end
    end)
  end
  ...
  defp split_table_row(line) do
    if String.contains?(line, "|") do
      ...
      had_outer_pipe = String.starts_with?(trimmed, "|") || String.ends_with?(trimmed, "|")
      line = String.trim(line)
      line = if String.starts_with?(line, "|"), do: binary_part(line, 1, byte_size(line) - 1), else: line
      line = if String.ends_with?(line, "|"), do: binary_part(line, 0, byte_size(line) - 1), else: line
      {cells, current, _escaped, pipe_count} = split_table_chars(String.graphemes(line), [], "", false, 0)
      cells = cells ++ [String.trim(current)]

      cond do
        pipe_count == 0 && !had_outer_pipe -> nil
        true -> cells
      end
    else
      nil
  ...
  defp split_table_chars([], cells, current, escaped, pipe_count), do: {cells, current, escaped, pipe_count}
  defp split_table_chars([ch | rest], cells, current, true, pipe_count),
    do: split_table_chars(rest, cells, current <> ch, false, pipe_count)
  defp split_table_chars(["\\" | rest], cells, current, false, pipe_count),
    do: split_table_chars(rest, cells, current <> "\\", true, pipe_count)
  defp split_table_chars(["|" | rest], cells, current, false, pipe_count),
    do: split_table_chars(rest, cells ++ [String.trim(current)], "", false, pipe_count + 1)
  defp split_table_chars([ch | rest], cells, current, false, pipe_count),
    do: split_table_chars(rest, cells, current <> ch, false, pipe_count)

  defp normalize_table_row(cells, width) do
    cells =
      cond do
        length(cells) < width -> cells ++ List.duplicate("", width - length(cells))
        length(cells) > width -> Enum.take(cells, width)
        true -> cells
      end

    Enum.map(cells, &String.trim/1)
  end
