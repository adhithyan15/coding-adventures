...
namespace CodingAdventures.PDF417;

// =============================================================================
// PDF417.cs — PDF417 Stacked Linear Barcode Encoder — ISO/IEC 15438:2015
// =============================================================================
//
...
//   17 modules total per codeword
//
// "417" = 4 elements × 17 modules per element group.
//
// PDF417 is a *stacked linear* barcode — not a true matrix code like QR Code
// or Data Matrix.  It consists of many rows of 1D-like encoded codewords stacked
// vertically.  A single linear scanner sweeping one horizontal row can read that
// row independently; row indicator codewords carry enough context to reconstruct
...
//
// ## Encoding pipeline (v0.1.0 — byte compaction only)
//
//   raw bytes
//     → byte compaction        (codeword 924 latch + 6-bytes→5-codewords base-900)
//     → length descriptor      (codeword 0 = total codewords including itself + ECC)
...
/// <summary>Base class for all PDF417 encoding errors.</summary>
public class PDF417Exception : Exception
{
    /// <summary>Create a new PDF417Exception with a message.</summary>
    public PDF417Exception(string message) : base(message) { }
}

/// <summary>Input data is too long to fit in any valid PDF417 symbol.</summary>
public class InputTooLongException : PDF417Exception
{
    /// <summary>Create a new InputTooLongException.</summary>
    public InputTooLongException(string message) : base(message) { }
}

/// <summary>User-supplied rows or columns are out of the valid range.</summary>
public class InvalidDimensionsException : PDF417Exception
{
    /// <summary>Create a new InvalidDimensionsException.</summary>
    public InvalidDimensionsException(string message) : base(message) { }
}

/// <summary>ECC level is outside the valid range 0–8.</summary>
public class InvalidECCLevelException : PDF417Exception
{
    /// <summary>Create a new InvalidECCLevelException.</summary>
    public InvalidECCLevelException(string message) : base(message) { }
}
...
/// <summary>Options controlling how the PDF417 symbol is encoded.</summary>
public sealed class PDF417Options
{
    /// <summary>
    /// Reed-Solomon error correction level (0–8).
    ///
    /// Higher levels add more ECC codewords, recovering from greater physical
    /// damage at the cost of fewer data characters fitting in the symbol.
    ///
    /// When null (default), the level is auto-selected based on data length:
    ///   ≤40 codewords → level 2, ≤160 → level 3, ≤320 → level 4,
    ///   ≤863 → level 5, otherwise → level 6.
    /// </summary>
    public int? EccLevel { get; init; }

    /// <summary>
    /// Number of data columns (1–30).
    ///
    /// When null (default), auto-selected to produce a roughly square symbol
    /// using the heuristic: c = ceil(sqrt(total / 3)), clamped to 1–30.
    /// </summary>
    ...
    ///
    /// Larger values produce taller symbols, which improves scan reliability on
    /// low-resolution printing.  Default: 3.
    /// </summary>
    public int RowHeight { get; init; } = 3;
}
...
/// <summary>
/// PDF417 encoder — ISO/IEC 15438:2015 compliant.
///
/// <para>v0.1.0 implements byte compaction only. All inputs are treated as raw
/// bytes regardless of content. Text and numeric compaction are planned for
/// v0.2.0.</para>
///
/// <para>Usage:</para>
/// <code>
///   var grid = PDF417Encoder.Encode("HELLO WORLD"u8.ToArray());
///   // grid is a ModuleGrid — pass to Barcode2D.Layout() for rendering.
/// </code>
/// </summary>
public static class PDF417Encoder
{
    // ── Symbol dimension constraints ─────────────────────────────────────────
    ...
    //
    // The PDF417 codeword alphabet is 0–928 (929 values).  Values 900–928 are
    // reserved for control purposes.  v0.1.0 uses only three:
    //
    //   924 — latch to byte compaction (any length)
    //   900 — padding / latch to text (neutral filler after data region)
    private const int LatchByte = 924;
    private const int PaddingCw = 900;

    /// <summary>
    /// Encode <paramref name="data"/> bytes as a PDF417 symbol and return the
    /// <see cref="ModuleGrid"/>.
    ///
    /// <para>The grid is in abstract module units.  Pass it to
    /// <c>Barcode2D.Layout()</c> to get a pixel-resolved PaintScene.</para>
    /// </summary>
    /// <param name="data">Raw byte payload to encode.</param>
    /// <param name="options">Encoding options (ECC level, columns, row height).
    /// Pass null to use all defaults.</param>
    /// <exception cref="InvalidECCLevelException">
    /// Thrown when <see cref="PDF417Options.EccLevel"/> is not in 0–8.
    /// </exception>
    /// <exception cref="InvalidDimensionsException">
    /// Thrown when <see cref="PDF417Options.Columns"/> is outside 1–30.
    /// </exception>
    /// <exception cref="InputTooLongException">
    /// Thrown when the data exceeds the maximum symbol capacity.
    /// </exception>
    public static ModuleGrid Encode(byte[] data, PDF417Options? options = null)
    {
        options ??= new PDF417Options();

        // ── Validate ECC level ────────────────────────────────────────────────
        if (options.EccLevel.HasValue &&
            (options.EccLevel.Value < 0 || options.EccLevel.Value > 8))
        {
            throw new InvalidECCLevelException(
                $"ECC level must be 0–8, got {options.EccLevel.Value}");
        }

        // ── Step 1: Byte compaction ───────────────────────────────────────────
        //
        // Every input byte is encoded using byte compaction mode.  This is the
        // simplest compaction mode and handles any byte sequence.
        //
        // The latch codeword 924 precedes the compacted data.  Full 6-byte
        // groups are converted to 5 base-900 codewords; remaining bytes are
        // encoded one codeword per byte.
        var dataCwords = ByteCompact(data);

        // ── Step 2: Auto-select ECC level ─────────────────────────────────────
        //
        // The +1 accounts for the length descriptor which is also counted in
        // the data region for ECC auto-selection purposes.
        int eccLevel = options.EccLevel ?? AutoEccLevel(dataCwords.Count + 1);
        int eccCount = 1 << (eccLevel + 1); // 2^(eccLevel+1), range 2–512

        // ── Step 3: Compute length descriptor ────────────────────────────────
        //
        // The length descriptor (first codeword in the symbol's data region)
        // contains the total number of data codewords, counting:
        //   - itself (1)
        //   - all compacted data codewords
        //   - all ECC codewords
        // It does NOT count row indicator codewords.
        int lengthDesc = 1 + dataCwords.Count + eccCount;

        // Full data sequence for RS encoding: [lengthDesc, ...dataCwords]
        var fullData = new List<int>(dataCwords.Count + 1) { lengthDesc };
        fullData.AddRange(dataCwords);

        // ── Step 4: RS ECC ────────────────────────────────────────────────────
        //
        // Compute Reed-Solomon ECC codewords over GF(929) using the b=3
        // convention (roots are α^3, α^4, ..., α^{k+2}).
        var eccCwords = RsEncode(fullData, eccLevel);

        // ── Step 5: Choose dimensions ─────────────────────────────────────────
        int totalCwords = fullData.Count + eccCwords.Count;

        int cols, rows;

        if (options.Columns.HasValue)
        {
            int c = options.Columns.Value;
            if (c < MinCols || c > MaxCols)
                throw new InvalidDimensionsException(
                    $"columns must be 1–30, got {c}");

            cols = c;
            rows = Math.Max(MinRows, (int)Math.Ceiling((double)totalCwords / cols));
            if (rows > MaxRows)
        ...
        //
        // Padding uses codeword 900 (latch-to-text), which is a neutral filler:
        // a scanner encountering it in the data region simply switches to text
        // mode, producing no output until actual character codewords follow.
        int paddingCount = cols * rows - totalCwords;
        for (int i = 0; i < paddingCount; i++)
            fullData.Add(PaddingCw);

        // Full sequence: [data+padding, ecc]
        var fullSequence = new List<int>(cols * rows);
        fullSequence.AddRange(fullData);
        fullSequence.AddRange(eccCwords);
    ...
    /// <summary>
    /// Convenience overload: encode a UTF-8 string.
    /// </summary>
    public static ModuleGrid Encode(string text, PDF417Options? options = null)
        => Encode(System.Text.Encoding.UTF8.GetBytes(text), options);

    // ─────────────────────────────────────────────────────────────────────────
    // Byte compaction
    // ─────────────────────────────────────────────────────────────────────────
    //
    // Every 6 input bytes are treated as a 48-bit big-endian integer and
    // expressed in base 900 to produce 5 codewords.  The math:
    //
    //   256^6 = 281,474,976,710,656   (max 6-byte value)
    //   900^5 = 590,490,000,000,000   (always ≥ 256^6)
    //
    // So 5 base-900 codewords always hold 6 bytes.  The compression ratio is
    // 6/5 = 1.2 bytes per codeword.
    //
    // Remaining 1–5 bytes are mapped one-to-one as raw codeword values (0–255
    // are all valid codeword values).
    //
    // We use ulong (64-bit) arithmetic.  256^6 < 2^48 < 2^64, so no overflow.

    internal static List<int> ByteCompact(byte[] bytes)
    {
        var cwords = new List<int>();
        cwords.Add(LatchByte); // latch to byte compaction
        ...
        while (i + 6 <= len)
        {
            ...
            // Convert to base 900 → 5 codewords, most-significant first.
            var group = new int[5];
            for (int j = 4; j >= 0; j--)
            {
                group[j] = (int)(n % 900);
            ...
            cwords.AddRange(group);
            i += 6;
        }

        // Remaining bytes: 1 codeword per byte (direct mapping).
        while (i < len)
        {
            cwords.Add(bytes[i]);
            i++;
    ...
    // ─────────────────────────────────────────────────────────────────────────
    //
    // The recommended minimum ECC level is based on the data codeword count.
    // More data → higher level to maintain a reasonable redundancy ratio.

    internal static int AutoEccLevel(int dataCount)
    {
    ...
    //
    // The factor of 3 accounts for PDF417 rows being roughly 3× wider than
    // tall (each row is at least 17 modules wide per codeword, while default
    // row height is 3 module-rows).

    internal static (int cols, int rows) ChooseDimensions(int total)
    {
        int c = (int)Math.Ceiling(Math.Sqrt(total / 3.0));
        c = Math.Max(MinCols, Math.Min(MaxCols, c));
        int r = (int)Math.Ceiling((double)total / c);
        r = Math.Max(MinRows, Math.Min(MaxRows, r));
        return (c, r);
    }
    ...
    // ─────────────────────────────────────────────────────────────────────────
    //
    // Each row in the symbol carries two row indicator codewords (Left and
    // Right) that together encode three metadata values about the symbol:
    //
    //   R_info = (R - 1) / 3        where R = total rows (3..90) → 0..29
    //   C_info = C - 1              where C = data columns (1..30) → 0..29
    //   L_info = 3 * ECC_level + (R - 1) mod 3    encodes ECC + row parity
    //
    // For row r (0-indexed), cluster = r % 3, and the three values are
    // distributed across LRI and RRI as follows (from TypeScript reference):
    //
    ...
    //
    // The base 30*(r/3) encodes the "row group" (groups of 3 rows).

    internal static int ComputeLri(int r, int rows, int cols, int eccLevel)
    {
        int rInfo = (rows - 1) / 3;
        int cInfo = cols - 1;
        ...
        return cluster switch
        {
            0 => 30 * rowGroup + rInfo,
            1 => 30 * rowGroup + lInfo,
            _ => 30 * rowGroup + cInfo, // cluster 2
        };
    }

    internal static int ComputeRri(int r, int rows, int cols, int eccLevel)
    {
        int rInfo = (rows - 1) / 3;
        int cInfo = cols - 1;
        int lInfo = 3 * eccLevel + (rows - 1) % 3;
    ...
    //
    // We use precomputed log and antilog (exp) tables for O(1) multiplication:
    //
    //   GF_EXP[i] = α^i mod 929   (α = 3, the primitive root)
    //   GF_LOG[v] = i  such that  α^i ≡ v (mod 929)
    //
    // Fast multiply:
    //   gfMul(a, b) = GF_EXP[(GF_LOG[a] + GF_LOG[b]) mod 928]
    //               = 0 if either operand is 0
    //
    // The tables take 929 × 2 arrays of int (about 7 KB) and are built once
    // at class initialization in a static constructor — negligible cost.

    private const int Gf929Prime = 929;
    private const int Gf929Alpha = 3;   // primitive root mod 929
    private const int Gf929Order = 928; // multiplicative group order = prime - 1

    internal static readonly int[] GfExp = new int[930]; // index 0..928; [928]==[0] wrap
    internal static readonly int[] GfLog = new int[929]; // index 1..928; [0] undefined

    static PDF417Encoder()
    {
        // Build the discrete logarithm and antilogarithm tables.
        int val = 1;
        for (int i = 0; i < Gf929Order; i++)
        {
            GfExp[i] = val;
            GfLog[val] = i;
            val = (val * Gf929Alpha) % Gf929Prime;
        }
        // Wrap-around entry for convenience in multiplication.
        GfExp[Gf929Order] = GfExp[0]; // GfExp[928] = 1
    }

    /// <summary>GF(929) add: (a + b) mod 929.</summary>
    internal static int GfAdd(int a, int b) => (a + b) % Gf929Prime;

    /// <summary>GF(929) multiply using log/antilog tables.</summary>
    internal static int GfMul(int a, int b)
    {
        if (a == 0 || b == 0) return 0;
        return GfExp[(GfLog[a] + GfLog[b]) % Gf929Order];
    }
    ...
    // ─────────────────────────────────────────────────────────────────────────
    //
    // For ECC level L, the number of ECC codewords is k = 2^(L+1).
    // The generator polynomial uses the b=3 convention — roots are
    // α^3, α^4, ..., α^{k+2}:
    //
    ...
    //   Start:  g = [1]
    //   Multiply by (x − α^j):
    //     new_g[i]   += g[i]        (shift up one degree)
    //     new_g[i+1] += g[i] × (−α^j)
    //
    // In GF(929), −α^j = (929 − α^j) mod 929.

    internal static int[] BuildGenerator(int eccLevel)
    {
        ...
        for (int j = 3; j <= k + 2; j++)
        {
            int root = GfExp[j % Gf929Order]; // α^j
            int negRoot = Gf929Prime - root;  // −α^j in GF(929)
    ...
    // ─────────────────────────────────────────────────────────────────────────
    //
    // Given data codewords D and generator polynomial g of degree k, compute k
    // ECC codewords = D(x) × x^k mod g(x).
    //
    // The standard shift-register (LFSR) algorithm:
    //
    //   ecc = [0, 0, ..., 0]   (k zeros)
    //
    //   for each data codeword d:
    //     feedback = d + ecc[0]        (in GF(929))
    //     shift ecc left by 1 (drop ecc[0], append 0)
    //     for each i:
    //       ecc[i] += g[k-i] × feedback
    //
    // No interleaving — unlike QR Code, all data goes to a single RS encoder.
    // This significantly simplifies the encoder compared to QR Code.

    internal static List<int> RsEncode(List<int> data, int eccLevel)
    {
        ...
        foreach (int d in data)
        {
            ...
            for (int i = 0; i < k - 1; i++)
                ecc[i] = ecc[i + 1];
            ecc[k - 1] = 0;
            // Add feedback × generator coefficient.
            for (int i = 0; i < k; i++)
                ecc[i] = GfAdd(ecc[i], GfMul(g[k - i], feedback));
        }

        return new List<int>(ecc);
    }

    // ─────────────────────────────────────────────────────────────────────────
    // Codeword → 17-module expansion
    // ─────────────────────────────────────────────────────────────────────────
    //
    // Each codeword (0–928) has a different 17-module bar/space pattern in each
    // cluster.  The pattern is looked up from the cluster table and stored as a
    ...
    //
    // Each field is 4 bits (value 1–6).  b_i = bar width, s_i = space width.
    // The alternating sequence is: bar, space, bar, space, bar, space, bar, space.
    // Sum of all 8 widths = 17 modules.

    private static void ExpandPattern(uint packed, bool[] modules, ref int offset)
    {
        ...
        for (int i = 0; i < b1; i++) modules[offset++] = true;
        for (int i = 0; i < s1; i++) modules[offset++] = false;
        for (int i = 0; i < b2; i++) modules[offset++] = true;
        for (int i = 0; i < s2; i++) modules[offset++] = false;
        for (int i = 0; i < b3; i++) modules[offset++] = true;
        for (int i = 0; i < s3; i++) modules[offset++] = false;
        for (int i = 0; i < b4; i++) modules[offset++] = true;
        for (int i = 0; i < s4; i++) modules[offset++] = false;
    }
    ...
    //   [start pattern (17)] [LRI (17)] [data_0..data_{c-1} (17 each)] [RRI (17)] [stop (18)]
    //
    // Total module width per row = 17 + 17 + 17c + 17 + 18 = 69 + 17c.
    //
    // The grid height is rows × rowHeight (each logical row is repeated
    // rowHeight times vertically).
    //
    // Start pattern:   11111111010101000  (17 modules)
    //   widths: [8, 1, 1, 1, 1, 1, 1, 3]
    //
    // Stop pattern:    111111101000101001 (18 modules)
    //   widths: [7, 1, 1, 3, 1, 1, 1, 2, 1]

    // Start pattern: bar(8), space(1), bar(1), space(1), bar(1), space(1), bar(1), space(3)
    private static readonly bool[] StartModules = ExpandWidths([8, 1, 1, 1, 1, 1, 1, 3]);

    // Stop pattern: bar(7), space(1), bar(1), space(3), bar(1), space(1), bar(1), space(2), bar(1)
    private static readonly bool[] StopModules = ExpandWidths([7, 1, 1, 3, 1, 1, 1, 2, 1]);

    private static bool[] ExpandWidths(int[] widths)
    {
        int total = 0;
        foreach (int w in widths) total += w;
        bool[] mods = new bool[total];
        int pos = 0;
        bool dark = true;
    ...
    private static ModuleGrid Rasterize(
        List<int> sequence, int rows, int cols, int eccLevel, int rowHeight)
    {
        // module width = 17(start) + 17(LRI) + 17*cols(data) + 17(RRI) + 18(stop)
        int moduleWidth  = 69 + 17 * cols;
        int moduleHeight = rows * rowHeight;

        var grid = ModuleGrid.Create(moduleHeight, moduleWidth, ModuleShape.Square);

        for (int r = 0; r < rows; r++)
        {
            int cluster = r % 3;
            uint[] clusterTable = ClusterTables.Tables[cluster];

            bool[] rowMods = new bool[moduleWidth];
            int offset = 0;

            // 1. Start pattern (17 modules) — same for every row and cluster.
            Array.Copy(StartModules, 0, rowMods, offset, StartModules.Length);
            offset += StartModules.Length;

            // 2. Left Row Indicator codeword (17 modules).
            int lri = ComputeLri(r, rows, cols, eccLevel);
            ExpandPattern(clusterTable[lri], rowMods, ref offset);

            // 3. Data codewords (17 modules each).
            for (int j = 0; j < cols; j++)
            {
                int cw = sequence[r * cols + j];
                ExpandPattern(clusterTable[cw], rowMods, ref offset);
            }

            // 4. Right Row Indicator codeword (17 modules).
            int rri = ComputeRri(r, rows, cols, eccLevel);
            ExpandPattern(clusterTable[rri], rowMods, ref offset);

            // 5. Stop pattern (18 modules).
            Array.Copy(StopModules, 0, rowMods, offset, StopModules.Length);
            offset += StopModules.Length;

            // Assertion: module count must match exactly.
            if (offset != moduleWidth)
                throw new InvalidOperationException(
                    $"Row {r}: expected {moduleWidth} modules but produced {offset}.");

            // Write this row rowHeight times into the grid.
            int baseModuleRow = r * rowHeight;
            for (int h = 0; h < rowHeight; h++)
            {
                int moduleRow = baseModuleRow + h;
                for (int col = 0; col < moduleWidth; col++)
                {
                    if (rowMods[col])
                        grid = grid.SetModule(moduleRow, col, true);
                }
            }
        }
...
// =============================================================================
// ClusterTables.cs — PDF417 Bar/Space Pattern Tables
// =============================================================================
//
// PDF417 uses three different mappings from codeword value to bar/space pattern,
// cycling row by row:
//
//   Row 0 → cluster 0   (row % 3 == 0)
//   Row 1 → cluster 1   (row % 3 == 1)
//   Row 2 → cluster 2   (row % 3 == 2)
//   Row 3 → cluster 0   ...
//
// Why three clusters? A scanner sweeping a single row needs to verify it is
// reading a complete PDF417 row, not a spurious slice.  Because cluster 0, 1,
// and 2 use different bar/space patterns for the same codeword value, a reader
// that knows the cluster can verify consistency with the row indicators.
//
// Each table entry packs 8 element widths into a 32-bit unsigned integer:
...
//   bits 3..0   = s4
//
// All widths are in modules (1–6); b1+s1+b2+s2+b3+s3+b4+s4 = 17.
//
// The tables were extracted from the Python pdf417 library (MIT License) and
// verified to be consistent with ISO/IEC 15438:2015 Annex B.
//
// Three tables × 929 entries = 2,787 entries × 4 bytes = ~11 KB.

/// <summary>
/// Internal cluster table storage for PDF417 codeword → module pattern lookup.
/// </summary>
internal static class ClusterTables
{
    /// <summary>
    /// Three cluster tables indexed by <c>row % 3</c>.
    /// Each sub-array has 929 entries covering codeword values 0–928.
    /// </summary>
    internal static readonly uint[][] Tables =
    [
        // ── Cluster 0 (rows where r % 3 == 0) ──────────────────────────────
        [
