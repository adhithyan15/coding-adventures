...
use language_parser::create_fsharp_parser;
...
pub fn parse(source: &str, version: &str) -> Result<String, JsValue> {
    let mut parser = create_fsharp_parser(source, resolve_version(version)).map_err(to_js_error)?;
    let ast = parser
        .parse()
        .map_err(|e| to_js_error(format!("F# parse failed: {e}")))?;
...
mod tests {
    ...
    fn wrapper_parses_to_json() {
        let json = parse("let value = 1", "").unwrap();
