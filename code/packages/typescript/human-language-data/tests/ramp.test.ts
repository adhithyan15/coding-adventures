...
describe("the script ramp against the real corpus", () => {
  it("pins the script ramp, which no gate had ever measured", () => {
    ...
    expect(report.summary.lessonViolations).toBe(61); // HL-C259: -1 -- IMPROVEMENT, one fewer ramp violation once gujarati declares its atoms // HL-C134: the hand-written prose carried back into the lessons is now visible to this measurement — the words were always on the page, only the markdown had not seen them
