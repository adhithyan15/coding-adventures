 ...
 */
export class ControlHazardDetector {
  /**
   * Check if a branch in the EX stage was mispredicted.
   *
   * @param exStage - The instruction currently in the EX stage. We check if it's
   *     a branch and whether its prediction was correct.
   * @returns FLUSH if mispredicted (with flushCount=2 for IF and ID),
   *     NONE otherwise.
   *
   * === Decision Logic ===
   *
   ...
   * one boolean (branchPredictedTaken) against another
   * (branchTaken). All the complexity of prediction lives in the
   * branch predictor — we just check if it was right.
   */
  detect(exStage: PipelineSlot): HazardResult {
    ...
    if (exStage.branchPredictedTaken === exStage.branchTaken) {
      return new HazardResult({
        action: HazardAction.NONE,
        reason:
          `branch at PC=0x${exStage.pc.toString(16).toUpperCase().padStart(4, "0")} correctly predicted ` +
          `${exStage.branchTaken ? "taken" : "not taken"}`,
      });
    }
    ...
    return new HazardResult({
      ...
      flushCount: 2, // flush IF and ID stages
      reason:
        `branch misprediction at PC=0x${exStage.pc.toString(16).toUpperCase().padStart(4, "0")}: ` +
        `${direction} — flushing IF and ID stages`,
    });
  }
}
