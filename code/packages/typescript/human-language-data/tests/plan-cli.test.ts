...
describe("the plan CLI", () => {
  ...
  it("counts a duplicated inventory once", () => {
    ...
    expect(out).toMatch(/642 uncovered point\(s\) across 7 written/);
    ...
    expect(out).toMatch(/642 uncovered point\(s\) across 7 written/);
    // 686 -> 642. The Telugu chapter 74-80 vocabulary tranche was authored
    ...
    // exactly that. Telugu went 169/326 (52%) to 213/326 (65%) against an
    // unchanged denominator: no point was added, removed or reworded to move it.
    // Re-measured with `npm run plan` against the merged tree, not subtracted.
    //
