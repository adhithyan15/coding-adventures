...
% canonical-source-hash: fnv1a64:ed3208131198dff3
...
Only the first of those is real Latin. \emph{Nomen} and \emph{culmen} are neuter, and a neuter accusative matches its nominative -- so Latin had \emph{nomen} and \emph{culmen}, full stop. The other two are \textbf{reconstructed}, remade later by analogy with \emph{hominem}. A dictionary prints a star in front of a form like that to say that nobody ever wrote it down.

The vowel is irregular too. A short \emph{u} normally lands as \emph{o} in Spanish, so this word should have come out \emph{combre}. Corominas blames the \emph{l}, which pulled the vowel upward and then vanished into it. So do not use \emph{cumbre} as your model for what a short \emph{u} does.

One last thing, because it is the opposite of what you would guess. The word closest to \emph{culmen} is not English \textbf{culminate}, a late borrowing taken straight off \emph{culmen} itself. It is Latin \textbf{columen} -- \emph{culmen} is that word with its middle syllable squeezed out, one word in two states of compression.

Latin \textbf{columna}, the pillar, is built on the same root by a different route, which makes it a \textbf{cousin} rather than a twin. The difference is the point: a twin is one word in two shapes, a cousin is a separate word built from the same material. A roof ridge, a pillar and a mountaintop are all the thing that stands highest.
\end{cousinweb}
