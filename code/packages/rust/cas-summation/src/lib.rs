...
/// contributing 0. Using the ×2 integer trick: `effective_x2 = 2·m`.
/// Caller checks `2·den_deg > effective_x2`.
fn twenty_four_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut log_count: usize = 0;
    let mut poly_deg: i64 = 0;
...
fn one_sqrt_twenty_four_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_deg_x2: Option<i64> = None;
    let mut log_count: usize = 0;
...
/// `effective_x2 = a + b + 2·m`. Caller checks `2·den_deg > effective_x2`.
fn two_sqrt_twenty_four_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 2 || log_count != 24 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + 2 * poly_deg)
}
...
/// `effective_x2 = a + b + c + 2·m`. Caller checks `2·den_deg > effective_x2`.
fn three_sqrt_twenty_four_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 3 || log_count != 24 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2] + 2 * poly_deg)
}
...
fn four_sqrt_twenty_four_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 4 || log_count != 24 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2]
...
fn five_sqrt_twenty_four_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 5 || log_count != 24 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2]
        + sqrt_degs_x2[3] + sqrt_degs_x2[4] + 2 * poly_deg)
}
...
/// Phase 197 — Twenty-Five-Log × polynomial numerator.
///
/// Effective growth: `log(k)²⁵ · k^m`. `log²⁵(k)` is sub-polynomial (`o(k^ε)`),
/// contributing 0. Using the ×2 integer trick: `effective_x2 = 2·m`.
/// Caller checks `2·den_deg > effective_x2`.
fn twenty_five_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut log_count: usize = 0;
    let mut poly_deg: i64 = 0;
...
/// Effective growth: `sqrt(k^a) · log(k)²⁵ · k^m`. Using the ×2 integer trick:
/// `effective_x2 = a + 2·m`. Caller checks `2·den_deg > effective_x2`.
fn one_sqrt_twenty_five_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_deg_x2: Option<i64> = None;
    let mut log_count: usize = 0;
...
/// Effective growth: `sqrt(k^a)·sqrt(k^b) · log(k)²⁵ · k^m`. Using the ×2 integer trick:
/// `effective_x2 = a + b + 2·m`. Caller checks `2·den_deg > effective_x2`.
fn two_sqrt_twenty_five_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 2 || log_count != 25 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + 2 * poly_deg)
}
...
/// Effective growth: `sqrt(k^a)·sqrt(k^b)·sqrt(k^c) · log(k)²⁵ · k^m`. Using the ×2 integer trick:
/// `effective_x2 = a + b + c + 2·m`. Caller checks `2·den_deg > effective_x2`.
fn three_sqrt_twenty_five_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 3 || log_count != 25 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2] + 2 * poly_deg)
}
...
/// Effective growth: four `sqrt(k^aᵢ)` factors × `log(k)²⁵` × `k^m`. Using the ×2 integer trick:
/// `effective_x2 = a₁+a₂+a₃+a₄ + 2·m`. Caller checks `2·den_deg > effective_x2`.
fn four_sqrt_twenty_five_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 4 || log_count != 25 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2]
        + sqrt_degs_x2[3] + 2 * poly_deg)
}
...
fn five_sqrt_twenty_five_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 5 || log_count != 25 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2]
        + sqrt_degs_x2[3] + sqrt_degs_x2[4] + 2 * poly_deg)
}
...
/// Effective growth: `log(k)²⁷ · k^m`. Using the ×2 integer trick:
/// `effective_x2 = 2·m`. Caller checks `2·den_deg > effective_x2`.
fn twenty_eight_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut log_count: usize = 0;
    let mut poly_deg: i64 = 0;
...
/// Effective growth: `sqrt(k^a) · log(k)²⁸ · k^m`. Using the ×2 integer trick:
/// `effective_x2 = a + 2·m`. Caller checks `2·den_deg > effective_x2`.
fn one_sqrt_twenty_eight_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_deg_x2: Option<i64> = None;
    let mut log_count: usize = 0;
...
/// Effective growth: `sqrt(k^a)·sqrt(k^b) · log(k)²⁸ · k^m`. Using the ×2 integer trick:
/// `effective_x2 = a + b + 2·m`. Caller checks `2·den_deg > effective_x2`.
fn two_sqrt_twenty_eight_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 2 || log_count != 28 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + 2 * poly_deg)
}
...
/// Effective growth: `sqrt(k^a)·sqrt(k^b)·sqrt(k^c) · log(k)²⁸ · k^m`. Using the ×2 integer trick:
/// `effective_x2 = a + b + c + 2·m`. Caller checks `2·den_deg > effective_x2`.
fn three_sqrt_twenty_eight_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 3 || log_count != 28 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2] + 2 * poly_deg)
}
...
/// Effective growth: four `sqrt(k^aᵢ)` factors × `log(k)²⁸` × `k^m`. Using the ×2 integer trick:
/// `effective_x2 = a₁+a₂+a₃+a₄ + 2·m`. Caller checks `2·den_deg > effective_x2`.
fn four_sqrt_twenty_eight_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 4 || log_count != 28 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2]
        + sqrt_degs_x2[3] + 2 * poly_deg)
}
...
fn five_sqrt_twenty_eight_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 5 || log_count != 28 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2]
        + sqrt_degs_x2[3] + sqrt_degs_x2[4] + 2 * poly_deg)
}
...
/// Effective growth: `log(k)²⁹ · k^m`. Using the ×2 integer trick:
/// `effective_x2 = 2·m`. Caller checks `2·den_deg > effective_x2`.
fn twenty_nine_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut log_count: usize = 0;
    let mut poly_deg: i64 = 0;
...
/// Effective growth: `sqrt(k^a) · log(k)²⁹ · k^m`. Using the ×2 integer trick:
/// `effective_x2 = a + 2·m`. Caller checks `2·den_deg > effective_x2`.
fn one_sqrt_twenty_nine_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_deg_x2: Option<i64> = None;
    let mut log_count: usize = 0;
...
fn two_sqrt_twenty_nine_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 2 || log_count != 29 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + 2 * poly_deg)
}

/// Phase 224 — Three-Sqrt × Twenty-Nine-Log × polynomial numerator.
fn three_sqrt_twenty_nine_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 3 || log_count != 29 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2] + 2 * poly_deg)
}

/// Phase 225 — Four-Sqrt × Twenty-Nine-Log × polynomial numerator.
fn four_sqrt_twenty_nine_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 4 || log_count != 29 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2]
        + sqrt_degs_x2[3] + 2 * poly_deg)
}
...
fn five_sqrt_twenty_nine_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 5 || log_count != 29 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2]
        + sqrt_degs_x2[3] + sqrt_degs_x2[4] + 2 * poly_deg)
}
...
/// Effective growth: `log(k)³¹ · k^m`. Using the ×2 integer trick:
/// `effective_x2 = 2·m`. Caller checks `2·den_deg > effective_x2`.
fn thirty_one_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut log_count: usize = 0;
    let mut poly_deg: i64 = 0;
...
/// Effective growth: `sqrt(k^a) · log(k)³¹ · k^m`. Using the ×2 integer trick:
/// `effective_x2 = a + 2·m`. Caller checks `2·den_deg > effective_x2`.
fn one_sqrt_thirty_one_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_deg_x2: Option<i64> = None;
    let mut log_count: usize = 0;
...
fn two_sqrt_thirty_one_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 2 || log_count != 31 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + 2 * poly_deg)
}

/// Phase 236 — Three-Sqrt × Thirty-One-Log × polynomial numerator.
fn three_sqrt_thirty_one_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 3 || log_count != 31 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2] + 2 * poly_deg)
}

/// Phase 237 — Four-Sqrt × Thirty-One-Log × polynomial numerator.
fn four_sqrt_thirty_one_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 4 || log_count != 31 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2]
        + sqrt_degs_x2[3] + 2 * poly_deg)
}
...
fn five_sqrt_thirty_one_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 5 || log_count != 31 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2]
        + sqrt_degs_x2[3] + sqrt_degs_x2[4] + 2 * poly_deg)
}
...
fn thirty_five_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
...
fn one_sqrt_thirty_five_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_deg_x2: Option<i64> = None;
    let mut log_count: i64 = 0;
    ...
    if sqrt_deg_x2.is_none() || log_count != 35 { return None; }
    Some(sqrt_deg_x2.unwrap() + 2 * poly_deg_sum)
}

/// Phase 259 — Two-Sqrt × Thirty-Five-Log × polynomial numerator.
fn two_sqrt_thirty_five_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 2 || log_count != 35 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 260 — Three-Sqrt × Thirty-Five-Log × polynomial numerator.
fn three_sqrt_thirty_five_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 3 || log_count != 35 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 261 — Four-Sqrt × Thirty-Five-Log × polynomial numerator.
fn four_sqrt_thirty_five_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 4 || log_count != 35 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 262 — Five-Sqrt × Thirty-Five-Log × polynomial numerator.
/// Completes the Thirty-Five-Log family (Phases 257–262).
fn five_sqrt_thirty_five_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 5 || log_count != 35 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}
...
fn thirty_six_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut log_count: i64 = 0;
    let mut poly_deg_sum: i64 = 0;
...
fn one_sqrt_thirty_six_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_deg_x2: Option<i64> = None;
    let mut log_count: i64 = 0;
    ...
    if sqrt_deg_x2.is_none() || log_count != 36 { return None; }
    Some(sqrt_deg_x2.unwrap() + 2 * poly_deg_sum)
}

/// Phase 265 — Two-Sqrt × Thirty-Six-Log × polynomial numerator.
fn two_sqrt_thirty_six_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 2 || log_count != 36 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 266 — Three-Sqrt × Thirty-Six-Log × polynomial numerator.
fn three_sqrt_thirty_six_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 3 || log_count != 36 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 267 — Four-Sqrt × Thirty-Six-Log × polynomial numerator.
fn four_sqrt_thirty_six_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 4 || log_count != 36 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 268 — Five-Sqrt × Thirty-Six-Log × polynomial numerator.
/// Completes the Thirty-Six-Log family (Phases 263–268).
fn five_sqrt_thirty_six_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 5 || log_count != 36 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}
...
fn thirty_eight_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut log_count: i64 = 0;
    let mut poly_deg_sum: i64 = 0;
...
fn one_sqrt_thirty_eight_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_deg_x2: Option<i64> = None;
    let mut log_count: i64 = 0;
    ...
    if sqrt_deg_x2.is_none() || log_count != 38 { return None; }
    Some(sqrt_deg_x2.unwrap() + 2 * poly_deg_sum)
}

/// Phase 277 — Two-Sqrt × Thirty-Eight-Log × polynomial numerator.
fn two_sqrt_thirty_eight_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 2 || log_count != 38 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 278 — Three-Sqrt × Thirty-Eight-Log × polynomial numerator.
fn three_sqrt_thirty_eight_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 3 || log_count != 38 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 279 — Four-Sqrt × Thirty-Eight-Log × polynomial numerator.
fn four_sqrt_thirty_eight_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 4 || log_count != 38 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 280 — Five-Sqrt × Thirty-Eight-Log × polynomial numerator.
/// Completes the Thirty-Eight-Log family (Phases 275–280).
fn five_sqrt_thirty_eight_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 5 || log_count != 38 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}
...
fn thirty_nine_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut log_count: i64 = 0;
    let mut poly_deg_sum: i64 = 0;
...
fn one_sqrt_thirty_nine_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_deg_x2: Option<i64> = None;
    let mut log_count: i64 = 0;
    ...
    if sqrt_deg_x2.is_none() || log_count != 39 { return None; }
    Some(sqrt_deg_x2.unwrap() + 2 * poly_deg_sum)
}

/// Phase 283 — Two-Sqrt × Thirty-Nine-Log × polynomial numerator.
fn two_sqrt_thirty_nine_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 2 || log_count != 39 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 284 — Three-Sqrt × Thirty-Nine-Log × polynomial numerator.
fn three_sqrt_thirty_nine_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 3 || log_count != 39 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 285 — Four-Sqrt × Thirty-Nine-Log × polynomial numerator.
fn four_sqrt_thirty_nine_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 4 || log_count != 39 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 286 — Five-Sqrt × Thirty-Nine-Log × polynomial numerator.
/// Completes the Thirty-Nine-Log family (Phases 281–286).
fn five_sqrt_thirty_nine_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 5 || log_count != 39 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}
...
fn forty_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut log_count: i64 = 0;
    let mut poly_deg_sum: i64 = 0;
...
fn one_sqrt_forty_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_deg_x2: Option<i64> = None;
    let mut log_count: i64 = 0;
    ...
    if sqrt_deg_x2.is_none() || log_count != 40 { return None; }
    Some(sqrt_deg_x2.unwrap() + 2 * poly_deg_sum)
}

/// Phase 289 — Two-Sqrt × Forty-Log × polynomial numerator.
fn two_sqrt_forty_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 2 || log_count != 40 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 290 — Three-Sqrt × Forty-Log × polynomial numerator.
fn three_sqrt_forty_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 3 || log_count != 40 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 291 — Four-Sqrt × Forty-Log × polynomial numerator.
fn four_sqrt_forty_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 4 || log_count != 40 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 292 — Five-Sqrt × Forty-Log × polynomial numerator.
/// Completes the Forty-Log family (Phases 287–292).
fn five_sqrt_forty_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 5 || log_count != 40 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}
...
fn forty_two_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut log_count: i64 = 0;
    let mut poly_deg_sum: i64 = 0;
...
fn one_sqrt_forty_two_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_deg_x2: Option<i64> = None;
    let mut log_count: i64 = 0;
    ...
    if sqrt_deg_x2.is_none() || log_count != 42 { return None; }
    Some(sqrt_deg_x2.unwrap() + 2 * poly_deg_sum)
}

/// Phase 301 — Two-Sqrt × Forty-Two-Log × polynomial numerator.
fn two_sqrt_forty_two_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 2 || log_count != 42 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 302 — Three-Sqrt × Forty-Two-Log × polynomial numerator.
fn three_sqrt_forty_two_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 3 || log_count != 42 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 303 — Four-Sqrt × Forty-Two-Log × polynomial numerator.
fn four_sqrt_forty_two_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 4 || log_count != 42 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 304 — Five-Sqrt × Forty-Two-Log × polynomial numerator.
/// Completes the Forty-Two-Log family (Phases 299–304).
fn five_sqrt_forty_two_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 5 || log_count != 42 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}
...
fn five_sqrt_forty_seven_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 5 || log_count != 47 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 333 — Four-Sqrt × Forty-Seven-Log × polynomial numerator.
fn four_sqrt_forty_seven_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 4 || log_count != 47 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 332 — Three-Sqrt × Forty-Seven-Log × polynomial numerator.
fn three_sqrt_forty_seven_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 3 || log_count != 47 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 331 — Two-Sqrt × Forty-Seven-Log × polynomial numerator.
fn two_sqrt_forty_seven_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 2 || log_count != 47 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 330 — One-Sqrt × Forty-Seven-Log × polynomial numerator.
fn one_sqrt_forty_seven_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_deg: Option<i64> = None;
    let mut log_count: i64 = 0;
...
fn fifty_one_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut log_count: i64 = 0;
    let mut poly_deg_sum: i64 = 0;
...
fn one_sqrt_fifty_one_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_deg: Option<i64> = None;
    let mut log_count: i64 = 0;
...
fn two_sqrt_fifty_one_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 2 || log_count != 51 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 356 — Three-Sqrt × Fifty-One-Log × polynomial numerator.
fn three_sqrt_fifty_one_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 3 || log_count != 51 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 357 — Four-Sqrt × Fifty-One-Log × polynomial numerator.
fn four_sqrt_fifty_one_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 4 || log_count != 51 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 358 — Five-Sqrt × Fifty-One-Log × polynomial numerator.
/// Completes the Fifty-One-Log family (Phases 353–358).
fn five_sqrt_fifty_one_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 5 || log_count != 51 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}
...
fn fifty_six_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut log_count: i64 = 0;
    let mut poly_deg_sum: i64 = 0;
...
fn one_sqrt_fifty_six_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_deg: Option<i64> = None;
    let mut log_count: i64 = 0;
    ...
    if sqrt_deg.is_none() || log_count != 56 { return None; }
    Some(sqrt_deg.unwrap() + 2 * poly_deg_sum)
}

/// Phase 385 — Two-Sqrt × Fifty-Six-Log × polynomial numerator.
fn two_sqrt_fifty_six_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 2 || log_count != 56 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 386 — Three-Sqrt × Fifty-Six-Log × polynomial numerator.
fn three_sqrt_fifty_six_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 3 || log_count != 56 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 387 — Four-Sqrt × Fifty-Six-Log × polynomial numerator.
fn four_sqrt_fifty_six_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 4 || log_count != 56 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 388 — Five-Sqrt × Fifty-Six-Log × polynomial numerator.
/// Completes the Fifty-Six-Log family (Phases 383–388).
fn five_sqrt_fifty_six_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 5 || log_count != 56 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}
...
fn fifty_seven_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut log_count: i64 = 0;
    let mut poly_deg_sum: i64 = 0;
...
fn one_sqrt_fifty_seven_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_deg: Option<i64> = None;
    let mut log_count: i64 = 0;
    ...
    if sqrt_deg.is_none() || log_count != 57 { return None; }
    Some(sqrt_deg.unwrap() + 2 * poly_deg_sum)
}

/// Phase 391 — Two-Sqrt × Fifty-Seven-Log × polynomial numerator.
fn two_sqrt_fifty_seven_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 2 || log_count != 57 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 392 — Three-Sqrt × Fifty-Seven-Log × polynomial numerator.
fn three_sqrt_fifty_seven_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 3 || log_count != 57 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 393 — Four-Sqrt × Fifty-Seven-Log × polynomial numerator.
fn four_sqrt_fifty_seven_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 4 || log_count != 57 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 394 — Five-Sqrt × Fifty-Seven-Log × polynomial numerator.
/// Completes the Fifty-Seven-Log family (Phases 389–394).
fn five_sqrt_fifty_seven_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 5 || log_count != 57 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}
...
/// summands whose numerator contains exactly sixty-three logarithmic factors
/// and zero to five square-root factors.  This is Phase 425: zero sqrts.
fn sixty_three_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut log_count: i64 = 0;
    let mut poly_deg_sum: i64 = 0;
...
fn one_sqrt_sixty_three_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_deg: Option<i64> = None;
    let mut log_count: i64 = 0;
...
fn two_sqrt_sixty_three_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 2 || log_count != 63 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 428 — Three-Sqrt × Sixty-Three-Log × polynomial numerator.
fn three_sqrt_sixty_three_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 3 || log_count != 63 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 429 — Four-Sqrt × Sixty-Three-Log × polynomial numerator.
fn four_sqrt_sixty_three_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 4 || log_count != 63 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}
...
fn five_sqrt_sixty_three_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 5 || log_count != 63 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 419 — Zero-Sqrt × Sixty-Two-Log × polynomial numerator.
///
/// The Sixty-Two-Log family (Phases 419–424) extends the recogniser to
/// summands whose numerator contains exactly sixty-two logarithmic factors
/// and zero to five square-root factors.  This is Phase 419: zero sqrts.
fn sixty_two_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut log_count: i64 = 0;
    let mut poly_deg_sum: i64 = 0;
...
fn one_sqrt_sixty_two_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_deg: Option<i64> = None;
    let mut log_count: i64 = 0;
...
fn two_sqrt_sixty_two_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 2 || log_count != 62 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 422 — Three-Sqrt × Sixty-Two-Log × polynomial numerator.
fn three_sqrt_sixty_two_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 3 || log_count != 62 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 423 — Four-Sqrt × Sixty-Two-Log × polynomial numerator.
fn four_sqrt_sixty_two_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 4 || log_count != 62 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}
...
fn five_sqrt_sixty_two_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 5 || log_count != 62 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}
...
/// summands whose numerator contains exactly sixty logarithmic factors
/// and zero to five square-root factors.  This is Phase 407: zero sqrts.
fn sixty_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut log_count: i64 = 0;
    let mut poly_deg_sum: i64 = 0;
...
fn one_sqrt_sixty_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_deg: Option<i64> = None;
    let mut log_count: i64 = 0;
    ...
    if sqrt_deg.is_none() || log_count != 60 { return None; }
    Some(sqrt_deg.unwrap() + 2 * poly_deg_sum)
}

/// Phase 409 — Two-Sqrt × Sixty-Log × polynomial numerator.
fn two_sqrt_sixty_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 2 || log_count != 60 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 410 — Three-Sqrt × Sixty-Log × polynomial numerator.
fn three_sqrt_sixty_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 3 || log_count != 60 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 411 — Four-Sqrt × Sixty-Log × polynomial numerator.
fn four_sqrt_sixty_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 4 || log_count != 60 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}
...
fn five_sqrt_sixty_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 5 || log_count != 60 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}
...
fn fifty_eight_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut log_count: i64 = 0;
    let mut poly_deg_sum: i64 = 0;
...
fn one_sqrt_fifty_eight_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_deg: Option<i64> = None;
    let mut log_count: i64 = 0;
    ...
    if sqrt_deg.is_none() || log_count != 58 { return None; }
    Some(sqrt_deg.unwrap() + 2 * poly_deg_sum)
}

/// Phase 397 — Two-Sqrt × Fifty-Eight-Log × polynomial numerator.
fn two_sqrt_fifty_eight_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 2 || log_count != 58 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 398 — Three-Sqrt × Fifty-Eight-Log × polynomial numerator.
fn three_sqrt_fifty_eight_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 3 || log_count != 58 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 399 — Four-Sqrt × Fifty-Eight-Log × polynomial numerator.
fn four_sqrt_fifty_eight_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 4 || log_count != 58 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 400 — Five-Sqrt × Fifty-Eight-Log × polynomial numerator.
/// Completes the Fifty-Eight-Log family (Phases 395–400).
fn five_sqrt_fifty_eight_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 5 || log_count != 58 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}
...
fn fifty_five_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut log_count: i64 = 0;
    let mut poly_deg_sum: i64 = 0;
...
fn one_sqrt_fifty_five_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_deg: Option<i64> = None;
    let mut log_count: i64 = 0;
...
fn two_sqrt_fifty_five_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 2 || log_count != 55 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 380 — Three-Sqrt × Fifty-Five-Log × polynomial numerator.
fn three_sqrt_fifty_five_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 3 || log_count != 55 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 381 — Four-Sqrt × Fifty-Five-Log × polynomial numerator.
fn four_sqrt_fifty_five_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 4 || log_count != 55 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 382 — Five-Sqrt × Fifty-Five-Log × polynomial numerator.
/// Completes the Fifty-Five-Log family (Phases 377–382).
fn five_sqrt_fifty_five_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 5 || log_count != 55 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}
...
fn fifty_four_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut log_count: i64 = 0;
    let mut poly_deg_sum: i64 = 0;
...
fn one_sqrt_fifty_four_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_deg: Option<i64> = None;
    let mut log_count: i64 = 0;
...
fn two_sqrt_fifty_four_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 2 || log_count != 54 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 374 — Three-Sqrt × Fifty-Four-Log × polynomial numerator.
fn three_sqrt_fifty_four_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 3 || log_count != 54 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 375 — Four-Sqrt × Fifty-Four-Log × polynomial numerator.
fn four_sqrt_fifty_four_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 4 || log_count != 54 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 376 — Five-Sqrt × Fifty-Four-Log × polynomial numerator.
/// Completes the Fifty-Four-Log family (Phases 371–376).
fn five_sqrt_fifty_four_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 5 || log_count != 54 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}
...
fn fifty_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut log_count: i64 = 0;
    let mut poly_deg_sum: i64 = 0;
...
fn one_sqrt_fifty_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_deg: Option<i64> = None;
    let mut log_count: i64 = 0;
...
fn two_sqrt_fifty_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 2 || log_count != 50 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 350 — Three-Sqrt × Fifty-Log × polynomial numerator.
fn three_sqrt_fifty_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 3 || log_count != 50 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 351 — Four-Sqrt × Fifty-Log × polynomial numerator.
fn four_sqrt_fifty_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 4 || log_count != 50 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 352 — Five-Sqrt × Fifty-Log × polynomial numerator.
/// Completes the Fifty-Log family (Phases 347–352).
fn five_sqrt_fifty_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 5 || log_count != 50 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}
...
fn forty_nine_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut log_count: i64 = 0;
    let mut poly_deg_sum: i64 = 0;
...
fn one_sqrt_forty_nine_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_deg: Option<i64> = None;
    let mut log_count: i64 = 0;
...
fn two_sqrt_forty_nine_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 2 || log_count != 49 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 344 — Three-Sqrt × Forty-Nine-Log × polynomial numerator.
fn three_sqrt_forty_nine_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 3 || log_count != 49 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 345 — Four-Sqrt × Forty-Nine-Log × polynomial numerator.
fn four_sqrt_forty_nine_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 4 || log_count != 49 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 346 — Five-Sqrt × Forty-Nine-Log × polynomial numerator.
/// Completes the Forty-Nine-Log family (Phases 341–346).
fn five_sqrt_forty_nine_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 5 || log_count != 49 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}
...
fn forty_eight_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut log_count: i64 = 0;
    let mut poly_deg_sum: i64 = 0;
...
fn one_sqrt_forty_eight_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_deg: Option<i64> = None;
    let mut log_count: i64 = 0;
...
fn two_sqrt_forty_eight_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 2 || log_count != 48 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 338 — Three-Sqrt × Forty-Eight-Log × polynomial numerator.
fn three_sqrt_forty_eight_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 3 || log_count != 48 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 339 — Four-Sqrt × Forty-Eight-Log × polynomial numerator.
fn four_sqrt_forty_eight_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 4 || log_count != 48 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 340 — Five-Sqrt × Forty-Eight-Log × polynomial numerator.
/// Completes the Forty-Eight-Log family (Phases 335–340).
fn five_sqrt_forty_eight_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 5 || log_count != 48 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}
...
fn forty_seven_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut log_count: i64 = 0;
    let mut poly_deg_sum: i64 = 0;
...
fn five_sqrt_forty_six_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 5 || log_count != 46 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 327 — Four-Sqrt × Forty-Six-Log × polynomial numerator.
fn four_sqrt_forty_six_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 4 || log_count != 46 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 326 — Three-Sqrt × Forty-Six-Log × polynomial numerator.
fn three_sqrt_forty_six_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 3 || log_count != 46 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 325 — Two-Sqrt × Forty-Six-Log × polynomial numerator.
fn two_sqrt_forty_six_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 2 || log_count != 46 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 324 — One-Sqrt × Forty-Six-Log × polynomial numerator.
fn one_sqrt_forty_six_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_deg: Option<i64> = None;
    let mut log_count: i64 = 0;
...
fn forty_six_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut log_count: i64 = 0;
    let mut poly_deg_sum: i64 = 0;
...
fn five_sqrt_forty_five_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 5 || log_count != 45 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 321 — Four-Sqrt × Forty-Five-Log × polynomial numerator.
fn four_sqrt_forty_five_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 4 || log_count != 45 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 320 — Three-Sqrt × Forty-Five-Log × polynomial numerator.
fn three_sqrt_forty_five_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 3 || log_count != 45 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 319 — Two-Sqrt × Forty-Five-Log × polynomial numerator.
fn two_sqrt_forty_five_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 2 || log_count != 45 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 318 — One-Sqrt × Forty-Five-Log × polynomial numerator.
fn one_sqrt_forty_five_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_deg: Option<i64> = None;
    let mut log_count: i64 = 0;
...
fn forty_five_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut log_count: i64 = 0;
    let mut poly_deg_sum: i64 = 0;
...
fn five_sqrt_forty_four_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 5 || log_count != 44 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 315 — Four-Sqrt × Forty-Four-Log × polynomial numerator.
fn four_sqrt_forty_four_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 4 || log_count != 44 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 314 — Three-Sqrt × Forty-Four-Log × polynomial numerator.
fn three_sqrt_forty_four_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 3 || log_count != 44 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 313 — Two-Sqrt × Forty-Four-Log × polynomial numerator.
fn two_sqrt_forty_four_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 2 || log_count != 44 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 312 — One-Sqrt × Forty-Four-Log × polynomial numerator.
fn one_sqrt_forty_four_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_deg: Option<i64> = None;
    let mut log_count: i64 = 0;
...
fn forty_four_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut log_count: i64 = 0;
    let mut poly_deg_sum: i64 = 0;
...
fn forty_one_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut log_count: i64 = 0;
    let mut poly_deg_sum: i64 = 0;
...
fn one_sqrt_forty_one_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_deg_x2: Option<i64> = None;
    let mut log_count: i64 = 0;
    ...
    if sqrt_deg_x2.is_none() || log_count != 41 { return None; }
    Some(sqrt_deg_x2.unwrap() + 2 * poly_deg_sum)
}

/// Phase 295 — Two-Sqrt × Forty-One-Log × polynomial numerator.
fn two_sqrt_forty_one_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 2 || log_count != 41 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 296 — Three-Sqrt × Forty-One-Log × polynomial numerator.
fn three_sqrt_forty_one_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 3 || log_count != 41 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 297 — Four-Sqrt × Forty-One-Log × polynomial numerator.
fn four_sqrt_forty_one_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 4 || log_count != 41 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}

/// Phase 298 — Five-Sqrt × Forty-One-Log × polynomial numerator.
/// Completes the Forty-One-Log family (Phases 293–298).
fn five_sqrt_forty_one_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let IRNode::Apply(app) = node else { return None; };
    if app.head != sym(MUL) { return None; }
    let mut sqrt_degs: Vec<i64> = Vec::new();
    let mut log_count: i64 = 0;
    ...
    if sqrt_degs.len() != 5 || log_count != 41 { return None; }
    Some(sqrt_degs.iter().sum::<i64>() + 2 * poly_deg_sum)
}
...
fn thirty_four_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut log_count: usize = 0;
    let mut poly_deg: i64 = 0;
...
/// Effective growth: `sqrt(k^a) · log(k)³⁴ · k^m`. Using the ×2 integer trick:
/// `effective_x2 = a + 2·m`. Caller checks `2·den_deg > effective_x2`.
fn one_sqrt_thirty_four_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_deg_x2: Option<i64> = None;
    let mut log_count: usize = 0;
...
fn two_sqrt_thirty_four_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 2 || log_count != 34 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + 2 * poly_deg)
}

/// Phase 254 — Three-Sqrt × Thirty-Four-Log × polynomial numerator.
fn three_sqrt_thirty_four_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 3 || log_count != 34 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2] + 2 * poly_deg)
}

/// Phase 255 — Four-Sqrt × Thirty-Four-Log × polynomial numerator.
fn four_sqrt_thirty_four_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 4 || log_count != 34 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2]
        + sqrt_degs_x2[3] + 2 * poly_deg)
}
...
fn five_sqrt_thirty_four_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 5 || log_count != 34 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2]
        + sqrt_degs_x2[3] + sqrt_degs_x2[4] + 2 * poly_deg)
}
...
fn thirty_three_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut log_count: usize = 0;
    let mut poly_deg: i64 = 0;
...
/// Effective growth: `sqrt(k^a) · log(k)³³ · k^m`. Using the ×2 integer trick:
/// `effective_x2 = a + 2·m`. Caller checks `2·den_deg > effective_x2`.
fn one_sqrt_thirty_three_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_deg_x2: Option<i64> = None;
    let mut log_count: usize = 0;
...
fn two_sqrt_thirty_three_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 2 || log_count != 33 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + 2 * poly_deg)
}

/// Phase 248 — Three-Sqrt × Thirty-Three-Log × polynomial numerator.
fn three_sqrt_thirty_three_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 3 || log_count != 33 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2] + 2 * poly_deg)
}

/// Phase 249 — Four-Sqrt × Thirty-Three-Log × polynomial numerator.
fn four_sqrt_thirty_three_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 4 || log_count != 33 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2]
        + sqrt_degs_x2[3] + 2 * poly_deg)
}
...
fn five_sqrt_thirty_three_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 5 || log_count != 33 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2]
        + sqrt_degs_x2[3] + sqrt_degs_x2[4] + 2 * poly_deg)
}
...
fn thirty_two_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut log_count: usize = 0;
    let mut poly_deg: i64 = 0;
...
/// Effective growth: `sqrt(k^a) · log(k)³² · k^m`. Using the ×2 integer trick:
/// `effective_x2 = a + 2·m`. Caller checks `2·den_deg > effective_x2`.
fn one_sqrt_thirty_two_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_deg_x2: Option<i64> = None;
    let mut log_count: usize = 0;
...
fn two_sqrt_thirty_two_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 2 || log_count != 32 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + 2 * poly_deg)
}

/// Phase 242 — Three-Sqrt × Thirty-Two-Log × polynomial numerator.
fn three_sqrt_thirty_two_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 3 || log_count != 32 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2] + 2 * poly_deg)
}

/// Phase 243 — Four-Sqrt × Thirty-Two-Log × polynomial numerator.
fn four_sqrt_thirty_two_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 4 || log_count != 32 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2]
        + sqrt_degs_x2[3] + 2 * poly_deg)
}
...
fn five_sqrt_thirty_two_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 5 || log_count != 32 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2]
        + sqrt_degs_x2[3] + sqrt_degs_x2[4] + 2 * poly_deg)
}
...
/// Effective growth: `log(k)³⁰ · k^m`. Using the ×2 integer trick:
/// `effective_x2 = 2·m`. Caller checks `2·den_deg > effective_x2`.
fn thirty_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut log_count: usize = 0;
    let mut poly_deg: i64 = 0;
...
/// Effective growth: `sqrt(k^a) · log(k)³⁰ · k^m`. Using the ×2 integer trick:
/// `effective_x2 = a + 2·m`. Caller checks `2·den_deg > effective_x2`.
fn one_sqrt_thirty_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_deg_x2: Option<i64> = None;
    let mut log_count: usize = 0;
...
fn two_sqrt_thirty_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 2 || log_count != 30 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + 2 * poly_deg)
}

/// Phase 230 — Three-Sqrt × Thirty-Log × polynomial numerator.
fn three_sqrt_thirty_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 3 || log_count != 30 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2] + 2 * poly_deg)
}

/// Phase 231 — Four-Sqrt × Thirty-Log × polynomial numerator.
fn four_sqrt_thirty_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 4 || log_count != 30 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2]
        + sqrt_degs_x2[3] + 2 * poly_deg)
}
...
fn five_sqrt_thirty_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 5 || log_count != 30 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2]
        + sqrt_degs_x2[3] + sqrt_degs_x2[4] + 2 * poly_deg)
}

fn twenty_seven_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut log_count: usize = 0;
    let mut poly_deg: i64 = 0;
...
/// Effective growth: `sqrt(k^a) · log(k)²⁷ · k^m`. Using the ×2 integer trick:
/// `effective_x2 = a + 2·m`. Caller checks `2·den_deg > effective_x2`.
fn one_sqrt_twenty_seven_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_deg_x2: Option<i64> = None;
    let mut log_count: usize = 0;
...
/// Effective growth: `sqrt(k^a)·sqrt(k^b) · log(k)²⁷ · k^m`. Using the ×2 integer trick:
/// `effective_x2 = a + b + 2·m`. Caller checks `2·den_deg > effective_x2`.
fn two_sqrt_twenty_seven_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 2 || log_count != 27 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + 2 * poly_deg)
}
...
/// Effective growth: `sqrt(k^a)·sqrt(k^b)·sqrt(k^c) · log(k)²⁷ · k^m`. Using the ×2 integer trick:
/// `effective_x2 = a + b + c + 2·m`. Caller checks `2·den_deg > effective_x2`.
fn three_sqrt_twenty_seven_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 3 || log_count != 27 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2] + 2 * poly_deg)
}
...
/// Effective growth: four `sqrt(k^aᵢ)` factors × `log(k)²⁷` × `k^m`. Using the ×2 integer trick:
/// `effective_x2 = a₁+a₂+a₃+a₄ + 2·m`. Caller checks `2·den_deg > effective_x2`.
fn four_sqrt_twenty_seven_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 4 || log_count != 27 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2]
        + sqrt_degs_x2[3] + 2 * poly_deg)
}
...
fn five_sqrt_twenty_seven_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 5 || log_count != 27 { return None; }
    Some(sqrt_degs_x2[0] + sqrt_degs_x2[1] + sqrt_degs_x2[2]
        + sqrt_degs_x2[3] + sqrt_degs_x2[4] + 2 * poly_deg)
}
...
fn g_vanishes_at_infinity(g: &IRNode, k: &IRNode) -> bool {
    ...
    if let Some(s5l63_x2) = five_sqrt_sixty_three_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l63) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l63 > s5l63_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 429: Mul(Sqrt(P1)×4, Log(h1)×63, ...) numerator.
    if let Some(s4l63_x2) = four_sqrt_sixty_three_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l63) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l63 > s4l63_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 428: Mul(Sqrt(P1)×3, Log(h1)×63, ...) numerator.
    if let Some(s3l63_x2) = three_sqrt_sixty_three_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l63) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l63 > s3l63_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 427: Mul(Sqrt(P), Sqrt(P2), Log(h1)×63, ...) numerator.
    if let Some(s2l63_x2) = two_sqrt_sixty_three_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l63) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l63 > s2l63_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 426: Mul(Sqrt(P), Log(h1)×63, ...) numerator.
    if let Some(s1l63_x2) = one_sqrt_sixty_three_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l63) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l63 > s1l63_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 425: Mul(Log(h1)×63, ...) numerator.
    if let Some(sl63_x2) = sixty_three_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl63) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl63 > sl63_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 424: Mul(Sqrt(P1)×5, Log(h1)×62, ...) numerator.
    if let Some(s5l62_x2) = five_sqrt_sixty_two_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l62) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l62 > s5l62_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 423: Mul(Sqrt(P1)×4, Log(h1)×62, ...) numerator.
    if let Some(s4l62_x2) = four_sqrt_sixty_two_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l62) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l62 > s4l62_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 422: Mul(Sqrt(P1)×3, Log(h1)×62, ...) numerator.
    if let Some(s3l62_x2) = three_sqrt_sixty_two_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l62) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l62 > s3l62_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 421: Mul(Sqrt(P), Sqrt(P2), Log(h1)×62, ...) numerator.
    if let Some(s2l62_x2) = two_sqrt_sixty_two_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l62) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l62 > s2l62_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 420: Mul(Sqrt(P), Log(h1)×62, ...) numerator.
    if let Some(s1l62_x2) = one_sqrt_sixty_two_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l62) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l62 > s1l62_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 419: Mul(Log(h1)×62, ...) numerator.
    if let Some(sl62_x2) = sixty_two_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl62) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl62 > sl62_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    ...
    if let Some(s5l60_x2) = five_sqrt_sixty_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l60) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l60 > s5l60_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 411: Mul(Sqrt(P1)×4, Log(h1)×60, ...) numerator.
    if let Some(s4l60_x2) = four_sqrt_sixty_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l60) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l60 > s4l60_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 410: Mul(Sqrt(P1)×3, Log(h1)×60, ...) numerator.
    if let Some(s3l60_x2) = three_sqrt_sixty_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l60) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l60 > s3l60_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 409: Mul(Sqrt(P), Sqrt(P2), Log(h1)×60, ...) numerator.
    if let Some(s2l60_x2) = two_sqrt_sixty_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l60) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l60 > s2l60_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 408: Mul(Sqrt(P), Log(h1)×60, ...) numerator.
    if let Some(s1l60_x2) = one_sqrt_sixty_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l60) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l60 > s1l60_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 407: Mul(Log(h1)×60, ...) numerator.
    if let Some(sl60_x2) = sixty_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl60) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl60 > sl60_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    ...
    if let Some(s5l58_x2) = five_sqrt_fifty_eight_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l58) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l58 > s5l58_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 399: Mul(Sqrt(P1)×4, Log(h1)×58, ...) numerator.
    if let Some(s4l58_x2) = four_sqrt_fifty_eight_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l58) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l58 > s4l58_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 398: Mul(Sqrt(P1)×3, Log(h1)×58, ...) numerator.
    if let Some(s3l58_x2) = three_sqrt_fifty_eight_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l58) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l58 > s3l58_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 397: Mul(Sqrt(P), Sqrt(P2), Log(h1)×58, ...) numerator.
    if let Some(s2l58_x2) = two_sqrt_fifty_eight_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l58) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l58 > s2l58_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 396: Mul(Sqrt(P), Log(h1)×58, ...) numerator.
    if let Some(s1l58_x2) = one_sqrt_fifty_eight_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l58) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l58 > s1l58_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 395: Mul(Log(h1)×58, ...) numerator.
    if let Some(sl58_x2) = fifty_eight_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl58) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl58 > sl58_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 394: Mul(Sqrt(P1)×5, Log(h1)×57, ...) numerator.
    if let Some(s5l57_x2) = five_sqrt_fifty_seven_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l57) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l57 > s5l57_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 393: Mul(Sqrt(P1)×4, Log(h1)×57, ...) numerator.
    if let Some(s4l57_x2) = four_sqrt_fifty_seven_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l57) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l57 > s4l57_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 392: Mul(Sqrt(P1)×3, Log(h1)×57, ...) numerator.
    if let Some(s3l57_x2) = three_sqrt_fifty_seven_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l57) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l57 > s3l57_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 391: Mul(Sqrt(P), Sqrt(P2), Log(h1)×57, ...) numerator.
    if let Some(s2l57_x2) = two_sqrt_fifty_seven_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l57) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l57 > s2l57_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 390: Mul(Sqrt(P), Log(h1)×57, ...) numerator.
    if let Some(s1l57_x2) = one_sqrt_fifty_seven_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l57) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l57 > s1l57_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 389: Mul(Log(h1)×57, ...) numerator.
    if let Some(sl57_x2) = fifty_seven_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl57) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl57 > sl57_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 388: Mul(Sqrt(P1)×5, Log(h1)×56, ...) numerator.
    if let Some(s5l56_x2) = five_sqrt_fifty_six_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l56) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l56 > s5l56_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 387: Mul(Sqrt(P1)×4, Log(h1)×56, ...) numerator.
    if let Some(s4l56_x2) = four_sqrt_fifty_six_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l56) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l56 > s4l56_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 386: Mul(Sqrt(P1)×3, Log(h1)×56, ...) numerator.
    if let Some(s3l56_x2) = three_sqrt_fifty_six_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l56) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l56 > s3l56_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 385: Mul(Sqrt(P), Sqrt(P2), Log(h1)×56, ...) numerator.
    if let Some(s2l56_x2) = two_sqrt_fifty_six_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l56) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l56 > s2l56_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 384: Mul(Sqrt(P), Log(h1)×56, ...) numerator.
    if let Some(s1l56_x2) = one_sqrt_fifty_six_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l56) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l56 > s1l56_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 383: Mul(Log(h1)×56, ...) numerator.
    if let Some(sl56_x2) = fifty_six_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl56) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl56 > sl56_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 382: Mul(Sqrt(P1)×5, Log(h1)×55, ...) numerator.
    if let Some(s5l55_x2) = five_sqrt_fifty_five_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l55) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l55 > s5l55_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 381: Mul(Sqrt(P1)×4, Log(h1)×55, ...) numerator.
    if let Some(s4l55_x2) = four_sqrt_fifty_five_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l55) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l55 > s4l55_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 380: Mul(Sqrt(P1)×3, Log(h1)×55, ...) numerator.
    if let Some(s3l55_x2) = three_sqrt_fifty_five_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l55) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l55 > s3l55_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 379: Mul(Sqrt(P), Sqrt(P2), Log(h1)×55, ...) numerator.
    if let Some(s2l55_x2) = two_sqrt_fifty_five_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l55) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l55 > s2l55_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 378: Mul(Sqrt(P), Log(h1)×55, ...) numerator.
    if let Some(s1l55_x2) = one_sqrt_fifty_five_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l55) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l55 > s1l55_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 377: Mul(Log(h1)×55, ...) numerator.
    if let Some(sl55_x2) = fifty_five_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl55) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl55 > sl55_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 376: Mul(Sqrt(P1)×5, Log(h1)×54, ...) numerator.
    if let Some(s5l54_x2) = five_sqrt_fifty_four_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l54) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l54 > s5l54_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 375: Mul(Sqrt(P1)×4, Log(h1)×54, ...) numerator.
    if let Some(s4l54_x2) = four_sqrt_fifty_four_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l54) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l54 > s4l54_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 374: Mul(Sqrt(P1)×3, Log(h1)×54, ...) numerator.
    if let Some(s3l54_x2) = three_sqrt_fifty_four_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l54) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l54 > s3l54_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 373: Mul(Sqrt(P), Sqrt(P2), Log(h1)×54, ...) numerator.
    if let Some(s2l54_x2) = two_sqrt_fifty_four_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l54) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l54 > s2l54_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 372: Mul(Sqrt(P), Log(h1)×54, ...) numerator.
    if let Some(s1l54_x2) = one_sqrt_fifty_four_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l54) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l54 > s1l54_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 371: Mul(Log(h1)×54, ...) numerator.
    if let Some(sl54_x2) = fifty_four_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl54) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl54 > sl54_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    ...
    if let Some(s5l51_x2) = five_sqrt_fifty_one_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l51) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l51 > s5l51_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 357: Mul(Sqrt(P1)×4, Log(h1)×51, ...) numerator.
    if let Some(s4l51_x2) = four_sqrt_fifty_one_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l51) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l51 > s4l51_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 356: Mul(Sqrt(P1)×3, Log(h1)×51, ...) numerator.
    if let Some(s3l51_x2) = three_sqrt_fifty_one_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l51) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l51 > s3l51_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 355: Mul(Sqrt(P), Sqrt(P2), Log(h1)×51, ...) numerator.
    if let Some(s2l51_x2) = two_sqrt_fifty_one_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l51) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l51 > s2l51_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 354: Mul(Sqrt(P), Log(h1)×51, ...) numerator.
    if let Some(s1l51_x2) = one_sqrt_fifty_one_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l51) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l51 > s1l51_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 353: Mul(Log(h1)×51, ...) numerator.
    if let Some(sl51_x2) = fifty_one_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl51) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl51 > sl51_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 352: Mul(Sqrt(P1)×5, Log(h1)×50, ...) numerator.
    if let Some(s5l50_x2) = five_sqrt_fifty_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l50) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l50 > s5l50_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 351: Mul(Sqrt(P1)×4, Log(h1)×50, ...) numerator.
    if let Some(s4l50_x2) = four_sqrt_fifty_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l50) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l50 > s4l50_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 350: Mul(Sqrt(P1)×3, Log(h1)×50, ...) numerator.
    if let Some(s3l50_x2) = three_sqrt_fifty_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l50) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l50 > s3l50_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 349: Mul(Sqrt(P), Sqrt(P2), Log(h1)×50, ...) numerator.
    if let Some(s2l50_x2) = two_sqrt_fifty_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l50) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l50 > s2l50_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 348: Mul(Sqrt(P), Log(h1)×50, ...) numerator.
    if let Some(s1l50_x2) = one_sqrt_fifty_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l50) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l50 > s1l50_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 347: Mul(Log(h1)×50, ...) numerator.
    if let Some(sl50_x2) = fifty_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl50) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl50 > sl50_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 346: Mul(Sqrt(P1)×5, Log(h1)×49, ...) numerator.
    if let Some(s5l49_x2) = five_sqrt_forty_nine_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l49) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l49 > s5l49_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 345: Mul(Sqrt(P1)×4, Log(h1)×49, ...) numerator.
    if let Some(s4l49_x2) = four_sqrt_forty_nine_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l49) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l49 > s4l49_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 344: Mul(Sqrt(P1)×3, Log(h1)×49, ...) numerator.
    if let Some(s3l49_x2) = three_sqrt_forty_nine_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l49) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l49 > s3l49_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 343: Mul(Sqrt(P), Sqrt(P2), Log(h1)×49, ...) numerator.
    if let Some(s2l49_x2) = two_sqrt_forty_nine_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l49) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l49 > s2l49_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 342: Mul(Sqrt(P), Log(h1)×49, ...) numerator.
    if let Some(s1l49_x2) = one_sqrt_forty_nine_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l49) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l49 > s1l49_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 341: Mul(Log(h1)×49, ...) numerator.
    if let Some(sl49_x2) = forty_nine_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl49) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl49 > sl49_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 340: Mul(Sqrt(P1)×5, Log(h1)×48, ...) numerator.
    if let Some(s5l48_x2) = five_sqrt_forty_eight_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l48) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l48 > s5l48_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 339: Mul(Sqrt(P1)×4, Log(h1)×48, ...) numerator.
    if let Some(s4l48_x2) = four_sqrt_forty_eight_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l48) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l48 > s4l48_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 338: Mul(Sqrt(P1)×3, Log(h1)×48, ...) numerator.
    if let Some(s3l48_x2) = three_sqrt_forty_eight_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l48) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l48 > s3l48_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 337: Mul(Sqrt(P), Sqrt(P2), Log(h1)×48, ...) numerator.
    if let Some(s2l48_x2) = two_sqrt_forty_eight_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l48) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l48 > s2l48_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 336: Mul(Sqrt(P), Log(h1)×48, ...) numerator.
    if let Some(s1l48_x2) = one_sqrt_forty_eight_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l48) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l48 > s1l48_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 335: Mul(Log(h1)×48, ...) numerator.
    if let Some(sl48_x2) = forty_eight_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl48) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl48 > sl48_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 334: Mul(Sqrt(P1)×5, Log(h1)×47, ...) numerator.
    if let Some(s5l47_x2) = five_sqrt_forty_seven_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l47) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l47 > s5l47_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 333: Mul(Sqrt(P1)×4, Log(h1)×47, ...) numerator.
    if let Some(s4l47_x2) = four_sqrt_forty_seven_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l47) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l47 > s4l47_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 332: Mul(Sqrt(P1)×3, Log(h1)×47, ...) numerator.
    if let Some(s3l47_x2) = three_sqrt_forty_seven_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l47) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l47 > s3l47_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 331: Mul(Sqrt(P), Sqrt(P2), Log(h1)×47, ...) numerator.
    if let Some(s2l47_x2) = two_sqrt_forty_seven_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l47) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l47 > s2l47_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 330: Mul(Sqrt(P), Log(h1)×47, ...) numerator.
    if let Some(s1l47_x2) = one_sqrt_forty_seven_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l47) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l47 > s1l47_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 329: Mul(Log(h1)×47, ...) numerator.
    if let Some(sl47_x2) = forty_seven_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl47) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl47 > sl47_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 328: Mul(Sqrt(P1)×5, Log(h1)×46, ...) numerator.
    if let Some(s5l46_x2) = five_sqrt_forty_six_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l46) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l46 > s5l46_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 327: Mul(Sqrt(P1)×4, Log(h1)×46, ...) numerator.
    if let Some(s4l46_x2) = four_sqrt_forty_six_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l46) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l46 > s4l46_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 326: Mul(Sqrt(P1)×3, Log(h1)×46, ...) numerator.
    if let Some(s3l46_x2) = three_sqrt_forty_six_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l46) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l46 > s3l46_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 325: Mul(Sqrt(P), Sqrt(P2), Log(h1)×46, ...) numerator.
    if let Some(s2l46_x2) = two_sqrt_forty_six_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l46) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l46 > s2l46_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 324: Mul(Sqrt(P), Log(h1)×46, ...) numerator.
    if let Some(s1l46_x2) = one_sqrt_forty_six_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l46) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l46 > s1l46_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 323: Mul(Log(h1)×46, ...) numerator.
    if let Some(sl46_x2) = forty_six_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl46) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl46 > sl46_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 322: Mul(Sqrt(P1)×5, Log(h1)×45, ...) numerator.
    if let Some(s5l45_x2) = five_sqrt_forty_five_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l45) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l45 > s5l45_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 321: Mul(Sqrt(P1)×4, Log(h1)×45, ...) numerator.
    if let Some(s4l45_x2) = four_sqrt_forty_five_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l45) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l45 > s4l45_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 320: Mul(Sqrt(P1)×3, Log(h1)×45, ...) numerator.
    if let Some(s3l45_x2) = three_sqrt_forty_five_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l45) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l45 > s3l45_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 319: Mul(Sqrt(P), Sqrt(P2), Log(h1)×45, ...) numerator.
    if let Some(s2l45_x2) = two_sqrt_forty_five_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l45) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l45 > s2l45_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 318: Mul(Sqrt(P), Log(h1)×45, ...) numerator.
    if let Some(s1l45_x2) = one_sqrt_forty_five_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l45) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l45 > s1l45_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 317: Mul(Log(h1)×45, ...) numerator.
    if let Some(sl45_x2) = forty_five_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl45) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl45 > sl45_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 316: Mul(Sqrt(P1)×5, Log(h1)×44, ...) numerator.
    if let Some(s5l44_x2) = five_sqrt_forty_four_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l44) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l44 > s5l44_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 315: Mul(Sqrt(P1)×4, Log(h1)×44, ...) numerator.
    if let Some(s4l44_x2) = four_sqrt_forty_four_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l44) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l44 > s4l44_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 314: Mul(Sqrt(P1)×3, Log(h1)×44, ...) numerator.
    if let Some(s3l44_x2) = three_sqrt_forty_four_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l44) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l44 > s3l44_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 313: Mul(Sqrt(P), Sqrt(P2), Log(h1)×44, ...) numerator.
    if let Some(s2l44_x2) = two_sqrt_forty_four_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l44) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l44 > s2l44_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 312: Mul(Sqrt(P), Log(h1)×44, ...) numerator.
    if let Some(s1l44_x2) = one_sqrt_forty_four_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l44) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l44 > s1l44_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 311: Mul(Log(h1)×44, ...) numerator.
    if let Some(sl44_x2) = forty_four_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl44) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl44 > sl44_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    ...
    if let Some(s5l42_x2) = five_sqrt_forty_two_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l42) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l42 > s5l42_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 303: Mul(Sqrt(P1)×4, Log(h1)×42, ...) numerator.
    if let Some(s4l42_x2) = four_sqrt_forty_two_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l42) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l42 > s4l42_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 302: Mul(Sqrt(P1)×3, Log(h1)×42, ...) numerator.
    if let Some(s3l42_x2) = three_sqrt_forty_two_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l42) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l42 > s3l42_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 301: Mul(Sqrt(P), Sqrt(P2), Log(h1)×42, ...) numerator.
    if let Some(s2l42_x2) = two_sqrt_forty_two_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l42) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l42 > s2l42_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 300: Mul(Sqrt(P), Log(h1)×42, ...) numerator.
    if let Some(s1l42_x2) = one_sqrt_forty_two_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l42) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l42 > s1l42_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 299: Mul(Log(h1)×42, ...) numerator.
    if let Some(sl42_x2) = forty_two_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl42) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl42 > sl42_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 298: Mul(Sqrt(P1)×5, Log(h1)×41, ...) numerator.
    if let Some(s5l41_x2) = five_sqrt_forty_one_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l41) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l41 > s5l41_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 297: Mul(Sqrt(P1)×4, Log(h1)×41, ...) numerator.
    if let Some(s4l41_x2) = four_sqrt_forty_one_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l41) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l41 > s4l41_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 296: Mul(Sqrt(P1)×3, Log(h1)×41, ...) numerator.
    if let Some(s3l41_x2) = three_sqrt_forty_one_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l41) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l41 > s3l41_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 295: Mul(Sqrt(P), Sqrt(P2), Log(h1)×41, ...) numerator.
    if let Some(s2l41_x2) = two_sqrt_forty_one_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l41) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l41 > s2l41_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 294: Mul(Sqrt(P), Log(h1)×41, ...) numerator.
    if let Some(s1l41_x2) = one_sqrt_forty_one_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l41) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l41 > s1l41_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 293: Mul(Log(h1)×41, ...) numerator.
    if let Some(sl41_x2) = forty_one_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl41) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl41 > sl41_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 292: Mul(Sqrt(P1)×5, Log(h1)×40, ...) numerator.
    if let Some(s5l40_x2) = five_sqrt_forty_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l40) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l40 > s5l40_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 291: Mul(Sqrt(P1)×4, Log(h1)×40, ...) numerator.
    if let Some(s4l40_x2) = four_sqrt_forty_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l40) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l40 > s4l40_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 290: Mul(Sqrt(P1)×3, Log(h1)×40, ...) numerator.
    if let Some(s3l40_x2) = three_sqrt_forty_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l40) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l40 > s3l40_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 289: Mul(Sqrt(P), Sqrt(P2), Log(h1)×40, ...) numerator.
    if let Some(s2l40_x2) = two_sqrt_forty_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l40) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l40 > s2l40_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 288: Mul(Sqrt(P), Log(h1)×40, ...) numerator.
    if let Some(s1l40_x2) = one_sqrt_forty_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l40) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l40 > s1l40_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 287: Mul(Log(h1)×40, ...) numerator.
    if let Some(sl40_x2) = forty_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl40) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl40 > sl40_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 286: Mul(Sqrt(P1)×5, Log(h1)×39, ...) numerator.
    if let Some(s5l39_x2) = five_sqrt_thirty_nine_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l39) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l39 > s5l39_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 285: Mul(Sqrt(P1)×4, Log(h1)×39, ...) numerator.
    if let Some(s4l39_x2) = four_sqrt_thirty_nine_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l39) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l39 > s4l39_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 284: Mul(Sqrt(P1)×3, Log(h1)×39, ...) numerator.
    if let Some(s3l39_x2) = three_sqrt_thirty_nine_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l39) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l39 > s3l39_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 283: Mul(Sqrt(P), Sqrt(P2), Log(h1)×39, ...) numerator.
    if let Some(s2l39_x2) = two_sqrt_thirty_nine_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l39) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l39 > s2l39_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 282: Mul(Sqrt(P), Log(h1)×39, ...) numerator.
    if let Some(s1l39_x2) = one_sqrt_thirty_nine_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l39) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l39 > s1l39_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 281: Mul(Log(h1)×39, ...) numerator.
    if let Some(sl39_x2) = thirty_nine_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl39) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl39 > sl39_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 280: Mul(Sqrt(P1)×5, Log(h1)×38, ...) numerator.
    if let Some(s5l38_x2) = five_sqrt_thirty_eight_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l38) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l38 > s5l38_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 279: Mul(Sqrt(P1)×4, Log(h1)×38, ...) numerator.
    if let Some(s4l38_x2) = four_sqrt_thirty_eight_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l38) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l38 > s4l38_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 278: Mul(Sqrt(P1)×3, Log(h1)×38, ...) numerator.
    if let Some(s3l38_x2) = three_sqrt_thirty_eight_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l38) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l38 > s3l38_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 277: Mul(Sqrt(P), Sqrt(P2), Log(h1)×38, ...) numerator.
    if let Some(s2l38_x2) = two_sqrt_thirty_eight_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l38) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l38 > s2l38_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 276: Mul(Sqrt(P), Log(h1)×38, ...) numerator.
    if let Some(s1l38_x2) = one_sqrt_thirty_eight_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l38) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l38 > s1l38_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 275: Mul(Log(h1)×38, ...) numerator.
    if let Some(sl38_x2) = thirty_eight_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl38) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl38 > sl38_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    ...
    if let Some(s5l36_x2) = five_sqrt_thirty_six_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l36) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l36 > s5l36_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 267: Mul(Sqrt(P1)×4, Log(h1)×36, ...) numerator.
    if let Some(s4l36_x2) = four_sqrt_thirty_six_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l36) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l36 > s4l36_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 266: Mul(Sqrt(P1)×3, Log(h1)×36, ...) numerator.
    if let Some(s3l36_x2) = three_sqrt_thirty_six_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l36) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l36 > s3l36_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 265: Mul(Sqrt(P), Sqrt(P2), Log(h1)×36, ...) numerator.
    if let Some(s2l36_x2) = two_sqrt_thirty_six_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l36) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l36 > s2l36_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 264: Mul(Sqrt(P), Log(h1)×36, ...) numerator.
    if let Some(s1l36_x2) = one_sqrt_thirty_six_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l36) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l36 > s1l36_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 263: Mul(Log(h1)×36, ...) numerator.
    if let Some(sl36_x2) = thirty_six_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl36) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl36 > sl36_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 262: Mul(Sqrt(P1)×5, Log(h1)×35, ...) numerator.
    if let Some(s5l35_x2) = five_sqrt_thirty_five_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l35) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l35 > s5l35_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 261: Mul(Sqrt(P1)×4, Log(h1)×35, ...) numerator.
    if let Some(s4l35_x2) = four_sqrt_thirty_five_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l35) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l35 > s4l35_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 260: Mul(Sqrt(P1)×3, Log(h1)×35, ...) numerator.
    if let Some(s3l35_x2) = three_sqrt_thirty_five_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l35) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l35 > s3l35_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 259: Mul(Sqrt(P), Sqrt(P2), Log(h1)×35, ...) numerator.
    if let Some(s2l35_x2) = two_sqrt_thirty_five_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l35) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l35 > s2l35_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 258: Mul(Sqrt(P), Log(h1)×35, ...) numerator.
    if let Some(s1l35_x2) = one_sqrt_thirty_five_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l35) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l35 > s1l35_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 257: Mul(Log(h1)×35, ...) numerator.
    if let Some(sl35_x2) = thirty_five_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl35) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl35 > sl35_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 256: Mul(Sqrt(P1)×5, Log(h1)×34, ...) numerator.
    if let Some(s5l34_x2) = five_sqrt_thirty_four_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l34) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l34 > s5l34_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 255: Mul(Sqrt(P1)×4, Log(h1)×34, ...) numerator.
    if let Some(s4l34_x2) = four_sqrt_thirty_four_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l34) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l34 > s4l34_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 254: Mul(Sqrt(P1)×3, Log(h1)×34, ...) numerator.
    if let Some(s3l34_x2) = three_sqrt_thirty_four_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l34) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l34 > s3l34_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 253: Mul(Sqrt(P), Sqrt(P2), Log(h1)×34, ...) numerator.
    if let Some(s2l34_x2) = two_sqrt_thirty_four_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l34) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l34 > s2l34_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 252: Mul(Sqrt(P), Log(h1)×34, ...) numerator.
    if let Some(s1l34_x2) = one_sqrt_thirty_four_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l34) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l34 > s1l34_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 251: Mul(Log(h1)×34, ...) numerator.
    if let Some(sl34_x2) = thirty_four_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl34) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl34 > sl34_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 250: Mul(Sqrt(P1)×5, Log(h1)×33, ...) numerator.
    if let Some(s5l33_x2) = five_sqrt_thirty_three_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l33) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l33 > s5l33_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 249: Mul(Sqrt(P1)×4, Log(h1)×33, ...) numerator.
    if let Some(s4l33_x2) = four_sqrt_thirty_three_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l33) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l33 > s4l33_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 248: Mul(Sqrt(P1)×3, Log(h1)×33, ...) numerator.
    if let Some(s3l33_x2) = three_sqrt_thirty_three_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l33) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l33 > s3l33_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 247: Mul(Sqrt(P), Sqrt(P2), Log(h1)×33, ...) numerator.
    if let Some(s2l33_x2) = two_sqrt_thirty_three_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l33) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l33 > s2l33_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 246: Mul(Sqrt(P), Log(h1)×33, ...) numerator.
    if let Some(s1l33_x2) = one_sqrt_thirty_three_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l33) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l33 > s1l33_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 245: Mul(Log(h1)×33, ...) numerator.
    if let Some(sl33_x2) = thirty_three_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl33) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl33 > sl33_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 244: Mul(Sqrt(P1)×5, Log(h1)×32, ...) numerator.
    if let Some(s5l32_x2) = five_sqrt_thirty_two_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l32) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l32 > s5l32_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 243: Mul(Sqrt(P1)×4, Log(h1)×32, ...) numerator.
    if let Some(s4l32_x2) = four_sqrt_thirty_two_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l32) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l32 > s4l32_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 242: Mul(Sqrt(P1)×3, Log(h1)×32, ...) numerator.
    if let Some(s3l32_x2) = three_sqrt_thirty_two_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l32) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l32 > s3l32_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 241: Mul(Sqrt(P), Sqrt(P2), Log(h1)×32, ...) numerator.
    if let Some(s2l32_x2) = two_sqrt_thirty_two_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l32) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l32 > s2l32_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 240: Mul(Sqrt(P), Log(h1)×32, ...) numerator.
    if let Some(s1l32_x2) = one_sqrt_thirty_two_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l32) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l32 > s1l32_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 239: Mul(Log(h1)×32, ...) numerator.
    if let Some(sl32_x2) = thirty_two_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl32) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl32 > sl32_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 238: Mul(Sqrt(P1)×5, Log(h1)×31, ...) numerator.
    if let Some(s5l31_x2) = five_sqrt_thirty_one_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l31) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l31 > s5l31_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 237: Mul(Sqrt(P1)×4, Log(h1)×31, ...) numerator.
    if let Some(s4l31_x2) = four_sqrt_thirty_one_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l31) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l31 > s4l31_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 236: Mul(Sqrt(P1)×3, Log(h1)×31, ...) numerator.
    if let Some(s3l31_x2) = three_sqrt_thirty_one_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l31) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l31 > s3l31_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 235: Mul(Sqrt(P), Sqrt(P2), Log(h1)×31, ...) numerator.
    if let Some(s2l31_x2) = two_sqrt_thirty_one_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l31) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l31 > s2l31_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 234: Mul(Sqrt(P), Log(h1)×31, ...) numerator.
    if let Some(s1l31_x2) = one_sqrt_thirty_one_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l31) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l31 > s1l31_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 233: Mul(Log(h1)×31, ...) numerator.
    if let Some(sl31_x2) = thirty_one_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl31) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl31 > sl31_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 232: Mul(Sqrt(P1)×5, Log(h1)×30, ...) numerator.
    if let Some(s5l30_x2) = five_sqrt_thirty_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l30) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l30 > s5l30_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 231: Mul(Sqrt(P1)×4, Log(h1)×30, ...) numerator.
    if let Some(s4l30_x2) = four_sqrt_thirty_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l30) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l30 > s4l30_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 230: Mul(Sqrt(P1)×3, Log(h1)×30, ...) numerator.
    if let Some(s3l30_x2) = three_sqrt_thirty_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l30) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l30 > s3l30_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 229: Mul(Sqrt(P), Sqrt(P2), Log(h1)×30, ...) numerator.
    if let Some(s2l30_x2) = two_sqrt_thirty_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l30) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l30 > s2l30_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 228: Mul(Sqrt(P), Log(h1)×30, ...) numerator.
    if let Some(s1l30_x2) = one_sqrt_thirty_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l30) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l30 > s1l30_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 227: Mul(Log(h1)×30, ...) numerator.
    if let Some(sl30_x2) = thirty_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl30) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl30 > sl30_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 226: Mul(Sqrt(P1)×5, Log(h1)×29, ...) numerator.
    if let Some(s5l29_x2) = five_sqrt_twenty_nine_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l29) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l29 > s5l29_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 225: Mul(Sqrt(P1)×4, Log(h1)×29, ...) numerator.
    if let Some(s4l29_x2) = four_sqrt_twenty_nine_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l29) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l29 > s4l29_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 224: Mul(Sqrt(P1)×3, Log(h1)×29, ...) numerator.
    if let Some(s3l29_x2) = three_sqrt_twenty_nine_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l29) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l29 > s3l29_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 223: Mul(Sqrt(P), Sqrt(P2), Log(h1)×29, ...) numerator.
    if let Some(s2l29_x2) = two_sqrt_twenty_nine_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l29) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l29 > s2l29_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 222: Mul(Sqrt(P), Log(h1)×29, ...) numerator.
    if let Some(s1l29_x2) = one_sqrt_twenty_nine_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l29) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l29 > s1l29_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 221: Mul(Log(h1)×29, ...) numerator.
    if let Some(sl29_x2) = twenty_nine_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl29) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl29 > sl29_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 220: Mul(Sqrt(P1)×5, Log(h1)×28, ...) numerator.
    if let Some(s5l28_x2) = five_sqrt_twenty_eight_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l28) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l28 > s5l28_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 219: Mul(Sqrt(P1)×4, Log(h1)×28, ...) numerator.
    if let Some(s4l28_x2) = four_sqrt_twenty_eight_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l28) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l28 > s4l28_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 218: Mul(Sqrt(P1)×3, Log(h1)×28, ...) numerator.
    if let Some(s3l28_x2) = three_sqrt_twenty_eight_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l28) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l28 > s3l28_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 217: Mul(Sqrt(P), Sqrt(P2), Log(h1)×28, ...) numerator.
    if let Some(s2l28_x2) = two_sqrt_twenty_eight_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l28) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l28 > s2l28_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 216: Mul(Sqrt(P), Log(h1)×28, ...) numerator.
    if let Some(s1l28_x2) = one_sqrt_twenty_eight_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l28) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l28 > s1l28_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 215: Mul(Log(h1)×28, ...) numerator.
    if let Some(sl28_x2) = twenty_eight_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl28) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl28 > sl28_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 214: Mul(Sqrt(P1)×5, Log(h1)×27, ...) numerator.
    if let Some(s5l27_x2) = five_sqrt_twenty_seven_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l27) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l27 > s5l27_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 213: Mul(Sqrt(P1)×4, Log(h1)×27, ...) numerator.
    if let Some(s4l27_x2) = four_sqrt_twenty_seven_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l27) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l27 > s4l27_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 212: Mul(Sqrt(P1)×3, Log(h1)×27, ...) numerator.
    if let Some(s3l27_x2) = three_sqrt_twenty_seven_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l27) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l27 > s3l27_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 211: Mul(Sqrt(P), Sqrt(P2), Log(h1)×27, ...) numerator.
    if let Some(s2l27_x2) = two_sqrt_twenty_seven_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l27) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l27 > s2l27_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 210: Mul(Sqrt(P), Log(h1)×27, ...) numerator.
    if let Some(s1l27_x2) = one_sqrt_twenty_seven_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l27) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l27 > s1l27_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 209: Mul(Log(h1)×27, ...) numerator.
    if let Some(sl27_x2) = twenty_seven_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl27) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl27 > sl27_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    ...
    if let Some(s5l25_x2) = five_sqrt_twenty_five_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l25) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l25 > s5l25_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 201: Mul(Sqrt(P1)×4, Log(h1)×25, ...) numerator.
    if let Some(s4l25_x2) = four_sqrt_twenty_five_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l25) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l25 > s4l25_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 200: Mul(Sqrt(P1)×3, Log(h1)×25, ...) numerator.
    if let Some(s3l25_x2) = three_sqrt_twenty_five_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l25) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l25 > s3l25_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 199: Mul(Sqrt(P), Sqrt(P2), Log(h1)×25, ...) numerator.
    if let Some(s2l25_x2) = two_sqrt_twenty_five_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l25) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l25 > s2l25_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 198: Mul(Sqrt(P), Log(h1)×25, ...) numerator.
    if let Some(s1l25_x2) = one_sqrt_twenty_five_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l25) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l25 > s1l25_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 197: Mul(Log(h1)×25, ...) numerator.
    if let Some(sl25_x2) = twenty_five_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl25) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl25 > sl25_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 196: Mul(Sqrt(P1)×5, Log(h1)×24, ...) numerator.
    if let Some(s5l24_x2) = five_sqrt_twenty_four_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l24) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l24 > s5l24_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 195: Mul(Sqrt(P1)×4, Log(h1)×24, ...) numerator.
    if let Some(s4l24_x2) = four_sqrt_twenty_four_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l24) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l24 > s4l24_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 194: Mul(Sqrt(P1)×3, Log(h1)×24, ...) numerator.
    if let Some(s3l24_x2) = three_sqrt_twenty_four_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s3l24) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s3l24 > s3l24_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 193: Mul(Sqrt(P), Sqrt(P2), Log(h1)×24, ...) numerator.
    if let Some(s2l24_x2) = two_sqrt_twenty_four_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s2l24) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s2l24 > s2l24_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 192: Mul(Sqrt(P), Log(h1)×24, ...) numerator.
    if let Some(s1l24_x2) = one_sqrt_twenty_four_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s1l24) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s1l24 > s1l24_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
    // Phase 191: Mul(Log(h1)×24, ...) numerator.
    if let Some(sl24_x2) = twenty_four_log_poly_effective_x2(num, k) {
        if let Some(den_deg_sl24) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_sl24 > sl24_x2 { return true; }
        } else if h_diverges_at_infinity(den, k) { return true; }
    }
