...
const PROGRAMS: &[Prog] = &[
    ...
    // ALGOL 60 — checked local integer arithmetic snapshots may widen into
    ...
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer n, saved; n := (8 - 1) * 6; saved := n; n := 9; output(saved + 0.5) end",
...
#[test]
fn algol_static_integer_arithmetic_widening_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("n := (8 - 1) * 6; saved := n; n := 9; output(saved + 0.5)")
        })
        .expect("the static integer-arithmetic program must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but static integer-arithmetic output did not complete"
            );
            continue;
        };
        assert_cell(backend, program, result);
