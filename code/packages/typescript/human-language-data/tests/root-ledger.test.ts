...
describe("the committed corpus", () => {
  ...
  it("pins the first ledger, and it is the reason the rule exists", () => {
    ...
    /* Superseded French-only totals:
    expect(l.summary.roots).toBe(3083); // Italian, French, and Portuguese typed openers each record an etymon atom.
    expect(l.summary.underspent).toBe(2968); // All three opener etymons begin below the reuse floor.
    */
    expect(l.summary.roots).toBe(3085); // French adds one etymon to merged Arabic and Portuguese roots.
    expect(l.summary.underspent).toBe(2970); // The French opener etymon begins below the reuse floor.
    ...
    */
    ...
    /* Superseded French-only namespace total:
    expect(byNamespace).toEqual({ roots: 2194, "etymon-atom": 889 }); // +3: Italian, French, and Portuguese opener etymons.
    */
    expect(byNamespace).toEqual({ roots: 2196, "etymon-atom": 889 }); // French adds one etymon to merged Arabic and Portuguese entries.
