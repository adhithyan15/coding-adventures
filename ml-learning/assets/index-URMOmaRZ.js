...
`)}_emitBlockItem(e,t){return e.children.length>0?this._emitNode(e.children[0],t):``}_emitDeclarationOrNested(e,t){return e.children.length>0?this._emitNode(e.children[0],t):``}_emitDeclaration(e,t){let n=``,r=``,i=``;for(let t of e.children){if(!ch(t))continue;let e=t;e.ruleName===`property`?n=this._emitProperty(e,0):e.ruleName===`value_list`?r=this._emitValueList(e,0):e.ruleName===`priority`&&(i=` !important`)}return this.minified?`${n}:${r}${i};`:`${n}: ${r}${i};`}_emitProperty(e,t){return e.children.length>0?e.children[0].value:``}_emitPriority(e,t){return`!important`}_emitValueList(e,t){let n=[];for(let r of e.children){let e=this._emitNode(r,t);n.push(e)}let r=n.join(` `);return r=r.replace(/ , /g,`, `).replace(/ ,/g,`,`),r}_emitValue(e,t){let n=e.children;if(n.length===1){let e=n[0];if(!ch(e)){let t=e;return lh(t)===`STRING`?`"${t.value}"`:t.value}return this._emitNode(e,t)}return this._emitDefault(e,t)}_emitFunctionCall(e,t){let n=e.children;if(n.length===1)return n[0].value;let r=[];for(let e of n)if(ch(e))r.push(this._emitNode(e,t));else{let t=e,n=lh(t);n===`FUNCTION`?r.push(t.value):n===`RPAREN`?r.push(`)`):r.push(t.value)}return r.join(``)}_emitFunctionArgs(e,t){let n=[];for(let r of e.children)n.push(this._emitNode(r,t));let r=n.join(` `);return r=r.replace(/ , /g,`, `).replace(/ ,/g,`,`),r}_emitFunctionArg(e,t){let n=e.children;if(n.length===1){let e=n[0];return ch(e)?this._emitNode(e,t):e.value}let r=[];for(let e of n)if(ch(e))r.push(this._emitNode(e,t));else{let t=e;r.push(lh(t)===`RPAREN`?`)`:t.value)}return r.join(``)}_emitDefault(e,t){let n=[];for(let r of e.children)n.push(this._emitNode(r,t));return n.join(` `)}};function dh(e,t={}){let n=Cp(e),r=new sh().transform(n);return new uh(t.indent??`  `,t.minified??!1).emit(r)}function fh(e,t={}){return dh(e,t)}var ph=`$paper: #f7f8f3;
$ink: #17201c;
$muted: #5d6d68;
...
`,mh=`coding-adventures-lattice-styles`;function hh(){if(document.getElementById(mh)===null)try{let e=document.createElement(`style`);e.id=mh,e.textContent=fh(ph),document.head.append(e)}catch(e){console.error(`Failed to install Lattice styles`,e)}}hh(),(0,u.createRoot)(document.getElementById(`root`)).render((0,E.jsx)(l.StrictMode,{children:(0,E.jsx)(np,{})}));