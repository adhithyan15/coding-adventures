...
- **Status:** OPEN — discovered by CLOC14.36. `minify_unary_minus_paren` ignored.
- **Inputs:** `-(a)` → `-a`; `!(a)` → `!a`; `~(a)` → `~a`; and the same-sign case `-(-a)` → `- -a`, `+(+a)` → `+ +a`.
- **What it needs:** Strip the redundant grouping parens around the operand of a PREFIX unary SYMBOL operator (`-`/`+`/`!`/`~`) when the operand is a simple reference — exactly the `is_safe_unary_operand` machinery used for the keyword operators (gap-070/071), but anchored on the punctuation operator at `kept[i]` (`is_structural_punct`-gated) rather than a word-like keyword. **Same-sign caveat:** when the operand itself begins with the same sign (`-(-a)`), gluing the operators yields the spurious compound `--a` (decrement); a single separating space must be emitted (`- -a`). The existing gap-063 `needs_separator` same-sign rule already inserts that space once the parens are gone, so the elision pre-pass only needs to drop the parens. **Operator-vs-binary caveat:** `-`/`+` are also binary operators; the pre-pass must only fire when the `-`/`+` is genuinely PREFIX (preceded by another operator, `(`, `,`, `=`, `return`, statement start, …) — a binary `a-(b)` is a separate concern. Operator operands like `-(a+b)` keep their parens (`is_safe_unary_operand` rejects them).
