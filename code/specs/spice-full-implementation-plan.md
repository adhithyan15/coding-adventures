...
6. **Post-Berkeley product tracks** (not Berkeley completion blockers).
   ...
   - Result-table and source-span-diagnostic interaction is complete in PR
     #14652. This coherent UI phase delivers web waveform/axis interaction
     from the existing `BerkeleyAppWaveformSeries` contract: select a result
     series, normalize its adjacent-point line segments in the Rust app
...
500. Python and TypeScript parser/engine model-card mismatch closeout audit.
   - Status: completed, including the subsequent JFET `B` policy resolution.
...
501. Cross-language JFET `B` semantic resolution.
   - Status: implemented in the JFET `B` policy slice.
   - `B` lowers only into the Parker-Skellern doping-tail field, matching the
     engine and Berkeley/ngspice model definition. `BETA` and `BET` remain the
     explicit transconductance inputs, so one model-card value can never
     silently populate both engine fields.
   - All parser facades reject non-finite `B` values before lowering.

502. Rust JFET threshold-alias parity.
   - Status: implemented in the Rust JFET threshold-alias slice.
   - The Rust element lowerer gives canonical `VTO` precedence over `VT0` and
     `VTH`, matching engine normalization and the Python and TypeScript
     facades, and rejects non-finite threshold inputs before lowering.

503. Rust JFET channel-length-modulation alias parity.
   - Status: implemented in the Rust JFET channel-length-modulation slice.
   - The Rust element lowerer gives canonical `LAMBDA` precedence over `LAM`,
     matching the engine and Python and TypeScript facades, and rejects
     non-finite channel-length modulation inputs before lowering.

504. Rust JFET gate-capacitance alias parity.
   - Status: implemented in the Rust JFET gate-capacitance slice.
   - The Rust element lowerer gives canonical `CGS` / `CGD` precedence over
     `CGS0` / `CGD0`, passes the selected values through to the engine, and
     rejects negative or non-finite capacitance inputs before lowering.

505. Rust JFET flicker-noise parameter parity.
   - Status: implemented in the Rust JFET flicker-noise slice.
...
510. Rust JFET bandgap-voltage parser parity.
   - Status: implemented in the Rust JFET bandgap slice.
   - The Rust element lowerer passes valid `EG` values through to the engine
     and rejects values that are non-finite or non-positive before lowering.

511. Rust JFET noise-equation-level parser parity.
   - Status: implemented in the Rust JFET noise-level slice.
   - The Rust element lowerer passes valid integral `NLEV` values through to the
     engine and rejects values that are non-finite, below `1`, or fractional.

## Backlog

1. Python and TypeScript Berkeley SPICE model-card validation parity.
   - Status: completed by the JFET `B` policy slice.
   - `B` is now a Parker-Skellern doping-tail parameter in every parser facade;
...
3. Rust JFET `LAMBDA` / `LAM` parser parity.
   ...
   - Canonical `LAMBDA` now takes precedence over `LAM`, and both aliases are
     rejected when non-finite before element lowering.

4. Rust JFET capacitance-alias parser parity.
   - Status: completed by the gate-capacitance slice.
   - Canonical `CGS` / `CGD` now take precedence over `CGS0` / `CGD0`, and all
     aliases are rejected when negative or non-finite before element lowering.

5. Rust JFET flicker-noise parser parity.
   - Status: completed by the flicker-noise slice.
   - Finite, non-negative `KF` and `AF` values now lower into the engine JFET
...
10. Rust JFET bandgap-voltage parser parity.
   - Status: completed by the JFET bandgap slice.
   - Positive finite `EG` values now lower into the engine JFET field, while
     invalid inputs are rejected before element lowering.

11. Rust JFET noise-equation-level parser parity.
   - Status: completed by the JFET noise-level slice.
   - Integral `NLEV` values of at least `1` now lower into the engine JFET
     field, while invalid inputs are rejected before element lowering.

12. Rust JFET channel-noise-coefficient parser parity.
   - Status: completed by the JFET channel-noise slice.
   - Finite, non-negative `GDSNOI` values now lower into the engine JFET
     channel-noise-coefficient field, while invalid inputs are rejected before
     element lowering.

13. Rust JFET drain-resistance parser parity.
   - Status: completed by the JFET drain-resistance slice.
   - The engine and Python/TypeScript facades lower finite non-negative JFET
     `RD` values into the drain-resistance field; Rust now does the same and
     rejects invalid inputs before element lowering.

14. Rust JFET source-resistance parser parity.
   - Status: completed by the JFET source-resistance slice.
   - The engine and Python/TypeScript facades lower finite non-negative JFET
     `RS` values into the source-resistance field; Rust now does the same and
     rejects invalid inputs before element lowering.
...
16. Rust JFET alternative-threshold-temperature-coefficient parser parity.
   - Status: completed by the JFET alternative-threshold-temperature slice.
   - The engine and Python/TypeScript facades lower finite JFET `VTOTC` values
     ...
     Rust now does the same while preserving omitted `None` semantics and
     rejecting non-finite inputs before element lowering.

17. Rust JFET nominal-temperature parser parity.
   - Status: completed by the JFET nominal-temperature slice.
   - The engine and Python/TypeScript facades lower finite, positive JFET
     `TNOM` / `T_NOM` values into the nominal-temperature field; Rust now uses
     canonical `TNOM` precedence, converts Celsius to Kelvin, and rejects an
...
22. Rust BJT energy-gap parser parity.
   - Status: completed by the Rust BJT energy-gap slice.
   - Positive finite BJT `EG` values now lower into the engine energy-gap
     field, while invalid input is rejected before element lowering.

23. Rust BJT forward-Early-voltage parser parity.
   - Status: completed by the Rust BJT forward-Early-voltage slice.
   - Finite non-negative BJT `VAF` values now lower into the engine
     forward-Early-voltage field, with `VA` as a fallback alias and canonical
...
19. Verilog-A and custom models.
