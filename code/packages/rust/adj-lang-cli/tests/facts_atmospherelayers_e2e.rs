...
///
/// Five rows, five sentences, one page.
#[test]
fn no_two_rows_share_a_span() {
    let (spans, locators) = row_fields("earth-science/atmosphere-layers.adj");
    assert_eq!(spans.len(), 5, "every row carries its own source: {spans:?}");
