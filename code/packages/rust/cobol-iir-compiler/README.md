...
| `INSPECT s TALLYING c FOR ALL d` (or `FOR LEADING d`) | a run-time **count loop** (the delimiter position is data-dependent): `len = str_len(s)`, a cursor `j`, a count accumulator `cnt` (init 0), and a single delimiter byte `d` (a `const` for a 1-char literal, or `str_index(item, 0)` for a `PIC X(1)` item). While `j < len` (`cmp_ge`) it reads `s[j]` with `str_index`, `cmp_eq`s it to `d`, and bumps `cnt` on a match; then the count is folded into the counter with the same numeric-store path `ADD` uses — INSPECT **adds** to the counter, it does not clear it first. `FOR ALL` counts EVERY match (the not-equal branch skips just the bump and keeps scanning); `FOR LEADING` counts only the run of consecutive matches at the START (the not-equal branch breaks out of the loop) — that jump target is the ONLY difference between the two forms. The `FOR ALL` form also accepts an optional `{BEFORE\|AFTER} x` **region**: a one-shot scan finds the FIRST occurrence of the single region delimiter `x` (a `found` flag + first index), the window `[start, end)` is derived with the ISO not-found asymmetry (`BEFORE`→whole source, `AFTER`→empty), and the count loop skips any position with `j < start \|\| j >= end`; with no region nothing extra is emitted. A `CHARACTERS` tally, a `FOR LEADING` region, a MULTI-character region delimiter, and several counters/`FOR` phrases are later rungs |
| `INSPECT s REPLACING ALL x BY y` (or `REPLACING LEADING x BY y`) | a per-position **rebuild** of the source, **unrolled** over its compile-time-known width `W` (the map is length-preserving since both `x` and `y` are single characters): at each `j`, `str_index` reads `s[j]`, `cmp_eq` tests it against the search byte `x`, and a branch splices either the replacement `y` (a 1-char string — a `str_const` literal or a `PIC X(1)` item register) or the original `str_slice(s, j, j+1)` onto a `str_concat` accumulator; the `W`-wide result is copied into the source register. `REPLACING ALL` replaces EVERY match; `REPLACING LEADING` replaces only the run at the START — it threads a runtime `active` flag (i64, init 1) through the unroll: position `j` is replaced iff `active AND (s[j] == x)` and `active := active AND (s[j] == x)` sticks at 0 after the first non-match, so a later `x` past the gap is left unchanged (the extra `and` is the ONLY difference, and it folds away for `ALL`). The `REPLACING ALL` form also accepts an optional `{BEFORE\|AFTER} z` **region**, REUSING the TALLYING side's `emit_inspect_region_window`: a one-shot scan finds the FIRST occurrence of the single region delimiter `z`, the window `[start, end)` is derived with the ISO not-found asymmetry (`BEFORE`→whole source, `AFTER`→empty), and the per-position unroll rewrites a match at `j` only when `start <= j < end` (else it keeps the original char); with no region the guard folds away. `REPLACING CHARACTERS`/`FIRST`, a `REPLACING LEADING` region, a MULTI-character region delimiter, several replace items, and a multi-character/figurative/wider search or replacement are later rungs (a `REPLACING LEADING` inside the combined form is supported, and a region on each `ALL` half of the combined form is now supported — see the combined row below) |
