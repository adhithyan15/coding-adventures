...
- Two-phase GFM 0.31.2 parser: block structure (Phase 1) + inline content (Phase 2)
- **Block parser** (`block_parser.rs`): node arena design, handles all GFM block constructs:
...
- **Inline parser** (`inline_parser.rs`): delimiter stack algorithm (GFM Appendix A):
...
- 652/652 (100%) GFM 0.31.2 spec examples pass
...
- **`serde` + `serde_json` dev-dependencies**: for loading the GFM spec JSON in integration tests
