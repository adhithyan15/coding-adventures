...
A thin pipeline package that combines the GFM parser and HTML renderer
into a single, convenient API. 100% conformant with the GFM 0.31.2
...
Parses a GitHub Flavored Markdown string into a `DocumentNode` AST (TE00). All link
...
enabled (required for full GFM spec compliance). **Do not use this for
...
Package version (`"0.1.0"`) and the GFM spec version supported
(`"0.31.2"`).
...
All 652 GFM 0.31.2 spec examples pass (`TestGFMSpec`). The spec
