...
/**
 * Scan a delimiter run of `*` or `_` starting at the current scanner
...
function resolveEmphasis(tokens: InlineToken[]): InlineNode[] {
  ...
  while (i < tokens.length) {
    ...
    const useLen = opener.count >= 2 && closer.count >= 2 ? 2 : 1;
    ...
    const emphNode: InlineNode = isStrong
...
export function resolveInlineContent(
  ...
  function walk(block: BlockNode): void {
    ...
    if ((b.type === "heading" || b.type === "paragraph") && b._rawId !== undefined) {
