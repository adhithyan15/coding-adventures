...
use language_lexer::{create_csharp_lexer, DEFAULT_VERSION, SUPPORTED_VERSIONS};
...
pub fn tokenize(source: &str, version: &str) -> Result<String, JsValue> {
    let mut lexer = create_csharp_lexer(source, resolve_version(version)).map_err(to_js_error)?;
    let tokens = lexer
        .tokenize()
        .map_err(|e| to_js_error(format!("C# tokenization failed: {e}")))?;
