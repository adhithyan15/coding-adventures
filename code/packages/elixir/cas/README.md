...
| Module | Role |
|---|---|
| `CodingAdventures.Cas.BlobStore` | Behaviour (interface) for storage backends |
| `CodingAdventures.Cas.Store` | CAS logic: SHA-1 hashing, integrity, prefix search |
| `CodingAdventures.Cas.LocalDiskStore` | Filesystem backend (Git 2/38 fanout layout) |
| `CodingAdventures.Cas.Hex` | Hex encode/decode utilities |
| `CodingAdventures.Cas.Error` | Typed error reason catalogue |

## Quick Start

```elixir
alias CodingAdventures.Cas.{Store, LocalDiskStore, Hex}
...
Implement the `CodingAdventures.Cas.BlobStore` behaviour:
...
store = CodingAdventures.Cas.Store.new(%MyStore{...})
```
