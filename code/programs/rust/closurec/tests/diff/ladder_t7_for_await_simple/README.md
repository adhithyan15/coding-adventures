# ladder_t7_for_await_simple

Rung 7.for_await of the CCR-066 differential complexity ladder, at `SIMPLE`.
...
java -Duser.language=en -Duser.country=US -Duser.timezone=UTC -Dfile.encoding=UTF-8 \
  ...
  --compilation_level SIMPLE_OPTIMIZATIONS --js tests/diff/ladder_t7_for_await_simple/input/a.js
```
...
closurec refuses valid JavaScript that upstream compiles: for-await-of fails at the typed-AST bridge, via its enclosing async function. This is a front-end capability gap, not an optimizer one. Note it fails only on the typed path — WHITESPACE_ONLY passes, because that path never parses.
