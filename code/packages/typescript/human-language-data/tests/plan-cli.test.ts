...
describe("the plan CLI", () => {
  ...
  it("counts a duplicated inventory once", () => {
    ...
    //
    // 851 -> 848: retiring hand-written French chapter 1 closed three A1 points
    // at once -- greetings and farewells, the definite article, and adjective
    // agreement. All three were taught on the book's first pages from the
    // beginning; none could be probed, because a hand-written chapter's prose
    // owns no atom. RE-MEASURED by running the CLI against the merged tree, not
    // derived by subtracting three.
    expect(out).toMatch(/848 uncovered point\(s\) across 10 written/);
