---
schema_version: 2
id: UR-C10-kaan
spine_node: SPINE-CHECK-WELLBEING
sequence: 380
chapter: 10
type: word
headword: کان
romanization: kān
gloss: ear — the first body word since Chapter 4 folded آپ کیسے ہیں؟ into a single wellbeing check
concept_tag: UR-EAR
prerequisites: [UR-C09-khandan]
sounds: [rtl, long-a]
roots: [sanskrit-karna]
etymology_hook: kān continues Sanskrit karṇa by way of Prakrit and Apabhramsa kaṇṇa, but karṇa's own origin is unsettled — dictionaries note only a possible link to Persian kar, "deaf", nothing as clean as bhāī's cousin brother.
duration:
  max_seconds: 250
requires:
  knowledge: [UR-LEX-KHANDAN, UR-ETYMON-KHANDAN-PERSIAN, UR-GRAMMAR-MERA-GENDER-AGREEMENT]
introduces:
  knowledge: [UR-LEX-KAAN, UR-ETYMON-KAAN-UNCERTAIN]
practises:
  knowledge: [UR-LEX-KHANDAN, UR-ETYMON-KHANDAN-PERSIAN, UR-GRAMMAR-MERA-GENDER-AGREEMENT, UR-LEX-KAAN, UR-ETYMON-KAAN-UNCERTAIN]
skills: [listening, speaking, reading]
modes: [interpretive, interpersonal, presentational]
strands: [meaning-input, meaning-output, language-focus]
register: neutral
variety: contemporary-standard-urdu
reviews_of: [UR-C09-khandan]
---

# کان — "ear," where the body words start

## Warm-up
<!-- hl-knowledge: introduces=[]; assesses=[UR-LEX-KHANDAN, UR-GRAMMAR-MERA-GENDER-AGREEMENT] -->

[PAUSE 2s] Say **merā khāndān** — my family, closing the last chapter. This
chapter turns from the people around you to the wellbeing check itself:
four words for the body, starting with the one you use to hear the
question.

## You'll want to know first — one word
<!-- hl-knowledge: introduces=[UR-LEX-KAAN]; assesses=[] -->

> **کان** — *kān* — **ear**

> **میرا کان۔** — *merā kān.* — "my ear."

**کان** is masculine, so **merā** keeps the form you already know from
**بھائی** and **خاندان**.

## The word, taken apart — honestly, not fully traced
<!-- hl-knowledge: introduces=[UR-ETYMON-KAAN-UNCERTAIN]; assesses=[UR-LEX-KAAN, UR-ETYMON-KHANDAN-PERSIAN] -->

**kān** continues Sanskrit *karṇa* by way of Prakrit and
Apabhramsa *kaṇṇa*. That much is solid. Where *karṇa* itself comes from is
not: the trail runs out inside Indo-Aryan, with only a loose, unconfirmed
suggestion linking it to Persian **کر** (*kar*), "deaf." No English cousin,
no settled Indo-European root — the same honest dead end **پڑھنا**'s *paṭh-*
hit two chapters ago.

Notice the pattern from last chapter still holds: **کان** is inherited,
straight from Sanskrit, the same road as **بھائی** and **بہن**. **خاندان**,
the word that closed that chapter, was Persian instead — the body's own
words stay native even where their history goes quiet.

## Guided Practice
<!-- hl-knowledge: introduces=[]; assesses=[UR-LEX-KAAN, UR-ETYMON-KAAN-UNCERTAIN, UR-GRAMMAR-MERA-GENDER-AGREEMENT] -->

- [YOU SAY: **kān** — ear; then **merā kān**]
- [YOU SAY: **khāndān**, then **kān** — one word ends, the body words begin]
- [YOU NAME: what's uncertain about *karṇa*'s own origin]

## Wrap-up Recall
<!-- hl-knowledge: introduces=[]; assesses=[UR-LEX-KAAN, UR-ETYMON-KAAN-UNCERTAIN] -->

[PAUSE 3s] Is **کان** masculine or feminine? (**Masculine.**) Does *karṇa*
have a secure English cousin? (**No — the origin is unsettled.**)

Sources: [Wiktionary: کان](https://en.wiktionary.org/wiki/%DA%A9%D8%A7%D9%86).
