...
describe("the real corpus", () => {
  ...
  it("pins what nothing had measured", { timeout: 30_000 }, () => {
    ...
    expect(report.summary.atomsTaught).toBe(2777); // +4: ES-LEX-VOS-01, ES-CULTURE-VOSEO-02 (ES-C03-vos) // +3: HL-C98 splits AR-PRESENT-SINGULAR into 1SG/2SG/3SG
    ...
    expect(report.summary.missedByWindow.R2).toBe(1900); // +1: ES-C02-concordancia (HL-C85). buenos dias/buenas tardes stop REQUIRING the agreement rule; it becomes a payoff lesson after the learner has used all three greetings.
  });
  ...
  it("shows what a declared reading order was worth", () => {
    ...
    expect(spanish).toMatchObject({
      ...
      lessonCount: 313, // +4: HL-C98 // +3: HL-C97 adds the repair kit (no entiendo, mas despacio) at chapter 14 // +8 payoff lessons // +1 ES-C03-vos, +1 ES-C02-concordancia
      ...
      atomsTaught: 501, // +3: HL-C98's per-cell atoms
