...
| **L3b-2c-2** | `pair?`/`not`/`equal?` (`ATOM`/`EQ`) lowering → `lispy_pair_p`/`lispy_not`/`lispy_equal`; `COND` truthiness on tagged booleans (`#t`/`#f`/nil). | `(ATOM (CONS 1 2))` → `#f`-driven branch (e.g. `(COND ((ATOM (CONS 1 2)) 1) ('T 7))` → 7); `(EQ 1 1)` → true. |
| **L3b-2c-3** | `make_symbol` lowering → `lispy_make_symbol` with string-literal emission for the name + symbol output. | `(CAR '(A B C))` → symbol `A`; `(EQ 'A 'A)` → true. |
