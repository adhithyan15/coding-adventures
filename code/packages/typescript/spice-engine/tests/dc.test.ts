...
describe("dcOp", () => {
  ...
  it("exports stable model-card supported parameter coverage", () => {
    ...
    expect(coverage).toHaveLength(167);
  ...
  it("passes the current model-card supported parameter coverage gate", () => {
    ...
    expect(report).toStrictEqual({
      ...
      kindCount: 7,
      expectedKindCount: 7,
      canonicalParameterCount: 167,
      expectedCanonicalParameterCount: 167,
      acceptedNameCount: 237,
      aliasedParameterCount: 57,
      maxAliasCount: 4,
      issues: [],
    });
    expect(formatModelCardSupportedParameterCoverageGateReport(report)).toBe(
      "passed\tkind_count\texpected_kind_count\tcanonical_parameter_count\texpected_canonical_parameter_count\taccepted_name_count\taliased_parameter_count\tmax_alias_count\tissue_count\ntrue\t7\t7\t167\t167\t237\t57\t4\t0",
    );
  ...
  it("reports missing model-card supported parameter alias families", () => {
    ...
    expect(report.canonicalParameterCount).toBe(166);
    expect(report.acceptedNameCount).toBe(234);
  ...
  it("normalizes model-card aliases into device instances", () => {
    ...
    const jfetCard = normalizeModelCard("Jn", "njfet", { BET: 9.0e-4, VT0: -1.8, LAM: 0.02, KF: 1.0e-12, AF: 1.3, VJ: 0.8, FC: 0.35, IS: 2.0e-13, RD: 125.0, RS: 75.0, T_NOM: 50.0, TCV: 0.01, VTOTC: -0.0025, BEX: 1.5, BETATCE: -0.5 });
    const jfetModel = jfetFromModelCard("J1", "d", "g", "s", jfetCard);
    expect(jfetCard.parameters).toStrictEqual({ BETA: 9.0e-4, VTO: -1.8, LAMBDA: 0.02, KF: 1.0e-12, AF: 1.3, PB: 0.8, FC: 0.35, IS: 2.0e-13, RD: 125.0, RS: 75.0, TNOM: 50.0, TCV: 0.01, VTOTC: -0.0025, BEX: 1.5, BETATCE: -0.5 });
