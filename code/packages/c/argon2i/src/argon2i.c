/*
 * argon2i.c — implementation of Argon2i (see argon2i.h). A faithful port of the
 * Rust `argon2i` crate. It shares the Argon2 G compression, permutation P, H'
 * extender, and index_alpha mapping with Argon2d; the difference is the
 * data-*independent* address stream (RFC 9106 §3.4.2) that supplies (J1, J2).
 */
#include "argon2i.h"
...
static void fill_segment(uint64_t *memory, size_t r, size_t lane, size_t sl,
    ...
    /* Consume the addresses for the skipped columns too, so the stream stays
     * in phase (RFC 9106 §3.4.2). */
    ...
    for (i = starting_c; i < sl_len; i++) {
        ...
        uint64_t pseudo_rand = address_stream_next(&addr);
...
static Argon2iStatus validate(size_t password_len, size_t salt_len,
    ...
    if ((uint64_t)password_len > 0xFFFFFFFFull) {
        return ARGON2I_PASSWORD_TOO_LONG;
    }
    if (salt_len < 8) {
        return ARGON2I_SALT_TOO_SHORT;
    }
    if ((uint64_t)salt_len > 0xFFFFFFFFull) {
        return ARGON2I_SALT_TOO_LONG;
    }
    if ((uint64_t)key_len > 0xFFFFFFFFull) {
        return ARGON2I_KEY_TOO_LONG;
    }
    if ((uint64_t)ad_len > 0xFFFFFFFFull) {
        return ARGON2I_AD_TOO_LONG;
    }
    if (tag_length < 4) {
        return ARGON2I_TAG_TOO_SMALL;
    }
    if (parallelism < 1 || parallelism > 0xFFFFFF) {
        return ARGON2I_INVALID_PARALLELISM;
    }
    if (memory_cost < 8 * parallelism) {
        return ARGON2I_MEMORY_TOO_SMALL;
    }
    if (time_cost < 1) {
        return ARGON2I_TIME_COST_ZERO;
    }
    if (version != ARGON2I_VERSION) {
        return ARGON2I_UNSUPPORTED_VERSION;
    }
    return ARGON2I_OK;
}
...
Argon2iStatus argon2i(const uint8_t *password, size_t password_len,
    ...
    size_t ad_len = 0;
    uint32_t version = ARGON2I_VERSION;
    Argon2iStatus rc;
    size_t p;
    size_t t;
