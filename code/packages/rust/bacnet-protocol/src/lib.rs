//! Bounded BACnet/IP Who-Is and I-Am protocol framing.
...
impl fmt::Display for BacnetError {
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::ApduType(actual) => write!(
                formatter,
                "BACnet I-Am must use an unconfirmed-request APDU, got 0x{actual:02x}"
            ),
            Self::ServiceChoice(actual) => {
                write!(
                    formatter,
                    "BACnet APDU service must be I-Am (0), got {actual}"
                )
            }
