...
use crate::bits::{and_64, invert_64, or_64, xor_64};
use crate::gate_flags::{
    adc_with_flags, add_with_flags, logic_flags, sbb_with_flags, sub_with_flags,
};
use crate::gate_networks::{
    address, condition_holds, divide_signed, divide_unsigned, multiply_signed, multiply_unsigned,
    shift, ShiftKind,
};
use x86_simulator::decode::{AluOp, Instr, Operand, ShiftOp, SseOp, XmmRm};
    ...
    match op {
        ...
        Operand::Mem {
            ...
            let base = if *rip {
                ...
                base.map_or(0, |register| st.get(register))
            };
            let index = index.map_or(0, |register| st.get(register));
...
pub fn exec_one(
    ...
    match ins {
        ...
        Instr::Imul { dst, src } => {
            let product = multiply_signed(st.get(*dst), read_op(st, mem, src, next_ip, 8)?);
        ...
        Instr::ImulImm { dst, src, imm } => {
            let product = multiply_signed(read_op(st, mem, src, next_ip, 8)?, *imm as u64);
            let result = product as u64;
            st.set(*dst, result);
            let truncated = product != i128::from(result as i64);
            st.flags.cf = truncated;
        ...
        Instr::Mul { src } => {
            ...
            let product = multiply_unsigned(st.get(Reg::Rax), source);
        ...
        Instr::Not { dst } => {
            ...
            write_op(st, mem, dst, next_ip, 8, invert_64(v))?;
            Ok(Flow::Next)
        }
        Instr::Neg { dst } => {
        ...
        Instr::Div { divisor, signed } => {
            ...
            let (quot, rem) = if *signed {
                let (q, r) =
                    divide_signed(dividend, d).ok_or(Trap::DivideError(instr_off as u64))?;
                ...
                let (q, r) =
                    divide_unsigned(dividend, d).ok_or(Trap::DivideError(instr_off as u64))?;
        ...
        Instr::Setcc { cond, dst } => {
            let bit = condition_holds(*cond, st.flags) as u64;
        ...
        Instr::Cmov { cond, dst, src } => {
            if condition_holds(*cond, st.flags) {
...
fn execute_shift(
    ...
    let (result, carry) = match op {
        ShiftOp::Rol => {
            let result = shift(value, count, ShiftKind::RotateLeft);
            (result, result & 1 != 0)
        }
        ShiftOp::Ror => {
            let result = shift(value, count, ShiftKind::RotateRight);
            (result, result >> 63 != 0)
        }
        ShiftOp::Shl => (
            shift(value, count, ShiftKind::Left),
            value >> (64 - count) & 1 != 0,
        ),
        ShiftOp::Shr => (
            shift(value, count, ShiftKind::Right),
            value >> (count - 1) & 1 != 0,
        ),
        ShiftOp::Sar => (
            shift(value, count, ShiftKind::ArithmeticRight),
            value >> (count - 1) & 1 != 0,
        ),
    };
