...
| HL-C09 | Queued — 51 of 228 verified | Expand `DUCTUS` to cover the ten scripts with prose stroke-order entries. | Add cited, font-checked ductus and verified pen-lift metadata for the remaining 177 entries measured by HL-C19; each passes the on-ink, join-tolerance, coverage, citation, and source-agreement invariants. |
...
| HL-C09BA | Queued — next; source recovery | Recover an auditable independent Arabic م demonstration from the source-backed **م ن** page or its media ledger. | The page contains no directly linked handwriting media, so inspect its embedded lesson and attachment inventory for an accessible primary demonstration before authoring any lift count. Keep Arabic Mim independent of the existing Persian and Urdu **م** provenance for the same Unicode glyph; if the media remains unavailable, record the block and move to **ن** source recovery. |
