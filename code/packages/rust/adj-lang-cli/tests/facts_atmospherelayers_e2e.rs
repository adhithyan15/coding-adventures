...
fn earth_science_atmosphere_layer_recall_binds_feature_with_citation() {
    ...
    assert!(
        out.contains("\"source\":\"Most of Earth’s weather happens here, and almost all clouds that are generated by weather are found here, with the exception of cumulonimbus thunder clouds, whose tops can rise into the lowest parts of the neighboring stratosphere.\""),
...
const ATMOSPHERE_LAYERS_PIN: &str = r#""bindings":{"Feature":"weather"},"citations":[{"source":"Most of Earth’s weather happens here, and almost all clouds that are generated by weather are found here, with the exception of cumulonimbus thunder clouds, whose tops can rise into the lowest parts of the neighboring stratosphere.","locator":"https://science.nasa.gov/earth/earth-atmosphere/earths-atmosphere-a-multi-layered-cake/","trust":"authoritative""#;

#[test]
fn atmosphere_layer_feature_citation_is_the_pages_whole_sentence() {
...
",
    ...
    let (ok, out) = run(&dir.join("case.adj"));
    assert!(ok, "cli should succeed: {out}");
    // The shipped citation used to stop at "found here." -- a truncation that
    // appeared on no page, because the sentence continues through the
    // cumulonimbus clause. Restoring it keeps the same grounding for this row
    // and adds a caveat about cloud tops, not a contradiction.
    assert!(
        out.contains(ATMOSPHERE_LAYERS_PIN),
        "the troposphere citation is the page's whole sentence: {out}"
    );
}
