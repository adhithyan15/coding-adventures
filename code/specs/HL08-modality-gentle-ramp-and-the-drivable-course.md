...
`core/generated-book-hashes/*.json` is: nobody edits it, and it cannot drift, because
a stale language-owned shard fails the build. The drift is not cosmetic — a lesson that quietly
...
`<track>/narration/chNN.txt` and `.json` for all 375 chapters, hash-gated by the
language-owned `core/generated-narration-hashes/*.json` shards and checked
byte-for-byte in CI. Three points
