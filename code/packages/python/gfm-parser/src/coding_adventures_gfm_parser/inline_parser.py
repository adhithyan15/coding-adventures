...
from coding_adventures_gfm_parser.entities import decode_entities, decode_entity
...
def parse_inline(raw: str, link_refs: dict) -> list[InlineNode]:
    ...
    while not scanner.done:
        ...
        if ch == "]":
            ...
            # GFM §6.3: when a link is formed inside brackets, the enclosing
            ...
            if opener.is_image:
                ...
                # GFM §6.3: links cannot contain other links.
        ...
        if ch in ("*", "_") or (ch == "~" and scanner.peek(1) == "~"):
            flush_text()
...
def _scan_delimiter_run(scanner: Scanner, char: str) -> DelimiterToken:
    """Scan a delimiter run of *, _, or ~ starting at the current scanner position.
    ...
    # Per GFM spec §6.4 emphasis rules:
    #
    # `*` rules (rules 1 & 2):
    ...
    if char == "*":
        ...
        can_open = count >= 2 and left_flanking
        can_close = count >= 2 and right_flanking
    else:  # _
...
# ─── Emphasis Resolution ───────────────────────────────────────────────────────
#
# Implements the GFM Appendix A delimiter stack algorithm.
#
...
def _resolve_emphasis(tokens: list[InlineToken]) -> list[InlineNode]:
    ...
    while i < len(tokens):
        ...
        use_len = 2 if closer.char == "~" else 2 if opener.count >= 2 and closer.count >= 2 else 1
        is_strong = use_len == 2
        ...
        if closer.char == "~":
            emph_node = {"type": "strikethrough", "children": inner_nodes}
        elif is_strong:
...
def _try_html_inline(scanner: Scanner) -> RawInlineNode | None:
    ...
    GFM spec §6.6 defines six inline HTML forms:
...
def _try_autolink(scanner: Scanner) -> AutolinkNode | None:
    ...
    RFC-5322-ish pattern (GFM spec §6.7).
    """
    if scanner.peek() != "<":
        return None
    saved_pos = scanner.pos
...
def _extract_plain_text(nodes: list[InlineNode]) -> str:
    ...
    GFM spec §6.4: "The content of the first sequence of brackets
...
def resolve_inline_content(
    ...
    def walk(block: BlockNode) -> None:
        ...
        if block_type in ("heading", "paragraph", "table_cell"):
            node_id = id(block)
