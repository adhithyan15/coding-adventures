# ladder_t7_logical_assign_simple

Rung 7.logical_assign of the CCR-066 differential complexity ladder, at `SIMPLE`.
...
java -Duser.language=en -Duser.country=US -Duser.timezone=UTC -Dfile.encoding=UTF-8 \
  ...
  --compilation_level SIMPLE_OPTIMIZATIONS --js tests/diff/ladder_t7_logical_assign_simple/input/a.js
```
...
```js
var a=null;a||=1;console.log(a);
```

closurec does not fold a logical assignment whose left operand is a known constant. Upstream propagates `a = null` and reduces `a ||= 1` to `a = 1`. CCR-068.
