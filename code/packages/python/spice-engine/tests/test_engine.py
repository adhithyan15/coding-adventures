...
def test_mos_model_card_rejects_negative_or_non_finite_bottom_junction_capacitance() -> None:
    for value in (0.0, 2.0e-3, 1.0):
        valid = normalize_model_card("Mvalid", "nmos", {"CJ": value})
        assert valid.parameters["CJ"] == pytest.approx(value)

    for invalid_capacitance in (-math.inf, -0.1, math.inf, math.nan):
        with pytest.raises(
            ValueError, match="MOSFET CJ must be finite and non-negative"
        ):
...
def test_mos_model_card_rejects_negative_or_non_finite_source_resistance() -> None:
    for value in (0.0, 10.0, 1.0e6):
        valid = normalize_model_card("Mvalid", "pmos", {"RS": value})
        assert valid.parameters["RS"] == pytest.approx(value)

    for invalid_resistance in (-math.inf, -0.1, math.inf, math.nan):
        with pytest.raises(ValueError, match="MOSFET RS must be finite and non-negative"):
...
def test_mos_model_card_rejects_negative_or_non_finite_flicker_noise_exponent() -> None:
    for value in (0.0, 1.0, 2.0):
        valid = normalize_model_card("Mvalid", "nmos", {"AF": value})
        assert valid.parameters["AF"] == pytest.approx(value)

    for invalid_exponent in (-math.inf, -0.1, math.inf, math.nan):
        with pytest.raises(ValueError, match="MOSFET AF must be finite and non-negative"):
