...
% canonical-source-hash: fnv1a64:0627f7ee6175ffb7
...
\section[namaskāraṁ]{Namaskaram — first hear the greeting}
...
\begin{quote}
Imagine meeting someone with your palms together and a small bow. First, only listen: \emph{namaskāraṁ}.
\end{quote}
...
\midrule
\textbf{Malayalam} & \emph{namaskāraṁ} & Sanskrit \\
Telugu & \emph{namaskāraṁ} & Sanskrit \\
...
\begin{itemize}
\raggedright
  \item \emph{namaskāraṁ} once as a greeting
...
\begin{quote}
Say \emph{na-ma} — the first two beats of \emph{namaskāraṁ}. Your hand meets only those two familiar sounds today.
\end{quote}
...
\end{tcolorbox}
\section[namaskāraṁ]{\ml{നമസ്കാരം} — every shape is already yours}
...
\begin{quote}
Say \emph{namaskāraṁ}. Now the written greeting may appear: it contains no unfamiliar shape.
\end{quote}
...
Together: \textbf{\ml{നമസ്കാരം}}, \emph{namaskāraṁ}. The conjunct \textbf{\ml{സ്ക}} is still just \textbf{\ml{സ}} with its vowel removed by \textbf{\ml{്}}, joined to \textbf{\ml{ക}}.
...
\end{tcolorbox}
\section[namaskāraṁ]{One heard greeting — write the whole}
...
\emph{Hear:} \emph{namaskāraṁ} — “greetings”
...
\hlblockfigure{\detokenize{figures/ML-S133-letter-a-filmstrip.pdf}}{How \ml{അ} is written, stroke by stroke}
...
\end{tcolorbox}
\section[śari]{\ml{ശരി} (śari) — \textquotedblleft{}okay,\textquotedblright{} the whole-family word once more}
...
\midrule
\textbf{\ml{നമസ്കാരം}} & \emph{namaskāraṁ} & hello / greetings & Sanskrit \\
...
\begin{itemize}
\raggedright
  \item all five — namaskāraṁ · nandi · athe · illa · śari
  \item greet and thank — \textquotedblleft{}namaskāraṁ!\textquotedblright{} … \textquotedblleft{}nandi.\textquotedblright{}
\end{itemize}

\subsection*{Writing checkpoint}
Hide the model. Hear \emph{namaskāraṁ} once, write \textbf{\ml{നമസ്കാരം}}, then uncover the word in the table and check one group at a time: \textbf{\ml{നമ} · \ml{സ്ക} · \ml{ാരം}}. A mismatch is a signal to repeat the earlier two-minute shape lesson, not to push faster.
...
\begin{tcolorbox}[breakable,colback=teal!4,colframe=teal!35!black,title={Before you move on}]
Read all five aloud. Which one is Sanskrit, and which four are native Dravidian shared with Tamil? (Sanskrit: \emph{namaskāraṁ}. Native: \emph{nandi, athe, illa, śari}.) Which word can mean both \textquotedblleft{}okay\textquotedblright{} and a casual sign-off? (\emph{śari}.)
