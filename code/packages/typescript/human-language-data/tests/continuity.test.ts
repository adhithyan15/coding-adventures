...
describe("the real corpus", () => {
  ...
  it("pins what nothing had measured", { timeout: 30_000 }, () => {
    ...
    expect(report.summary.atomsTaught).toBe(2807); // +4: ES-LEX-VOS-01, ES-CULTURE-VOSEO-02 (ES-C03-vos) // +3: HL-C98 splits AR-PRESENT-SINGULAR into 1SG/2SG/3SG // +2: HL-C88 slice 7 (ES-C09-ncia)
    ...
    expect(report.summary.atomsNeverRevisited).toBe(481); // -17: payoff lessons plus si/no moving early // +2: HL-C98's per-cell atoms are revisited later than the R-windows reach // +1: HL-C88 slice 7 (ES-C09-ncia)
    ...
    expect(report.summary.forwardReferences).toBe(438); // -8: HL-C112 moves casa and libro ahead of the adjective arc, so uses that were early are now taught // -1: HL-C98 removes a forward reference // -5: HL-C103 census adds comes/hand/regular to ENGLISH_COLLISIONS, and drops an untaught tres from an accepted list
    ...
    // R1 stops at 3.
    expect(report.summary.missedByWindow.R1).toBe(904); // +1: HL-C112 moves casa and libro 14 chapters earlier, so one atom's next re-use now falls outside R1 // +2: HL-C98 // +1: HL-C88 slice 7 (ES-C09-ncia)
    ...
    expect(report.summary.missedByWindow.R2).toBe(1924); // +1: HL-C112, same cause as R1 above // +1: ES-C02-concordancia (HL-C85). buenos dias/buenas tardes stop REQUIRING the agreement rule; it becomes a payoff lesson after the learner has used all three greetings. // +2: HL-C88 slice 7 (ES-C09-ncia)
  });
  ...
  it("shows what a declared reading order was worth", () => {
    ...
    expect(spanish).toMatchObject({
      ...
      lessonCount: 337, // +4: HL-C98 // +3: HL-C97 adds the repair kit (no entiendo, mas despacio) at chapter 14 // +8 payoff lessons // +1 ES-C03-vos, +1 ES-C02-concordancia // +1: HL-C88 slice 7 (ES-C09-ncia)
      ...
      atomsTaught: 531, // +3: HL-C98's per-cell atoms // +2: HL-C88 slice 7 (ES-C09-ncia)
      atomsNeverRevisited: 72, // +2: HL-C98 // +1: HL-C88 slice 7 (ES-C09-ncia)
    });
  });
