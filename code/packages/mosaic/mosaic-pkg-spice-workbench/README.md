...
selection, a status area, and a raw result pane. The matching
`spice-mosaic-app` Rust adapter owns state and invokes the parser/engine.

Tables, waveform traces, schematic capture, and vendor-dialect controls are
deliberately later phases; this package gives each of them one real workbench
surface to extend.
