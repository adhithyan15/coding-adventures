...
// ============================================================================
// 16. WITHGLOBALS TESTS
// ============================================================================
//
// These tests verify that WithGlobals() injects pre-seeded variables into
// every VM instance created by the interpreter, including VMs created for
// load() calls.  This is the foundation for _ctx injection in the build tool.

func TestWithGlobalsSimpleValue(t *testing.T) {
	// A simple global variable should be accessible in Starlark code.
	interp := NewInterpreter(
		WithGlobals(map[string]interface{}{
			"greeting": "hello",
		}),
	)
	result, err := interp.Interpret("x = greeting\n")
	if err != nil {
		t.Fatal(err)
	}
...
func TestWithGlobalsDict(t *testing.T) {
	// A dict global (like _ctx) should be accessible via subscript.
	interp := NewInterpreter(
		WithGlobals(map[string]interface{}{
	...
	result, err := interp.Interpret("v = _ctx[\"version\"]\nos_name = _ctx[\"os\"]\n")
	if err != nil {
		t.Fatal(err)
	}
	expectVar(t, result, "v", 1)
	expectVar(t, result, "os_name", "darwin")
}

func TestWithGlobalsAvailableInLoadedFiles(t *testing.T) {
	// Globals injected via WithGlobals must be available in loaded files,
	// not just the top-level program.  This is critical: _ctx must work
	// in cmd.star which is loaded by BUILD files.
	files := map[string]string{
		"checker.star": "detected_os = _ctx[\"os\"]\n",
	}
	interp := NewInterpreter(
	...
	result, err := interp.Interpret("load(\"checker.star\", \"detected_os\")\nmy_os = detected_os\n")
	if err != nil {
		t.Fatal(err)
	}
	expectVar(t, result, "my_os", "linux")
}

func TestWithGlobalsInNestedLoads(t *testing.T) {
	// Globals must propagate through nested load() chains:
	// main.star -> mid.star -> leaf.star
	// All three should see _ctx.
	files := map[string]string{
		"leaf.star": "leaf_os = _ctx[\"os\"]\n",
		"mid.star":  "load(\"leaf.star\", \"leaf_os\")\nmid_os = _ctx[\"os\"]\n",
	}
	interp := NewInterpreter(
		WithFileResolver(DictResolver(files)),
		WithGlobals(map[string]interface{}{
	...
	result, err := interp.Interpret(
		"load(\"mid.star\", \"mid_os\", \"leaf_os\")\n" +
			"top_os = _ctx[\"os\"]\n",
	)
	if err != nil {
		t.Fatal(err)
	}
	expectVar(t, result, "top_os", "windows")
	expectVar(t, result, "mid_os", "windows")
	expectVar(t, result, "leaf_os", "windows")
}

func TestWithGlobalsNilIsNoOp(t *testing.T) {
	// WithGlobals(nil) should not break anything.
	interp := NewInterpreter(
		WithGlobals(nil),
	)
	result, err := interp.Interpret("x = 42\n")
...
func TestWithGlobalsCombinedWithOtherOptions(t *testing.T) {
	// WithGlobals should work alongside WithFileResolver and
	// WithMaxRecursionDepth without interference.
	files := map[string]string{
		"data.star": "val = _ctx[\"version\"]\n",
	}
	interp := NewInterpreter(
		WithFileResolver(DictResolver(files)),
		WithMaxRecursionDepth(300),
		WithGlobals(map[string]interface{}{
			"_ctx": map[string]interface{}{
				"version": 1,
			},
	...
	if interp.MaxRecursionDepth != 300 {
		t.Errorf("expected MaxRecursionDepth 300, got %d", interp.MaxRecursionDepth)
	}
	result, err := interp.Interpret("load(\"data.star\", \"val\")\nx = val\n")
	if err != nil {
		t.Fatal(err)
	}
	expectVar(t, result, "x", 1)
...
func TestWithGlobalsDoNotLeakBetweenExecutions(t *testing.T) {
	// Each Interpret() call gets a fresh VM.  Modifying a global in
	// one execution should not affect the next execution.
	interp := NewInterpreter(
		WithGlobals(map[string]interface{}{
			"counter": 0,
		}),
	)
	// First execution: read the counter.
	result1, err := interp.Interpret("x = counter\n")
	if err != nil {
		t.Fatal(err)
	}
	expectVar(t, result1, "x", 0)

	// Second execution: the counter should still be 0 (fresh VM).
	result2, err := interp.Interpret("y = counter\n")
	if err != nil {
		t.Fatal(err)
	}
...
func TestWithGlobalsCtxSimulation(t *testing.T) {
	...
	result, err := interp.Interpret(
		"load(\"platform.star\", \"current_os\", \"is_windows\", \"is_unix\")\n",
	)
	if err != nil {
		t.Fatal(err)
	}
	expectVar(t, result, "current_os", "darwin")
	expectVar(t, result, "is_windows", false)
	expectVar(t, result, "is_unix", true)
}
