...
describe("the committed corpus", () => {
  it("pins the first measurement", () => {
    ...
    expect(r.summary.lessonsUsingTerms).toBe(1735); // +8: HL-C94 // +4: HL-C98
    ...
    expect(r.summary.usesBeforeIntroduction).toBe(7964); // +5: ES-C03-vos and the two practice edits use ordinary terms (word, plural, ending) // +15: HL-C98's four lessons use ordinary terms (ending, verb, form)
    expect(r.summary.technicalUsesBeforeIntroduction).toBe(2368); // +4: HL-C98
    expect(r.summary.technicalLessons).toBe(1202); // +4: HL-C98

    expect(r.summary.worstTerms.slice(0, 2)).toEqual([
      { term: "verb", lessons: 827 }, // +4: HL-C98 // +2: HL-C99
