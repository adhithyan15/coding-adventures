...
## 0.329.0 — 2026-09-13 — VM-040 COBOL BEAM INSPECT TALLYING/REPLACING probe
...
The seven-backend ALGOL matrix now proves formatter-free output from a real
scalar recurrence simulated across every iteration of a statically bounded
integer-controlled `step` loop.
...
an equal exact variable-free built-in result in a conditional exponent.

## 0.323.0 — 2026-09-12 — ALGOL integer-backed standard-result powers

The seven-backend ALGOL matrix now proves a tracked real snapshot paired with
an equal exact built-in result over a losslessly widened tracked integer.

## 0.322.0 — 2026-09-12 — ALGOL snapshot/standard-result powers

The seven-backend ALGOL matrix now proves a path-independent power exponent
mixing a tracked real snapshot with an equal exact tracked built-in result.

## 0.321.0 — 2026-09-12 — ALGOL snapshot/literal arithmetic powers

The seven-backend ALGOL matrix now proves a path-independent power exponent
mixing a tracked real snapshot with equal variable-free real arithmetic.

## 0.320.0 — 2026-09-12 — ALGOL conditional real snapshot/literal powers

The seven-backend ALGOL matrix now proves path-independent conditional power
exponents that mix a tracked local real snapshot with an equal real literal.
...
local real snapshots can bound a plain real power without a standard-function
result in its base.

## 0.314.0 — 2026-09-12 — ALGOL standard-free conditional mixed powers

The seven-backend ALGOL matrix now proves that path-independent conditional
arithmetic mixing tracked local integer and real snapshots can bound a plain
real power while preserving runtime selector execution.
...
The seven-backend ALGOL matrix now proves that path-independent conditional
arithmetic mixing tracked local integer and real snapshots can bound a power
around a standard-function result while preserving runtime selector execution.

## 0.311.0 — 2026-09-12 — ALGOL mixed tracked numeric arithmetic powers

The seven-backend ALGOL matrix now proves that exact arithmetic mixing tracked
local integer and real snapshots can bound a power around a path-independent
standard-function result. The frontend retains runtime power for inexact
integer widening and other unsupported mixed forms.
...
## 0.303.0 — 2026-09-08 — conditional tracked-real arithmetic ALGOL powers

The seven-backend ALGOL matrix now proves that path-independent conditional
arithmetic over initialized tracked local reals may bound a power around a pure
built-in result without erasing its runtime selector.
...
real may bound a power around a path-independent pure built-in result while the
...
path-independent built-in result before another pure built-in exponent.
...
## 0.292.0 — 2026-09-07 — tracked ALGOL real standard-function exponents

The seven-backend ALGOL matrix now proves an exact built-in standard-function
result over a tracked local real snapshot can feed bounded real-power unrolling
while arbitrary tracked real arithmetic remains dynamic.
...
## 0.285.0 - 2026-09-01 (ALGOL composed tracked `sqrt` exponents)

The ALGOL matrix now proves on all seven standard backends that an exact
integral tracked square root may compose with checked exponent arithmetic while
retaining bounded multiplication and runtime result checks.
...
## 0.278.0 - 2026-08-28 (ALGOL composed standard-function real-power exponents)

The ALGOL matrix now proves on all seven standard backends that checked
integer arithmetic may compose pure tracked `abs` and `sign` results while
retaining bounded real multiplication lowering and runtime result checks.
...
## 0.276.0 - 2026-08-28 (ALGOL tracked standard-function real-power exponents)

The ALGOL matrix now proves on all seven standard backends that pure `abs`
over an exact tracked integer may provide a bounded real-power exponent while
retaining multiplication lowering and formatter-free snapshot output.
...
around an exact local integer exponent can retain bounded real multiplication
lowering and later formatter-free snapshot output.

## 0.272.0 - 2026-08-27 (ALGOL bare tracked real-power exponents)

The ALGOL matrix now proves on all seven standard backends that an exact bare
local integer exponent can retain bounded real multiplication lowering and
later formatter-free snapshot output.
...
## 0.268.0 - 2026-08-27 (ALGOL conditional exponent metadata)

The ALGOL matrix now proves on all seven standard backends that equal branches
of a runtime conditional can provide bounded real-power snapshot metadata
without removing the runtime branch or exponentiation.

## 0.267.0 - 2026-08-27 (ALGOL tracked function-exponent metadata)

The ALGOL matrix now proves on all seven standard backends that a tracked
integer can flow through an implemented standard function into bounded real
power snapshot metadata while runtime exponentiation remains dynamic.

## 0.266.0 - 2026-08-27 (ALGOL tracked integer exponent metadata)

The ALGOL matrix now proves on all seven standard backends that a real scalar
snapshot can retain an exact result computed with a tracked integer exponent,
while the runtime power operation remains dynamic.
...
negative integral arithmetic exponent on all seven standard backends. Unsafe,
fractional, oversized, dynamic, and zero-base reciprocal forms remain
conservative and require runtime real formatting.

## 0.261.0 - 2026-08-27 (ALGOL static real arithmetic exponents)

The ALGOL matrix now proves finite literal-only real powers with bounded
nonnegative integral arithmetic exponent operands on all seven standard
backends. Non-integral, unsafe, oversized, and dynamic forms remain
conservative and require runtime real formatting.
...
literal-only arithmetic expression raised to an integral real-literal zero
exponent on all seven standard backends. Dynamic bases remain conservative.
...
The ALGOL matrix now preserves a finite-real selector when subtracting an even
positive power of negative zero on all seven standard backends. Compiler
regressions cover signed-zero parity and keep zero exponents and unsafe sign
combinations conservative.
...
literal-only zero expression with the safe resulting sign on all seven standard
backends. Compiler regressions retain conservative handling for the opposite
zero sign.
...
negative-unit multiplications or divisions on all seven standard backends.
Compiler regressions keep odd-sign chains and division by a selector
conservative.
...
positive zero on all seven standard backends. Compiler regressions keep real
addition of positive zero and subtraction of negative zero conservative.
...
condition against a finite real transitive dependency. Compiler regressions
retain conservative handling when that dependency changes.
...
leaves preserve the terminal dependency in a ten-level selector chain. A
compiler regression keeps mixed identity/changing leaves conservative.

## 0.240.0 - 2026-08-25 (ALGOL grouped selector identities)

The seven-backend ALGOL matrix now proves a complete integer identity chain
remains stable through nested parentheses. Compiler regressions cover grouped
boolean and real identities while retaining conservative handling for grouped
changing expressions.

## 0.239.0 - 2026-08-25 (ALGOL signed neutral selector literals)

The seven-backend ALGOL matrix now proves unary-plus one and signed integer
zero remain neutral inside a bounded-while selector identity chain. Compiler
regressions cover integer and real forms while retaining conservative handling
for negative one.
...
## 0.230.0 - 2026-08-25 (ALGOL integer identity selector writes)

The seven-backend ALGOL matrix now proves a computed `choose := choose + 0`
write preserves a transitive bounded-while selector dependency. The compiler
also pins the other structural integer identity forms and a non-identity
fail-closed counterexample.

## 0.229.0 - 2026-08-24 (ALGOL boolean identity selector writes)

The seven-backend ALGOL matrix now proves a computed `flag := flag and true`
write preserves a transitive bounded-while selector dependency. The compiler
also pins the other boolean identity forms and a non-identity fail-closed
counterexample.
...
select the preserving leaf of a conditional assignment to a conditional
selector dependency.
...
a known predicate dependency stable while it selects the preserving leaf of a
conditional assignment to a known transitive selector.
...
The seven-backend ALGOL matrix now proves that equal bare-self conditional
leaves preserve a known transitive selector even with a dynamic predicate.

## 0.226.11 - 2026-08-13 (ALGOL idempotent transitive assignment selectors)

The seven-backend ALGOL matrix now proves that an exact selector
self-assignment preserves a known transitive selector while its unreachable
conditional leaf would change the dependency.
...
self-contained `while` control retains its first false-predicate value.

## 0.225.4 - 2026-08-13 (ALGOL static while-body snapshots)

The seven-backend ALGOL matrix now proves that a statically nonempty `while`
element retains a path-independent real constant established by every body pass.
...
assignment retains its reachable snapshot across all seven standard backends.
...
establish definite string initialization despite a dynamic conditional
selector, across all seven standard backends.

## 0.222.8 - 2026-08-12 (ALGOL right-dominating while predicates — seven backends)

The ALGOL initial-`while` matrix cell now proves that `dynamic or true` remains
unconditionally true and establishes definite string initialization on all
seven standard backends.

## 0.222.7 - 2026-08-12 (ALGOL truth-dominating while predicates — seven backends)

The ALGOL initial-`while` matrix cell now proves that `true or dynamic` remains
unconditionally true and establishes definite string initialization on all
seven standard backends.
