...
- `xnor_gate` now delegates directly to `CMOSXnor#evaluate_digital` (a dedicated
  XOR + Inverter gate instance) rather than composing `CMOS_INVERTER` and `CMOS_XOR`
  inline. Added `CMOS_XNOR` module-level constant alongside the other CMOS instances.
