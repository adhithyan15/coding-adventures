...
enum Disposition {
    UpstreamGolden,
    /// The pinned oracle REFUSES these inputs as invoked — nonzero exit, empty
    /// stdout, a named `JSC_` diagnostic — so there is no upstream output the
    /// golden could have come from. `upstream_golden` is not merely unverified
    /// for such a fixture, it is unachievable (CCR-081).
    ///
    /// The distinction that matters: an `UpstreamGolden` set claims upstream
    /// produced its bytes; this one claims upstream produced *nothing*, and the
    /// bytes are our own behaviour on input upstream declines. Whether
    /// `closurec` should decline it too is CCR-075 (#15860) and is not settled
    /// here.
    UpstreamRefuses,
    MixedContract,
    LocalExtension,
...
/// |---|---|
/// | keys == fixtures | a fixture quietly added to the set with no captured refusal behind it |
...
/// maintainer generated the report, exactly as the JAR download did.
fn validate_refusal_evidence(root: &Path, set: &FixtureSet, errors: &mut Vec<String>) {
    let Some(path) = resolve_existing_path(
        root,
        REFUSALS_REPORT_PATH,
        false,
        "refusal evidence",
        errors,
    ) else {
    ...
    let Ok(text) = std::fs::read_to_string(&path) else {
        errors.push(format!("refusal evidence {REFUSALS_REPORT_PATH} is unreadable"));
        return;
    };
    let Ok(report) = serde_json::from_str::<RefusalReport>(&text) else {
        errors.push(format!("refusal evidence {REFUSALS_REPORT_PATH} does not parse"));
        return;
    };
    ...
    for set in &manifest.fixture_sets {
        ...
        match set.disposition {
            ...
            Disposition::UpstreamRefuses => {
                ...
                validate_refusal_evidence(root, set, &mut errors);
            }
...
fn refusal_evidence_on_disk_is_a_genuine_capture() {
    ...
    // The four diagnostics families, pinned by name. `simple-try-catch` and
