...
fn emit_for_spread(
    ...
    match index_name {
        Some(idx) if dart_source_references_identifier(body_trimmed, &idx) => Ok(format!(
...
fn emit_native_data_table(
        ...
        .filter(|index| dart_source_references_identifier(header_widget, index))
    {
        Some(index) => {
            writeln!(
                out,
                "{field_pad}columns: {header_collection}.asMap().entries.map((_mosaicColumnEntry) {{"
            )
            .unwrap();
            writeln!(out, "{binding_pad}final {index} = _mosaicColumnEntry.key;").unwrap();
            writeln!(
                out,
                "{binding_pad}final {header_item} = _mosaicColumnEntry.value;"
            )
            .unwrap();
            writeln!(
                out,
    ...
    match row_index {
        Some(index) => {
            writeln!(
                out,
                "{field_pad}rows: {row_collection}.asMap().entries.map((_mosaicRowEntry) {{"
            )
            .unwrap();
            writeln!(out, "{binding_pad}final {index} = _mosaicRowEntry.key;").unwrap();
            writeln!(
                out,
                "{binding_pad}final {row_item} = _mosaicRowEntry.value;"
            )
            .unwrap();
        ...
        .filter(|index| dart_source_references_identifier(cell_widget, index))
    {
        Some(index) => {
            writeln!(
                out,
                "{return_pad}  cells: {cell_collection}.asMap().entries.map((_mosaicCellEntry) {{"
            )
            .unwrap();
            writeln!(out, "{return_pad}    final {index} = _mosaicCellEntry.key;").unwrap();
            writeln!(
                out,
                "{return_pad}    final {cell_item} = _mosaicCellEntry.value;"
            )
            .unwrap();
