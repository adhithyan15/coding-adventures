...
fn main() { out(1, VALUE); }\n",
    ...
    // ALGOL 60 — straight-line integer snapshots retain bounded powers whose
    // exponent operands are variable-free checked integer arithmetic.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer n, saved; n := 6 ^ ((2 ^ (2 - 1)) div 2) + 36; saved := n; n := 9; output(saved + 0.5) end",
        expect: Expect::Stdout("42.5"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    ...
    // ALGOL 60 — checked integer arithmetic over exact tracked snapshots is
    // side-effect-free and retains bounded real multiplication lowering.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer exponent; real saved; exponent := -2; saved := 6.0 ^ (0 - exponent) + 6.0; exponent := 3; output(saved + 0.5) end",
        expect: Expect::Stdout("42.5"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    ...
    // ALGOL 60 — built-in entier may consume composed tracked integer
    // arithmetic when widening that arithmetic to binary64 is exact.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer exponent; real saved; exponent := -2; saved := 6.0 ^ entier(abs(exponent) + sign(exponent) + 1) + 6.0; exponent := 3; if saved = 42.0 then output(42) else output(1) end",
        expect: Expect::Stdout("42"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    ...
    // ALGOL 60 — an exact integral tracked square root may compose with
    // checked integer-valued arithmetic before bounded real-power unrolling.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer exponent; real saved; exponent := 4; saved := 6.0 ^ (sqrt(exponent) + sign(exponent) - 1) + 6.0; exponent := 9; if saved = 42.0 then output(42) else output(1) end",
        expect: Expect::Stdout("42"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — equal exact integral tracked square-root branches may retain
    // bounded multiplication while their pure runtime selector still lowers.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer gate, exponent; real saved; exponent := 4; saved := 6.0 ^ (if gate = 0 then sqrt(exponent) else sqrt(exponent)) + 6.0; exponent := 9; if saved = 42.0 then output(42) else output(1) end",
        expect: Expect::Stdout("42"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    ...
    // ALGOL 60 — exact built-in results over tracked real snapshots may also
    // provide bounded exponents without admitting arbitrary real arithmetic.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin real exponent, saved; exponent := 0.0; saved := 6.0 ^ (cos(exponent) + 1) + 6.0; exponent := 9.0; if saved = 42.0 then output(42) else output(1) end",
        expect: Expect::Stdout("42"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    ...
    // ALGOL 60 — a pure built-in may collapse distinct conditional tracked
    // real operands to one exact exponent while the selector still executes.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin real gate, exponent, saved; exponent := -2.0; saved := 6.0 ^ abs(if gate = 0.0 then exponent else -exponent) + 6.0; gate := 1.0; exponent := 9.0; if saved = 42.0 then output(42) else output(1) end",
        expect: Expect::Stdout("42"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    ...
    // ALGOL 60 — exact literal arithmetic may compose around a
    // path-independent built-in result before another pure built-in.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin real gate, exponent, saved; exponent := -2.0; saved := 6.0 ^ entier(abs(if gate = 0.0 then exponent else -exponent) + 0.5) + 6.0; gate := 1.0; exponent := 9.0; if saved = 42.0 then output(42) else output(1) end",
        expect: Expect::Stdout("42"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    ...
    // ALGOL 60 — a finite tracked local real snapshot may join arithmetic
    // around a path-independent built-in result without erasing the selector.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
    ...
    // ALGOL 60 — an exact tracked integer may bound a power around a
    // path-independent built-in result without erasing the selector.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer power; real gate, exponent, saved; power := 1; exponent := -2.0; saved := 6.0 ^ entier((abs(if gate = 0.0 then exponent else -exponent) + 0.5) ^ power) + 6.0; gate := 1.0; power := 9; exponent := 9.0; if saved = 42.0 then output(42) else output(1) end",
        expect: Expect::Stdout("42"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — an exact integral tracked real may bound a power around a
    // path-independent built-in result without erasing the selector.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin real power, gate, exponent, saved; power := 1.0; exponent := -2.0; saved := 6.0 ^ entier((abs(if gate = 0.0 then exponent else -exponent) + 0.5) ^ power) + 6.0; gate := 1.0; power := 9.0; exponent := 9.0; if saved = 42.0 then output(42) else output(1) end",
        expect: Expect::Stdout("42"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    ...
    // ALGOL 60 — nested path-independent tracked real snapshot conditionals
    // retain every runtime selector while bounding a plain real power.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin boolean outer, inner; real first, second, third, saved; first := 2.0; second := 2.0; third := 2.0; saved := 6.0 ^ (if outer then if inner then first else second else third) + 6.0; outer := true; inner := true; first := 9.0; second := 9.0; third := 9.0; if saved = 42.0 then output(42) else output(1) end",
        expect: Expect::Stdout("42"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — a tracked real snapshot and equal real literal may form a
    // path-independent exponent while preserving runtime selector execution.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin boolean gate; real exponent, saved; exponent := 2.0; saved := 6.0 ^ (if gate then exponent else 2.0) + 6.0; gate := true; exponent := 9.0; if saved = 42.0 then output(42) else output(1) end",
        expect: Expect::Stdout("42"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — finite variable-free real arithmetic may match a tracked
    // snapshot leaf in a path-independent power exponent.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin boolean gate; real exponent, saved; exponent := 2.0; saved := 6.0 ^ (if gate then exponent else 1.0 + 1.0) + 6.0; gate := true; exponent := 9.0; if saved = 42.0 then output(42) else output(1) end",
        expect: Expect::Stdout("42"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    ...
    // ALGOL 60 — finite arithmetic over initialized tracked local reals may
    // bound a plain real power without a standard-function result in its base.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin real power, offset, saved; power := 0.5; offset := 1.5; saved := 6.0 ^ (power + offset) + 6.0; power := 9.0; offset := 9.0; if saved = 42.0 then output(42) else output(1) end",
        expect: Expect::Stdout("42"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — path-independent conditional arithmetic over tracked local
    // reals may bound a power while its runtime selector remains emitted.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin real power, offset, gate, exponent, saved; power := 0.5; offset := 0.5; exponent := -2.0; saved := 6.0 ^ entier((abs(if gate = 0.0 then exponent else -exponent) + 0.5) ^ (if gate = 0.0 then power + offset else offset + power)) + 6.0; gate := 1.0; power := 9.0; offset := 9.0; exponent := 9.0; if saved = 42.0 then output(42) else output(1) end",
        expect: Expect::Stdout("42"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — path-independent conditional arithmetic over tracked local
    ...
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer whole; real part, gate, exponent, saved; whole := 1; part := 0.0; exponent := -2.0; saved := 6.0 ^ entier((abs(if gate = 0.0 then exponent else -exponent) + 0.5) ^ (whole + part)) + 6.0; gate := 1.0; whole := 9; part := 9.0; exponent := 9.0; if saved = 42.0 then output(42) else output(1) end",
        expect: Expect::Stdout("42"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    ...
    // ALGOL 60 — path-independent conditional mixed tracked arithmetic may
    // bound a plain real power while preserving its runtime selector.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer whole; real part, gate, saved; whole := 1; part := 1.0; saved := 6.0 ^ (if gate = 0.0 then whole + part else part + whole) + 6.0; gate := 1.0; whole := 9; part := 9.0; if saved = 42.0 then output(42) else output(1) end",
        expect: Expect::Stdout("42"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    ...
    // ALGOL 60 — mutual recursion carries one direct procedure formal through
    // a finite two-sibling graph. `even` and `odd` each forward `twice` to the
    // other; the active specialised pair closes the cycle and the base call
    // remains direct without a dynamic procedure descriptor.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer result; \
    ...
    // ALGOL 60 — a standard output procedure travels through nested direct
    // `procedure` formals. The specialised wrappers retain the direct `print`
    // target and reuse its statement-only `print_str` lowering, so no
    // function-pointer ABI or output-procedure signature is needed.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin \
                  procedure dispatch(p, s); value s; procedure p; string s; \
                    begin procedure forward(p, s); value s; procedure p; string s; \
                          p(s); \
                          forward(p, s) end; \
                  dispatch(print, 'FORMAL') end",
        expect: Expect::Stdout("FORMAL"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    ...
    // global alias. The literal assignment writes through that binding and the
    // equality read observes the runtime string on all seven backends.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin string s; integer result; \
    ...
    // ALGOL 60 — static start, step, and limit values prove this ascending
    // step loop enters its body. The assignment therefore establishes definite
    // string initialization for the print after the loop on every backend.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, result; string s; for i := 1 step 1 until 2 do s := 'OK'; if s = 'OK' then result := 42 else result := 0 end",
        expect: Expect::Exit(42),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — straight-line integer snapshots can prove a later step-loop
    // header nonempty before loop lowering invalidates numeric metadata.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, first, stride, last, result; string s; first := 1; stride := 1; last := 2; for i := first step stride until last do s := 'OK'; if s = 'OK' then result := 42 else result := 0 end",
        expect: Expect::Exit(42),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    ...
    // ALGOL 60 — a static selector preserves the reachable numeric branch's
    // snapshot even when the conditional expression's branches differ.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin boolean flag; real x; flag := false; x := if flag then 1.5 else 42.0; print(x) end",
        expect: Expect::Stdout("42"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — a statically unreachable procedure result does not require
    // runtime real formatting when the selected output branch is a literal.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin real procedure choose; choose := 1.5; boolean flag; flag := false; print(if flag then choose() else 42.0) end",
    ...
    // in the loop preheader before testing the bound.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin real x; x := 9.0; for x := 2.0 step 1.0 until 1.0 do x := 1.5; print(x) end",
        expect: Expect::Stdout("2"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    ...
    // ALGOL 60 — bounded simulation mirrors each emitted f64 addition and
    // retains the first real control value beyond the limit.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin real x; for x := 1.0 step 0.5 until 2.0 do print(''); print(x + 0.25) end",
        expect: Expect::Stdout("2.75"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — each finite iteration establishes the same body constant
    // independently of the controlled scalar.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i; real r; for i := 1 step 1 until 3 do r := 6.25; print(r) end",
        expect: Expect::Stdout("6.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    ...
    // ALGOL 60 — bounded step analysis simulates a simple scalar recurrence
    // once per iteration, retaining its exact final formatter-free value.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i; real r; r := 0.25; for i := 1 step 1 until 3 do r := r + i; print(r) end",
        expect: Expect::Stdout("6.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — the exact single-assignment extractor also recognizes a
    // recurrence wrapped in one compound statement.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i; real r; boolean flag; flag := false; for i := 1 step 1 until 3 do begin flag := not flag end; if flag then r := 42.0 else r := 0.5; print(r) end",
        expect: Expect::Stdout("42"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    ...
    // ALGOL 60 — an exact while recurrence remains analyzable when every
    // additional compound-body statement is an inert local scalar assignment.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, pad, hold; real r; boolean flag; i := 0; pad := 7; hold := 9; flag := false; for i := i + 1 while i <= 3 do begin pad := pad; flag := not flag; hold := hold end; if flag then r := 42.0 else r := 0.5; print(r) end",
        expect: Expect::Stdout("42"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    ...
    // whose while predicate applies an exact standard function to a stable
    // local dependency that is read but not written by the body.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n; n := -3; i := 0; for i := i + 1 while i < abs(n) do if n = -3 then print(''); print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — capped abstract execution mirrors each emitted binary64
    // update and retains the first real control value whose predicate is false.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin real x; x := 0.0; for x := x + 0.5 while x < 2.0 do print(''); print(x + 0.25) end",
    ...
    // ALGOL 60 — a complete integer identity also leaves a transitive value
    // dependency stable without recursive effect inference.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit; n := 3; limit := 3; i := 0; for i := i + 1 while i < n do begin n := limit; limit := 0 + limit * 1 div 1 - 0 end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — a real dependency makes the while comparison mixed numeric;
    // the tracked integer side widens only while it remains exact in binary64.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i; real n, limit; n := 3.0; limit := 3.0; i := 0; for i := i + 1 while i < n do begin n := limit; limit := limit end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    ...
    // ALGOL 60 — an exact scalar self-assignment leaves a known selector
    // stable while it chooses the preserving transitive dependency leaf.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit; boolean choose; n := 3; limit := 3; choose := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose then limit else limit + 1; choose := choose end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — equal bare-self conditional leaves leave a known selector
    // stable even when their selector is dynamic.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit; boolean choose, other; n := 3; limit := 3; choose := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose then limit else limit + 1; choose := if other then choose else choose end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — a variable-free static conditional assignment selector may
    // discard a changing leaf while leaving a known selector stable.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
    ...
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit, choose; boolean other; n := 3; limit := 3; choose := 1; other := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1 then limit else limit + 1; choose := if other then choose else 0 end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — an exact self-assignment leaves a known conditional-selector
    // dependency stable while its predicate selects the preserving leaf.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit, choose; boolean other; n := 3; limit := 3; choose := 1; other := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1 then limit else limit + 1; choose := if other then choose else 0; other := other end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — equal bare-self conditional leaves preserve a known
    // conditional-selector dependency even when their selector is dynamic.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit, choose; boolean other, flag; n := 3; limit := 3; choose := 1; other := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1 then limit else limit + 1; choose := if other then choose else 0; other := if flag then other else other end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — an unchanged known scalar may select the preserving leaf of
    // a conditional assignment to a conditional-selector dependency.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit, choose; boolean other, flag; n := 3; limit := 3; choose := 1; other := true; flag := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1 then limit else limit + 1; choose := if other then choose else 0; other := if flag then other else false; flag := flag end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — a second unchanged known selector may preserve the next
    // dependency in the bounded conditional-selector chain.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit, choose; boolean other, flag, gate; n := 3; limit := 3; choose := 1; other := true; flag := true; gate := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1 then limit else limit + 1; choose := if other then choose else 0; other := if flag then other else false; flag := if gate then flag else false; gate := gate end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    ...
    // ALGOL 60 — a tenth unchanged known selector may preserve the next
    // dependency while the analysis remains explicitly depth-bounded.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
    ...
    // ALGOL 60 — a computed boolean identity write cannot change the selector,
    // so its transitive dependency remains stable during bounded loop analysis.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit, choose; boolean other, flag; n := 3; limit := 3; choose := 1; other := true; flag := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1 then limit else limit + 1; choose := if other then choose else 0; other := if flag then other else false; flag := flag and true end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — a computed integer identity write cannot change the selector,
    // so its transitive dependency remains stable during bounded loop analysis.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit, choose; boolean other; n := 3; limit := 3; choose := 1; other := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1 then limit else limit + 1; choose := if other then choose else 0; other := other; choose := choose + 0 end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — multiplication by exact positive one preserves a finite real
    // selector without invoking the unsound additive-zero identity.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit; real choose; boolean other; n := 3; limit := 3; choose := 1.0; other := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1.0 then limit else limit + 1; choose := if other then choose else 0.0; other := other; choose := choose * 1.0 end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — subtracting positive zero preserves every finite real,
    // including negative zero, so a selector dependency remains stable.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit; real choose; boolean other; n := 3; limit := 3; choose := 1.0; other := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1.0 then limit else limit + 1; choose := if other then choose else 0.0; other := other; choose := choose - 0.0 end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — adding negative real zero is the sign-symmetric exact
    // identity, so this selector dependency also remains stable.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit; real choose; boolean other; n := 3; limit := 3; choose := 1.0; other := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1.0 then limit else limit + 1; choose := if other then choose else 0.0; other := other; choose := choose + (-0.0) end; print(i + 0.25) end",
    ...
    // ALGOL 60 — a grouped literal-only subtraction can produce exact
    // positive zero, which remains safe on the subtractive identity path.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit; real choose; boolean other; n := 3; limit := 3; choose := 1.0; other := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1.0 then limit else limit + 1; choose := if other then choose else 0.0; other := other; choose := choose - ((-0.0) - (-0.0)) end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — exact positive units preserve a finite-real selector as a chain.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit; real choose; boolean other; n := 3; limit := 3; choose := 1.0; other := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1.0 then limit else limit + 1; choose := if other then choose else 0.0; other := other; choose := 1.0 * choose * 1.0 / 1.0 end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — an even pair of exact negative-unit operations preserves a
    // finite-real selector, including the sign bit of zero.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit; real choose; boolean other; n := 3; limit := 3; choose := 1.0; other := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1.0 then limit else limit + 1; choose := if other then choose else 0.0; other := other; choose := (-1.0) * choose / (-1.0) end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    ...
    // ALGOL 60 — a negative unit raised to an even literal power is an exact
    // positive unit and therefore preserves a finite-real selector.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit; real choose; boolean other; n := 3; limit := 3; choose := 1.0; other := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1.0 then limit else limit + 1; choose := if other then choose else 0.0; other := other; choose := choose * ((-1.0) ^ 2) end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — an even positive power of negative zero is exact positive
    // zero and therefore preserves a finite-real selector under subtraction.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit; real choose; boolean other; n := 3; limit := 3; choose := 1.0; other := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1.0 then limit else limit + 1; choose := if other then choose else 0.0; other := other; choose := choose - ((-0.0) ^ 2) end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — the bounded unrolled power path returns exact positive one
    // for a literal base with a complete zero exponent chain.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit; real choose; boolean other; n := 3; limit := 3; choose := 1.0; other := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1.0 then limit else limit + 1; choose := if other then choose else 0.0; other := other; choose := choose * (2.0 ^ 0) end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — a finite literal-only arithmetic base is still pure, so the
    // bounded zero-exponent path returns an exact positive unit.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit; real choose; boolean other; n := 3; limit := 3; choose := 1.0; other := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1.0 then limit else limit + 1; choose := if other then choose else 0.0; other := other; choose := choose * ((2.0 + 3.0) ^ 0) end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — an integral real-literal exponent chain may also evaluate to
    // zero while its finite literal-only base remains pure.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit; real choose; boolean other; n := 3; limit := 3; choose := 1.0; other := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1.0 then limit else limit + 1; choose := if other then choose else 0.0; other := other; choose := choose * ((2.0 + 3.0) ^ 0.0) end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    ...
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit, choose; boolean other, flag; n := 3; limit := 3; choose := 1; other := true; flag := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1 then limit else limit + 1; choose := if other then choose else 0; other := if flag then other else false; flag := not not flag end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    ...
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit, choose; boolean other; n := 3; limit := 3; choose := 1; other := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1 then limit else limit + 1; choose := if other then choose else 0; other := other; choose := 0 + choose * 1 div 1 - 0 end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — a literal exponent chain equal to one lowers to its base.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit, choose; boolean other; n := 3; limit := 3; choose := 1; other := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1 then limit else limit + 1; choose := if other then choose else 0; other := other; choose := choose ^ 1 ^ 1 end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — checked integer arithmetic exponent operands retain the
    // bounded integer power path and preserve an integer selector exactly.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit, choose; boolean other; n := 3; limit := 3; choose := 1; other := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1 then limit else limit + 1; choose := if other then choose else 0; other := other; choose := choose ^ ((6 div 3) - 1) end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — checked exponent arithmetic recurses through bounded power
    // operands while preserving the integer selector and result type.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit, choose; boolean other; n := 3; limit := 3; choose := 1; other := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1 then limit else limit + 1; choose := if other then choose else 0; other := other; choose := choose ^ ((2 ^ (2 - 1)) div 2) end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — a bounded integral real-literal exponent chain equal to one
    // preserves a finite-real selector on the runtime power path.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit; real choose; boolean other; n := 3; limit := 3; choose := 1.0; other := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1.0 then limit else limit + 1; choose := if other then choose else 0.0; other := other; choose := choose ^ 1.0 ^ 1 end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — a variable-free literal arithmetic exponent equal to one
    // preserves a finite-real selector on the runtime power path.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit; real choose; boolean other; n := 3; limit := 3; choose := 1.0; other := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1.0 then limit else limit + 1; choose := if other then choose else 0.0; other := other; choose := choose ^ (2.0 - 1.0) end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — checked integer literal arithmetic may also produce the
    // exact unit exponent for a finite-real selector.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit; real choose; boolean other; n := 3; limit := 3; choose := 1.0; other := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1.0 then limit else limit + 1; choose := if other then choose else 0.0; other := other; choose := choose ^ (2 - 1) end; print(i + 0.25) end",
        expect: Expect::Stdout("3.25"),
        backends: &[NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit],
    },
    // ALGOL 60 — leading unary plus is an exact no-op before an identity tail.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit, choose; boolean other; n := 3; limit := 3; choose := 1; other := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1 then limit else limit + 1; choose := if other then choose else 0; other := other; choose := +choose + 0 end; print(i + 0.25) end",
    ...
    // ALGOL 60 — parentheses preserve exact self-references and neutral literals.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
    ...
    // ALGOL 60 — signed neutral literals retain their exact identity values.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer i, n, limit, choose; boolean other; n := 3; limit := 3; choose := 1; other := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1 then limit else limit + 1; choose := if other then choose else 0; other := other; choose := (+0) + choose * (+1) div (+1) - (-0) end; print(i + 0.25) end",
    ...
    // ALGOL 60 — unconditional identity leaves do not consume an extra
    // selector-dependency level at the bounded proof cap.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
    ...
    //
    // BEAM03: this is BASIC's own paradigm case for the just-added `iir-to-beam`
    // f64 lowering (const/add/sub/mul/div/cmp_*/int_to_real/real_to_int_trunc,
    // see BEAM03-float-lowering.md) — the numeric baseline row the VM-040 BASIC
    // BEAM pure-string slice explicitly deferred. `__basic_print_real`'s sign/
    // zero/magnitude-bucket dispatch and its `real_to_int_trunc` digit-extraction
    // chain both exercise real, non-synthetic float lowering, not a bespoke probe.
    Prog {
    ...
    //
    // BEAM03: the second promoted numeric-baseline row — `dartmouth-basic-iir-
    // compiler`'s `literal_power_lowers_to_repeated_f64_mul` fast path emits one
    // `mul` (base * base) then a top-level `add`, so this exercises float `mul`/
    // `add` beyond what the bare-literal row above needs.
    Prog {
        lang: Language::DartmouthBasic,
        ext: "bas",
        src: "10 PRINT 6 ^ 2 + 6\n20 END\n",
...
#[test]
fn algol_integer_arithmetic_exponent_snapshot_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("n := 6 ^ ((2 ^ (2 - 1)) div 2) + 36; saved := n")
        })
        .expect("the ALGOL integer arithmetic-exponent snapshot must remain in the matrix");
...
#[test]
fn algol_tracked_arithmetic_real_power_exponents_run_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("saved := 6.0 ^ (0 - exponent) + 6.0; exponent := 3")
        })
        .expect(
            "the ALGOL tracked-arithmetic real-power exponent program must remain in the matrix",
        );
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but tracked-arithmetic real-power exponent did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_tracked_entier_real_power_exponents_run_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("entier(abs(exponent) + sign(exponent) + 1)")
        })
        .expect("the ALGOL tracked entier exponent program must remain in the matrix");
...
#[test]
fn algol_composed_tracked_sqrt_real_power_exponents_run_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("sqrt(exponent) + sign(exponent) - 1")
        })
        .expect("the ALGOL composed tracked sqrt exponent program must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the composed tracked sqrt exponent did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_conditional_tracked_sqrt_real_power_exponents_run_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("then sqrt(exponent) else sqrt(exponent)")
        })
        .expect("the ALGOL conditional tracked sqrt exponent program must remain in the matrix");
...
#[test]
fn algol_tracked_real_standard_function_exponents_run_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("real exponent, saved; exponent := 0.0")
                && program.src.contains("6.0 ^ (cos(exponent) + 1)")
        })
        .expect("the ALGOL tracked real standard-function program must remain in the matrix");
...
#[test]
fn algol_path_independent_standard_function_results_run_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("abs(if gate = 0.0 then exponent else -exponent)")
        })
        .expect("the path-independent standard-function result must remain in the matrix");
...
#[test]
fn algol_path_independent_standard_result_arithmetic_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains(
                    "entier(abs(if gate = 0.0 then exponent else -exponent) + 0.5)",
                )
        })
        .expect("the path-independent standard-result arithmetic program must remain in the matrix");
...
#[test]
fn algol_tracked_real_standard_result_arithmetic_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                ...
                && program.src.contains("offset := 0.0")
        })
        .expect("tracked-real standard-result arithmetic must remain in the matrix");
...
#[test]
fn algol_tracked_integer_powered_standard_results_run_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains(
                    "entier((abs(if gate = 0.0 then exponent else -exponent) + 0.5) ^ power)",
                )
        })
        .expect("tracked-integer powered standard result must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the tracked-integer powered standard result did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_tracked_real_powered_standard_results_run_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                ...
                && program.src.contains("power := 1.0")
        })
        .expect("tracked-real powered standard result must remain in the matrix");
...
#[test]
fn algol_nested_conditional_real_snapshots_run_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains("if outer then if inner then first")
        })
        .expect("nested conditional real snapshots must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but nested real snapshots did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_conditional_real_snapshot_literals_run_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains("if gate then exponent else 2.0")
        })
        .expect("conditional real snapshot/literal power must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but snapshot/literal power did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_conditional_real_snapshot_literal_arithmetic_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains("else 1.0 + 1.0) + 6.0")
        })
        .expect("conditional snapshot/literal arithmetic power must remain in the matrix");
...
#[test]
fn algol_standard_free_tracked_real_arithmetic_powers_run_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("saved := 6.0 ^ (power + offset) + 6.0")
                && program.src.contains("offset := 1.5")
        })
        .expect("standard-free tracked-real arithmetic power must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but standard-free tracked-real arithmetic did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_conditional_tracked_real_arithmetic_powers_run_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains(
                    "if gate = 0.0 then power + offset else offset + power",
                )
                && program.src.contains("power := 0.5")
        })
        .expect("conditional tracked-real arithmetic power must remain in the matrix");
...
#[test]
fn algol_mixed_tracked_numeric_arithmetic_powers_run_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains("^ (whole + part)")
                && program.src.contains("whole := 1")
        })
        .expect("mixed tracked numeric arithmetic power must remain in the matrix");
...
#[test]
fn algol_standard_free_conditional_mixed_tracked_numeric_powers_run_on_every_available_standard_backend()
{
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains(
                    "saved := 6.0 ^ (if gate = 0.0 then whole + part else part + whole) + 6.0",
                )
        })
        .expect("standard-free conditional mixed tracked numeric power must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but standard-free conditional mixed arithmetic did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_mutual_recursive_formal_procedure_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains("integer procedure even(p, n)")
                && program.src.contains("integer procedure odd(p, n)")
                && program.src.contains("result := even(twice, 4)")
        })
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but mutual-recursive formal-procedure execution did not complete"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_standard_output_formal_procedure_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains("dispatch(print, 'FORMAL')")
                && program.src.contains("procedure forward(p, s)")
        })
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but standard-output formal-procedure execution did not complete"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_call_by_name_string_forwarding_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains("integer procedure forward(x); string x")
                && program.src.contains("integer procedure consume(x); string x")
                && program.src.contains("s := 'NO'; result := forward(s)")
        })
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but string call-by-name forwarding did not complete"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_static_step_loop_initialization_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains("for i := 1 step 1 until 2")
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but static step-loop initialization did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_tracked_step_bounds_run_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains("for i := first step stride until last")
                && program.src.contains("string s")
        })
...
fn algol_static_statement_condition_runs_on_every_available_standard_backend() {
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but truth-dominating while predicate did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_static_conditional_assignment_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains("x := if flag then 1.5 else 42.0")
        })
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the static conditional assignment did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_static_conditional_real_output_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("print(if flag then choose() else 42.0)")
        })
...
#[test]
fn algol_zero_trip_controlled_snapshot_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("for x := 2.0 step 1.0 until 1.0 do x := 1.5")
        })
...
#[test]
fn algol_single_step_control_snapshot_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("for x := 1.0 step 1.0 until 1.0 do x := 6.25")
        })
...
#[test]
fn algol_finite_real_step_control_exit_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("for x := 1.0 step 0.5 until 2.0 do print('')")
        })
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the finite real step control exit did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_finite_loop_body_snapshot_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("for i := 1 step 1 until 3 do r := 6.25")
        })
...
#[test]
fn algol_step_loop_scalar_recurrence_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("for i := 1 step 1 until 3 do r := r + i")
        })
        .expect("the ALGOL step-loop scalar recurrence must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the step-loop scalar recurrence did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_step_loop_single_compound_recurrence_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("step 1 until 3 do begin flag := not flag end")
        })
        .expect("the ALGOL compound step-loop recurrence must remain in the matrix");
...
#[test]
fn algol_inert_sibling_while_recurrence_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains("pad := pad; flag := not flag; hold := hold")
        })
        .expect("the inert-sibling while recurrence must remain in the matrix");
...
#[test]
fn algol_static_while_control_exit_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("while i < abs(n) do if n = -3 then print('')")
        })
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the static while control exit did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_static_real_while_control_exit_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("for x := x + 0.5 while x < 2.0 do print('')")
        })
...
#[test]
fn algol_transitive_identity_while_dependency_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("n := limit; limit := 0 + limit * 1 div 1 - 0")
        })
        .expect("the ALGOL transitive identity dependency program must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the transitive identity dependency did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_real_transitive_while_dependency_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("real n, limit; n := 3.0; limit := 3.0")
        })
        .expect("the ALGOL real transitive dependency program must remain in the matrix");
...
#[test]
fn algol_idempotent_transitive_selector_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains(
                    "limit := if choose then limit else limit + 1; choose := choose end",
                )
        })
        .expect("the ALGOL idempotent transitive selector program must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the idempotent transitive selector did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_conditional_transitive_selector_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains(
                    "choose := if other then choose else choose end; print(i + 0.25)",
                )
        })
        .expect("the ALGOL conditional transitive selector program must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the conditional transitive selector did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_static_conditional_selector_assignment_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains(
                    "choose := if true then choose else false end; print(i + 0.25)",
                )
        })
        .expect("the ALGOL static conditional selector assignment must remain in the matrix");
...
#[test]
fn algol_stable_conditional_selector_assignment_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains(
                    "other := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1 then limit else limit + 1; choose := if other then choose else 0 end",
                )
        })
        .expect("the ALGOL stable conditional selector assignment must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jit, Jvm, Clr, Vm] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the stable conditional selector assignment did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_idempotent_conditional_selector_dependency_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains(
                    "choose := if other then choose else 0; other := other end",
                )
        })
        .expect("the ALGOL idempotent conditional selector dependency must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jit, Jvm, Clr, Vm] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the idempotent conditional selector dependency did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_conditional_idempotent_selector_dependency_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains(
                    "other := if flag then other else other end",
                )
        })
        .expect("the ALGOL conditional idempotent selector dependency must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jit, Jvm, Clr, Vm] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the conditional idempotent selector dependency did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_stable_nested_selector_dependency_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains(
                    "other := if flag then other else false; flag := flag end",
                )
        })
        .expect("the ALGOL stable nested selector dependency must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jit, Jvm, Clr, Vm] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the stable nested selector dependency did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_two_level_selector_dependencies_run_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains(
                    "flag := if gate then flag else false; gate := gate end",
                )
        })
        .expect("the ALGOL two-level selector dependencies must remain in the matrix");
...
#[test]
fn algol_ten_level_selector_dependencies_run_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains(
                    "frontier := if boundary then frontier else false; boundary := boundary end",
                )
        })
        .expect("the ALGOL ten-level selector dependencies must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jit, Jvm, Clr, Vm] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the ten-level selector dependencies did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_boolean_identity_selector_write_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains("flag := flag and true end")
        })
        .expect("the ALGOL boolean-identity selector program must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the boolean-identity selector did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_integer_identity_selector_write_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains("choose := choose + 0 end")
        })
        .expect("the ALGOL integer-identity selector program must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the integer-identity selector did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_real_unit_selector_write_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains("choose := choose * 1.0 end")
        })
        .expect("the ALGOL real-unit selector program must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the real-unit selector did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_real_subtractive_zero_selector_write_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains("choose := choose - 0.0 end")
        })
        .expect("the ALGOL real subtractive-zero selector program must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the real subtractive-zero selector did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_real_additive_negative_zero_selector_write_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
        ...
        .expect("the ALGOL real additive-negative-zero selector program must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the real additive-negative-zero selector did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_real_nested_zero_identity_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("choose := choose - ((-0.0) - (-0.0))")
        })
        .expect("the ALGOL nested zero selector identity must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the nested zero selector identity did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_real_unit_selector_chain_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains("choose := 1.0 * choose * 1.0 / 1.0")
        })
        .expect("the ALGOL real-unit selector-chain program must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the real-unit selector chain did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_real_signed_unit_selector_chain_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("choose := (-1.0) * choose / (-1.0)")
        })
        .expect("the ALGOL real signed-unit selector chain must remain in the matrix");
...
#[test]
fn algol_real_power_unit_identity_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains("choose := choose * ((-1.0) ^ 2)")
        })
        .expect("the ALGOL real power-unit selector identity must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the real power-unit selector identity did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_real_power_zero_identity_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains("choose := choose - ((-0.0) ^ 2)")
        })
        .expect("the ALGOL real power-zero selector identity must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the real power-zero selector identity did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_real_zero_exponent_unit_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains("choose := choose * (2.0 ^ 0)")
        })
        .expect("the ALGOL real zero-exponent unit must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the real zero-exponent unit did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_real_zero_exponent_literal_expression_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("choose := choose * ((2.0 + 3.0) ^ 0)")
        })
        .expect("the ALGOL real zero-exponent literal expression must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the real zero-exponent literal expression did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_real_literal_zero_exponent_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("choose := choose * ((2.0 + 3.0) ^ 0.0)")
        })
        .expect("the ALGOL real-literal zero exponent must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the real-literal zero exponent did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_even_boolean_negation_selector_write_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains("flag := not not flag end")
        })
        .expect("the ALGOL even-negation selector program must remain in the matrix");
...
#[test]
fn algol_integer_identity_selector_chain_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains("choose := 0 + choose * 1 div 1 - 0")
        })
        .expect("the ALGOL integer identity-chain program must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the integer identity chain did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_power_one_selector_write_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains("choose := choose ^ 1 ^ 1")
        })
        .expect("the ALGOL power-one selector program must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the power-one selector did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_integer_checked_exponent_selector_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("choose := choose ^ ((6 div 3) - 1)")
        })
        .expect("the ALGOL integer checked-exponent selector must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the integer checked-exponent selector did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_integer_nested_checked_exponent_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("choose := choose ^ ((2 ^ (2 - 1)) div 2)")
        })
        .expect("the ALGOL nested checked-exponent selector must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the nested checked-exponent selector did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_real_integral_power_one_selector_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains("choose := choose ^ 1.0 ^ 1")
        })
        .expect("the ALGOL real integral power-one selector must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the real integral power-one selector did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_real_arithmetic_power_one_selector_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("choose := choose ^ (2.0 - 1.0)")
        })
        .expect("the ALGOL real arithmetic power-one selector must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the real arithmetic power-one selector did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_checked_integer_power_one_selector_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains("choose := choose ^ (2 - 1)")
        })
        .expect("the ALGOL checked integer power-one selector must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the checked integer power-one selector did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_unary_plus_selector_write_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program.src.contains("choose := +choose + 0")
        })
        .expect("the ALGOL unary-plus selector program must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the unary-plus selector did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_parenthesized_selector_write_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
        ...
        .expect("the ALGOL parenthesized selector program must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the parenthesized selector did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn algol_signed_neutral_selector_write_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("choose := (+0) + choose * (+1) div (+1) - (-0)")
        })
        .expect("the ALGOL signed-neutral selector program must remain in the matrix");
...
#[test]
fn algol_conditional_identity_selector_write_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("boundary := if threshold then boundary and true else false or boundary")
        })
        .expect("the ALGOL conditional identity selector must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but the conditional identity selector did not run"
            );
            continue;
        };
        assert_cell(backend, program, result);
...
#[test]
fn portable_text_stdout_nib_beam_corpus() {
    if !erl_ok() {
        eprintln!("SKIP Nib BEAM corpus: erl unavailable");
        return;
    }
    let mut executed = 0;
    for program in PROGRAMS.iter().filter(|p| p.lang == Language::Nib) {
        let result = run_beam(program).expect("detected erl must execute Nib");
        assert_cell(Beam, program, result);
        executed += 1;
    }
...
// BEAM03: the two numeric-baseline Dartmouth BASIC rows, promoted once
// `iir-to-beam` gained f64 lowering (see BEAM03-float-lowering.md). These are
// the two rows immediately BEFORE the 18-row pure-string family above (which
// intentionally skipped them — see that slice's own "why this slice does not
// close all of it" note). Real Erlang execution proves the full print-helper
// op chain (const/add/mul/sub/div/cmp_lt/cmp_eq/cmp_ge/int_to_real/
// real_to_int_trunc), not just that a float const loads.
#[test]
fn portable_text_stdout_dartmouth_basic_beam_numeric_baseline() {
    if !erl_ok() {
        eprintln!("SKIP Dartmouth BASIC BEAM numeric baseline: erl unavailable");
        return;
    }
    let programs: Vec<_> = PROGRAMS.iter()
        .filter(|p| p.lang == Language::DartmouthBasic)
