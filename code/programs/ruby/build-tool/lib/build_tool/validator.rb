    ...
    def validate_lua_isolated_build_files(packages)
      packages.filter_map do |pkg|
        ...
        lua_build_files(pkg.path).flat_map do |build_path|
          ...
          if guarded_local_lua_install?(lines) && !self_install_disables_deps?(lines, self_rock)
            errors << "#{build_path.to_s.tr('\\', '/')}: Lua BUILD uses guarded sibling rock " \
                      "installs but the final self-install does not pass --deps-mode=none or --no-manifest"
          end
