...
When detecting a new block type on a line, GFM has a strict priority:
...
from coding_adventures_gfm_parser.entities import decode_entities
from coding_adventures_gfm_parser.scanner import (
...
def _apply_backslash_escapes(s: str) -> str:
    ...
    Per GFM §2.4: any ASCII punctuation character may be backslash-escaped.
...
#
# GFM defines 7 types of HTML blocks. Each has different opening
...
# GFM spec §4.6: type-2 HTML blocks end at --> (standard comment close).
...
def _extract_info_string(line: str) -> str:
    ...
    Per GFM: only the first word is the language.
...
def convert_to_ast(
    ...
    def convert_block(block: MutableBlock) -> BlockNode | None:
        ...
        if block.kind == "document":
            ...
            if not block.lines:
                return None
            # Strip leading whitespace from each line per the GFM spec.
...
def apply_gfm_block_extensions(
    document: DocumentNode,
    raw_inline_content: dict[int, str],
) -> DocumentNode:
    """Upgrade GFM blocks into richer GFM nodes."""

    def transform_blocks(blocks: list[BlockNode]) -> list[BlockNode]:
        return [transform_block(block) for block in blocks]

    def transform_block(block: BlockNode) -> BlockNode:
        block_type = block["type"]
        if block_type == "document":
            return {"type": "document", "children": transform_blocks(block["children"])}
        if block_type == "blockquote":
            return {"type": "blockquote", "children": transform_blocks(block["children"])}
        if block_type == "list":
            return {
                "type": "list",
                "ordered": block["ordered"],
                "start": block["start"],
                "tight": block["tight"],
                "children": [transform_list_child(item) for item in block["children"]],
            }
        if block_type in ("list_item", "task_item"):
            return transform_list_child(block)
        if block_type == "paragraph":
            table = maybe_transform_paragraph_to_table(block, raw_inline_content)
            return table or block
        return block

    def transform_list_child(item: ListItemNode | TaskItemNode) -> ListItemNode | TaskItemNode:
        children = transform_blocks(item["children"])
        next_item: ListItemNode | TaskItemNode
        if item["type"] == "task_item":
            next_item = {"type": "task_item", "checked": item["checked"], "children": children}
        else:
            next_item = {"type": "list_item", "children": children}

        if not children or children[0]["type"] != "paragraph":
            return next_item

        raw = raw_inline_content.get(id(children[0]))
        if raw is None:
            return next_item

        match = re.match(r"^\[( |x|X)\](?:[ \t]+|$)", raw)
        if match is None:
            return next_item

        raw_inline_content[id(children[0])] = raw[match.end():]
        return {"type": "task_item", "checked": match.group(1).lower() == "x", "children": children}

    document["children"] = transform_blocks(document["children"])
    return document


def maybe_transform_paragraph_to_table(
    block: ParagraphNode,
    raw_inline_content: dict[int, str],
) -> TableNode | None:
    raw = raw_inline_content.get(id(block))
    if raw is None:
        return None

    parsed = try_parse_table(raw)
    if parsed is None:
        return None

    def make_cell(content: str) -> TableCellNode:
        cell: TableCellNode = {"type": "table_cell", "children": []}
        raw_inline_content[id(cell)] = content
        return cell

    def make_row(cells: list[str], is_header: bool) -> TableRowNode:
        return {
            "type": "table_row",
            "isHeader": is_header,
            "children": [make_cell(cell) for cell in cells],
        }

    del raw_inline_content[id(block)]
    return {
        "type": "table",
        "align": parsed["align"],
        "children": [
            make_row(parsed["header"], True),
            *[make_row(row, False) for row in parsed["rows"]],
        ],
    }


def try_parse_table(raw: str) -> dict[str, list] | None:
    lines = raw.split("\n")
    if len(lines) < 2:
        return None
    ...
    if header_cells is None or delimiter_cells is None:
        return None
    if not header_cells or len(header_cells) != len(delimiter_cells):
        return None

    align: list[TableAlignment] = []
    for cell in delimiter_cells:
        trimmed = cell.strip()
        if re.match(r"^:?-{3,}:?$", trimmed) is None:
            return None
    ...
    for line in lines[2:]:
        ...
        if cells is None:
            return None
        rows.append(normalize_table_row(cells, len(header_cells)))

    return {
        "align": align,
        "header": normalize_table_row(header_cells, len(delimiter_cells)),
        "rows": rows,
    }
...
def split_table_row(line: str) -> list[str] | None:
    ...
    stripped = line.strip()
    had_outer_pipe = stripped.startswith("|") or stripped.endswith("|")
    start = 0
    end = len(line)
    while start < end and is_ascii_whitespace(line[start]):
        start += 1
    while end > start and is_ascii_whitespace(line[end - 1]):
        end -= 1
    ...
    cells.append("".join(current).strip())
    if pipe_count == 0 and not had_outer_pipe:
        return None
    return cells


def normalize_table_row(cells: list[str], width: int) -> list[str]:
