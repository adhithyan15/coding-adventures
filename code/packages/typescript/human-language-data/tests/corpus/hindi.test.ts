...
it("pins Hindi lesson-content budgets", () =>
  expectLanguageLessonBudgets("hindi", {
    ...
    lessons: 506, // 501 -> 506 (chapter 105, address and the written message). THE FIRST DRAFT HAD SIX LESSONS AND ONE OF THEM WAS MISPLACED BY EIGHTY-THREE LESSONS: HI-C97-sunie taught सुनिए as the idiomatic attention-getter, and forward-language went 22 -> 23 because HI-C85-sintesis-suchna already prints कृपया सुनिए at chapter 93, where the reader builds it productively from the respectful -iye ending that chapter teaches. Teaching it as a lexical item twelve chapters later is backwards, so it was withdrawn rather than reworded; HL-C389 carries the sense to a lesson beside chapter 93. It also carried the chapter's only script-closure debt, the independent vowel i spelled out in the phrase -इए. THE WRITING LESSON WAS SPLIT IN TWO for two separate reasons that the snapshot diff surfaced together: as one lesson it computed 301s against the 300s ceiling, and its atom was introduced in the chapter's LAST lesson, so nothing could revisit it and atomsNeverRevisited went 33 -> 34. HI-C97-sandesh now introduces the message shape with a model in front of the reader and HI-C97-sintesis-sandesh produces it from a cue alone under an explicit zero-new contract. // 495 -> 501 (chapter 104, the tens, plus sau moved into the money chapter). NUM-03 IS DELIBERATELY NOT PROBED: the chapter teaches the seven rungs thirty to ninety and chapter 79 teaches sau, which covers every price, age and house number that LANDS ON a rung including mock 2's pachaas rupaye, but not the seventy-two eroded words between them. HL-C386 is the precedent for not claiming a range the corpus does not teach. SAU WAS DRAFTED IN CHAPTER 104 AND MOVED: HI-C75-fifth teaches sauvaan (hundredth) at chapter 82, so teaching sau at 104 made that a forward reference and forward-language went 22 -> 23. Moving it to chapter 79 fixed the ordering rather than the metric -- sau now sits beside rupayaa where a price needs it, and HI-C75-fifth pays it off. // 359 -> 360: HI-C79-dopahar-raat-ka-khana // 360 -> 362: HI-C80-aasaan-mushkil, HI-C80-sundar-badsurat // 362 -> 366: HI-C81 school pair plus its two retrieval lessons
    idioms: 21,
...
it("pins Hindi A1 coverage, and the numeral column the ordinal tranche moved", () => {
  ...
  expect(formatExamCoverage(coverage)).toContain(
    "hindi A1 (partial inventory): 239/282 points covered (85%)",
  );
}, 60_000);
