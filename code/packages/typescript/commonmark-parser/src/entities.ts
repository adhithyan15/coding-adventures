 ...
 *
 * There are ~2200 HTML5 named entities. We include all of them here for
...
// ─── Named Entity Table ────────────────────────────────────────────────────────
//
// This is a subset of the full HTML5 named character references table,
// covering the entities most commonly encountered in Markdown documents.
...
/**
 * Decode a single HTML character reference, including the surrounding
 * `&` and `;`. Returns the decoded character string, or the original
 * input if the reference is unrecognised.
 *
 ...
 * decodeEntity("&#65;")   // "A"
 * decodeEntity("&#x41;")  // "A"
 * decodeEntity("&bogus;") // "&bogus;"  (unrecognised — returned as-is)
 * ```
 */
export function decodeEntity(ref: string): string {
  if (!ref.startsWith("&") || !ref.endsWith(";")) {
    return ref;
  }

  const inner = ref.slice(1, -1); // strip & and ;
  ...
  if (inner.startsWith("#")) {
    ...
    if (rest.startsWith("x") || rest.startsWith("X")) {
      // Hex reference: &#xHHH;
      codePoint = parseInt(rest.slice(1), 16);
    } else {
      // Decimal reference: &#NNN;
      codePoint = parseInt(rest, 10);
    }

    // Invalid code point — return original
    if (isNaN(codePoint) || codePoint === 0 || codePoint > 0x10ffff) {
      return "\uFFFD"; // CommonMark replaces invalid with replacement char
    }

    // U+0000 is replaced per CommonMark spec
...
export function decodeEntities(text: string): string {
  ...
  // Replace all &...; patterns. The regex matches:
  //   &name;    — named entity (1-32 alphanumeric chars)
  //   &#DDD;    — decimal numeric entity
  //   &#xHHH;   — hex numeric entity
  return text.replace(/&(?:#[xX][0-9a-fA-F]{1,6}|#[0-9]{1,7}|[a-zA-Z][a-zA-Z0-9]{0,31});/g, decodeEntity);
}

/**
 * Encode characters that must be escaped in HTML attribute values and
 * text content. Used by the renderer to produce safe HTML output.
 *
 * Characters escaped: & < > "
 *
 * Note: we do NOT escape apostrophes here because CommonMark's reference
