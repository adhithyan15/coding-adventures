...
fn parses_bjt_models_into_operating_point_circuits() {
    let parsed = parse_netlist(
        r#"
.model fast NPN(IS=1e-13 BF=120 VT=26m CJE=2p CJC=3p TF=4n TR=5n XTI=2.5 EG=1.05 VAF=75 VA=120 VAR=60 VB=90 IKF=3m IK=5m IKR=4m TNOM=27 T_NOM=35 KF=2p AF=1.3 PTF=30 XTF=0.4 ITF=2m VTF=0.6 RE=5 RC=7 RB=9 RBM=4 IRB=2m XCJC=0.6 ISE=2p NE=1.5 ISC=3p NC=2.5 XTB=-0.3 BR=2 BETA_R=-1 NR=1.4 NF=1.2 VJE=0.8 PE=-1)
Vcc vcc 0 DC 5
Vbase base 0 DC 0.7
...
"#,
    ...
    assert_close(bjt.energy_gap_electron_volts, 1.05);
    assert_close(bjt.forward_early_voltage, 75.0);
    assert_close(bjt.reverse_early_voltage, 60.0);
...
#[test]
fn rejects_invalid_bjt_energy_gap() {
    for value in ["0", "-0.1", "1e999"] {
        let error = parse_netlist(&format!(".model bad NPN(EG={value})\nQ1 c b e bad")).unwrap_err();

        assert!(error
            .to_string()
            .contains("BJT EG must be finite and positive"));
    }
}

#[test]
fn parses_bjt_forward_early_voltage_alias() {
    let parsed = parse_netlist(".model shaped NPN(VA=80)\nQ1 c b e shaped").unwrap();

    let Element::Bjt(bjt) = &parsed.circuit.elements()[0] else {
        panic!("expected BJT");
    };
    assert_close(bjt.forward_early_voltage, 80.0);
}

#[test]
fn rejects_invalid_bjt_forward_early_voltage() {
    for value in ["-0.1", "1e999"] {
        let error = parse_netlist(&format!(".model bad NPN(VAF={value})\nQ1 c b e bad")).unwrap_err();

        assert!(error
            .to_string()
            .contains("BJT VAF must be finite and non-negative"));
    }
}

#[test]
fn parses_bjt_reverse_early_voltage_alias() {
    let parsed = parse_netlist(".model shaped NPN(VB=80)\nQ1 c b e shaped").unwrap();

    let Element::Bjt(bjt) = &parsed.circuit.elements()[0] else {
        panic!("expected BJT");
    };
    assert_close(bjt.reverse_early_voltage, 80.0);
}

#[test]
fn rejects_invalid_bjt_reverse_early_voltage() {
    for value in ["-0.1", "1e999"] {
        let error = parse_netlist(&format!(".model bad NPN(VAR={value})\nQ1 c b e bad")).unwrap_err();

        assert!(error
            .to_string()
            .contains("BJT VAR must be finite and non-negative"));
    }
}

#[test]
fn parses_bjt_forward_beta_rolloff_current_alias() {
    let parsed = parse_netlist(".model shaped NPN(IK=2m)\nQ1 c b e shaped").unwrap();

    let Element::Bjt(bjt) = &parsed.circuit.elements()[0] else {
        panic!("expected BJT");
    };
    assert_close(bjt.forward_beta_rolloff_current, 2.0e-3);
}

#[test]
fn rejects_invalid_bjt_forward_beta_rolloff_current() {
    for value in ["-0.1", "1e999"] {
        let error =
            parse_netlist(&format!(".model bad NPN(IKF={value})\nQ1 c b e bad")).unwrap_err();

        assert!(error
            .to_string()
            .contains("BJT IKF must be finite and non-negative"));
    }
}

#[test]
fn parses_bjt_reverse_beta_rolloff_current() {
    let parsed = parse_netlist(".model shaped NPN(IKR=2m)\nQ1 c b e shaped").unwrap();

    let Element::Bjt(bjt) = &parsed.circuit.elements()[0] else {
        panic!("expected BJT");
    };
    assert_close(bjt.reverse_beta_rolloff_current, 2.0e-3);
}

#[test]
fn rejects_invalid_bjt_reverse_beta_rolloff_current() {
    for value in ["-0.1", "1e999"] {
        let error =
            parse_netlist(&format!(".model bad NPN(IKR={value})\nQ1 c b e bad")).unwrap_err();

        assert!(error
            .to_string()
            .contains("BJT IKR must be finite and non-negative"));
    }
}

#[test]
fn parses_bjt_nominal_temperature_alias() {
    let parsed = parse_netlist(".model shaped NPN(T_NOM=30)\nQ1 c b e shaped").unwrap();

    let Element::Bjt(bjt) = &parsed.circuit.elements()[0] else {
        panic!("expected BJT");
    };
    assert_close(bjt.nominal_temperature_kelvin.unwrap(), 303.15);
}

#[test]
fn rejects_invalid_bjt_nominal_temperature() {
    for value in ["0", "-0.1", "1e999"] {
        let error =
            parse_netlist(&format!(".model bad NPN(TNOM={value})\nQ1 c b e bad")).unwrap_err();
...
#[test]
fn parses_jfet_capacitance_aliases_with_canonical_precedence() {
    let parsed = parse_netlist(
        r#"
.model canonical NJF(CGS=2p CGS0=3p CGD=4p CGD0=5p)
.model aliased NJF(CGS0=6p CGD0=7p)
J1 drain gate source canonical
J2 drain gate source aliased
"#,
    )
    ...
    assert_close(canonical.gate_source_capacitance, 2.0e-12);
    assert_close(canonical.gate_drain_capacitance, 4.0e-12);
    assert_close(aliased.gate_source_capacitance, 6.0e-12);
    assert_close(aliased.gate_drain_capacitance, 7.0e-12);
}

#[test]
fn rejects_invalid_jfet_capacitance_aliases() {
    for (parameter, value, message) in [
        ("CGS", "-1p", "JFET CGS must be finite and non-negative"),
        ("CGS0", "1e999", "JFET CGS must be finite and non-negative"),
        ("CGD", "-1p", "JFET CGD must be finite and non-negative"),
        ("CGD0", "1e999", "JFET CGD must be finite and non-negative"),
    ] {
        let error = parse_netlist(&format!(
...
#[test]
fn parses_jfet_bandgap_voltage() {
    let parsed = parse_netlist(".model shaped NJF(EG=1.05)\nJ1 drain gate source shaped").unwrap();

    let Element::Jfet(jfet) = &parsed.circuit.elements()[0] else {
        panic!("expected JFET");
    };
    assert_close(jfet.bandgap_voltage, 1.05);
}

#[test]
fn rejects_invalid_jfet_bandgap_voltage() {
    for bandgap_voltage in ["0", "-0.1", "1e999"] {
        let error = parse_netlist(&format!(
            ".model bad NJF(EG={bandgap_voltage})\nJ1 drain gate source bad"
        ))
        .unwrap_err();

        assert!(error
            .to_string()
            .contains("JFET EG must be finite and positive"));
    }
}
...
#[test]
fn parses_jfet_drain_resistance() {
    let parsed = parse_netlist(".model shaped NJF(RD=125)\nJ1 drain gate source shaped").unwrap();

    let Element::Jfet(jfet) = &parsed.circuit.elements()[0] else {
        panic!("expected JFET");
    };
    assert_close(jfet.drain_resistance, 125.0);
}

#[test]
fn rejects_invalid_jfet_drain_resistance() {
    for drain_resistance in ["-0.1", "1e999"] {
        let error = parse_netlist(&format!(
            ".model bad NJF(RD={drain_resistance})\nJ1 drain gate source bad"
        ))
        .unwrap_err();

        assert!(error
            .to_string()
            .contains("JFET RD must be finite and non-negative"));
    }
}

#[test]
fn parses_jfet_source_resistance() {
    let parsed = parse_netlist(".model shaped NJF(RS=75)\nJ1 drain gate source shaped").unwrap();

    let Element::Jfet(jfet) = &parsed.circuit.elements()[0] else {
        panic!("expected JFET");
    };
    assert_close(jfet.source_resistance, 75.0);
}

#[test]
fn rejects_invalid_jfet_source_resistance() {
    for source_resistance in ["-0.1", "1e999"] {
        let error = parse_netlist(&format!(
            ".model bad NJF(RS={source_resistance})\nJ1 drain gate source bad"
        ))
        .unwrap_err();

        assert!(error
            .to_string()
            .contains("JFET RS must be finite and non-negative"));
    }
}
