...
| **Mixed-signal coupling with `hardware-vm`** | AMS simulation — digital events feed into analog SPICE nodes and vice versa. Long-range project; `hardware-vm.md` spec describes the interface. A first SPICE-side Rust foothold now exposes binary digital event timelines as finite-edge PWL voltage sources and samples transient probes back into thresholded digital events; full `hardware-vm` scheduler integration remains future work. |
