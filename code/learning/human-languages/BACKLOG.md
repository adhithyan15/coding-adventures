...
| HL-C89 | Queued — **scope corrected by measurement** | Build the metalanguage ramp and the dummy-friendly prose lint (HL10 §7.4–7.5). | **The prose lint is nearly a no-op and would have been the wrong half to build.** A naive denylist flags 535 of 1,694 lessons (`just` 359, `simply` 184); narrowed to genuinely dismissive senses it drops to **23**, and reading those, most are still innocent — "*Desde luego* means 'of course'" is teaching the phrase, not talking down to the reader. Ship the narrow lint cheaply and move on. **The real gap is the metalanguage**: `infinitive` appears in 87 lessons, `participle` 67, `dative` 53, `accusative` 19, `declension` 14, `nominative` 12 — with nothing anywhere introducing those words to a reader who never studied grammar. That is the hidden prerequisite every textbook has and none admits to. ~40 terms, each introduced only after the learner can already do the thing it names, and re-glossed after a long gap. |
| HL-C91 | Queued — found while scoping HL-C82 | Add the irregular and stem-changing overlays to the Spanish cell inventory. | HL10 §5.1 sizes the Spanish verb system at ~630 cells; HL-C82 shipped the **231 regular** ones. The remaining ~400 are overlays — the four stem-change patterns (e→ie, o→ue, e→i, u→ue), the `-go` club, the strong preterites (*fui*, *hice*, *dije*, *tuve*, *estuve*, *pude*, *puse*, *supe*, *quise*, *vine*, *traje*), the three imperfect irregulars, and the irregular participles. Each attaches to the regular cell it deviates from, so the DAG gains depth rather than breadth, and each needs a frequency-ordered position. **HL-C90 is deliberately skipped here**: it is claimed by the in-flight HL-C80 branch (control-character neutralization), and reusing it would recreate the duplicate-id tangle HL-I06 exists to prevent. |
