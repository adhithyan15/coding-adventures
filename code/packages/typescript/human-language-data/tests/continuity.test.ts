...
describe("the real corpus", () => {
  ...
  it("pins what nothing had measured", { timeout: 30_000 }, () => {
    ...
    expect(report.summary.atomsTaught).toBe(3000); // +4: ES-LEX-VOS-01, ES-CULTURE-VOSEO-02 (ES-C03-vos) // +3: HL-C98 splits AR-PRESENT-SINGULAR into 1SG/2SG/3SG // +83: vocabulary wave 5 (persian ch9-11, telugu ch35-40, malayalam ch35-40) // +7: HL-C88 slices 5-6 (Spanish) // +2: HL-C88 slice 8 // +103: vocabulary wave 6, round 2 (russian/persian/urdu/bengali) // +3: HL-C113 (B1 si-condition rung)
    ...
    // R1 stops at 3.
    expect(report.summary.missedByWindow.R1).toBe(923); // +2: HL-C98 // +2: vocabulary wave 5 // +4: HL-C88 slices 5-6 // +1: HL-C112 moves casa and libro 14 chapters earlier, so one atom's next re-use now falls outside R1 // +12: vocabulary wave 6 // +1: HL-C113 (B1 si-condition rung)
    ...
    expect(report.summary.missedByWindow.R2).toBe(2103); // +1: ES-C02-concordancia (HL-C85). buenos dias/buenas tardes stop REQUIRING the agreement rule; it becomes a payoff lesson after the learner has used all three greetings. // +84: vocabulary wave 5, new pre-A1 nouns landing late in already-long chapters // +4: HL-C88 slices 5-6 // +1: HL-C112, same cause as R1 above // +1: HL-C88 slice 8 // +87: vocabulary wave 6 // +4: HL-C113 (B1 si-condition rung)
  });
  ...
  it("shows what a declared reading order was worth", () => {
    ...
    expect(spanish).toMatchObject({
      ...
      lessonCount: 344, // +4: HL-C98 // +3: HL-C97 adds the repair kit (no entiendo, mas despacio) at chapter 14 // +8 payoff lessons // +1 ES-C03-vos, +1 ES-C02-concordancia // +4: HL-C88 slices 5-6 // +1: HL-C88 slice 7 (ES-C09-ncia) // +3: HL-C88 slice 8 (-ario, review, synthesis) // +1: HL-C88 slice 9 (falsos amigos) // +3: B1 si-condition rung
      ...
      atomsTaught: 538, // +3: HL-C98's per-cell atoms // +7: HL-C88 slices 5-6 // +2: HL-C88 slice 7 (ES-C09-ncia) // +2: HL-C88 slice 8 (-ario, review, synthesis) // +2: HL-C88 slice 9 (falsos amigos) // +3: B1 si-condition rung
      atomsNeverRevisited: 74, // slice 8 nets zero: -1 ES-FRIEND-NCIA-NCE (the new review and synthesis revisit it), +1 ES-ETYMON-ARIUS (introduced, never re-practised) // +2: HL-C98 // +4: HL-C88 slices 5-6 // +1: HL-C88 slice 7 (ES-C09-ncia) // +1: HL-C88 slice 9 (falsos amigos) // +1: B1 si-condition rung
    });
  });
