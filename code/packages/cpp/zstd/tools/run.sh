...
# via the shared iso-harness (code/packages/c/iso-harness). Depends on
# cpp/lzss's headers (the LZ77 tokenizer this package's block compressor
# reuses).
set -e
...
# Include this package's headers, cpp/lzss's headers, and the harness's (for
# iso_test.h).
...
iso_build_and_run cpp zstd-tests tests/zstd_test.cpp
