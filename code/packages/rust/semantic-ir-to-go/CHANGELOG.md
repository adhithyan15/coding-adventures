...
- Ruby overloads `+` and `*` by receiver type, and every case lowers to the
  ...
  - `_sir_times`: `string Ã— Integer` → repeat via `strings.Repeat` (`"ab"*3` →
    `"ababab"`; a non-positive count yields `""`, clamped so `strings.Repeat`
    never panics); `*Seq Ã— Integer` → repeat the element list into a fresh slice
    (`[0]*3` → `[0, 0, 0]`; non-positive → empty array); `*Seq Ã— string` → join
...
- **User-defined class OOP — method dispatch, `new`, `self`, `super` (O4).**
  ...
  methodâ†”class association — which the Ruby frontend loses when it HOISTS every
...
- **Maps** — the runtime gains a `*Map` value (a struct
  ...
  - `MapGet` → `_sir_map_get(map, key)` (missing key â‡’ `nil`).
