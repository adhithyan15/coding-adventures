...
final class MosaicRuntimeHost: NSObject, MosaicHostBridgeObject {
  ...
  static func load(libraryPath: String? = nil) -> MosaicRuntimeHost? {
    ...
    func create(_ snapshot: Any?) throws -> [String: Any] {
      ...
      // What the platform knows before the first frame (UI48 ENV4). The window's
      // real size arrives with the view's first environment report.
      for (axis, value) in initialEnvironment() { start[axis] = value }
  ...
  /// The UI48 axes knowable before a window exists: the platform's pointer
  /// and hover, the reduced-motion setting and, on iOS, the screen's size
  /// class and orientation. Wire names and values are `mosaic-app-runtime`'s.
  static func initialEnvironment() -> [String: String] {
    #if os(iOS)
    ...
    return [
      ...
      "orientation": bounds.height > width ? "portrait" : "landscape",
      "reducedMotion": UIAccessibility.isReduceMotionEnabled ? "reduce" : "no-preference",
    ]
    #elseif os(macOS)
    return [
      "pointer": "fine",
