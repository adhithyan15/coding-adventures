...
describe("corpus snapshot", () => {
  ...
  it("pins the first published chapter-gate snapshot", () => {
    ...
    expect(report.summary.bookChapters).toBe(655); // +8: HL-C94 splits the four over-budget opening chapters into twelve // +16: vocabulary wave 4, 4 tracks x 4 chapters // +4: HL-C98 gives the first paradigm one cell per chapter (3 teaching + review + synthesis)
    expect(report.summary.declaredChapters).toBe(655); // +98: handwritten capability closure // +4: HL-C98
    ...
    expect(report.summary.payoffsNotRepresentative).toBe(26);
  });
