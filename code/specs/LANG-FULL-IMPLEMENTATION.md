...
- ✅ **AL7** — direct scalar call-by-name (Jensen-style expression thunks).
  ...
  nested calls even when the callee reuses the same spelling. Name-array/string
  formals and recursive name-formal dispatch remain explicitly rejected pending
  a closure-capable ABI.
