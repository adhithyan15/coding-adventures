...
| Column       | `Column { ... }`                          |
| Text         | `Text(text = ...)`; with its part's `color` / `font-size` / `font-weight` / monospace (or a container's, threaded down): `Text(..., color = ..., fontSize = ...)`; its part's `text-align` adds `textAlign = TextAlign.*` (and, outside a Row, `fillMaxWidth()` for `center`/`end` when the part sets no width) |
