...
- One-shot API: `sha256(data)` returning 32-byte digest
- Hex convenience: `sha256_hex(data)` returning 64-char lowercase hex string
- Streaming API: `SHA256Hasher` class with `update()`, `digest()`, `hex_digest()`, `copy()`
...
- Literate programming style with truth tables, diagrams, and analogies
