...
#[test]
fn phase185_log29_k_over_k_refused() {
    // log(k)^27·k / k: effective_x2=2; 2·1=2 not > 2 → refused (boundary: denom too small).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let log_k = apply(sym(LOG), vec![k.clone()]);
    let log_kp1 = apply(sym(LOG), vec![kp1.clone()]);
    let num_k = apply(sym(MUL), vec![
        ...
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k, // 28th log
        k.clone(),
    ]);
    let num_kp1 = apply(sym(MUL), vec![
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        ...
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1, // 28th log
        kp1.clone(),
    ]);
...
#[test]
fn phase186_sqrt_k_log29_k_over_k_refused() {
    // √k·log(k)^28·k / k: effective_x2=3; 2·1=2 not > 3 → refused (boundary: denom too small).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
...
#[test]
fn phase187_sqrt2_k_log29_k_over_k_refused() {
    // √k^2·log(k)^28·k / k: effective_x2=4; 2·1=2 not > 4 → refused (boundary: denom too small).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
...
#[test]
fn phase188_sqrt3_k_log29_k_over_k_refused() {
    // √k^3·log(k)^28·k / k: effective_x2=5; 2·1=2 not > 5 → refused (boundary: denom too small).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
...
#[test]
fn phase189_sqrt4_k_log29_k_over_k_refused() {
    // √k^4·log(k)^28·k / k: effective_x2=6; 2·1=2 not > 6 → refused (boundary: denom too small).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
...
#[test]
fn phase190_sqrt5_k_log29_k_over_k_refused() {
    // √k^5·log(k)^28·k / k: effective_x2=7; 2·1=2 not > 7 → refused (boundary: denom too small).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
...
#[test]
fn phase191_log29_k_times_k_over_k2_refused() {
    // log(k)^29·k / k^2: effective_x2=2; 2·2=4 not > 2 with 30 logs → refused.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let log_k = apply(sym(LOG), vec![k.clone()]);
    let log_kp1 = apply(sym(LOG), vec![kp1.clone()]);
    let num_k = apply(sym(MUL), vec![
        ...
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), // 31st log
        log_k.clone(), // 32nd log
        log_k, // 33rd log
        k.clone(),
    ]);
    let num_kp1 = apply(sym(MUL), vec![
        ...
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), // 31st log
        log_kp1.clone(), // 32nd log
        log_kp1, // 33rd log
        kp1.clone(),
    ]);
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone()]);
...
#[test]
fn phase192_sqrt_k_log29_k_times_k_over_k2_refused() {
    // √k·log(k)^29·k / k^2: 30 logs → refused.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
...
#[test]
fn phase193_sqrt2_k_log29_k_over_k_refused() {
    // √k^2·log(k)^28·k / k: 26 logs → refused.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
...
#[test]
fn phase194_sqrt3_k_log29_k_over_k_refused() {
    // √k^3·log(k)^28·k / k: 26 logs → refused.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
...
#[test]
fn phase195_sqrt4_k_log29_k_over_k_refused() {
    // √k^4·log(k)^28·k / k: 26 logs → refused.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
...
#[test]
fn phase196_sqrt5_k_log29_k_over_k_refused() {
    // √k^5·log(k)^28·k / k: 26 logs → refused.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
...
#[test]
fn phase197_log29_k_times_k_over_k2_refused() {
    // log(k)^28·k / k^2: 26 logs → refused.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let log_k = apply(sym(LOG), vec![k.clone()]);
    let log_kp1 = apply(sym(LOG), vec![kp1.clone()]);
    let num_k = apply(sym(MUL), vec![
        ...
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), // 31 logs
        log_k.clone(), // 32 logs
        log_k, // 33rd log
        k.clone(),
    ]);
    let num_kp1 = apply(sym(MUL), vec![
        ...
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), // 31 logs
        log_kp1.clone(), // 32 logs
        log_kp1, // 33rd log
        kp1.clone(),
    ]);
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone()]);
...
#[test]
fn phase198_sqrt_k_log29_k_over_k_refused() {
    // √k·log(k)^28·k / k: 26 logs → refused.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
...
#[test]
fn phase199_sqrt2_k_log29_k_over_k_refused() {
    // √k^2·log(k)^28·k / k: 26 logs → refused.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
...
#[test]
fn phase200_sqrt3_k_log29_k_over_k_refused() {
    // √k^3·log(k)^28·k / k: 26 logs → refused.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
...
#[test]
fn phase201_sqrt4_k_log29_k_over_k_refused() {
    // √k^4·log(k)^28·k / k: 26 logs → refused.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
...
#[test]
fn phase202_sqrt5_k_log29_k_over_k_refused() {
    // √k^5·log(k)^28·k / k: 26 logs → refused.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
...
// ── Phase 203: 0-sqrt × 26-log × polynomial ──────────────────────────────────

#[test]
fn phase203_log26_k_over_k4_converges() {
    // log(k)^26 / k^4: effective_x2=0; 2·4=8 > 0 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let log_k = apply(sym(LOG), vec![k.clone()]);
    let log_kp1 = apply(sym(LOG), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k203a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_203a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f203a = apply(sym(SUB), vec![g_k203a, g_kp1_203a]);
    let out = evaluate_sum(f203a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase203_log26_k_times_k_over_k5_converges() {
    // log(k)^26·k / k^5: effective_x2=2; 2·5=10 > 2 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let log_k = apply(sym(LOG), vec![k.clone()]);
    let log_kp1 = apply(sym(LOG), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k203b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_203b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f203b = apply(sym(SUB), vec![g_k203b, g_kp1_203b]);
    let out = evaluate_sum(f203b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase203_log29_k_times_k_over_k2_refused() {
    // log(k)^29·k / k^2: 30 logs → refused (Phase 203 handles exactly 27).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let log_k = apply(sym(LOG), vec![k.clone()]);
    let log_kp1 = apply(sym(LOG), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone()]);
    let g_k203c = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_203c = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f203c = apply(sym(SUB), vec![g_k203c, g_kp1_203c]);
    let out = evaluate_sum(f203c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 204: 1-sqrt × 26-log × polynomial ──────────────────────────────────

#[test]
fn phase204_sqrt_k_log26_k_over_k4_converges() {
    // √k·log(k)^26 / k^4: effective_x2=1; 2·4=8 > 1 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k204a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_204a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f204a = apply(sym(SUB), vec![g_k204a, g_kp1_204a]);
    let out = evaluate_sum(f204a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase204_sqrt_k_log26_k_times_k_over_k5_converges() {
    // √k·log(k)^26·k / k^5: effective_x2=3; 2·5=10 > 3 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k204b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_204b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f204b = apply(sym(SUB), vec![g_k204b, g_kp1_204b]);
    let out = evaluate_sum(f204b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase204_sqrt_k_log29_k_over_k_refused() {
    // √k·log(k)^28·k / k: 30 logs → refused (Phase 204 handles exactly 28).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_204c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f204c = apply(sym(SUB), vec![g_k204c, g_kp1_204c]);
    let out = evaluate_sum(f204c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 205: 2-sqrt × 26-log × polynomial ──────────────────────────────────

#[test]
fn phase205_sqrt2_k_log26_k_over_k4_converges() {
    // √k^2·log(k)^26 / k^4: effective_x2=2; 2·4=8 > 2 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k205a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_205a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f205a = apply(sym(SUB), vec![g_k205a, g_kp1_205a]);
    let out = evaluate_sum(f205a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase205_sqrt2_k_log26_k_times_k_over_k5_converges() {
    // √k^2·log(k)^26·k / k^5: effective_x2=4; 2·5=10 > 4 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k205b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_205b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f205b = apply(sym(SUB), vec![g_k205b, g_kp1_205b]);
    let out = evaluate_sum(f205b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase205_sqrt2_k_log29_k_over_k_refused() {
    // √k^2·log(k)^28·k / k: 30 logs → refused (Phase 205 handles exactly 28).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_205c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f205c = apply(sym(SUB), vec![g_k205c, g_kp1_205c]);
    let out = evaluate_sum(f205c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 206: 3-sqrt × 26-log × polynomial ──────────────────────────────────

#[test]
fn phase206_sqrt3_k_log26_k_over_k4_converges() {
    // √k^3·log(k)^26 / k^4: effective_x2=3; 2·4=8 > 3 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k206a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_206a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f206a = apply(sym(SUB), vec![g_k206a, g_kp1_206a]);
    let out = evaluate_sum(f206a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase206_sqrt3_k_log26_k_times_k_over_k5_converges() {
    // √k^3·log(k)^26·k / k^5: effective_x2=5; 2·5=10 > 5 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k206b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_206b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f206b = apply(sym(SUB), vec![g_k206b, g_kp1_206b]);
    let out = evaluate_sum(f206b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase206_sqrt3_k_log29_k_over_k_refused() {
    // √k^3·log(k)^28·k / k: 30 logs → refused (Phase 206 handles exactly 28).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_206c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f206c = apply(sym(SUB), vec![g_k206c, g_kp1_206c]);
    let out = evaluate_sum(f206c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 207: 4-sqrt × 26-log × polynomial ──────────────────────────────────

#[test]
fn phase207_sqrt4_k_log26_k_over_k4_converges() {
    // √k^4·log(k)^26 / k^4: effective_x2=4; 2·4=8 > 4 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k207a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_207a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f207a = apply(sym(SUB), vec![g_k207a, g_kp1_207a]);
    let out = evaluate_sum(f207a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase207_sqrt4_k_log26_k_times_k_over_k5_converges() {
    // √k^4·log(k)^26·k / k^5: effective_x2=6; 2·5=10 > 6 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k207b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_207b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f207b = apply(sym(SUB), vec![g_k207b, g_kp1_207b]);
    let out = evaluate_sum(f207b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase207_sqrt4_k_log29_k_over_k_refused() {
    // √k^4·log(k)^28·k / k: 30 logs → refused (Phase 207 handles exactly 28).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_207c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f207c = apply(sym(SUB), vec![g_k207c, g_kp1_207c]);
    let out = evaluate_sum(f207c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 208: 5-sqrt × 26-log × polynomial ──────────────────────────────────

#[test]
fn phase208_sqrt5_k_log26_k_over_k4_converges() {
    // √k^5·log(k)^26 / k^4: effective_x2=5; 2·4=8 > 5 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k208a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_208a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f208a = apply(sym(SUB), vec![g_k208a, g_kp1_208a]);
    let out = evaluate_sum(f208a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase208_sqrt5_k_log26_k_times_k_over_k5_converges() {
    // √k^5·log(k)^26·k / k^5: effective_x2=7; 2·5=10 > 7 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k208b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_208b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f208b = apply(sym(SUB), vec![g_k208b, g_kp1_208b]);
    let out = evaluate_sum(f208b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase208_sqrt5_k_log29_k_over_k_refused() {
    // √k^5·log(k)^28·k / k: 30 logs → refused (Phase 208 handles exactly 28).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_208c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f208c = apply(sym(SUB), vec![g_k208c, g_kp1_208c]);
    let out = evaluate_sum(f208c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 209: 0-sqrt × 27-log × polynomial ──────────────────────────────────

#[test]
fn phase209_log27_k_over_k5_converges() {
    // log(k)^27 / k^5: effective_x2=0; 2·5=10 > 0 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let log_k = apply(sym(LOG), vec![k.clone()]);
    let log_kp1 = apply(sym(LOG), vec![kp1.clone()]);
    ...
    let num_kp1 = apply(sym(MUL), vec![
        ...
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1, // 27 logs
    ]);
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k209a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_209a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f209a = apply(sym(SUB), vec![g_k209a, g_kp1_209a]);
    let out = evaluate_sum(f209a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase209_log27_k_times_k_over_k5_converges() {
    // log(k)^27·k / k^5: effective_x2=2; 2·5=10 > 2 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let log_k = apply(sym(LOG), vec![k.clone()]);
    let log_kp1 = apply(sym(LOG), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k209b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_209b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f209b = apply(sym(SUB), vec![g_k209b, g_kp1_209b]);
    let out = evaluate_sum(f209b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase209_log29_k_times_k_over_k2_refused() {
    // log(k)^29·k / k^2: 30 logs → refused (Phase 209 handles exactly 27).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let log_k = apply(sym(LOG), vec![k.clone()]);
    let log_kp1 = apply(sym(LOG), vec![kp1.clone()]);
    let num_k = apply(sym(MUL), vec![
        ...
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), // 31 logs
        log_k.clone(), // 32 logs
        log_k, // 33rd log
        k.clone(),
    ]);
    let num_kp1 = apply(sym(MUL), vec![
        ...
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), // 31 logs
        log_kp1.clone(), // 32 logs
        log_kp1, // 33rd log
        kp1.clone(),
    ]);
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone()]);
    let g_k209c = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_209c = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f209c = apply(sym(SUB), vec![g_k209c, g_kp1_209c]);
    let out = evaluate_sum(f209c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 210: 1-sqrt × 27-log × polynomial ──────────────────────────────────

#[test]
fn phase210_sqrt_k_log27_k_over_k4_converges() {
    // √k·log(k)^27 / k^4: effective_x2=1; 2·4=8 > 1 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k210a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_210a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f210a = apply(sym(SUB), vec![g_k210a, g_kp1_210a]);
    let out = evaluate_sum(f210a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase210_sqrt_k_log27_k_times_k_over_k5_converges() {
    // √k·log(k)^27·k / k^5: effective_x2=3; 2·5=10 > 3 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k210b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_210b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f210b = apply(sym(SUB), vec![g_k210b, g_kp1_210b]);
    let out = evaluate_sum(f210b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase210_sqrt_k_log29_k_over_k_refused() {
    // √k·log(k)^28·k / k: 30 logs → refused (Phase 210 handles exactly 28).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_210c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f210c = apply(sym(SUB), vec![g_k210c, g_kp1_210c]);
    let out = evaluate_sum(f210c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 211: 2-sqrt × 27-log × polynomial ──────────────────────────────────

#[test]
fn phase211_sqrt2_k_log27_k_over_k4_converges() {
    // √k^2·log(k)^27 / k^4: effective_x2=2; 2·4=8 > 2 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k211a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_211a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f211a = apply(sym(SUB), vec![g_k211a, g_kp1_211a]);
    let out = evaluate_sum(f211a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase211_sqrt2_k_log27_k_times_k_over_k5_converges() {
    // √k^2·log(k)^27·k / k^5: effective_x2=4; 2·5=10 > 4 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k211b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_211b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f211b = apply(sym(SUB), vec![g_k211b, g_kp1_211b]);
    let out = evaluate_sum(f211b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase211_sqrt2_k_log29_k_over_k_refused() {
    // √k^2·log(k)^28·k / k: 30 logs → refused (Phase 211 handles exactly 28).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_211c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f211c = apply(sym(SUB), vec![g_k211c, g_kp1_211c]);
    let out = evaluate_sum(f211c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 212: 3-sqrt × 27-log × polynomial ──────────────────────────────────

#[test]
fn phase212_sqrt3_k_log27_k_over_k4_converges() {
    // √k^3·log(k)^27 / k^4: effective_x2=3; 2·4=8 > 3 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k212a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_212a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f212a = apply(sym(SUB), vec![g_k212a, g_kp1_212a]);
    let out = evaluate_sum(f212a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase212_sqrt3_k_log27_k_times_k_over_k5_converges() {
    // √k^3·log(k)^27·k / k^5: effective_x2=5; 2·5=10 > 5 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k212b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_212b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f212b = apply(sym(SUB), vec![g_k212b, g_kp1_212b]);
    let out = evaluate_sum(f212b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase212_sqrt3_k_log29_k_over_k_refused() {
    // √k^3·log(k)^28·k / k: 30 logs → refused (Phase 212 handles exactly 28).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_212c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f212c = apply(sym(SUB), vec![g_k212c, g_kp1_212c]);
    let out = evaluate_sum(f212c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 213: 4-sqrt × 27-log × polynomial ──────────────────────────────────

#[test]
fn phase213_sqrt4_k_log27_k_over_k4_converges() {
    // √k^4·log(k)^27 / k^4: effective_x2=4; 2·4=8 > 4 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k213a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_213a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f213a = apply(sym(SUB), vec![g_k213a, g_kp1_213a]);
    let out = evaluate_sum(f213a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase213_sqrt4_k_log27_k_times_k_over_k5_converges() {
    // √k^4·log(k)^27·k / k^5: effective_x2=6; 2·5=10 > 6 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k213b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_213b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f213b = apply(sym(SUB), vec![g_k213b, g_kp1_213b]);
    let out = evaluate_sum(f213b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase213_sqrt4_k_log29_k_over_k_refused() {
    // √k^4·log(k)^28·k / k: 30 logs → refused (Phase 213 handles exactly 28).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_213c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f213c = apply(sym(SUB), vec![g_k213c, g_kp1_213c]);
    let out = evaluate_sum(f213c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 214: 5-sqrt × 27-log × polynomial ──────────────────────────────────

#[test]
fn phase214_sqrt5_k_log27_k_over_k4_converges() {
    // √k^5·log(k)^27 / k^4: effective_x2=5; 2·4=8 > 5 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k214a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_214a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f214a = apply(sym(SUB), vec![g_k214a, g_kp1_214a]);
    let out = evaluate_sum(f214a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase214_sqrt5_k_log27_k_times_k_over_k5_converges() {
    // √k^5·log(k)^27·k / k^5: effective_x2=7; 2·5=10 > 7 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k214b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_214b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f214b = apply(sym(SUB), vec![g_k214b, g_kp1_214b]);
    let out = evaluate_sum(f214b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase214_sqrt5_k_log29_k_over_k_refused() {
    // √k^5·log(k)^28·k / k: 30 logs → refused (Phase 214 handles exactly 28).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_214c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f214c = apply(sym(SUB), vec![g_k214c, g_kp1_214c]);
    let out = evaluate_sum(f214c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 215: 0-sqrt × 28-log × polynomial ──────────────────────────────────

#[test]
fn phase215_log28_k_over_k5_converges() {
    // log(k)^28 / k^5: effective_x2=0; 2·5=10 > 0 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let log_k = apply(sym(LOG), vec![k.clone()]);
    let log_kp1 = apply(sym(LOG), vec![kp1.clone()]);
    let num_k = apply(sym(MUL), vec![
        ...
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k, // 28 logs
    ]);
    let num_kp1 = apply(sym(MUL), vec![
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        ...
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1, // 28 logs
    ]);
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k215a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_215a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f215a = apply(sym(SUB), vec![g_k215a, g_kp1_215a]);
    let out = evaluate_sum(f215a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase215_log28_k_times_k_over_k5_converges() {
    // log(k)^28·k / k^5: effective_x2=2; 2·5=10 > 2 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let log_k = apply(sym(LOG), vec![k.clone()]);
    let log_kp1 = apply(sym(LOG), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k215b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_215b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f215b = apply(sym(SUB), vec![g_k215b, g_kp1_215b]);
    let out = evaluate_sum(f215b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase215_log29_k_times_k_over_k2_refused() {
    // log(k)^29·k / k^2: 30 logs → refused (Phase 215 handles exactly 28).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let log_k = apply(sym(LOG), vec![k.clone()]);
    let log_kp1 = apply(sym(LOG), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone()]);
    let g_k215c = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_215c = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f215c = apply(sym(SUB), vec![g_k215c, g_kp1_215c]);
    let out = evaluate_sum(f215c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 216: 1-sqrt × 28-log × polynomial ──────────────────────────────────

#[test]
fn phase216_sqrt_k_log28_k_over_k4_converges() {
    // √k·log(k)^28 / k^4: effective_x2=1; 2·4=8 > 1 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k216a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_216a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f216a = apply(sym(SUB), vec![g_k216a, g_kp1_216a]);
    let out = evaluate_sum(f216a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase216_sqrt_k_log28_k_times_k_over_k5_converges() {
    // √k·log(k)^28·k / k^5: effective_x2=3; 2·5=10 > 3 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k216b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_216b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f216b = apply(sym(SUB), vec![g_k216b, g_kp1_216b]);
    let out = evaluate_sum(f216b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase216_sqrt_k_log29_k_over_k_refused() {
    // √k·log(k)^29·k / k: 30 logs → refused (Phase 216 handles exactly 28).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_216c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f216c = apply(sym(SUB), vec![g_k216c, g_kp1_216c]);
    let out = evaluate_sum(f216c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 217: 2-sqrt × 28-log × polynomial ──────────────────────────────────

#[test]
fn phase217_sqrt2_k_log28_k_over_k4_converges() {
    // √k^2·log(k)^28 / k^4: effective_x2=2; 2·4=8 > 2 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let num_kp1 = apply(sym(MUL), vec![
        ...
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1, // 28 logs
    ]);
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k217a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_217a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f217a = apply(sym(SUB), vec![g_k217a, g_kp1_217a]);
    let out = evaluate_sum(f217a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase217_sqrt2_k_log28_k_times_k_over_k5_converges() {
    // √k^2·log(k)^28·k / k^5: effective_x2=4; 2·5=10 > 4 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k217b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_217b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f217b = apply(sym(SUB), vec![g_k217b, g_kp1_217b]);
    let out = evaluate_sum(f217b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase217_sqrt2_k_log29_k_over_k_refused() {
    // √k^2·log(k)^29·k / k: 30 logs → refused (Phase 217 handles exactly 28).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let num_k = apply(sym(MUL), vec![
        ...
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), // 31 logs
        log_k.clone(), // 32 logs
        log_k, // 33rd log
        k.clone(),
    ]);
    let num_kp1 = apply(sym(MUL), vec![
        ...
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), // 31 logs
        log_kp1.clone(), // 32 logs
        log_kp1, // 33rd log
        kp1.clone(),
    ]);
    let g_k217c = apply(sym(DIV), vec![num_k, k.clone()]);
    let g_kp1_217c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f217c = apply(sym(SUB), vec![g_k217c, g_kp1_217c]);
    let out = evaluate_sum(f217c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 218: 3-sqrt × 28-log × polynomial ──────────────────────────────────

#[test]
fn phase218_sqrt3_k_log28_k_over_k4_converges() {
    // √k^3·log(k)^28 / k^4: effective_x2=3; 2·4=8 > 3 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k218a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_218a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f218a = apply(sym(SUB), vec![g_k218a, g_kp1_218a]);
    let out = evaluate_sum(f218a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase218_sqrt3_k_log28_k_times_k_over_k5_converges() {
    // √k^3·log(k)^28·k / k^5: effective_x2=5; 2·5=10 > 5 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k218b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_218b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f218b = apply(sym(SUB), vec![g_k218b, g_kp1_218b]);
    let out = evaluate_sum(f218b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase218_sqrt3_k_log29_k_over_k_refused() {
    // √k^3·log(k)^29·k / k: 30 logs → refused (Phase 218 handles exactly 28).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_218c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f218c = apply(sym(SUB), vec![g_k218c, g_kp1_218c]);
    let out = evaluate_sum(f218c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 219: 4-sqrt × 28-log × polynomial ──────────────────────────────────

#[test]
fn phase219_sqrt4_k_log28_k_over_k4_converges() {
    // √k^4·log(k)^28 / k^4: effective_x2=4; 2·4=8 > 4 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k219a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_219a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f219a = apply(sym(SUB), vec![g_k219a, g_kp1_219a]);
    let out = evaluate_sum(f219a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase219_sqrt4_k_log28_k_times_k_over_k5_converges() {
    // √k^4·log(k)^28·k / k^5: effective_x2=6; 2·5=10 > 6 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k219b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_219b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f219b = apply(sym(SUB), vec![g_k219b, g_kp1_219b]);
    let out = evaluate_sum(f219b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase219_sqrt4_k_log29_k_over_k_refused() {
    // √k^4·log(k)^29·k / k: 30 logs → refused (Phase 219 handles exactly 28).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_219c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f219c = apply(sym(SUB), vec![g_k219c, g_kp1_219c]);
    let out = evaluate_sum(f219c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 220: 5-sqrt × 28-log × polynomial ──────────────────────────────────

#[test]
fn phase220_sqrt5_k_log28_k_over_k4_converges() {
    // √k^5·log(k)^28 / k^4: effective_x2=5; 2·4=8 > 5 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k220a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_220a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f220a = apply(sym(SUB), vec![g_k220a, g_kp1_220a]);
    let out = evaluate_sum(f220a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase220_sqrt5_k_log28_k_times_k_over_k5_converges() {
    // √k^5·log(k)^28·k / k^5: effective_x2=7; 2·5=10 > 7 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k220b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_220b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f220b = apply(sym(SUB), vec![g_k220b, g_kp1_220b]);
    let out = evaluate_sum(f220b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase220_sqrt5_k_log29_k_over_k_refused() {
    // √k^5·log(k)^29·k / k: 30 logs → refused (Phase 220 handles exactly 28).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_220c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f220c = apply(sym(SUB), vec![g_k220c, g_kp1_220c]);
    let out = evaluate_sum(f220c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 221: 0-sqrt × 29-log × polynomial ──────────────────────────────────

#[test]
fn phase221_log29_k_over_k5_converges() {
    // log(k)^29 / k^5: effective_x2=0; 2·5=10 > 0 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let log_k = apply(sym(LOG), vec![k.clone()]);
    let log_kp1 = apply(sym(LOG), vec![kp1.clone()]);
    let num_k = apply(sym(MUL), vec![
        ...
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k.clone(), log_k, // 29 logs
    ]);
    let num_kp1 = apply(sym(MUL), vec![
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        ...
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1, // 29 logs
    ]);
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k221a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_221a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f221a = apply(sym(SUB), vec![g_k221a, g_kp1_221a]);
    let out = evaluate_sum(f221a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase221_log29_k_times_k_over_k5_converges() {
    // log(k)^29·k / k^5: effective_x2=2; 2·5=10 > 2 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let log_k = apply(sym(LOG), vec![k.clone()]);
    let log_kp1 = apply(sym(LOG), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k221b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_221b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f221b = apply(sym(SUB), vec![g_k221b, g_kp1_221b]);
    let out = evaluate_sum(f221b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase221_log30_k_times_k_over_k2_refused() {
    // log(k)^30·k / k^2: 30 logs → refused (Phase 221 handles exactly 29).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let log_k = apply(sym(LOG), vec![k.clone()]);
    let log_kp1 = apply(sym(LOG), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone()]);
    let g_k221c = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_221c = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f221c = apply(sym(SUB), vec![g_k221c, g_kp1_221c]);
    let out = evaluate_sum(f221c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 222: 1-sqrt × 29-log × polynomial ──────────────────────────────────

#[test]
fn phase222_sqrt1_k_log29_k_over_k4_converges() {
    // √k^1·log(k)^29 / k^4: effective_x2=1; 2·4=8 > 1 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k222a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_222a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f222a = apply(sym(SUB), vec![g_k222a, g_kp1_222a]);
    let out = evaluate_sum(f222a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase222_sqrt1_k_log29_k_times_k_over_k5_converges() {
    // √k^1·log(k)^29·k / k^5: effective_x2=3; 2·5=10 > 3 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k222b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_222b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f222b = apply(sym(SUB), vec![g_k222b, g_kp1_222b]);
    let out = evaluate_sum(f222b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase222_sqrt1_k_log30_k_over_k_refused() {
    // √k^1·log(k)^30·k / k: 30 logs → refused (Phase 222 handles exactly 29).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_222c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f222c = apply(sym(SUB), vec![g_k222c, g_kp1_222c]);
    let out = evaluate_sum(f222c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 223: 2-sqrt × 29-log × polynomial ──────────────────────────────────

#[test]
fn phase223_sqrt2_k_log29_k_over_k4_converges() {
    // √k^2·log(k)^29 / k^4: effective_x2=2; 2·4=8 > 2 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k223a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_223a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f223a = apply(sym(SUB), vec![g_k223a, g_kp1_223a]);
    let out = evaluate_sum(f223a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase223_sqrt2_k_log29_k_times_k_over_k5_converges() {
    // √k^2·log(k)^29·k / k^5: effective_x2=4; 2·5=10 > 4 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k223b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_223b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f223b = apply(sym(SUB), vec![g_k223b, g_kp1_223b]);
    let out = evaluate_sum(f223b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase223_sqrt2_k_log30_k_over_k_refused() {
    // √k^2·log(k)^30·k / k: 30 logs → refused (Phase 223 handles exactly 29).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_223c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f223c = apply(sym(SUB), vec![g_k223c, g_kp1_223c]);
    let out = evaluate_sum(f223c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 224: 3-sqrt × 29-log × polynomial ──────────────────────────────────

#[test]
fn phase224_sqrt3_k_log29_k_over_k4_converges() {
    // √k^3·log(k)^29 / k^4: effective_x2=3; 2·4=8 > 3 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k224a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_224a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f224a = apply(sym(SUB), vec![g_k224a, g_kp1_224a]);
    let out = evaluate_sum(f224a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase224_sqrt3_k_log29_k_times_k_over_k5_converges() {
    // √k^3·log(k)^29·k / k^5: effective_x2=5; 2·5=10 > 5 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k224b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_224b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f224b = apply(sym(SUB), vec![g_k224b, g_kp1_224b]);
    let out = evaluate_sum(f224b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase224_sqrt3_k_log30_k_over_k_refused() {
    // √k^3·log(k)^30·k / k: 30 logs → refused (Phase 224 handles exactly 29).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_224c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f224c = apply(sym(SUB), vec![g_k224c, g_kp1_224c]);
    let out = evaluate_sum(f224c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 225: 4-sqrt × 29-log × polynomial ──────────────────────────────────

#[test]
fn phase225_sqrt4_k_log29_k_over_k4_converges() {
    // √k^4·log(k)^29 / k^4: effective_x2=4; 2·4=8 > 4 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k225a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_225a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f225a = apply(sym(SUB), vec![g_k225a, g_kp1_225a]);
    let out = evaluate_sum(f225a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase225_sqrt4_k_log29_k_times_k_over_k5_converges() {
    // √k^4·log(k)^29·k / k^5: effective_x2=6; 2·5=10 > 6 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k225b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_225b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f225b = apply(sym(SUB), vec![g_k225b, g_kp1_225b]);
    let out = evaluate_sum(f225b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase225_sqrt4_k_log30_k_over_k_refused() {
    // √k^4·log(k)^30·k / k: 30 logs → refused (Phase 225 handles exactly 29).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_225c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f225c = apply(sym(SUB), vec![g_k225c, g_kp1_225c]);
    let out = evaluate_sum(f225c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 226: 5-sqrt × 29-log × polynomial ──────────────────────────────────

#[test]
fn phase226_sqrt5_k_log29_k_over_k4_converges() {
    // √k^5·log(k)^29 / k^4: effective_x2=5; 2·4=8 > 5 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k226a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_226a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f226a = apply(sym(SUB), vec![g_k226a, g_kp1_226a]);
    let out = evaluate_sum(f226a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase226_sqrt5_k_log29_k_times_k_over_k5_converges() {
    // √k^5·log(k)^29·k / k^5: effective_x2=7; 2·5=10 > 7 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k226b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_226b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f226b = apply(sym(SUB), vec![g_k226b, g_kp1_226b]);
    let out = evaluate_sum(f226b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase226_sqrt5_k_log30_k_over_k_refused() {
    // √k^5·log(k)^30·k / k: 30 logs → refused (Phase 226 handles exactly 29).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_226c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f226c = apply(sym(SUB), vec![g_k226c, g_kp1_226c]);
    let out = evaluate_sum(f226c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 227: 0-sqrt × 30-log × polynomial ──────────────────────────────────

#[test]
fn phase227_log30_k_over_k5_converges() {
    // log(k)^30 / k^5: effective_x2=0; 2·5=10 > 0 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let log_k = apply(sym(LOG), vec![k.clone()]);
    let log_kp1 = apply(sym(LOG), vec![kp1.clone()]);
    let num_k = apply(sym(MUL), vec![
        ...
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k, // 30 logs
    ]);
    let num_kp1 = apply(sym(MUL), vec![
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        ...
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1, // 30 logs
    ]);
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k227a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_227a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f227a = apply(sym(SUB), vec![g_k227a, g_kp1_227a]);
    let out = evaluate_sum(f227a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase227_log30_k_times_k_over_k5_converges() {
    // log(k)^30·k / k^5: effective_x2=2; 2·5=10 > 2 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let log_k = apply(sym(LOG), vec![k.clone()]);
    let log_kp1 = apply(sym(LOG), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k227b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_227b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f227b = apply(sym(SUB), vec![g_k227b, g_kp1_227b]);
    let out = evaluate_sum(f227b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase227_log31_k_times_k_over_k2_refused() {
    // log(k)^31·k / k^2: 31 logs → refused (Phase 227 handles exactly 30).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let log_k = apply(sym(LOG), vec![k.clone()]);
    let log_kp1 = apply(sym(LOG), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone()]);
    let g_k227c = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_227c = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f227c = apply(sym(SUB), vec![g_k227c, g_kp1_227c]);
    let out = evaluate_sum(f227c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 228: 1-sqrt × 30-log × polynomial ──────────────────────────────────

#[test]
fn phase228_sqrt1_k_log30_k_over_k4_converges() {
    // √k^1·log(k)^30 / k^4: effective_x2=1; 2·4=8 > 1 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k228a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_228a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f228a = apply(sym(SUB), vec![g_k228a, g_kp1_228a]);
    let out = evaluate_sum(f228a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase228_sqrt1_k_log30_k_times_k_over_k5_converges() {
    // √k^1·log(k)^30·k / k^5: effective_x2=3; 2·5=10 > 3 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k228b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_228b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f228b = apply(sym(SUB), vec![g_k228b, g_kp1_228b]);
    let out = evaluate_sum(f228b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase228_sqrt1_k_log31_k_times_k_over_k_refused() {
    // √k^1·log(k)^31·k / k: 31 logs → refused (Phase 228 handles exactly 30).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_228c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f228c = apply(sym(SUB), vec![g_k228c, g_kp1_228c]);
    let out = evaluate_sum(f228c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 229: 2-sqrt × 30-log × polynomial ──────────────────────────────────

#[test]
fn phase229_sqrt2_k_log30_k_over_k4_converges() {
    // √k^2·log(k)^30 / k^4: effective_x2=2; 2·4=8 > 2 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k229a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_229a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f229a = apply(sym(SUB), vec![g_k229a, g_kp1_229a]);
    let out = evaluate_sum(f229a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase229_sqrt2_k_log30_k_times_k_over_k5_converges() {
    // √k^2·log(k)^30·k / k^5: effective_x2=4; 2·5=10 > 4 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k229b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_229b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f229b = apply(sym(SUB), vec![g_k229b, g_kp1_229b]);
    let out = evaluate_sum(f229b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase229_sqrt2_k_log31_k_times_k_over_k_refused() {
    // √k^2·log(k)^31·k / k: 31 logs → refused (Phase 229 handles exactly 30).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_229c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f229c = apply(sym(SUB), vec![g_k229c, g_kp1_229c]);
    let out = evaluate_sum(f229c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 230: 3-sqrt × 30-log × polynomial ──────────────────────────────────

#[test]
fn phase230_sqrt3_k_log30_k_over_k4_converges() {
    // √k^3·log(k)^30 / k^4: effective_x2=3; 2·4=8 > 3 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k230a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_230a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f230a = apply(sym(SUB), vec![g_k230a, g_kp1_230a]);
    let out = evaluate_sum(f230a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase230_sqrt3_k_log30_k_times_k_over_k5_converges() {
    // √k^3·log(k)^30·k / k^5: effective_x2=5; 2·5=10 > 5 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k230b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_230b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f230b = apply(sym(SUB), vec![g_k230b, g_kp1_230b]);
    let out = evaluate_sum(f230b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase230_sqrt3_k_log31_k_over_k_refused() {
    // √k^3·log(k)^31·k / k: 31 logs → refused (Phase 230 handles exactly 30).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_230c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f230c = apply(sym(SUB), vec![g_k230c, g_kp1_230c]);
    let out = evaluate_sum(f230c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 231: 4-sqrt × 30-log × polynomial ──────────────────────────────────

#[test]
fn phase231_sqrt4_k_log30_k_over_k4_converges() {
    // √k^4·log(k)^30 / k^4: effective_x2=4; 2·4=8 > 4 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k231a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_231a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f231a = apply(sym(SUB), vec![g_k231a, g_kp1_231a]);
    let out = evaluate_sum(f231a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase231_sqrt4_k_log30_k_times_k_over_k5_converges() {
    // √k^4·log(k)^30·k / k^5: effective_x2=6; 2·5=10 > 6 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k231b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_231b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f231b = apply(sym(SUB), vec![g_k231b, g_kp1_231b]);
    let out = evaluate_sum(f231b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase231_sqrt4_k_log31_k_over_k_refused() {
    // √k^4·log(k)^31·k / k: 31 logs → refused (Phase 231 handles exactly 30).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_231c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f231c = apply(sym(SUB), vec![g_k231c, g_kp1_231c]);
    let out = evaluate_sum(f231c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 232: 5-sqrt × 30-log × polynomial ──────────────────────────────────

#[test]
fn phase232_sqrt5_k_log30_k_over_k4_converges() {
    // √k^5·log(k)^30 / k^4: effective_x2=5; 2·4=8 > 5 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let num_k = apply(sym(MUL), vec![
        ...
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k.clone(), log_k, // 30 logs (34 total with sqrts)
    ]);
    let num_kp1 = apply(sym(MUL), vec![
        sqrt_kp1.clone(), sqrt_kp1.clone(), sqrt_kp1.clone(), sqrt_kp1.clone(), sqrt_kp1,
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        ...
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1, // 30 logs
    ]);
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k232a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_232a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f232a = apply(sym(SUB), vec![g_k232a, g_kp1_232a]);
    let out = evaluate_sum(f232a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase232_sqrt5_k_log30_k_times_k_over_k5_converges() {
    // √k^5·log(k)^30·k / k^5: effective_x2=7; 2·5=10 > 7 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let num_k = apply(sym(MUL), vec![
        ...
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k.clone(), log_k, // 30 logs
        k.clone(),
    ]);
    let num_kp1 = apply(sym(MUL), vec![
        sqrt_kp1.clone(), sqrt_kp1.clone(), sqrt_kp1.clone(), sqrt_kp1.clone(), sqrt_kp1,
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k232b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_232b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f232b = apply(sym(SUB), vec![g_k232b, g_kp1_232b]);
    let out = evaluate_sum(f232b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase232_sqrt5_k_log31_k_over_k_refused() {
    // √k^5·log(k)^31·k / k: 31 logs → refused (Phase 232 handles exactly 30).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_232c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f232c = apply(sym(SUB), vec![g_k232c, g_kp1_232c]);
    let out = evaluate_sum(f232c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 233: 0-sqrt × 31-log × polynomial ──────────────────────────────────

#[test]
fn phase233_log31_k_over_k5_converges() {
    // log(k)^31 / k^5: effective_x2=0; 2·5=10 > 0 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let log_k = apply(sym(LOG), vec![k.clone()]);
    let log_kp1 = apply(sym(LOG), vec![kp1.clone()]);
    let num_k = apply(sym(MUL), vec![
        ...
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k, // 31 logs
    ]);
    let num_kp1 = apply(sym(MUL), vec![
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        ...
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1, // 31 logs
    ]);
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k233a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_233a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f233a = apply(sym(SUB), vec![g_k233a, g_kp1_233a]);
    let out = evaluate_sum(f233a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase233_log31_k_times_k_over_k5_converges() {
    // log(k)^31·k / k^5: effective_x2=2; 2·5=10 > 2 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let log_k = apply(sym(LOG), vec![k.clone()]);
    let log_kp1 = apply(sym(LOG), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k233b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_233b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f233b = apply(sym(SUB), vec![g_k233b, g_kp1_233b]);
    let out = evaluate_sum(f233b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase233_log32_k_times_k_over_k2_refused() {
    // log(k)^32·k / k^2: 32 logs → refused (Phase 233 handles exactly 31).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let log_k = apply(sym(LOG), vec![k.clone()]);
    let log_kp1 = apply(sym(LOG), vec![kp1.clone()]);
    let num_k = apply(sym(MUL), vec![
        ...
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), // 32 logs
        log_k, // 33rd log
        k.clone(),
    ]);
    let num_kp1 = apply(sym(MUL), vec![
        ...
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), // 32 logs
        log_kp1, // 33rd log
        kp1.clone(),
    ]);
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone()]);
    let g_k233c = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_233c = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f233c = apply(sym(SUB), vec![g_k233c, g_kp1_233c]);
    let out = evaluate_sum(f233c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 234: 1-sqrt × 31-log × polynomial ──────────────────────────────────

#[test]
fn phase234_sqrt1_k_log31_k_over_k4_converges() {
    // √k^1·log(k)^31 / k^4: effective_x2=1; 2·4=8 > 1 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k234a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_234a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f234a = apply(sym(SUB), vec![g_k234a, g_kp1_234a]);
    let out = evaluate_sum(f234a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase234_sqrt1_k_log31_k_times_k_over_k5_converges() {
    // √k^1·log(k)^31·k / k^5: effective_x2=3; 2·5=10 > 3 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k234b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_234b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f234b = apply(sym(SUB), vec![g_k234b, g_kp1_234b]);
    let out = evaluate_sum(f234b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase234_sqrt1_k_log32_k_times_k_over_k_refused() {
    // √k^1·log(k)^32·k / k: 32 logs → refused (Phase 234 handles exactly 31).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_234c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f234c = apply(sym(SUB), vec![g_k234c, g_kp1_234c]);
    let out = evaluate_sum(f234c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 235: 2-sqrt × 31-log × polynomial ──────────────────────────────────

#[test]
fn phase235_sqrt2_k_log31_k_over_k4_converges() {
    // √k^2·log(k)^31 / k^4: effective_x2=2; 2·4=8 > 2 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k235a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_235a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f235a = apply(sym(SUB), vec![g_k235a, g_kp1_235a]);
    let out = evaluate_sum(f235a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase235_sqrt2_k_log31_k_times_k_over_k5_converges() {
    // √k^2·log(k)^31·k / k^5: effective_x2=4; 2·5=10 > 4 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k235b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_235b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f235b = apply(sym(SUB), vec![g_k235b, g_kp1_235b]);
    let out = evaluate_sum(f235b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase235_sqrt2_k_log32_k_over_k_refused() {
    // √k^2·log(k)^32 / k: 32 logs → refused (Phase 235 handles exactly 31).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let f235c = apply(sym(SUB), vec![g_k235c, g_kp1_235c]);
    let out = evaluate_sum(f235c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 236: 3-sqrt × 31-log × polynomial ──────────────────────────────────

#[test]
fn phase236_sqrt3_k_log31_k_over_k4_converges() {
    // √k^3·log(k)^31 / k^4: effective_x2=3; 2·4=8 > 3 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k236a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_236a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f236a = apply(sym(SUB), vec![g_k236a, g_kp1_236a]);
    let out = evaluate_sum(f236a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase236_sqrt3_k_log31_k_times_k_over_k5_converges() {
    // √k^3·log(k)^31·k / k^5: effective_x2=5; 2·5=10 > 5 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k236b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_236b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f236b = apply(sym(SUB), vec![g_k236b, g_kp1_236b]);
    let out = evaluate_sum(f236b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase236_sqrt3_k_log32_k_over_k_refused() {
    // √k^3·log(k)^32·k / k: 32 logs → refused (Phase 236 handles exactly 31).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_236c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f236c = apply(sym(SUB), vec![g_k236c, g_kp1_236c]);
    let out = evaluate_sum(f236c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 237: 4-sqrt × 31-log × polynomial ──────────────────────────────────

#[test]
fn phase237_sqrt4_k_log31_k_over_k4_converges() {
    // √k^4·log(k)^31 / k^4: effective_x2=4; 2·4=8 > 4 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k237a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_237a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f237a = apply(sym(SUB), vec![g_k237a, g_kp1_237a]);
    let out = evaluate_sum(f237a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase237_sqrt4_k_log31_k_times_k_over_k5_converges() {
    // √k^4·log(k)^31·k / k^5: effective_x2=6; 2·5=10 > 6 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k237b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_237b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f237b = apply(sym(SUB), vec![g_k237b, g_kp1_237b]);
    let out = evaluate_sum(f237b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase237_sqrt4_k_log32_k_over_k_refused() {
    // √k^4·log(k)^32·k / k: 32 logs → refused (Phase 237 handles exactly 31).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_237c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f237c = apply(sym(SUB), vec![g_k237c, g_kp1_237c]);
    let out = evaluate_sum(f237c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 238: 5-sqrt × 31-log × polynomial ──────────────────────────────────

#[test]
fn phase238_sqrt5_k_log31_k_over_k4_converges() {
    // √k^5·log(k)^31 / k^4: effective_x2=5; 2·4=8 > 5 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k238a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_238a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f238a = apply(sym(SUB), vec![g_k238a, g_kp1_238a]);
    let out = evaluate_sum(f238a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase238_sqrt5_k_log31_k_times_k_over_k5_converges() {
    // √k^5·log(k)^31·k / k^5: effective_x2=7; 2·5=10 > 7 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k238b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_238b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f238b = apply(sym(SUB), vec![g_k238b, g_kp1_238b]);
    let out = evaluate_sum(f238b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase238_sqrt5_k_log32_k_over_k_refused() {
    // √k^5·log(k)^32·k / k: 32 logs → refused (Phase 238 handles exactly 31).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_238c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f238c = apply(sym(SUB), vec![g_k238c, g_kp1_238c]);
    let out = evaluate_sum(f238c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 239: 0-sqrt × 32-log × polynomial ──────────────────────────────────

#[test]
fn phase239_log32_k_over_k5_converges() {
    // log(k)^32 / k^5: effective_x2=0; 2·5=10 > 0 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let log_k = apply(sym(LOG), vec![k.clone()]);
    let log_kp1 = apply(sym(LOG), vec![kp1.clone()]);
    ...
    let num_kp1 = apply(sym(MUL), vec![
        ...
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1, // 32 logs
    ]);
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k239a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_239a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f239a = apply(sym(SUB), vec![g_k239a, g_kp1_239a]);
    let out = evaluate_sum(f239a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase239_log32_k_times_k_over_k5_converges() {
    // log(k)^32·k / k^5: effective_x2=2; 2·5=10 > 2 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let log_k = apply(sym(LOG), vec![k.clone()]);
    let log_kp1 = apply(sym(LOG), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k239b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_239b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f239b = apply(sym(SUB), vec![g_k239b, g_kp1_239b]);
    let out = evaluate_sum(f239b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase239_log33_k_times_k_over_k2_refused() {
    // log(k)^33·k / k^2: 33 logs → refused (Phase 239 handles exactly 32).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let log_k = apply(sym(LOG), vec![k.clone()]);
    let log_kp1 = apply(sym(LOG), vec![kp1.clone()]);
    let num_k = apply(sym(MUL), vec![
        ...
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k, // 33 logs
        k.clone(),
    ]);
    let num_kp1 = apply(sym(MUL), vec![
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        ...
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1, // 33 logs
        kp1.clone(),
    ]);
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone()]);
    let g_k239c = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_239c = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f239c = apply(sym(SUB), vec![g_k239c, g_kp1_239c]);
    let out = evaluate_sum(f239c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 240: 1-sqrt × 32-log × polynomial ──────────────────────────────────

#[test]
fn phase240_sqrt1_k_log32_k_over_k4_converges() {
    // √k^1·log(k)^32 / k^4: effective_x2=1; 2·4=8 > 1 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k240a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_240a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f240a = apply(sym(SUB), vec![g_k240a, g_kp1_240a]);
    let out = evaluate_sum(f240a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase240_sqrt1_k_log32_k_times_k_over_k5_converges() {
    // √k·log(k)^32·k / k^5: effective_x2=3; 2·5=10 > 3 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k240b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_240b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f240b = apply(sym(SUB), vec![g_k240b, g_kp1_240b]);
    let out = evaluate_sum(f240b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase240_sqrt1_k_log33_k_times_k_over_k_refused() {
    // √k·log(k)^33·k / k: 33 logs → refused (Phase 240 handles exactly 32).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let g_kp1_240c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f240c = apply(sym(SUB), vec![g_k240c, g_kp1_240c]);
    let out = evaluate_sum(f240c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 241: 2-sqrt × 32-log × polynomial ──────────────────────────────────

#[test]
fn phase241_sqrt2_k_log32_k_over_k4_converges() {
    // √k^2·log(k)^32 / k^4: effective_x2=2; 2·4=8 > 2 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k241a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_241a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f241a = apply(sym(SUB), vec![g_k241a, g_kp1_241a]);
    let out = evaluate_sum(f241a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase241_sqrt2_k_log32_k_times_k_over_k5_converges() {
    // √k^2·log(k)^32·k / k^5: effective_x2=4; 2·5=10 > 4 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k241b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_241b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f241b = apply(sym(SUB), vec![g_k241b, g_kp1_241b]);
    let out = evaluate_sum(f241b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase241_sqrt2_k_log33_k_over_k_refused() {
    // √k^2·log(k)^33 / k: 33 logs → refused (Phase 241 handles exactly 32).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let f241c = apply(sym(SUB), vec![g_k241c, g_kp1_241c]);
    let out = evaluate_sum(f241c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 242: 3-sqrt × 32-log × polynomial ──────────────────────────────────

#[test]
fn phase242_sqrt3_k_log32_k_over_k4_converges() {
    // √k^3·log(k)^32 / k^4: effective_x2=3; 2·4=8 > 3 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k242a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_242a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f242a = apply(sym(SUB), vec![g_k242a, g_kp1_242a]);
    let out = evaluate_sum(f242a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase242_sqrt3_k_log32_k_times_k_over_k5_converges() {
    // √k^3·log(k)^32·k / k^5: effective_x2=5; 2·5=10 > 5 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k242b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_242b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f242b = apply(sym(SUB), vec![g_k242b, g_kp1_242b]);
    let out = evaluate_sum(f242b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase242_sqrt3_k_log33_k_over_k_refused() {
    // √k^3·log(k)^33 / k: 33 logs → refused (Phase 242 handles exactly 32).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let f242c = apply(sym(SUB), vec![g_k242c, g_kp1_242c]);
    let out = evaluate_sum(f242c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 243: 4-sqrt × 32-log × polynomial ──────────────────────────────────

#[test]
fn phase243_sqrt4_k_log32_k_over_k4_converges() {
    // √k^4·log(k)^32 / k^4: effective_x2=4; 2·4=8 > 4 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k243a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_243a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f243a = apply(sym(SUB), vec![g_k243a, g_kp1_243a]);
    let out = evaluate_sum(f243a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase243_sqrt4_k_log32_k_times_k_over_k5_converges() {
    // √k^4·log(k)^32·k / k^5: effective_x2=6; 2·5=10 > 6 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k243b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_243b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f243b = apply(sym(SUB), vec![g_k243b, g_kp1_243b]);
    let out = evaluate_sum(f243b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase243_sqrt4_k_log33_k_over_k_refused() {
    // √k^4·log(k)^33·k / k: 33 logs → refused (Phase 243 handles exactly 32).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let f243c = apply(sym(SUB), vec![g_k243c, g_kp1_243c]);
    let out = evaluate_sum(f243c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

// ── Phase 244: 5-sqrt × 32-log × polynomial ──────────────────────────────────

#[test]
fn phase244_sqrt5_k_log32_k_over_k4_converges() {
    // √k^5·log(k)^32 / k^4: effective_x2=5; 2·4=8 > 5 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let num_k = apply(sym(MUL), vec![
        ...
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(),
        log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k.clone(), log_k, // 32 logs
    ]);
    let num_kp1 = apply(sym(MUL), vec![
        sqrt_kp1.clone(), sqrt_kp1.clone(), sqrt_kp1.clone(), sqrt_kp1.clone(), sqrt_kp1,
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        ...
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1, // 32 logs
    ]);
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k244a = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_244a = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f244a = apply(sym(SUB), vec![g_k244a, g_kp1_244a]);
    let out = evaluate_sum(f244a, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase244_sqrt5_k_log32_k_times_k_over_k5_converges() {
    // √k^5·log(k)^32·k / k^5: effective_x2=7; 2·5=10 > 7 → converges.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let den_k = apply(sym(MUL), vec![k.clone(), k.clone(), k.clone(), k.clone(), k.clone()]);
    let den_kp1 = apply(sym(MUL), vec![kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone(), kp1.clone()]);
    let g_k244b = apply(sym(DIV), vec![num_k, den_k]);
    let g_kp1_244b = apply(sym(DIV), vec![num_kp1, den_kp1]);
    let f244b = apply(sym(SUB), vec![g_k244b, g_kp1_244b]);
    let out = evaluate_sum(f244b, k, int(1), sym("%inf"), eval);
    assert!(!matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}

#[test]
fn phase244_sqrt5_k_log33_k_over_k_refused() {
    // √k^5·log(k)^33·k / k: 33 logs → refused (Phase 244 handles exactly 32).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
    ...
    let num_kp1 = apply(sym(MUL), vec![
        ...
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(), log_kp1.clone(),
        log_kp1.clone(), log_kp1, // 33 logs
        kp1.clone(),
    ]);
    let g_k244c = apply(sym(DIV), vec![num_k, k.clone()]);
    let g_kp1_244c = apply(sym(DIV), vec![num_kp1, kp1.clone()]);
    let f244c = apply(sym(SUB), vec![g_k244c, g_kp1_244c]);
    let out = evaluate_sum(f244c, k, int(1), sym("%inf"), eval);
    assert!(matches!(&out, IRNode::Apply(node) if node.head == sym(SUM)));
}
