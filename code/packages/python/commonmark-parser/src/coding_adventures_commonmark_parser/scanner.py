...
def is_ascii_punctuation(ch: str) -> bool:
    ...
    The CommonMark ASCII punctuation set is:
      ! " # $ % & ' ( ) * + , - . / : ; < = > ? @ [ \\ ] ^ _ ` { | } ~
    """
...
def is_unicode_punctuation(ch: str) -> bool:
    ...
    which matches what the CommonMark spec requires.
    """
    if not ch:
        return False
...
def is_unicode_whitespace(ch: str) -> bool:
    ...
    We check the standard whitespace categories as defined by CommonMark's
