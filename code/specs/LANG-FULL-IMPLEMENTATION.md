...
- ◑ **AL4** — literal string `print`/`output` I/O runs on all 7 backends via
  **E4**. Undeclared statement-position `print('HI')`/`output('HI')` calls lower
  ...
  domains must be present. Uninitialized or nonlocal values, inexact integer
  widening, fractional or non-finite results, conditionals, calls, nested
  powers, and standard-free bases remain on `f64_pow`.
