...
| `language/` | common word → a word that sounds the same but is spelled/means differently (`homophone(word, sound_alike)`, there/flower/to → their/flour/too) — a sibling library to the already-shipped `opposites.adj`/`synonyms.adj`, same source family and trust tier | English Wiktionary "Homophones" lines (consensus; see `homophones.adj`'s header) |
...
| `language/` | example word → which part of speech it is, in a sentence that shows it doing that job (`part_of_speech(word, category)`, queen/run/brown → noun/verb/adjective) — picked via the mandatory full-tree-grep-before-scoping check, confirming zero prior coverage | Grammarly "The 8 Parts of Speech" (consensus; see `part-of-speech.adj`'s header) |
...
| `language/` | common simile → what it actually means (`simile_meaning(simile, meaning)`, as_brave_as_a_lion/like_a_needle_in_a_haystack/as_free_as_a_bird → extremely_courageous/very_difficult_to_find/free_or_unrestricted) — a sibling figurative-language library to `idiom-meaning.adj`, picked via the mandatory full-tree-grep-before-scoping check, confirming zero prior coverage | Grammarly "Simile: Definition and Examples" (consensus; see `simile-meaning.adj`'s header) |
| `language/` | common word-morphology prefix → what it actually means (`prefix_meaning(prefix, meaning)`, un_/re_/dis_ → negation_or_absence/doing_again/negation_or_reversal) — a genuinely new topic distinct from `metric-prefixes.adj` (metric UNIT prefixes), introduces a new trailing-underscore atom-label convention for prefix position | Grammarly "Prefixes: Definition and Examples" (consensus; see `prefix-meaning.adj`'s header) |
...
| `language/` | noun type → what it actually is (`noun_type(type, definition)`, common_noun/collective_noun/abstract_noun → generic_name_of_an_item_in_a_class_or_group/denotes_a_group_or_collection_of_people_or_things/cannot_be_perceived_by_the_senses) — picked via the mandatory full-tree-grep-before-scoping check, confirming zero prior coverage; honest abstention on `possessive_noun` (a real noun type the source mentions and describes functionally, but for which it gives no single formal definition sentence, and not one of these three tabled here) | Grammarly "Nouns: Definition and Examples" (consensus; see `noun-type.adj`'s header) |
| `language/` | verb type → what it actually is (`verb_type(type, description)`, action_verb/linking_verb/auxiliary_verb → physical_action_or_activity_that_can_be_seen_or_heard/connects_the_subject_to_other_words_in_the_sentence/changes_another_verbs_tense_voice_or_mood) — picked via the mandatory full-tree-grep-before-scoping check, confirming zero prior coverage; honest abstention on `transitive_verb` (a real verb category the source covers, but only through worked examples rather than a single formal definition sentence, and not one of these three tabled here) | Grammarly "Verbs: Definition and Examples" (consensus; see `verb-type.adj`'s header) |
