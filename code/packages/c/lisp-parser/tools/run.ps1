# Build and run the lisp-parser tests under MSVC (cl.exe) — pure ISO C/C++ — via the
...
$env:ISO_INCLUDE = "include ..\lisp-lexer\include $harness\include"
. (Join-Path $harness 'lib\iso-lib.ps1')
Iso-BuildAndRun -Lang c -Name lisp_parser-tests -Sources @("tests\lisp_parser_test.c", "src\lisp_parser.c", "..\lisp-lexer\src\lisp_lexer.c")
