...
**Status:** XAML, Flutter, Compose, and Qt implemented; SwiftUI not yet — see §4.
...
| Flutter | **implemented** — lowers to `SizedBox(width:, height:, child: CircularProgressIndicator(value: (v).toDouble() / 100.0))` (`mosaic-emit-flutter::emit_host_progress_ring`), reading the same `width`/`height` style props every other primitive's part-style lookup already resolves (omitted from `SizedBox` entirely when unset). `a11y-label` wraps the ring in `Semantics(label: ...)`, matching `HostSlider`'s own pattern. Same widget already used indeterminate (no `value` arg) for the Icon spinner case. Verified against the real toolchain: a real `mosaic-compile pkg --backend flutter --profile native-complete` build produces `nativeComplete: true` with zero degradations, and the generated `Ring` widget — both standalone and mounted with real constructor args inside a `MaterialApp` — passes `dart analyze` cleanly. |
| Compose | **implemented** — lowers to `CircularProgressIndicator(progress = (v).toFloat() / 100f, modifier = Modifier.width(...).height(...).semantics { contentDescription = ... })` (`mosaic-emit-compose::emit_host_progress_ring`), reusing `compose_style_for_node`'s existing `.width().height()` modifier chain for sizing and the same `CircularProgressIndicator` widget already used indeterminate for the Icon spinner case. Widened that widget's Kotlin import gate from `uses_icon`-only to `uses_icon || uses_progress_ring`, since a component using `HostProgressRing` without an `Icon` would otherwise reference an unresolved symbol. Verified against the real toolchain: a scratch `org.jetbrains.compose` 1.6.11 Gradle project confirmed the plain-`Float` `progress:` overload compiles with `gradle compileKotlin` (no deprecation warning — this pinned Material1 version predates the newer `progress: () -> Float` lambda form) and mounts without crashing via `gradle run`; after implementation, `analyze_package_degradations` against a real probe package confirms `nativeComplete: true` with zero degradations on the Compose backend. |
| Qt | **implemented** — Qt Quick Controls 2 has no out-of-the-box circular *determinate* ring (`ProgressBar` is linear; `BusyIndicator`, already used for the Icon spinner, is indeterminate-only with no `value` prop), so this draws the arc by hand: `Shape { ShapePath { PathAngleArc { ... } } }` from `QtQuick.Shapes` (`mosaic-emit-qt::emit_host_progress_ring_qml`), the same module already imported for `Path`'s `line`/`curve` kinds. `centerX`/`centerY`/`radiusX`/`radiusY` are computed as literal numbers in Rust from the part's `width`/`height` style (falling back to 34px when unstyled) rather than read back from the `Shape` at runtime — a real circular binding loop (`Shape`'s own `implicitWidth` derives from its path's rendered bounds, which depended on radii computed from that same `implicitWidth`) and a `ReferenceError` from unqualified property lookups inside `pragma ComponentBehavior: Bound` were both found via the real end-to-end verification below, not the isolated probe, and fixed by computing geometry as plain numbers at emit time instead. `value` supports the full `Number`/`SlotRef`/`Expr` binding via a new `required_progress_ring_value` helper; a sibling `property real value` on the `Shape` lets Qt's generic accessibility bridge (`QAccessibleQuickItem`) expose it through `QAccessibleValueInterface`. Verified against the real toolchain in two passes: an isolated `Shape`/`ShapePath`/`PathAngleArc` probe confirmed the approach was feasible via `qmllint` and a real `qml` runtime window (Qt 6.8.1, the version this repo pins); after implementation, a real `.mil`/`.mll`/`.msl` triple compiled through `mosaic-compile --backend qt` produced output that passed `qmllint` with zero warnings and ran cleanly in a real `qml` window while cycling `value` through 0, 50, 100, and a fractional 33.5. |
