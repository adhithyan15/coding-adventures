...
smallest tranche remains Urdu: HL-C09V audits independent ج from the textbook's
dedicated jīm chapter and preserves its attested head-shape variation.
...
| HL-C09 | Queued — 21 of 228 verified | Expand `DUCTUS` to cover the ten scripts with prose stroke-order entries. | Add cited, font-checked ductus and verified pen-lift metadata for the remaining 207 entries measured by HL-C19; each passes the on-ink, join-tolerance, coverage, citation, and source-agreement invariants. |
...
| HL-C09V | Queued — next | Locate and verify independent Urdu ج in *Zer o Zabar*'s source-backed jīm chapter. | Audit the independent handwritten animation, its pointed-versus-flat head variation, and the dot as separate evidence before authoring the smallest next Urdu path with source, lifts, font, and filmstrip agreement. |
