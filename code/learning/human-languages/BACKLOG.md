...
one lift precedes a restarted sweep through the trailing bowl. The same page
directly links `FullSizeRender-5.mov` for Daad next. Dhaal is source-available but is not one of
this Arabic ledger's 21 measured entries.
...
| HL-C09 | Queued — 44 of 228 verified | Expand `DUCTUS` to cover the ten scripts with prose stroke-order entries. | Add cited, font-checked ductus and verified pen-lift metadata for the remaining 184 entries measured by HL-C19; each passes the on-ink, join-tolerance, coverage, citation, and source-agreement invariants. |
...
| HL-C09AT | Queued — next | Audit independent Arabic ض from the same source-backed alphabet page. | Inspect the page's directly linked `FullSizeRender-5.mov`, verify whether Daad repeats Saad's body and bowl before or after its upper dot, and record every lift without assuming the shared skeleton fixes the dot order. |
