...
`str_eq`, or `str_concat` for the result path. The dynamic `call_builtin` paths
remain in place for non-literal string values.
...
| Direct literal string metadata (`string-length`, `string-ref`, `string=?`, `string-append`) | `str_const` + `str_len`/`str_index`/`str_eq`/`str_concat` |
