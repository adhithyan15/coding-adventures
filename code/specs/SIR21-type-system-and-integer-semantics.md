...
| T3c-3 | backends | Have each backend's emitter consult `resolve_binary` when lowering a `BuiltinCall` binary op (currently every operand is `Dynamic`, so all resolve to `RuntimeDispatch` — behaviour-preserving until a typed frontend or sized-int lowering supplies types). |
