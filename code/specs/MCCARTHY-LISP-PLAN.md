...
| **L4** ✓ | `ibm704-encoder` + `ibm704-backend` v0.1.0 (minimal viable: `const_*` + `ret_*`, just like the Phase 5/6 minimal-viable backends).  36-bit IBM 704 words packed 5 bytes per word (low byte first, top nibble of high byte zeroed).  `HTR 0` (opcode `0o420`) as halt sentinel, `CLA n` (opcode `0o500`) for accumulator load.  Twig `42` → `[CLA 42, HTR 0]` = `[0xA_0000_002A, 0x8_8000_0000]` = `[0x2A, 0x00, 0x00, 0x00, 0x0A, 0x00, 0x00, 0x00, 0x80, 0x08]`.  26 unit tests (13 encoder + 12 backend + 1 doctest) pin every byte. |
| **L5** ✓ | Wire `lang-aot --emit=ibm704` (aliases `ibm-704`, `704`) through `ibm704-backend`.  `lang-aot::compile_file_to_ibm704_bin` routes source through `aot_core::infer` + `aot_core::specialise` + `ibm704_backend::compile`.  Two e2e smoke tests pin **Twig `42` → 10 bytes** and **McCarthy `42` → identical 10 bytes** (the IIR convergence proof — same source, same backend, same machine code, two frontends).  `lang-aot` bumped to v0.45.0. |
