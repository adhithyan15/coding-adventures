...
pub fn token_names(grammar: &TokenGrammar) -> HashSet<String> {
    ...
    // A `keywords:` block synthesizes a KEYWORD token that never appears as an
    // explicit definition. `GrammarLexer::resolve_token_type` promotes any NAME
    // whose value is in the keyword set:
    //
    //     if token_name == "NAME" && self.keyword_set.contains(lookup_value) {
    //         return Ok((TokenType::Keyword, Some("KEYWORD".to_string())));
    //     }
    //
    // so the parser really does see a token named `KEYWORD`, and a parser
    // grammar may legitimately reference it. Without this, every grammar that
    // declares keywords and matches them by token type is reported as
    // referencing an undefined token — which is what happened to all four
    // Mosaic grammars (#14067). The Mosaic compilers have worked the whole
    // time; only this function's view of them was wrong.
    //
    // Gated on the block being non-empty so a grammar with no keywords still
    // gets a real error if it references KEYWORD.
    if !grammar.keywords.is_empty() || !grammar.layout_keywords.is_empty() {
...
pub fn validate_token_grammar(grammar: &TokenGrammar) -> Vec<String> {
    ...
    //
    // Two modes are legal, and they name the two branches the lexer actually
    // has (`GrammarLexer`, STRING handling):
    //
    //   escapes: none      leave escape sequences as raw text after stripping
    //                      quotes. CSS and TOML use this, because their
    //                      semantic layer owns escape processing.
    //   escapes: standard  process escapes normally.
    //
    // `standard` is the lexer's default — the branch there is
    // `if escape_mode == Some("none") { raw } else { process_escapes() }`,
    // so an absent directive and `standard` are the same code path. Naming it
    // explicitly is therefore documentation, not behavior.
    //
    // It has to be accepted because the four Mosaic grammars (mosmodel,
    // moslayout, mosstyle, mosaic) declare it, and rejecting it made their
    // `.tokens` files fail to parse at all. That cascaded: the `keywords:`
    // block never produced its tokens, so cross-validation then reported
    // `Undefined token reference: 'KEYWORD'` — a symptom that looked like a
    // second, unrelated bug. Those grammars could not be regenerated by the
    // command their own generated files name (#14067).
    if let Some(ref esc) = grammar.escapes {
        if esc != "none" && esc != "standard" {
            issues.push(format!(
                "Unknown escape mode '{}' (supported: 'none', 'standard')",
                esc
            ));
        }
    }
...
case_sensitive: true,
    ...
    /// `escapes: standard` is a legal mode, not an error.
    ///
    /// The lexer's STRING branch is `if escape_mode == Some("none") { raw }
    /// else { process_escapes() }`, so `standard` names the default path.
    /// Rejecting it made all four Mosaic `.tokens` files fail to parse, which
    /// cascaded into a bogus "Undefined token reference: 'KEYWORD'" (#14067).
    #[test]
    fn test_escapes_standard_is_accepted() {
        let src = "escapes: standard\n\nNAME = /[a-z]+/\n";
        let g = parse_token_grammar(src).expect("should parse");
        assert_eq!(g.escapes.as_deref(), Some("standard"));
        let issues = validate_token_grammar(&g);
        assert!(
            !issues.iter().any(|i| i.contains("escape mode")),
            "standard must not be reported as unknown: {issues:?}"
        );
    }

    /// An escape mode that is neither `none` nor `standard` is still rejected,
    /// so widening the allowlist did not turn the check off.
    #[test]
    fn test_unknown_escape_mode_still_rejected() {
        let src = "escapes: klingon\n\nNAME = /[a-z]+/\n";
        let g = parse_token_grammar(src).expect("should parse");
        let issues = validate_token_grammar(&g);
        assert!(
            issues.iter().any(|i| i.contains("Unknown escape mode")),
            "an unknown mode must still error: {issues:?}"
        );
    }

    /// A `keywords:` block synthesizes a KEYWORD token, so `token_names` must
    /// report it. `GrammarLexer` promotes any NAME in the keyword set to
    /// `(TokenType::Keyword, Some("KEYWORD"))`, and parser grammars reference
    /// `KEYWORD` directly — all four Mosaic grammars do, in 6+ rules each.
    #[test]
    fn test_keywords_block_defines_the_keyword_token() {
        let src = "keywords:\n  component\n  slot\n\nNAME = /[a-z]+/\n";
        let g = parse_token_grammar(src).expect("should parse");
        let names = token_names(&g);
        assert!(names.contains("KEYWORD"), "expected KEYWORD in {names:?}");
        assert!(names.contains("NAME"));
    }

    /// Without a keywords block there is no KEYWORD token, so referencing one
    /// stays a genuine error rather than being silently allowed everywhere.
    #[test]
    fn test_no_keywords_block_means_no_keyword_token() {
        let src = "NAME = /[a-z]+/\n";
        let g = parse_token_grammar(src).expect("should parse");
        assert!(!token_names(&g).contains("KEYWORD"));
    }
