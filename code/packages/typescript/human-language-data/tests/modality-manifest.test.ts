...
describe("corpus regression", () => {
  ...
  it("pins the corpus summary the manifest publishes", () => {
    ...
    expect(manifest.summary).toEqual({
      ...
      pen: 240, // HL-C168: +1 -- Kannada's ledger closes at 24 of 24 // HL-C156: 85 script segments across the six Indic tracks // chinese script chapter: +7 lessons, +1 chapter -- the seven components 你好 is built from, one glyph each // japanese hiragana tranche: +10 lessons, +1 chapter -- eight signs, one per lesson, plus two assemblies that introduce none // gujarati script tranche: +10 lessons, +1 chapter (ch13) -- nine pieces, one per lesson, plus one assembly // marathi script tranche: +11 lessons, +1 chapter (ch14) -- ten pieces, one per lesson, plus one assembly
