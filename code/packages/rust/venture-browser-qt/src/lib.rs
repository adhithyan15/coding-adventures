//! Cairo page-content bridge for Venture's Mosaic-generated Qt Quick shell.
//!
//! The Qt adapter owns no browser behavior. Navigation, chrome projection,
//! scrolling, hover, link activation, and retained-page reflow all delegate to
//! `venture-browser-core::BrowserHostController`; this crate only composes the
//! shared page pipeline and exposes Cairo-rendered RGBA pixels to Qt.
...
/// One browser session shared by generated Mosaic chrome and the Qt surface.
pub struct QtBrowserHost {
...
impl QtBrowserHost {
    ...
    pub fn render_rgba(&self) -> Result<(u32, u32, Vec<u8>), String> {
        let scene = self
            .controller
            .session()
            .viewport()
            .ok_or_else(|| "Venture browser has no retained viewport".to_string())?
            .viewport_scene();
        let pixels = paint_vm_cairo::render(&scene).map_err(|error| format!("{error:?}"))?;
        Ok((pixels.width, pixels.height, pixels.data))
    }
}
...
mod ffi {
    ...
    fn response(host: &QtBrowserHost, error: Option<&str>) -> *mut c_char {
    ...
    #[no_mangle]
    pub extern "C" fn venture_browser_qt_new(
        ...
        catch_unwind(|| QtBrowserHost::new(&start_url, width, height))
            .ok()
            .and_then(Result::ok)
            .map(Box::new)
            .map(Box::into_raw)
    ...
    #[no_mangle]
    pub unsafe extern "C" fn venture_browser_qt_free(host: *mut QtBrowserHost) {
        if !host.is_null() {
    ...
    #[no_mangle]
    pub unsafe extern "C" fn venture_browser_qt_apply_props(
    ...
    #[no_mangle]
    pub unsafe extern "C" fn venture_browser_qt_handle_event(
        host: *mut QtBrowserHost,
        name: *const c_char,
        value: *const c_char,
    ) -> *mut c_char {
    ...
    #[no_mangle]
    pub unsafe extern "C" fn venture_browser_qt_scroll(
        host: *mut QtBrowserHost,
        delta_y: f64,
    ) -> u8 {
        host.as_mut()
    ...
    #[no_mangle]
    pub unsafe extern "C" fn venture_browser_qt_scroll_command(
        host: *mut QtBrowserHost,
        name: *const c_char,
    ) -> u8 {
    ...
    #[no_mangle]
    pub unsafe extern "C" fn venture_browser_qt_activate_link(
        host: *mut QtBrowserHost,
        x: f64,
        y: f64,
    ) -> u8 {
    ...
    #[no_mangle]
    pub unsafe extern "C" fn venture_browser_qt_update_hover(
        host: *mut QtBrowserHost,
        x: f64,
        y: f64,
    ) -> u8 {
    ...
    #[no_mangle]
    pub unsafe extern "C" fn venture_browser_qt_scroll_metrics(
        host: *mut QtBrowserHost,
        offset_y: *mut f64,
        ...
        let Some(metrics) = host.as_ref().and_then(QtBrowserHost::scroll_metrics) else {
            return 0;
        };
        if !offset_y.is_null() {
    ...
    #[no_mangle]
    pub unsafe extern "C" fn venture_browser_qt_scroll_to(
        host: *mut QtBrowserHost,
        offset_y: f64,
    ) -> u8 {
        host.as_mut()
    ...
    #[no_mangle]
    pub unsafe extern "C" fn venture_browser_qt_resize(
        host: *mut QtBrowserHost,
        width: f64,
        height: f64,
    ) -> u8 {
    ...
    /// Render RGBA8 pixels for a Qt `QImage::Format_RGBA8888` surface.
    #[no_mangle]
    pub unsafe extern "C" fn venture_browser_qt_render_rgba(
        host: *mut QtBrowserHost,
        output: *mut u8,
        capacity: usize,
        width: *mut u32,
        ...
        catch_unwind(AssertUnwindSafe(|| {
            ...
            let Ok((pixel_width, pixel_height, pixels)) = host.render_rgba() else {
                return 0;
            };
            if !width.is_null() {
                *width = pixel_width;
    ...
    #[no_mangle]
    pub unsafe extern "C" fn venture_browser_qt_string_free(value: *mut c_char) {
        if !value.is_null() {
...
mod tests {
    ...
    #[test]
    fn generated_chrome_and_cairo_surface_share_one_browser_session() {
        let fetcher = |url: &str| match url {
            "http://example.test/" => Ok(page(
                url,
                "Home",
                "<a href='http://example.test/next'>Next</a><p>Rendered by Cairo</p>",
            )),
            "http://example.test/next" => Ok(page(url, "Next", "done")),
            _ => Err(format!("unexpected URL {url}")),
        };
        let mut host = QtBrowserHost::new_with_fetcher(
            "http://example.test/",
            320.0,
            180.0,
            Box::new(fetcher),
        ...
        let (width, height, pixels) = host.render_rgba().expect("Cairo renders the page");
