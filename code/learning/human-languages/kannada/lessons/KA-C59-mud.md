---
schema_version: 2
id: KA-C59-mud
spine_node: SPINE-TAKE-LEAVE
sequence: 1820
chapter: 59
type: word
headword: "ಕೆಸರು"
gloss: "mud"
romanization: "kesaru"
concept_tag: KA-NOUN-MUD
prerequisites: [KA-C59-sand, KA-C57-shoulder]
sounds: []
roots: []
duration:
  max_seconds: 210
requires:
  knowledge: [KA-LEX-C59-GROUND-03]
introduces:
  knowledge: [KA-LEX-C59-GROUND-04]
practises:
  knowledge: [KA-LEX-C59-GROUND-01, KA-LEX-C59-GROUND-02, KA-LEX-C59-GROUND-03, KA-LEX-C59-GROUND-04, KA-LEX-C57-BODY-04]
skills: [listening, speaking, reading]
modes: [interpretive, interpersonal, presentational]
strands: [meaning-input, meaning-output]
register: neutral
variety: standard-colloquial
reviews_of: [KA-C59-sand]
---

# ಕೆಸರು (kesaru) — mud

## Warm-up
<!-- hl-knowledge: introduces=[]; assesses=[KA-LEX-C59-GROUND-01, KA-LEX-C59-GROUND-02, KA-LEX-C59-GROUND-03] -->

[PAUSE 2s] Before the new one: what did *maraḷu* mean?

## You'll want to know: ಕೆಸರು
<!-- hl-knowledge: introduces=[KA-LEX-C59-GROUND-04]; assesses=[] -->

**ಕೆಸರು** (*kesaru*) — "mud".

Inherited, and unavoidable. From June onward the ಮಳೆಗಾಲ turns every ದಾರಿ that is not paved into ಕೆಸರು, and a ಹಳ್ಳಿ measures the season by how much of it there is between the house and the road.

It is also the mud a rice field wants. Paddy is planted into standing water and soft ground, so ಕೆಸರು is not a complaint in a farmer's mouth the way it is in a city one — the ರೈತ needs it, and a dry season without it is the bad outcome.

Then, as everywhere, it goes figurative. To throw ಕೆಸರು at somebody is to smear their name, and Kannada arrived at that picture on its own, from a place where mud is genuinely at hand for half the year.

## What you've built
<!-- hl-knowledge: introduces=[]; assesses=[] -->

Four, and this one the season makes.

## Guided Practice
<!-- hl-knowledge: introduces=[]; assesses=[KA-LEX-C59-GROUND-01, KA-LEX-C59-GROUND-02, KA-LEX-C59-GROUND-03, KA-LEX-C59-GROUND-04, KA-LEX-C57-BODY-04] -->

[PAUSE 1s]
- [YOU SAY: *kesaru*]
- [YOU SAY: it once more, slowly]
- [YOU SAY: *maraḷu*, then *kesaru*, and say which one the rain makes]
- [YOU RECALL: read **ಭುಜ**]

## Wrap-up Recall
<!-- hl-knowledge: introduces=[]; assesses=[KA-LEX-C59-GROUND-01, KA-LEX-C59-GROUND-02, KA-LEX-C59-GROUND-03, KA-LEX-C59-GROUND-04] -->

[PAUSE 3s] What does ಕೆಸರು mean? ("mud".) Say it once more.
