...
Tokenizes Java source code using the grammar-driven lexer engine. A thin
...
```go
import javalexer "github.com/adhithyan15/coding-adventures/code/packages/go/java-lexer"

// Default grammar (Java 21) — best when you don't know the exact Java version.
tokens, err := javalexer.TokenizeJava("int x = 1 + 2;", "")

// Versioned grammar — pin to a specific Java release.
tokens, err := javalexer.TokenizeJava("var x = 1;", "10")
```
