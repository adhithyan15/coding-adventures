...
describe("the real corpus", () => {
  ...
  it("pins what nothing had measured", { timeout: 30_000 }, () => {
    ...
    expect(report.summary.atomsTaught).toBe(2881); // +4: ES-LEX-VOS-01, ES-CULTURE-VOSEO-02 (ES-C03-vos) // +3: HL-C98 splits AR-PRESENT-SINGULAR into 1SG/2SG/3SG // +83: vocabulary wave 5 (persian ch9-11, telugu ch35-40, malayalam ch35-40)
    ...
    expect(report.summary.atomsNeverRevisited).toBe(468); // -17: payoff lessons plus si/no moving early // +2: HL-C98's per-cell atoms are revisited later than the R-windows reach // -8: vocabulary wave 5 rescues net orphan atoms via reach-back payoffs
    ...
    expect(report.summary.forwardReferences).toBe(451); // -1: HL-C98 removes a forward reference // -5: HL-C103 census adds comes/hand/regular to ENGLISH_COLLISIONS, and drops an untaught tres from an accepted list // +19: vocabulary wave 5
    ...
    // R1 stops at 3.
    expect(report.summary.missedByWindow.R1).toBe(902); // +2: HL-C98 // +2: vocabulary wave 5
    ...
    expect(report.summary.missedByWindow.R2).toBe(2002); // +1: ES-C02-concordancia (HL-C85). buenos dias/buenas tardes stop REQUIRING the agreement rule; it becomes a payoff lesson after the learner has used all three greetings. // +84: vocabulary wave 5, new pre-A1 nouns landing late in already-long chapters
  });
