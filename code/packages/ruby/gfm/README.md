# coding_adventures_gfm

GFM 0.31.2 pipeline convenience package — parse Markdown to HTML in one call.

## Overview

`coding_adventures_gfm` is the thin top-level package that wires
`coding_adventures_gfm_parser` and `coding_adventures_document_ast_to_html`
...
coding_adventures_document_ast           ← format-agnostic types (TE00)
         ↓ types                                 ↓ types
coding_adventures_gfm_parser    coding_adventures_document_ast_to_html
  parse(markdown) → DocumentNode         to_html(doc) → String
         ↓ depends on both
coding_adventures_gfm            ← you are here
...
Combines **TE01 — GFM Parser** and **TE02 — Document AST to HTML**, both
built on **TE00 — Document AST**.

## Requirements

- Ruby >= 3.2.0
- `coding_adventures_gfm_parser` ~> 0.1
