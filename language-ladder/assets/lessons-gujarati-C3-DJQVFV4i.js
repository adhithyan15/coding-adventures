import{t as e}from"./rolldown-runtime-DK3Fl9T5.js";var t=e({default:()=>n}),n=`---
schema_version: 2
id: GU-C03-hun
spine_node: SPINE-EXCHANGE-NAMES
sequence: 180
chapter: 9
type: word
headword: હું
romanization: "hũ"
gloss: I
concept_tag: PRONOUN-I
prerequisites: [GU-C02-maarun, GU-W02-ya]
sounds: [ha, u-nasal]
roots: [aham-first-person]
duration:
  max_seconds: 180
requires:
  knowledge: [GU-CONCEPT-C02-MAARUN-01]
introduces:
  knowledge: [GU-CONCEPT-C03-HUN-01]
introduces_idioms: []
introduces_senses: []
introduces_culture_claims: []
practises:
  knowledge: [GU-CONCEPT-C02-MAARUN-01, GU-CONCEPT-C03-HUN-01, GU-SCRIPT-YA-01]
skills: [listening, speaking, reading, writing]
modes: [interpretive, interpersonal, presentational]
strands: [meaning-input, meaning-output, language-focus]
register: neutral
variety: standard-gujarati
reviews_of: [GU-C02-maarun]
---

# હું (hũ) — "I"

## Warm-up
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C02-MAARUN-01] -->

[PAUSE 2s] To answer "how are you?" you first need "I." In the name exchange you learned
*mārũ* ("my"); here is its owner.

## The letters in this word
<!-- hl-knowledge: introduces=[GU-CONCEPT-C03-HUN-01]; assesses=[GU-CONCEPT-C02-MAARUN-01] -->

**હ** (*ha*) + the *u*-sign + nasal → **હું** (*hũ*). One nasal syllable.

## The word, taken apart
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C02-MAARUN-01, GU-CONCEPT-C03-HUN-01] -->

**હું** (*hũ*, "I") descends from Sanskrit **અહમ્** (*aham*, "I"), worn down
through Old Gujarati (*haũ*) to *hũ*. That *aham* is from Proto-Indo-European
**\\*h₁eǵ(H)om** — the ancestor of Latin **ego** and English **I**. So Gujarati's
*hũ*, Latin *ego*, and English *I* are one ancient word. Its possessive is the
**મારું** (*mārũ*, "my") from that name exchange.

## Grammar Lens: "I" plus the copula
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C02-MAARUN-01, GU-CONCEPT-C03-HUN-01] -->

*Hũ* takes the copula **છું** (*chhũ*, "am"): *hũ … chhũ*. So the answer to *tame
kem chho?* begins **હું** (*hũ …*).

## Guided Practice
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C02-MAARUN-01, GU-CONCEPT-C03-HUN-01] -->

[PAUSE 1s]
- [YOU SAY: "hũ"]
- [YOU SAY: the family — *hũ* / Latin *ego* / English **I**]
- [YOU SAY: "hũ … chhũ" — I am …]

## Script retrieval
<!-- hl-knowledge: introduces=[]; assesses=[GU-SCRIPT-YA-01] -->
<!-- hl-activity: {"id":"GU-C03-hun-ya-retrieval","kind":"text","assesses":["GU-SCRIPT-YA-01"],"prompt":"Without looking back, write ય once.","answer":"ય","accepted":[],"feedback":{"correct":"Good: one careful retrieval is enough.","incorrect":"Look at ય once, hide it, and write it one time."},"response_seconds":12} -->

Hide the earlier model for **ય**. Write it once, compare, and repair only one
part if needed.

## Wrap-up Recall
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C02-MAARUN-01, GU-CONCEPT-C03-HUN-01] -->

[PAUSE 3s] What Sanskrit word is *hũ* from, and its cousins? (*Aham*; Latin *ego*,
English **I**.)
`,r=e({default:()=>i}),i=`---
schema_version: 2
id: GU-C03-kem
spine_node: SPINE-CHECK-WELLBEING
sequence: 190
chapter: 9
type: word
headword: કેમ
romanization: "kem"
gloss: how (also "why")
concept_tag: QUESTION-HOW
prerequisites: [GU-C02-shun, GU-W02-bha]
sounds: [e-sign]
roots: [ka-interrogative]
duration:
  max_seconds: 240
requires:
  knowledge: [GU-CONCEPT-C02-SHUN-01]
introduces:
  knowledge: [GU-CONCEPT-C03-KEM-01]
introduces_idioms: []
introduces_senses: [GU-SENSE-KEM-HOW-WHY-01]
introduces_culture_claims: [GU-CULTURE-KEM-CHHO-EVERYDAY-GUJARATI-HELLO-01]
practises:
  knowledge: [GU-CONCEPT-C02-SHUN-01, GU-CONCEPT-C03-KEM-01, GU-SCRIPT-BHA-01]
skills: [listening, speaking, reading, writing]
modes: [interpretive, interpersonal, presentational]
strands: [meaning-input, meaning-output, language-focus]
register: neutral
variety: standard-gujarati
reviews_of: [GU-C02-shun]
---

# કેમ (kem) — "how," a proper k- question

## Warm-up
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C02-SHUN-01] -->

[PAUSE 2s] Where *shũ* ("what") hid its k-, this one wears it plainly — and it
opens the most Gujarati of greetings.

## The letters in this word
<!-- hl-knowledge: introduces=[GU-CONCEPT-C03-KEM-01]; assesses=[GU-CONCEPT-C02-SHUN-01] -->

**ક** (*ka*) + **ે** (the *e*-sign) + **મ** (*ma*) → **કેમ** (*kem*).

## The word, taken apart
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C02-SHUN-01, GU-CONCEPT-C03-KEM-01] -->

**કેમ** (*kem*, "how"; also "why") is from the interrogative stem **ક-** (*ka-*),
PIE **\\*kʷo-** — the same root behind English **wh-** and Latin *qu-*. It heads
the plain *k-* family Gujarati mostly kept: *kem* (how), *kōṇ* (who), *kyā̃*
(where), *kyāre* (when). And **કેમ છો?** (*kem chho?*, "how are you?") is, for
most Gujaratis, the real everyday hello — warmer than *namaste*.

## Guided Practice
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C02-SHUN-01, GU-CONCEPT-C03-KEM-01] -->

[PAUSE 1s]
- [YOU SAY: "kem"]
- [YOU SAY: the k- question family — *kem, kōṇ, kyā̃, kyāre*]

## Script retrieval
<!-- hl-knowledge: introduces=[]; assesses=[GU-SCRIPT-BHA-01] -->
<!-- hl-activity: {"id":"GU-C03-kem-bha-retrieval","kind":"text","assesses":["GU-SCRIPT-BHA-01"],"prompt":"Without looking back, write ભ once.","answer":"ભ","accepted":[],"feedback":{"correct":"Good: one careful retrieval is enough.","incorrect":"Look at ભ once, hide it, and write it one time."},"response_seconds":12} -->

Hide the earlier model for **ભ**. Write it once, compare, and stop after one
possible repair.

## Wrap-up Recall
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C02-SHUN-01, GU-CONCEPT-C03-KEM-01] -->

[PAUSE 3s] What root does *kem* share with English *how/what*? (The interrogative
*ka-*, PIE *\\*kʷo-*.)
`,a=e({default:()=>o}),o=`---
schema_version: 2
id: GU-C03-majaa
spine_node: SPINE-CHECK-WELLBEING
sequence: 210
chapter: 9
type: word
headword: મજા
romanization: "majā"
gloss: enjoyment, fun — and the reply "હું મજામાં છું"
concept_tag: WORD-WELL
prerequisites: [GU-C03-hun, GU-C03-tame-kem-chho, GU-W02-vocalic-r]
sounds: [ja, long-aa]
roots: [maza-persian]
duration:
  max_seconds: 240
requires:
  knowledge: [GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C03-TAMEKEMCHHO-01]
introduces:
  knowledge: [GU-CONCEPT-C03-MAJAA-01]
introduces_idioms: [GU-IDIOM-HUN-MAJAMA-CHHUN-WELLBEING-REPLY-01]
introduces_senses: [GU-SENSE-MAJAA-ENJOYMENT-WELLBEING-01]
introduces_culture_claims: [GU-CULTURE-MAJAA-PERSIAN-ARABIAN-SEA-COMMERCE-01]
practises:
  knowledge: [GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C03-MAJAA-01, GU-CONCEPT-C03-TAMEKEMCHHO-01, GU-SCRIPT-VOCALIC-R-MATRA-01]
skills: [listening, speaking, reading, writing]
modes: [interpretive, interpersonal, presentational]
strands: [meaning-input, meaning-output, language-focus]
register: neutral
variety: standard-gujarati
reviews_of: [GU-C03-hun]
---

# મજા (majā) — "enjoyment," and how to answer

## Warm-up
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C03-TAMEKEMCHHO-01] -->

[PAUSE 2s] The word that answers "how are you?" — and it comes not from Sanskrit
but from Persia.

## The letters in this word
<!-- hl-knowledge: introduces=[GU-CONCEPT-C03-MAJAA-01]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C03-TAMEKEMCHHO-01] -->

**મ** (*ma*) + **જા** (*jā*) → **મજા** (*majā*).

## The word, taken apart
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C03-MAJAA-01, GU-CONCEPT-C03-TAMEKEMCHHO-01] -->

**મજા** (*majā*, "enjoyment, fun, relish, pleasure") is **not** Sanskrit — it is
**Perso-Arabic**, from Persian **maza** ("taste, relish"). Gujaratis, a
great trading people, wove thousands of Persian and Arabic words into their speech
over centuries of commerce across the Arabian Sea — and even "how are you" is
answered with one. To say "**I'm well**," Gujarati says **હું મજામાં છું** (*hũ
majāmā chhũ*) — literally "**I am in enjoyment**" (*majā* + *-mā* "in" + *chhũ*
"am").

## The exchange — the reply
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C03-MAJAA-01, GU-CONCEPT-C03-TAMEKEMCHHO-01] -->

> — *tame kem chho?* ("How are you?")
> — *majāmā, ābhār.* ("Well [lit. 'in enjoyment'], thank you.")

## Guided Practice
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C03-MAJAA-01, GU-CONCEPT-C03-TAMEKEMCHHO-01] -->

[PAUSE 1s]
- [YOU SAY: "majā"]
- [YOU SAY: the full reply — *hũ majāmā chhũ*]
- [YOU SAY: which heritage *majā* belongs to (Persian, not Sanskrit)]

## Script retrieval
<!-- hl-knowledge: introduces=[]; assesses=[GU-SCRIPT-VOCALIC-R-MATRA-01] -->
<!-- hl-activity: {"id":"GU-C03-majaa-vocalic-r-retrieval","kind":"text","assesses":["GU-SCRIPT-VOCALIC-R-MATRA-01"],"prompt":"Without looking back, write the vowel sign ૃ once.","answer":"ૃ","accepted":[],"feedback":{"correct":"Good: ૃ is the small vowel sign written below its consonant.","incorrect":"Look once at ૃ below a consonant, hide it, and try the sign one time."},"response_seconds":12} -->

Hide the earlier model for **ૃ**. Write the below-consonant sign once, compare,
and stop after one possible repair.

## Wrap-up Recall
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C03-MAJAA-01, GU-CONCEPT-C03-TAMEKEMCHHO-01] -->

[PAUSE 3s] How does Gujarati say "I'm well," and where does *majā* come from?
(*Hũ majāmā chhũ*, "I'm in enjoyment"; Persian *maza*, "taste/relish.")
`,s=e({default:()=>c}),c=`---
schema_version: 2
id: GU-C03-practice
spine_node: SPINE-CHECK-WELLBEING
sequence: 230
chapter: 9
type: practice
headword: (dialogue)
gloss: Chapter 5 recap — the how-are-you exchange
concept_tag: REVIEW
prerequisites: [GU-C03-tame-kem-chho, GU-C03-majaa, GU-C03-vandho-nahi]
sounds: []
roots: []
duration:
  max_seconds: 240
requires:
  knowledge: [GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C03-KEM-01, GU-CONCEPT-C03-MAJAA-01, GU-CONCEPT-C03-TAMEKEMCHHO-01, GU-CONCEPT-C03-VANDHONAHI-01]
introduces:
  knowledge: [GU-CONCEPT-C03-PRACTICE-01]
introduces_idioms: []
introduces_senses: []
introduces_culture_claims: []
practises:
  knowledge: [GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C03-KEM-01, GU-CONCEPT-C03-MAJAA-01, GU-CONCEPT-C03-PRACTICE-01, GU-CONCEPT-C03-TAMEKEMCHHO-01, GU-CONCEPT-C03-VANDHONAHI-01]
skills: [listening, speaking, reading]
modes: [interpretive, interpersonal, presentational]
strands: [meaning-input, meaning-output, language-focus]
register: neutral
variety: standard-gujarati
reviews_of: [GU-C03-kem, GU-C03-tame-kem-chho, GU-C03-hun, GU-C03-majaa, GU-C03-vandho-nahi]
---

# Chapter 5 — The how-are-you exchange

## Warm-up
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C03-KEM-01, GU-CONCEPT-C03-MAJAA-01, GU-CONCEPT-C03-TAMEKEMCHHO-01, GU-CONCEPT-C03-VANDHONAHI-01] -->

[PAUSE 2s] No new words. The whole responding cycle, from atoms you now own.

## The exchange
<!-- hl-knowledge: introduces=[GU-CONCEPT-C03-PRACTICE-01]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C03-KEM-01, GU-CONCEPT-C03-MAJAA-01, GU-CONCEPT-C03-TAMEKEMCHHO-01, GU-CONCEPT-C03-VANDHONAHI-01] -->

[PAUSE 1s each]
- [YOU SAY: "How are you?" (the everyday hello) — *tame kem chho?*]
- [YOU SAY: "I'm well, thank you." — *hũ majāmā chhũ, ābhār.*]
- [YOU SAY: "No problem / you're welcome." — *vāndho nahĩ.*]
- [YOU SAY: the familiar question — *tũ kem chhe?*]

## The word, taken apart — the atoms
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C03-KEM-01, GU-CONCEPT-C03-MAJAA-01, GU-CONCEPT-C03-PRACTICE-01, GU-CONCEPT-C03-TAMEKEMCHHO-01, GU-CONCEPT-C03-VANDHONAHI-01] -->

[PAUSE 2s]
- [YOU SAY: the k- question-word for "how" (*kem*)]
- [YOU SAY: "I" and its copula (*hũ*, *chhũ*)]
- [YOU SAY: which of your words is Persian, not Sanskrit (*majā*)]

## What you've built — roots you now carry
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C03-KEM-01, GU-CONCEPT-C03-MAJAA-01, GU-CONCEPT-C03-PRACTICE-01, GU-CONCEPT-C03-TAMEKEMCHHO-01, GU-CONCEPT-C03-VANDHONAHI-01] -->

[PAUSE 2s]
- *kem* ← the interrogative *k-* (PIE *\\*kʷo-*), cousin of English *how/what*
- *hũ* ← Sanskrit *aham* → Latin *ego*, English **I**
- *majā* ← **Persian** *maza*, "taste/relish" — the trade-language layer
- *nahĩ* ← PIE *\\*ne*, English *no* (in *vāndho nahĩ*)

## Wrap-up Recall
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C03-KEM-01, GU-CONCEPT-C03-MAJAA-01, GU-CONCEPT-C03-PRACTICE-01, GU-CONCEPT-C03-TAMEKEMCHHO-01, GU-CONCEPT-C03-VANDHONAHI-01] -->

[PAUSE 3s] Someone greets you with *tame kem chho?* — reply that you're well and
thank them. (*Hũ majāmā chhũ, ābhār* — and to their thanks, *vāndho nahĩ*.)
`,l=e({default:()=>u}),u=`---
schema_version: 2
id: GU-C03-tame-kem-chho
spine_node: SPINE-CHECK-WELLBEING
sequence: 200
chapter: 9
type: phrase
headword: તમે કેમ છો?
romanization: "tame kem chho?"
gloss: how are you? (respectful)
concept_tag: STATE-HOW-ARE-YOU
prerequisites: [GU-C03-kem, GU-C02-tu-tame, GU-C02-chhe, GU-W02-dha]
sounds: [o-ending]
roots: []
duration:
  max_seconds: 240
requires:
  knowledge: [GU-CONCEPT-C02-CHHE-01, GU-CONCEPT-C02-TUTAME-01, GU-CONCEPT-C03-KEM-01]
introduces:
  knowledge: [GU-CONCEPT-C03-TAMEKEMCHHO-01]
introduces_idioms: [GU-IDIOM-TAME-KEM-CHHO-WELLBEING-01]
introduces_senses: []
introduces_culture_claims: []
practises:
  knowledge: [GU-CONCEPT-C02-CHHE-01, GU-CONCEPT-C02-TUTAME-01, GU-CONCEPT-C03-KEM-01, GU-CONCEPT-C03-TAMEKEMCHHO-01, GU-SCRIPT-DHA-01]
skills: [listening, speaking, reading, writing]
modes: [interpretive, interpersonal, presentational]
strands: [meaning-input, meaning-output, language-focus]
register: neutral
variety: standard-gujarati
reviews_of: [GU-C02-tu-tame, GU-C02-chhe]
---

# તમે કેમ છો? (tame kem chho?) — "how are you?"

## Warm-up
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C02-CHHE-01, GU-CONCEPT-C02-TUTAME-01, GU-CONCEPT-C03-KEM-01] -->

[PAUSE 2s] The greeting most Gujaratis actually use — three words you already
own.

## The phrase, taken apart
<!-- hl-knowledge: introduces=[GU-CONCEPT-C03-TAMEKEMCHHO-01]; assesses=[GU-CONCEPT-C02-CHHE-01, GU-CONCEPT-C02-TUTAME-01, GU-CONCEPT-C03-KEM-01] -->

**તમે કેમ છો?** = **તમે** (*tame*, respectful "you") + **કેમ** (*kem*, "how") +
**છો** (*chho*, "are") — literally "**you how are?**", the verb last. *Chho* is
the respectful/plural form of the copula *chhe* you met in the name exchange (*hũ chhũ* I
am / *tame chho* you are). To a friend (*tũ*): **તું કેમ છે?** (*tũ kem chhe?*).

## Grammar Lens: match the "you" to the copula
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C02-CHHE-01, GU-CONCEPT-C02-TUTAME-01, GU-CONCEPT-C03-KEM-01, GU-CONCEPT-C03-TAMEKEMCHHO-01] -->

- Respectful: **તમે કેમ છો?** (*tame … chho*)
- Familiar: **તું કેમ છે?** (*tũ … chhe*)

Same pattern as every Indo-Aryan track: the "you" and its verb-form travel
together.

## Guided Practice
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C02-CHHE-01, GU-CONCEPT-C02-TUTAME-01, GU-CONCEPT-C03-KEM-01, GU-CONCEPT-C03-TAMEKEMCHHO-01] -->

[PAUSE 1s]
- [YOU SAY: the respectful greeting — *tame kem chho?*]
- [YOU SAY: the familiar version — *tũ kem chhe?*]
- [YOU SAY: where the verb sits (at the end)]

## Script retrieval
<!-- hl-knowledge: introduces=[]; assesses=[GU-SCRIPT-DHA-01] -->
<!-- hl-activity: {"id":"GU-C03-tame-kem-chho-dha-retrieval","kind":"text","assesses":["GU-SCRIPT-DHA-01"],"prompt":"Without looking back, write ધ once.","answer":"ધ","accepted":[],"feedback":{"correct":"Good: one careful retrieval is enough.","incorrect":"Look at ધ once, hide it, and write it one time."},"response_seconds":12} -->

Hide the earlier model for **ધ**. Write it once, compare, make at most one
repair, and stop.

## Wrap-up Recall
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C02-CHHE-01, GU-CONCEPT-C02-TUTAME-01, GU-CONCEPT-C03-KEM-01, GU-CONCEPT-C03-TAMEKEMCHHO-01] -->

[PAUSE 3s] Give the everyday Gujarati "how are you?", and its copula form.
(*Tame kem chho?*; *chho* — the respectful form of *chhe*.)
`,d=e({default:()=>f}),f=`---
schema_version: 2
id: GU-C03-vandho-nahi
spine_node: SPINE-COURTESY-THANK
sequence: 220
chapter: 9
type: phrase
headword: વાંધો નહીં
romanization: "vāndho nahĩ"
gloss: no problem / it's okay / you're welcome
concept_tag: COURTESY-YOUREWELCOME
prerequisites: [GU-C01-haa-naa, GU-C01-aabhaar]
sounds: [nasal-aa, no-top-line]
roots: [na-negative]
duration:
  max_seconds: 240
requires:
  knowledge: [GU-CONCEPT-C01-AABHAAR-01, GU-CONCEPT-C01-HAANAA-01]
introduces:
  knowledge: [GU-CONCEPT-C03-VANDHONAHI-01]
introduces_idioms: [GU-IDIOM-VANDHO-NAHI-NO-PROBLEM-01]
introduces_senses: []
introduces_culture_claims: []
practises:
  knowledge: [GU-CONCEPT-C01-AABHAAR-01, GU-CONCEPT-C01-HAANAA-01, GU-CONCEPT-C03-VANDHONAHI-01]
skills: [listening, speaking, reading]
modes: [interpretive, interpersonal, presentational]
strands: [meaning-input, meaning-output, language-focus]
register: neutral
variety: standard-gujarati
reviews_of: [GU-C01-haa-naa]
---

# વાંધો નહીં (vāndho nahĩ) — "no problem"

## Warm-up
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C01-AABHAAR-01, GU-CONCEPT-C01-HAANAA-01] -->

[PAUSE 2s] When someone thanks you — or apologises — this is the easy, gracious
reply.

## The letters in this word
<!-- hl-knowledge: introduces=[GU-CONCEPT-C03-VANDHONAHI-01]; assesses=[GU-CONCEPT-C01-AABHAAR-01, GU-CONCEPT-C01-HAANAA-01] -->

**વાંધો** (*vāndho*, "objection, problem" — with the nasal on **વાં**) + **નહીં**
(*nahĩ*, "not") → *vāndho nahĩ*.

## The word, taken apart
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C01-AABHAAR-01, GU-CONCEPT-C01-HAANAA-01, GU-CONCEPT-C03-VANDHONAHI-01] -->

**વાંધો નહીં** literally means "**[there is] no objection**" — *vāndho*
("objection, difficulty, harm") + **નહીં** (*nahĩ*, "not / no"). It serves as
"it's okay / no worries / no problem / you're welcome." The negative **નહીં**
(*nahĩ*) is the fuller form of the **ના** (*nā*) from the earlier yes/no lesson — both from
the ancient **na-**, Proto-Indo-European **\\*ne**, behind English **no**,
**not**, and Hindi *nahīṁ*.

## Guided Practice
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C01-AABHAAR-01, GU-CONCEPT-C01-HAANAA-01, GU-CONCEPT-C03-VANDHONAHI-01] -->

[PAUSE 1s]
- [YOU SAY: "vāndho nahĩ"]
- [YOU SAY: what it literally means ("no objection")]
- [YOU SAY: the exchange — *ābhār* / *vāndho nahĩ*]

## Wrap-up Recall
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C01-AABHAAR-01, GU-CONCEPT-C01-HAANAA-01, GU-CONCEPT-C03-VANDHONAHI-01] -->

[PAUSE 3s] What does *vāndho nahĩ* literally say, and what ancient root is its
*nahĩ* from? ("No objection"; PIE *\\*ne* — English *no/not*.)
`,p=e({default:()=>m}),m=`---
schema_version: 2
id: GU-C04-kaale
spine_node: SPINE-TAKE-LEAVE
sequence: 250
chapter: 10
type: phrase
headword: કાલે મળીશું
romanization: "kāle maḷīshũ"
gloss: see you tomorrow (and the કાલ puzzle)
concept_tag: FAREWELL-TOMORROW
prerequisites: [GU-C04-malishun]
sounds: [long-aa, e-sign]
roots: [sanskrit-kaala-time]
duration:
  max_seconds: 240
requires:
  knowledge: [GU-CONCEPT-C04-MALISHUN-01]
introduces:
  knowledge: [GU-CONCEPT-C04-KAALE-01]
introduces_idioms: [GU-IDIOM-KAALE-MALISHUN-FAREWELL-01]
introduces_senses: [GU-SENSE-KAAL-TOMORROW-YESTERDAY-01]
introduces_culture_claims: []
practises:
  knowledge: [GU-CONCEPT-C04-KAALE-01, GU-CONCEPT-C04-MALISHUN-01]
skills: [listening, speaking, reading]
modes: [interpretive, interpersonal, presentational]
strands: [meaning-input, meaning-output, language-focus]
register: neutral
variety: standard-gujarati
reviews_of: [GU-C04-malishun]
---

# કાલે મળીશું (kāle maḷīshũ) — "see you tomorrow"

## Warm-up
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C04-MALISHUN-01] -->

[PAUSE 2s] A goodbye for tomorrow — built on a word that means both **tomorrow**
and **yesterday**.

## The letters in this word
<!-- hl-knowledge: introduces=[GU-CONCEPT-C04-KAALE-01]; assesses=[GU-CONCEPT-C04-MALISHUN-01] -->

**કા** (*kā*) + **લે** (*le*) → **કાલે** (*kāle*, "on [the] *kāl*"). The base is
**કાલ** (*kāl*).

## The word, taken apart
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C04-KAALE-01, GU-CONCEPT-C04-MALISHUN-01] -->

**કાલ** (*kāl*) means **both "tomorrow" and "yesterday"** — the day on either side
of today — from Sanskrit **કાલ** (*kāla*, "time"). (It is the very same *kāla*
behind Hindi's *kal*, Bengali's *kāl*, and Punjabi's *akāl* "timeless.") The verb's
tense tells which: *kāle maḷīshũ* (future → "**tomorrow** we'll meet"), *kāle maḷyā*
(past → "we met **yesterday**"). So **કાલે મળીશું** = "tomorrow we'll meet" = "see
you tomorrow."

## Guided Practice
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C04-KAALE-01, GU-CONCEPT-C04-MALISHUN-01] -->

[PAUSE 1s]
- [YOU SAY: "kāle maḷīshũ"]
- [YOU SAY: the puzzle — *kāl* means both **tomorrow** and **yesterday**]
- [YOU SAY: the root and its cousins (*kāla* "time"; Hindi *kal*, Punjabi *akāl*)]

## Wrap-up Recall
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C04-KAALE-01, GU-CONCEPT-C04-MALISHUN-01] -->

[PAUSE 3s] What two opposite days does *kāl* mean, and what tells them apart?
(Both tomorrow and yesterday; the verb's tense.)
`,h=e({default:()=>g}),g=`---
schema_version: 2
id: GU-C04-malishun
spine_node: SPINE-TAKE-LEAVE
sequence: 240
chapter: 10
type: word
headword: મળીશું
romanization: "maḷīshũ"
gloss: (we) will meet
concept_tag: GU-VERB-MALVU
prerequisites: [GU-C03-hun]
sounds: [long-ii, u-nasal]
roots: [mil-meet]
duration:
  max_seconds: 240
requires:
  knowledge: [GU-CONCEPT-C03-HUN-01]
introduces:
  knowledge: [GU-CONCEPT-C04-MALISHUN-01]
introduces_idioms: []
introduces_senses: []
introduces_culture_claims: []
practises:
  knowledge: [GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C04-MALISHUN-01]
skills: [listening, speaking, reading]
modes: [interpretive, interpersonal, presentational]
strands: [meaning-input, meaning-output, language-focus]
register: neutral
variety: standard-gujarati
reviews_of: []
---

# મળીશું (maḷīshũ) — "(we) will meet"

## Warm-up
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01] -->

[PAUSE 2s] A future-tense verb — the other half of "see you again."

## The letters in this word
<!-- hl-knowledge: introduces=[GU-CONCEPT-C04-MALISHUN-01]; assesses=[GU-CONCEPT-C03-HUN-01] -->

**મ** (*ma*) + **ળી** (*ḷī*, with the retroflex **ળ** *ḷ*) + **શું** (*shũ*) →
**મળીશું** (*maḷīshũ*).

## The word, taken apart
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C04-MALISHUN-01] -->

**મળીશું** (*maḷīshũ*, "we will meet") is the future of **મળવું** (*maḷvũ*, "to
meet"), from Sanskrit **મિલ્** (*mil-*, "to join, unite") — the same root as Hindi
*milnā* and Punjabi *milṇā*. Note the retroflex **ળ** (*ḷ*): Gujarati keeps this
extra letter — like Marathi and the Dravidian south, and unlike Hindi. The ending
**-શું** (*-shũ*) is the "we will" future.

## Grammar Lens: the future is an ending
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C04-MALISHUN-01] -->

Gujarati marks the future with an ending on the stem: *maḷ-* ("meet") + *-īshũ*
("we will") → *maḷīshũ*. No helper-word like English "will."

## Guided Practice
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C04-MALISHUN-01] -->

[PAUSE 1s]
- [YOU SAY: "maḷīshũ"]
- [YOU SAY: the verb it's from (*maḷvũ*, "to meet," ← Sanskrit *mil-*)]
- [YOU SAY: the extra letter it shows (retroflex ળ, *ḷ*)]

## Wrap-up Recall
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C04-MALISHUN-01] -->

[PAUSE 3s] What verb is *maḷīshũ* from, and what special letter does it use?
(*Maḷvũ* "to meet"; the retroflex ળ *ḷ*, which Hindi lacks.)
`,_=e({default:()=>v}),v=`---
schema_version: 2
id: GU-C04-pachha-malishun
spine_node: SPINE-TAKE-LEAVE
sequence: 270
chapter: 10
type: phrase
headword: પાછા મળીશું
romanization: "pāchhā maḷīshũ"
gloss: we'll meet again (goodbye)
concept_tag: FAREWELL-LATER
prerequisites: [GU-C04-pachha, GU-C04-malishun, GU-C01-aavjo]
sounds: []
roots: []
duration:
  max_seconds: 180
requires:
  knowledge: [GU-CONCEPT-C01-AAVJO-01, GU-CONCEPT-C04-MALISHUN-01, GU-CONCEPT-C04-PACHHA-01]
introduces:
  knowledge: [GU-CONCEPT-C04-PACHHAMALISHUN-01]
introduces_idioms: [GU-IDIOM-PACHHA-MALISHUN-FAREWELL-01]
introduces_senses: []
introduces_culture_claims: []
practises:
  knowledge: [GU-CONCEPT-C01-AAVJO-01, GU-CONCEPT-C04-MALISHUN-01, GU-CONCEPT-C04-PACHHA-01, GU-CONCEPT-C04-PACHHAMALISHUN-01]
skills: [listening, speaking, reading]
modes: [interpretive, interpersonal, presentational]
strands: [meaning-input, meaning-output, language-focus]
register: neutral
variety: standard-gujarati
reviews_of: [GU-C01-aavjo]
---

# પાછા મળીશું (pāchhā maḷīshũ) — "we'll meet again"

## Warm-up
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C01-AAVJO-01, GU-CONCEPT-C04-MALISHUN-01, GU-CONCEPT-C04-PACHHA-01] -->

[PAUSE 2s] The warm, explicit goodbye — the companion of the earlier *āvjo*.

## The phrase, taken apart
<!-- hl-knowledge: introduces=[GU-CONCEPT-C04-PACHHAMALISHUN-01]; assesses=[GU-CONCEPT-C01-AAVJO-01, GU-CONCEPT-C04-MALISHUN-01, GU-CONCEPT-C04-PACHHA-01] -->

**પાછા મળીશું** = **પાછા** (*pāchhā*, "again") + **મળીશું** (*maḷīshũ*, "we will
meet") — "**we'll meet again**." Where the earlier **આવજો** (*āvjo*) is the quick,
warm "come again," *pāchhā maḷīshũ* spells out the promise: we will meet once
more. It is the same "promise of return" the whole subcontinent prefers (Hindi
*phir milenge*, Bengali *āshi*, Tamil *pōy varugiṟēṉ*).

## Guided Practice
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C01-AAVJO-01, GU-CONCEPT-C04-MALISHUN-01, GU-CONCEPT-C04-PACHHA-01, GU-CONCEPT-C04-PACHHAMALISHUN-01] -->

[PAUSE 1s]
- [YOU SAY: the warm goodbye — *pāchhā maḷīshũ*]
- [YOU SAY: the two words it's built from (*pāchhā* + *maḷīshũ*)]
- [YOU SAY: the Chapter-1 goodbye it partners (*āvjo*)]

## Wrap-up Recall
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C01-AAVJO-01, GU-CONCEPT-C04-MALISHUN-01, GU-CONCEPT-C04-PACHHA-01, GU-CONCEPT-C04-PACHHAMALISHUN-01] -->

[PAUSE 3s] What does *pāchhā maḷīshũ* literally say, and how does it relate to
*āvjo*? ("We'll meet again"; it's the explicit form of Ch.1's "come again"
goodbye.)
`,y=e({default:()=>b}),b=`---
schema_version: 2
id: GU-C04-pachha
spine_node: SPINE-TAKE-LEAVE
sequence: 260
chapter: 10
type: word
headword: પાછા
romanization: "pāchhā"
gloss: again, back
concept_tag: GU-WORD-PACHHA
prerequisites: []
sounds: [long-aa, chh]
roots: [pashcha-behind]
duration:
  max_seconds: 180
requires:
  knowledge: []
introduces:
  knowledge: [GU-CONCEPT-C04-PACHHA-01]
introduces_idioms: []
introduces_senses: []
introduces_culture_claims: []
practises:
  knowledge: [GU-CONCEPT-C04-PACHHA-01]
skills: [listening, speaking, reading]
modes: [interpretive, interpersonal, presentational]
strands: [meaning-input, meaning-output, language-focus]
register: neutral
variety: standard-gujarati
reviews_of: []
---

# પાછા (pāchhā) — "again, back"

## Warm-up
<!-- hl-knowledge: introduces=[]; assesses=[] -->

[PAUSE 2s] One small word turns a goodbye into "**till next time**." You already
have the other half — *āvjo* ("come again") — from the earlier courtesy chapter.

## The letters in this word
<!-- hl-knowledge: introduces=[GU-CONCEPT-C04-PACHHA-01]; assesses=[] -->

**પા** (*pā*) + **છા** (*chhā*) → **પાછા** (*pāchhā*).

## The word, taken apart
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C04-PACHHA-01] -->

**પાછા** (*pāchhā*, "back, again, returning") is from Sanskrit **પશ્ચાત્**
(*paścāt*, "behind, afterward") — the same "behind/after" root behind English
*post-* words, distantly. It carries the sense of *return*: *pāchhā āvvũ* ("to
come back"), *pāchhā jāvũ* ("to go back"). Add it to "we'll meet" and you get the
warm goodbye of the next lesson.

## Guided Practice
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C04-PACHHA-01] -->

[PAUSE 1s]
- [YOU SAY: "pāchhā"]
- [YOU SAY: what it adds to a goodbye ("again / back")]

## Wrap-up Recall
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C04-PACHHA-01] -->

[PAUSE 3s] What does *pāchhā* mean, and what sense does it carry? ("Back / again";
the sense of return — *pāchhā āvvũ*, "to come back.")
`,x=e({default:()=>S}),S=`---
schema_version: 2
id: GU-C04-practice
spine_node: SPINE-TAKE-LEAVE
sequence: 280
chapter: 10
type: practice
headword: (dialogue)
gloss: Chapter 6 recap — the farewells
concept_tag: REVIEW
prerequisites: [GU-C04-pachha-malishun, GU-C04-kaale]
sounds: []
roots: []
duration:
  max_seconds: 240
requires:
  knowledge: [GU-CONCEPT-C01-AAVJO-01, GU-CONCEPT-C04-KAALE-01, GU-CONCEPT-C04-MALISHUN-01, GU-CONCEPT-C04-PACHHA-01, GU-CONCEPT-C04-PACHHAMALISHUN-01]
introduces:
  knowledge: [GU-CONCEPT-C04-PRACTICE-01]
introduces_idioms: []
introduces_senses: []
introduces_culture_claims: []
practises:
  knowledge: [GU-CONCEPT-C01-AAVJO-01, GU-CONCEPT-C04-KAALE-01, GU-CONCEPT-C04-MALISHUN-01, GU-CONCEPT-C04-PACHHA-01, GU-CONCEPT-C04-PACHHAMALISHUN-01, GU-CONCEPT-C04-PRACTICE-01]
skills: [listening, speaking, reading]
modes: [interpretive, interpersonal, presentational]
strands: [meaning-input, meaning-output, language-focus]
register: neutral
variety: standard-gujarati
reviews_of: [GU-C01-aavjo, GU-C04-pachha, GU-C04-malishun, GU-C04-pachha-malishun, GU-C04-kaale]
---

# Chapter 6 — The farewells

## Warm-up
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C01-AAVJO-01, GU-CONCEPT-C04-KAALE-01, GU-CONCEPT-C04-MALISHUN-01, GU-CONCEPT-C04-PACHHA-01, GU-CONCEPT-C04-PACHHAMALISHUN-01] -->

[PAUSE 2s] No new words. Three ways to part — none of them a blunt "goodbye."

## Why it's said this way — the farewells
<!-- hl-knowledge: introduces=[GU-CONCEPT-C04-PRACTICE-01]; assesses=[GU-CONCEPT-C01-AAVJO-01, GU-CONCEPT-C04-KAALE-01, GU-CONCEPT-C04-MALISHUN-01, GU-CONCEPT-C04-PACHHA-01, GU-CONCEPT-C04-PACHHAMALISHUN-01] -->

[PAUSE 1s each]
- [YOU SAY: the quick "come again" from the earlier courtesy chapter — *āvjo*]
- [YOU SAY: the explicit "we'll meet again" — *pāchhā maḷīshũ*]
- [YOU SAY: "see you tomorrow" — *kāle maḷīshũ*]

## The word, taken apart — the atoms
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C01-AAVJO-01, GU-CONCEPT-C04-KAALE-01, GU-CONCEPT-C04-MALISHUN-01, GU-CONCEPT-C04-PACHHA-01, GU-CONCEPT-C04-PACHHAMALISHUN-01, GU-CONCEPT-C04-PRACTICE-01] -->

[PAUSE 2s]
- [YOU SAY: "again" and "(we) will meet" (*pāchhā*, *maḷīshũ*)]
- [YOU SAY: the word that means BOTH tomorrow and yesterday (*kāl*)]
- [YOU SAY: the retroflex letter in *maḷīshũ* (ળ, *ḷ*)]

## What you've built — roots you now carry
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C01-AAVJO-01, GU-CONCEPT-C04-KAALE-01, GU-CONCEPT-C04-MALISHUN-01, GU-CONCEPT-C04-PACHHA-01, GU-CONCEPT-C04-PACHHAMALISHUN-01, GU-CONCEPT-C04-PRACTICE-01] -->

[PAUSE 2s]
- *āvjo* ("goodbye") = "come [again]," from *āvvũ* "to come"
- *maḷīshũ* ← *maḷvũ* "to meet" (Sanskrit *mil-*); the retroflex ળ
- *kāl* ← *kāla* "time" (Hindi *kal*, Punjabi *akāl*)

## Wrap-up Recall
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C01-AAVJO-01, GU-CONCEPT-C04-KAALE-01, GU-CONCEPT-C04-MALISHUN-01, GU-CONCEPT-C04-PACHHA-01, GU-CONCEPT-C04-PACHHAMALISHUN-01, GU-CONCEPT-C04-PRACTICE-01] -->

[PAUSE 3s] You're leaving a friend and will see them tomorrow. Give the goodbye
and the "tomorrow" version. (*Āvjo* — and *kāle maḷīshũ*.)
`,C=e({default:()=>w}),w=`---
schema_version: 2
id: GU-C05-bolvun
spine_node: SPINE-SAY-WHAT-I-DO
sequence: 290
chapter: 11
type: word
headword: બોલવું
romanization: "bolvũ"
gloss: to speak
concept_tag: GU-VERB-BOLVU
prerequisites: [GU-C03-hun]
sounds: [o-sign, u-nasal]
roots: [bol-speak]
duration:
  max_seconds: 240
requires:
  knowledge: [GU-CONCEPT-C03-HUN-01]
introduces:
  knowledge: [GU-CONCEPT-C05-BOLVUN-01]
introduces_idioms: []
introduces_senses: []
introduces_culture_claims: []
practises:
  knowledge: [GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C05-BOLVUN-01]
skills: [listening, speaking, reading]
modes: [interpretive, interpersonal, presentational]
strands: [meaning-input, meaning-output, language-focus]
register: neutral
variety: standard-gujarati
reviews_of: []
---

# બોલવું (bolvũ) — "to speak," your first full verb

## Warm-up
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01] -->

[PAUSE 2s] So far, "to be." Here is a verb that *does* something — in a shape
you'll see on every Gujarati verb.

## The letters in this word
<!-- hl-knowledge: introduces=[GU-CONCEPT-C05-BOLVUN-01]; assesses=[GU-CONCEPT-C03-HUN-01] -->

**બો** (*bo*) + **લ** (*la*) + **વું** (*vũ*, with the nasal) → **બોલવું**
(*bolvũ*).

## The word, taken apart
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C05-BOLVUN-01] -->

**બોલવું** (*bolvũ*, "to speak") is a native Indo-Aryan verb (the same *bol-* as
Hindi *bolnā* and Punjabi *bolṇā*). Its ending **-વું** (*-vũ*) is Gujarati's
infinitive marker — the "to ___" form: *bol-vũ* "to speak," *maḷ-vũ* "to meet"
(from *maḷīshũ*), *āv-vũ* "to come" (behind *āvjo*). Strip the *-vũ* and you have
the **stem** (*bol-*).

## Grammar Lens: stem + person + copula
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C05-BOLVUN-01] -->

To say "I speak," Gujarati adds a person-ending to the stem and follows it with
the copula:

- *hũ bolũ chhũ* — I speak
- *tũ bole chhe* — you speak (familiar)
- *tame bolo chho* — you speak (respectful)
- *te bole chhe* — he/she speaks

The simple present is the same for a man or a woman — Gujarati saves its **three
genders** for adjectives and the past tense, not the plain present.

## Guided Practice
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C05-BOLVUN-01] -->

[PAUSE 1s]
- [YOU SAY: "bolvũ"]
- [YOU SAY: the infinitive ending every Gujarati verb takes (*-vũ*)]
- [YOU SAY: "I speak" — *hũ bolũ chhũ*]

## Wrap-up Recall
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C05-BOLVUN-01] -->

[PAUSE 3s] What ending marks a Gujarati infinitive, and how do you say "I speak"?
(*-vũ*; *hũ bolũ chhũ*.)
`,T=e({default:()=>E}),E=`---
schema_version: 2
id: GU-C05-hun-gujarati-bolun-chhun
spine_node: SPINE-SAY-WHAT-I-DO
sequence: 300
chapter: 11
type: phrase
headword: હું ગુજરાતી બોલું છું
romanization: "hũ gujarātī bolũ chhũ"
gloss: I speak Gujarati
concept_tag: GU-WORD-GUJARATI
prerequisites: [GU-C05-bolvun, GU-C03-hun]
sounds: [long-aa, u-nasal]
roots: [gurjar]
duration:
  max_seconds: 240
requires:
  knowledge: [GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C05-BOLVUN-01]
introduces:
  knowledge: [GU-CONCEPT-C05-HUNGUJARATIBOLUNCHHUN-01]
introduces_idioms: []
introduces_senses: []
introduces_culture_claims: [GU-CULTURE-GUJARATI-GURJAR-PEOPLE-NAME-01]
practises:
  knowledge: [GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C05-BOLVUN-01, GU-CONCEPT-C05-HUNGUJARATIBOLUNCHHUN-01]
skills: [listening, speaking, reading]
modes: [interpretive, interpersonal, presentational]
strands: [meaning-input, meaning-output, language-focus]
register: neutral
variety: standard-gujarati
reviews_of: [GU-C05-bolvun, GU-C03-hun]
---

# હું ગુજરાતી બોલું છું (hũ gujarātī bolũ chhũ) — "I speak Gujarati"

## Warm-up
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C05-BOLVUN-01] -->

[PAUSE 2s] Your first complete, moving sentence — and the language names itself
after a people who arrived on horseback.

## The letters in this word
<!-- hl-knowledge: introduces=[GU-CONCEPT-C05-HUNGUJARATIBOLUNCHHUN-01]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C05-BOLVUN-01] -->

**ગુજરાતી** (*gujarātī*): **ગુ** (*gu*) + **જ** (*ja*) + **રા** (*rā*) + **તી**
(*tī*).

## The sentence, taken apart
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C05-BOLVUN-01, GU-CONCEPT-C05-HUNGUJARATIBOLUNCHHUN-01] -->

**હું ગુજરાતી બોલું છું** = **હું** (*hũ*, "I") + **ગુજરાતી** (*gujarātī*, the
object) + **બોલું છું** (*bolũ chhũ*, "speak") — "I Gujarati speak," verb last.
The name **ગુજરાત** (*Gujarāt*) comes from the **Gurjar** — a people who entered
western India around the 6th–8th centuries and gave their name to the land
(*Gurjara-deśa* → *Gujarāt*); *gujarātī* is "[the language] of Gujarat." So the
language is named after a people, and the region after them in turn.

## Grammar Lens: no gender in the plain present
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C05-BOLVUN-01, GU-CONCEPT-C05-HUNGUJARATIBOLUNCHHUN-01] -->

*Bolũ chhũ* is the same whether a man or a woman says it — Gujarati's **three
genders** live in its adjectives (*sāro/sārī/sārũ*) and its past tense, but the
simple present "I speak" is gender-free. And because *chhũ* already carries "I,"
you could drop *hũ*: *gujarātī bolũ chhũ*.

## Guided Practice
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C05-BOLVUN-01, GU-CONCEPT-C05-HUNGUJARATIBOLUNCHHUN-01] -->

[PAUSE 1s]
- [YOU SAY: "hũ gujarātī bolũ chhũ"]
- [YOU SAY: what *Gujarāt* is named after (the *Gurjar* people)]
- [YOU SAY: does "I speak" change for a man vs. a woman? (No — not in the plain present)]

## Wrap-up Recall
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C05-BOLVUN-01, GU-CONCEPT-C05-HUNGUJARATIBOLUNCHHUN-01] -->

[PAUSE 3s] Say "I speak Gujarati," and give the origin of the name. (*Hũ gujarātī
bolũ chhũ*; from the *Gurjar* people, who named the land Gujarat.)
`,D=e({default:()=>O}),O=`---
schema_version: 2
id: GU-C05-kaam-karvun
spine_node: SPINE-SAY-WHAT-I-DO
sequence: 310
chapter: 11
type: word
headword: કામ કરવું
romanization: "kām karvũ"
gloss: to work (lit. "work-do")
concept_tag: GU-VERB-KARVU
prerequisites: [GU-C05-bolvun]
sounds: [long-aa, u-nasal]
roots: [kr-do, karya-work]
duration:
  max_seconds: 240
requires:
  knowledge: [GU-CONCEPT-C05-BOLVUN-01]
introduces:
  knowledge: [GU-CONCEPT-C05-KAAMKARVUN-01]
introduces_idioms: []
introduces_senses: []
introduces_culture_claims: []
practises:
  knowledge: [GU-CONCEPT-C05-BOLVUN-01, GU-CONCEPT-C05-KAAMKARVUN-01]
skills: [listening, speaking, reading]
modes: [interpretive, interpersonal, presentational]
strands: [meaning-input, meaning-output, language-focus]
register: neutral
variety: standard-gujarati
reviews_of: [GU-C05-bolvun]
---

# કામ કરવું (kām karvũ) — "to work"

## Warm-up
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C05-BOLVUN-01] -->

[PAUSE 2s] The last verb of the chapter — from the single most productive root in
the whole family, one you have met before.

## The letters in this word
<!-- hl-knowledge: introduces=[GU-CONCEPT-C05-KAAMKARVUN-01]; assesses=[GU-CONCEPT-C05-BOLVUN-01] -->

**કામ** (*kām*, "work"): **કા** (*kā*) + **મ** (*ma*). **કરવું** (*karvũ*, "to
do"): **ક** (*ka*) + **ર** (*ra*) + **વું** (*vũ*).

## The word, taken apart
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C05-BOLVUN-01, GU-CONCEPT-C05-KAAMKARVUN-01] -->

**કરવું** (*karvũ*, "to do, to make") is from Sanskrit **√કૃ** (*√kṛ*, "to do"),
from PIE **\\*kʷer-** — one of Sanskrit's most productive roots, met in *namaskār*
("the **making** of a bow") and *karma* ("a thing **done**"). And **કામ** (*kām*,
"work") is itself from Sanskrit *karma* — so "work-do" is, at root, "*kṛ*-thing
*kṛ*-do." Like Hindi, Gujarati builds compound verbs as **noun + કરવું**:

- **કામ કરવું** (*kām karvũ*) = "work-do" = "**to work**"
- **વાત કરવી** (*vāt karvī*) = "to converse" (*vāt*, "talk")

So *hũ kām karũ chhũ* = "I work." Learn *karvũ* and a whole grammar of "do-verbs"
opens up — the same one behind Hindi's *kām karnā* and Bengali's *kāj karā*.

## Guided Practice
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C05-BOLVUN-01, GU-CONCEPT-C05-KAAMKARVUN-01] -->

[PAUSE 1s]
- [YOU SAY: "kām karvũ" (to work)]
- [YOU SAY: "I work" — *hũ kām karũ chhũ*]
- [YOU SAY: the Chapter-1 word that shares its root (*namaskār*-style — actually *ābhār*? no; recall *karma*, from *kṛ*)]

## Wrap-up Recall
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C05-BOLVUN-01, GU-CONCEPT-C05-KAAMKARVUN-01] -->

[PAUSE 3s] What Sanskrit root is *karvũ* from, and what is *kām* ("work") from?
(*√kṛ*, "to do"; *kām* ← Sanskrit *karma* — both from *kṛ*.)
`,k=e({default:()=>A}),A=`---
schema_version: 2
id: GU-C05-practice
spine_node: SPINE-SAY-WHAT-I-DO
sequence: 330
chapter: 11
type: practice
headword: (dialogue)
gloss: Chapter 7 recap — the first verbs
concept_tag: REVIEW
prerequisites: [GU-C05-bolvun, GU-C05-hun-gujarati-bolun-chhun, GU-C05-rahevun, GU-C05-kaam-karvun]
sounds: []
roots: []
duration:
  max_seconds: 240
requires:
  knowledge: [GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C05-BOLVUN-01, GU-CONCEPT-C05-HUNGUJARATIBOLUNCHHUN-01, GU-CONCEPT-C05-KAAMKARVUN-01, GU-CONCEPT-C05-RAHEVUN-01]
introduces:
  knowledge: [GU-CONCEPT-C05-PRACTICE-01]
introduces_idioms: []
introduces_senses: []
introduces_culture_claims: []
practises:
  knowledge: [GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C05-BOLVUN-01, GU-CONCEPT-C05-HUNGUJARATIBOLUNCHHUN-01, GU-CONCEPT-C05-KAAMKARVUN-01, GU-CONCEPT-C05-PRACTICE-01, GU-CONCEPT-C05-RAHEVUN-01]
skills: [listening, speaking, reading]
modes: [interpretive, interpersonal, presentational]
strands: [meaning-input, meaning-output, language-focus]
register: neutral
variety: standard-gujarati
reviews_of: [GU-C05-bolvun, GU-C05-hun-gujarati-bolun-chhun, GU-C05-rahevun, GU-C05-kaam-karvun, GU-C03-hun]
---

# Chapter 7 — The first verbs

## Warm-up
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C05-BOLVUN-01, GU-CONCEPT-C05-HUNGUJARATIBOLUNCHHUN-01, GU-CONCEPT-C05-KAAMKARVUN-01, GU-CONCEPT-C05-RAHEVUN-01] -->

[PAUSE 2s] No new words. Three verbs, one engine — build real sentences about
yourself.

## How to answer — build the sentences
<!-- hl-knowledge: introduces=[GU-CONCEPT-C05-PRACTICE-01]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C05-BOLVUN-01, GU-CONCEPT-C05-HUNGUJARATIBOLUNCHHUN-01, GU-CONCEPT-C05-KAAMKARVUN-01, GU-CONCEPT-C05-RAHEVUN-01] -->

[PAUSE 1s each]
- [YOU SAY: "I speak Gujarati" — *hũ gujarātī bolũ chhũ*]
- [YOU SAY: "I live in Ahmedabad" — *hũ Amdāvādmā rahũ chhũ*]
- [YOU SAY: "I work" — *hũ kām karũ chhũ*]
- [YOU SAY: "you (resp.) speak Gujarati" — *tame gujarātī bolo chho*]

## Grammar lens — the engine
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C05-BOLVUN-01, GU-CONCEPT-C05-HUNGUJARATIBOLUNCHHUN-01, GU-CONCEPT-C05-KAAMKARVUN-01, GU-CONCEPT-C05-PRACTICE-01, GU-CONCEPT-C05-RAHEVUN-01] -->

[PAUSE 2s]
- [YOU SAY: the ending every infinitive carries (*-vũ*)]
- [YOU SAY: the shape of the present (stem + person + the copula *chhũ/chhe/chho*)]
- [YOU SAY: where the verb sits, and where "in" sits (verb last; *-mā* after the noun)]

## What you've built — roots you now carry
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C05-BOLVUN-01, GU-CONCEPT-C05-HUNGUJARATIBOLUNCHHUN-01, GU-CONCEPT-C05-KAAMKARVUN-01, GU-CONCEPT-C05-PRACTICE-01, GU-CONCEPT-C05-RAHEVUN-01] -->

[PAUSE 2s]
- *bolvũ* "to speak" (native; *bol-*, like Hindi/Punjabi)
- *rahevũ* "to live" ← *rah-* "to remain"
- *karvũ* "to do" ← *√kṛ*; *kām* "work" ← *karma* (both from *kṛ*)
- *gujarātī* ← the *Gurjar* people, who named the land Gujarat

## Wrap-up Recall
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C03-HUN-01, GU-CONCEPT-C05-BOLVUN-01, GU-CONCEPT-C05-HUNGUJARATIBOLUNCHHUN-01, GU-CONCEPT-C05-KAAMKARVUN-01, GU-CONCEPT-C05-PRACTICE-01, GU-CONCEPT-C05-RAHEVUN-01] -->

[PAUSE 3s] In one breath, tell me your language, your city, and your work in
Gujarati. (*Hũ gujarātī bolũ chhũ. Hũ … rahũ chhũ. Hũ kām karũ chhũ.*)
`,j=e({default:()=>M}),M=`---
schema_version: 2
id: GU-C05-rahevun
spine_node: SPINE-SAY-WHAT-I-DO
sequence: 320
chapter: 11
type: word
headword: રહેવું
romanization: "rahevũ"
gloss: to live, to stay
concept_tag: GU-VERB-RAHEVU
prerequisites: [GU-C05-bolvun]
sounds: [e-sign, u-nasal]
roots: [rah-remain]
duration:
  max_seconds: 240
requires:
  knowledge: [GU-CONCEPT-C05-BOLVUN-01]
introduces:
  knowledge: [GU-CONCEPT-C05-RAHEVUN-01]
introduces_idioms: []
introduces_senses: [GU-SENSE-RAHEVUN-LIVE-STAY-REMAIN-01]
introduces_culture_claims: []
practises:
  knowledge: [GU-CONCEPT-C05-BOLVUN-01, GU-CONCEPT-C05-RAHEVUN-01]
skills: [listening, speaking, reading]
modes: [interpretive, interpersonal, presentational]
strands: [meaning-input, meaning-output, language-focus]
register: neutral
variety: standard-gujarati
reviews_of: [GU-C05-bolvun, GU-C05-hun-gujarati-bolun-chhun]
---

# રહેવું (rahevũ) — "to live, to stay"

## Warm-up
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C05-BOLVUN-01] -->

[PAUSE 2s] A second verb, so you can say where you live.

## The letters in this word
<!-- hl-knowledge: introduces=[GU-CONCEPT-C05-RAHEVUN-01]; assesses=[GU-CONCEPT-C05-BOLVUN-01] -->

**ર** (*ra*) + **હે** (*he*) + **વું** (*vũ*) → **રહેવું** (*rahevũ*). The familiar
*-vũ* infinitive.

## The word, taken apart
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C05-BOLVUN-01, GU-CONCEPT-C05-RAHEVUN-01] -->

**રહેવું** (*rahevũ*, "to live, to remain, to stay") is from Sanskrit **રહ્**
(*rah-*, "to remain") — the same root as Hindi *rahnā* and Punjabi *rahiṇā*. "I
live in Ahmedabad" is **હું અમદાવાદમાં રહું છું** (*hũ Amdāvādmā rahũ chhũ*) —
stem *rah-* + person + copula, exactly like *bolvũ*.

## Grammar Lens: "in" comes after the noun
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C05-BOLVUN-01, GU-CONCEPT-C05-RAHEVUN-01] -->

**અમદાવાદમાં** (*Amdāvādmā*) = *Ahmedabad* + **-માં** (*-mā*, "in"), glued to the
**end** of the noun. Gujarati, like Hindi, uses **post**positions where English
uses prepositions: "Ahmedabad-in I live," the verb last.

## Guided Practice
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C05-BOLVUN-01, GU-CONCEPT-C05-RAHEVUN-01] -->

[PAUSE 1s]
- [YOU SAY: "rahevũ"]
- [YOU SAY: "I live in Ahmedabad" — *hũ Amdāvādmā rahũ chhũ*]
- [YOU SAY: where "in" (*-mā*) sits — glued to the end of the noun]

## Wrap-up Recall
<!-- hl-knowledge: introduces=[]; assesses=[GU-CONCEPT-C05-BOLVUN-01, GU-CONCEPT-C05-RAHEVUN-01] -->

[PAUSE 3s] What root is *rahevũ* from, and where does "in" go? (Sanskrit *rah-*
"to remain"; *-mā* "in" attaches to the **end** of the noun.)
`;export{C as a,_ as c,d,l as f,t as g,r as h,T as i,h as l,a as m,k as n,x as o,s as p,D as r,y as s,j as t,p as u};