...
export default [
  {
    ...
    suiteOrder: 10,
    caseOrder: 40,
    name: "keeps Malayalam independent അ, ആ, ഇ, ഉ, ഊ, and എ sourced while the remaining vowels stay unverified",
    verify: ({ SCRIPTS }) => {
      ...
      expect(iv.filter((_, index) => ![0, 1, 2, 4, 5, 6].includes(index)).every((v) => v.strokeOrder.length === 0)).toBe(true);
    },
  },
  {
