...
use language_lexer::{create_typescript_lexer, DEFAULT_VERSION, SUPPORTED_VERSIONS};
...
pub fn tokenize(source: &str, version: &str) -> Result<String, JsValue> {
    let mut lexer = create_typescript_lexer(source, resolve_version(version)).map_err(to_js_error)?;
    let tokens = lexer
        .tokenize()
        .map_err(|e| to_js_error(format!("TypeScript tokenization failed: {e}")))?;
...
mod tests {
    ...
    fn wrapper_tokenizes_to_json() {
        let json = tokenize("let x: number = 1;", "").unwrap();
