...
pub struct ProjectFiles {
    ...
    /// loading the component QML as the root from the embedded
    /// QML module. Falls through gracefully if the engine fails
    /// to instantiate (returns -1).
    pub main_cpp: String,
...
fn build_main_cpp(name: &str, module_name: &str) -> String {
    ...
    out.push_str("#include <QQmlApplicationEngine>\n");
    ...
    out.push_str("{\n");
    out.push_str("  QGuiApplication app(argc, argv);\n");
    out.push_str("  QQmlApplicationEngine engine;\n\n");
    out.push_str("  // Load the component as the root QML object from the embedded\n");
    out.push_str("  // module. The qrc:// path is set up by qt_add_qml_module in\n");
    writeln!(
        out,
    ...
    .unwrap();
    out.push_str("  QObject::connect(&engine, &QQmlApplicationEngine::objectCreationFailed,\n");
    out.push_str("                   &app, [](){ qExit(-1); }, Qt::QueuedConnection);\n");
    out.push_str("  engine.load(url);\n");
    out.push_str("  if (engine.rootObjects().isEmpty()) {\n");
    out.push_str("    return -1;\n");
    out.push_str("  }\n\n");
    out.push_str("  QObject *root = engine.rootObjects().constFirst();\n");
...
mod tests {
    ...
    fn ui32_project_shell_exposes_optional_mosaic_host_bridge() {
        ...
        assert!(
            proj.cmake_lists
                .contains("foreach(_mosaic_native_library IN ITEMS engram_capi.dll libengram_capi.dylib libengram_capi.so)"),
