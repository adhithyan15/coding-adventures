// build.rs — platform-specific linking for the silicon Ruby native extension.
//
// Mirrors matrix-rust-ruby-native/build.rs exactly (same linker dance, same
// Ruby symbol set).  Keeping both files in sync is intentional: one correct
// working incantation for each platform is simpler than a shared crate.
//
// * macOS:   -undefined dynamic_lookup so Ruby symbols stay unresolved until
//            dlopen() hands them to the running interpreter.
// * Linux:   nothing — ELF auto-resolves at dlopen() time.
// * Windows: discover libruby via rbconfig and link against it.
...
fn link_ruby_on_windows() {
    let Some(ruby) = find_ruby() else { return };
