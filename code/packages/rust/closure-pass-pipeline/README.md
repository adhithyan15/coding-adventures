...
- **`IterationPolicy::OneShot | FixedPoint`** — FixedPoint is
  accepted but executes once in v1 with a diagnostic note
  (fixed-point looping lands with the first mutating pass).
...
- **FixedPoint iteration** — the loop arrives with the first
  mutating pass; v1 just notes the limitation.
