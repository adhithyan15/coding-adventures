...
counterclockwise handwritten loop. The next smallest starter entry is ن;
HL-C09AC queues its source-backed bowl-and-dot audit.
...
| HL-C09 | Queued — 28 of 228 verified | Expand `DUCTUS` to cover the ten scripts with prose stroke-order entries. | Add cited, font-checked ductus and verified pen-lift metadata for the remaining 200 entries measured by HL-C19; each passes the on-ink, join-tolerance, coverage, citation, and source-agreement invariants. |
...
| HL-C09AC | Queued — next | Locate and verify independent Urdu ن in *Zer o Zabar*'s source-backed chapter *Sīn, shīn, baṛī he, nūn, and nūn-e ġhunna*. | Audit the independent bowl-and-dot animations, determine the exact lift and placement order, reconcile that motion with the Noto Naskh fallback, then author the smallest next path with source, lifts, font, and filmstrip agreement. |
