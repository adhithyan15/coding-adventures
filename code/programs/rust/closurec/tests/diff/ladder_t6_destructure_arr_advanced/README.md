# ladder_t6_destructure_arr_advanced

Rung 6.destructure_arr of the CCR-066 differential complexity ladder, at `ADVANCED`.

## What this rung is for

Exercises an array destructuring binding.
...
java -Duser.language=en -Duser.country=US -Duser.timezone=UTC -Dfile.encoding=UTF-8 \
  ...
  --compilation_level ADVANCED_OPTIMIZATIONS --js tests/diff/ladder_t6_destructure_arr_advanced/input/a.js
```
...
closurec refuses valid JavaScript that upstream compiles: array destructuring fails at the typed-AST bridge; binding targets are identifier-only. This is a front-end capability gap, not an optimizer one. Note it fails only on the typed path — WHITESPACE_ONLY passes, because that path never parses.
