//! # Argon2id — hybrid memory-hard password hashing (RFC 9106, RFC-recommended)
//!
//! Argon2id is the hybrid member of the Argon2 family: the first half of the
//! first pass uses side-channel-resistant data-independent addressing
//! (Argon2i), and everything afterwards uses GPU/ASIC-resistant data-dependent
//! addressing (Argon2d). Pick this variant unless you have a specific reason
...
fn compress(x: &[u64], y: &[u64]) -> Vec<u64> {
    ...
    let mut q = r.clone();
    for i in 0..8 { permutation_p(&mut q[i * 16..(i + 1) * 16]); }
    ...
    for i in 0..BLOCK_WORDS { out[i] = r[i] ^ q[i]; }
    out
}

fn block_to_bytes(block: &[u64]) -> Vec<u8> {
    let mut out = Vec::with_capacity(BLOCK_SIZE);
    for w in block { out.extend_from_slice(&w.to_le_bytes()); }
    out
}
...
struct AddressStream {
    r: u64, lane: u64, sl: u64, m_prime: u64, t: u64, type_id: u64,
    counter: u64, buf: Vec<u64>, idx: usize,
}

impl AddressStream {
    fn new(r: usize, lane: usize, sl: usize, m_prime: usize, t: usize) -> Self {
        Self {
            r: r as u64, lane: lane as u64, sl: sl as u64,
            m_prime: m_prime as u64, t: t as u64, type_id: TYPE_ID as u64,
            counter: 0, buf: vec![0u64; BLOCK_WORDS], idx: ADDRESSES_PER_BLOCK,
        }
    }
...
/// Argon2id addressing mode per RFC 9106 §3.4: data-independent for the
...
fn fill_segment(
    ...
    p: usize, m_prime: usize, t: usize,
) {
    let starting_c = if r == 0 && sl == 0 { 2 } else { 0 };
    let di = data_independent(r, sl);

    let mut addr = if di { Some(AddressStream::new(r, lane, sl, m_prime, t)) } else { None };
    if di {
        if let Some(a) = addr.as_mut() {
            for _ in 0..starting_c { a.next(); }
        }
    }
    ...
    for i in starting_c..sl_len {
        ...
        let pseudo_rand = if di {
            addr.as_mut().unwrap().next()
        } else {
            prev_block[0]
        };
        ...
        let l_prime = if r == 0 && sl == 0 { lane } else { (j2 % p as u64) as usize };
        ...
        if r == 0 {
            ...
            let merged: Vec<u64> = existing.iter().zip(new_block.iter()).map(|(a, b)| a ^ b).collect();
...
fn validate(
    ...
    key: &[u8], ad: &[u8], version: u32,
) -> Result<(), Argon2Error> {
    if password.len() as u64 > 0xFFFF_FFFF { return Err(Argon2Error::PasswordTooLong(password.len())); }
    if salt.len() < 8 { return Err(Argon2Error::SaltTooShort(salt.len())); }
    if salt.len() as u64 > 0xFFFF_FFFF { return Err(Argon2Error::SaltTooLong(salt.len())); }
...
pub fn argon2id(
    ...
    h0_in.extend_from_slice(&le32(TYPE_ID));
...
pub fn argon2id_hex(
    ...
    let tag = argon2id(password, salt, time_cost, memory_cost, parallelism, tag_length, opts)?;
    let mut s = String::with_capacity(tag.len() * 2);
    for b in &tag { s.push_str(&format!("{:02x}", b)); }
    Ok(s)
}
...
mod tests {
    ...
    fn filled(n: usize, b: u8) -> Vec<u8> { vec![b; n] }
    ...
    #[test]
    fn rfc9106_vector_5_3() {
        let password = filled(32, 0x01);
        let salt = filled(16, 0x02);
        let key = filled(8, 0x03);
        let ad = filled(12, 0x04);
        let expected = hex_decode("0d640df58d78766c08c037a34a8b53c9d01ef0452d75b65eb52520e96b01e659");

        let tag = argon2id(&password, &salt, 3, 32, 4, 32,
            &Options { key: Some(&key), associated_data: Some(&ad), version: None }).unwrap();
        assert_eq!(tag, expected);
    }

    #[test]
    fn hex_matches_bytes() {
        let tag = argon2id(b"password", b"saltsalt", 1, 8, 1, 32, &Options::default()).unwrap();
        let h = argon2id_hex(b"password", b"saltsalt", 1, 8, 1, 32, &Options::default()).unwrap();
        assert_eq!(h, tag.iter().map(|b| format!("{:02x}", b)).collect::<String>());
    }

    #[test]
    fn short_salt_rejected() { assert!(argon2id(b"pw", b"short", 1, 8, 1, 32, &Options::default()).is_err()); }

    #[test]
    fn tag_length_too_small_rejected() { assert!(argon2id(b"pw", b"saltsalt", 1, 8, 1, 3, &Options::default()).is_err()); }

    #[test]
    fn memory_below_minimum_rejected() { assert!(argon2id(b"pw", b"saltsalt", 1, 1, 1, 32, &Options::default()).is_err()); }

    #[test]
    fn zero_time_cost_rejected() { assert!(argon2id(b"pw", b"saltsalt", 0, 8, 1, 32, &Options::default()).is_err()); }

    #[test]
    fn zero_parallelism_rejected() { assert!(argon2id(b"pw", b"saltsalt", 1, 8, 0, 32, &Options::default()).is_err()); }

    #[test]
    fn unsupported_version_rejected() {
        assert!(argon2id(b"pw", b"saltsalt", 1, 8, 1, 32,
            &Options { key: None, associated_data: None, version: Some(0x10) }).is_err());
    }

    #[test]
    fn deterministic() {
        let a = argon2id(b"password", b"somesalt", 1, 8, 1, 32, &Options::default()).unwrap();
        let b = argon2id(b"password", b"somesalt", 1, 8, 1, 32, &Options::default()).unwrap();
        assert_eq!(a, b);
        assert_eq!(a.len(), 32);
    }
    ...
    fn different_passwords_differ() {
        let a = argon2id(b"password1", b"somesalt", 1, 8, 1, 32, &Options::default()).unwrap();
        let b = argon2id(b"password2", b"somesalt", 1, 8, 1, 32, &Options::default()).unwrap();
        assert_ne!(a, b);
    }

    #[test]
    fn different_salts_differ() {
        let a = argon2id(b"password", b"saltsalt", 1, 8, 1, 32, &Options::default()).unwrap();
        let b = argon2id(b"password", b"saltsal2", 1, 8, 1, 32, &Options::default()).unwrap();
        assert_ne!(a, b);
    }
    ...
    fn key_binds() {
        let a = argon2id(b"password", b"saltsalt", 1, 8, 1, 32, &Options::default()).unwrap();
        let b = argon2id(b"password", b"saltsalt", 1, 8, 1, 32,
    ...
    fn tag_length_variants() {
        for t in [4, 16, 32, 64, 65, 128] {
            let tag = argon2id(b"password", b"saltsalt", 1, 8, 1, t, &Options::default()).unwrap();
    ...
    fn multi_lane_parameters() {
        let tag = argon2id(&filled(32, 0x01), &filled(16, 0x02), 3, 32, 4, 32, &Options::default()).unwrap();
        assert_eq!(tag.len(), 32);
    }

    #[test]
    fn multiple_passes() {
        let t1 = argon2id(b"password", b"saltsalt", 1, 8, 1, 32, &Options::default()).unwrap();
        let t2 = argon2id(b"password", b"saltsalt", 2, 8, 1, 32, &Options::default()).unwrap();
        let t3 = argon2id(b"password", b"saltsalt", 3, 8, 1, 32, &Options::default()).unwrap();
        assert_ne!(t1, t2);
        assert_ne!(t2, t3);
        assert_ne!(t1, t3);
    }
