...
describe("corpus snapshot", () => {
  ...
  it("pins the ramp, and the size of its blind spot", () => {
    ...
    // #12356 adds one atom-bearing guided copy and one intentionally atom-free
    // delayed copy. The latter introduces nothing by design: it revisits the
    // writing atom eight lessons later, so the blind-spot count moves +1 while
    // the R2 reinforcement ledger does not regress.
    expect(report.summary.unmeasurableLessons).toBe(493); // The delayed copy adds one intentional atom-free review to the combined Urdu stack.
    ...
    */
    ...
    expect(report.summary.measurablePercent).toBe(86); // Urdu migrates three atom-blind opening lessons and crosses the rounding boundary.
  });
...
describe("the script ramp against the real corpus", () => {
  it("pins the script ramp, which no gate had ever measured", () => {
    ...
    // #12356: 51 -> 50. ML-C02-santosham now shows only its usable one-word
    // form, notices its two new shapes inside the three-glyph budget, and puts a
    // separate two-minute guided copy after recognition. The three other glyphs
    // belonged only to an untaught fuller phrase and are deferred with it.
    expect(report.summary.lessonViolations).toBe(50); // Arabic's six shape steps plus immediate recall remove all three chapter-1 glyph cliffs; only the separately queued harakat step remains.
