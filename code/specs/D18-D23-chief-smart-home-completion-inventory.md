...
| #130 Message abstraction | Open | D19's generic Actor message exists in all six languages, while the production Rust D18 envelope supplies the signed XChaCha20-Poly1305 header, SHA-256 plaintext binding, and D18M binary record. Its message fields, header, ciphertext, tag, and signature are structurally immutable behind read-only accessors (#11642). D18F fixes that production format as the portable binary baseline and specifies lossless JSON, rich payloads, deterministic sources, validation order, and stable errors (#11649). Rust implements that profile and owns the checked-in shared positive, negative, and oversize fixture manifest (#11652). TypeScript consumes the corpus with runtime-private frozen values and defensive byte copies (#11655), Python does the same with frozen, slotted values and immutable byte ownership (#11654), Go uses unexported values with copying accessors (#11653), and Ruby uses frozen values with copied byte storage (#11658); all four portable ports provide exact binary/JSON codecs and injected UUID/clock sources. Elixir and the final cross-language fixture gate remain. |
