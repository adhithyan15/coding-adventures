...
export function renderWithCanvas(
  ...
  // replaceChildren() removes all child nodes without going through the HTML
  // parser — avoids any inadvertent innerHTML semantics on clear.
...
/**
 * Render a PaintScene to an inline SVG element and attach it to `container`.
 *
 * The SVG backend produces a string via renderToSvgString(). We parse that
 * string with DOMParser (MIME type "image/svg+xml") rather than setting
 * innerHTML directly. DOMParser in SVG mode does NOT execute <script> elements
 * inside the parsed document, making it safer than innerHTML even when the
 * SVG content is entirely generated by our own code. This is a defence-in-depth
 * measure: if a future change accidentally introduces an unescaped field into
 * the SVG string, innerHTML would execute any <script> it contained; DOMParser
 * will not.
 *
 * Note: document.adoptNode() moves the element from the DOMParser's document
 * into the host document. importNode() would deep-copy it instead — either
 * works, adoptNode is slightly more efficient for a single use.
 */
export function renderWithSvg(scene: PaintScene, container: HTMLElement): void {
  // Clear previous content without innerHTML (avoids a redundant HTML parse).
  container.replaceChildren();

  const svgStr = renderToSvgString(scene);

  // Parse as XML/SVG — scripts in <script> elements do NOT execute.
  const parser = new DOMParser();
  const svgDoc = parser.parseFromString(svgStr, "image/svg+xml");
  const svgEl = svgDoc.documentElement;

  // Move the root <svg> element into our page's document.
