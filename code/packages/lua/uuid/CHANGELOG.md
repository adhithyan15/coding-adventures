# Changelog — coding-adventures-uuid (Lua)
...
- `parse(uuid_str)` — parses UUID into `{version, variant, bytes}` table
...
- Comprehensive busted test suite including RFC 4122 Appendix B test vectors
