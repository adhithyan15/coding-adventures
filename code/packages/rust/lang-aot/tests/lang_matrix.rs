...
const PROGRAMS: &[Prog] = &[
    ...
    // ALGOL 60 — exact local integer snapshots may widen into bounded static
    // real expressions. Copying preserves the destination when the source is
    // reassigned; control flow, calls, captures, dynamic values, and integers
    // outside binary64's exact range still require the deferred formatter.
    Prog {
        lang: Language::Algol60,
        ext: "alg",
        src: "begin integer n, saved; n := 40; saved := n; n := 9; output(saved + 2.5) end",
...
#[test]
fn algol_static_integer_scalar_widening_runs_on_every_available_standard_backend() {
    let program = PROGRAMS
        .iter()
        .find(|program| {
            program.lang == Language::Algol60
                && program
                    .src
                    .contains("n := 40; saved := n; n := 9; output(saved + 2.5)")
        })
        .expect("the static integer-scalar program must remain in the matrix");
    ...
    for backend in [NativeAot, Llvm, Wasm, Jvm, Clr, Vm, Jit] {
        ...
        let Some(result) = run(backend, program) else {
            assert!(
                !toolchain_available,
                "{backend:?} toolchain is present but static integer-scalar output did not complete"
            );
            continue;
        };
        assert_cell(backend, program, result);
