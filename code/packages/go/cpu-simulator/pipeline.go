...
func (p *PipelineTrace) FormatPipeline() string {
	...
	if len(decodeLines) > maxLines { maxLines = len(decodeLines) }
	if len(executeLines) > maxLines { maxLines = len(executeLines) }
	
	for len(fetchLines) < maxLines { fetchLines = append(fetchLines, "") }
	for len(decodeLines) < maxLines { decodeLines = append(decodeLines, "") }
	for len(executeLines) < maxLines { executeLines = append(executeLines, "") }
	
	colWidth := 20
	// We use %-[width]s to pad the strings to the required column width explicitly.
	result := []string{fmt.Sprintf("--- Cycle %d ---", p.Cycle)}
	for i := 0; i < maxLines; i++ {
		f := fmt.Sprintf("%-*s", colWidth, fetchLines[i])
		d := fmt.Sprintf("%-*s", colWidth, decodeLines[i])
		e := fmt.Sprintf("%-*s", colWidth, executeLines[i])
		result = append(result, fmt.Sprintf("  %s | %s | %s", f, d, e))
	}
