defmodule CodingAdventures.Argon2id do
  @moduledoc """
  Argon2id (RFC 9106) — the RFC-recommended memory-hard password hashing
  function.
  ...
  @spec argon2id(binary(), binary(), pos_integer(), pos_integer(),
  ...
  def argon2id(password, salt, time_cost, memory_cost, parallelism, tag_length,
    ...
    h0 = compute_h0(password, salt, time_cost, memory_cost, parallelism,
                    tag_length, key, ad, version, @type_id)
  ...
  @spec argon2id_hex(binary(), binary(), pos_integer(), pos_integer(),
  ...
  defp permutation_p(v) do
    v
    |> g_mix(0, 4, 8, 12) |> g_mix(1, 5, 9, 13)
    |> g_mix(2, 6, 10, 14) |> g_mix(3, 7, 11, 15)
    |> g_mix(0, 5, 10, 15) |> g_mix(1, 6, 11, 12)
    |> g_mix(2, 7, 8, 13) |> g_mix(3, 4, 9, 14)
  end
  ...
  defp compress(x, y) do
    ...
    q_tuple =
      Enum.reduce(0..7, r_tuple, fn row, acc ->
        row_tuple = for(i <- 0..15, do: elem(acc, row * 16 + i)) |> List.to_tuple()
    ...
    q_tuple =
      Enum.reduce(0..7, q_tuple, fn c, acc ->
        col_tuple = for(rr <- 0..7, k <- 0..1, do: elem(acc, rr * 16 + 2 * c + k)) |> List.to_tuple()
    ...
    for i <- 0..(@block_words - 1), do: bxor(elem(r_tuple, i), elem(q_tuple, i))
  end
  ...
  # Argon2id: data-independent for (pass=0, slice in {0,1}), data-dependent after.
  defp fill_segment(memory, r, lane, sl, q, sl_len, p, m_prime, t_total) do
    data_independent = r == 0 and sl < 2
    zero_block = List.duplicate(0, @block_words)

    initial_input =
      if data_independent do
        zero_block
        |> List.replace_at(0, r)
    ...
    {initial_input, initial_block} =
      if data_independent and starting_c != 0 do
    ...
    final =
      Enum.reduce(starting_c..(sl_len - 1)//1, state, fn i, st ->
        {input_now, address_now} =
          if data_independent and rem(i, @block_words) == 0 and
               not (r == 0 and sl == 0 and i == 2) do
        ...
        pseudo_rand =
          if data_independent,
            do: Enum.at(address_now, rem(i, @block_words)),
            else: List.first(prev_block)
