...
The next smallest starter entry is ے; HL-C09AG queues its source-backed audit
from Chapter 4.
...
| HL-C09 | Queued — 32 of 228 verified | Expand `DUCTUS` to cover the ten scripts with prose stroke-order entries. | Add cited, font-checked ductus and verified pen-lift metadata for the remaining 196 entries measured by HL-C19; each passes the on-ink, join-tolerance, coverage, citation, and source-agreement invariants. |
...
| HL-C09AG | Queued — next | Locate and verify independent Urdu ے in *Zer o Zabar*'s source-backed chapter *Chhoṭī he, do-chashmī he, chhoṭī ye, baṛī ye, and punctuation*. | Audit the independent baṛī ye animations and prose, determine its broad final-bowl direction and exact lift order, then reconcile that motion with the canonical Noto Naskh fallback before authoring the final Urdu starter path. |
