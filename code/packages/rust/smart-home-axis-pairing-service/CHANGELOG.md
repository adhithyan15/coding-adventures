...
- Add D23-authorized Axis credential provisioning from one-shot owner-only
  secret files.
- Verify exact installed-bridge correspondence through the pending session,
  HTTPS endpoint identifier, authenticated VAPIX inspection, stable serial,
  and enabled camera-1 JPEG capability.
