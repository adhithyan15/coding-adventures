...
import { ACC_PUBLIC, ACC_STATIC, buildMinimalClassFile, ClassFileFormatError, parseClassFile, } from "../src/index.js";
function helloWorldFixture() {
...
function richFixture() {
    ...
    const klass = (nameIndex) => bytes(7, u2(nameIndex));
    const stringInfo = (stringIndex) => bytes(8, u2(stringIndex));
    const integer = (value) => {
        const buffer = new ArrayBuffer(4);
        new DataView(buffer).setInt32(0, value, false);
        return bytes(3, new Uint8Array(buffer));
    };
    const nameAndType = (nameIndex, descriptorIndex) => bytes(12, u2(nameIndex), u2(descriptorIndex));
    const fieldref = (classIndex, nameAndTypeIndex) => bytes(9, u2(classIndex), u2(nameAndTypeIndex));
    const methodref = (classIndex, nameAndTypeIndex) => bytes(10, u2(classIndex), u2(nameAndTypeIndex));
    const entries = [
        utf8("demo/Rich"),
        klass(1),
    ...
    const codeAttribute = bytes(u2(5), u4(19), u2(0), u2(1), u4(code.length), code, u2(0), u2(1), u2(5), u4(0));
    const method = bytes(u2(ACC_PUBLIC | ACC_STATIC), u2(19), u2(20), u2(1), codeAttribute);
    return bytes(u4(0xcafebabe), u2(0), u2(49), u2(entries.length + 1), ...entries, u2(ACC_PUBLIC), u2(2), u2(4), u2(0), u2(0), u2(1), method, u2(0));
}
describe("jvm-class-file", () => {
    it("builds and parses a minimal class file", () => {
        ...
        expect(parsed.findMethod("_start", "()I")?.codeAttribute()?.code).toEqual(Uint8Array.of(0x03, 0xac));
    });
...
//# sourceMappingURL=jvm-class-file.test.js.map