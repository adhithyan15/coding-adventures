...
///
/// One sentence covers both ovarian hormones.
#[test]
fn the_shared_spans_are_exactly_the_declared_ones() {
    let pairs = row_spans("biology/hormone-glands.adj");
    assert_eq!(pairs.len(), 12, "every row carries its own source: {pairs:?}");
    let groups = sharing_groups(&pairs);
    let expected: Vec<Vec<&str>> = vec![
        vec!["estrogen, ovary", "progesterone, ovary"],
    ];
    assert_eq!(
        groups, expected,
    ...
    assert_eq!(
        pairs.len() - shared,
        10,
        "and 10 rows have a sentence to themselves: {groups:?}"
    );
}
