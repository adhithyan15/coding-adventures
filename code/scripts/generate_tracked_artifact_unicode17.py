...
def _download_sources() -> dict[str, str]:
    ...
    for name, expected_hash in SOURCES.items():
        # The destination is a fixed HTTPS origin and every payload is pinned
        # by SHA-256 immediately below; user input cannot influence the URL.
        with urllib.request.urlopen(  # nosec B310
            UCD_BASE + name, timeout=30
        ) as response:
            payload = response.read()
        actual_hash = hashlib.sha256(payload).hexdigest()
        if actual_hash != expected_hash:
            raise RuntimeError(
                f"Unicode source hash mismatch for {name}: "
...
// Sources: {UCD_BASE}
// {hashes}
// Unicode License v3: code/specs/fixtures/build-tool-v1/UNICODE-LICENSE.txt
// sha256:{LICENSE_SHA256}
...
def main() -> int:
    ...
    # This is a fixed HTTPS URL whose exact response digest is checked below.
    with urllib.request.urlopen(LICENSE_URL, timeout=30) as response:  # nosec B310
        upstream_license = response.read()
    if hashlib.sha256(upstream_license).hexdigest() != LICENSE_SHA256:
        raise RuntimeError("upstream Unicode License v3 notice hash changed")
