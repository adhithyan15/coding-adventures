...
and independent/final sound distinction remain explicit. Arabic is now the
smallest remaining starter inventory at 21 entries; HL-C09AH queues a
...
| HL-C09 | Queued — 33 of 228 verified | Expand `DUCTUS` to cover the ten scripts with prose stroke-order entries. | Add cited, font-checked ductus and verified pen-lift metadata for the remaining 195 entries measured by HL-C19; each passes the on-ink, join-tolerance, coverage, citation, and source-agreement invariants. |
...
| HL-C09AH | Queued — next | Audit a source for independent Arabic ا and preserve its script identity before authoring the first path in the smallest remaining inventory. | Locate a source that exposes the independent Arabic handwriting motion and lift order, then keep its Arabic provenance addressable separately from Persian and Urdu ا while reconciling the learner path with the canonical Noto Naskh fallback. |
