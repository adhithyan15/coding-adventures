...
HEAP_ROOT = REPO_ROOT / "code" / "packages" / "python" / "heap"


class PythonHeapBuildFrontTests(unittest.TestCase):
    ...
    def _recipe(name: str) -> list[str]:
        return [
            line
            for line in (HEAP_ROOT / name).read_text(encoding="utf-8").splitlines()
