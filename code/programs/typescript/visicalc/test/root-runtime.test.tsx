...
it("recalculates SUM chains after entering a formula through the generated root", async () => {
  const module = await loadMosaicModule(readFileSync("public/visicalc_mosaic_app.wasm"));
  const host = module.create({ colorScheme: "light" });
  const container = document.createElement("div");
  document.body.append(container);
  ...
  try {
    ...
    expect(input.value).toBe("15");
    await act(async () => {
      Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, "value")!.set!.call(input, "=20+22");
      input.dispatchEvent(new Event("input", { bubbles: true }));
    });
    await act(async () => { input.dispatchEvent(new KeyboardEvent("keydown", { key: "Enter", bubbles: true })); });
    expect(container.querySelectorAll("tbody:not([data-mosaic-spacer]) tr")[4].querySelectorAll("td")[4].textContent).toBe("196");
    expect(host.update.props.formula).toBe("=20+22");
  } finally {
    await act(async () => root.unmount());
    container.remove();
  }
