...
% canonical-source-hash: fnv1a64:7cc1d9ec6aee25f9
...
\emph{Reducir costes} — to cut costs. \emph{Reducir la velocidad} — to slow down, the wording on a road sign. \emph{Han reducido la plantilla} — they've cut the staff.
...
\emph{Reducir} belongs to a group whose \textbf{first person} gains a \textbf{c}:
...
Everything else is regular — \emph{reduces}, \emph{reduce}, \emph{reducimos}. Only the form ending in \textbf{-o} takes the extra consonant, and it does so because a Latin sound sat there and survived in exactly that one slot.

If you have ever heard somebody say \emph{conozco} and wondered where the \textbf{z} came from, this is the same group, and there are perhaps forty of them.
\end{grammarlens}
...
\emph{Ducere} is everywhere once you see it. \textbf{La ducha} is the same root through French \emph{douche} and Italian \emph{doccia}: a pipe that \textbf{leads} water. English has \textbf{conduct}, \textbf{produce}, \textbf{educate} (to lead out), \textbf{duke} — a leader — and \textbf{aqueduct}, which leads water exactly as a shower does.
...
\begin{quote}
Say \textbf{decidir} and \textbf{acercar}. You already know what putting \textbf{-se} on the second one did. Do it to the first.
\end{quote}
...
So \emph{el jefe} and \emph{la cabeza} are one Latin noun arriving twice: once worn down through Spanish speech into the body part, once borrowed from French as the person in charge. A boss is literally \textbf{a head}, exactly as in English \emph{head of department}.
...
You have watched this twice before — \emph{rueda} against \emph{rotativo}, \emph{consejo} against \emph{consultar} — but those pairs both came from Latin. This one arrived through a \textbf{third} language, which is why it looks like neither.
\end{grammarlens}
