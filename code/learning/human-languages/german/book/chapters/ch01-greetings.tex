...
contrast. Writing begins with the same familiar cousin: follow one visible
\emph{Hallo}, then copy it once while the model stays in sight. No hidden
spelling and no second word are due.

% canonical-insertion: GE-C01-hallo
...
Keep \emph{Hallo} visible. Point to the tall capital \emph{H}, then trace the
whole greeting once in large, slow letters from \emph{H} to \emph{o}. Say
\emph{HA-lo} while your finger moves. This is observation, not a spelling test:
do not hide the model or write from memory.

% canonical-insertion: GE-W01-hallo-guided-copy
\section{\emph{Hallo} --- one guided copy with the model visible}
\label{lesson:GE-W01-hallo-guided-copy}

Point to \emph{Hallo} and say \emph{HA-lo} once. Keep that model in sight. Copy
the greeting \textbf{once}: begin with the tall capital \emph{H}, then follow
with \emph{allo}. Look back whenever you need. Draw one small line under the
\emph{H} shared by German \emph{Hallo} and its English hailing family.

Compare your copy with the model. If one letter differs, repair only that
letter, say \emph{HA-lo}, and stop. No hiding, no memory spelling, and no second
word today.
