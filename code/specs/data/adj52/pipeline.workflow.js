...
6. Write this case's PROGRAM, separate and swappable (Write tool, absolute): ${CASE_DIR}/${id}/program.adj — containing ONLY observe <term> lines for THIS patient (same vocabulary as the rulebook) and ? <conclusion> query lines for each candidate + the next step. NO rules in the program. ALSO add exactly one directive comment line declaring the mutually-EXCLUSIVE single diagnoses so the runner normalizes them into one coherent differential: "% @exclusive diagnosis(a), diagnosis(b), diagnosis(c)" — list the atomic single diagnoses that genuinely compete; do NOT include any "coexisting"/combinable hypothesis in that list (leave those out so they are reported separately).
...
8. Return: domain_key, rules_added (count you added this case), compiled_ok, top_conclusion + top_posterior (taken from the runner's "Coherent differential" section — the softmax-normalized top entry, NOT a raw independent per-hypothesis posterior), recommended_next_step, engine_output_excerpt (the coherent differential + per-query posteriors), and framework_answer_text — a neutral judge-facing rendering that LEADS WITH THE COHERENT DIFFERENTIAL (the normalized competing diagnoses that sum to 100%), then the KEY fired clauses WITH citations (especially any discriminator that fired negative against a tempting wrong answer), the recommended next step, the management/disposition implied by the leading diagnosis, and any open uncertainties. Express confidence as the coherent-differential probability; do NOT report multiple independent ~100% posteriors as if they were a ranking.
