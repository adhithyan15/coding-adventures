...
- **`BUILD` file — this crate is now built, tested and linted in CI.**
  ...
  The BUILD is the repo-standard one-liner, `cargo test -p numeric-tower -- --nocapture`,
...
- **`README.md`** — the five rungs, the promotion order (`Complex > Float >
  Decimal > Rational > Integer`), why demotion is fallible, and the thirteen
  crates downstream of this one.
