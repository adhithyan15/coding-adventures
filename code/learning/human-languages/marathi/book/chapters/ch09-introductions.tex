...
\hlchaptermodality{9}
% canonical-insertion: MR-R09-runway-tail-r2 (schema-v2 lesson in a hand-written chapter)
...
\section[Practice]{The last six signs, one chapter on}
\label{lesson:MR-R09-runway-tail-r2}

\begin{quote}
No new language and nothing to look at. The six signs that closed the second
runway are asked for cold, a chapter after you learned them --- the first real
retrieval of the set.
\end{quote}

\subsection*{Your turn}
Answer aloud, with the page shut:

\begin{enumerate}
\raggedright
  \item which \emph{l} does Marathi keep that Hindi does not? (the curled one,
    \mr{ळ})
  \item which two sibilants are told apart only in spelling? (\emph{\'sa} and
    \emph{\d{s}a})
  \item which vowel opens the word for ``one''? (\emph{e}, in \emph{ek})
  \item how many written forms does the \emph{u} vowel have? (four --- two
    letters and two marks)
\end{enumerate}

Repair one group only. This is the first retrieval, not the last: the same six
come back at a durable gap in Chapter~13 and at book distance in Chapter~22.
