...
| #130 Message abstraction | Open | D19's generic Actor message exists in all six languages, while the production Rust D18 envelope supplies the signed XChaCha20-Poly1305 header, SHA-256 plaintext binding, and D18M binary record. Its message fields, header, ciphertext, tag, and signature are structurally immutable behind read-only accessors (#11642). D18F now fixes that production format as the portable binary baseline and specifies lossless JSON, rich payloads, deterministic sources, validation order, stable errors, and the fixture plan (#11649). A Rust profile/JSON adapter and fixture lock, the other five language implementations, and the shared cross-language fixture gate remain. |
