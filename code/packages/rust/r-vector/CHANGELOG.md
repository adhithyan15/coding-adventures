...
- **`BUILD` file — this crate is now built, tested and linted in CI.**
  ...
  The BUILD is the repo-standard one-liner, `cargo test -p r-vector -- --nocapture`,
...
- **`README.md`** — how `NA` is carried as a reserved NaN payload
  (`0x7ff0_0000_0000_07a2`), why `is_nan_real` is deliberately
  "NaN **and not** the sentinel", and the twelve crates downstream.
