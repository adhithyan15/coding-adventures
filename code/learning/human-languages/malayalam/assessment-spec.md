# Coding Adventures Malayalam Assessment
...
Malayalam awarding body, and the Council of Europe states that it does not
verify or validate an examination's claimed link to CEFR. Every target here is
therefore labelled `project-defined`.

Malayalam has a real graded ladder for learners outside Kerala, and it is worth
naming precisely because it is the closest thing to what this book is for. The
**Malayalam Mission**, run by the Government of Kerala's Department of Cultural
Affairs, teaches Malayalam to the diaspora through four named stages, each
called after a flower:

| stage | name | duration |
|---|---|---|
| 1 | കണിക്കൊന്ന (Kanikonna) | two years |
| 2 | സൂര്യകാന്തി (Sooryakanthi) | two years |
| 3 | അമ്പൽ (Ambal) | three years |
| 4 | നീലകുരിഞ്ഞി (Neelakurinji) | three years |

A learner who completes all four becomes eligible for a certificate the state
treats as equivalent to the tenth standard.

That is a genuine ten-year ladder aimed at exactly this book's reader — and it
is a **school-equivalence** ladder rather than a four-skill proficiency
examination, with no published per-skill pass rule and no independent reading,
listening, writing and speaking papers. It certifies a course completed, not a
proficiency measured. Borrowing its name would claim something this project does
not test. The Mission's existence is recorded here as evidence that the audience
and the need are real, not as the target.

### The script question, which Malayalam cannot leave unanswered

Malayalam is written in two orthographies and an examination has to say which it
accepts. A Kerala government committee reported in 1969, and in 1971 the
Government of Kerala made a reformed script official: it detached the -u, -ū and
-r̥ vowel signs from their consonants, linearised most consonant clusters using
the chandrakkala instead of stacking them, unified and detached the special
combining forms for y, r and v, and retained the chillu letters and certain
native clusters unchanged.

The point that decides this section is what the reform was FOR. The simplified
orthography was intended for **printing** — it made typesetting and later
typewriting tractable — while the traditional orthography was retained for
**handwriting**. Every writing paper in this contract is handwritten. A rule
that accepted only the printed convention would be marking candidates down for
writing the way the reform expected them to write.

So:

- **Both orthographies are accepted** in reading input and in written responses.
  A candidate who writes a traditional stacked conjunct where the reformed
  script would linearise it has not made an error, and neither has one who does
  the reverse.
- **The book's own model is the reformed convention**, which is also what
  Unicode encodes by default and what the corpus writes: the track uses the five
  chillu letters ൺ ൻ ർ ൽ ൾ, and writes ന്റ throughout — thirty-nine times, with
  the chillu-based ൻ്റ spelling appearing nowhere.
- Reading input at pre-A1 and A1 uses the reformed script only, because a
  learner at those rungs has been taught one set of shapes and a mixed paper
  would measure exposure rather than reading. From A2 onward, at least one
  reading part per paper is set in the traditional script, since a reader of
  real Malayalam print — and of any Malayalam handwriting — will meet it.

Documented regional forms receive credit when used consistently and when they
satisfy the task; a trained Malayalam reviewer adjudicates unfamiliar forms.
...
- [Malayalam Mission course stages, Kerala Association of Delaware Valley zonal centre](https://www.kadavu.org/malayalam-mission/)
- [Karthik Malli, "Malayalam: scripting tradition and modernity", Typotheque, 25 June 2023](https://www.typotheque.com/research/malayalam-scripting-tradition-and-modernity)
...
3. range and control of Malayalam vocabulary and grammar;
4. Malayalam orthographic control for writing, or pronunciation and fluency for
   speaking.

Each dimension is scored 0–5, then scaled to the paper's points. A response that
is not meaningfully in Malayalam cannot earn task-fulfilment credit.

Dimension 4 for writing is specific to the script and is scored on three things
the Malayalam writing system actually requires:

- the **chandrakkala** ്, which does two different jobs and is marked for both.
  Inside a word it joins two consonants into one shape; at the end of a word it
  is the half-u, breathed rather than said. Omitting it word-finally is an error
  even though nothing audible is lost by a careless ear;
- **conjunct formation**, whether written as a traditional ligature or as the
  reformed split — consistently one or the other within a response;
- **legibility and word boundary.**

Both orthographies are correct; mixing them within a single response is scored
under dimension 2 as an organisation cost, not under dimension 4 as an error.

## Administration rules

- Listening recordings use Malayalam speakers. Words-per-minute bands below are
  form-assembly targets, not a claim that Malayalam has one natural speaking
  rate.
- Pre-A1 and A1 directions may also be supplied in a declared support language,
  so misunderstanding directions is not confused with Malayalam proficiency.
  From A2 onward directions are in Malayalam, with one unscored example.
...
evidence. Before release, at least eight qualified Malayalam educators or
...
|---|---:|---|
| reading | 10 | 6 Malayalam shape/word matches, 4 short phrase or notice matches, and 4 personal-detail selections; no text over 8 words |
