/* Tests for the C argon2d, using the header-only iso_test.h harness (pure ISO).
 * The primary vector is RFC 9106 §5.1 (Argon2d), matching the Rust crate. */
#include "iso_test.h"
...
int main(void) {
    /* ── RFC 9106 §5.1 Argon2d known-answer ─────────────────────────────── */
    {
        ...
        ISO_CHECK_EQ_INT(
            (int)argon2d(password, 32, salt, 16, 3, 32, 4, 32, &opts, tag),
            (int)ARGON2D_OK);
        from_hex("512b391b6f1162975371d30919734294f868e3be3984f3c1a13a4db9fabe4acb",
                 want);
    ...
    /* ── determinism, and sensitivity to inputs ─────────────────────────── */
    {
        uint8_t a[32];
        uint8_t b[32];
        ISO_CHECK_EQ_INT((int)argon2d((const uint8_t *)"password", 8,
        ...
        argon2d((const uint8_t *)"password1", 9, (const uint8_t *)"somesalt", 8,
                1, 8, 1, 32, NULL, a);
        argon2d((const uint8_t *)"password2", 9, (const uint8_t *)"somesalt", 8,
                1, 8, 1, 32, NULL, b);
        ISO_CHECK(memcmp(a, b, 32) != 0); /* different passwords */

        argon2d((const uint8_t *)"password", 8, (const uint8_t *)"saltsalt", 8, 1,
                8, 1, 32, NULL, a);
        argon2d((const uint8_t *)"password", 8, (const uint8_t *)"saltsal2", 8, 1,
                8, 1, 32, NULL, b);
        ...
        argon2d((const uint8_t *)"password", 8, (const uint8_t *)"saltsalt", 8, 1,
                8, 1, 32, NULL, a);
        argon2d((const uint8_t *)"password", 8, (const uint8_t *)"saltsalt", 8, 2,
                8, 1, 32, NULL, b);
    ...
    /* ── multi-lane parameters produce a 32-byte tag ────────────────────── */
    {
        ...
        memset(salt, 0x02, 16);
        ISO_CHECK_EQ_INT(
            (int)argon2d(password, 32, salt, 16, 3, 32, 4, 32, NULL, tag),
            (int)ARGON2D_OK);
    }
    ...
    {
        uint8_t tag[32];
        Argon2dOptions badver;
        ISO_CHECK_EQ_INT((int)argon2d((const uint8_t *)"pw", 2,
                                      (const uint8_t *)"short", 5, 1, 8, 1, 32,
                                      NULL, tag),
                         (int)ARGON2D_SALT_TOO_SHORT);
        ISO_CHECK_EQ_INT((int)argon2d((const uint8_t *)"pw", 2,
                                      (const uint8_t *)"saltsalt", 8, 1, 8, 1, 3,
                                      NULL, tag),
                         (int)ARGON2D_TAG_TOO_SMALL);
        ISO_CHECK_EQ_INT((int)argon2d((const uint8_t *)"pw", 2,
                                      (const uint8_t *)"saltsalt", 8, 1, 1, 1, 32,
                                      NULL, tag),
                         (int)ARGON2D_MEMORY_TOO_SMALL);
        ISO_CHECK_EQ_INT((int)argon2d((const uint8_t *)"pw", 2,
                                      (const uint8_t *)"saltsalt", 8, 0, 8, 1, 32,
                                      NULL, tag),
                         (int)ARGON2D_TIME_COST_ZERO);
        ISO_CHECK_EQ_INT((int)argon2d((const uint8_t *)"pw", 2,
                                      (const uint8_t *)"saltsalt", 8, 1, 8, 0, 32,
                                      NULL, tag),
                         (int)ARGON2D_INVALID_PARALLELISM);
        badver.key = NULL;
        badver.key_len = 0;
        ...
        ISO_CHECK_EQ_INT((int)argon2d((const uint8_t *)"pw", 2,
                                      ...
                                      &badver, tag),
                         (int)ARGON2D_UNSUPPORTED_VERSION);
    }
