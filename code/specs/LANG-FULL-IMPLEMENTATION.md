...
snapshot exponents and nested pure conditionals whose snapshot leaves prove
the same value over plain real bases; these do not require a standard function
in the base.
...
- ◑ **AL4** — literal string `print`/`output` I/O runs on all 7 backends via
  **E4**. Undeclared statement-position `print('HI')`/`output('HI')` calls lower
  ...
  `f64_pow`. Nested pure conditionals over such snapshots may also participate
  when every leaf proves the same exact value and each selector is call-free;
  all selector execution remains in emitted IIR.
