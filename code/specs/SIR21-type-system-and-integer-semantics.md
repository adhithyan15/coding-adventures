...
| T3b | `semantic-ir` | Division split: distinct `div_floor` / `div_trunc` / `div_true` ops so division semantics are explicit in the IR (Ruby `7/2==3` floor, C `-7/2==-3` trunc, Python `7/2==3.5` true) instead of one overloaded `_sir_divide` runtime helper. Frontend emits the right one; each backend lowers to the matching primitive. |
| T3c | `semantic-ir` | Type-directed op-selection rules: an arithmetic/comparison node resolves to a concrete op from operand static types, falling back to runtime dispatch only when an operand is `Dynamic`. |
