...
- Mixed-signal coupling with `hardware-vm.md` for AMS simulation. A first
  ...
  direct and named-corner digital-input transient bridge fixtures, and samples
  one or more transient probes back into thresholded named digital event
  streams, with stable tab-separated single-stream, named multi-signal, and
  cornered bridge event-stream text output;
