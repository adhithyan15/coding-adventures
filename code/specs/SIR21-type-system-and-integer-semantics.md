...
| T2c | `sir-conformance` | Coverage gate (P5): for every `(SirType × op × backend)` a backend's manifest accepts on the Phase-1 net, assert a passing conformance case exists; accepted-but-untested fails CI. Split from T2b (needs per-backend accept-manifests, a structural addition). |
