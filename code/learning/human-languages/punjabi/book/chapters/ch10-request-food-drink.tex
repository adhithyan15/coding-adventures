...
% canonical-source-hash: fnv1a64:256a90394a392134
% canonical-lessons: PA-C10-paani, PA-C10-chaa, PA-S10-chachcha, PA-C10-dudh, PA-C10-roti, PA-S10-addak-dhadda
...
\end{tcolorbox}
\section[ca]{\pa{ਚ} — one piece met inside a word you already say}

\label{lesson:PA-S10-chachcha}



\begin{quote}
One consonant, and it opens the word you are about to order.

First, say the last things you learned once more, out loud and from memory:

\begin{itemize}
\raggedright
  \item \emph{Say it:} \emph{madad karnā} · \emph{pasand} · \emph{pāṇī}
\end{itemize}
\end{quote}

\subsection*{Script you'll notice: \pa{ਚ}}
\begin{quote}
\pa{ਚ}
\end{quote}

\textbf{chachchā} — \emph{ca}. A consonant, the \emph{ch} of \emph{church} but with no puff of breath after it.

Now put them where you have already heard them:

\begin{quote}
\textbf{\pa{ਚ}} + \textbf{\pa{ਾ}} + \textbf{\pa{ਹ}} $\to$ \textbf{\pa{ਚਾਹ}} (\emph{cāh})
\end{quote}
...
Keep \pa{ਚ} in front of you and do not cover it. Follow each shape once with a finger, then copy it once with a pencil, larger than it is printed. Say its sound as your hand stops.
...
\begin{itemize}
\raggedright
  \item \emph{Look:} at \textbf{\pa{ਚਾਹ}} and point to each of today's piece in turn
  \item \emph{Say it:} \emph{ca}, then the whole word — \emph{cāh}
  \item \emph{Look:} back at the headword at the top of any earlier lesson in this chapter and find one shape you now know
\end{itemize}

\begin{tcolorbox}[breakable,colback=teal!4,colframe=teal!35!black,title={Before you move on}]
Name today's piece: \emph{chachchā}. Which of them is a \textbf{mātrā} or a mark rather than a letter, and what does \textbf{\pa{ਚਾਹ}} say? (\emph{cāh}, “tea”.)
...
\end{tcolorbox}
\section[(doubled) · dha]{\pa{ੱ} · \pa{ਧ} — two pieces met inside a word you already say}

\label{lesson:PA-S10-addak-dhadda}



\begin{quote}
A mark that makes no sound of its own, and still changes what a word means.

First, say the last things you learned once more, out loud and from memory:

\begin{itemize}
\raggedright
  \item \emph{Say it:} \emph{pāṇī} · \emph{cāh}
\end{itemize}
\end{quote}

\subsection*{Script you'll notice: \pa{ੱ} · \pa{ਧ}}
\begin{quote}
\pa{ੱ}
\end{quote}

\textbf{aḍḍak} — \emph{(doubled)}. Not a sound of its own. It sits above the line and says \textbf{double the consonant that follows} — and doubling changes meaning in Punjabi, so this small mark is load-bearing.

\begin{quote}
\pa{ਧ}
\end{quote}

\textbf{dhaddā} — \emph{dha}. A third letter whose breath became \textbf{pitch}: written breathy-\emph{d}, said today with a falling tone.

Now put them where you have already heard them:

\begin{quote}
\textbf{\pa{ਦ}} + \textbf{\pa{ੁ}} + \textbf{\pa{ੱ}} + \textbf{\pa{ਧ}} $\to$ \textbf{\pa{ਦੁੱਧ}} (\emph{dudh})
\end{quote}
...
\subsection*{Writing: copy what you can see}
Keep \pa{ੱ} · \pa{ਧ} in front of you and do not cover any of them. Follow each shape once with a finger, then copy it once with a pencil, larger than it is printed. Say its sound as your hand stops.
...
\begin{itemize}
\raggedright
  \item \emph{Look:} at \textbf{\pa{ਦੁੱਧ}} and point to each of today's pieces in turn
  \item \emph{Say it:} \emph{(doubled)}, \emph{dha}, then the whole word — \emph{dudh}
  \item \emph{Look:} back at the headword at the top of any earlier lesson in this chapter and find one shape you now know
\end{itemize}

\begin{tcolorbox}[breakable,colback=teal!4,colframe=teal!35!black,title={Before you move on}]
Name today's pieces: \emph{aḍḍak}, \emph{dhaddā}. Which of them is a \textbf{mātrā} or a mark rather than a letter, and what does \textbf{\pa{ਦੁੱਧ}} say? (\emph{dudh}, “milk”.)
