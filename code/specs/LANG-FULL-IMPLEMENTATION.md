...
The original LANG-PLATFORM matrix (`LANG-PLATFORM-MATRIX.md`) proved the
...
value over plain real bases. Exact variable-free built-in-result leaves may
also participate; none of these forms requires a standard function in the base.
...
recurring timer (CI failures, merge conflicts, merged-check). Enable auto-merge
once required checks are healthy unless the campaign owner requests manual
review. On `state==MERGED`: clean up the worktree, then loop to the next ☐
item. Safety valve: stop and ask on a genuine architectural fork (flagged ⚠
below) or after 2 consecutive failures.
...
- **E6 — General `call_builtin` / closures / dynamic dispatch on code-gen backends.**
  ✅ **COMPLETE on the five code-generation backends.** McCarthy Lisp's boxed
...
- ✅ **AL3** — typed procedures with value parameters. `integer procedure sq(x);
  ...
  propagated its dead seed → only the JIT returned 0). **Limits (follow-ups):** direct call-by-name
  is AL7; zero-argument calls may use explicit `f()` in
...
- ◑ **AL4** — literal string `print`/`output` I/O runs on all 7 backends via
  **E4**. Undeclared statement-position `print('HI')`/`output('HI')` calls lower
  ...
  A `while` element whose initial predicate is statically false likewise
  preserves entry scalar snapshots and records the controlled scalar's static
  initial value; unknown and potentially repeating `while` elements remain
  conservative.
  ...
  dependencies still fail closed. Exact integer or real control assignments
  ...
  selectors also accept bounded side-effect-free checked integer arithmetic
  exponent operands backed by exact local scalar snapshots; user calls, globals,
  and invalidated values remain dynamic. Chains equal to one return the base
  ...
  arithmetic, effectful selectors, differing branches, user calls and
  overrides, globals, and invalidated values stay on `f64_pow`.
  A real power also accepts an exact integral initialized ordinary local real
  ...
  may also participate, as may exact finite variable-free built-in results,
  when at least one leaf is a tracked real snapshot, every leaf proves the same
  exact value, and each selector is call-free; all selector execution remains
  in emitted IIR. User calls and built-in overrides, nested powers, fractional
  or non-finite results, inexact integer widening, and differing leaves fail
  closed.
  ...
  value requires a standard-function result in the base. Runtime selector
  execution is preserved. Uninitialized or nonlocal values, differing
  branches, effectful selectors, fractional and non-finite results, calls, and
  nested powers remain on `f64_pow`.
  A real power accepts finite arithmetic that mixes initialized ordinary local
  integer and real snapshots when the result is an exact nonnegative integer
  within the same unroll cap; the base need not contain a standard-function
  result. Every integer must widen exactly to binary64, and both numeric
  domains must be present. A pure conditional exponent may also participate if
  every runtime branch proves the same exact bounded value; its selector
  remains in emitted IIR. Uninitialized or nonlocal values, inexact integer
  widening,
  fractional or non-finite results, differing branches, effectful selectors,
  calls, and nested powers remain on `f64_pow`.
  ...
  and a statically known statement condition preserves only the selected
  branch's state, while dynamic assignments and existing control-flow/call
...
- ✅ **AL7** — direct call-by-name (Jensen-style expression thunks).
  ...
  stored caller expression; scalar writes target a bare-variable or subscripted
  array-element actual. Caller dependencies are published through the existing typed globals,
  ...
  a finite remapping such as `flip(n, y, x)` while preserving each caller
  expression and write target. Changed recursive scalar actuals still require
  a dynamic thunk frame. String writes retain normal copy semantics
  before storing through the caller binding. A
  name array is restricted to a bare array actual and uses the ordinary typed
  descriptor ABI (handle, lower bounds, and strides), so direct and forwarded
  element writes retain caller storage. A direct sibling with only name-array
  formals can now re-enter itself or another in-flight name-array sibling,
  including mutual recursion, because every recursive call carries that same
...
- ✅ **AL14** — direct formal procedures. A name- or value-mode `procedure p`
  formal accepts a direct declared procedure (including a nested procedure
  capturing an enclosing value formal or self-/mutually-recursively forwarded
  through active specialised siblings), implemented standard-function, or
...
- ✅ **TW3** — list / cons ops. `cons`/`car`/`cdr`, `list`, `length`,
  ...
  See [`lang-full-e6-dispatch.md`](lang-full-e6-dispatch.md).
- ✅ **TW4** — typed E4 strings on code-gen backends. Direct literals,
  ...
  dynamic-`any` string path still need dedicated representation work.
- ✅ **TW5** — closures / lambdas. Direct and capturing closures lower to
...
- ✅ **MC1** — F1–F7 are complete on all eight applicable backends. The W16
  conformance suite runs the same 19 programs across the dedicated VM, native
  AOT, JIT, WASM, JVM, CLR, BEAM, and LLVM; see
  [`MCCARTHY-LISP-PLATFORM-MATRIX.md`](MCCARTHY-LISP-PLATFORM-MATRIX.md).
