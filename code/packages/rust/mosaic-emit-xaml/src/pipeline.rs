...
fn emit_csproj(_name: &str, options: &EmitOptions) -> String {
    ...
    format!(
        "<Project Sdk=\"Microsoft.NET.Sdk\">\n\
         ...
         \n\
           <!-- dotnet publish does not carry the unpackaged WinUI app PRI or\n\
                generated XBF files from OutDir into PublishDir by default.\n\
                A publish can therefore build cleanly and then die during XAML\n\
                startup. Copy only those generated build outputs, preserving\n\
                AssemblyName overrides used for product release identities. -->\n\
           <Target Name=\"CopyWinUIResourcesToPublishDirectory\" AfterTargets=\"Publish\"\n\
                   Condition=\"'$(PublishDir)' != ''\">\n\
             <ItemGroup>\n\
               <_WinUIPublishResources Include=\"$(OutDir)$(AssemblyName).pri\"\n\
                                       Condition=\"Exists('$(OutDir)$(AssemblyName).pri')\" />\n\
               <_WinUIPublishXbf Include=\"$(OutDir)*.xbf\" />\n\
               <_WinUIPublishResources Include=\"@(_WinUIPublishXbf)\" />\n\
             </ItemGroup>\n\
             <Error Condition=\"!Exists('$(OutDir)$(AssemblyName).pri')\"\n\
                    Text=\"WinUI application PRI is missing from $(OutDir)\" />\n\
             <Error Condition=\"'@(_WinUIPublishXbf)' == ''\"\n\
                    Text=\"WinUI application XBF files are missing from $(OutDir)\" />\n\
             <Copy SourceFiles=\"@(_WinUIPublishResources)\" DestinationFolder=\"$(PublishDir)\"\n\
                   SkipUnchangedFiles=\"true\" />\n\
           </Target>\n\
         \n\
           <!-- The Mosaic host adapter carries an app-specific native DLL,\n\
                an FFI bridge generated by the app's Rust core, always named\n\
                mosaic_app.dll (mosaic-package-artifact-builder's\n\
                install_xaml_runtime_library validates this exact name before\n\
                ever placing the file, and unreachable!()s on anything else).\n\
                A package build with a bundled runtime library places it\n\
                beside this project file; copy it next to the executable so\n\
                DllImport/NativeLibrary can resolve it at launch.\n\
         \n\
                #12026: this used to glob `*.dll`, copying anything with that\n\
                extension that happened to be in the project directory next\n\
                to the built executable: a DLL planting primitive, since the\n\
                app already loads mosaic_app.dll from its own directory by\n\
                convention, so a planted file with that name would be loaded\n\
                directly. Narrowed to the one exact, known filename that's\n\
                ever legitimately placed there. Bundling a runtime library is\n\
                optional even with project emission on, so mosaic_app.dll may\n\
                legitimately not exist: Condition=\"Exists(...)\" on the\n\
                Target (not just the ItemGroup) skips the whole target\n\
                rather than letting <Copy> error on a missing source file,\n\
                which is what a literal (non-wildcard) Include would do. -->\n\
           <Target Name=\"CopyMosaicNativeHostLibraries\" AfterTargets=\"Build\"\n\
                   Condition=\"Exists('$(MSBuildProjectDirectory)\\mosaic_app.dll')\">\n\
             <ItemGroup>\n\
               <_MosaicNativeHostLibraries Include=\"$(MSBuildProjectDirectory)\\mosaic_app.dll\" />\n\
             </ItemGroup>\n\
...
mod tests {
    ...
    fn project_field_populated_when_emit_project_true() {
        ...
        assert!(p.csproj.contains("CopyWinUIResourcesToPublishDirectory"));
        assert!(p.csproj.contains("$(OutDir)$(AssemblyName).pri"));
        assert!(p.csproj.contains("<_WinUIPublishXbf Include=\"$(OutDir)*.xbf\""));
        assert!(p
            .csproj
            .contains("!Exists('$(OutDir)$(AssemblyName).pri')"));
        assert!(p.csproj.contains("'@(_WinUIPublishXbf)' == ''"));
        assert!(p.csproj.contains("DestinationFolder=\"$(PublishDir)\""));
        assert!(p.csproj.contains("CopyMosaicNativeHostLibraries"));
        // #12026: narrowed from a `*.dll` glob (a DLL-planting primitive --
        // anything dropped in the project directory got copied next to the
        // built executable) to the one exact, known runtime filename.
        // `FlattenNativeRuntimeDlls`'s own, separate, already-safe
        // `$(OutDir)runtimes/win-x64/native/*.dll` glob is unrelated to this
        // fix (a build-output path, not the source project directory) and
        // legitimately keeps its wildcard.
        assert!(p.csproj.contains("$(MSBuildProjectDirectory)\\mosaic_app.dll"));
        assert!(
            !p.csproj.contains("$(MSBuildProjectDirectory)\\*.dll"),
            "the broad *.dll glob over the project directory must not reappear:\n{}",
            p.csproj
        );
        assert!(p.csproj.contains("Condition=\"Exists('$(MSBuildProjectDirectory)\\mosaic_app.dll')\""));
