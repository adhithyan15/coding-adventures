...
The simulator always models the complete 64 KiB address space. Reads from
`$FF00-$FFEF` use 240 input latches and writes update 240 output latches,
matching the repository's Python reference. Public operations return typed
atomic errors, `step` returns full before/after state, bounded runs are
transactional on error, and snapshots own registers, flags, memory, control,
and I/O state.

All 256 first bytes are classified. The 151 official opcodes are pinned to a
full-state oracle generated by the Python implementation; the other 105 are
rejected without mutation. Core line coverage is 93.87% (704/750).
