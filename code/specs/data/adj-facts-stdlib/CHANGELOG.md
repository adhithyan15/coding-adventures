...
- **#14986: thirteen brain rows cited a page that never mentions them.**
  ...
  **21 of 21 mutants killed, one control.** All fifteen rows broken one at a time — nine of them share
  the brainstem sentence — plus warranting a brainstem row with the cerebrum sentence on the SEER page,
  repointing the hippocampus row at SEER, rebinding a function, dropping a row's locator, and
  fabricating the envelope.

  **One mutant survived first, and it was not a gap.** Dropping the HYPOTHALAMUS row's locator changed
  nothing: the envelope's locator is that row's own page, so the inherited value was already correct.
  The envelope's page is one of the three, so that mutant only bites on the other two — re-aimed at
  the cerebrum and hippocampus rows, both of which now die.
