...
% canonical-source-hash: fnv1a64:d3d04aa5ae46e30e
...
\hlblockfigure{\detokenize{figures/ZH-W07-zao-filmstrip.pdf}}{How \zh{早} is written, stroke by stroke}
...
1-4. \textbf{\zh{日}, complete.} Left side; top and right in one turning stroke; the middle bar; the bottom bar closing it.

\begin{enumerate}
\raggedright
  \item \textbf{\zh{十}'s horizontal}, left to right
  \item \textbf{\zh{十}'s vertical}, coming down \emph{through} that horizontal
\end{enumerate}
...
\end{tcolorbox}
\section[shàng]{\zh{上} — the same rule, read the other way}
...
\subsection*{Script — the shape}
\hlblockfigure{\detokenize{figures/ZH-W07-shang-filmstrip.pdf}}{How \zh{上} is written, stroke by stroke}
...
\begin{enumerate}
\raggedright
  \item the \textbf{central vertical}, from the top down
  \item the \textbf{short horizontal}, left to right, starting at the vertical
  \item the \textbf{long base horizontal}, left to right
\end{enumerate}

Two pen lifts.

\textbf{The vertical comes first}, and you wrote the opposite one lesson ago. Inside \zh{早}, the two strokes of \zh{十} went \textbf{horizontal first, then the vertical down through it}. Here the order is reversed.

The difference is what the two strokes do to each other. In \zh{十} they \textbf{cross}, and the horizontal leads. In \zh{上} the short bar \textbf{starts at} the vertical instead of crossing it — so there is nothing to cross, and the vertical goes first.

That is worth meeting on a three-stroke character, because it tells you something true about the system: these are \textbf{strong tendencies with conditions}, not laws. Learn the condition and you can predict the order instead of memorising it.
...
\begin{itemize}
\raggedright
  \item \emph{Write it:} \zh{上} — vertical first, even though your hand will want the short bar
...
\begin{quote}
Say \textbf{zǎo}. Low and dipping, and hold the bottom of it.
\end{quote}
...
Careful or slow speech sometimes restores the full falling tone, \emph{zǎoshàng}, and you will hear it. The standard reading, and the one to imitate, is the light one.
...
\begin{tcolorbox}[breakable,colback=teal!4,colframe=teal!35!black,title={Before you move on}]
Does \zh{上} keep its falling tone when it stands alone? (\textbf{Yes} — the light reading belongs to the word, not to the character.)
...
\begin{quote}
\zh{你} + \zh{好} $\to$ \textbf{\zh{你好}}, and \zh{早上} + \zh{好} $\to$ \textbf{\zh{早上好}}
\end{quote}
