...
- **Miri belongs on unsafe code, not the integration seam.** PR 7 moved `twig-vm` Miri off the per-PR critical path because every PR adds wallclock (PR 7 hit 1h30m+ on Linux CI even with the 90-min cap from PR 5).  The unsafe lives in `lang-runtime-core` + `lispy-runtime` (tagged-pointer scheme, heap accessors, FFI); `twig-vm` has *zero* unsafe.  Per-PR Miri now runs the two unsafe-bearing crates as **blocking** (~5 min total).  `twig-vm` Miri runs as `continue-on-error: true` informational + a nightly cron (`lang-runtime-safety-nightly.yml`).  Engineers run `scripts/miri-twig-vm.sh` locally before pushing twig-vm changes — that's the canonical verification.  Lesson: when Miri wallclock exceeds the per-PR budget, *split it by where the unsafe lives*, not by tightening the cap further.
