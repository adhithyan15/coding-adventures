...
>     - **L2c-3b** — *free-variable capture.*  A lambda lifts with an extra environment parameter; free variables in its body are captured (by value) into the closure's `env` at creation and bound from `env` on entry.  `LABEL`-as-value (recursive closures) lands here too.
...
| **L2c-3b** | Closures, part 2 — free-variable capture (env threading) + `LABEL`-as-value (recursive closures). |
