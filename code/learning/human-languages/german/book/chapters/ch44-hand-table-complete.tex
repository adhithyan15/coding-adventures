...
% canonical-source-hash: fnv1a64:5bafea439f73d264
% canonical-lessons: GE-C31-arm, GE-C31-finger, GE-C31-fuss, GE-C31-herz, GE-R31-zuletzt-einmal-gesehen
...
\end{tcolorbox}
\section[Practice]{Herz — and the last few this book showed you exactly once}

\label{lesson:GE-R31-zuletzt-einmal-gesehen}



\begin{quote}
The heart. Say it, and let the \textbf{z} land as \emph{ts} rather than as an English \emph{z}.
\end{quote}
...
\textbf{Herz} and English \textbf{heart} are the same word, separated by one regular sound change: the \textbf{k} of the older form became \textbf{h} in the Germanic line, the same swap already heard between \emph{hören} and \emph{hear}, and between \emph{Hund} and \emph{hound}. Once you have seen it three times it stops being a curiosity and becomes something you can use to guess.

\textbf{Haupt} and \textbf{head} are that same pair — and this is the one place the guess would mislead you, because the word it predicts is not the word Germans say. For the thing on your shoulders they reach for the old cup-word instead. A sound law tells you which words are cousins. It does not tell you which cousin people chose.
\end{tcolorbox}

\subsection*{Your turn: the apology, and the two warm thank-yous}
Say these aloud:

\begin{itemize}
\raggedright
  \item bump into someone — \emph{Entschuldigung}
  \item stop a stranger to ask something — and notice it is the same word
  \item the thank-you with the adjective — \emph{danke schön}
  \item the warmest everyday one — \emph{vielen Dank}
\end{itemize}

English splits \emph{sorry} from \emph{excuse me}; German does not, and your voice carries the difference the vocabulary declines to.

\begin{tcolorbox}[breakable,colback=teal!4,colframe=teal!35!black,title={Before you move on}]
What sound does \textbf{z} make in \emph{Herz}? (\emph{\textbf{ts}}.) Which sound change links \emph{Herz} to \emph{heart}? (\textbf{k becoming h} in the Germanic line.) Does German use one word or two for \emph{sorry} and \emph{excuse me}? (\textbf{One.})
\end{tcolorbox}
