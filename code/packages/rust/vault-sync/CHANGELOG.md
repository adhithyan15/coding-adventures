...
- `SyncStateSummary` and `InMemorySyncServer::state_summary()`
  for redacted aggregate diagnostics over canonical sync state,
  including per-namespace counts, writer/device counts, wrapped
  record counts, byte totals, and the latest writer timestamp
  without exposing ciphertext, wrap-set bytes, or record keys.
