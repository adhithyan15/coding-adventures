...
- Initial pure-ISO C17 port of the Rust `argon2id` crate (RFC 9106): the
  hybrid (data-independent then data-dependent) memory-hard password-hashing function.
- `argon2id(...)` writing the tag into a caller-provided buffer with an
  `Argon2idStatus` result; full parameter validation matching the Rust variants.
- Argon2 G compression, permutation P, the H' variable-length BLAKE2b extender,
  the RFC 9106 §3.4 `index_alpha` reference mapping, and hybrid segment
  fill.
