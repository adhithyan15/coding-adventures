...
describe("corpus snapshot", () => {
  ...
  it("pins where the corpus actually stands on the ladder", () => {
    ...
    expect(summary.byLevel["pre-A1"]).toBe(896); // +3: HL-C97 adds the repair kit (no entiendo, mas despacio) at chapter 14 // +4: HL-C98
    ...
    expect(summary.byLevel.A1).toBe(315); // +1: ES-C02-concordancia sits on SPINE-TIME-OF-DAY
    ...
    expect(summary.byLevel.A2).toBe(428); // +39: Spanish chapters 11-18 plus prerequisite closure
  ...
  it("can already build a ramp-to-A1 edition from the canonical corpus", () => {
    ...
    expect(ramp).toHaveLength(1211); // +3: HL-C97 adds the repair kit (no entiendo, mas despacio) at chapter 14
