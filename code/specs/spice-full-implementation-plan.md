...
2. Rust JFET model-card parser parity audit.
   - Status: in progress; threshold and channel-length-modulation aliases are
...
4. Rust JFET capacitance-alias parser parity.
   - Status: prioritized next after the channel-length-modulation slice merges.
   - The engine and Python/TypeScript facades normalize `CGS0` / `CGD0` into
     canonical JFET gate-source / gate-drain capacitances; confirm Rust
     lowering and finite/non-negative validation, then implement the smallest
