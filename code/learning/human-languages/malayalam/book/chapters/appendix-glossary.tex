% GENERATED FILE. Edit canonical lesson frontmatter, then run npm run generate:books.
% canonical-entries: 213

\chapter*{Glossary}
\addcontentsline{toc}{chapter}{Glossary}
\markboth{Glossary}{Glossary}

This glossary collects every word and phrase taught in the book. Pronunciation appears when it differs from the written form; chapter numbers show where each entry is introduced.

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{-\ml{ിക്ക്}}\enspace\emph{-ikku}\par
\small the dative suffix -ikku, 'to / for' — and the idea of stacking\par
\footnotesize Introduced in Chapter 6.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{aaru ezhu ettu ompathu pathu}\par
\small hear and say six to ten before meeting their written forms\par
\footnotesize Introduced in Chapter 7.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{അധ്യാപകൻ}}\enspace\emph{adhyāpakan}\par
\small a teacher\par
\footnotesize Introduced in Chapter 48.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ആകാശം}}\enspace\emph{ākāśaṁ}\par
\small the sky\par
\footnotesize Introduced in Chapter 53.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ആന}}\enspace\emph{āna}\par
\small an elephant\par
\footnotesize Introduced in Chapter 60.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{അങ്ങനെ}}\enspace\emph{aṅṅane}\par
\small that way, just so\par
\footnotesize Introduced in Chapter 49.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{അനുഗ്രഹം}}\enspace\emph{anugrahaṁ}\par
\small a blessing\par
\footnotesize Introduced in Chapter 51.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{അനുവാദം}}\enspace\emph{anuvādaṁ}\par
\small permission\par
\footnotesize Introduced in Chapter 65.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{അപേക്ഷ}}\enspace\emph{apēkṣa}\par
\small a request\par
\footnotesize Introduced in Chapter 65.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{അറിയുക}}\enspace\emph{aṟiyuka}\par
\small to know — the verb inside \ml{എനിക്ക്} \ml{മലയാളം} \ml{അറിയാം}, and the one that will not let you be the subject\par
\footnotesize Introduced in Chapter 32.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ആര്}}\enspace\emph{ārŭ}\par
\small who? — asking about a person\par
\footnotesize Introduced in Chapter 41.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{അതിഥി}}\enspace\emph{atithi}\par
\small a guest\par
\footnotesize Introduced in Chapter 48.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{അത്}}\enspace\emph{atŭ}\par
\small that one — the thing over there\par
\footnotesize Introduced in Chapter 41.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ആട്}}\enspace\emph{āṭŭ}\par
\small a goat\par
\footnotesize Introduced in Chapter 60.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{അവിടെ}}\enspace\emph{aviṭe}\par
\small there — where I am not\par
\footnotesize Introduced in Chapter 41.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ബഹുമാനം}}\enspace\emph{bahumānaṁ}\par
\small respect\par
\footnotesize Introduced in Chapter 51.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ഭർത്താവ്}}\enspace\emph{bharttāvŭ}\par
\small husband\par
\footnotesize Introduced in Chapter 45.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ഭാര്യ}}\enspace\emph{bhārya}\par
\small wife\par
\footnotesize Introduced in Chapter 45.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ചന്ദ്രൻ}}\enspace\emph{candran}\par
\small the moon\par
\footnotesize Introduced in Chapter 53.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ചെളി}}\enspace\emph{ceḷi}\par
\small mud\par
\footnotesize Introduced in Chapter 59.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ചെറിയ}}\enspace\emph{ceṟiya}\par
\small small\par
\footnotesize Introduced in Chapter 42.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ചെവി}}\enspace\emph{cevi}\par
\small ear — where three of the four sisters agree, and the fourth alone keeps the older sound\par
\footnotesize Introduced in Chapter 37.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ചായ}}\enspace\emph{chaaya}\par
\small tea — Chinese, carried overland through Persian and Hindi before it ever reached Kerala\par
\footnotesize Introduced in Chapter 39.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ചിലപ്പോൾ}}\enspace\emph{cilappōḷ}\par
\small sometimes\par
\footnotesize Introduced in Chapter 64.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ചിന്തിക്കുക}}\enspace\emph{cintikkuka}\par
\small to think — a Sanskrit noun turned into a verb by the same ‑\ml{ഇക്കുക} that made \ml{ക്ഷമിക്കുക}, and the first of four verbs about what happens inside a head\par
\footnotesize Introduced in Chapter 33.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ചോദിക്കുക}}\enspace\emph{cōdikkuka}\par
\small to ask — a Sanskrit word for pushing that walked into a gap Malayalam had made by narrowing its own inherited verb to \textquotedblleft{}hear\textquotedblright{}\par
\footnotesize Introduced in Chapter 34.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ചൂല്}}\enspace\emph{cūlŭ}\par
\small a broom\par
\footnotesize Introduced in Chapter 62.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ചുമ}}\enspace\emph{cuma}\par
\small a cough\par
\footnotesize Introduced in Chapter 63.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ചുണ്ട്}}\enspace\emph{cuṇṭŭ}\par
\small a lip\par
\footnotesize Introduced in Chapter 57.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ദയവായി}}\enspace\emph{dayavāyi}\par
\small please (dayavāyi — \textquotedblleft{}as a compassion,\textquotedblright{} from \ml{ദയ} daya \textquotedblleft{}compassion\textquotedblright{})\par
\footnotesize Introduced in Chapter 8.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ദിവസം}}\enspace\emph{divasam}\par
\small \textquotedblleft{}day\textquotedblright{} — everyday Sanskrit divasam beside the more literary Sanskrit dinam\par
\footnotesize Introduced in Chapter 23.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{എല്ല്}}\enspace\emph{ellŭ}\par
\small a bone\par
\footnotesize Introduced in Chapter 57.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{എഴുതൂ}}\enspace\emph{eḻutū}\par
\small write\par
\footnotesize Introduced in Chapter 44.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{എഴുതുക}}\enspace\emph{eḻutuka}\par
\small to write — the same word as Tamil \ta{எழுது}, carrying the \ml{ഴ} that only these two sisters kept, and the chapter's closing count of four\par
\footnotesize Introduced in Chapter 33.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{എനിക്ക്} \ml{മലയാളം} \ml{അറിയാം}}\enspace\emph{enikku malayāḷam aṟiyām}\par
\small 'I know Malayalam' — built with no subject, because knowing happens TO you\par
\footnotesize Introduced in Chapter 6.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{എനിക്ക്} \ml{മലയാളം} \ml{ഇഷ്ടമാണ്}}\enspace\emph{enikku malayāḷaṁ iṣṭamāṇŭ}\par
\small 'I like Malayalam' — a noun, a copula, and a dative; the one sentence in this book where putting yourself in the subject slot is not merely odd but impossible\par
\footnotesize Introduced in Chapter 34.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{എണ്ണ}}\enspace\emph{eṇṇa}\par
\small oil\par
\footnotesize Introduced in Chapter 61.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{എന്നാൽ}}\enspace\emph{ennāl}\par
\small but, on the other hand\par
\footnotesize Introduced in Chapter 64.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{എങ്ങനെ}}\enspace\emph{eṅṅane}\par
\small how\par
\footnotesize Introduced in Chapter 3.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{എന്റെ}}\enspace\emph{enṟe}\par
\small my\par
\footnotesize Introduced in Chapter 2.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{എടുക്കുക}}\enspace\emph{eṭukkuka}\par
\small to take — native to the bone, yet ending in ‑\ml{ക്കുക} like the borrowings; the letter that actually does the stamping is the \ml{ഇ}\par
\footnotesize Introduced in Chapter 34.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{എവിടെ}}\enspace\emph{eviṭe}\par
\small where? — asking about a place\par
\footnotesize Introduced in Chapter 41.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ഹൃദയം}}\enspace\emph{hṛdayaṁ}\par
\small heart — Sanskrit tatsama, and a genuine (if distant) cousin of the English word\par
\footnotesize Introduced in Chapter 38.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ഇല്ല}}\enspace\emph{illa}\par
\small no / there isn't (illa — the negative)\par
\footnotesize Introduced in Chapter 1.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ഇപ്പോൾ}}\enspace\emph{ippōḷ}\par
\small now\par
\footnotesize Introduced in Chapter 50.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ഇത്}}\enspace\emph{itŭ}\par
\small this one — the thing near me\par
\footnotesize Introduced in Chapter 41.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ഇവിടെ}}\enspace\emph{iviṭe}\par
\small here — where I am\par
\footnotesize Introduced in Chapter 41.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ജോലി} \ml{ചെയ്യുക}}\enspace\emph{jōli ceyyuka}\par
\small to work (lit. \textquotedblleft{}work-do\textquotedblright{})\par
\footnotesize Introduced in Chapter 5.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കാപ്പി}}\enspace\emph{kaapi}\par
\small coffee — an English colonial-era loan, itself from Arabic by way of Turkish and Italian; tea and coffee arrived by opposite roads\par
\footnotesize Introduced in Chapter 39.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കാക്ക}}\enspace\emph{kākka}\par
\small a crow\par
\footnotesize Introduced in Chapter 60.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കാൽ}}\enspace\emph{kāl}\par
\small leg, foot\par
\footnotesize Introduced in Chapter 47.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കലപ്പ}}\enspace\emph{kalappa}\par
\small a plough\par
\footnotesize Introduced in Chapter 66.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കാലാവസ്ഥ}}\enspace\emph{kālāvastha}\par
\small weather — a Sanskrit compound literally meaning \textquotedblleft{}state of TIME\textquotedblright{}: kaala (\textquotedblleft{}time\textquotedblright{}) + avastha (\textquotedblleft{}state, condition\textquotedblright{}) — a real echo of Spanish's tiempo meaning both \textquotedblleft{}time\textquotedblright{} AND \textquotedblleft{}weather\textquotedblright{}\par
\footnotesize Introduced in Chapter 20.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കഴുത്ത്}}\enspace\emph{kaḻuttŭ}\par
\small the neck\par
\footnotesize Introduced in Chapter 57.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കനൽ}}\enspace\emph{kanal}\par
\small an ember, a live coal\par
\footnotesize Introduced in Chapter 59.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കണ്ണ്}}\enspace\emph{kaṇṇŭ}\par
\small eye — Proto-Dravidian, nearly identical across all four literary Dravidian languages\par
\footnotesize Introduced in Chapter 37.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കാണുക}}\enspace\emph{kāṇuka}\par
\small to see — the family's shared see-root, kept as Malayalam's everyday word, and the mood endings that share one slot with tense\par
\footnotesize Introduced in Chapter 32.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കാറ്റ്}}\enspace\emph{kāṟṟŭ}\par
\small wind\par
\footnotesize Introduced in Chapter 59.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കർഷകൻ}}\enspace\emph{karṣakan}\par
\small a farmer\par
\footnotesize Introduced in Chapter 48.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കസേര}}\enspace\emph{kasēra}\par
\small a chair\par
\footnotesize Introduced in Chapter 52.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കതിര്}}\enspace\emph{katirŭ}\par
\small an ear of ripe paddy\par
\footnotesize Introduced in Chapter 66.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കത്തി}}\enspace\emph{katti}\par
\small a knife\par
\footnotesize Introduced in Chapter 56.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കയർ}}\enspace\emph{kayaṟ}\par
\small coir rope\par
\footnotesize Introduced in Chapter 62.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കേൾക്കൂ}}\enspace\emph{kēḷkkū}\par
\small listen\par
\footnotesize Introduced in Chapter 44.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കോലം}}\enspace\emph{kōlaṁ}\par
\small the pattern drawn at a doorway\par
\footnotesize Introduced in Chapter 52.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കോഴി}}\enspace\emph{kōḻi}\par
\small a hen\par
\footnotesize Introduced in Chapter 60.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കൊമ്പ്}}\enspace\emph{kompŭ}\par
\small a branch\par
\footnotesize Introduced in Chapter 54.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കൊട്ട}}\enspace\emph{koṭṭa}\par
\small a basket\par
\footnotesize Introduced in Chapter 56.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കൊയ്ത്ത്}}\enspace\emph{koyttŭ}\par
\small the harvest, the reaping\par
\footnotesize Introduced in Chapter 66.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കൃതജ്ഞത}}\enspace\emph{kṛtajñata}\par
\small gratitude\par
\footnotesize Introduced in Chapter 51.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ക്ഷമിക്കണം}}\enspace\emph{kṣamikkaṇaṁ}\par
\small (you) should forgive / sorry (kṣamikkaṇaṁ — from kṣamikkuka \textquotedblleft{}to forgive,\textquotedblright{} Sanskrit kṣama + necessitative -aṇaṁ)\par
\footnotesize Introduced in Chapter 9.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ക്ഷീണം}}\enspace\emph{kṣīṇaṁ}\par
\small tiredness\par
\footnotesize Introduced in Chapter 63.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കുറച്ച്}}\enspace\emph{kuṟaccŭ}\par
\small a little, a few\par
\footnotesize Introduced in Chapter 58.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കുറവ്}}\enspace\emph{kuṟavŭ}\par
\small less, a shortage\par
\footnotesize Introduced in Chapter 58.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കുട്ടി}}\enspace\emph{kuṭṭi}\par
\small child, kid — the general word, native Dravidian, before Malayalam narrows to son or daughter\par
\footnotesize Introduced in Chapter 36.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കുടുംബം}}\enspace\emph{kuṭumbaṁ}\par
\small family — the collective word Chapter 12's six people-words never needed, because each of them already named one specific person\par
\footnotesize Introduced in Chapter 35.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കൂടുതൽ}}\enspace\emph{kūṭutal}\par
\small more\par
\footnotesize Introduced in Chapter 58.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{മകൾ}}\enspace\emph{makaḷ}\par
\small daughter — identical to Tamil's own word, sound for sound, closer even than the son-word\par
\footnotesize Introduced in Chapter 36.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{മകൻ}}\enspace\emph{makan}\par
\small son — Proto-Dravidian, closely matching Tamil, where Kannada softened the same root\par
\footnotesize Introduced in Chapter 36.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{മാല}}\enspace\emph{māla}\par
\small a garland\par
\footnotesize Introduced in Chapter 52.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{മണൽ}}\enspace\emph{maṇal}\par
\small sand\par
\footnotesize Introduced in Chapter 59.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{മനസ്സിലാക്കുക}}\enspace\emph{manassilākkuka}\par
\small to understand — literally \textquotedblleft{}to put it in the mind,\textquotedblright{} a three-piece word you can take apart by ear, with a second form that hands the understanding to you instead\par
\footnotesize Introduced in Chapter 33.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{മരം}}\enspace\emph{maraṁ}\par
\small a tree\par
\footnotesize Introduced in Chapter 54.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{മറ്റന്നാൾ}}\enspace\emph{maṟṟannāḷ}\par
\small the day after tomorrow\par
\footnotesize Introduced in Chapter 50.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{മര്യാദ}}\enspace\emph{maryāda}\par
\small good manners\par
\footnotesize Introduced in Chapter 65.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{മതി}}\enspace\emph{mati}\par
\small enough, that will do\par
\footnotesize Introduced in Chapter 49.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{മാത്രം}}\enspace\emph{mātraṁ}\par
\small only, that much and no more\par
\footnotesize Introduced in Chapter 64.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{മൂക്ക്}}\enspace\emph{mūkkŭ}\par
\small nose — native Dravidian, a fourth near-perfect match across the family\par
\footnotesize Introduced in Chapter 37.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{മുളക്}}\enspace\emph{muḷakŭ}\par
\small a chilli\par
\footnotesize Introduced in Chapter 61.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{മുറം}}\enspace\emph{muṟaṁ}\par
\small a winnowing tray\par
\footnotesize Introduced in Chapter 62.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{മുടി}}\enspace\emph{muṭi}\par
\small hair\par
\footnotesize Introduced in Chapter 47.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{മുത്തച്ഛൻ}}\enspace\emph{muttacchan}\par
\small grandfather\par
\footnotesize Introduced in Chapter 45.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{മുത്തശ്ശി}}\enspace\emph{muttaśśi}\par
\small grandmother\par
\footnotesize Introduced in Chapter 45.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{മുതുക്}}\enspace\emph{mutukŭ}\par
\small the back\par
\footnotesize Introduced in Chapter 57.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{നഖം}}\enspace\emph{nakhaṁ}\par
\small a nail, a claw\par
\footnotesize Introduced in Chapter 57.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{നക്ഷത്രം}}\enspace\emph{nakṣatraṁ}\par
\small a star\par
\footnotesize Introduced in Chapter 53.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{നാളെ} \ml{കാണാം}}\enspace\emph{nāḷe kāṇāṁ}\par
\small see you tomorrow\par
\footnotesize Introduced in Chapter 4.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{നല്ല}}\enspace\emph{nalla}\par
\small good\par
\footnotesize Introduced in Chapter 42.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{നമസ്കാരം}}\enspace\emph{namaskāram}\par
\small hello / greetings (namaskāram — a making of a bow)\par
\footnotesize Introduced in Chapter 1.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{നന്ദി}}\enspace\emph{nandi}\par
\small thank you (nandi — the native Dravidian word, Tamil's cousin)\par
\footnotesize Introduced in Chapter 1.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{നാട്}}\enspace\emph{nāṭŭ}\par
\small home country, the place you are from\par
\footnotesize Introduced in Chapter 55.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{നെല്ല്}}\enspace\emph{nellŭ}\par
\small unhusked rice, rice still in the husk\par
\footnotesize Introduced in Chapter 66.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{നെഞ്ച്}}\enspace\emph{nenchŭ}\par
\small chest — Malayalam's other heart, native this time, and the one everyday idiom reaches for\par
\footnotesize Introduced in Chapter 38.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{നിൽക്കൂ}}\enspace\emph{nilkkū}\par
\small stand\par
\footnotesize Introduced in Chapter 43.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{നോക്കൂ}}\enspace\emph{nōkkū}\par
\small look\par
\footnotesize Introduced in Chapter 44.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{നൂല്}}\enspace\emph{nūlŭ}\par
\small thread\par
\footnotesize Introduced in Chapter 62.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{onnu randu moonnu naalu anchu}\par
\small hear and say one to five before meeting their written forms\par
\footnotesize Introduced in Chapter 7.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ഒരുപക്ഷേ}}\enspace\emph{orupakṣē}\par
\small perhaps\par
\footnotesize Introduced in Chapter 49.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ഓടൂ}}\enspace\emph{ōṭū}\par
\small run\par
\footnotesize Introduced in Chapter 43.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പാൽ}}\enspace\emph{paal}\par
\small milk — native, matching Tamil exactly, and matching Kannada by the family's regular p-to-h shift\par
\footnotesize Introduced in Chapter 39.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പഴം}}\enspace\emph{paḻaṁ}\par
\small a fruit\par
\footnotesize Introduced in Chapter 46.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പഴയ}}\enspace\emph{paḻaya}\par
\small old — of a thing, not of a person\par
\footnotesize Introduced in Chapter 42.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പല്ല്}}\enspace\emph{pallŭ}\par
\small a tooth\par
\footnotesize Introduced in Chapter 47.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പനി}}\enspace\emph{pani}\par
\small a fever\par
\footnotesize Introduced in Chapter 63.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പാറ}}\enspace\emph{pāṟa}\par
\small a rock\par
\footnotesize Introduced in Chapter 55.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പശു}}\enspace\emph{paśu}\par
\small a cow\par
\footnotesize Introduced in Chapter 60.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പാത്രം}}\enspace\emph{pātraṁ}\par
\small a vessel, a container\par
\footnotesize Introduced in Chapter 56.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പായ}}\enspace\emph{pāya}\par
\small a mat\par
\footnotesize Introduced in Chapter 56.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പേര്}}\enspace\emph{pēr}\par
\small name\par
\footnotesize Introduced in Chapter 2.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പെട്ടി}}\enspace\emph{peṭṭi}\par
\small a box\par
\footnotesize Introduced in Chapter 56.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പിന്നെ}}\enspace\emph{pinne}\par
\small then, after that\par
\footnotesize Introduced in Chapter 64.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പോകുക}}\enspace\emph{pōkuka}\par
\small to go — one stem, three tense endings, and each of the three is the entire conjugation\par
\footnotesize Introduced in Chapter 32.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പോരും}}\enspace\emph{pōruṁ}\par
\small that will do, that is enough\par
\footnotesize Introduced in Chapter 58.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പോയി} \ml{വരാം}}\enspace\emph{pōyi varāṁ}\par
\small goodbye (lit. \textquotedblleft{}I'll go and come back\textquotedblright{})\par
\footnotesize Introduced in Chapter 4.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പുക}}\enspace\emph{puka}\par
\small smoke\par
\footnotesize Introduced in Chapter 59.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പുഴ}}\enspace\emph{puḻa}\par
\small a river\par
\footnotesize Introduced in Chapter 55.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പുല്ല്}}\enspace\emph{pullŭ}\par
\small grass\par
\footnotesize Introduced in Chapter 66.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പുറപ്പെടുക}}\enspace\emph{puṟappeṭuka}\par
\small to set out\par
\footnotesize Introduced in Chapter 50.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പുസ്തകം}}\enspace\emph{pustakaṁ}\par
\small a book\par
\footnotesize Introduced in Chapter 46.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പുതിയ}}\enspace\emph{putiya}\par
\small new\par
\footnotesize Introduced in Chapter 42.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പൂവ്}}\enspace\emph{pūvŭ}\par
\small a flower\par
\footnotesize Introduced in Chapter 52.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{രാവിലെ}}\enspace\emph{rāvile}\par
\small \textquotedblleft{}morning\textquotedblright{} (rāvile) — a compound of \ml{രാവ്} (rāvŭ, \textquotedblleft{}night,\textquotedblright{} a cognate doublet of \ml{ഇരവ്}/iravŭ already met in ML-C24 — same root, not literally the same word) + locative-ish suffixes -il/-e; literally built from NIGHT, not light or rising — a genuinely different strategy than Kannada's \textquotedblleft{}the shining\textquotedblright{} or Telugu's Sanskrit \textquotedblleft{}rise\textquotedblright{}; \ml{പ്രഭാതം} (Sanskrit tatsama, the same prabhāta root Telugu draws on) and \ml{പുലർച്ച} (native, thinner etymology) also exist as alternatives\par
\footnotesize Introduced in Chapter 26.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{സഹായിക്കുക}}\enspace\emph{sahāyikkuka}\par
\small to help — \textquotedblleft{}to go along with somebody,\textquotedblright{} and the fourth time in two chapters that a Sanskrit word has taken an everyday job from an inherited one\par
\footnotesize Introduced in Chapter 34.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{സമ്മതം}}\enspace\emph{sammataṁ}\par
\small consent, an agreeing\par
\footnotesize Introduced in Chapter 65.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{സംസാരിക്കൂ}}\enspace\emph{saṁsārikkū}\par
\small speak\par
\footnotesize Introduced in Chapter 44.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{സന്തോഷം}}\enspace\emph{santōṣam}\par
\small joy / pleased to meet you\par
\footnotesize Introduced in Chapter 2.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{സാരമില്ല}}\enspace\emph{sāramilla}\par
\small it doesn't matter / no problem / you're welcome\par
\footnotesize Introduced in Chapter 3.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ശരി}}\enspace\emph{sari}\par
\small okay / alright / correct (śari)\par
\footnotesize Introduced in Chapter 1.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ശർക്കര}}\enspace\emph{śarkkara}\par
\small jaggery\par
\footnotesize Introduced in Chapter 61.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{സത്യം}}\enspace\emph{satyaṁ}\par
\small true, the truth\par
\footnotesize Introduced in Chapter 49.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ശുഭ} \ml{മധ്യാഹ്നം}}\enspace\emph{śubha madhyāhnaṁ}\par
\small \textquotedblleft{}good afternoon\textquotedblright{} (śubha madhyāhnaṁ), a formal greeting built on a noon synonym rather than the everyday afternoon compound\par
\footnotesize Introduced in Chapter 31.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ശുഭ} \ml{സായാഹ്നം}}\enspace\emph{śubha sāyāhnaṁ}\par
\small \textquotedblleft{}good evening\textquotedblright{} (śubha sāyāhnaṁ) — confirmed real, used \textquotedblleft{}from late afternoon until sunset\textquotedblright{} per a directly-fetched source; built on \ml{ശുഭ} (ML-C25) + \ml{സായാഹ്നം} (sāyāhnaṁ), a SEPARATE Sanskrit tatsama for \textquotedblleft{}evening,\textquotedblright{} Wiktionary-labeled \textquotedblleft{}poetic\textquotedblright{} — NOT built on ML-C27's native \ml{വൈകുന്നേരം}; \ml{സായാഹ്നം}'s own root, \dv{साय} (\textquotedblleft{}end\textquotedblright{}), is CONFIRMED via Wiktionary's separate \dv{साय} entry to trace to the same PIE \emph{seh\textsubscript{1}- as Telugu's \te{సాయంత్రం} (sāyam) and Latin sērus — a properly-verified cognate, not just a plausible guess}\par
\footnotesize Introduced in Chapter 30.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{സൂചി}}\enspace\emph{sūci}\par
\small a needle\par
\footnotesize Introduced in Chapter 62.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{സുഹൃത്ത്}}\enspace\emph{suhṛttŭ}\par
\small friend — Sanskrit \textquotedblleft{}good-heart,\textquotedblright{} a root this chapter's own heart-word will meet again\par
\footnotesize Introduced in Chapter 35.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{സുഖം}}\enspace\emph{sukhaṁ}\par
\small well-being — and the reply \textquotedblleft{}\ml{സുഖമാണ്}\textquotedblright{}\par
\footnotesize Introduced in Chapter 3.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{സുപ്രഭാതം}}\enspace\emph{suprabhātaṁ}\par
\small \textquotedblleft{}good morning\textquotedblright{} (suprabhātaṁ) — Malayalam's actual, most common morning greeting, confirmed used in BOTH formal and informal contexts, and independently corroborated as the \textquotedblleft{}most general\textquotedblright{} morning greeting by a SECOND directly-fetched source (not just one); built from su- (\textquotedblleft{}good\textquotedblright{}) + \ml{പ്രഭാതം} (already met, ML-C26, \textquotedblleft{}morning/dawn\textquotedblright{}), NOT from \ml{ശുഭ} (ML-C25's \textquotedblleft{}auspicious,\textquotedblright{} used for Malayalam's own GREETING-GOODNIGHT) — a genuinely different Sanskrit \textquotedblleft{}good\textquotedblright{} morpheme; the SAME word as Telugu's \te{సుప్రభాతం}, but with an inverted role: PRIMARY greeting in Malayalam, only a SECONDARY alternative in Telugu (where \te{శుభోదయం}, a different root, is primary)\par
\footnotesize Introduced in Chapter 29.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{സൂര്യൻ}}\enspace\emph{sūryan}\par
\small the sun\par
\footnotesize Introduced in Chapter 53.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{തൈര്}}\enspace\emph{tairŭ}\par
\small curd\par
\footnotesize Introduced in Chapter 61.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{താമസിക്കുക}}\enspace\emph{tāmasikkuka}\par
\small to live, to stay\par
\footnotesize Introduced in Chapter 5.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{തരൂ}}\enspace\emph{tarū}\par
\small give\par
\footnotesize Introduced in Chapter 43.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{തടി}}\enspace\emph{taṭi}\par
\small a trunk, timber\par
\footnotesize Introduced in Chapter 54.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{തേങ്ങ}}\enspace\emph{tēṅṅa}\par
\small a coconut\par
\footnotesize Introduced in Chapter 61.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{തിന്നുക}}\enspace\emph{tinnuka}\par
\small to eat — the inherited Dravidian eat-word Malayalam kept as its everyday one, and the ending that tells a native verb from a borrowed one\par
\footnotesize Introduced in Chapter 32.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{തീർച്ചയായും}}\enspace\emph{tīrccayāyuṁ}\par
\small certainly, for sure\par
\footnotesize Introduced in Chapter 49.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{തുണി}}\enspace\emph{tuṇi}\par
\small cloth\par
\footnotesize Introduced in Chapter 46.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ഉച്ച}}\enspace\emph{ucca}\par
\small noon, most likely from a different Sanskrit metaphor than Kannada and Telugu — height rather than middle\par
\footnotesize Introduced in Chapter 17.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ഊണ്}}\enspace\emph{ūṇ}\par
\small a meal — the noun behind \ml{ഉണ്ണുക}, Malayalam's second eat-verb, standing beside Chapter 32's everyday \ml{തിന്നുക}\par
\footnotesize Introduced in Chapter 40.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ഉണ്ട്}}\enspace\emph{uṇṭŭ}\par
\small there is, there are, {[}someone{]} has — and \ml{ഇരിക്കുക}, to be somewhere; the two-piece machine every Malayalam verb is built on\par
\footnotesize Introduced in Chapter 32.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ഉപകാരം}}\enspace\emph{upakāraṁ}\par
\small a favour, a good turn\par
\footnotesize Introduced in Chapter 51.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ഉപ്പ്}}\enspace\emph{uppŭ}\par
\small salt\par
\footnotesize Introduced in Chapter 46.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ഉറങ്ങൂ}}\enspace\emph{uṟaṅṅū}\par
\small sleep\par
\footnotesize Introduced in Chapter 43.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ഉടനെ}}\enspace\emph{uṭane}\par
\small soon, right away\par
\footnotesize Introduced in Chapter 64.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{വൈദ്യൻ}}\enspace\emph{vaidyan}\par
\small a doctor\par
\footnotesize Introduced in Chapter 48.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{വൈകുന്നേരം}}\enspace\emph{vaikunnēraṁ}\par
\small \textquotedblleft{}evening\textquotedblright{} (vaikunnēraṁ) — native Dravidian compound, Wiktionary-glossed \textquotedblleft{}latening time,\textquotedblright{} from \ml{വൈക്} (vaikŭ, plausibly the root of \ml{വൈകുക}, \textquotedblleft{}to get late\textquotedblright{} — a reasonable inference, not something Wiktionary's compound entry states outright) + -\ml{ഉം} (-uṁ) + \ml{നേരം} (nēraṁ, \textquotedblleft{}time,\textquotedblright{} itself native Dravidian, not Sanskrit); Wiktionary itself ALSO lists \textquotedblleft{}afternoon\textquotedblright{} as a second sense, though phrasebook sources cleanly split evening (this word) from afternoon (a separate compound built on \ml{ഉച്ച}, ML-C17's \textquotedblleft{}noon,\textquotedblright{} plus \ml{കഴിഞ്ഞ്}) — an honest source-tension worth flagging, not smoothing over\par
\footnotesize Introduced in Chapter 27.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{വഴി}}\enspace\emph{vaḻi}\par
\small a way, a path\par
\footnotesize Introduced in Chapter 55.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{വലിയ}}\enspace\emph{valiya}\par
\small big\par
\footnotesize Introduced in Chapter 42.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{വന്ദനം}}\enspace\emph{vandanaṁ}\par
\small a salutation\par
\footnotesize Introduced in Chapter 51.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{വരുക}}\enspace\emph{varuka}\par
\small to come — and the past tense, the one slot where a Malayalam verb can still surprise you\par
\footnotesize Introduced in Chapter 32.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{വാതിൽ}}\enspace\emph{vātil}\par
\small a door\par
\footnotesize Introduced in Chapter 52.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{വായ്}}\enspace\emph{vāy}\par
\small mouth — the word Chapter 33 already named, only to rule it out\par
\footnotesize Introduced in Chapter 37.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{വയൽ}}\enspace\emph{vayal}\par
\small a paddy field\par
\footnotesize Introduced in Chapter 55.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{വയറ്}}\enspace\emph{vayaṟŭ}\par
\small the stomach, the belly\par
\footnotesize Introduced in Chapter 47.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{വായിക്കുക}}\enspace\emph{vāyikkuka}\par
\small to read — a word that looks as though it were built on \ml{വായ്}, \textquotedblleft{}mouth,\textquotedblright{} and is not; the trail runs to Sanskrit \ml{വാച്}, \textquotedblleft{}speech\textquotedblright{}\par
\footnotesize Introduced in Chapter 33.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{വേദന}}\enspace\emph{vēdana}\par
\small pain\par
\footnotesize Introduced in Chapter 63.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{വേണ്ട}}\enspace\emph{vēṇṭa}\par
\small no need, no thank you\par
\footnotesize Introduced in Chapter 58.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{വേര്}}\enspace\emph{vērŭ}\par
\small a root\par
\footnotesize Introduced in Chapter 54.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{വെയിൽ}}\enspace\emph{veyil}\par
\small sunshine, the sun's heat\par
\footnotesize Introduced in Chapter 53.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{വിദ്യാർത്ഥി}}\enspace\emph{vidyārtthi}\par
\small a student\par
\footnotesize Introduced in Chapter 48.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{വിളക്ക്}}\enspace\emph{viḷakkŭ}\par
\small a lamp\par
\footnotesize Introduced in Chapter 46.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{വിരൽ}}\enspace\emph{viral}\par
\small a finger\par
\footnotesize Introduced in Chapter 47.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{വിശപ്പ്}}\enspace\emph{viśappŭ}\par
\small hunger\par
\footnotesize Introduced in Chapter 63.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{വിശ്വാസം}}\enspace\emph{viśvāsaṁ}\par
\small trust\par
\footnotesize Introduced in Chapter 65.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{വിട}}\enspace\emph{viṭa}\par
\small leave, a farewell\par
\footnotesize Introduced in Chapter 50.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{വിത്ത്}}\enspace\emph{vittŭ}\par
\small a seed\par
\footnotesize Introduced in Chapter 54.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{യാത്ര}}\enspace\emph{yātra}\par
\small a journey\par
\footnotesize Introduced in Chapter 50.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{അച്ഛൻ} \ml{അമ്മ} \ml{ചേട്ടൻ} \ml{അനിയൻ} \ml{ചേച്ചി} \ml{അനിയത്തി}}\par
\small father, mother, and four age-graded sibling words — Malayalam's own distinct set, still following the shared Dravidian age-first pattern\par
\footnotesize Introduced in Chapter 12.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{അതെ}}\par
\small yes (athe — \textquotedblleft{}that {[}is so{]}\textquotedblright{})\par
\footnotesize Introduced in Chapter 1.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ആണ്}}\par
\small is (the copula)\par
\footnotesize Introduced in Chapter 2.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ഉച്ചകഴിഞ്ഞ്}}\par
\small \textquotedblleft{}afternoon\textquotedblright{} (uchakaḻiññ) — a transparent two-word phrase, not a single dictionary headword: \ml{ഉച്ച} (ML-C17's \textquotedblleft{}noon\textquotedblright{}) + \ml{കഴിഞ്ഞ്}, the past converb form of \ml{കഴിയുക} (kaḻiyuka, Wiktionary-confirmed Proto-Dravidian \emph{kaẓi, \textquotedblleft{}to be over, spent\textquotedblright{}) — Wiktionary itself directly shows only the finite past tense \ml{കഴിഞ്ഞു} (kaḻiññu); the converb form used in this compound is a reasonable grammatical inference, not itself the form Wiktionary lists; literally \textquotedblleft{}noon having passed\textquotedblright{} — a structurally different strategy than Telugu's, where the SAME noon-word (\te{మధ్యాహ్నం}) simply widened its own meaning rather than compounding with a second word}\par
\footnotesize Introduced in Chapter 28.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{എന്ത്}}\par
\small what\par
\footnotesize Introduced in Chapter 2.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{എന്റെ} \ml{പേര്} … \ml{ആണ്}}\par
\small my name is…\par
\footnotesize Introduced in Chapter 2.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{കറുപ്പ്} \ml{വെള്ള} \ml{ചുവപ്പ്} \ml{നീല}}\par
\small black, white, red, blue — Malayalam shares Tamil's native colors, and its Sanskrit blue too\par
\footnotesize Introduced in Chapter 11.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ചിങ്ങം} \ml{കന്നി} \ml{തുലാം} \ml{വൃശ്ചികം} \ml{ധനു} \ml{മകരം} \ml{കുംഭം} \ml{മീനം} \ml{മേടം} \ml{ഇടവം} \ml{മിഥുനം} \ml{കർക്കടകം}}\par
\small the twelve months of the Malayalam (Kollam Era) solar calendar — also solar, like Tamil's, but named for ZODIAC SIGNS rather than nakshatras\par
\footnotesize Introduced in Chapter 16.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ഞാൻ}}\par
\small I\par
\footnotesize Introduced in Chapter 3.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ഞാൻ} \ml{മലയാളം} \ml{സംസാരിക്കുന്നു}}\par
\small I speak Malayalam\par
\footnotesize Introduced in Chapter 5.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{തിങ്കൾ} \ml{ചൊവ്വ} \ml{ബുധൻ} \ml{വ്യാഴം} \ml{വെള്ളി} \ml{ശനി} \ml{ഞായർ}}\par
\small the seven weekdays — Malayalam shares Tamil's native planet-words, being its closest cousin\par
\footnotesize Introduced in Chapter 10.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{തല} \ml{കൈ}}\par
\small head and hand — matching Tamil's native pair almost exactly\par
\footnotesize Introduced in Chapter 13.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{നീ} / \ml{നിങ്ങൾ}}\par
\small you (familiar / respectful)\par
\footnotesize Introduced in Chapter 2.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{നിങ്ങൾക്ക്} \ml{എത്ര} \ml{വയസ്സുണ്ട്}?}\par
\small how old are you? — literally \textquotedblleft{}to you how much age exists?\textquotedblright{}; the SAME Sanskrit vayas word, but Malayalam adds a dative-subject + existential verb, a genuinely different shape than Kannada/Telugu's plain possessive\par
\footnotesize Introduced in Chapter 19.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{നിന്റെ} \ml{പേര്} \ml{എന്താണ്}?}\par
\small what's your name?\par
\footnotesize Introduced in Chapter 2.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{നായ}, \ml{പൂച്ച}}\par
\small dog and cat — naaya continues the solid, ancient native Dravidian dog-root shared with Kannada and Tamil, no mystery; poocha matches Tamil's everyday cat-word closely, part of the pattern where Dravidian genuinely splits on \textquotedblleft{}cat\textquotedblright{} more than \textquotedblleft{}dog\textquotedblright{}\par
\footnotesize Introduced in Chapter 21.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പോകുക}}\par
\small to go (and \ml{വരിക}, to come)\par
\footnotesize Introduced in Chapter 4.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പച്ച}, \ml{മഞ്ഞ}}\par
\small green and yellow — paccha matches Tamil's paccai almost exactly (the same pan-Dravidian \emph{pac- root already seen in Kannada and Telugu); manja is a completely separate native Dravidian root, a doublet of manjal (\textquotedblleft{}turmeric\textquotedblright{}), matching Tamil's own mañcaḷ — a third, independent solution to this arc's recurring green/yellow puzzle}\par
\footnotesize Introduced in Chapter 22.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പതിനൊന്ന്} — \ml{ഇരുപത്}}\par
\small 11-20 — additive \textquotedblleft{}ten-echo\textquotedblright{} compounds for the teens, then irupathu (20), transparently \textquotedblleft{}two-tens\textquotedblright{} — iru (\textquotedblleft{}two\textquotedblright{}) + pathu (\textquotedblleft{}ten\textquotedblright{}), matching Tamil closely\par
\footnotesize Introduced in Chapter 20.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{പാതിരാ}}\par
\small midnight, built on Malayalam paathi \textquotedblleft{}half\textquotedblright{} rather than Sanskrit ardha\par
\footnotesize Introduced in Chapter 17.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{മണി}}\par
\small hour — Malayalam's OWN dictionary tradition treats bell/hour/gem as ONE Sanskrit word, unlike Tamil's closely related mani, which its own dictionaries split into two unrelated homophones\par
\footnotesize Introduced in Chapter 18.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{രാത്രി}}\par
\small \textquotedblleft{}night\textquotedblright{} — the Sanskrit tatsama already shortened inside paathira, with a different PIE root from Latin nox\par
\footnotesize Introduced in Chapter 24.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{വീണ്ടും} \ml{കാണാം}}\par
\small we'll meet again\par
\footnotesize Introduced in Chapter 4.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{വെള്ളം} \ml{അരി} \ml{ചോറ്}}\par
\small water (a genuinely DIFFERENT word than Tamil's), and rice raw/cooked, closely matching Tamil\par
\footnotesize Introduced in Chapter 15.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{വസന്തകാലം} \ml{വേനൽക്കാലം} \ml{മഴക്കാലം} \ml{ശൈത്യകാലം}}\par
\small spring, summer, monsoon, winter — matching Tamil's pattern of native heat/rain/cold words plus a Sanskrit \textquotedblleft{}spring\textquotedblright{}\par
\footnotesize Introduced in Chapter 14.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{ശുഭ} \ml{രാത്രി}}\par
\small \textquotedblleft{}good night\textquotedblright{} — the standard Sanskrit-tatsama phrase (rāthri already met, śubha new); some sources describe modern Malayalam speakers often code-switching to English \textquotedblleft{}good night\textquotedblright{} instead — the same claim, similarly thinly sourced, is also made about Telugu\par
\footnotesize Introduced in Chapter 25.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{സുഖമാണോ}?}\par
\small how are you? (lit. \textquotedblleft{}are you well?\textquotedblright{})\par
\footnotesize Introduced in Chapter 3.
\end{minipage}\par\medskip

\noindent\begin{minipage}[t]{\linewidth}
\raggedright
\textbf{\ml{സംസാരിക്കുക}}\par
\small to speak, to converse\par
\footnotesize Introduced in Chapter 5.
\end{minipage}\par\medskip
