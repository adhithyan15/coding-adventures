# Coding Adventures Telugu Assessment
...
This specification names the assessment that the complete Telugu book must
...
learner-facing name at each rung is **Coding Adventures Telugu <Level>
Assessment — project-defined equivalent**.

No `assessment.json` is checked in for Telugu yet, and that omission is
...
Telugu awarding body, and the Council of Europe states that it does not verify
or validate an examination's claimed link to CEFR. Every target here is
therefore labelled `project-defined`.

Telugu has the closest thing to a four-skill ladder of any Indic track in this
corpus, and it deserves saying plainly rather than being waved at. **SiliconAndhra
ManaBadi**, a diaspora Telugu school programme affiliated to **Potti Sreeramulu
Telugu University**, runs eight named levels in order — BalaBadi I, BalaBadi II,
Pravesam, Prasunam, Prakasam, Pramodam, Prabhasam, Prabodham — with a **Junior
Certificate** awarded at Prakasam and a **Senior Certificate** at Prabhasam.
Both are earned by passing an annual PSTU examination, and that examination
tests **reading, writing, listening, speaking and comprehension**: five
components, one more than this contract has.

So why is this still project-defined?

- It is a **school ladder for heritage learners**, structured by school year and
  aimed largely at children in Telugu-speaking families abroad. Its rungs are
  years of attendance, not proficiency bands.
- It publishes **no per-skill pass rule** and **no CEFR mapping**. Passing the
  PSTU examination is reported as passing; it does not report whether each of
  the five components independently reached a standard.
- PSTU's own certificate programmes require **prior** proficiency in reading,
  writing and speaking Telugu to be admitted, so they begin above where this
  book begins rather than at its first rung.

The infrastructure is real and its audience overlaps this book's almost exactly.
What is missing is the published per-skill rule, and that is the one thing a
contract cannot borrow. This project therefore states its own in the open.

The assessed variety is contemporary standard Telugu in the Telugu script. The
current track's taught forms define the initial model. Documented regional forms
receive credit when used consistently and when they satisfy the task; a trained
Telugu reviewer adjudicates unfamiliar forms.
...
- [SiliconAndhra ManaBadi — Potti Sreeramulu Telugu University affiliation and course levels](https://manabadi.siliconandhra.org/potti-sreeramulu-telugu-university/)
...
3. range and control of Telugu vocabulary and grammar;
4. Telugu orthographic control for writing, or pronunciation and fluency for
   speaking.

Each dimension is scored 0–5, then scaled to the paper's points. A response that
is not meaningfully in Telugu cannot earn task-fulfilment credit.

Dimension 4 for writing is scored on three things the Telugu writing system
actually requires:

- the **వత్తు** (*vattu*), the subscript consonant. Telugu writes a conjunct by
  hanging a reduced second consonant BELOW the base letter rather than beside
  it, so a Telugu line occupies three tiers and a reader's eye has to look down
  as well as along. Writing a vattu beside the base instead of under it is an
  error of a kind Latin-script learners have no experience of, and the corpus
  builds 1,683 of them;
- the **తలకట్టు** (*talakattu*), the headstroke that sits on top of most
  letters. It is what makes a line of Telugu read as a line rather than as
  separate marks, and a hand that leaves it off has removed that;
- **vowel-sign placement and legibility.**

## The writing ramp, measured rather than assumed

Telugu has **49 script lessons** in its learning record and **not one
...
is a contract that reads as satisfied by forty-nine lessons that do not stage
anything. Note that the track's script coverage itself is in good order — 64
glyphs taught against 64 shown, none never-taught — so what is missing is the
staging of the hand, not the letters.

## Administration rules

- Listening recordings use Telugu speakers. Words-per-minute bands below are
  form-assembly targets, not a claim that Telugu has one natural speaking rate.
- Pre-A1 and A1 directions may also be supplied in a declared support language,
  so misunderstanding directions is not confused with Telugu proficiency. From
  A2 onward directions are in Telugu, with one unscored example.
...
evidence. Before release, at least eight qualified Telugu educators or linguists
...
|---|---:|---|
| reading | 10 | 6 Telugu shape/word matches, 4 short phrase or notice matches, and 4 personal-detail selections; no text over 8 words |
