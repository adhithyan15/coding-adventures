...
describe("the committed corpus", () => {
  ...
  it("pins the first ledger, and it is the reason the rule exists", () => {
    ...
    expect(l.summary.roots).toBe(2731); // +5: vos-latin slug and ES-ETYMON-VOS-03
    expect(l.summary.underspent).toBe(2628); // +3: a payoff lesson re-spends a root // +1: the vos-latin slug is unspent; ES-ETYMON-VOS-03 is spent three times, so it is NOT latin-vos, spent once so far // -1: HL-C98 spends fabulari-latin a third time
    ...
    expect(byNamespace).toEqual({ roots: 1972, "etymon-atom": 759 });
  });
  ...
  it("pins Spanish, the pilot track", () => {
    ...
    expect(l.summary).toMatchObject({
      roots: 317, // +2: vos-latin and ES-ETYMON-VOS-03
      underspent: 294, // -1: HL-C98 spends fabulari-latin a third time
