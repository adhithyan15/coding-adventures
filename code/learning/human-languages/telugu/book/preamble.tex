% Preamble for "Telugu, Step by Step, Root by Root" (HL00 book format).
% XeLaTeX only. Uses the vendored static Noto Sans Telugu (see _fonts/),
% loaded by relative path so local and CI builds are identical.
%
% The Telugu script is rendered via \newfontfamily with Script=Telugu (XeTeX
% shapes from the font's OpenType tables). We do NOT load a polyglossia Telugu
% language module — only english as main — so the build never depends on a
% gloss-telugu.ldf being present.
