...
describe("the committed corpus", () => {
  ...
  it("pins the first ledger, and it is the reason the rule exists", () => {
    ...
    expect(l.summary.roots).toBe(3079); // +1: Italian's typed ciao opener records its etymon atom
    expect(l.summary.underspent).toBe(2964); // Italian adds one after Latin's guided-copy bridge re-spends salvē
    ...
    expect(byNamespace).toEqual({ roots: 2194, "etymon-atom": 885 }); // +1 etymon-atom: Italian ciao
  });
