...
class ConstantPoolBuilder {
    ...
    nameAndType(name, descriptor) {
        return this.add(`NameAndType:${name}:${descriptor}`, concatBytes(u1(12), u2(this.utf8(name)), u2(this.utf8(descriptor))));
    }
    fieldRef(owner, name, descriptor) {
        return this.add(`Fieldref:${owner}:${name}:${descriptor}`, concatBytes(u1(9), u2(this.classRef(owner)), u2(this.nameAndType(name, descriptor))));
    }
    methodRef(owner, name, descriptor) {
        return this.add(`Methodref:${owner}:${name}:${descriptor}`, concatBytes(u1(10), u2(this.classRef(owner)), u2(this.nameAndType(name, descriptor))));
    }
    entriesBytes() {
...
class Lowerer {
    ...
    emitMainWrapper;
    regField = "__ca_regs";
    memField = "__ca_memory";
    regGetHelper = "__ca_regGet";
    regSetHelper = "__ca_regSet";
    memLoadByteHelper = "__ca_memLoadByte";
    memStoreByteHelper = "__ca_memStoreByte";
    loadWordHelper = "__ca_loadWord";
    storeWordHelper = "__ca_storeWord";
    syscallHelper = "__ca_syscall";
    compareId = 0;
    ...
    ensureNoHelperCollisions(callableLabels) {
    ...
    buildClassInitializer(pool, registerCount, dataOffsets) {
    ...
    buildCallableRegion(pool, region, dataOffsets) {
        const code = new BytecodeBuilder();
    ...
    lowerInstruction(code, pool, instruction, dataOffsets) {
    ...
    emitBinaryOp(code, pool, instruction, opcode) {
        this.emitRegSetFromStack(code, pool, registerIndex(instruction.operands[0]), () => {
            this.emitRegGet(code, pool, registerIndex(instruction.operands[1]));
    ...
    emitCompare(code, pool, instruction, branchOpcode) {
...
export function lowerIrToJvmClassFile(program, config) {
    return new Lowerer(program, config).lower();
}
export function writeClassFile(artifact, outputDir) {
...
function emitStoreIntLocal(code, index) {
...
function encodeClassFile(params) {
    ...
    const fields = params.fields.map((field) => concatBytes(u2(field.accessFlags), u2(pool.utf8(field.name)), u2(pool.utf8(field.descriptor)), u2(0)));
    const methods = params.methods.map((method) => {
        const codeAttribute = concatBytes(u2(codeNameIndex), u4(12 + method.code.length), u2(method.maxStack), u2(method.maxLocals), u4(method.code.length), method.code, u2(0), u2(0));
        return concatBytes(u2(method.accessFlags), u2(pool.utf8(method.name)), u2(pool.utf8(method.descriptor)), u2(1), codeAttribute);
    });
    return concatBytes(u4(0xcafebabe), u2(params.minorVersion), u2(params.majorVersion), u2(pool.size()), pool.entriesBytes(), u2(params.accessFlags), u2(thisClassIndex), u2(superClassIndex), u2(0), u2(fields.length), ...fields, u2(methods.length), ...methods, u2(0));
}
function collectCallableLabels(program) {
    const labels = new Set([program.entryLabel]);
...
function splitRegions(program, callableLabels) {
...
function totalStaticData(program) {
...
function registerIndex(operand) {
    if (operand.kind !== "register") {
...
function ensureNoSymlinkPath(path) {
    try {
