...
runtime rediscovery steps for native-USB and serial-adapter boards. That plan
can also become a typed process-launch contract, including stdout/stderr
capture requirements and success exit codes, so frontends do not invent their
own Arduino CLI spawn conventions. After the frontend runs that process, the
same Rust-owned path classifies the exit code and CLI diagnostics into upload
status, retry, and remediation hints.
