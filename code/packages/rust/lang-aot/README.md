...
> **LANG-FULL E4 — literal string `PRINT` and `string-length` run on all 7 backends (v0.111.0):**
> `tests/lang_matrix.rs` now proves `10 PRINT "HELLO"` on native-AOT + LLVM + WASM + JVM + CLR + VM + JIT.
> It also proves Twig `(string-length "HELLO")` returns exit code `5` on the same seven backends.
...
> Literal length is folded/read from metadata on native/LLVM/WASM and uses host string length on JVM/CLR.
> Richer byte-string ops and string variables stay follow-up E4/BA4 slices.
