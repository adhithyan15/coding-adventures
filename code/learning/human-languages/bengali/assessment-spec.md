# Coding Adventures Bengali Assessment
...
learner-facing name at each rung is **Coding Adventures Bengali <Level>
Assessment — project-defined equivalent**.

No `assessment.json` is checked in for Bengali yet, and that omission is
...
Bengali awarding body, and the Council of Europe states that it does not verify
or validate an examination's claimed link to CEFR. Every target here is
therefore labelled `project-defined`.

There is one institution whose Bengali provision is aimed at exactly this book's
reader, and it is worth naming for that reason. The **Institute of Modern
Languages** at the University of Dhaka teaches fourteen languages in one-year,
120-hour non-degree courses leading to a Certificate of Proficiency and a
Diploma of Arts in Modern Languages. Of those fourteen, **Bangla is the one
reserved for foreign students** — everything else is offered to Bangladeshi
learners, and Bangla is offered only to people learning it from outside.

That is a real programme for a real audience, and it is **not** a four-skill
proficiency examination. It is measured in course hours, and it publishes no
per-skill examination structure, no pass rule, and no rubric. A certificate
awarded for completing 120 hours certifies attendance and study; it does not
report whether reading, listening, writing and speaking each reached a standard.
This project therefore defines its own transparent approximation, and states its
rule in the open rather than borrowing an authority that does not publish one.

The assessed variety is contemporary standard Bengali (চলিত, the modern standard
written and spoken register) in the Bengali script. The current track's taught
forms define the initial model. Documented regional forms receive credit when
used consistently and when they satisfy the task; a trained Bengali reviewer
adjudicates unfamiliar forms.
...
- [Institute of Modern Languages, University of Dhaka](https://www.du.ac.bd/body/about/IML)
...
3. range and control of Bengali vocabulary and grammar;
4. Bengali orthographic control for writing, or pronunciation and fluency for
   speaking.

Each dimension is scored 0–5, then scaled to the paper's points. A response that
is not meaningfully in Bengali cannot earn task-fulfilment credit.

Dimension 4 for writing is scored on three things the Bengali writing system
actually requires:

- the **মাত্রা**, the headline stroke. Bengali letters hang from a horizontal
  line drawn across the top, and that line is what a reader's eye follows to
  find where one word stops and the next starts. A hand that omits it or breaks
  it mid-word has not made a cosmetic error; it has removed the thing that
  groups the letters;
- the **hasanta** ্, which cancels the inherent vowel and is what builds a
  conjunct. The corpus writes it 284 times, and a learner who drops it writes a
  different word;
- **vowel-sign side and legibility.** Bengali writes ি to the LEFT of its
  consonant and ী to the right, and ে to the left while ো wraps around both
  sides. A sign on the wrong side is an orthographic error and a common one.

## The writing ramp, measured rather than assumed

Bengali has **74 script lessons** in its learning record and **not one
...
proved, not proved late, unproved.

That is stated here rather than left to a backlog note, because the pre-A1
writing paper below requires delayed recall and dictation. The inventory
describes the examination, not the corpus. A learner the book has never taken
past showing them a letter cannot answer a delayed-recall item, and the gap
between the two documents is the work. Naming it is what makes it fixable; the
alternative is a contract that reads as satisfied by seventy-four lessons that
do not stage anything.

## Administration rules

- Listening recordings use Bengali speakers. Words-per-minute bands below are
  form-assembly targets, not a claim that Bengali has one natural speaking rate.
- Pre-A1 and A1 directions may also be supplied in a declared support language,
  so misunderstanding directions is not confused with Bengali proficiency. From
  A2 onward directions are in Bengali, with one unscored example.
...
evidence. Before release, at least eight qualified Bengali educators or
...
|---|---:|---|
| reading | 10 | 6 Bengali shape/word matches, 4 short phrase or notice matches, and 4 personal-detail selections; no text over 8 words |
