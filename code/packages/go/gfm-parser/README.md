...
A GFM 0.31.2-compliant Markdown parser in Go. Converts a Markdown string
into a `DocumentNode` abstract syntax tree (Document AST, spec TE00).

## Architecture

Parsing is a two-phase process modelled on the GFM reference implementation:
...
2. **Phase 2 — Inline content** (`inline_parser.go`): walks each heading and
   paragraph node, runs the GFM delimiter-stack algorithm (Appendix A),
...
100% — all 652 GFM 0.31.2 spec examples pass (verified by the
`commonmark` package's `TestGFMSpec`).
