...
`);
    ...
    const plotCard = {
      ...
      analysis: "ac",
      probes: [
        { kind: "voltage", target: "in", negativeTarget: "out" },
        { kind: "voltage", target: "out" },
      ],
    };
...
`);
    ...
    const saveCard = {
      kind: "save",
      probes: [
        { kind: "voltage", target: "out", negativeTarget: "in" },
        { kind: "current", target: "Vin" },
      ],
    };
    const probeCard = {
      kind: "probe",
      analysis: "tran",
      probes: [{ kind: "voltage", target: "out", negativeTarget: "in" }],
    };
    const measureCard = {
      kind: "measure",
      analysis: "tran",
      name: "peak",
      operation: "max",
      probe: { kind: "voltage", target: "out", negativeTarget: "in" },
      start: 0,
      stop: 1.0e-3,
    };
...
`;
    ...
    const positive = outputs[2].rows[0].values.get("V(in)") as { real: number; imag: number };
    ...
    expect(measures[1].value).toBeCloseTo(0.5, 9);
    expect(measures[2].value).toBeCloseTo(outputs[3].rows.at(-1)!.values.get("V(out)") as number, 9);
    expect(measures[3].value).toBeGreaterThan(0.0);
    expect(measures[3].value).toBeLessThanOrEqual(outputs[3].rows.at(-1)!.values.get("V(out)") as number);
  });
