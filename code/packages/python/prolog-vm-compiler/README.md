...
The Prolog-on-Logic-VM implementation track covers PR00 through PR78. The
...
`read_file_to_codes/2`. The manifest also names the remaining advanced-dialect
work that is intentionally outside the completed batches: full external dialect
emulation, tabling and well-founded negation, generalized
attributed-variable/coroutining services, non-FD constraint domains, stream
handles, foreign predicates, engines, and concurrency.
