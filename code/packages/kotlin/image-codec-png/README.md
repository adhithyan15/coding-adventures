# image-codec-png (Kotlin)

Native Kotlin implementation of the bounded IC18 PNG profile. It encodes RGBA8
...
- `encodePng(PixelContainer)` returns a complete PNG byte array.
- `decodePng(ByteArray)` decodes with the default 32-mebipixel ceiling.
- `decodePng(ByteArray, Double)` accepts a positive integral lower ceiling.
- `adler32(ByteArray)` exposes the RFC 1950 checksum.
- `PngCodec` implements the shared `ImageCodec` contract with MIME type
  `image/png`.
- `PngError.code` is one of the 29 ordered identifiers in `ERROR_CODES`; its
  message is exactly that payload-blind code.

`MAX_DIMENSION` is 16,384 and `DEFAULT_MAX_PIXELS` is 33,554,432. Both edge
and product checks happen before filtered or RGBA allocation. Decode also caps
raw inflation at the exact scanline size promised by IHDR and requires exact
DEFLATE byte consumption, Adler-32, chunk CRCs, and legal chunk order.
