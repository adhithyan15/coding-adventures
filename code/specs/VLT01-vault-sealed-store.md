...
from the operator password. After a rotation the old KEK entry remains
in the manifest with status `retired`, together with its original salt,
so it can still be verified (and the records it wrapped can still be
unwrapped in a crash-recovery path).

An injected entry omits `salt`: the caller supplies an already-random
32-byte KEK, normally generated by `vault-key-custody::fresh_random_key`
and persisted only in custodian-wrapped form outside this layer. The
sealed store persists only its verifier. `source` is additive in the v1
manifest: readers MUST treat an absent `source` as `password-derived` so
...
```text
    ┌────────────┐  init(password, params) / init_with_kek(kek)  ┌────────────┐
    │  Absent    │ ──────────────────────────────────────> │ Unsealed   │
    ...
    └────────────┘                                         └────────────┘
          ...
          │ (never)                                            │   │
          │                                                    │   │
          │          unseal(password) / unseal_with_kek(kek) ✓   ▼   │
    ┌────────────┐ ◄────────────────────────────────────── ┌────────────┐
    │  Sealed    │                                         │ Unsealed   │
    │ (no KEK)   │ ──────────────────────────────────────> │ (KEK in RAM)│
    └────────────┘             seal()                      └────────────┘
...
impl SealedStore {
    ...
    /// Initialize with a caller-supplied random 32-byte KEK. The key is
    /// verified but never persisted. The caller owns its custody.
    pub fn init_with_kek(&self, kek: &[u8; 32])
        -> Result<(), SealedStoreError>;

    /// Load an injected-KEK manifest and verify the supplied KEK. Password-
    /// derived entries are deliberately ignored by this entry point.
    pub fn unseal_with_kek(&self, kek: &[u8; 32])
        -> Result<(), SealedStoreError>;
...
- `InvalidKek` — an injected KEK did not verify against an injected entry.
...
3. **Confidentiality of the KEK** — never persisted; either derived from
   the password via Argon2id or supplied by a caller-owned custodian; held
   in RAM only during unsealed sessions and wiped on `seal()` / `drop`.
...
- injected random KEK init → seal → custodian unwrap → unseal roundtrip
- wrong injected KEK → `InvalidKek`; password unseal ignores injected entries
- pre-extension manifests without `source` still unseal with their password
...
- Hardware-backed KEK implementations (Secure Enclave / TPM / YubiKey).
  VLT01 accepts their unwrapped key material through `unseal_with_kek`,
  while the custody provider remains responsible for the platform unlock.
- Rotation between injected KEKs. Password-derived `rotate_kek` remains
  unchanged; custodian-backed rekeying requires a separate crash-safe API.
