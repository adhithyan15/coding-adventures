...
use language_parser::create_java_parser;
...
pub fn parse(source: &str, version: &str) -> Result<String, JsValue> {
    let mut parser = create_java_parser(source, resolve_version(version)).map_err(to_js_error)?;
    let ast = parser
        .parse()
        .map_err(|e| to_js_error(format!("Java parse failed: {e}")))?;
...
mod tests {
    ...
    fn wrapper_parses_to_json() {
        let json = parse("class Hello {}", "").unwrap();
