...
describe("corpus snapshot", () => {
  ...
  it("pins the first published chapter-gate snapshot", () => {
    ...
    expect(report.summary.bookChapters).toBe(679); // +8: HL-C94 splits the four over-budget opening chapters into twelve // +16: vocabulary wave 4, 4 tracks x 4 chapters // +4: HL-C98 gives the first paradigm one cell per chapter (3 teaching + review + synthesis) // +15: vocabulary wave 5 (persian +3, telugu +6, malayalam +6) // +4: HL-C88 slices 5-6 (Spanish friend-ending chapters)
    expect(report.summary.declaredChapters).toBe(679); // +98: handwritten capability closure // +4: HL-C98 // +15: vocabulary wave 5 // +4: HL-C88 slices 5-6
