...
impl RiscVSimulator {
    ...
    pub fn step(&mut self) -> String {
        ...
        if mnemonic == "ecall" && self.csr.read(crate::csr::CSR_MTVEC) == 0 {
            match self.regs.read(HOST_ECALL_SERVICE_REGISTER) {
                ...
                HOST_ECALL_F64_ADD | HOST_ECALL_F64_SUB | HOST_ECALL_F64_MUL
                | HOST_ECALL_F64_DIV => {
                ...
                HOST_ECALL_F64_TO_I64_TRUNC => {
                    ...
                    {
                        self.host_events
                            .push(HostEvent::Exit(HOST_ECALL_FLOAT_CONVERSION_FAULT_EXIT_CODE));
                        self.exit_code = Some(HOST_ECALL_FLOAT_CONVERSION_FAULT_EXIT_CODE);
                        self.halted = true;
                        return mnemonic;
                    }
                    self.set_host_i64_result(value.trunc() as i64);
                    self.pc += 4;
                    return mnemonic;
                }
                HOST_ECALL_F64_FLOOR => {
                    let value = self.host_f64_argument(
                        HOST_ECALL_ARGUMENT_LOW_REGISTER,
                        HOST_ECALL_ARGUMENT_HIGH_REGISTER,
                    );
    ...
    fn host_f64_argument(&self, low_register: usize, high_register: usize) -> f64 {
        let bits =
            self.regs.read(low_register) as u64 | ((self.regs.read(high_register) as u64) << 32);
        f64::from_bits(bits)
    }
...
mod tests {
    ...
    fn test_xori() {
        ...
        assert_eq!(sim.regs.read(2), 0xF0);
    }
    #[test]
    fn test_ori() {
        let sim = run_program(&[
        ...
        assert_eq!(sim.regs.read(2), 0x5F);
    }
    #[test]
    fn test_andi() {
        let sim = run_program(&[
    ...
    fn test_slli() {
        ...
        assert_eq!(sim.regs.read(3), 0x80000000);
    }
    #[test]
    fn test_srli() {
        let sim = run_program(&[
        ...
        assert_eq!(sim.regs.read(2), 0x0FFFFFFF);
    ...
    fn test_srai() {
        ...
        assert_eq!(sim.regs.read(2) as i32, -4);
    ...
    fn test_add_sub() {
        ...
        assert_eq!(sim.regs.read(3), 30);
        assert_eq!(sim.regs.read(4) as i32, -10);
    }
    #[test]
    fn test_mul_and_mulhu() {
        let sim = run_program(&[
            encode_addi(1, 0, -1),
            encode_addi(2, 0, 2),
            encode_mul(3, 1, 2),
        ...
        assert_eq!(sim.regs.read(3), 0xffff_fffe);
    ...
    fn test_xor() {
        ...
        assert_eq!(sim.regs.read(3), 0xF0);
    }
    #[test]
    fn test_srl() {
        let sim = run_program(&[
        ...
        assert_eq!(sim.regs.read(3), 0x0FFFFFFF);
    }
    #[test]
    fn test_sra() {
        let sim = run_program(&[
        ...
        assert_eq!(sim.regs.read(3) as i32, -4);
    }
    #[test]
    fn test_or() {
        let sim = run_program(&[
        ...
        assert_eq!(sim.regs.read(3), 0x5F);
    }
    #[test]
    fn test_and() {
        let sim = run_program(&[
    ...
    fn test_sw_lw() {
        ...
        assert_eq!(sim.regs.read(3), 0x42);
    }
    #[test]
    fn test_sb_lbu() {
        let sim = run_program(&[
        ...
        assert_eq!(sim.regs.read(3), 0xAB);
    }
    #[test]
    fn test_lb_sign() {
        let sim = run_program(&[
        ...
        assert_eq!(sim.regs.read(4), 0xFF);
    }
    #[test]
    fn test_sh_lhu() {
        let sim = run_program(&[
        ...
        assert_eq!(sim.regs.read(3), 0x1FF);
    }
    #[test]
    fn test_lh_sign() {
        let sim = run_program(&[
        ...
        assert_eq!(sim.regs.read(3) as i32, -1);
        assert_eq!(sim.regs.read(4), 0xFFFF);
    }
    #[test]
    ...
    fn test_call_return() {
        ...
        assert_eq!(sim.regs.read(1), 4);
        assert_eq!(sim.regs.read(10), 42);
        assert_eq!(sim.regs.read(11), 99);
    }

    // LUI / AUIPC
    #[test]
    fn test_lui() {
        let sim = run_program(&[encode_lui(1, 0x12345), encode_ecall()]);
        assert_eq!(sim.regs.read(1), 0x12345000);
    }
    #[test]
    fn test_lui_addi() {
        let sim = run_program(&[
        ...
        assert_eq!(sim.regs.read(1), 0x12345678);
    }
    #[test]
    fn test_auipc() {
        let sim = run_program(&[encode_auipc(1, 1), encode_ecall()]);
        assert_eq!(sim.regs.read(1), 0x1000);
    }
    #[test]
    fn test_auipc_nonzero() {
        let sim = run_program(&[encode_addi(0, 0, 0), encode_auipc(1, 2), encode_ecall()]);
        assert_eq!(sim.regs.read(1), 0x2004);
    }

    // x0 hardwired
    #[test]
    fn test_x0_hardwired() {
        let sim = run_program(&[encode_addi(0, 0, 42), encode_ecall()]);
    ...
    fn test_csrrw() {
        ...
        assert_eq!(sim.regs.read(2), 0);
        assert_eq!(sim.regs.read(3), 0x100);
    }
    #[test]
    fn test_csrrs() {
        let mut sim = RiscVSimulator::new(65536);
    ...
    #[test]
    fn test_ecall_halt() {
        let sim = run_program(&[encode_addi(1, 0, 42), encode_ecall()]);
        assert!(sim.halted);
        assert_eq!(sim.regs.read(1), 42);
    }
    ...
    fn host_ecall_reads_and_writes_bytes() {
        ...
        sim.run_instructions(&[
            encode_addi(
                HOST_ECALL_SERVICE_REGISTER as u32,
                0,
                HOST_ECALL_READ_BYTE as i32,
            ),
            encode_ecall(),
            encode_addi(
                HOST_ECALL_SERVICE_REGISTER as u32,
                0,
                HOST_ECALL_WRITE_BYTE as i32,
            ),
            encode_ecall(),
            encode_addi(
                HOST_ECALL_SERVICE_REGISTER as u32,
        ...
        assert_eq!(
            sim.host_events,
            vec![
                HostEvent::ReadByte(i32::from(b'A')),
                HostEvent::WriteByte(b'A'),
                HostEvent::ReadByte(-1)
            ]
        );
    ...
    fn host_ecall_softfloat_services_operate_on_ieee754_bit_pairs() {
        ...
        sim.regs
            .write(HOST_ECALL_SERVICE_REGISTER, HOST_ECALL_F64_ADD);
        ...
        sim.regs
            .write(HOST_ECALL_SERVICE_REGISTER, HOST_ECALL_F64_CMP);
        assert_eq!(sim.step(), "ecall");
        ...
        sim.regs
            .write(HOST_ECALL_SERVICE_REGISTER, HOST_ECALL_I64_TO_F64);
        assert_eq!(sim.step(), "ecall");
        ...
        sim.regs
            .write(HOST_ECALL_SERVICE_REGISTER, HOST_ECALL_F64_TO_I64_TRUNC);
        assert_eq!(sim.step(), "ecall");
        ...
        sim.regs
            .write(HOST_ECALL_SERVICE_REGISTER, HOST_ECALL_F64_FLOOR);
        assert_eq!(sim.step(), "ecall");
        ...
        sim.regs
            .write(HOST_ECALL_SERVICE_REGISTER, HOST_ECALL_WRITE_F64);
        assert_eq!(sim.step(), "ecall");
    ...
    #[test]
    fn host_ecall_checked_f64_to_i64_halts_on_invalid_values() {
        fn set_pair(sim: &mut RiscVSimulator, value: u64) {
            sim.regs
                .write(HOST_ECALL_ARGUMENT_LOW_REGISTER, value as u32);
            sim.regs
                .write(HOST_ECALL_ARGUMENT_HIGH_REGISTER, (value >> 32) as u32);
        }

        for value in [f64::NAN, f64::INFINITY, 9_223_372_036_854_775_808.0] {
            let mut sim = RiscVSimulator::new(256);
            sim.load_program(&assemble(&[encode_ecall()]));
            set_pair(&mut sim, value.to_bits());
            sim.regs
                .write(HOST_ECALL_SERVICE_REGISTER, HOST_ECALL_F64_TO_I64_TRUNC);
            assert_eq!(sim.step(), "ecall");
            assert!(sim.halted, "{value:?} must fail closed");
            assert_eq!(
                sim.exit_code,
    ...
    fn test_ecall_trap_handler() {
        ...
        for _ in 0..pad_count {
            main_code.push(encode_addi(0, 0, 0));
        }
        ...
        assert_eq!(sim.regs.read(10), 99);
        assert_eq!(sim.regs.read(11), 77);
        assert!(sim.halted);
    }

    #[test]
    fn test_ecall_sets_csrs() {
        let mut sim = RiscVSimulator::new(65536);
        let mut code = vec![
            encode_addi(1, 0, 0x200),
            encode_csrrw(0, CSR_MTVEC, 1),
        ...
        assert_eq!(sim.regs.read(20), 16);
        assert_eq!(sim.regs.read(21), CAUSE_ECALL_M_MODE);
        assert_eq!(sim.regs.read(22) & MIE, 0);
    }
    ...
    #[test]
    fn test_unknown() {
        let sim = run_program(&[0xFFFFFFFF, encode_ecall()]);
        assert_eq!(sim.regs.read(1), 0);
    }
    #[test]
    fn test_neg_imm() {
        let sim = run_program(&[encode_addi(1, 0, -5), encode_ecall()]);
        assert_eq!(sim.regs.read(1) as i32, -5);
    }
    ...
    fn test_memcpy() {
        let mut sim = RiscVSimulator::new(65536);
        sim.mem.write_byte(0x200, 0xDE);
        sim.mem.write_byte(0x201, 0xAD);
        sim.mem.write_byte(0x202, 0xBE);
        sim.mem.write_byte(0x203, 0xEF);
        sim.run_instructions(&[
            encode_addi(1, 0, 0x200),
        ...
        for i in 0..4 {
            assert_eq!(sim.mem.read_byte(0x200 + i), sim.mem.read_byte(0x300 + i));
        }
    }
    #[test]
    fn test_stack() {
        ...
        assert_eq!(sim.regs.read(12), 99);
        assert_eq!(sim.regs.read(13), 42);
        assert_eq!(sim.regs.read(2), 0x400);
    }
    ...
    fn test_csr_rw() {
        ...
        assert_eq!(csr.read(CSR_MSTATUS), 0);
        csr.write(CSR_MSTATUS, 0x1234);
        assert_eq!(csr.read(CSR_MSTATUS), 0x1234);
    }
    #[test]
    fn test_csr_rw_atomic() {
        let mut csr = CSRFile::new();
        csr.write(CSR_MSCRATCH, 42);
        let old = csr.read_write(CSR_MSCRATCH, 99);
        assert_eq!(old, 42);
        assert_eq!(csr.read(CSR_MSCRATCH), 99);
    }
    #[test]
    fn test_csr_rs() {
        let mut csr = CSRFile::new();
        csr.write(CSR_MSTATUS, 0xF0);
        let old = csr.read_set(CSR_MSTATUS, 0x0F);
        assert_eq!(old, 0xF0);
        assert_eq!(csr.read(CSR_MSTATUS), 0xFF);
    }
    #[test]
    fn test_csr_rc() {
        let mut csr = CSRFile::new();
        csr.write(CSR_MSTATUS, 0xFF);
        let old = csr.read_clear(CSR_MSTATUS, 0x0F);
        assert_eq!(old, 0xFF);
        assert_eq!(csr.read(CSR_MSTATUS), 0xF0);
    }
