...
//! through the built CLI: a native `table` naming the second, structural
//! feature the SAME USGS Feature Type Thesaurus spans already state for
//! three landforms -- a sibling to the already-shipped `landforms.adj`
//! (which only carries each landform's single descriptor), decoding the
//! structural-clause half of spans already sitting unused inside that
//! table's own header. Resolves binding-query recall (both directions)
//! with the source's citation, and abstains on a landform (mountain) the
//! cited spans give no secondary feature for -- 0 model calls.
