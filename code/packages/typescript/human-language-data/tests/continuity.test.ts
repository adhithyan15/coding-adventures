...
describe("the real corpus", () => {
  ...
  it("pins what nothing had measured", { timeout: 30_000 }, () => {
    ...
    expect(report.summary.atomsTaught).toBe(2894); // +4: ES-LEX-VOS-01, ES-CULTURE-VOSEO-02 (ES-C03-vos) // +3: HL-C98 splits AR-PRESENT-SINGULAR into 1SG/2SG/3SG // +83: vocabulary wave 5 (persian ch9-11, telugu ch35-40, malayalam ch35-40) // +7: HL-C88 slices 5-6 (Spanish) // +2: HL-C88 slice 8 // +2: HL-C88 slice 9 (falsos amigos)
    ...
    // This RISES by one here, which is the direction it is not meant to travel.
    // ES-FRIENDS-FALSE-FRIENDS is introduced by the false-friends chapter and
    // nothing after it revisits the atom, because nothing comes after it yet --
    // it is currently the last chapter of the friends arc. When the cousin
    // panels land they should practise it, and this should fall back.
    expect(report.summary.atomsNeverRevisited).toBe(474); // -17: payoff lessons plus si/no moving early // +2: HL-C98's per-cell atoms are revisited later than the R-windows reach // -8: vocabulary wave 5 rescues net orphan atoms via reach-back payoffs // +4: HL-C88 slices 5-6
    ...
    // R1 stops at 3.
    expect(report.summary.missedByWindow.R1).toBe(910); // +2: HL-C98 // +2: vocabulary wave 5 // +4: HL-C88 slices 5-6 // +1: HL-C112 moves casa and libro 14 chapters earlier, so one atom's next re-use now falls outside R1 // +2: HL-C88 slice 9 (falsos amigos)
    ...
    expect(report.summary.missedByWindow.R2).toBe(2012); // +1: ES-C02-concordancia (HL-C85). buenos dias/buenas tardes stop REQUIRING the agreement rule; it becomes a payoff lesson after the learner has used all three greetings. // +84: vocabulary wave 5, new pre-A1 nouns landing late in already-long chapters // +4: HL-C88 slices 5-6 // +1: HL-C112, same cause as R1 above // +1: HL-C88 slice 8 // +2: HL-C88 slice 9 (falsos amigos)
  });
  ...
  it("shows what a declared reading order was worth", () => {
    ...
    expect(spanish).toMatchObject({
      ...
      lessonCount: 341, // +4: HL-C98 // +3: HL-C97 adds the repair kit (no entiendo, mas despacio) at chapter 14 // +8 payoff lessons // +1 ES-C03-vos, +1 ES-C02-concordancia // +4: HL-C88 slices 5-6 // +1: HL-C88 slice 7 (ES-C09-ncia) // +3: HL-C88 slice 8 (-ario, review, synthesis) // +1: HL-C88 slice 9 (falsos amigos)
      ...
      atomsTaught: 535, // +3: HL-C98's per-cell atoms // +7: HL-C88 slices 5-6 // +2: HL-C88 slice 7 (ES-C09-ncia) // +2: HL-C88 slice 8 (-ario, review, synthesis) // +2: HL-C88 slice 9 (falsos amigos)
      atomsNeverRevisited: 73, // slice 8 nets zero: -1 ES-FRIEND-NCIA-NCE (the new review and synthesis revisit it), +1 ES-ETYMON-ARIUS (introduced, never re-practised) // +2: HL-C98 // +4: HL-C88 slices 5-6 // +1: HL-C88 slice 7 (ES-C09-ncia) // +1: HL-C88 slice 9 (falsos amigos)
    });
  });
