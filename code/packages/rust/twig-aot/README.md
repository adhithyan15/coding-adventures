...
`str_len` and `str_eq` over direct literals fold to integer constants before
machine-code lowering, so Twig `(string-length "HELLO")` and
`(string=? "HELLO" "HELLO")` run natively through the same preparation pass.
