...
describe("fromAST (via parse)", () => {
  ...
  it("parses string with unicode escape", () => {
    /**
     * The JSON \uXXXX escape sequence. The lexer strips surrounding quotes
     * but leaves escape sequences as raw text. The json-value layer decodes
     * \u0041 to "A" (Unicode code point U+0041 = Latin capital letter A).
     */
    const value = parse('"\\u0041"');
    expect(value.type).toBe("string");
    if (value.type === "string") {
