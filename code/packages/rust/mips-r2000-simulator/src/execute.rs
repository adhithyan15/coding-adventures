...
pub(crate) fn read_word_be(mem: &Memory, addr: usize) -> u32 {
    let size = mem.size();
    let b0 = mem.read_byte(addr % size) as u32;
    let b1 = mem.read_byte((addr + 1) % size) as u32;
    let b2 = mem.read_byte((addr + 2) % size) as u32;
    let b3 = mem.read_byte((addr + 3) % size) as u32;
    (b0 << 24) | (b1 << 16) | (b2 << 8) | b3
}

fn write_word_be(mem: &mut Memory, addr: usize, value: u32) {
    let size = mem.size();
    mem.write_byte(addr % size, (value >> 24) as u8);
    mem.write_byte((addr + 1) % size, (value >> 16) as u8);
    mem.write_byte((addr + 2) % size, (value >> 8) as u8);
    mem.write_byte((addr + 3) % size, value as u8);
}

fn read_half_be(mem: &Memory, addr: usize) -> u16 {
    let size = mem.size();
    let hi = mem.read_byte(addr % size) as u16;
    let lo = mem.read_byte((addr + 1) % size) as u16;
    (hi << 8) | lo
}

fn write_half_be(mem: &mut Memory, addr: usize, value: u16) {
    let size = mem.size();
    mem.write_byte(addr % size, (value >> 8) as u8);
    mem.write_byte((addr + 1) % size, value as u8);
}
...
pub fn execute(
    ...
    match decoded.mnemonic.as_str() {
        ...
        "slt" => exec_reg_arith(decoded, regs, pc, |a, b| u32::from((a as i32) < (b as i32))),
        ...
        // Loads
        "lb" | "lh" | "lwl" | "lw" | "lbu" | "lhu" | "lwr" => exec_load(decoded, regs, mem, pc),
        // Stores
        "sb" | "sh" | "swl" | "sw" | "swr" => exec_store(decoded, regs, mem, pc),
...
fn exec_load(d: &DecodeResult, regs: &mut RegisterFile, mem: &Memory, pc: i32) -> ExecuteResult {
    ...
    let rs = get_field(d, "rs");
    let imm = get_field(d, "imm");
    let addr = (regs.read(rs as usize) as i32).wrapping_add(imm) as usize % mem.size();
    ...
    let result = match d.mnemonic.as_str() {
        ...
        "lwl" => {
            let word = read_word_be(mem, addr & !3);
            let shift = (3 - (addr & 3)) * 8;
            if shift == 0 {
                word
            } else {
                (word & !((1u32 << shift) - 1)) | (regs.read(rt as usize) & ((1u32 << shift) - 1))
            }
        }
        "lwr" => {
            let word = read_word_be(mem, addr & !3);
            ...
            if shift == 0 {
                ...
                let low = 32 - shift;
                (regs.read(rt as usize) & !((1u32 << low) - 1)) | (word & ((1u32 << low) - 1))
            }
        }
        _ => 0,
    };
...
fn exec_store(
    ...
    match d.mnemonic.as_str() {
        ...
        "swl" => {
            let aligned = addr & !3;
            let old = read_word_be(mem, aligned);
            let shift = (3 - (addr & 3)) * 8;
            let merged = if shift == 0 {
                val
            } else {
                (val & !((1u32 << shift) - 1)) | (old & ((1u32 << shift) - 1))
            };
            write_word_be(mem, aligned, merged);
        }
        "swr" => {
            let aligned = addr & !3;
            let old = read_word_be(mem, aligned);
