defmodule CodingAdventures.Cas.Store do
  @moduledoc """
  ...
  the `CodingAdventures.Cas.BlobStore` behaviour.

      store = CodingAdventures.Cas.Store.new(
        CodingAdventures.Cas.LocalDiskStore.new!("/tmp/objects")
      )

  ## Return Values

  All public functions return `{:ok, value}` on success and `{:error, reason}`
  on failure. See `CodingAdventures.Cas.Error` for the full reason catalogue.
  """
  ...
  `CodingAdventures.Cas.BlobStore` behaviour.

  ## Examples

      iex> backend = CodingAdventures.Cas.LocalDiskStore.new!(System.tmp_dir!() <> "/cas-new")
      iex> store = CodingAdventures.Cas.Store.new(backend)
      iex> is_struct(store, CodingAdventures.Cas.Store)
      true
  """
  @spec new(struct()) :: t()
  ...
  ## Examples
      ...
      iex> backend = CodingAdventures.Cas.LocalDiskStore.new!(System.tmp_dir!() <> "/cas-put")
      iex> store = CodingAdventures.Cas.Store.new(backend)
      iex> {:ok, key1} = CodingAdventures.Cas.Store.put(store, "hello")
      iex> {:ok, key2} = CodingAdventures.Cas.Store.put(store, "hello")
      iex> key1 == key2
      true
  """
  ...
  ## Examples
      ...
      iex> backend = CodingAdventures.Cas.LocalDiskStore.new!(System.tmp_dir!() <> "/cas-get")
      iex> store = CodingAdventures.Cas.Store.new(backend)
      iex> {:ok, key} = CodingAdventures.Cas.Store.put(store, "hello, world")
      iex> CodingAdventures.Cas.Store.get(store, key)
      {:ok, "hello, world"}
  """
  ...
  ## Examples
      ...
      iex> backend = CodingAdventures.Cas.LocalDiskStore.new!(System.tmp_dir!() <> "/cas-prefix")
      iex> store = CodingAdventures.Cas.Store.new(backend)
      iex> {:ok, key} = CodingAdventures.Cas.Store.put(store, "hello")
      iex> short = String.slice(CodingAdventures.Cas.Hex.key_to_hex(key), 0, 8)
      iex> {:ok, found} = CodingAdventures.Cas.Store.find_by_prefix(store, short)
      iex> found == key
      true
  """
