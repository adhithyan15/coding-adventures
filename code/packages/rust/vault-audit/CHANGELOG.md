...
- `AuditEntrySummary` and `AuditChainSummary` payload-free read
  models for supervisor dashboards and higher-tier audit indexes.
- Default `AuditSink::entry_summaries` and
  `AuditSink::chain_summary` helpers for append-only sinks that
  can materialize entries.
- Resource/detail coverage counters and helpers on `AuditChainSummary`.
