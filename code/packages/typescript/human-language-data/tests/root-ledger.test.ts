...
describe("the committed corpus", () => {
  ...
  it("pins the first ledger, and it is the reason the rule exists", () => {
    ...
    expect(l.summary.roots).toBe(3080); // Tamil's schema-v2 opener exposes its etymon atom too.
    expect(l.summary.underspent).toBe(2965); // The new Tamil etymon begins below the reuse floor.
    ...
    expect(byNamespace).toEqual({ roots: 2194, "etymon-atom": 886 }); // +1: Tamil's migrated opener etymon.
  });
