...
The previous entry measured the corpus's R2 misses, attributed a third of them
to the one-new-word-per-lesson chapter shape, and left the rest uncharacterised.
It also listed seven tracks — Arabic, German, Italian, Marathi, Persian,
Portuguese, Urdu — as "do not apply this fix", because none of their R2-missing
atoms sit in a chapter matching that shape's signature.
...
Measured on the merged tree, before Sanskrit. For every R2-missing atom outside
the tranche signature, take the distances at which the corpus does practise it
and bucket by where they fall relative to the 5-15 window:

    corpus R2 misses                                   4782
      attributable to the one-per-lesson shape         1626
      everything else                                  3156
        onlyEarly  every retrieval at distance 1-4     1620   51%
        never      no retrieval at any distance         640   20%
        straddles  some before 5, some after 15,
                   none inside                          599   19%
        onlyLate   first retrieval later than 15        297    9%
...
**onlyEarly (1620)** is the *same defect* as the tranche shape. The retrieval
...
Persian (58) are in here — five of the seven tracks the previous entry told the
next agent to skip.

**straddles (599)** is the genuine spacing gap: the course retrieves inside R1
...
**never (640)** is a different defect and no spacing rule touches it: the atom
...
**onlyLate (297)** is the reverse defect — nothing until after lesson 15. These
...
**A mid-window reach addresses 3845 of 4782 R2 misses** — the 1626 carrying the
tranche signature plus 1620 `onlyEarly` plus 599 `straddles`. That is 80% of the
debt, not the third the previous entry claimed. The 937 it cannot touch are 640
`never` and 297 `onlyLate`.

**Arabic is the only track that is genuinely mostly out of scope**: 123 of its
165 misses are `never`, so it needs atoms to be retrieved at all before spacing
them is a meaningful question. The other six tracks on the old do-not-apply list
are in scope and should be treated after the Indic tranches, since their debt is
...
only 58 `onlyEarly`. Hindi's 172 attributable atoms are still worth the PR, but
do not expect its total to fall the way Telugu's and Sanskrit's did.
