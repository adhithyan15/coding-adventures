...
> **LANG-FULL E4 — literal string `PRINT`, `string-length`, and `string=?` run on all 7 backends (v0.111.0):**
...
> `(string=? "HELLO" "HELLO")` returns exit code `1` on the same seven backends.
...
> Literal length/equality is folded/read from metadata on native/LLVM/WASM and
> uses host string APIs on JVM/CLR. Richer byte-string ops, non-literal equality,
> and string variables stay follow-up E4/BA4 slices.
