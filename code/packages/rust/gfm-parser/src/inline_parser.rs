...
//! on what precedes and follows the run. GFM Appendix A defines the
...
pub fn parse_inline(raw: &str, link_refs: &LinkRefMap) -> Vec<InlineNode> {
    ...
    while !scanner.done() {
        ...
        if ch == '*' || ch == '_' || (ch == '~' && scanner.peek_char(1) == '~') {
            flush_text(&mut tokens, &mut text_buf);
...
fn resolve_emphasis(mut tokens: Vec<Token>) -> Vec<InlineNode> {
    ...
    while i < tokens.len() {
        ...
        let use_len = if closer.ch == '~' { 2 } else if opener_count >= 2 && closer_count >= 2 { 2 } else { 1 };
        let is_strong = closer.ch != '~' && use_len == 2;
...
fn resolve_block(block: FinalBlock, link_refs: &LinkRefMap) -> Option<BlockNode> {
    match block {
        ...
        FinalBlock::List { ordered, start, tight, items } => {
            let list_items: Vec<ListChildNode> =
                items.into_iter().filter_map(|item| resolve_list_item(item, link_refs)).collect();
...
fn resolve_list_item(item: FinalBlock, link_refs: &LinkRefMap) -> Option<ListChildNode> {
    let FinalBlock::ListItem { mut children } = item else {
        return None;
    };

    if let Some(checked) = extract_task_item(&mut children) {
        let resolved_children = resolve_blocks(children, link_refs);
        return Some(ListChildNode::TaskItem(TaskItemNode {
            checked,
    ...
    let resolved_children = resolve_blocks(children, link_refs);
    Some(ListChildNode::ListItem(ListItemNode {
        children: resolved_children,
    }))
}

fn extract_task_item(children: &mut [FinalBlock]) -> Option<bool> {
    let Some(FinalBlock::Paragraph { raw_content }) = children.first_mut() else {
        return None;
    };
    ...
    *raw_content = rest.trim_start().to_string();
    Some(checked)
}

fn try_parse_table_block(raw: &str, link_refs: &LinkRefMap) -> Option<TableNode> {
    let parsed = try_parse_table(raw)?;

    let mut rows = Vec::with_capacity(1 + parsed.rows.len());
    rows.push(TableRowNode {
        is_header: true,
        children: normalize_table_row(parsed.header, parsed.align.len())
            .into_iter()
            .map(|cell| TableCellNode {
                children: parse_inline(&cell, link_refs),
            })
            .collect(),
    });
    ...
    for row in parsed.rows {
        rows.push(TableRowNode {
            is_header: false,
            children: normalize_table_row(row, parsed.align.len())
                .into_iter()
                .map(|cell| TableCellNode {
                    children: parse_inline(&cell, link_refs),
                })
...
struct ParsedTable {
    align: Vec<TableAlignment>,
    header: Vec<String>,
    rows: Vec<Vec<String>>,
}

fn try_parse_table(raw: &str) -> Option<ParsedTable> {
    ...
    let delimiter_cells = split_table_row(lines[1])?;
    ...
    for cell in &delimiter_cells {
        ...
        if trimmed.len() < 3 || !trimmed.chars().all(|ch| ch == ':' || ch == '-') {
            return None;
        }
        if trimmed.trim_matches(':').chars().count() < 3
            || !trimmed.trim_matches(':').chars().all(|ch| ch == '-')
        {
            return None;
        }
...
fn split_table_row(line: &str) -> Option<Vec<String>> {
    ...
    let had_outer_pipe = stripped.starts_with('|') || stripped.ends_with('|');
    ...
    while end > start {
        let ch = line[..end].chars().last().unwrap();
        if !is_ascii_whitespace(ch) {
            break;
        }
        end -= ch.len_utf8();
    }
    if start < end && line[start..].starts_with('|') {
        start += 1;
    }
    if end > start && line[..end].ends_with('|') {
        end -= 1;
    }
