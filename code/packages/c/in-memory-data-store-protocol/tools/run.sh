#!/bin/sh
# Build and run the in-memory-data-store-protocol tests under EVERY available
# C/C++ compiler (pure ISO), via the shared iso-harness (code/packages/c/iso-harness).
set -e
...
iso_build_and_run c imds_protocol-tests tests/imds_protocol_test.c src/imds_protocol.c
