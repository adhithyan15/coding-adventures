...
- Added five adversarial graph and diff-selection cases for the empty graph,
  ...
  Tightened cycle evidence to exactly one stable diagnostic. The corpus now
  has 162 cases and retains 285 staged files.
- Defined canonical graph edge ordering and added unsorted-edge,
  ...
  authority. The corpus then had 157 cases and retained 285 staged files.
- Added a process-free CI-gate character-class case that distinguishes the
  ...
  ranges, and unmatched `[` literals. The corpus now has 154 cases.
- Added exact-at-limit and over-limit process-free CI-gate selection cases for
  ...
  Unicode-scalar accounting across two gates, full pattern/file preflight even
  when the first pair matches, zero matcher calls on rejection, and stable
  empty-result `CI_GATE_MATCH_LIMIT_EXCEEDED` failure.
  The then-153-case corpus kept the empty fixture workspaces and
...
- Added nine source-collection cases for Haskell, Lua, Python, Rust, TypeScript,
  Starlark, human-language, cross-language workspace, and VisiCalc inputs,
  including exact, descendant, exclusion, generated-pruning, and inert
  registered-path symlink/reparse behavior plus a tracked-repository projection
  regression and domain-separated boundary digest validation.
...
- Added the toolchain-declaration CRLF grammar boundary case. It accepts a
  ...
  declarations ending in a lone CR, a CR followed by trailing ASCII space or
  tab, or doubled CR before LF.
