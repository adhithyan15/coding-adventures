...
fn seeded_state(cpu: &Rv32ISimulator, name: &str, raw: u32, seed: usize) -> Rv32IState {
...
fn python_full_state_corpus_matches() {
        ...
        .filter(|line| !line.starts_with('#'))
    {
        ...
        let mut cpu = Rv32ISimulator::new();
        cpu.load_checked(&raw.to_le_bytes())
            .expect("valid raw program");
        cpu.restore(&seeded_state(&cpu, name, raw, seed))
            .expect("valid seeded state");
        cpu.step_checked()
    ...
    assert_eq!(count, 256, "corpus must retain every generated vector");
}
