...
def _parse_index_columns(sql: str) -> list[str]:
    ...
    Locates the parenthesised column list and splits on commas.
    ...
    Raises :class:`ValueError` if the SQL contains no parentheses (should
    never happen for well-formed ``CREATE INDEX`` SQL generated by this
    module).
    """
    start = sql.index("(") + 1
    end = sql.rindex(")")
    return [col.strip() for col in sql[start:end].split(",") if col.strip()]


def _columns_to_index_sql(name: str, table: str, columns: list[str]) -> str:
    """Serialise an index definition as a ``CREATE INDEX`` statement.

    Generates the canonical SQL string stored in ``sqlite_schema``.
    The output is parseable by both :func:`_parse_index_columns` and by
    the real ``sqlite3`` CLI::

        _columns_to_index_sql("idx_orders_user_id", "orders", ["user_id"])
        # → "CREATE INDEX idx_orders_user_id ON orders (user_id)"
    """
    col_list = ", ".join(columns)
    return f"CREATE INDEX {name} ON {table} ({col_list})"
