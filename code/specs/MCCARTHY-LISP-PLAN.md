...
| **L3b-2** | **Symbols** (`make_symbol` / interning) in the native + other backends — the gap blocking the literal `(CAR '(A B C))` → `A`; plus `ATOM`/`EQ` (`pair?`/`not`/`equal?`). |
| **L3b-3** | The other `--emit` backends for cons (wasm/jvm/clr/beam already lower `alloc`/`field_*`; wire McCarthy's lowered IIR through them) + the per-`--emit` acceptance matrix below. |
