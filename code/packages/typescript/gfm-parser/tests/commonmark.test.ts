...
describe("GFM extensions", () => {
  it("parses strikethrough into Document AST", () => {
    const doc = parse("~~gone~~\n");
    const para = doc.children[0];
    expect(para?.type).toBe("paragraph");
    if (para?.type === "paragraph") {
  ...
  it("parses task list items", () => {
    ...
    expect(list?.type).toBe("list");
    if (list?.type === "list") {
      expect(list.children[0]).toMatchObject({ type: "task_item", checked: true });
      expect(list.children[1]).toMatchObject({ type: "task_item", checked: false });
    }
  });
  ...
  it("parses pipe tables into table nodes", () => {
    const doc = parse("| Name | Role |\n| :--- | ---: |\n| Ada | Math |\n");
    const table = doc.children[0];
    expect(table?.type).toBe("table");
    if (table?.type === "table") {
      expect(table.align).toEqual(["left", "right"]);
      expect(table.children[0]).toMatchObject({ type: "table_row", isHeader: true });
      expect(table.children[1]).toMatchObject({ type: "table_row", isHeader: false });
    }
  });

  it("renders pipe tables to HTML", () => {
    expect(toHtml(parse("| Name | Role |\n| :--- | ---: |\n| Ada | Math |\n"))).toBe(
