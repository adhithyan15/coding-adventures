...
% canonical-source-hash: fnv1a64:89df17ddc6babed1
% canonical-lessons: TE-S01-letter-ta, TE-S01-copy-in-a-word, TE-S01-delayed-copy, TE-S01-dictation, TE-C05-maatlaadu, TE-S147-digit-8, TE-C05-nenu-telugu-maatlaadataanu, TE-S111-letter-sa, TE-S158-letter-ssa, TE-C05-undu, TE-C05-hamsa, TE-S125-letter-ha, TE-C05-pani-ceyu, TE-S148-digit-9, TE-C05-practice
...
\end{tcolorbox}
\section[ta]{\te{త} — copy it where it lives}

\label{lesson:TE-S01-copy-in-a-word}



\begin{quote}
Find \textbf{\te{త}} inside \textbf{\te{సంతోషం}} — \emph{santōṣaṁ} — and put your finger on it.
\end{quote}

\subsection*{Writing — guided copy}
The model stays on the page. Copy \textbf{\te{త}} once on its own, then copy it a second time \textbf{with the letter that follows it in the word}, keeping the two the same size and the same distance apart as the printed pair.

That second copy is the whole lesson. A letter traced alone teaches your hand one shape; a letter copied beside its neighbour teaches it where the shape has to stop, and a hand that has only ever drawn letters alone writes them too large and too far apart the first time it writes a word.

Look back at the model as often as you want. Nothing is hidden today.

\begin{tcolorbox}[breakable,colback=teal!4,colframe=teal!35!black,title={Before you move on}]
Hold your two copies next to the printed word. Is the gap you left the same one the book left? That gap is not spacing — it is the difference between two letters and one word.
\end{tcolorbox}
\section[ta]{\te{త} — write it with the model covered}

\label{lesson:TE-S01-delayed-copy}



\begin{quote}
Look at \textbf{\te{త}} once and say \emph{ta}. Then cover it, and every other \textbf{\te{త}} on the page.
\end{quote}

\subsection*{Writing — delayed copy}
With the model covered, wait ten seconds, then write \textbf{\te{త}} once from memory.

Ten seconds is not long and is not meant to be. It is long enough that your hand cannot be tracing a shape it can still see, and short enough that a letter you met a minute ago is still there to be found.
...
\begin{tcolorbox}[breakable,colback=teal!4,colframe=teal!35!black,title={Before you move on}]
The thing to look at is not whether your letter is beautiful. It is whether it is the \textbf{same letter}, and in this script that usually comes down to one curve closing where the model's closes, or staying open where the model's stays open. A hand drawing from memory rounds off what it is unsure about.
\end{tcolorbox}
\section[ta]{\te{త} — the letter from the sound}

\label{lesson:TE-S01-dictation}



\begin{quote}
Cover every \textbf{\te{త}} on the page. Nothing to copy and nothing to uncover.
\end{quote}

\subsection*{Writing — dictation}
Say \emph{ta} aloud, or play it, and write the letter that carries it. Once.

This is a different job from the last lesson even though the letter is the same. A delayed copy asks your hand to remember a shape. Dictation asks it to turn a \textbf{sound} into a shape, which is the thing writing is actually for, and the two do not always arrive together.

\begin{tcolorbox}[breakable,colback=teal!4,colframe=teal!35!black,title={Before you move on}]
Uncover the model and compare — and then notice what you were asked to write.

Not \emph{t}. \textbf{ta}. The vowel is already inside the letter and nobody wrote it, which is the fact about this script that the first three lessons showed you and this one makes you act on. Hearing \emph{ta} and writing one shape is the whole difference between an alphabet and what you are learning.
\end{tcolorbox}
...
\end{tcolorbox}
\section[ṣa]{\te{ష} — the second of the s letters}

\label{lesson:TE-S158-letter-ssa}



\begin{quote}
Before the new one: \te{స} — what sound does it carry, and which word from chapter two opens with it?

That word carries a second s-shape as well, further along. Today's character is the second one.
\end{quote}

\subsection*{Script you'll notice: \te{ష}}
\textbf{\te{ష}} — \emph{ṣa}.
...
\begin{itemize}
\raggedright
  \item \textbf{\te{సంతోషం}} \emph{santōṣam} — joy; and the way you say you are pleased to meet somebody (santōṣam — \textquotedblleft{}gladness\textquotedblright{})
\end{itemize}

Look at that word end to end. It opens on \te{స} and closes on \te{ష}, with the \emph{o} sign between them. \textbf{Two different s-shapes inside one short word}, and until now you could read the first and not the second.

Your tongue makes them in two different places. For \te{స} the tip touches the ridge behind your teeth. For \te{ష} it curls back along the roof of your mouth, the same backward curl you already make for \te{ట}. The romanization marks the difference with one dot under the letter — \emph{s} against \emph{ṣ} — which is easy to miss in print and impossible to miss on the page, where they are two unlike shapes.

\subsection*{Writing: \te{ష} — copy what you see}
Put your pen on \te{ష} and follow its line. Copy the shape you can see — slowly, and larger than it is printed. Then write \te{స} beside it and look at the two.
...
\begin{itemize}
\raggedright
  \item \emph{Look:} at the word below and put a finger on each of its two s-shapes in turn
\end{itemize}
...
\begin{itemize}
\raggedright
  \item \emph{Trace it:} \te{ష} three times, saying \emph{ṣa} as you finish each one
  \item \emph{Say it:} which of the two has your tongue curled back
\end{itemize}

\begin{tcolorbox}[breakable,colback=teal!4,colframe=teal!35!black,title={Before you move on}]
Which character is this — \te{ష}? What sound does it carry? (\emph{\textbf{ṣa}}.) Which end of \textbf{\te{సంతోషం}} does it sit at? (\textbf{The far end} — \te{స} opens the word, \te{ష} closes it.) Which other letter puts your tongue in the same curled place? (\textbf{\te{ట}}.)
\end{tcolorbox}
...
\end{tcolorbox}
\section[haṃsa]{\te{హంస} (haṃsa) — a swan}

\label{lesson:TE-C05-hamsa}
...
\subsection*{You'll want to know: \te{హంస}}
\textbf{\te{హంస}} — \emph{haṃsa} — \textquotedblleft{}a swan\textquotedblright{}.

Say it: \emph{haṃsa}. Look at its shape as you say it: the next lesson takes one piece of it, \textbf{\te{హ}}, and writes it on its own.
...
\raggedright
  \item \emph{haṃsa}
  \item \emph{haṃsa}, and what it means
\end{itemize}

\begin{tcolorbox}[breakable,colback=teal!4,colframe=teal!35!black,title={Before you move on}]
What is \textquotedblleft{}a swan\textquotedblright{}? (\textbf{\te{హంస}}, \emph{haṃsa}.)
\end{tcolorbox}
