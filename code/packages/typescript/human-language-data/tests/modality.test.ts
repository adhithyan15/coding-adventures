...
describe("corpus regression", () => {
  ...
  it("pins the core as strictly better than the whole, now that writing segments exist", () => {
    ...
    expect(summary.lessonsWithWritingSegments).toBe(136); // HL-C251: +3 -- the migrated russian ch1 lessons declare script blocks the modality pass can now see // HL-C230: UNCHANGED, and that is the interesting part. The chapter adds five writing lessons, and not one registers here: this counter measures word lessons carrying a DETACHABLE writing SEGMENT, not lessons that are wholly about writing. Chinese chapter 3 puts the strokes in their own `type: writing` lessons instead of hanging them off the word lessons, so there is nothing to detach // HL12: +30 recognition segments (telugu/kannada/malayalam 8 each, sanskrit 6). Each carries a detachable tracing block, so the hands-free view keeps the rest of the lesson // HL12 payment two: +8 Hindi segments // HL-C154: Tamil's letter ledger completed — 15 more one-character segments, 24/24 positions taught // HL-C156: letter ledgers replicated to all six — 85 one-character segments — every segment carries a detachable tracing block // HL-C168: +1 -- Kannada's ledger closes at 24 of 24
