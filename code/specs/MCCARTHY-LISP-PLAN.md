...
| **L3b** | Lower the `lispy-runtime` value model (symbol interning + heap cons cells, and the `cons`/`car`/`cdr`/`pair?`/`not`/`equal?` builtins) into the AOT/native + other backends, so symbol/cons-returning programs like `(CAR '(A B C))` → `A` emit a non-trivial artifact on every target (the per-`--emit` acceptance matrix below). |
