...
my $M = 'CodingAdventures::Blake2b';

# Helper: bytes_from_range(a, b) -> string of chr(a & 0xFF), chr((a+1) & 0xFF), ... chr((b-1) & 0xFF)
sub bytes_from_range {
    my ($start, $stop) = @_;
    return join('', map { chr($_ & 0xff) } $start .. $stop - 1);
}

# ---------------------------------------------------------------------------
# Meta / version
# ---------------------------------------------------------------------------
is($M->VERSION, '0.01', 'has VERSION 0.01');
...
is(
    CodingAdventures::Blake2b::blake2b_hex(""),
...
is(
    CodingAdventures::Blake2b::blake2b_hex("abc"),
    "ba80a53f981c4d0d6a2797b69f12f6e94c212f14685ac4b74b12bb6fdbffa2d1"
  . "7d87c5392aab792dc252d5de4533cc9518d38aa8dbf1925ab92386edd4009923",
    'abc (RFC 7693 Appendix A)'
);

is(
    CodingAdventures::Blake2b::blake2b_hex("The quick brown fox jumps over the lazy dog"),
...
is(
    CodingAdventures::Blake2b::blake2b_hex("", digest_size => 32),
    "0e5751c026e543b2e8ab2eb06099daa1d1e5df47778f7787faab45cdf12fe3a8",
    'truncated digest_size=32 of empty'
);

# Keyed long vector: key = bytes 1..64, data = bytes 0..255
{
    my $key = bytes_from_range(1, 65);
    my $data = bytes_from_range(0, 256);
    is(
        CodingAdventures::Blake2b::blake2b_hex($data, key => $key),
...
for my $kat (@BLOCK_KATS) {
    my ($size, $want) = @$kat;
    my $data = join('', map { chr(($_ * 7 + 3) & 0xff) } 0 .. $size - 1);
    is(
        CodingAdventures::Blake2b::blake2b_hex($data),
        $want,
        "block-boundary size $size",
    );
}
...
for my $kat (@DIGEST_SIZE_KATS) {
    my ($ds, $want) = @$kat;
    my $data = "The quick brown fox jumps over the lazy dog";
    my $out = CodingAdventures::Blake2b::blake2b($data, digest_size => $ds);
    is(length($out), $ds, "digest_size $ds byte length");
    is(unpack("H*", $out), $want, "digest_size $ds hex");
}
...
for my $kat (@KEYED_KATS) {
    my ($klen, $want) = @$kat;
    my $key = bytes_from_range(1, $klen + 1);
    is(
        CodingAdventures::Blake2b::blake2b_hex(
            "secret message body", key => $key, digest_size => 32,
        ),
        $want,
        "keyed keyLen $klen",
    );
...
{
    my $salt = bytes_from_range(0, 16);
    my $personal = bytes_from_range(16, 32);
    is(
        CodingAdventures::Blake2b::blake2b_hex(
            "parameterized hash", salt => $salt, personal => $personal,
        ),
...

{
    ...
    is($h->hex_digest, CodingAdventures::Blake2b::blake2b_hex("hello world"),
       'streaming single chunk matches one-shot');
}

{
    my $data = bytes_from_range(0, 200);
    my $h = $M->new(digest_size => 32);
    for my $i (0 .. length($data) - 1) {
        $h->update(substr($data, $i, 1));
    }
    is($h->hex_digest,
       CodingAdventures::Blake2b::blake2b_hex($data, digest_size => 32),
       'streaming byte-by-byte matches one-shot');
}

{
    # Across the 128-byte block boundary.
    my $data = bytes_from_range(0, 129);
    my $h = $M->new;
    $h->update(substr($data, 0, 127));
    $h->update(substr($data, 127));
    is($h->hex_digest, CodingAdventures::Blake2b::blake2b_hex($data),
       'streaming across block boundary');
}

{
    # Classic exact-block-then-more off-by-one.
    my $data = join('', map { chr($_ & 0xff) } 0 .. 131);
    my $h = $M->new;
    $h->update(substr($data, 0, 128));
    $h->update(substr($data, 128, 4));
    is($h->hex_digest, CodingAdventures::Blake2b::blake2b_hex($data),
       'streaming exact-block-then-more');
}

{
    my $h = $M->new;
    $h->update("hello");
    is($h->hex_digest, $h->hex_digest, 'digest is idempotent');
}

{
    ...
    $h->update("world");
    is($h->hex_digest,
       CodingAdventures::Blake2b::blake2b_hex("hello world", digest_size => 32),
       'update after digest continues stream');
}

{
    ...
    $c->update("path B");
    is($h->hex_digest,
       CodingAdventures::Blake2b::blake2b_hex("prefix path A"),
       'copy: original continues independently');
    is($c->hex_digest,
       CodingAdventures::Blake2b::blake2b_hex("prefix path B"),
       'copy: copy advances independently');
}
...
like(
    dies { CodingAdventures::Blake2b::blake2b("", digest_size => 0) },
    qr/digest_size/,
    'rejects digest_size 0',
);
like(
    dies { CodingAdventures::Blake2b::blake2b("", digest_size => 65) },
    qr/digest_size/,
    'rejects digest_size 65',
);
like(
    dies { CodingAdventures::Blake2b::blake2b("", key => ("a" x 65)) },
    qr/key length/,
    'rejects key length > 64',
);
like(
    dies { CodingAdventures::Blake2b::blake2b("", salt => ("a" x 8)) },
    qr/salt/,
    'rejects salt of wrong length',
);
like(
    dies { CodingAdventures::Blake2b::blake2b("", personal => ("a" x 20)) },
    qr/personal/,
    'rejects personal of wrong length',
);

# Accepts maximum 64-byte key without dying.
ok(
    lives { CodingAdventures::Blake2b::blake2b("x", key => ("\x41" x 64)) },
    'accepts 64-byte key',
);
