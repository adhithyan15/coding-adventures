namespace CodingAdventures.Argon2i.FSharp
...
/// Optional Argon2i inputs from RFC 9106.
type Argon2iOptions =
    {
        Key: byte array
...
/// Pure F# Argon2i password hashing as specified by RFC 9106.
...
module Argon2i =
    ...
    /// Compute an Argon2i tag with empty key and associated data.
    let deriveDefault password salt timeCost memoryCost parallelism tagLength =
        derive password salt timeCost memoryCost parallelism tagLength Argon2iOptions.Default

    /// Compute an Argon2i tag and return lowercase hexadecimal.
    ...
    /// Compute a default-option Argon2i tag and return lowercase hexadecimal.
    let deriveHexDefault password salt timeCost memoryCost parallelism tagLength =
        deriveHex password salt timeCost memoryCost parallelism tagLength Argon2iOptions.Default
