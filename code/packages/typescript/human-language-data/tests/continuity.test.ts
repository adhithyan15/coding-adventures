...
describe("the real corpus", () => {
  ...
  it("pins what nothing had measured", { timeout: 30_000 }, () => {
    ...
    // R1 stops at 3.
    expect(report.summary.missedByWindow.R1).toBe(922); // +2: HL-C98 // +2: vocabulary wave 5 // +4: HL-C88 slices 5-6 // +1: HL-C112 moves casa and libro 14 chapters earlier, so one atom's next re-use now falls outside R1 // +12: vocabulary wave 6
    ...
    expect(report.summary.missedByWindow.R2).toBe(2099); // +1: ES-C02-concordancia (HL-C85). buenos dias/buenas tardes stop REQUIRING the agreement rule; it becomes a payoff lesson after the learner has used all three greetings. // +84: vocabulary wave 5, new pre-A1 nouns landing late in already-long chapters // +4: HL-C88 slices 5-6 // +1: HL-C112, same cause as R1 above // +1: HL-C88 slice 8 // +87: vocabulary wave 6
  });
