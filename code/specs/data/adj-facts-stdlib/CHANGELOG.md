...
- `earth-science/water-cycle.adj` — **a false claim deleted, and an envelope that
  ...
  **The page does not state the step numbers.** The second column is an INTEGER, and no
  ...
  stage. Measured 2026-09-14 with a control that can fail: the ordinal pattern matches
  synthetic probes **3 of 3**, so a zero from it is a real zero; sentences naming a stage *and*
  an ordinal or step number: **0**; sentences containing any digit at all: **3** (*"oceans
  cover 70% of the Earth's surface"*, a CSS `font-size` rule, and the footer URL), none of
  which numbers a stage; and the words `order`, `begins`, `next`, `follow`: **0 occurrences**.
  ...
  **A false claim is deleted.** The header said the page "names these stages and describes them
  in this process order". It does not: the page's own `<h3>` sections run Condensation →
  Evaporation → Groundwater → Precipitation → Runoff → The Sun, which is **alphabetical**
  (sorting that list returns it unchanged), and is not the table's order. The *handoff* half of
  the claim is true and is kept; the *ordering* half is gone.
  ...
  **A mutant survived, and it is the finding, not a footnote.** Renumbering `runoff` from 4 to
  ...
  artifact pin and not a warrant. 9 of 9 mutants killed, green baseline before and after (local
  scratch harness, so that count is not reproducible from the repo).
