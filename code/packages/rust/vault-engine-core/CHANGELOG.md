...
- `MintContextSummary` and `MintContext::summary()` for value-redacted
  request-shape summaries that report path/input/CAS presence and byte counts
  without cloning principal, path, CAS token, or plaintext values.
