...
Scope, so the next reader does not over-read this: Qt is the only host with a
shape check at all. The SwiftUI, Compose, Flutter and Electron hosts normalise
instead — they trim and dot-prefix whatever arrives — so this fix has no twin to
apply there today, and `QRegularExpression` appears exactly once in the tree.
The divergence it leaves is that Qt now *rejects* `".apkg\n"` where the others
*accept* it by trimming. Identical behaviour with today's producers, which send
static literals, but a real source of host-specific filter differences if that
