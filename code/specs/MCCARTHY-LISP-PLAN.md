...
>     - **L2c-3c** — *capture for `LABEL` + `LABEL`-as-value (recursive closures).*  A labelled body captures enclosing free variables too (its `functions_in_scope` entry records the captured names so a recursive `(F …)` forwards them); a `LABEL` in value position becomes a recursive closure value.
...
| **L2c-3c** | Closures, part 3 — capture for `LABEL` + `LABEL`-as-value (recursive closures). |
