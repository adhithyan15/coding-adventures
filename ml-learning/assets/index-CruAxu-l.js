...
`)}_emitBlockItem(e,t){return e.children.length>0?this._emitNode(e.children[0],t):``}_emitDeclarationOrNested(e,t){return e.children.length>0?this._emitNode(e.children[0],t):``}_emitDeclaration(e,t){let n=``,r=``,i=``;for(let t of e.children){if(!Dm(t))continue;let e=t;e.ruleName===`property`?n=this._emitProperty(e,0):e.ruleName===`value_list`?r=this._emitValueList(e,0):e.ruleName===`priority`&&(i=` !important`)}return this.minified?`${n}:${r}${i};`:`${n}: ${r}${i};`}_emitProperty(e,t){return e.children.length>0?e.children[0].value:``}_emitPriority(e,t){return`!important`}_emitValueList(e,t){let n=[];for(let r of e.children){let e=this._emitNode(r,t);n.push(e)}let r=n.join(` `);return r=r.replace(/ , /g,`, `).replace(/ ,/g,`,`),r}_emitValue(e,t){let n=e.children;if(n.length===1){let e=n[0];if(!Dm(e)){let t=e;return Om(t)===`STRING`?`"${t.value}"`:t.value}return this._emitNode(e,t)}return this._emitDefault(e,t)}_emitFunctionCall(e,t){let n=e.children;if(n.length===1)return n[0].value;let r=[];for(let e of n)if(Dm(e))r.push(this._emitNode(e,t));else{let t=e,n=Om(t);n===`FUNCTION`?r.push(t.value):n===`RPAREN`?r.push(`)`):r.push(t.value)}return r.join(``)}_emitFunctionArgs(e,t){let n=[];for(let r of e.children)n.push(this._emitNode(r,t));let r=n.join(` `);return r=r.replace(/ , /g,`, `).replace(/ ,/g,`,`),r}_emitFunctionArg(e,t){let n=e.children;if(n.length===1){let e=n[0];return Dm(e)?this._emitNode(e,t):e.value}let r=[];for(let e of n)if(Dm(e))r.push(this._emitNode(e,t));else{let t=e;r.push(Om(t)===`RPAREN`?`)`:t.value)}return r.join(``)}_emitDefault(e,t){let n=[];for(let r of e.children)n.push(this._emitNode(r,t));return n.join(` `)}};function Am(e,t={}){let n=Hf(e),r=new Em().transform(n);return new km(t.indent??`  `,t.minified??!1).emit(r)}function jm(e,t={}){return Am(e,t)}var Mm=`$paper: #f7f8f3;
$ink: #17201c;
$muted: #5d6d68;
...
`,Nm=`coding-adventures-lattice-styles`;function Pm(){if(document.getElementById(Nm)===null)try{let e=document.createElement(`style`);e.id=Nm,e.textContent=jm(Mm),document.head.append(e)}catch(e){console.error(`Failed to install Lattice styles`,e)}}Pm(),(0,u.createRoot)(document.getElementById(`root`)).render((0,E.jsx)(l.StrictMode,{children:(0,E.jsx)(xf,{})}));