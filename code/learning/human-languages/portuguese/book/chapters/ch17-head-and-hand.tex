...
\begin{itemize}
\raggedright
  \item {[}YOU SAY: \textquotedblleft{}a mão, as mãos\textquotedblright{}{]}
  \item {[}YOU SAY: the change — \textquotedblleft{}ma\textbf{n}us … mã\textbf{o}\textquotedblright{}{]}
  \item {[}YOU SAY: the family — \textquotedblleft{}lua, boa, pão — all lost an \textbf{n}\textquotedblright{}{]}
  \item {[}YOU SAY: the contrast — \textquotedblleft{}Spanish \emph{mano}, Portuguese \emph{mão}\textquotedblright{}{]}
\end{itemize}

\begin{tcolorbox}[breakable,colback=teal!4,colframe=teal!35!black,title={Wrap-up recall}]
{[}PAUSE 3s{]} Say \textquotedblleft{}the hand\textquotedblright{} and \textquotedblleft{}the hands.\textquotedblright{} (\emph{A mão} / \emph{as mãos}.) What happened to the \emph{n} of \emph{manus}? (It \textbf{dissolved between vowels}, leaving a nasal vowel — which the \textbf{til} marks.) Name two other words that did the same. (\emph{Lua} $\leftarrow$ \emph{lūna}, \emph{boa} $\leftarrow$ \emph{bona}, \emph{pão} — Ch. 11.) Did they all keep the nasality? (\textbf{No} — \emph{mão} and \emph{pão} did; \emph{lua} and \emph{boa} lost it again.) What did Spanish do? (\textbf{Kept the \emph{n}} in all — \emph{mano}, \emph{luna}, \emph{pan}.) Why is \emph{mão} feminine? (Latin \emph{manus} was \textbf{fourth-declension feminine} — the same reason Italian says \emph{la mano}.) Next: the rest of the body.
\end{tcolorbox}
