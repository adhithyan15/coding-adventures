...
Audit base: `cd73f3ad86` (2026-09-05); corpus counts updated for VM-047b, VM-057, VM-047c, VM-039b, VM-061, VM-042 and VM-040 (COBOL BEAM boolean/EVALUATE, then string ops/reference modification, then reference-modification MOVE/trap). This is an inventory of the implemented
...
| COBOL-60 | 58 | 434 | 28 of those cells are BEAM (VM-040 COBOL BEAM slices); much larger frontend JIT/oracle suite |
...
The normal non-ALGOL capstone therefore declares 210 programs and 1563
...
1559; this slice (VM-040 COBOL BEAM reference-modification MOVE/trap) added
four more real Beam cells to COBOL-60 once more (430 → 434 declared), so a
fresh run on a host with `erl` now reports 1353 exercised + 210 skipped =
1563; all fifteen new cells since VM-061 are exercised, not skipped, since
`erl` was present when they were promoted. The "Declared cells" column counts every backend a
