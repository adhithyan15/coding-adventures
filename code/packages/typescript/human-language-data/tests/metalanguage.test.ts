...
describe("the committed corpus", () => {
  it("pins the first measurement", () => {
    ...
    expect(r.summary.usesBeforeIntroduction).toBe(9138); // +5: ES-C03-vos and the two practice edits use ordinary terms (word, plural, ending) // +15: HL-C98's four lessons use ordinary terms (ending, verb, form) // +112: vocabulary wave 5 // +17: HL-C88 slices 5-6 // +11: HL-C88 slice 8 // +235: vocabulary wave 6 // HL-C113: grammar chapters cite tense and person names heavily, so these move in steps of ten rather than ones // HL11: +23. The nine letter segments talk about letters -- vowel, consonant, baseline, stroke, abugida -- and none of those terms is introduced first. Exactly the debt HL10 section 7.5 exists to burn down, now visible in new content rather than only in old // HL12: +63. The 30 recognition segments talk about letters -- consonant, vowel sign, abugida, nasal -- and none of those terms is introduced first. Exactly the debt HL10 section 7.5 exists to burn down, and it now shows up in four more tracks // HL12 payment two: +22, Hindi's eight segments using the same untaught terms
    expect(r.summary.technicalUsesBeforeIntroduction).toBe(2744); // +4: HL-C98 // +26: vocabulary wave 5 // +7: HL-C88 slices 5-6 // +3: HL-C88 slice 8 // +49: vocabulary wave 6 // HL-C113: grammar chapters cite tense and person names heavily, so these move in steps of ten rather than ones
    expect(r.summary.technicalLessons).toBe(1362); // +4: HL-C98 // +21: vocabulary wave 5 // +4: HL-C88 slices 5-6 // +2: HL-C88 slice 8 // +27: vocabulary wave 6 // HL-C113: grammar chapters cite tense and person names heavily, so these move in steps of ten rather than ones

    expect(r.summary.worstTerms.slice(0, 2)).toEqual([
      { term: "verb", lessons: 920 }, // +4: HL-C98 // +2: HL-C99 // +8: vocabulary wave 5 // +3: HL-C88 slices 5-6 // +11: vocabulary wave 6 // +2: B1 si-condition rung // +3: HL-C113 preterite plural // +2: HL-C113 preterite close (strong plurals, review, synthesis) // +2: HL-C113 imperfect subjunctive // +3: HL-C113 unreal condition // HL-C113 step 7: +4 // HL-C128 step 3: +1
      { term: "noun", lessons: 461 }, // +1: ES-C02-concordancia names the noun it agrees with // +4: vocabulary wave 5 // +3: HL-C88 slices 5-6 // +1: HL-C88 slice 7 (ES-C09-ncia) // +1: HL-C88 slice 8 (-ario, review, synthesis) // +14: vocabulary wave 6 // HL-C113 step 6: +1 // HL-C128 step 2: +4 // HL-C128 step 3: +4
    ]);
  });
