...
| HL-C99 | Queued — found by HL-C98 | One verb per chapter (HL10 §5.7), and an etymology for every verb. | **12 of 19** Spanish verb-teaching chapters introduce more than one verb; chapters 43, 44 and 48 introduce **four** apiece. Each verb owes the reader an origin, so a four-verb chapter either dumps four etymologies or silently skips three. Separately, `beber`, `preguntar` and `tomar` are taught with **no etymology atom at all** — the only three of 42 verbs in that state. Split largest-first; the etymologies are the cheap half and can land independently. |
