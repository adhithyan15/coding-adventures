...
| T3c-2 | `semantic-ir` + backends | Fold string-concat (`+` on `Str`) and comparison operators into the resolver, and have each backend's emitter consult it (currently every operand is `Dynamic`, so all resolve to `RuntimeDispatch` — no change until a typed frontend or sized-int lowering supplies types). |
