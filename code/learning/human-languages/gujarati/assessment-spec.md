# Coding Adventures Gujarati Assessment
...
This specification names the assessment that the complete Gujarati book must
...
learner-facing name at each rung is **Coding Adventures Gujarati <Level>
...
a Gujarati awarding body. The Council of Europe also states that it does not
verify or validate an examination's claimed link to CEFR. Every target here is
therefore labelled `project-defined`.

The assessed variety is contemporary standard Gujarati written in Gujarati
script. The current track's taught forms define the initial model. Documented
regional forms receive credit when used consistently and when they satisfy the
task; a trained Gujarati reviewer adjudicates unfamiliar forms. Hindi or English
replacement is not silently accepted as Gujarati, while natural code-switching
is assessed only when a task explicitly licenses it.
...
3. range and control of Gujarati vocabulary and grammar;
4. Gujarati orthographic control for writing, or pronunciation and fluency for
   speaking.

Each dimension is scored 0–5, then scaled to the paper's points. A response that
is not meaningfully in Gujarati cannot earn task-fulfilment credit. Documented
regional vocabulary, morphology, pronunciation, and spelling variants are not
errors merely because they differ from the standard-variety model.

The 60% threshold is a transparent provisional project standard, not validation
evidence. Before release, at least eight qualified Gujarati educators or
...
- Listening recordings use Gujarati speakers. Words-per-minute bands below are
  form-assembly targets, not a claim that Gujarati has one natural speaking rate.
- Pre-A1 and A1 directions may also be supplied in a declared support language,
  so misunderstanding directions is not confused with Gujarati proficiency.
  From A2 onward directions are in Gujarati, with one unscored example.
...
|---|---:|---|
| reading | 10 | 6 script/word matches, 4 short phrase or notice matches, and 4 personal-detail selections; no text over 8 words |
