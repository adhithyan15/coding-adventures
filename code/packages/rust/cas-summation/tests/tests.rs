...
#[test]
fn phase87_four_sqrt_k_log6_k_over_k3_closes() {
    // √k×4 · log(k)^6 / k³: effective_x2=4; 2·3=6 > 4 → closes.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
...
#[test]
fn phase87_four_sqrt_k_log6_k_times_k_over_k4_closes() {
    // √k×4 · log(k)^6 · k / k⁴: effective_x2=4+2=6; 2·4=8 > 6 → closes.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
...
#[test]
fn phase87_four_sqrt_k_log6_k_times_k_over_k3_refused() {
    // √k×4 · log(k)^6 · k / k³: effective_x2=6; 2·3=6 not > 6 → refused (diverges).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
...
// ─── Phase 88 — Five-Sqrt × Six-Log × polynomial numerator ──────────────────

#[test]
fn phase88_five_sqrt_k_log6_k_over_k4_closes() {
    // √k×5 · log(k)^6 / k⁴: effective_x2=5; 2·4=8 > 5 → closes.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
...
#[test]
fn phase88_five_sqrt_k_log6_k_times_k_over_k4_closes() {
    // √k×5 · log(k)^6 · k / k⁴: effective_x2=5+2=7; 2·4=8 > 7 → closes.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
...
#[test]
fn phase88_five_sqrt_k_log6_k_times_k_over_k3_refused() {
    // √k×5 · log(k)^6 · k / k³: effective_x2=7; 2·3=6 not > 7 → refused (diverges).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
