...
enum StaticBodyAction<'a> {
    Assignment(StaticBodyAssignment<'a>),
    Conditional {
        condition: &'a GrammarASTNode,
        then_actions: Vec<StaticBodyAction<'a>>,
        else_actions: Vec<StaticBodyAction<'a>>,
    },
}
...
impl Compiler {
    ...
    /// Admit exact tracked real snapshots, including pure conditionals whose
    /// branches prove the same snapshot value. The snapshot must be integral
    /// and fit the existing bounded-unroll cap.
    fn static_nonnegative_tracked_real_snapshot_power_chain(
        ...
        self.static_nonnegative_power_chain_with(nodes, &|node| {
            if !self.exact_tracked_real_snapshot_operand(node)
                && !self.path_independent_tracked_real_snapshot_operand(node)
            {
                return None;
            }
    ...
    /// it proves an integral result within the bounded-unroll cap. Optionally,
    /// a pure conditional may participate when every runtime path proves the
    /// same value; calls and nested powers stay dynamic.
    ...
    fn exact_tracked_integral_exponent_expression(&self, node: &GrammarASTNode) -> bool {
        ...
        if let Some((condition, then_node, else_node)) = self.conditional_expression_parts(node) {
            return self.static_predicate_dependencies(condition).is_some()
                && self.exact_tracked_integral_exponent_expression(then_node)
                && self.exact_tracked_integral_exponent_expression(else_node)
                && self
        ...
        let actuals = self.standard_fn_actuals(node);
        if actuals.len() != 1 || !self.exact_tracked_standard_function_operand(actuals[0]) {
            return false;
        }
        ...
        self.static_tracked_exponent_real_value(node)
            .is_some_and(|value| value.fract() == 0.0)
    }
    ...
    fn exact_tracked_standard_function_operand(&self, node: &GrammarASTNode) -> bool {
        if let Some((condition, then_node, else_node)) = self.conditional_expression_parts(node) {
            return self.static_predicate_dependencies(condition).is_some()
                && self.exact_tracked_standard_function_operand(then_node)
                && self.exact_tracked_standard_function_operand(else_node);
        }
        if let Some(child) = single_parenthesized_child(node) {
                ...
                .any(|piece| matches!(piece, Piece::Op(op) if op == "^" || op == "**"))
        {
            ...
            let exponent = literal_nonnegative_integral_arithmetic_power_chain(exponents)
                ...
                .or_else(|| {
                    self.static_nonnegative_tracked_real_arithmetic_power_chain(exponents, false)
                })
                .or_else(|| {
    ...
    fn path_independent_tracked_real_snapshot_operand(&self, node: &GrammarASTNode) -> bool {
        self.path_independent_tracked_real_snapshot_value(node)
            .is_some_and(|(_, has_snapshot)| has_snapshot)
    }

    fn path_independent_tracked_real_snapshot_value(
        &self,
        node: &GrammarASTNode,
    ) -> Option<(f64, bool)> {
        if let Some(child) = single_parenthesized_child(node) {
            return self.path_independent_tracked_real_snapshot_value(child);
        }
        if direct_tokens(node).is_empty() {
            let children = direct_nodes(node);
            if children.len() == 1 {
                return self.path_independent_tracked_real_snapshot_value(children[0]);
            }
        }
        if self.exact_tracked_real_snapshot_operand(node) {
            return self
                .static_tracked_numeric_value(node, false)
                .map(|value| (value, true));
        }
        if let Some(value) = expr_real_literal_text(node)
            .and_then(|literal| literal.parse::<f64>().ok())
            .filter(|value| value.is_finite())
        {
        ...
        let (then_value, then_has_snapshot) =
            self.path_independent_tracked_real_snapshot_value(then_node)?;
        let (else_value, else_has_snapshot) =
            self.path_independent_tracked_real_snapshot_value(else_node)?;
        (then_value.to_bits() == else_value.to_bits())
            .then_some((then_value, then_has_snapshot || else_has_snapshot))
    }
    ...
    fn exact_tracked_standard_integer_expression(&self, node: &GrammarASTNode) -> bool {
        if let Some(child) = single_parenthesized_child(node) {
            return self.exact_tracked_standard_integer_expression(child);
        }
        if node.rule_name != "proc_call" {
            let children = direct_nodes(node);
            return direct_tokens(node).is_empty()
                && children.len() == 1
                && self.exact_tracked_standard_integer_expression(children[0]);
        }
        let Some(name) = direct_tokens(node)
            .into_iter()
            .find(|token| token.effective_type_name() == "NAME")
    ...
    fn emit_for(&mut self, node: &GrammarASTNode) -> Result<(), CompileError> {
        ...
        for elem in elems {
            ...
            let (static_step_exit_real, static_step_exit_integer) = if is_step_element
                && !entry_tracking_disabled
            {
                self.for_step_exit_snapshot(target, var_ty, elem, body)
            } else {
                (None, None)
            };
    ...
    fn for_step_exit_snapshot(
        ...
        match target_ty {
            ScalarType::Integer => {
                ...
                let Some(delta) = iterations.checked_mul(step) else {
                    return (None, None);
                };
                let Some(exit) = start.checked_add(delta) else {
                    return (None, None);
                };
                (None, i64::try_from(exit).ok())
            }
            ScalarType::Real => {
    ...
    fn for_step_executes_exactly_once(
        ...
        match target_ty {
            ScalarType::Integer => {
                let (Some(start), Some(step), Some(limit)) = (
                    self.static_assigned_integer_value(values[0]),
                    self.static_assigned_integer_value(values[1]),
                    self.static_assigned_integer_value(values[2]),
                ) else {
                    return false;
                };
            ...
            ScalarType::Real => {
                let (Some(start), Some(step), Some(limit)) = (
                    self.static_assigned_real_value(values[0]),
                    self.static_assigned_real_value(values[1]),
                    self.static_assigned_real_value(values[2]),
                ) else {
                    return false;
                };
                let next = start + step;
    ...
    fn for_body_recurrence_actions<'a>(
        &self,
        body: &'a GrammarASTNode,
    ) -> Option<Vec<StaticBodyAction<'a>>> {
        let mut actions = Vec::new();
        self.collect_for_body_recurrence_actions(body, &mut actions)?;
        (!actions.is_empty()).then_some(actions)
    }

    fn collect_for_body_recurrence_actions<'a>(
        &self,
        node: &'a GrammarASTNode,
        actions: &mut Vec<StaticBodyAction<'a>>,
    ) -> Option<()> {
        match node.rule_name.as_str() {
            "statement" => {
                let children = direct_nodes(node);
                ...
                self.collect_for_body_recurrence_actions(child, actions)
            }
            "unlabeled_stmt" => {
                let children = direct_nodes(node);
                let [child] = children.as_slice() else {
            ...
            "compound_stmt" => {
                ...
                {
                    self.collect_for_body_recurrence_actions(statement, actions)?;
                }
                Some(())
            }
            "dummy_stmt" => Some(()),
            "assign_stmt" => {
                ...
                let name = self.simple_variable_name(variable).ok()?;
                let expression = first_direct_node(node, "expression")?;
                let binding = self.require_var(&name).ok()?;
                if binding.is_global
                    || binding.array.is_some()
                    || self.active_by_name_binding(&name).is_some()
                ...
                actions.push(StaticBodyAction::Assignment(StaticBodyAssignment {
                    name,
            ...
            "cond_stmt" => {
                ...
                self.collect_for_body_recurrence_actions(then_branch, &mut then_actions)?;
                let mut else_actions = Vec::new();
                if let Some(else_branch) = branches.get(1) {
                    self.collect_for_body_recurrence_actions(else_branch, &mut else_actions)?;
                }
                if !then_actions.is_empty() || !else_actions.is_empty() {
                    actions.push(StaticBodyAction::Conditional {
                        condition,
                        then_actions,
                        else_actions,
                    });
    ...
    fn evaluate_static_body_actions(
        &mut self,
            ...
            match action {
                StaticBodyAction::Assignment(assignment) => {
                    ...
                    if let Some((_, snapshot)) = snapshots
                        .iter_mut()
                        .find(|(slot, _)| slot == &assignment.slot)
                    {
                        *snapshot = value;
                    } else {
                        snapshots.push((assignment.slot.clone(), value));
                    }
                }
        ...
        let exponent = self
            ...
            .or_else(|| {
                (base.ty == ScalarType::Real)
                    .then(|| {
                        self.static_nonnegative_tracked_integer_expression_power_chain(
                            exponent_nodes,
                        )
                    })
                    .flatten()
...
const MAX_STATIC_SELECTOR_DEPENDENCY_DEPTH: usize = 10;
    ...
    if literal_nonnegative_integral_power_chain(exponents) == Some(0)
        && (literal_integer_value(base).is_some()
            || expr_real_literal_text(base)
                .and_then(|literal| literal.parse::<f64>().ok())
                .is_some_and(f64::is_finite)
            || expr_static_real_arithmetic_value_with(base, &|_| None).is_some())
    {
        return Some(false);
    }
...
mod tests {
    ...
    #[test]
    fn al4_tracked_integer_standard_function_exponents_unroll_real_powers() {
        for source in [
            "begin integer exponent; real saved; exponent := -2; saved := 6.0 ^ abs(exponent) + 6.0; exponent := 3; output(saved + 0.5) end",
            "begin integer exponent; real saved; exponent := 2; saved := 6.0 ^ sign(exponent) + 36.0; exponent := 3; output(saved + 0.5) end",
        ] {
    ...
    #[test]
    fn al4_path_independent_conditional_tracked_sqrt_exponents_unroll_real_powers() {
        let module = compile_source(
            "begin integer gate, exponent; real saved; exponent := 4; saved := 6.0 ^ (if gate = 0 then sqrt(exponent) else sqrt(exponent)) + 6.0; exponent := 9; if saved = 42.0 then output(42) else output(1) end",
            "test",
        )
        .expect("equal conditional square-root exponents should unroll");
        let main = module.get_function("main").expect("has main");
        assert!(main.instructions.iter().any(|instr| instr.op == "jmp_if_false"));
        assert!(main.instructions.iter().all(|instr| instr.op != "f64_pow"));
        assert!(main.instructions.iter().any(|instr| instr.op == "mul"));
    ...
    #[test]
    fn al4_conditional_tracked_sqrt_exponents_retain_unsafe_runtime_power() {
        for source in [
            "begin integer gate, exponent; real saved; exponent := 4; saved := 6.0 ^ (if gate = 0 then sqrt(exponent) else sqrt(exponent) + 1) end",
            "begin integer gate, exponent; real saved; exponent := 2; saved := 6.0 ^ (if gate = 0 then sqrt(exponent) else sqrt(exponent)) end",
            "begin boolean procedure choose; choose := true; integer exponent; real saved; exponent := 4; saved := 6.0 ^ (if choose() then sqrt(exponent) else sqrt(exponent)) end",
            "begin real procedure sqrt(x); value x; integer x; sqrt := 2.0; integer gate, exponent; real saved; exponent := 4; saved := 6.0 ^ (if gate = 0 then sqrt(exponent) else sqrt(exponent)) end",
        ] {
            let module = compile_source(source, "test")
                .expect("unsafe conditional square-root exponents must remain dynamic");
            let main = module.get_function("main").expect("has main");
            assert!(main.instructions.iter().any(|instr| instr.op == "f64_pow"));
            if source.contains("procedure choose") || source.contains("procedure sqrt") {
                assert!(main.instructions.iter().any(|instr| instr.op == "call"));
            }
        }
    }
    ...
    #[test]
    fn al4_nested_exact_tracked_sqrt_exponents_unroll_real_powers() {
        let module = compile_source(
            "begin integer exponent; real saved; exponent := 16; saved := 6.0 ^ sqrt(sqrt(exponent)) + 6.0; exponent := 81; if saved = 42.0 then output(42) else output(1) end",
            "test",
        )
        .expect("nested exact integral square-root exponents should unroll");
        let main = module.get_function("main").expect("has main");
        assert!(main.instructions.iter().all(|instr| instr.op != "f64_pow"));
        assert!(main.instructions.iter().any(|instr| instr.op == "mul"));
    }
    ...
    #[test]
    fn al4_nested_tracked_sqrt_exponents_retain_unsafe_runtime_power() {
        for source in [
            "begin integer exponent; real saved; exponent := 4; saved := 6.0 ^ sqrt(sqrt(exponent)) end",
            "begin integer exponent; real saved; exponent := -1; saved := 6.0 ^ sqrt(sqrt(exponent)) end",
            "begin real procedure sqrt(x); value x; real x; sqrt := 2.0; integer exponent; real saved; exponent := 16; saved := 6.0 ^ sqrt(sqrt(exponent)) end",
        ] {
            let module = compile_source(source, "test")
                .expect("unsafe nested square-root exponents must remain dynamic");
            let main = module.get_function("main").expect("has main");
            assert!(main.instructions.iter().any(|instr| instr.op == "f64_pow"));
            if source.contains("real procedure sqrt") {
                assert!(main.instructions.iter().any(|instr| instr.op == "call"));
    ...
    #[test]
    fn al4_exact_tracked_sqrt_standard_function_exponents_unroll_real_powers() {
        for exponent_expression in [
            "abs(sqrt(exponent))",
            "sign(sqrt(exponent)) + 1",
            "entier(sqrt(exponent))",
            "entier(abs(sqrt(exponent)))",
        ] {
            let source = format!(
                "begin integer exponent; real saved; exponent := 4; saved := 6.0 ^ ({exponent_expression}) + 6.0; exponent := 9; if saved = 42.0 then output(42) else output(1) end"
            );
            let module = compile_source(&source, "test")
                .expect("an exact tracked square root may feed a pure standard function");
            let main = module.get_function("main").expect("has main");
            assert!(
    ...
    #[test]
    fn al4_tracked_sqrt_standard_function_exponents_retain_unsafe_runtime_power() {
        for source in [
            "begin integer exponent; real saved; exponent := 2; saved := 6.0 ^ entier(sqrt(exponent)) end",
            "begin integer exponent; real saved; exponent := -1; saved := 6.0 ^ abs(sqrt(exponent)) end",
            "begin integer procedure sign(x); value x; real x; sign := 2; integer exponent; real saved; exponent := 4; saved := 6.0 ^ sign(sqrt(exponent)) end",
            "begin real procedure sqrt(x); value x; integer x; sqrt := 2.0; integer exponent; real saved; exponent := 4; saved := 6.0 ^ entier(sqrt(exponent)) end",
        ] {
            let module = compile_source(source, "test")
                .expect("unsafe tracked square-root standard functions must remain dynamic");
            let main = module.get_function("main").expect("has main");
            assert!(main.instructions.iter().any(|instr| instr.op == "f64_pow"));
    ...
    #[test]
    fn al4_standard_free_conditional_tracked_real_snapshot_power_unrolls() {
        let module = compile_source(
            "begin real gate, left, right, saved; left := 2.0; right := 2.0; saved := 6.0 ^ (if gate = 0.0 then left else right) + 6.0; gate := 1.0; left := 9.0; right := 9.0; if saved = 42.0 then output(42) else output(1) end",
            "test",
        )
        .expect("equal conditional tracked real snapshots may bound a plain real power");
        let main = module.get_function("main").expect("has main");
        assert!(main.instructions.iter().any(|instr| instr.op == "jmp_if_false"));
        assert!(main.instructions.iter().all(|instr| instr.op != "f64_pow"));
        assert!(main.instructions.iter().any(|instr| instr.op == "mul"));
    ...
    #[test]
    fn al4_conditional_real_snapshot_and_literal_power_unrolls() {
        let module = compile_source(
            "begin boolean gate; real exponent, saved; exponent := 2.0; saved := 6.0 ^ (if gate then exponent else 2.0) + 6.0; gate := true; exponent := 9.0; if saved = 42.0 then output(42) else output(1) end",
            "test",
        )
        .expect("an equal tracked real snapshot and literal may bound a plain real power");
        let main = module.get_function("main").expect("has main");
        assert!(main.instructions.iter().any(|instr| instr.op == "jmp_if_false"));
        assert!(main.instructions.iter().all(|instr| instr.op != "f64_pow"));
        assert!(main.instructions.iter().any(|instr| instr.op == "mul"));
    ...
    #[test]
    fn al4_path_independent_standard_function_results_unroll_real_powers() {
        let module = compile_source(
            "begin real gate, exponent, saved; exponent := -2.0; saved := 6.0 ^ abs(if gate = 0.0 then exponent else -exponent) + 6.0; gate := 1.0; exponent := 9.0; if saved = 42.0 then output(42) else output(1) end",
            "test",
        )
        .expect("a built-in may collapse distinct pure conditional operands to one exponent");
        let main = module.get_function("main").expect("has main");
        assert!(main.instructions.iter().any(|instr| instr.op == "jmp_if_false"));
        assert!(main.instructions.iter().all(|instr| instr.op != "f64_pow"));
        assert!(main.instructions.iter().any(|instr| instr.op == "mul"));
    ...
    #[test]
    fn al4_path_independent_standard_function_results_fail_closed() {
        for source in [
            "begin real gate, exponent, saved; exponent := -2.0; saved := 6.0 ^ abs(if gate = 0.0 then exponent else exponent - 1.0) end",
            "begin real procedure choose(x); value x; real x; choose := x; real gate, exponent, saved; exponent := -2.0; saved := 6.0 ^ abs(if choose(gate) = 0.0 then exponent else -exponent) end",
            "begin real procedure abs(x); value x; real x; abs := 2.0; real gate, exponent, saved; exponent := -2.0; saved := 6.0 ^ abs(if gate = 0.0 then exponent else -exponent) end",
            "begin real gate, exponent, saved; exponent := -2.0; saved := 6.0 ^ (if gate = 0.0 then exponent else -exponent) end",
        ] {
            let module = compile_source(source, "test")
                .expect("unsafe conditional standard-function exponents retain runtime power");
            let main = module.get_function("main").expect("has main");
            assert!(
                main.instructions.iter().any(|instr| instr.op == "f64_pow"),
                "{source}"
    ...
    #[test]
    fn al4_path_independent_standard_results_compose_with_tracked_integers() {
        let module = compile_source(
            "begin integer offset; real gate, exponent, saved; offset := 0; exponent := -2.0; saved := 6.0 ^ entier(abs(if gate = 0.0 then exponent else -exponent) + offset + 0.5) + 6.0; gate := 1.0; offset := 9; exponent := 9.0; if saved = 42.0 then output(42) else output(1) end",
            "test",
        )
        .expect("a path-independent built-in result may compose with an exact integer snapshot");
        let main = module.get_function("main").expect("has main");
        assert!(main.instructions.iter().any(|instr| instr.op == "jmp_if_false"));
        assert!(main.instructions.iter().all(|instr| instr.op != "f64_pow"));
        assert!(main.instructions.iter().any(|instr| instr.op == "mul"));
    ...
    #[test]
    fn al4_path_independent_conditional_tracked_real_arithmetic_powers_unroll() {
        let module = compile_source(
            "begin real power, offset, gate, exponent, saved; power := 0.5; offset := 0.5; exponent := -2.0; saved := 6.0 ^ entier((abs(if gate = 0.0 then exponent else -exponent) + 0.5) ^ (if gate = 0.0 then power + offset else offset + power)) + 6.0; gate := 1.0; power := 9.0; offset := 9.0; exponent := 9.0; if saved = 42.0 then output(42) else output(1) end",
            "test",
        )
        .expect("path-independent tracked-real arithmetic power branches may unroll");
        let main = module.get_function("main").expect("has main");
        assert!(
            main.instructions
                .iter()
    ...
    #[test]
    fn al4_standard_free_mixed_tracked_numeric_power_arithmetic_unrolls() {
        let module = compile_source(
            "begin integer whole; real part, saved; whole := 1; part := 1.0; saved := 6.0 ^ (whole + part) + 6.0; whole := 9; part := 9.0; if saved = 42.0 then output(42) else output(1) end",
            "test",
        )
        .expect("exact mixed tracked numeric arithmetic may bound a standard-free real power");
        let main = module.get_function("main").expect("has main");
        assert!(main.instructions.iter().all(|instr| instr.op != "f64_pow"));
        assert!(main.instructions.iter().any(|instr| instr.op == "mul"));
    }
    ...
    #[test]
    fn al4_static_conditional_selector_assignment_selects_preserving_leaf() {
        compile_source(
            "begin integer i, n, limit; boolean choose; n := 3; limit := 3; choose := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose then limit else limit + 1; choose := if true then choose else false end; print(i + 0.25) end",
            "test",
        )
    ...
    #[test]
    fn al4_tenth_nested_selector_dependency_stays_stable() {
        compile_source(
    ...
    #[test]
    fn al4_real_zero_exponent_literal_expression_units_stay_stable() {
        for write in [
            "choose * ((2.0 + 3.0) ^ 0)",
            ...
            .unwrap_or_else(|_| {
                panic!("real zero-exponent literal-expression write {write:?} must stay stable")
            });
        }
    }
    ...
    #[test]
    fn al4_eleventh_nested_selector_dependency_remains_conservative() {
        let err = compile_source(
            "begin integer i, n, limit, choose; boolean other, flag, gate, key, last, final, ultimate, terminal, horizon, frontier, boundary, threshold; n := 3; limit := 3; choose := 1; other := true; flag := true; gate := true; key := true; last := true; final := true; ultimate := true; terminal := true; horizon := true; frontier := true; boundary := true; threshold := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose = 1 then limit else limit + 1; choose := if other then choose else 0; other := if flag then other else false; flag := if gate then flag else false; gate := if key then gate else false; key := if last then key else false; last := if final then last else false; final := if ultimate then final else false; ultimate := if terminal then ultimate else false; terminal := if horizon then terminal else false; horizon := if frontier then horizon else false; frontier := if boundary then frontier else false; boundary := if threshold then boundary else false; threshold := threshold end; print(i + 0.25) end",
            "test",
        )
        .expect_err("selector dependency proofs beyond the ten-level cap must fail closed");
        assert!(format!("{err:?}").contains("cannot print a real value"));
    }
    ...
    #[test]
    fn al4_static_conditional_selector_assignment_selecting_change_remains_conservative() {
        let err = compile_source(
            "begin integer i, n, limit; boolean choose; n := 3; limit := 3; choose := true; i := 0; for i := i + 1 while i < n do begin n := limit; limit := if choose then limit else limit + 1; choose := if false then choose else false end; print(i + 0.25) end",
            "test",
        )
        .expect_err("a static selector that chooses a changing leaf must invalidate the selector");
        assert!(format!("{err:?}").contains("cannot print a real value"));
    }
    ...
    fn al4_bounded_loops_track_statically_selected_recurrence_branches() {
        ...
        .expect("bounded loops retain statically selected recurrence branches");
        let main = module.get_function("main").expect("has main");
        assert_eq!(
            main.instructions
                .iter()
                .filter(|instr| {
                    instr.op == "str_const"
                        && matches!(instr.srcs.first(), Some(Operand::Str(text)) if text == "4" || text == "6")
                })
                .count(),
            4
        );
    ...
    #[test]
    fn al4_bounded_recurrence_rejects_dynamic_statement_selector() {
        let err = compile_source(
            "begin integer i; real x, y; boolean take; x := 0.0; y := 0.0; for i := 1 step 1 until 4 do if take then x := x + i else y := y + i; print(x) end",
            "test",
        )
        .expect_err("an unknown statement selector keeps recurrence snapshots conservative");
        assert!(format!("{err:?}").contains("cannot print a real value"));
    }
    ...
    #[test]
    fn al4_finite_integer_step_loop_preserves_control_exit_snapshot() {
        compile_source(
            "begin integer i; for i := 1 step 1 until 3 do print(''); print(i + 2.25) end",
            "test",
        )
        .expect("a finite integer step loop has a statically known exit value");
    }

    #[test]
    fn al4_descending_integer_step_loop_preserves_control_exit_snapshot() {
        compile_source(
            "begin integer i; for i := 3 step -1 until 1 do print(''); print(i + 3.25) end",
            "test",
        )
    ...
    #[test]
    fn al4_zero_step_loop_remains_conservative() {
        let err = compile_source(
            "begin integer i; for i := 1 step 0 until 3 do print(''); print(i + 2.25) end",
            "test",
        )
        .expect_err("a zero step does not establish a finite exit");
        assert!(format!("{err:?}").contains("cannot print a real value"));
    }

    #[test]
    fn al4_reentering_control_assignment_remains_conservative() {
        let err = compile_source(
            "begin integer i; for i := 1 step 1 until 3 do i := 1; print(i + 2.25) end",
            "test",
        )
        .expect_err("a target assignment that re-enters the loop has no bounded exit proof");
        assert!(format!("{err:?}").contains("cannot print a real value"));
    }
