# Coding Adventures Tamil Assessment
...
This specification names the assessment that the complete Tamil book must
...
learner-facing name at each rung is **Coding Adventures Tamil <Level>
Assessment — project-defined equivalent**.

No `assessment.json` is checked in for Tamil yet, and that omission is
...
Tamil awarding body, and the Council of Europe states that it does not verify or
validate an examination's claimed link to CEFR. Every target here is therefore
labelled `project-defined`.

Tamil assessment infrastructure exists and is worth naming precisely, because
what it is NOT is the reason this project defines its own. The **Tamil Virtual
Academy** (தமிழ் இணையக் கல்விக்கழகம்), a Government of Tamil Nadu institution,
awards Certificate courses at Basic, Intermediate and Advanced levels and Higher
Certificate courses in three grades, examined twice a year in June and December.
Its regulations require an average of not less than 40% across the online and
written papers, with 60% marked first class and 80% distinction, and the
Certificate levels include an oral test and viva voce beside the online and
written components.

That is a real, government-run, graded Tamil examination — and it is a
**school-year ladder mapped to classes 1-12**, not a public pre-A1-to-C2
four-skill ladder, and its pass rule aggregates across papers rather than
requiring each skill to pass independently. Borrowing its name would misdescribe
both what this book teaches and what it would certify. This project therefore
defines its own transparent approximation instead.

### The variety question, which Tamil cannot leave unanswered

Tamil is diglossic, and a four-skill examination has to say so before it can
score a speaking paper at all. The written standard and the everyday spoken
register differ in verb endings, pronouns, negation and a good deal of
vocabulary — they are not one language written down two ways, and a candidate
who writes exactly what they say will be marked wrong by a reader who has only
the written standard in mind.

This specification settles it as follows:

- **Reading and writing** are assessed in contemporary standard written Tamil.
  The current track's taught forms define the initial model.
- **Speaking and listening** accept standard spoken Tamil. A candidate who
  speaks the written register is not penalised for it, and a candidate who
  speaks the spoken register is not penalised either; what is scored is whether
  the response does the task and is intelligible to a Tamil listener.
- A response that mixes the two registers within one turn is not an error at
  pre-A1, where the learner has memorised chunks rather than internalised a
  register. From A2 onward, register consistency is one of the scored
  dimensions, and the task states which register it expects.

Documented regional forms receive credit when used consistently and when they
satisfy the task; a trained Tamil reviewer adjudicates unfamiliar forms. The
book's own script model includes the grantha letters the corpus actually uses
for loanwords — ஷ and ஸ appear in the taught vocabulary — and a candidate is
neither required to use grantha nor penalised for using it where standard
orthography does.
...
- [Tamil Virtual Academy certificate rules and regulations](https://www.tamilvu.org/en/coresite-html-cwsylcrl-342515)
- [Tamil Virtual Academy higher certificate course](https://www.tamilvu.org/en/highercertificate)
...
Note that this differs from the Tamil Virtual Academy rule quoted above in the
one way that matters: TVA averages, and this does not. A ladder whose whole
claim is four independent skills cannot let a strong reader carry a silent
writer.
...
3. range and control of Tamil vocabulary and grammar;
4. Tamil orthographic control for writing, or pronunciation and fluency for
   speaking.

Each dimension is scored 0–5, then scaled to the paper's points. A response that
is not meaningfully in Tamil cannot earn task-fulfilment credit.

Dimension 4 for writing is specific to the script and is scored on three things
the Tamil writing system actually requires:

- the **puḷḷi**, the dot written ் above a consonant that carries no vowel. It is
  not optional decoration; its absence changes the syllable. The corpus writes
  it 3,954 times, which is a fair measure of how central it is;
- **vowel signs on both sides of the consonant.** Tamil writes ெ, ே and ை to the
  LEFT of the letter they modify, ா, ி, ீ, ு and ூ to the right, and ொ, ோ, ௌ on
  both sides at once. A sign placed on the wrong side is an orthographic error,
  and it is the commonest one a learner makes;
- **legibility and word boundary.**

## Administration rules

- Listening recordings use Tamil speakers. Words-per-minute bands below are
  form-assembly targets, not a claim that Tamil has one natural speaking rate.
- Pre-A1 and A1 directions may also be supplied in a declared support language,
  so misunderstanding directions is not confused with Tamil proficiency. From A2
  onward directions are in Tamil, with one unscored example.
...
- Speaking is recorded and independently scored by two trained raters. A third
  ...
  the first two raters disagree about a documented regional form or about which
  register a response is in.
...
evidence. Before release, at least eight qualified Tamil educators or linguists
...
the threshold or task envelope.
...
and only the script- and variety-specific rules above differ per track.

### pre-A1

| paper | minutes | required task envelope |
|---|---:|---|
| reading | 10 | 6 Tamil shape/word matches, 4 short phrase or notice matches, and 4 personal-detail selections; no text over 8 words |
