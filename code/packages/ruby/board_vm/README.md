...
CodingAdventures::BoardVM.uno_r4_wifi(port: "/dev/cu.usbmodem1101") do |board|
  ...
  run_report = result.results.last.decoded_response
end
```

Ruby selects a friendly mode name, while the module bytes and protocol frames
are still generated by `board-vm-language-core`.
...
`hello`, `capabilities`, `upload_blink`, `upload_gpio_read`, `run`, `stop`,
`blink`, `gpio_read`, and `run_command` return dispatch results with the
