...
`letter-haa.mov` and `waw.mov`; independent **ه** is therefore reprioritized as
the next measurable entry. Dhaal is source-available but is not one of this
Arabic ledger's 21 measured entries.
...
| HL-C09 | Queued — 49 of 228 verified | Expand `DUCTUS` to cover the ten scripts with prose stroke-order entries. | Add cited, font-checked ductus and verified pen-lift metadata for the remaining 179 entries measured by HL-C19; each passes the on-ink, join-tolerance, coverage, citation, and source-agreement invariants. |
...
| HL-C09AY | Queued — next; reprioritized after source audit | Audit independent Arabic ه from the source-backed **ه و ي** alphabet page. | The earlier **م ن** page contains no linked handwriting media. The **ه و ي** page directly links `letter-haa.mov`, so inspect independent **ه** frame by frame, record every restart, and reconcile the handwritten demonstration with Noto Naskh's wider isolated two-counter outline and the existing Persian **ه** provenance. |
