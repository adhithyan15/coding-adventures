...
describe("the committed corpus", () => {
  ...
  it("pins the first ledger, and it is the reason the rule exists", () => {
    ...
    expect(l.summary.roots).toBe(3079); // +1: Persian's schema-v2 opener exposes its s-l-m etymon atom.
    expect(l.summary.underspent).toBe(2964); // The new Persian etymon begins underspent.
    ...
    expect(byNamespace).toEqual({ roots: 2194, "etymon-atom": 885 }); // +1 Persian s-l-m etymon atom.
  });
