...
% canonical-source-hash: fnv1a64:932b67ab7cc2c1f6
% canonical-lessons: SA-C01-namaste, SA-C01-namaskara, SA-S02-letter-na, SA-S02-copy-the-three-strokes, SA-S02-delayed-copy, SA-S02-dictation, SA-C01-dhanyavada, SA-C01-svagatam, SA-S200-letter-da, SA-C01-am, SA-C01-na, SA-C01-practice, SA-S05-sign-virama
...
\hlblockfigure{\detokenize{figures/SA-S02-letter-na-filmstrip.pdf}}{How \sk{न} is written, stroke by stroke}
...
\end{tcolorbox}
\section[na]{\sk{न} — copy it in three strokes, counting the lifts}

\label{lesson:SA-S02-copy-the-three-strokes}



\begin{quote}
Put the stroke order from the last lesson beside you. It stays visible for all of this one.
\end{quote}

\subsection*{Writing — guided copy}
\hlblockfigure{\detokenize{figures/SA-S02-copy-the-three-strokes-filmstrip.pdf}}{How \sk{न} is written, stroke by stroke}

Copy \textbf{\sk{न}} once, following the three strokes in order: the loop and shoulder, the right stem top to bottom, then the \textbf{śirorekhā} across the top, left to right.

Count your pen lifts as you go. There should be \textbf{two} — after the shoulder and after the stem — and no more. If you lifted three or four times, you drew a picture of the letter rather than the letter; do it once more and watch only the lifts.

The last stroke is the one that matters most and is the easiest to treat as decoration. The śirorekhā is drawn \textbf{last and left to right}, and in running Devanagari it joins up across a whole word — so it is not a lid on one letter, it is the line the word will eventually hang from.

\begin{tcolorbox}[breakable,colback=teal!4,colframe=teal!35!black,title={Before you move on}]
How many pen lifts? (\textbf{Two.}) Which stroke came last? (\textbf{The śirorekhā}, left to right.)

One caution the source itself carries: this is one attested teaching order, not a national standard. Everyday handwriting joins and simplifies it, and a hand that has learned a different order is not wrong.
\end{tcolorbox}
\section[na]{\sk{न} — write it with the model covered}

\label{lesson:SA-S02-delayed-copy}



\begin{quote}
Look at \textbf{\sk{न}} once and say \emph{na}. Then cover it, and the stroke order with it.
\end{quote}

\subsection*{Writing — delayed copy}
\hlblockfigure{\detokenize{figures/SA-S02-delayed-copy-filmstrip.pdf}}{How \sk{न} is written, stroke by stroke}

With everything covered, wait ten seconds, then write \textbf{\sk{न}} once from memory.

Ten seconds is long enough that your hand cannot be copying a shape it can still see, and short enough that a letter you met a minute ago is still there.

Only after the letter is finished, uncover the model and compare.

\begin{tcolorbox}[breakable,colback=teal!4,colframe=teal!35!black,title={Before you move on}]
Check the \textbf{śirorekhā} first, and check its length rather than its presence.

A hand writing from memory usually remembers to put the bar there and draws it only as wide as the letter's own body. In running text the bar has to reach the full width of the letter, because the next letter's bar will meet it. A bar drawn short is the one mistake that looks completely correct on a single letter and falls apart the moment there are two.
\end{tcolorbox}
\section[na]{\sk{न} — the letter from the sound}

\label{lesson:SA-S02-dictation}



\begin{quote}
Cover every \textbf{\sk{न}} on the page. Nothing to copy, nothing to uncover, and no stroke order in front of you.
\end{quote}

\subsection*{Writing — dictation}
\hlblockfigure{\detokenize{figures/SA-S02-dictation-filmstrip.pdf}}{How \sk{न} is written, stroke by stroke}
...
\begin{tcolorbox}[breakable,colback=teal!4,colframe=teal!35!black,title={Before you move on}]
Uncover the model, compare — and notice what you were asked to write.

Not \emph{n}. \textbf{na}. One shape came out of your hand for two sounds, and nobody wrote the vowel. That is what an abugida is, and it is the reason the letters of this script cannot be counted the way an alphabet's can: \textbf{\sk{न}} is not a letter plus a silent vowel, it is a syllable with a consonant you can change.
\end{tcolorbox}
...
\hlblockfigure{\detokenize{figures/SA-S200-letter-da-filmstrip.pdf}}{How \sk{द} is written, stroke by stroke}
