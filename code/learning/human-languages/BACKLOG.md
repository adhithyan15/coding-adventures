...
Arabic remains the smallest inventory at 19 entries, so HL-C09AJ queues ت.
...
| HL-C09 | Queued — 35 of 228 verified | Expand `DUCTUS` to cover the ten scripts with prose stroke-order entries. | Add cited, font-checked ductus and verified pen-lift metadata for the remaining 193 entries measured by HL-C19; each passes the on-ink, join-tolerance, coverage, citation, and source-agreement invariants. |
...
| HL-C09AJ | Queued — next | Locate and verify independent Arabic ت as the next entry in the smallest remaining inventory. | Find an instructional source that exposes the bowl and two-dot order, preserve Arabic identity separately from Persian ت, and reconcile the independent path and lifts with the canonical Noto Naskh outline. |
