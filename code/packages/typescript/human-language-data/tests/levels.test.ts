...
describe("corpus snapshot", () => {
  ...
  it("pins where the corpus actually stands on the ladder", () => {
    ...
    expect(summary.byLevel.A1).toBe(341); // +1: ES-C02-concordancia sits on SPINE-TIME-OF-DAY
  ...
  it("can already build a ramp-to-A1 edition from the canonical corpus", () => {
    ...
    expect(ramp).toHaveLength(1237); // +3: HL-C97 adds the repair kit (no entiendo, mas despacio) at chapter 14
