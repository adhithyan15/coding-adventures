...
| authentication | `vault-auth` | password, TOTP, and `WebAuthnPrfAuthenticator` factors (real CTAP2 hardware I/O via `vault-webauthn-ctap2-hid`, real ECDSA P-256 assertion-signature verification via `ring`) | signature-counter / cloned-authenticator replay state where enabled (`VLT-PM51` §22) |
...
15. Google Drive `appDataFolder` adapter and sandbox conformance, decomposed
    ...
    7. `storage add gdrive`, verified migration/mirroring, and explicit audited
