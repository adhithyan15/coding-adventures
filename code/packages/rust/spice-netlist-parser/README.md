...
spice-netlist-parser inspect --json circuit.cir
```
...
`inspect --json` returns the complete runnable plan in source-analysis order,
including the source analysis index and kind, without executing the circuit.
