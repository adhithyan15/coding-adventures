...
//!
//! The `false` command is the counterpart of `true`: it does nothing
//! and exits with status 1 (failure). Like `true`, it has no business
//! logic — the only thing to test is that the CLI Builder spec is
//! valid and handles `--help` and `--version`.
...
fn spec_path() -> String {
    ...
    let path = PathBuf::from(manifest_dir).join("false.json");
    path.to_string_lossy().into_owned()
}

fn make_parser() -> Parser {
    let spec = load_spec_from_file(&spec_path()).expect("failed to load false.json");
    Parser::new(spec)
}
...
mod spec_loading {
    ...
    /// The false.json spec should load without errors.
    #[test]
    fn spec_loads() {
        let spec = load_spec_from_file(&spec_path());
        assert!(spec.is_ok(), "false.json should load successfully");
    }
}
...
mod default_behavior {
    ...
    /// When invoked with no flags, false should return a Parse variant.
    /// The program would then exit with code 1.
    #[test]
    fn no_flags_returns_parse_result() {
        match parse_argv(&["false"]) {
    ...
    fn no_user_flags_set_to_true() {
        match parse_argv(&["false"]) {
            ParserOutput::Parse(result) => {
                // false has no user-defined flags, so none should be true
    ...
    fn help_text_contains_program_name() {
        match parse_argv(&["false", "--help"]) {
            ParserOutput::Help(help) => {
                assert!(
                    help.text.contains("false"),
                    "help text should contain 'false', got: {}",
                    help.text
                );
            }
    ...
    fn help_text_contains_description() {
        match parse_argv(&["false", "--help"]) {
            ParserOutput::Help(help) => {
                ...
                assert!(
                    lower.contains("unsuccessfully") || lower.contains("nothing"),
                    "help text should mention 'unsuccessfully' or 'nothing', got: {}",
                    help.text
                );
            }
            _ => panic!("expected Help"),
...
mod version_flag {
    ...
    fn version_returns_version_result() {
        match parse_argv(&["false", "--version"]) {
