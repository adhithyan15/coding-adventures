# coding_adventures_cas

Generic Content-Addressable Storage (CAS) for Ruby, ported from the
[Rust implementation](../../rust/cas). This package implements the CAS
...
```ruby
gem "coding_adventures_cas"
```

Or install directly:

```sh
gem install coding_adventures_cas
```
...
Dir.mktmpdir do |root|
  ...
  hex = CodingAdventures::Cas.key_to_hex(key)
...
| Method | Description |
|---|---|
| `CodingAdventures::Cas.key_to_hex(key)` | 20-byte binary String → 40-char hex String |
| `CodingAdventures::Cas.hex_to_key(hex)` | 40-char hex String → 20-byte binary String |
...
```ruby
class MyStore
  include CodingAdventures::Cas::BlobStore
...
```ruby
store = CodingAdventures::Cas::LocalDiskStore.new("/path/to/objects")
```
...
All errors inherit from `CodingAdventures::Cas::CasError < StandardError`.
