package CodingAdventures::StarlarkParser::ASTNode;

# ============================================================================
# CodingAdventures::StarlarkParser::ASTNode — AST node for the Starlark parser
# ============================================================================
#
# Every node in the Starlark Abstract Syntax Tree is an instance of this class.
# There are two kinds of nodes:
#
#   Leaf nodes   — wrap a single token from the lexer.
#                  `is_leaf` returns 1, `token` returns the token hashref.
#
#   Inner nodes  — produced by grammar rules like assign_stmt, def_stmt.
...
#
#   {
#     rule_name => "assign_stmt",  # the grammar rule that produced this
#     children  => [ $node, ... ],
#     is_leaf   => 0,
#   }
#
...
#
# Arguments:
#   $token  hashref  — a token as produced by CodingAdventures::StarlarkLexer
...
=head1 SYNOPSIS
    ...
    # Inner node
    my $node = CodingAdventures::StarlarkParser::ASTNode->new('assign_stmt', \@children);
    print $node->rule_name;   # "assign_stmt"
    print $node->is_leaf;     # 0

    # Leaf node
    my $leaf = CodingAdventures::StarlarkParser::ASTNode->new_leaf($token);
    print $leaf->is_leaf;         # 1
    print $leaf->token->{value};  # e.g. "def"

=head1 DESCRIPTION

Lightweight AST node class used by C<CodingAdventures::StarlarkParser>.
