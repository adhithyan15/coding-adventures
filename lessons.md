...
- **A shared Mosaic width fix must be measured against every strict native emitter, not only the backend that exposed the original clipping.** Giving TaskApp's progress group a 90px Compose-safe bound kept its caption visible there but made Flutter's real widget test fail with a 58px `RenderFlex` overflow. Use the failing backend's exact overflow measurement to size the shared group (150px here), then rerun the native lifecycle on both the original backend and every backend whose layout consumes the same `.msl` width.
