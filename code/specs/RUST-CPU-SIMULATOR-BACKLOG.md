...
RCPU-049 is complete locally. The new `apple-m1-simulator` extends the complete
AArch64 integer core with exact 32x128-bit vector/FP state, the full Spec 07z
scalar-FP, vector-memory, NEON integer/FP, DUP, and FMLA surface, structured
encoders, checked restore/load/direct access, typed atomic faults, complete
traces, and transactional runs. Ten Rust tests and all 360 reproducible Python
Apple-specific full-state vectors pass; the delegated AArch64 core retains its
836-vector differential. All 109 Python Apple M1 tests and Rust AArch64
...
| 2026-09-01 | RCPU-045 baseline audit found no Rust RV64I package or gate partner. The Python `riscv-rv64i-simulator` has 96 passing tests and 97.54% statement coverage over RV64I+M, but retains a legacy permissive lifecycle: memory and fetch wrap at 64 KiB; loads silently depend on slice behavior; unknown/reserved funct encodings and unsupported load/store/branch/system forms execute as NOPs; zero words, ECALL, and EBREAK conflate halt paths; post-halt steps succeed; step-limit exhaustion returns ordinary non-halted state; snapshots can expose a dirty x0; and there is no checked origin/install range, validated restore/direct access, typed fault surface, transition-atomic step, transactional bounded run, instruction-bearing complete trace/result, structured Rust encoders, or reproducible Python full-state corpus. Spec 07y documents the instruction surface but codifies the same wrapping/NOP lifecycle. | P0, chronological functional completion, blocks RCPU-046 | Create `riscv-rv64i-simulator` with exact 64 KiB little-endian state, 32x64-bit GPRs with x0 enforced, 64-bit aligned PC, halt and installed-range metadata; checked origin-aware loading/restore/direct access; typed fail-closed fetch/alignment/range/malformed-decode faults; atomic steps and transactional bounded runs; complete traces/results; structured encoders; the full Python RV64I+M decode surface; a reproducible Python full-state corpus plus manual strict-fault edges; corrected normative Spec 07y completion text; strict checks; consumers; and at least 80% coverage. Preserve the reset SP convention and distinguish the repository zero-word sentinel from architectural ECALL/EBREAK in trace semantics. |
| 2026-09-01 | RCPU-044 is complete. | Resolved; selects RCPU-045 chronologically | `riscv-gatelevel` now has the exact 525,505-DFF topology, independent complete RV32I+CSR/MRET execution, repository-gate strict decode/Boolean/ripple arithmetic/compare/barrel shift/address/branch/CSR/trap networks, the shared typed atomic lifecycle, all 256 Python full-state hashes in complete functional trace/state lockstep, manual topology/lifecycle/fault suites, normative Spec 07a2, strict checks, all six dependent functional consumers, and 97.96% package line coverage (769/785). |
| 2026-09-01 | RCPU-044 baseline audit found no Rust, Python, or other RV32I gate-level package and no normative Spec 07a2. The completed RCPU-043 state establishes exactly 525,505 persistent bits: 524,288 memory bits, 1,024 x0-x31 bits (with writes to x0 discarded and restore requiring zero), 32 PC bits, 160 bits across the five documented M-mode CSRs, and one halt bit. Installed origin/length remain validated lifecycle metadata. The MIPS and ARM gate packages provide reusable DFF packing and lifecycle patterns but model different instruction encodings and semantics. | P0, chronological gate completion | Create `riscv-gatelevel` and normative Spec 07a2 with exactly 525,505 clocked DFFs, repository-gate opcode/funct decode, Boolean/ripple arithmetic/compare/barrel-shift/address/branch networks, an independent complete RV32I+CSR/MRET execution path, the shared typed atomic functional lifecycle, all 256 Python and functional full-state/trace vectors plus every manual fault/spec edge, strict checks, consumers, and at least 80% coverage. |
| 2026-09-01 | RCPU-043 is complete. | Resolved; selects RCPU-044 by pair priority | `riscv-simulator::Rv32ISimulator` now preserves every compiler/host/core-adapter consumer while adding the exact checked 64 KiB RV32I machine, 32x32-bit GPRs, PC, five M-mode CSRs, halt and installed-range state, strict origin/range/alignment validation, typed fail-closed malformed-decode/CSR/data/fetch faults, atomic steps, transactional bounded runs, complete traces/results, corrected normative Spec 07a text, seven lifecycle/fault suites, and a reproducible 256-vector Python full-state differential over every RV32I/CSR/MRET family. All 106 package test functions and six dependent consumer packages pass; strict Rustfmt, Clippy, and rustdoc are green; package line coverage is 91.46% (1,125/1,230). |
| 2026-09-01 | RCPU-043 baseline audit found a mature but legacy `riscv-simulator`: 98 Rust tests cover RV32I plus selected RV32M, M-mode CSRs/traps, a compiler host ABI, and a second generic-core adapter, while the Python oracle has 118 tests and protocol-shaped snapshots/results. The public Rust machine is nevertheless caller-sized, exposes mutable register/memory/CSR internals and a signed PC, loads only at zero without checked range/alignment metadata, treats unknown encodings as NOPs, can panic on fetch or memory bounds, returns only mnemonic strings from steps, conflates a zero-`mtvec` architectural ECALL with host services/halt, and has no exact restore/direct-access state, typed failures, transition-atomic step, transactional bounded run, complete traces/results, or reproducible Python full-state corpus. Package Clippy and rustdoc are green, but package Rustfmt is not. Spec 07a itself still describes only a three-instruction MVP and labels the remaining RV32I surface as future work even though both implementations already exceed it. | P0, chronological functional completion, blocks RCPU-044 | Preserve the existing compiler/host/core-adapter APIs as consumers while adding an exact checked 64 KiB RV32I architectural machine boundary with 32x32-bit GPRs, PC, the five documented M-mode CSRs, halt and installed-range state; checked origin-aware loading/restore/direct access; typed fail-closed alignment/range/decode faults; transition-atomic step and bounded transactional run; complete traces/results; the full existing RV32I+CSR/MRET decode surface; a reproducible Python full-state corpus plus manual fault edges; corrected normative Spec 07a completion text; strict formatting/lints/docs; consumers; and at least 80% package coverage. Keep RV32M and the host ABI as explicitly non-normative legacy extensions rather than allowing them to redefine the RV32I cell. |
| 2026-09-01 | RCPU-042 is complete. | Resolved; selects RCPU-043 chronologically | `armv7a-gatelevel` now has the exact 524,833-DFF topology, independent complete mixed-width Thumb-2 execution, repository-gate decode/conditions/Boolean/ripple arithmetic/barrel shift/rotate/fixed multiply/address/flag networks, the shared typed atomic lifecycle, all 417 Python and functional full-state/trace differentials, every manual functional/spec edge, normative Spec 07x2, strict checks, and 97.41% package line coverage (716/735). |
| 2026-08-31 | RCPU-042 baseline audit found no Rust, Python, or other gate-level package and no normative Spec 07x2 for the distinct Thumb-2 machine. The completed functional oracle establishes 524,833 persistent bits: 524,288 memory bits, 512 R0-R15 bits with PC stored once in R15, 32 CPSR bits (including NZCV/T), and one halt bit; installed origin/length remain validated lifecycle metadata. The earlier `armv7-gatelevel` package models the materially different fixed-width A32 educational Spec 07b machine and cannot serve as this mixed-width decoder/execution partner. | P0, chronological gate completion | Create `armv7a-gatelevel` and normative Spec 07x2 with exactly 524,833 clocked DFFs, repository-gate mixed-width decode, condition/Boolean/ripple-add-sub/barrel-shift/rotate/address/multiply networks, an independent complete Spec 07x execution path, the shared typed atomic functional lifecycle, all 417 Python common-surface vectors plus every manual functional/spec edge, strict checks, consumers, and at least 80% coverage. |
