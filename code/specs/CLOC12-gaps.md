...
- **Status:** AST + emitter RESOLVED in CLOC12.33. **Empty-switch elimination** RESOLVED in CLOC12.34 (closure-pass-dce 0.6.0). **Drop-after-break in case consequents** RESOLVED in CLOC12.35 (closure-pass-dce 0.7.0 — truncates statements after `break`/`return`/`throw` inside each case body). **Constant-discriminant collapse** (`switch(1){case 1:body;default:other;}` → `body;`) remains a follow-up.
