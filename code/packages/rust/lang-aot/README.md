...
| MacroOct        | `.macrooct`     | `macrooct-iir-compiler` (PREP01 slices 1–2) | full — **Oct, plus a preprocessor, and nothing else**. `@include` / `@if` / `@else` / `@end` / `@define` (object- and function-like) are consumed by the generic `source-preprocessor` engine *before* parsing; Oct's parser grammar, type checker and `compile_ast` are reused unchanged and no backend learns anything. The 15 matrix rows are therefore not feature proofs: each is paired with the Oct source a human would hand-expand it to, and asserted to lower to **identical IIR** by `macrooct_rows_lower_to_iir_identical_to_hand_expanded_oct`. `@include` is unreachable from this harness by construction — `compile_source_to_iir` takes a source *string*, which has no directory to resolve a relative path against |
