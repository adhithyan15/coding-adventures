...
% canonical-source-hash: fnv1a64:e538160cb28f04ce
% canonical-lessons: GU-C11-mitra, GU-C11-kutumb, GU-C11-bhai, GU-C11-bahen, GU-R13-doorway-nine-r3
...
\end{tcolorbox}
\section[retroflex nna — ja — u sign — sha …]{The doorway nine return in a new order}

\label{lesson:GU-R13-doorway-nine-r3}



\begin{quote}
Without looking back, write \emph{ja}, the nasal mark, and \emph{ka}. Check only a missed form, cover the model again, and make one repair.
\end{quote}

\subsection*{Your turn}
Read the shuffled row once. Identify each form on sight; do not rebuild the original doorway order. Record the reading score before the row is covered; repair only missed forms, then reach \textbf{9/9} once.

\subsection*{Writing — from sound}
Keep every model covered. Write the nine sound cues in their new order, then score the writing response separately from reading. Writing also needs \textbf{9/9}; the reading score cannot compensate for it. Repair only missed forms.

\begin{tcolorbox}[breakable,colback=teal!4,colframe=teal!35!black,title={Before you move on}]
How many marks and consonants returned? (\textbf{Four marks and five consonants.})
\end{tcolorbox}
