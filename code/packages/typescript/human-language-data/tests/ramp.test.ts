...
describe("corpus snapshot", () => {
  ...
  it("pins the ramp, and the size of its blind spot", () => {
    ...
    expect(report.summary.lessonViolations).toBe(45); // HL-C128 step 8: +2 -- ES-C65-ahora-hoy and ES-C65-vi-di each introduce 4 atoms against a budget of 3. In both cases TWO of the four are etymons, which the level gate already waives from reinforcement as read-once rather than drilled, so the load on the reader is two new forms apiece. Recorded rather than hidden; if the budget should count etymons separately that is a change to the budget, not to these lessons.
    ...
    expect(report.summary.measurablePercent).toBe(71); // +1: HL-C98 // +1: vocabulary wave 5 added 40 schema-v2 lessons, all measurable // +1: vocabulary wave 6 added 54 schema-v2 lessons, all measurable // HL12: +30 recognition segments (telugu/kannada/malayalam 8 each, sanskrit 6) -- all schema-v2 and all atom-bearing, so the measurable share rises
  });
