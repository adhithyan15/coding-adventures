...
fn emit_project_shell(
    ...
    match backend {
        ...
        Backend::Compose => {
            let readme = backend_dir.join("README.md");
            write_file(
                &readme,
                build_compose_readme(package_name, component).as_bytes(),
            )?;
            written.push(readme);
        }
        Backend::Qt => {
...
fn build_compose_readme(package_name: &str, component: &str) -> String {
    format!(
        "# {package_name} Compose artifacts\n\n\
Generated by `mosaic-compile pkg --backend compose --emit-project`.\n\n\
This backend currently emits Compose library sources rather than a full Gradle project shell.\n\
Add `{component}.kt` and `index.kt` to an Android, Desktop, or Compose Multiplatform source set,\n\
then wire `{component}`'s `dispatch` callback to your Mosaic host bridge. Generated events expose\n\
`mosaicEnvelope` for the same target-neutral event contract used by the other Mosaic backends.\n"
    )
}
...
"#
    ...
    fn ui32_m_emit_project_true_produces_expected_shell_per_backend() {
        for (backend, expected_files) in [
            ...
            (Backend::Compose, vec!["Grid.kt", "index.kt", "README.md"]),
            (
                Backend::Qt,
    ...
    fn compose_package_artifact_exposes_mosaic_event_envelope() {
        ...
        assert!(readme.contains("Compose library sources"));
