...
fn conversions_round_trip_runs_on_real_clang() {
    ...
    // (on macOS libm lives in libSystem, so it is harmless there). Windows
    // provides `floor` through the CRT; asking its linker for `m.lib` fails.
    // A real entier-using program links through the same platform contract.
    let mut clang = Command::new("clang");
    clang.arg("-x").arg("ir").arg(&ll_path);
    #[cfg(not(target_os = "windows"))]
    clang.arg("-lm");
    let built = clang.arg("-o").arg(&exe).output().expect("run clang");
    assert!(built.status.success(),
        "clang failed:\n{}\n--- .ll ---\n{ll}", String::from_utf8_lossy(&built.stderr));
    let run = Command::new(&exe).status().expect("run exe");
    let _ = std::fs::remove_dir_all(&dir);
