...
under `DUCTUS`'s current glyph-only identity; HL-C09U makes that shared-glyph
architecture and Urdu source availability the next explicit audit instead of
attaching language-specific evidence to the wrong script.
...
| HL-C09 | Queued — 20 of 228 verified | Expand `DUCTUS` to cover the ten scripts with prose stroke-order entries. | Add cited, font-checked ductus and verified pen-lift metadata for the remaining 208 entries measured by HL-C19; each passes the on-ink, join-tolerance, coverage, citation, and source-agreement invariants. |
...
| HL-C09U | Queued — next | Audit the smallest remaining Urdu inventory's sources and reconcile shared-glyph ductus identity before authoring its first tranche. | Establish cited Urdu-specific evidence and make Persian ا and Urdu ا independently addressable instead of letting the current glyph-only `DUCTUS` key attach one script's source to another; then author the smallest safe Urdu tranche with source, lifts, font, and filmstrip agreement. |
