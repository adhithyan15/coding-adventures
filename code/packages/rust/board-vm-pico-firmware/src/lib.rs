...
use board_vm_pico::{pico_device, pico_w_device, PicoBackend, PicoDevice};
use board_vm_runtime::BoardHal;

pub const PICO_DEFAULT_BOARD_NONCE: u32 = 0xC0DE_2040;
pub const PICO_W_DEFAULT_BOARD_NONCE: u32 = 0xC0DE_2041;

pub type PicoServerEndpoint<S> = DeviceStreamEndpoint<S, 1024, 512, 256>;

pub fn pico_server<B>(backend: B, board_nonce: u32) -> PicoDevice<B>
where
    B: PicoBackend,
{
    pico_device(backend, board_nonce)
}

pub fn pico_w_server<B>(backend: B, board_nonce: u32) -> PicoDevice<B>
where
    B: PicoBackend,
{
    pico_w_device(backend, board_nonce)
}

pub fn pico_endpoint<S>(stream: S) -> PicoServerEndpoint<S> {
    DeviceStreamEndpoint::new(stream)
}

pub fn serve_pico_once<S, B>(
    endpoint: &mut PicoServerEndpoint<S>,
    device: &mut PicoDevice<B>,
) -> Result<usize, DeviceStreamError<S::Error>>
where
    S: DeviceByteStream,
    B: PicoBackend,
    board_vm_pico::PicoBoard<B>: BoardHal,
{
    endpoint.serve_one(device)
}

pub fn serve_pico_available<S, B>(
    endpoint: &mut PicoServerEndpoint<S>,
    device: &mut PicoDevice<B>,
...
mod tests {
    ...
    #[test]
    fn pico_endpoint_serves_hello_ack() {
        let request = hello_wire_request();
        let mut endpoint = pico_endpoint(ScriptedStream::with_read(&request));
        let mut device = pico_server(FakeBackend, PICO_DEFAULT_BOARD_NONCE);

        let written_len = serve_pico_once(&mut endpoint, &mut device).unwrap();
        ...
        assert_eq!(hello.board_name, "raspberry-pi-pico");
        assert_eq!(hello.runtime_name, "board-vm-pico");
        assert_eq!(hello.board_nonce, PICO_DEFAULT_BOARD_NONCE);
    }

    #[test]
    fn pico_w_endpoint_serves_hello_ack() {
        let request = hello_wire_request();
        let mut endpoint = pico_endpoint(ScriptedStream::with_read(&request));
        let mut device = pico_w_server(FakeBackend, PICO_W_DEFAULT_BOARD_NONCE);

        serve_pico_once(&mut endpoint, &mut device).unwrap();
        let stream = endpoint.into_inner();
        let mut raw = [0u8; 256];
        let raw_len = decode_wire_frame(&stream.written, &mut raw).unwrap();
        let frame = decode_frame(&raw[..raw_len]).unwrap();
        ...
        assert_eq!(hello.board_name, "raspberry-pi-pico-w");
        assert_eq!(hello.board_nonce, PICO_W_DEFAULT_BOARD_NONCE);
    }

    #[test]
    fn pico_endpoint_reports_idle_when_no_bytes_are_available() {
        let mut endpoint = pico_endpoint(ScriptedStream::default());
        let mut device = pico_server(FakeBackend, PICO_DEFAULT_BOARD_NONCE);

        assert_eq!(
            serve_pico_available(&mut endpoint, &mut device),
