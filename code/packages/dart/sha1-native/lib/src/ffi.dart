// ffi.dart — dart:ffi bindings for the sha1-native Rust cdylib.
//
// Same design as sha256-native/md5-native: binary byte-buffer I/O with a
// caller-owned 20-byte digest buffer, plus an opaque streaming handle freed by
// a NativeFinalizer. See sha256-native for the full rationale.
//
...
//   HASHER* sha1_hasher_new/_update/_digest/_clone/_free
...
DynamicLibrary _load() {
  final envPath = Platform.environment['SHA1_NATIVE_PATH'];
  if (envPath != null && envPath.isNotEmpty) {
    if (!_isAbsolute(envPath)) {
      throw ArgumentError('SHA1_NATIVE_PATH must be an absolute path, got: $envPath');
    }
    return DynamicLibrary.open(envPath);
  }
  if (Platform.isMacOS) return DynamicLibrary.open('libsha1_native.dylib');
  if (Platform.isWindows) return DynamicLibrary.open('sha1_native.dll');
  return DynamicLibrary.open('libsha1_native.so');
}

bool _isAbsolute(String p) =>
    p.startsWith('/') || RegExp(r'^[A-Za-z]:[\\/]').hasMatch(p);

final _digest = _lib.lookupFunction<_DigestC, _DigestDart>('sha1_digest');
final _hex = _lib.lookupFunction<_HexC, _HexDart>('sha1_hex');
final _freeStr = _lib.lookupFunction<_FreeStrC, _FreeStrDart>('sha1_free_string');
final _hasherNew = _lib.lookupFunction<_HasherNewC, _HasherNewDart>('sha1_hasher_new');
final _hasherUpdate = _lib.lookupFunction<_HasherUpdateC, _HasherUpdateDart>('sha1_hasher_update');
final _hasherDigest = _lib.lookupFunction<_HasherDigestC, _HasherDigestDart>('sha1_hasher_digest');
final _hasherClone = _lib.lookupFunction<_HasherCloneC, _HasherCloneDart>('sha1_hasher_clone');
final _hasherFree = _lib.lookupFunction<_HasherFreeC, _HasherFreeDart>('sha1_hasher_free');

final NativeFinalizer _hasherFinalizer = NativeFinalizer(
    _lib.lookup<NativeFunction<Void Function(Pointer<Void>)>>('sha1_hasher_free'));
...
/// Compute the SHA-1 digest of [data] (executed in Rust) as 20 bytes.
Uint8List nativeSha1(List<int> data) {
  final input = _toNative(data);
  final out = malloc<Uint8>(20);
  try {
...
/// Compute the 40-character lowercase hex digest of [data] (executed in Rust).
String nativeSha1Hex(List<int> data) {
  final input = _toNative(data);
  try {
