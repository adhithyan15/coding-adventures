...
- The CLI now persists per-invocation cache entries and topology-scoped
  ...
  processes; replay-capable filesystem emitters can reconstruct missing output
  trees from validated artifacts while other capability-bearing stages remain
  conservative. Stream checkpoints now have a bounded content-addressed tree
  representation with manifest-last publication and pre-replay validation;
  scheduler integration remains in the active streaming tranche.
...
- The remaining headless-product gap is bounded concurrent streaming and the
  deploy runner. Both live
...
| 24 | FM-B036 | done | Schedule the exact changed-and-downstream set | Depends on FM-B035. The scheduler loads the prior ledger, identifies changed instances plus their transitive downstream closure, restores untouched capability-free outputs from validated cache checkpoints, reports skipped instances, and fails open to execution when reuse is unsafe or unavailable; per-item cache reuse remains available inside affected stream stages. |
| 25 | FM-B037 | done | Replay filesystem emitter side effects safely | Depends on FM-B036. The stage contract exposes an explicit replay hook; both filesystem emitters reconstruct their complete output tree from a validated `DeployArtifact` checkpoint; an unchanged fresh-process build restores a deleted output tree without re-running sources or pure transforms; and missing, corrupt, mismatched, or failed replay data falls back to normal execution. |
| 26 | FM-B032 | done | Complete external revisions, side-effect replay, and the exact affected set | Depends on FM-B035–FM-B037. Sources publish deterministic external-state manifests; scheduler state records per-instance input/output revisions; changed instances plus their downstream closure rerun; filesystem emitters replay or materialize outputs correctly after clean. |
| 27 | FM-B038 | done | Add bounded stream checkpoint storage | Depends on FM-B032. Stream values are encoded into content-addressed cache leaves and an incrementally built ordered tree with an O(log n) writer frontier; a small manifest is published last, a validation pass detects missing/corrupt/cyclic/count-mismatched trees before replay, and a second pass yields values lazily without materializing the stream. Tests prove bounded retained state beyond the default 64-item window and fail-open corruption handling. |
| 28 | FM-B039 | done | Implement bounded lazy stream fan-out | Depends on FM-B038. One lazily opened upstream iterator feeds every statically known consumer through ordered 64-item windows; a slow branch backpressures the producer, `return()` safely detaches a consumer, cancellation clears all branches and closes upstream once, source errors reach every attached branch after its queued prefix, and instrumentation tests bound both pulls and retained values. |
| 29 | FM-B040 | active | Implement the pipeline-wide concurrent scheduler | Depends on FM-B039. A stable ready queue and shared permit pool enforce `settings.maxConcurrency` across stage and per-item invocations, suspend permits while stream consumers wait on upstream to avoid `maxConcurrency: 1` deadlocks, preserve declaration/input order in outputs and summaries, and deterministically cancel and dispose in-flight work. |
| 30 | FM-B010 | blocked | Finish bounded streaming and parallel scheduling | Depends on FM-B038–FM-B040. Live fan-out, bounded checkpoints and buffers, backpressure, and pipeline-wide `maxConcurrency` replace eager stream draining without weakening exact affected-set reuse; clean and incremental product builds plus deterministic cancellation/reproducibility tests pass. |
| 31 | FM-B011 | ready | Reconcile the FM spec map | Resolve the FM05 numbering collision, publish the missing Interactivity IR/AOT/CLI spec locations, repair stale cross-links, and add an implementation-status ledger to every FM spec. |
| 32 | FM-B012 | ready | Implement the deploy runner | Build the FM05 core, filesystem adapter, GitHub Pages adapter, dry-run/reporting path, rollback/idempotency tests, and `forme deploy` composition. Filesystem publication stages the complete named-output set, rejects cross-artifact path collisions, swaps it as one tree, and prunes stale files without crossing output ownership. |
| 33 | FM-B013 | ready | Specify and implement Interactivity IR | Define the behavior/event/state schema and validator, integrate per-page island tracking, and prove a progressively enhanced interactive component with a no-JS fallback. |
| 34 | FM-B014 | blocked | Implement the plugin host and wire protocol | Depends on FM-B011 and the existing manifest parser. Stage discovery, handshake, typed streaming, capability mediation, diagnostics, cancellation, and crash isolation pass cross-process contract tests. |
| 35 | FM-B015 | blocked | Ship plugin installation, runtimes, and sandboxes | Depends on FM-B014. Signed/trusted install flow, grants persistence, TypeScript/Python/Rust runners, and macOS/Linux/Windows sandbox profiles pass adversarial filesystem/network/process tests. |
| 36 | FM-B016 | blocked | Build the authoring shell | Depends on FM-B009, FM-B013, and FM-B015. A non-developer can create, edit, preview, configure, and publish a site without hand-editing source or config files. |
| 37 | FM-B017 | blocked | Prove the backend boundary | Depends on FM-B005 and FM-B013. The same content and theme compile through HTML plus at least one of terminal, PDF/print, or email with explicit degradation tests. |
| 38 | FM-B018 | blocked | Close release-quality gates | Depends on the v1 product path. Add 1,000-page clean/incremental benchmarks, Lighthouse/accessibility budgets, package/API versioning, migration docs, security review, and supported-platform CI. |
| 39 | FM-B029 | later | Make duplicate PR CI cancellation and merge state unambiguous | One commit has one authoritative required CI suite; branch updates cancel obsolete runs completely; cancelling a redundant push suite cannot leave a stale final gate or misleading failed rollup; babysitting tooling identifies required checks and the current head. |
...
`FM-B007 → FM-B008 → FM-B009 → FM-B031 → FM-B033 → FM-B034 → FM-B035 → FM-B036 → FM-B037 → FM-B032 → FM-B038 → FM-B039 → FM-B040 → FM-B010 → FM-B012`.
...
| 2026-09-01 | The cache codec and scheduler were safe for pure stages, but the product CLI still constructed the default in-memory backend, so separate `forme build` processes could never hit it. | Split FM-B033 to wire only containment-checked project persistence and cross-process proof. FM-B032 retains source manifests, exact affected-set state, and side-effect replay so persistence cannot accidentally skip filesystem effects. |
...
| 2026-09-19 | Filesystem emitter outputs already contain every emitted byte in `DeployArtifact.files`, so side-effect replay does not need a second filesystem snapshot format. The missing boundary is an explicit, opt-in stage hook that rematerializes a validated checkpoint with the stage's declared capabilities. | FM-B037 adds `Stage.replay`, checkpoints only capability-bearing stages that opt in, and requires both filesystem emitters to write artifact paths through their existing containment checks. Replay errors fail open to the normal `run` path. |
