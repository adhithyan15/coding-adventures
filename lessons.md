...
- **Clean Vite deploys with source-level `file:` dependencies need `npm install --install-links`.** A regular local-path install can leave Vite following a package's `main: src/index.ts` symlink back into the repository, where that package has no `node_modules` on a fresh runner. Packing local dependencies into the app install keeps transitive resolution inside the app's dependency tree; prove the fix in a brand-new worktree with no pre-existing installs.
