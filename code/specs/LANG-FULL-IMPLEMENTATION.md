...
snapshot exponents and pure conditionals whose snapshot branches prove the
same value over plain real bases; these do not require a standard function in
the base.
...
- ◑ **AL4** — literal string `print`/`output` I/O runs on all 7 backends via
  **E4**. Undeclared statement-position `print('HI')`/`output('HI')` calls lower
  ...
  `f64_pow`. A pure conditional over two such snapshots may also participate
  when both branches prove the same exact value and the selector is
  call-free; selector execution remains in emitted IIR.
