...
describe("the chapter-owned real book-generation ledger", () => {
  ...
  it("has the measured stable owner counts", () => {
    ...
    // 61 -> 56 -> 51: Telugu, Malayalam and Sanskrit each retired chapters 1-5
    // in parallel, and both sides of the merge happened to read 61, so the
    // literal text-merged clean while the tree underneath it had moved. The
    // value below is the MEASURED count of the merged tree (HL-C242).
  ...
  it("covers exactly the chapters the tracks actually author", () => {
    ...
    // 51: Sanskrit chapters 1-5 are generated now, alongside Telugu's and
    // Malayalam's (HL-C242). Measured, not arithmetic -- see the note above.
