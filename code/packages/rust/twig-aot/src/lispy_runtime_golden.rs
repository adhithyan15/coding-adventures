...
extern "C" {
    fn __twig_lispy_box_int(n: i64) -> u64;
    fn __twig_lispy_unbox_int(v: u64) -> i64;
    fn __twig_lispy_nil() -> u64;
    fn __twig_lispy_cons(car: u64, cdr: u64) -> u64;
    fn __twig_lispy_car(pair: u64) -> u64;
    ...
    fn __twig_lispy_tag_int() -> u64;
    fn __twig_lispy_tag_nil() -> u64;
    fn __twig_lispy_tag_symbol() -> u64;
    fn __twig_lispy_tag_false() -> u64;
    fn __twig_lispy_tag_true() -> u64;
    fn __twig_lispy_tag_heap() -> u64;
    fn __twig_lispy_tag_mask() -> u64;
}
...
fn c_tag_constants_match_rust() {
    unsafe {
        assert_eq!(__twig_lispy_tag_int(), TAG_INT, "TAG_INT drift");
        assert_eq!(__twig_lispy_tag_nil(), TAG_NIL, "TAG_NIL drift");
        assert_eq!(__twig_lispy_tag_symbol(), TAG_SYMBOL, "TAG_SYMBOL drift");
        assert_eq!(__twig_lispy_tag_false(), TAG_FALSE, "TAG_FALSE drift");
        assert_eq!(__twig_lispy_tag_true(), TAG_TRUE, "TAG_TRUE drift");
        assert_eq!(__twig_lispy_tag_heap(), TAG_HEAP, "TAG_HEAP drift");
        assert_eq!(__twig_lispy_tag_mask(), TAG_BITS, "TAG_BITS drift");
    }
}
...
fn c_box_int_matches_rust_constructor() {
    ...
    for n in [0_i64, 1, 7, -1, 42, -42, 1 << 59, -(1 << 59), (1 << 60) - 1, -(1 << 60)] {
        let c = unsafe { __twig_lispy_box_int(n) };
        ...
        assert_eq!(unsafe { __twig_lispy_unbox_int(c) }, n, "unbox_int round-trip {n}");
    }
}
...
fn c_cons_car_cdr_round_trip() {
    let a = unsafe { __twig_lispy_box_int(7) };
    let b = unsafe { __twig_lispy_box_int(9) };
    let pair = unsafe { __twig_lispy_cons(a, b) };

    // A cons is heap-tagged.
    assert_eq!(pair & TAG_BITS, TAG_HEAP, "cons result not heap-tagged");
    // car/cdr recover the exact boxed values.
    assert_eq!(unsafe { __twig_lispy_car(pair) }, a, "car mismatch");
    assert_eq!(unsafe { __twig_lispy_cdr(pair) }, b, "cdr mismatch");
    // And those decode back to 7 / 9.
    assert_eq!(unsafe { __twig_lispy_unbox_int(__twig_lispy_car(pair)) }, 7);
    assert_eq!(unsafe { __twig_lispy_unbox_int(__twig_lispy_cdr(pair)) }, 9);
}

#[test]
fn c_nested_cons_round_trip() {
    // (CONS 1 (CONS 2 nil)) — a two-element list.
    let one = unsafe { __twig_lispy_box_int(1) };
    let two = unsafe { __twig_lispy_box_int(2) };
    let nil = unsafe { __twig_lispy_nil() };
    let inner = unsafe { __twig_lispy_cons(two, nil) };
    let outer = unsafe { __twig_lispy_cons(one, inner) };

    assert_eq!(unsafe { __twig_lispy_unbox_int(__twig_lispy_car(outer)) }, 1);
    let tail = unsafe { __twig_lispy_cdr(outer) };
    assert_eq!(unsafe { __twig_lispy_unbox_int(__twig_lispy_car(tail)) }, 2);
    assert_eq!(unsafe { __twig_lispy_cdr(tail) }, nil);
}
...
fn c_pair_p_returns_tagged_booleans() {
    let pair = unsafe { __twig_lispy_cons(__twig_lispy_box_int(1), __twig_lispy_nil()) };
    assert_eq!(unsafe { __twig_lispy_pair_p(pair) }, LispyValue::TRUE.bits());
    assert_eq!(unsafe { __twig_lispy_pair_p(__twig_lispy_box_int(7)) }, LispyValue::FALSE.bits());
    assert_eq!(unsafe { __twig_lispy_pair_p(__twig_lispy_nil()) }, LispyValue::FALSE.bits());
}
...
fn c_not_follows_lispy_truthiness() {
    ...
    let nil = unsafe { __twig_lispy_nil() };
    assert_eq!(unsafe { __twig_lispy_not(f) }, t, "not(#f) = #t");
    assert_eq!(unsafe { __twig_lispy_not(nil) }, t, "not(nil) = #t");
...
fn c_equal_atoms() {
    let t = LispyValue::TRUE.bits();
    let f = LispyValue::FALSE.bits();
    assert_eq!(unsafe { __twig_lispy_equal(__twig_lispy_box_int(5), __twig_lispy_box_int(5)) }, t);
    assert_eq!(unsafe { __twig_lispy_equal(__twig_lispy_box_int(5), __twig_lispy_box_int(6)) }, f);
    assert_eq!(unsafe { __twig_lispy_equal(__twig_lispy_nil(), __twig_lispy_nil()) }, t);
}

#[test]
fn c_equal_structural_pairs() {
    // (1 . 2) equals a freshly-built (1 . 2) even though the pointers differ.
    let p1 = unsafe { __twig_lispy_cons(__twig_lispy_box_int(1), __twig_lispy_box_int(2)) };
    let p2 = unsafe { __twig_lispy_cons(__twig_lispy_box_int(1), __twig_lispy_box_int(2)) };
    let p3 = unsafe { __twig_lispy_cons(__twig_lispy_box_int(1), __twig_lispy_box_int(3)) };
    assert_eq!(unsafe { __twig_lispy_equal(p1, p2) }, LispyValue::TRUE.bits(), "(1.2)=(1.2)");
    assert_eq!(unsafe { __twig_lispy_equal(p1, p3) }, LispyValue::FALSE.bits(), "(1.2)≠(1.3)");
    // A pair and an atom are never equal.
    assert_eq!(
        unsafe { __twig_lispy_equal(p1, __twig_lispy_box_int(1)) },
...
fn c_make_symbol_interns_consistently() {
    let foo1 = unsafe { __twig_lispy_make_symbol(b"FOO".as_ptr(), 3) };
    let foo2 = unsafe { __twig_lispy_make_symbol(b"FOO".as_ptr(), 3) };
    let bar = unsafe { __twig_lispy_make_symbol(b"BAR".as_ptr(), 3) };
    ...
    assert_eq!(unsafe { __twig_lispy_equal(foo1, foo2) }, LispyValue::TRUE.bits());
    assert_eq!(unsafe { __twig_lispy_equal(foo1, bar) }, LispyValue::FALSE.bits());
}

#[test]
fn c_make_symbol_empty_name() {
    // Degenerate but well-defined: the empty symbol interns consistently.
    let e1 = unsafe { __twig_lispy_make_symbol(b"".as_ptr(), 0) };
    let e2 = unsafe { __twig_lispy_make_symbol(b"".as_ptr(), 0) };
    assert_eq!(e1, e2);
