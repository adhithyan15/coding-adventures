...
namespace CodingAdventures.Argon2i;

/// <summary>Optional Argon2i inputs from RFC 9106.</summary>
public sealed class Argon2iOptions
{
    ...
    public uint Version { get; init; } = Argon2i.Version;
}

/// <summary>Pure C# Argon2i password hashing as specified by RFC 9106.</summary>
public static class Argon2i
{
    ...
    private const uint TypeI = 1;

    /// <summary>Compute an Argon2i tag.</summary>
    public static byte[] Derive(
        ...
        int parallelism,
        int tagLength,
        Argon2iOptions? options = null)
    {
        ArgumentNullException.ThrowIfNull(password);
        ArgumentNullException.ThrowIfNull(salt);
        options ??= new Argon2iOptions();
    ...
    /// <summary>Compute an Argon2i tag and return lowercase hexadecimal.</summary>
        ...
        int timeCost)
    {
        ...
        {
            if (index % BlockWords == 0 && !(pass == 0 && slice == 0 && index == 2))
            {
                NextAddresses();
            }
            ...
            var pseudoRandom = addressBlock[index % BlockWords];
            ...
            var newBlock = Compress(memory[lane][previousColumn], memory[referenceLane][referenceColumn]);
            if (pass == 0)
            {
