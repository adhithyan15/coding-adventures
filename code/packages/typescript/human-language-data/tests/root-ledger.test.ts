...
describe("the committed corpus", () => {
  ...
  it("pins the first ledger, and it is the reason the rule exists", () => {
    ...
    expect(l.summary.roots).toBe(3081); // Portuguese's typed olá opener records its etymon atom.
    expect(l.summary.underspent).toBe(2966); // The new Portuguese etymon begins below the reuse floor.
    ...
    expect(byNamespace).toEqual({ roots: 2194, "etymon-atom": 887 }); // +1: Portuguese's migrated opener etymon.
  });
