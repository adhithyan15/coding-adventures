...
Last prioritized: 2026-08-09, after merging #10195 and completing HL-C19 for publication.
...
| HL-C19 | Complete | Verify every prose `strokeOrder` against an authored ductus, so no letter's step list implies a pen lift nothing has checked. | The live audit found 228 prose entries across ten scripts, including the previously omitted Japanese inventory and four more Gujarati entries than the stale estimate: one letter — Tamil ம — carries `penLifts` + `strokeOrderSource` and the same cited, font-checked `DUCTUS`; all other 227 render as **shape parts in usual order, pen lifts unverified**. Validation rejects partial or malformed verification, and an integration gate proves every claimed count and source matches an authored ductus. [`data/scripts/README.md`](data/scripts/README.md) records the exact current breakdown. |
| HL-C78 | Queued next | Reconcile the early foundational backlog rows against what later deliveries actually shipped, starting with HL-C02: `chapters.json` types, `loadTrackChapters`, `core/chapter-policy.json`, representativeness policy, 22 track ledgers, and their round-trip tests now exist while the row still says queued. | Every pre-HL-C19 queued row is checked against direct code and test evidence; work that is already complete is closed with the PRs or concrete implementation that delivered it, partially complete rows state only their measured remainder, and genuinely absent work stays queued in priority order. |
