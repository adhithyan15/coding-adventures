...
Current selection: **RCPU-P005C**, the GE-225 controller-selector and optional
API slice. It implements exact `SEL`/`BCS` formats, opaque three-word command
blocks, eight fixed-priority controller plugs, deterministic selector and
controller completion events, busy alert halt, controller status predicates,
API source masks/latches, special X-group 32 save/vector behavior, deferred
priority-mode events, and the documented modified-branch return sequence.
RCPU-005 remains open for AAU and a final completion-coverage audit. Current
core coverage is 87.45% (1,680/1,921), above the 80% completion floor.
