\chapter{Farewells}
\label{ch:farewells}
\hlchaptermodality{4}
% canonical-insertion: PA-R04-wellbeing-r2 (mixed-schema guard: #13111)
% canonical-insertion: PA-S04-phappha-gagga (pre-A1 script runway: #13068)

Chapter 1 gave you the formal \pa{ਸਤਿ ਸ੍ਰੀ ਅਕਾਲ} (\emph{sat sr\=\i\ ak\=al}),
which doubles as a goodbye. Real days end more warmly:

\begin{center}
\pa{ਫਿਰ ਮਿਲਾਂਗੇ!} \quad(\emph{phir mil\=a\d{m}ge} --- ``We'll meet again!'')
\end{center}

\section[Script]{\pa{ਫ} $\cdot$ \pa{ਗ} $\cdot$ \pa{ਿ} --- three pieces met inside a word you already say}
\label{lesson:PA-S04-phappha-gagga}

The first word of this chapter is three pieces long, and the last one arrives
today.

\begin{center}
\pa{ਫ} \quad \pa{ਗ} \quad \pa{ਿ}
\end{center}

\pa{ਫ} is \emph{phapph\=a}, \emph{pha}: a consonant with a puff of breath after
it --- \emph{p} plus an audible \emph{h}, not the \emph{f} of English
\emph{phone}. \pa{ਗ} is \emph{gagg\=a}, \emph{ga}, the voiced \emph{g} of
\emph{go}, with no breath after it. \pa{ਿ} is the \emph{sih\=ar\=\i}, the short
\emph{i} sign, and the one that surprises everybody: it is \textbf{written
before} the consonant it is \textbf{spoken after}.

\begin{center}
\pa{ਫ} $+$ \pa{ਿ} $+$ \pa{ਰ} $\rightarrow$ \pa{ਫਿਰ} \quad(\emph{phir} --- ``again, then'')
\end{center}

You are not being asked to write any of this. You are being asked to
\textbf{recognise} it the next time it goes past.

\section{\pa{ਫਿਰ} \emph{(phir)} --- ``again''}
\label{lesson:phir}

\begin{cousinweb}
\pa{ਫਿਰ} (\emph{phir}) means ``again, then'' --- a native Indo-Aryan adverb (the
same word as Hindi \emph{phir}) with no tidy English cognate, but a familiar job:
it turns a parting into a promise.
\end{cousinweb}

\section{\pa{ਮਿਲਾਂਗੇ} \emph{(mil\=a\d{m}ge)} --- ``(we) will meet''}
\label{lesson:milaange}

\begin{cousinweb}
\pa{ਮਿਲਾਂਗੇ} (\emph{mil\=a\d{m}ge}, ``we will meet'') is the future of \pa{ਮਿਲਣਾ}
(\emph{mil\d{n}\=a}, ``to meet''), from Sanskrit \emph{mil-} (``to join'') --- the
root of \emph{mil\=ap} (``a meeting''). The ending \emph{-\=a\d{m}ge} is the plural
future.
\end{cousinweb}

\begin{grammarlens}
\textbf{The future is an ending.} \emph{mil-} $+$ \emph{-\=a\d{m}ge} $\to$
\emph{mil\=a\d{m}ge} --- no helper like English ``will.''
\end{grammarlens}

\section{\pa{ਫਿਰ ਮਿਲਾਂਗੇ} \emph{(phir mil\=a\d{m}ge)} --- ``we'll meet again''}
\label{lesson:phir-milaange}

\pa{ਫਿਰ} (again) $+$ \pa{ਮਿਲਾਂਗੇ} (we will meet) $=$ ``\textbf{we'll meet
again}'' --- the everyday, hopeful goodbye, where \emph{sat sr\=\i\ ak\=al} is the
formal one. It is the same ``promise of return'' the whole subcontinent prefers
(Hindi \emph{phir milenge}, Bengali \emph{\=ashi}, Tamil \emph{p\=oy
varugi\d{r}\=e\d{n}}).

\section[Practice]{The wellbeing cycle returns at R2}
\label{lesson:PA-R04-wellbeing-r2}

\begin{quote}
No new language. Keep three oral scores: respectful question, natural reply,
and courtesy close. Repair only a missed turn.
\end{quote}

\subsection*{Your turn}
Say these aloud from meaning alone:

\begin{enumerate}
\raggedright
  \item ask how someone is respectfully --- \emph{tusī̃ kivēṁ ho?}
  \item reply ``I am fine'' --- \emph{maiṁ ṭhīk hāṁ} --- then
    name the first-person \emph{m-} shared with English \emph{me/my}
  \item answer thanks with ``no problem'' --- \emph{koī gall nahīṁ}
    --- then name the negative \emph{n-} related to English \emph{no}
\end{enumerate}

Romanization records speech only. Gurmukhi reading and writing remain unscored
until \#13068 closes the load-bearing script runway.

\begin{tcolorbox}[breakable,colback=teal!4,colframe=teal!35!black,title={Before you move on}]
Perform all three turns once more without looking back. A typed romanization
never counts as Gurmukhi writing.
\end{tcolorbox}

\section{\pa{ਰੱਬ ਰਾਖਾ} \emph{(rabb r\=akh\=a)} --- ``God keep you''}
\label{lesson:rab-raakha}

\begin{cousinweb}
\pa{ਰੱਬ ਰਾਖਾ} means ``\textbf{[may] God [be your] keeper}'' --- \pa{ਰੱਬ}
(\emph{rabb}, ``God'') $+$ \pa{ਰਾਖਾ} (\emph{r\=akh\=a}, ``protector,'' from
\emph{rakh\d{n}\=a} $\leftarrow$ Sanskrit \emph{rak\d{s}-}, ``to guard''). The word
for God, \emph{rabb}, is itself \textbf{Arabic} (\emph{rabb}, ``lord'') --- so this
most Punjabi of farewells fuses an Arabic name for God with a Sanskrit verb of
keeping: the two vocabularies in one blessing.
\end{cousinweb}

\section{The farewells}
\label{PA-C04-practice}

\begin{center}
\begin{tabular}{@{}lll@{}}
\toprule
Punjabi & & Register \\
\midrule
\pa{ਸਤਿ ਸ੍ਰੀ ਅਕਾਲ} & \emph{sat sr\=\i\ ak\=al} & formal (Sikh greeting-goodbye) \\
\pa{ਫਿਰ ਮਿਲਾਂਗੇ} & \emph{phir mil\=a\d{m}ge} & everyday, ``we'll meet again'' \\
\pa{ਰੱਬ ਰਾਖਾ} & \emph{rabb r\=akh\=a} & a blessing, ``God keep you'' \\
\bottomrule
\end{tabular}
\end{center}

Roots banked: \emph{mil\=a\d{m}ge} (\emph{mil-} ``to join''), \emph{rabb} (Arabic
``God'') $+$ \emph{r\=akh\=a} (Sanskrit \emph{rak\d{s}-} ``to protect''). Next
chapter: the first real verbs --- \emph{mai\d{m} panj\=ab\=\i\ bold\=a h\=a\d{m}}.
