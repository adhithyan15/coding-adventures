...
- `CustodianCandidateSummary` and
  `summarize_custodian_candidates(candidates)` — count-only
  capability summaries over custodian candidate lists. The summary
  reports aggregate hardware/non-hardware, extractable/non-
  extractable, user-presence, remote, local, and total counts for
  startup planning without exposing custodian names, labels, wrapped
  blobs, or key bytes.
