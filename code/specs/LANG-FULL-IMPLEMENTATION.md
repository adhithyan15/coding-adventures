...
- ✅ **AL3** — typed procedures with value parameters. `integer procedure sq(x);
  ...
  propagated its dead seed → only the JIT returned 0). **Limits (follow-ups):** array formals use
  `value`; direct scalar call-by-name is AL7; zero-argument calls may use explicit `f()` in
...
- ✅ **AL7** — direct scalar call-by-name (Jensen-style expression thunks).
  ...
  retain normal copy semantics before storing through the caller binding.
  Name-array formals and recursive name-formal dispatch remain explicitly
  rejected pending a closure-capable ABI.
