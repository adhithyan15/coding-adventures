package CodingAdventures::PDF417;

# =============================================================================
# CodingAdventures::PDF417 — ISO/IEC 15438:2015 PDF417 stacked barcode encoder
# =============================================================================
#
...
# exactly 4 bars and 4 spaces (8 alternating elements) and every codeword
...
# ## Structural overview
#
# A PDF417 symbol is a rectangle of stacked rows.  Each row looks like:
#
#   [START 17] [LRI 17] [data_0 17] … [data_{c-1} 17] [RRI 17] [STOP 18]
#
# where c is the number of data columns.  Total modules per row = 69 + 17c.
#
# The codeword alphabet has 929 values (0–928).  Reed-Solomon error correction
# is computed over GF(929) — the integers modulo 929 (prime), not the binary
# extension field GF(256) used by QR Code and Data Matrix.
#
# ## Three codeword clusters
#
# PDF417 cycles through three bar/space pattern tables (clusters 0, 3, 6)
# row-by-row.  Row r uses cluster (r % 3) * 3.  The three patterns are different
# for the same codeword value, which lets a scanner verify it is reading a
# complete PDF417 row from the correct cluster, not an arbitrary horizontal
# slice through the symbol.
#
# ## v0.1.0 scope
#
# This release implements byte compaction only (codeword 924 latch).  All input
# bytes are encoded directly.  Text and numeric compaction are planned for
# v0.2.0.
#
# ## Encoding pipeline (in order)
#
#   1. Byte compaction — [924, c1, c2, ...] (6 bytes → 5 codewords in base 900)
#   2. Length descriptor — codeword 0 = total codewords in symbol
#   3. Auto ECC level — based on data codeword count
#   4. RS ECC — GF(929) Reed-Solomon, b=3 convention, α=3
#   5. Dimension selection — roughly square symbol
#   6. Padding — codeword 900 fills unused slots
#   7. Row indicators — LRI + RRI per row (encode R / C / ECC level)
#   8. Cluster table lookup — codeword → 17-module bar/space pattern
#   9. Start/stop patterns — fixed per row
#  10. ModuleGrid — abstract boolean grid (hashref with {modules, rows, cols})
#
# ## References
#
...
#   Cluster tables: extracted from the Python pdf417 library (MIT Licence)
#   Spec: code/specs/pdf417.md
#
# =============================================================================
...
# GF(929) is simply the integers modulo 929 — no irreducible polynomial
# construction is needed.
#
# The generator (primitive root) is α = 3.  This means the powers 3^0, 3^1,
# 3^2, ..., 3^{927} cycle through all 928 non-zero elements before repeating.
# ISO/IEC 15438:2015 Annex A.4 specifies α = 3.
#
# For fast multiplication we precompute log and antilog tables.  Given:
#   GF_EXP[i] = 3^i mod 929   (antilog / exponentiation table)
#   GF_LOG[v] = i such that 3^i = v  (discrete log table)
#
# Then  a × b = GF_EXP[(GF_LOG[a] + GF_LOG[b]) mod 928]
# This turns every GF(929) multiplication into two table lookups + one add.

use constant GF929_PRIME => 929;
use constant GF929_ALPHA => 3;
use constant GF929_ORDER => 928;   # multiplicative group order = PRIME - 1

our @GF929_EXP = (0) x 929;   # antilog:  GF929_EXP[i] = α^i mod 929
our @GF929_LOG = (0) x 929;   # log:      GF929_LOG[v] = i s.t. α^i ≡ v

# Build the GF(929) tables once at module load time.
# We iterate from i=0..927, computing 3^i mod 929.
#
# Example values to verify:
#   GF929_EXP[0] = 1   (3^0 = 1)
#   GF929_EXP[1] = 3   (3^1 = 3)
#   GF929_EXP[2] = 9   (3^2 = 9)
#   GF929_LOG[1] = 0, GF929_LOG[3] = 1, GF929_LOG[9] = 2
#
# Fermat's little theorem: 3^{928} ≡ 1 (mod 929), so the multiplicative
# group has order 928 and the tables cycle with period 928.
{
    my $val = 1;
    for my $i (0 .. GF929_ORDER - 1) {
        $GF929_EXP[$i] = $val;
        $GF929_LOG[$val] = $i;
        $val = ($val * GF929_ALPHA) % GF929_PRIME;
    }
    # GF929_EXP[928] = GF929_EXP[0] = 1 (wrap-around for index arithmetic)
    $GF929_EXP[GF929_ORDER] = $GF929_EXP[0];
}

# -----------------------------------------------------------------------------
# gf929_add($a, $b) — GF(929) addition: (a + b) mod 929
# -----------------------------------------------------------------------------
# In a prime field, addition is just modular integer addition.
# No XOR involved (unlike GF(256)).
...
# -----------------------------------------------------------------------------
# gf929_sub($a, $b) — GF(929) subtraction: (a - b + 929) mod 929
# -----------------------------------------------------------------------------
# We add 929 before reducing to ensure a non-negative result in Perl.

sub gf929_sub {
    my ($a, $b) = @_;
    return ($a - $b + GF929_PRIME) % GF929_PRIME;
}

# -----------------------------------------------------------------------------
# gf929_mul($a, $b) — GF(929) multiplication using log/antilog tables
# -----------------------------------------------------------------------------
# Special case: 0 times anything = 0 (log of zero is undefined).

sub gf929_mul {
    my ($a, $b) = @_;
    return 0 if $a == 0 || $b == 0;
    return $GF929_EXP[ ($GF929_LOG[$a] + $GF929_LOG[$b]) % GF929_ORDER ];
}
...
#          ∏_{j=3}^{k+2} (x − α^j)   over GF(929)
#
# We build g iteratively, starting from g = [1] and multiplying in each
# linear factor (x − α^j) = (x + (929 − α^j)) since −α^j ≡ 929 − α^j mod 929.
#
# After multiplying in all k factors, g is a degree-k polynomial:
#   g[0] = 1 (leading coefficient, always)
#   g[1..k] = computed coefficients

my %_gen_cache929;  # cache: ecc_level → arrayref of coefficients

sub _build_generator929 {
    my ($ecc_level) = @_;
    return $_gen_cache929{$ecc_level} if exists $_gen_cache929{$ecc_level};

    my $k = 1 << ($ecc_level + 1);   # 2^(ecc_level+1) ECC codewords
    my @g = (1);                       # g(x) = 1 initially

    for my $j (3 .. $k + 2) {
        my $root    = $GF929_EXP[$j % GF929_ORDER];  # α^j mod 929
        my $neg_root = GF929_PRIME - $root;           # −α^j in GF(929)

        my @new_g = (0) x (@g + 1);
        for my $i (0 .. $#g) {
            $new_g[$i]     = gf929_add($new_g[$i], $g[$i]);
            $new_g[$i + 1] = gf929_add($new_g[$i + 1], gf929_mul($g[$i], $neg_root));
        }
        @g = @new_g;
    }

    $_gen_cache929{$ecc_level} = \@g;
    return \@g;
}

# =============================================================================
# rs_encode(\@data, $ecc_level) — Reed-Solomon encoding over GF(929)
# =============================================================================
#
# Computes k = 2^(ecc_level+1) ECC codewords using the standard shift-register
# (LFSR) algorithm — equivalent to polynomial long division:
#
#   R(x) = D(x) × x^k  mod  g(x)
#
# The LFSR register has k cells, initially all zero.  For each data codeword d:
#   feedback = d + ecc[0]  (in GF(929): addition mod 929)
#   Shift: ecc[i] ← ecc[i+1]
#   ecc[k-1] ← 0
#   Add: ecc[i] += g[k-i] × feedback  for i in 0..k-1
#
# After processing all data, ecc[] contains the k ECC codewords.
#
# No interleaving: unlike QR Code, PDF417 feeds all data to a single RS encoder
# and produces all ECC in one pass.

sub rs_encode {
    my ($data_ref, $ecc_level) = @_;
    my @gen = @{ _build_generator929($ecc_level) };
    my $k   = $#gen;   # degree of generator = number of ECC codewords

    my @ecc = (0) x $k;

    for my $d (@$data_ref) {
        my $feedback = gf929_add($d, $ecc[0]);

        # Shift register left (ecc[0] is consumed, ecc[k-1] becomes 0)
        for my $i (0 .. $k - 2) {
            $ecc[$i] = $ecc[$i + 1];
        }
        $ecc[$k - 1] = 0;

        # Apply feedback through generator coefficients
        if ($feedback != 0) {
            for my $i (0 .. $k - 1) {
                $ecc[$i] = gf929_add($ecc[$i], gf929_mul($gen[$k - $i], $feedback));
            }
        }
    }
...
# =============================================================================
#
# Byte compaction encodes arbitrary byte sequences.  It operates in two modes:
#
# 1. Full-group encoding: every 6 bytes → 5 codewords
#    Treat 6 bytes as a 48-bit big-endian integer and express it in base 900.
#    256^6 = 281,474,976,710,656 < 900^5 = 590,490,000,000,000, so 6 bytes
#    always fit in exactly 5 base-900 codewords.
#
# 2. Remainder encoding: remaining 1–5 bytes → 1 codeword each (direct copy).
#    Values 0–255 are valid codeword values, so this needs no conversion.
#
# The sequence starts with codeword 924 (LATCH_BYTE), which signals the start
# of byte compaction to any reader.  We use 924 for all byte sequences
# regardless of whether the length is divisible by 6.

use constant LATCH_BYTE => 924;
use constant PADDING_CW => 900;

sub byte_compact {
    my ($bytes_ref) = @_;
    my @bytes = @$bytes_ref;
    my @codewords = (LATCH_BYTE);

    my $i   = 0;
    my $len = scalar @bytes;

    # Process full 6-byte groups: 6 bytes → 5 base-900 codewords.
    # We use Perl's big integer support via 64-bit native integers.
    # 256^6 = 2.81 × 10^14 which fits comfortably in a 64-bit integer
    # (max 9.2 × 10^18).
    while ($i + 6 <= $len) {
        # Build a 48-bit integer from 6 bytes (big-endian).
        my $n = 0;
        for my $j (0 .. 5) {
            $n = $n * 256 + $bytes[$i + $j];
        }

        # Convert to base 900: extract 5 codewords, least-significant first.
        my @group = (0) x 5;
        for my $j (4, 3, 2, 1, 0) {
            $group[$j] = $n % 900;
            # Integer division (Perl's int() truncates toward zero; n ≥ 0)
            $n = int($n / 900);
        }
        push @codewords, @group;
        $i += 6;
    }

    # Remaining 1–5 bytes: one codeword per byte (direct mapping).
    while ($i < $len) {
        push @codewords, $bytes[$i];
        $i++;
    }
...
# =============================================================================
#
# The auto-level formula is based on data codeword count (including the length
# descriptor but not ECC codewords).  Higher data counts get more ECC to
# maintain a useful error correction ratio.
#
#   Count ≤  40 → level 2 (8 ECC codewords, corrects 3 errors)
#   Count ≤ 160 → level 3 (16 ECC codewords, corrects 7 errors)
#   Count ≤ 320 → level 4 (32 ECC codewords, corrects 15 errors)
#   Count ≤ 863 → level 5 (64 ECC codewords, corrects 31 errors)
#   Otherwise   → level 6 (128 ECC codewords, corrects 63 errors)
...
# =============================================================================
#
# Choose the number of data columns c (1–30) and rows r (3–90) such that:
#   r × c ≥ total_codewords
#
# Heuristic: c = ceil(sqrt(total / 3)), clamped to 1–30.
# Then r = ceil(total / c), clamped to 3–90.
#
# The factor 3 accounts for the fact that PDF417 rows are typically about 3×
# wider than tall (each data codeword is 17 modules wide, while a row is
# typically 3–4 modules tall).  This produces a roughly square symbol.

use constant MIN_ROWS => 3;
use constant MAX_ROWS => 90;
use constant MIN_COLS => 1;
use constant MAX_COLS => 30;

sub choose_dimensions {
    my ($total) = @_;

    # c = ceil(sqrt(total / 3)), clamped to 1..30
    my $c = int(sqrt($total / 3) + 0.9999999);   # ceil via adding 1 - ε
    $c = MIN_COLS if $c < MIN_COLS;
    $c = MAX_COLS if $c > MAX_COLS;

    # r = ceil(total / c), clamped to 3..90
    my $r = int(($total + $c - 1) / $c);   # ceiling division
    $r = MIN_ROWS if $r < MIN_ROWS;
    $r = MAX_ROWS if $r > MAX_ROWS;
...
# =============================================================================
#
# Each row in a PDF417 symbol carries two indicator codewords:
#   LRI (Left Row Indicator) — left of the data codewords
#   RRI (Right Row Indicator) — right of the data codewords
#
# Together they encode three values distributed across the three clusters:
#   R_info = floor((R - 1) / 3)        — encodes total row count R
#   C_info = C - 1                     — encodes data column count C
#   L_info = 3 * ecc_level + (R-1)%3  — encodes ECC level + row parity
#
...
#
# Note: the RRI formula (Cluster 0 → C_info, Cluster 1 → R_info,
# Cluster 2 → L_info) follows the Python pdf417 library which produces
# verified scannable symbols; this differs slightly from the spec text wording.
#
# Example for R=10, C=3, ecc_level=2:
#   R_info = (10-1)/3 = 3
#   C_info = 3-1 = 2
#   L_info = 3*2 + (10-1)%3 = 6 + 0 = 6
#   Row 0 (cluster 0): LRI = 30*0 + 3 = 3, RRI = 30*0 + 2 = 2
#   Wait — this uses C_info for RRI in cluster 0.
#   Row 1 (cluster 1): LRI = 30*0 + 6 = 6, RRI = 30*0 + 3 = 3
#   Row 3 (cluster 0): LRI = 30*1 + 3 = 33, RRI = 30*1 + 2 = 32

sub compute_lri {
    my ($r, $rows, $cols, $ecc_level) = @_;
    my $r_info    = int(($rows - 1) / 3);
    my $c_info    = $cols - 1;
    my $l_info    = 3 * $ecc_level + ($rows - 1) % 3;
    my $row_group = int($r / 3);
    my $cluster   = $r % 3;

    return 30 * $row_group + $r_info if $cluster == 0;
    return 30 * $row_group + $l_info if $cluster == 1;
...
sub compute_rri {
    ...
    return 30 * $row_group + $c_info if $cluster == 0;
    return 30 * $row_group + $r_info if $cluster == 1;
...
# =============================================================================
#
# Each of the 929 codeword values (0–928) maps to a different bar/space pattern
# in each of the three clusters.  A pattern consists of 8 alternating elements:
# bar, space, bar, space, bar, space, bar, space.  The widths sum to exactly 17.
#
# The tables are stored as packed 32-bit integers:
#   bits 31..28 = b1 (width of first bar)
#   bits 27..24 = s1
#   bits 23..20 = b2
#   bits 19..16 = s2
#   bits 15..12 = b3
...
#   bits  3..0  = s4
#
# Three arrays × 929 entries = 2787 patterns total.
# These tables were extracted from the Python pdf417 library (MIT Licence)
# and verified against known test vectors.
#
# Index mapping: cluster 0 → $CLUSTER_TABLES[0], cluster 3 → $CLUSTER_TABLES[1],
# cluster 6 → $CLUSTER_TABLES[2].  We use $r % 3 as the index.
...
# Start pattern: 8 bar/space widths, 17 modules total
# Module sequence: 11111111010101000
# Widths: bar=8, space=1, bar=1, space=1, bar=1, space=1, bar=1, space=3
our @START_PATTERN = (8, 1, 1, 1, 1, 1, 1, 3);

# Stop pattern: 9 bar/space widths, 18 modules total
# Module sequence: 111111101000101001
# Widths: bar=7, space=1, bar=1, space=3, bar=1, space=1, bar=1, space=2, bar=1
our @STOP_PATTERN = (7, 1, 1, 3, 1, 1, 1, 2, 1);

# =============================================================================
# expand_codeword_to_modules($packed, $modules_ref)
# =============================================================================
#
# Unpacks a 32-bit cluster table entry into 17 boolean module values and
# appends them to the $modules_ref array.
#
# The packed format uses 4 bits per element:
#   bits 31..28 = b1 (first bar width), bits 27..24 = s1, ..., bits 3..0 = s4.
# Each element alternates: bar (dark=1), space (dark=0).

sub _expand_packed {
    my ($packed, $modules_ref) = @_;
    my @widths = (
        ($packed >> 28) & 0xF,   # b1
        ($packed >> 24) & 0xF,   # s1
        ($packed >> 20) & 0xF,   # b2
    ...
    my $dark = 1;
    for my $w (@widths) {
        push @$modules_ref, ($dark) x $w;
        $dark = 1 - $dark;
    }
}

# =============================================================================
# _expand_widths(\@widths, $modules_ref)
# =============================================================================
#
# Expand an array of bar/space widths into boolean module values.
# The first element is always a bar (dark=1); subsequent elements alternate.

sub _expand_widths {
    my ($widths_ref, $modules_ref) = @_;
    my $dark = 1;
    for my $w (@$widths_ref) {
        push @$modules_ref, ($dark) x $w;
        $dark = 1 - $dark;
...
# =============================================================================
# _rasterize($sequence_ref, $rows, $cols, $ecc_level, $row_height)
# =============================================================================
#
# Assembles all rows into a ModuleGrid (hashref).
#
# Each logical PDF417 row produces a strip of $row_height identical module rows.
# Within each row the module sequence is:
#   [start: 17] [LRI: 17] [data×cols: 17 each] [RRI: 17] [stop: 18]
# = 69 + 17c modules per row.

sub _rasterize {
    my ($sequence_ref, $rows, $cols, $ecc_level, $row_height) = @_;

    my $module_width  = 69 + 17 * $cols;
    my $module_height = $rows * $row_height;

    # Precompute the start and stop module arrays — same for every row.
    my @start_modules;
    _expand_widths(\@START_PATTERN, \@start_modules);

    my @stop_modules;
    _expand_widths(\@STOP_PATTERN, \@stop_modules);

    # Build the module grid as a flat array of ($module_height × $module_width)
    # cells, initialised to 0 (light).
    my @grid = (0) x ($module_height * $module_width);

    for my $r (0 .. $rows - 1) {
        my $cluster_idx = $r % 3;   # 0, 1, or 2
        my $cluster_ref = $CLUSTER_TABLES[$cluster_idx];

        # Assemble the 17+17+17c+17+18 = (69+17c) modules for this logical row.
        my @row_modules;
        _expand_widths(\@START_PATTERN, \@row_modules);

        # Left row indicator
        my $lri = compute_lri($r, $rows, $cols, $ecc_level);
        _expand_packed($cluster_ref->[$lri], \@row_modules);

        # Data codewords
        for my $j (0 .. $cols - 1) {
            my $cw = $sequence_ref->[$r * $cols + $j];
            _expand_packed($cluster_ref->[$cw], \@row_modules);
        }

        # Right row indicator
        my $rri = compute_rri($r, $rows, $cols, $ecc_level);
        _expand_packed($cluster_ref->[$rri], \@row_modules);

        _expand_widths(\@STOP_PATTERN, \@row_modules);

        # Sanity check: the row must be exactly $module_width modules wide.
        if (scalar(@row_modules) != $module_width) {
            croak sprintf(
                "Internal error: row %d has %d modules, expected %d",
                $r, scalar(@row_modules), $module_width
            );
        }

        # Write this module row $row_height times into the flat grid.
        my $row_base = $r * $row_height;
        for my $h (0 .. $row_height - 1) {
            my $offset = ($row_base + $h) * $module_width;
            for my $col (0 .. $module_width - 1) {
                $grid[$offset + $col] = $row_modules[$col];
            }
        }
    }

    # Wrap into a ModuleGrid hashref using CodingAdventures::Barcode2D.
    # make_module_grid($rows, $cols) creates an all-light grid; we then fill it
    # by directly setting the modules array (faster than calling set_module for
    # every dark pixel in a potentially large symbol).
    my $module_grid = CodingAdventures::Barcode2D->make_module_grid(
        $module_height, $module_width, 'square'
    );

    # Populate the grid from our flat array.
    for my $row (0 .. $module_height - 1) {
        for my $col (0 .. $module_width - 1) {
            $module_grid->{modules}[$row][$col] = $grid[$row * $module_width + $col];
        }
    }
...
# =============================================================================
# encode(\@bytes, \%options) — main entry point
# =============================================================================
#
# Encodes an array of byte values (0–255) as a PDF417 symbol and returns a
# ModuleGrid hashref.
#
# Options (all optional):
#   ecc_level  — integer 0–8; default: auto-selected from data length
#   columns    — integer 1–30; default: auto-selected for roughly square symbol
#   row_height — integer 1–10; default: 3
#
# The full encoding pipeline:
#   1. Byte compaction:  [924, c1, c2, ...]
#   2. Length descriptor: prepend (1 + n_data + n_ecc)
#   3. RS ECC over GF(929)
#   4. Choose dimensions r×c
#   5. Pad to r×c with codeword 900
#   6. full_sequence = [data+pad, ecc]
#   7. Rasterize row-by-row with cluster tables + row indicators

sub encode {
    my ($bytes_ref, $opts_ref) = @_;
    $opts_ref //= {};

    my @bytes = @$bytes_ref;

    # ── Validate ECC level ────────────────────────────────────────────────────
    if (defined $opts_ref->{ecc_level}) {
        my $lvl = $opts_ref->{ecc_level};
        croak "ecc_level must be 0–8, got $lvl"
            unless $lvl >= 0 && $lvl <= 8;
    }

    # ── Validate columns ──────────────────────────────────────────────────────
    if (defined $opts_ref->{columns}) {
        my $c = $opts_ref->{columns};
        croak "columns must be 1–30, got $c"
            unless $c >= MIN_COLS && $c <= MAX_COLS;
    }

    # ── Step 1: Byte compaction ───────────────────────────────────────────────
    my @data_cwords = @{ byte_compact(\@bytes) };

    # ── Step 2: Auto-select ECC level ─────────────────────────────────────────
    # Data count for ECC selection = 1 (length descriptor) + data codewords.
    my $ecc_level = $opts_ref->{ecc_level} // auto_ecc_level(1 + scalar @data_cwords);
    my $ecc_count = 1 << ($ecc_level + 1);   # 2^(ecc_level+1)

    # ── Step 3: Length descriptor ─────────────────────────────────────────────
    # The length descriptor = 1 + n_data + n_ecc (does NOT count padding).
    my $length_desc = 1 + scalar(@data_cwords) + $ecc_count;
    my @full_data = ($length_desc, @data_cwords);

    # ── Step 4: Reed-Solomon ECC ──────────────────────────────────────────────
    my @ecc_cwords = @{ rs_encode(\@full_data, $ecc_level) };

    # ── Step 5: Choose dimensions ─────────────────────────────────────────────
    my $total_cwords = scalar(@full_data) + scalar(@ecc_cwords);

    my ($rows, $cols);
    if (defined $opts_ref->{columns}) {
        $cols = $opts_ref->{columns};
        $rows = int(($total_cwords + $cols - 1) / $cols);   # ceiling division
        $rows = MIN_ROWS if $rows < MIN_ROWS;
        if ($rows > MAX_ROWS) {
            croak sprintf(
                "Data requires %d rows (max %d) with %d columns — input too long",
                $rows, MAX_ROWS, $cols
            );
        }
    } else {
        ($rows, $cols) = choose_dimensions($total_cwords);
    }

    # Verify capacity after dimension selection.
    if ($rows * $cols < $total_cwords) {
        croak sprintf(
            "Cannot fit %d codewords into %d×%d grid — input too long",
            $total_cwords, $rows, $cols
        );
    }

    # ── Step 6: Pad to fill the grid ─────────────────────────────────────────
    my $padding_count = $rows * $cols - $total_cwords;
    my @padded_data   = (@full_data, (PADDING_CW) x $padding_count);

    # full_sequence = [length_desc + data + padding] + [ecc]
    # Length: $rows × $cols codewords.
    my @full_sequence = (@padded_data, @ecc_cwords);

    # ── Step 7: Rasterize ─────────────────────────────────────────────────────
    my $row_height = $opts_ref->{row_height} // 3;
    $row_height = 1 if $row_height < 1;

    return _rasterize(\@full_sequence, $rows, $cols, $ecc_level, $row_height);
}

# =============================================================================
# encode_str($string, \%options) — convenience wrapper
# =============================================================================
#
# Encodes a Perl string as UTF-8 bytes.  For plain ASCII input the result is
# identical to encoding the ASCII byte values directly.

sub encode_str {
    my ($str, $opts_ref) = @_;
    $opts_ref //= {};

    # Encode the string as a sequence of byte values (0–255).
    # For ASCII input this is simply the ordinal value of each character.
    # For multi-byte UTF-8 we encode each byte of the UTF-8 representation.
    my @bytes;
    if (utf8::is_utf8($str)) {
        utf8::encode($str);   # converts to octets in-place (string is now bytes)
    }
    @bytes = map { ord($_) } split //, $str;

    return encode(\@bytes, $opts_ref);
}
...
=head1 SYNOPSIS
    ...
    # Encode raw bytes
    my $grid = encode([72, 101, 108, 108, 111]);
    ...
    # Encode with options
    my $grid = encode_str("HELLO WORLD", {
        ecc_level  => 4,   # ECC level 0-8 (default: auto)
...
PDF417 encoder conforming to ISO/IEC 15438:2015.

PDF417 (Portable Data File 417) is a stacked linear barcode used by:

=over 4

=item * AAMVA — North American driver's licences

=item * IATA BCBP — airline boarding passes

=item * USPS — domestic shipping labels

=item * US immigration — Form I-94

=item * Healthcare — patient wristbands, medication labels

=back

This v0.1.0 release implements byte compaction only.  Text and numeric
compaction are planned for v0.2.0.

=head1 FUNCTIONS

=head2 encode(\@bytes, \%options)

Encode a byte array as a PDF417 symbol.  Returns a ModuleGrid hashref.

=head2 encode_str($string, \%options)

Encode a Perl string (converted to bytes).

=head2 byte_compact(\@bytes)

Convert bytes to PDF417 byte-mode codewords.  Returns arrayref starting
with codeword 924.

=head2 auto_ecc_level($data_count)

Return the recommended ECC level (2–6) for the given data codeword count.

=head2 choose_dimensions($total_codewords)

Return C<($rows, $cols)> for a roughly square symbol.

=head2 compute_lri($r, $rows, $cols, $ecc_level)

Compute the Left Row Indicator codeword value for row C<$r>.

=head2 compute_rri($r, $rows, $cols, $ecc_level)

Compute the Right Row Indicator codeword value for row C<$r>.

=head2 rs_encode(\@data, $ecc_level)

Compute ECC codewords over GF(929).  Returns arrayref of ECC codewords.
