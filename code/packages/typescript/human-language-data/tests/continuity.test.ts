...
describe("the real corpus", () => {
  ...
  it("pins what nothing had measured", { timeout: 30_000 }, () => {
    ...
    */
    expect(report.summary.missedByWindow.R2).toBe(3685); // The Spanish forward-language practice revisits two atoms inside R2 after Italian's opener exposed them.
  });
  ...
  it("shows what a declared reading order was worth", () => {
    ...
    expect(spanish).toMatchObject({
      ...
      lessonCount: 585, // +1: the first forward-language practice lesson closes its own gentle one-concept rung.
      /* Historical cumulative lesson-count ledger retained below.
