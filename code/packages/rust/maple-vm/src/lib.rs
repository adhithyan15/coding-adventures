//! # `maple-vm` — Maple's own v0 interpreter.
//!
//! Executes the [`IIRModule`] produced by `maple-iir-compiler` against
//! the [`dynval_runtime`] value model and returns a [`LispyValue`]. See
//! [`maple-iir-vm.md`](../../../specs/maple-iir-vm.md) for the full
//! design.
//!
//! ## Why a dedicated VM
//!
...
//!     ],
//! );
//! let mut module = IIRModule::new("demo", "maple");
...
/// A failure while interpreting a Maple [`IIRModule`].
...
///
/// The operand encodings match what `maple-iir-compiler` emits (the
...
fn eval_const(instr: &IIRInstr) -> Result<LispyValue, VmError> {
    ...
    match src {
        ...
        Operand::Bool(_) => Err(VmError::Malformed(
            "Maple v0 has no boolean literals; unexpected Bool operand in `const`".into(),
        )),
        Operand::Float(_) => Err(VmError::Malformed(
            "Maple v0 has no floats (maple-iir-vm.md); unexpected Float operand in `const`".into(),
        )),
        Operand::Str(_) => Err(VmError::Malformed(
            "Maple v0 has no string literals; unexpected Str operand in `const`".into(),
        )),
    }
}
...
/// Map a Maple v0 builtin name to its `dynval-runtime` implementation.
...
mod tests {
    ...
    fn module(instrs: Vec<IIRInstr>) -> IIRModule {
        ...
        let mut m = IIRModule::new("test", "maple");
