...
describe("corpus snapshot", () => {
  ...
  it("pins where the corpus actually stands on the ladder", () => {
    ...
    expect(summary.byLevel["pre-A1"]).toBe(892); // +3: HL-C97 adds the repair kit (no entiendo, mas despacio) at chapter 14
  ...
  it("can already build a ramp-to-A1 edition from the canonical corpus", () => {
    ...
    expect(ramp).toHaveLength(1202); // +3: HL-C97 adds the repair kit (no entiendo, mas despacio) at chapter 14
