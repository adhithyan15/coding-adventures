...
- `CodingAdventures.Cas.BlobStore` — Elixir `@behaviour` defining the four
  callbacks (`put/3`, `get/2`, `exists?/2`, `keys_with_prefix/2`) that any
  storage backend must implement.

- `CodingAdventures.Cas.Store` — Content-addressable store that wraps any
  `BlobStore`:
...
- `CodingAdventures.Cas.LocalDiskStore` — Filesystem backend using Git's 2/38
...
- `CodingAdventures.Cas.Hex` — Hex utilities:
...
- `CodingAdventures.Cas.Error` — Typed error reason catalogue with `format/1`
  for human-readable messages.

- Full ExUnit test suite (`test/cas_test.exs`) covering:
