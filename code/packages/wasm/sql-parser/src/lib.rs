...
use language_parser::create_sql_parser;
...
pub fn parse(source: &str) -> Result<String, JsValue> {
    let mut parser = create_sql_parser(source).map_err(to_js_error)?;
    let ast = parser
        .parse()
        .map_err(|e| to_js_error(format!("SQL parse failed: {e}")))?;
...
mod tests {
    ...
    fn wrapper_parses_to_json() {
        let json = parse("SELECT 1 FROM t").unwrap();
