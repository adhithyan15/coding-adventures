...
- `Decision::{is_allow,is_deny,reason}` helpers for callers that
  need to branch on outcomes without pattern-matching every time.
- `PolicyDecisionRecord` and `decide_with_record` for compact,
