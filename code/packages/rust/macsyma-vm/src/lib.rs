//! # `macsyma-vm` — Macsyma's own v0 interpreter.
//!
//! Executes the [`IIRModule`] produced by `macsyma-iir-compiler` against
//! the [`dynval_runtime`] value model and returns a [`LispyValue`]. See
//! [`macsyma-iir-vm.md`](../../../specs/macsyma-iir-vm.md) for the full
//! design (the v0 scope, why arithmetic evaluates on the VM instead of
//! being frontend-folded, and the rollout waves beyond v0).
//!
//! ## Why a dedicated VM (and not `twig-vm` or `mccarthy-lisp-vm`)
//!
//! Twig and McCarthy Lisp already have their own small VMs on top of
//! [`dynval_runtime`]'s tagged-`i64` value model — deliberately *not*
//! shared with each other (see `mccarthy-lisp-vm`'s own module doc).
//! Macsyma follows the identical precedent: its own VM, sharing only the
//! `dynval_runtime` foundation, not `twig-vm`'s or `mccarthy-lisp-vm`'s
//! instruction set or opcodes.
//!
//! ## The v0 instruction set
//!
//! `macsyma-iir-compiler` emits a deliberately tiny IIR — no branches, no
...
//! concrete operands — genuinely executed here, not folded by the
//! frontend) and its unevaluated-symbolic-expression representation
//! (`cons`, building the same inert `(head arg0 arg1 …)` chain
//! `mccarthy-lisp-iir-compiler::lower_quote` uses for `QUOTE`).
//!
//! ## Quick start
//!
//! ```
...
//!     ],
//! );
//! let mut module = IIRModule::new("demo", "macsyma");
...
/// A failure while interpreting a Macsyma [`IIRModule`].
...
pub enum VmError {
    ...
    /// (`[-2^60, 2^60 - 1]`). A known v0 gap tied to Wave 2 (bignum
    /// support) — see `macsyma-iir-vm.md` §6.
...
///
/// The operand encodings match what `macsyma-iir-compiler` emits (the same
/// conventions `mccarthy-lisp-iir-compiler` established for this value
/// model):
///
...
fn eval_const(instr: &IIRInstr) -> Result<LispyValue, VmError> {
    ...
    match src {
        ...
        Operand::Bool(_) => Err(VmError::Malformed(
            "Macsyma v0 has no boolean literals; unexpected Bool operand in `const`".into(),
        )),
        Operand::Float(_) => Err(VmError::Malformed(
            "Macsyma v0 has no floats (macsyma-iir-vm.md §3); unexpected Float operand in `const`"
                .into(),
        )),
        Operand::Str(_) => Err(VmError::Malformed(
            "Macsyma v0 has no string literals; unexpected Str operand in `const`".into(),
        )),
    }
}
...
/// Map a Macsyma v0 builtin name to its `dynval-runtime` implementation.
///
...
/// `mccarthy-lisp-vm`'s precedent of a slightly-generous table — free to
...
mod tests {
    ...
    fn module(instrs: Vec<IIRInstr>) -> IIRModule {
        ...
        let mut m = IIRModule::new("test", "macsyma");
    ...
    fn div_truncates_toward_zero() {
        // Documents the known v0 gap (macsyma-iir-vm.md §3): dynval-runtime's
        // `/` is C-style truncating integer division, not Macsyma's exact
        // rational. `macsyma-iir-compiler` only ever emits `/` when it has
        // verified the division is exact, so this truncation is never
        // reachable through the compiler — this test pins the VM's own raw
        // behaviour for a hand-built module.
        let m = module(vec![
            konst("a", Operand::Int(7), "i64"),
            konst("b", Operand::Int(2), "i64"),
            builtin_instr("r", "/", &["a", "b"]),
            ret("r"),
        ]);
    ...
    fn cons_builds_an_inert_pair() {
        // The shape `macsyma-iir-compiler` uses for an unevaluated Apply
