...
describe("the committed corpus", () => {
  it("pins the first measurement", () => {
    ...
    expect(r.summary.worstTerms.slice(0, 2).map((e) => e.term)).toEqual(["verb", "noun"]); // FLOOR — content only grows; exact pins serialize parallel tranches
    expect(r.summary.worstTerms[0]?.lessons ?? 0).toBeGreaterThanOrEqual(1016); // spanish pre-A1 survival tranche: +15 lessons, +3 chapters (chapters 303-305): +2 // Marathi's explicit review sessions retrieve its verb families at distance. // FLOOR — content only grows; exact pins serialize parallel tranches
    expect(r.summary.worstTerms[1]?.lessons ?? 0).toBeGreaterThanOrEqual(533); // spanish pre-A1 survival tranche: +15 lessons, +3 chapters (chapters 303-305): +2 // Marathi's family review explicitly contrasts the noun histories. // FLOOR — content only grows; exact pins serialize parallel tranches
