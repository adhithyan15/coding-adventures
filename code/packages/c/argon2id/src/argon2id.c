/*
 * argon2id.c — implementation of Argon2id (see argon2id.h). A faithful port of the
 * Rust `argon2id` crate. It shares the Argon2 G compression, permutation P, H'
 * extender, and index_alpha mapping with Argon2d and Argon2i; the addressing is
 * *hybrid* (RFC 9106 §3.4): data-independent (address stream) for the first two
 * slices of the first pass, data-dependent (prev_block[0]) everywhere else.
 */
#include "argon2id.h"
...
#define TYPE_ID 2u
#define ADDRESSES_PER_BLOCK BLOCK_WORDS
...
/* Compression G(x, y). `out` must be distinct from x and y. */
...
/* ---- data-independent address stream (RFC 9106 §3.4.2) --------------- */
...
typedef struct {
    ...
    uint64_t t;
    uint64_t counter;
    uint64_t buf[BLOCK_WORDS];
    size_t idx;
} AddressStream;

static void address_stream_init(AddressStream *a, size_t r, size_t lane,
                                size_t sl, size_t m_prime, size_t t) {
    a->r = (uint64_t)r;
    a->lane = (uint64_t)lane;
    a->sl = (uint64_t)sl;
    a->m_prime = (uint64_t)m_prime;
    a->t = (uint64_t)t;
    a->counter = 0;
    a->idx = ADDRESSES_PER_BLOCK; /* force a refill on the first call */
}

static uint64_t address_stream_next(AddressStream *a) {
    uint64_t v;
    if (a->idx >= ADDRESSES_PER_BLOCK) {
        uint64_t zero[BLOCK_WORDS];
        uint64_t input_block[BLOCK_WORDS];
        uint64_t once[BLOCK_WORDS];
        memset(zero, 0, sizeof zero);
        memset(input_block, 0, sizeof input_block);
        a->counter += 1;
        input_block[0] = a->r;
        input_block[1] = a->lane;
        input_block[2] = a->sl;
        input_block[3] = a->m_prime;
        input_block[4] = a->t;
        input_block[5] = TYPE_ID;
        input_block[6] = a->counter;
        compress(zero, input_block, once);
        compress(zero, once, a->buf);
        a->idx = 0;
    }
    v = a->buf[a->idx];
    a->idx += 1;
...
static void fill_segment(uint64_t *memory, size_t r, size_t lane, size_t sl,
                         size_t q, size_t sl_len, size_t p, size_t m_prime,
                         size_t t) {
    size_t starting_c = (r == 0 && sl == 0) ? 2 : 0;
    /* Argon2id: data-independent addressing for the first two slices of the
     * first pass, data-dependent everywhere else (RFC 9106 §3.4). */
    int di = (r == 0 && sl < 2);
    size_t i;
    uint64_t newb[BLOCK_WORDS];
    AddressStream addr;

    address_stream_init(&addr, r, lane, sl, m_prime, t);
    if (di) {
        /* Consume the addresses for the skipped columns too, so the stream
         * stays in phase (RFC 9106 §3.4.2). */
        for (i = 0; i < starting_c; i++) {
            (void)address_stream_next(&addr);
        }
    }
    ...
    for (i = starting_c; i < sl_len; i++) {
        ...
        uint64_t pseudo_rand = di ? address_stream_next(&addr) : prev_block[0];
...
static Argon2idStatus validate(size_t password_len, size_t salt_len,
    ...
    if ((uint64_t)password_len > 0xFFFFFFFFull) {
        return ARGON2ID_PASSWORD_TOO_LONG;
    }
    if (salt_len < 8) {
        return ARGON2ID_SALT_TOO_SHORT;
    }
    if ((uint64_t)salt_len > 0xFFFFFFFFull) {
        return ARGON2ID_SALT_TOO_LONG;
    }
    if ((uint64_t)key_len > 0xFFFFFFFFull) {
        return ARGON2ID_KEY_TOO_LONG;
    }
    if ((uint64_t)ad_len > 0xFFFFFFFFull) {
        return ARGON2ID_AD_TOO_LONG;
    }
    if (tag_length < 4) {
        return ARGON2ID_TAG_TOO_SMALL;
    }
    if (parallelism < 1 || parallelism > 0xFFFFFF) {
        return ARGON2ID_INVALID_PARALLELISM;
    }
    if (memory_cost < 8 * parallelism) {
        return ARGON2ID_MEMORY_TOO_SMALL;
    }
    if (time_cost < 1) {
        return ARGON2ID_TIME_COST_ZERO;
    }
    if (version != ARGON2ID_VERSION) {
        return ARGON2ID_UNSUPPORTED_VERSION;
    }
    return ARGON2ID_OK;
}
...
Argon2idStatus argon2id(const uint8_t *password, size_t password_len,
    ...
    size_t ad_len = 0;
    uint32_t version = ARGON2ID_VERSION;
    Argon2idStatus rc;
    size_t p;
    size_t t;
