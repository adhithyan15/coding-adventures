...
A Python implementation of a VGA text-mode framebuffer display driver, simulating the classic 80x25 text mode that every x86 PC boots into.
...
```python
from display import DisplayDriver, DisplayConfig, BYTES_PER_CELL, make_attribute
from display import COLOR_WHITE, COLOR_BLUE
...
print(snap.contains("Hello"))  # True

# Write colored text at a specific position
attr = make_attribute(COLOR_WHITE, COLOR_BLUE)
driver.put_char_at(5, 10, ord('X'), attr)
```

## API Summary

- `DisplayDriver(config, memory)` --- create a driver backed by a memory region
