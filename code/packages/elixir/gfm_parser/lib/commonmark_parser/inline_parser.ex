defmodule CodingAdventures.CommonmarkParser.InlineParser do
  @moduledoc """
  Inline-Level Parser — Phase 2 of GFM parsing.
  ...
  ## Algorithm: Delimiter Stack (GFM Appendix A)
  ...
  defp do_scan(scanner, acc, refs) do
    if Scanner.done?(scanner) do
      ...
      cond do
        ...
        ch == "*" or ch == "_" or ch == "~" -> scan_delim_run(scanner, acc, refs)
  ...
  defp scan_html_comment(text) do
    # GFM spec: HTML comment = "<!--" + content + "-->" where:
  ...
  defp scan_html_tag(text) do
    # Per GFM spec, inline raw HTML tags must be:
    ...
    # Attribute values can span newlines per GFM spec (examples 642, 643).
  ...
  defp scan_bracket_close(scanner, acc, refs, _source) do
    ...
    # GFM algorithm: check if the top bracket opener is deactivated (dead).
      ...
      :active_or_none ->
        ...
        case find_link_opener_in(acc) do
          ...
          {_opener_ref, opener_char, opener_src_pos, before_acc, inner_acc} ->
            ...
            # Per GFM spec §4.7, link labels are compared using raw source
  ...
  #
  # Per GFM spec §4.7, link label matching uses the raw source text
  ...
  # ── Emphasis Resolution (GFM Appendix A) ────────────────────────────────

  # Items is a list in LEFT-TO-RIGHT order.
  # We implement the GFM algorithm:
  ...
  defp resolve_closers(arr, n, pos) do
    ...
    case item do
      ...
      when can_close and ((char in ["*", "_"] and count > 0) or (char == "~" and count >= 2)) ->
        case find_opener(arr, pos - 1, char, count, can_open_closer) do
          ...
          opener_pos ->
            ...
            use_count = determine_use_count(char, opener_count, count)
            ...
            node =
              cond do
                char == "~" -> DocumentAst.strikethrough(inner_nodes)
                use_count >= 2 -> DocumentAst.strong(inner_nodes)
                true -> DocumentAst.emphasis(inner_nodes)
              end
  ...
  defp find_opener(arr, pos, char, closer_count, closer_can_open) do
    item = elem(arr, pos)
    case item do
      {:delim, ^char, count, can_open, can_close}
      when can_open and ((char in ["*", "_"] and count > 0) or (char == "~" and count >= 2)) ->
        # Rule of 3 (GFM spec rule 14): When the sum of opener and closer
        ...
        use_count = determine_use_count(char, count, closer_count)
        ambiguous = char != "~" && (can_close || closer_can_open)
  ...
  defp determine_use_count("~", _opener_count, _closer_count), do: 2

  defp determine_use_count(_char, opener_count, closer_count) do
  ...
  defp extract_plain_text(nodes) do
    Enum.map_join(nodes, "", fn
      ...
      %{type: :strikethrough, children: c} -> extract_plain_text(c)
