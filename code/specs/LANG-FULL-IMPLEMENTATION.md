...
- ◑ **AL4** — literal string `print`/`output` I/O runs on all 7 backends via
  **E4**. Undeclared statement-position `print('HI')`/`output('HI')` calls lower
  ...
  A power whose base contains a pure standard-function result also accepts
  finite arithmetic that mixes initialized ordinary local integer and real
  snapshots when the result is an exact nonnegative integer within the same
  unroll cap. Every integer must widen exactly to binary64, and both numeric
  domains must be present. A pure conditional may participate when every
  runtime branch proves the same exact bounded value; its selector remains in
  emitted IIR. Uninitialized or nonlocal values, inexact integer widening,
  fractional or non-finite results, differing branches, effectful selectors,
  calls, nested powers, and standard-free bases remain on `f64_pow`.
