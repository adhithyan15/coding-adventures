...
**Status: v0.18.0 — LANG-FULL E4 literal string length.**  The backend now
...
It also materialises `str_len` for direct literals from compile-time metadata.
Richer byte-string ops remain outside this release.
...
| v0.18.0 | **Literal string length** (LANG-FULL E4). `str_len` over a direct `str_const` lowers to the compile-time byte count, proving `(string-length "HELLO")` on LLVM while dynamic string algebra stays rejected. |
