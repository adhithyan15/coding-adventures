...
class PythonRespTcpBuildFrontTests(unittest.TestCase):
    def test_resp_fronts_are_repeatable_and_use_their_pinned_venv(self) -> None:
        self.assertEqual(self._recipe("resp-protocol", "BUILD"), RESP_CANONICAL_RECIPE)
        self.assertEqual(
            self._recipe("resp-protocol", "BUILD_windows"), RESP_WINDOWS_RECIPE
        )

    def test_tcp_fronts_preserve_resp_order_and_use_their_pinned_venv(self) -> None:
        self.assertEqual(self._recipe("tcp-server", "BUILD"), TCP_CANONICAL_RECIPE)
        self.assertEqual(
            self._recipe("tcp-server", "BUILD_windows"), TCP_WINDOWS_RECIPE
        )
