...
- **A deferred answer reaches the UI on its own.** It is the return value of no
  ...
  `propsChangedHandler` it already had. Without that the app would advance while
  the screen kept showing the state from before.
...
templates say so and say to defer instead.
