...
fn f64_mul_and_compare_runs_locally() {
    let f = |op: &str, d: Option<&str>, s: Vec<CIROperand>, t: &str| CIRInstr {
        op: op.into(),
        dest: d.map(Into::into),
        srcs: s,
        ty: t.into(),
        deopt_to: None,
    };
    ...
    assert_eq!(
        run(cir),
        1,
        "2.5*2.0 == 5.0 → true; simulator runs real x86_64 SSE2 codegen"
    );
}
...
fn e8_floor_conversion_chain_runs_locally() {
    ...
    assert_eq!(
        run(cir),
        42,
        "floor(45.0−2.7)=42 — cvtsi2sd→subsd→roundsd→cvttsd2si on real x86_64 codegen"
    );
}
...
fn e8_trunc_conversion_chain_runs_locally() {
    ...
    assert_eq!(
        run(cir),
        42,
        "trunc(45.0−2.7)=42 — cvtsi2sd→subsd→cvttsd2si on real x86_64 codegen"
    );
}
