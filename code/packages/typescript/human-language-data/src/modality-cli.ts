...
//
//   generatedModalityOutputs()  a pure-ish path -> content map, so tests can inspect
...
// ---------------------------------------------------------------------------
//
// `MODALITY_MANIFEST_PATH` is the only path anything writes today, and a constant
// cannot escape a directory. The parameter exists because HL-C43 will want to emit an
...
export function generatedModalityOutputs(
  ...
  output: string = MODALITY_MANIFEST_PATH,
  options: ModalityOptions = {},
): Map<string, string> {
  // Validate before doing the expensive corpus walk: a bad path should fail in
  // milliseconds, not after parsing 1,096 lessons.
  safeOutput(root, output);
  const manifest = buildModalityManifest(loadLessons(root), options);
  return new Map([[output, serializeModalityManifest(manifest)]]);
}
...
export function runModalityManifest(
  ...
  let mismatch = false;
  for (const [relative, expected] of generatedModalityOutputs(root)) {
