...
describe("the real corpus", () => {
  ...
  it("pins what nothing had measured", { timeout: 30_000 }, () => {
    ...
    expect(report.summary.lessonsWithoutSequence).toBeLessThanOrEqual(70); // Urdu removes four unordered lessons; this debt ceiling may fall, never grow.
    expect(report.summary.tracksWithUnorderedLessons).toBe(2); // Urdu leaves the unordered-track list.
    ...
    expect(report.summary.lessonsWithoutSequence).toBeLessThanOrEqual(0) // Urdu order recovery: -4 and completes corpus-wide sequence coverage; Persian order recovery: -4; Portuguese order recovery: -9; Italian order recovery: -9; Arabic order recovery: -26; Punjabi order recovery: -31; CEILING — this is debt; it may fall, never grow;    // 19 -> 18: Tamil joins chinese, japanese and latin as a fully-ordered track — // HL11: -155. Hindi, Telugu, Kannada, Malayalam and Sanskrit each had ~30 lessons with no declared reading order, so the corpus believed their words came after their script -- the opposite of what their books print. The order was recovered from the books' own section labels, which is the only place it was ever written down
    // the fourth of 22, not the first.
    expect(report.summary.tracksWithUnorderedLessons).toBe(0); // Urdu order recovery: -1 -- IMPROVEMENT, every track now has complete sequence coverage // Persian order recovery: -1 -- IMPROVEMENT // Portuguese order recovery: -1 -- IMPROVEMENT // Italian order recovery: -1 -- IMPROVEMENT // Arabic order recovery: -1 -- IMPROVEMENT // Punjabi order recovery: -1 -- IMPROVEMENT // HL-C259: -1 -- IMPROVEMENT, gujarati leaves this list: migration gave it a complete sequence order // HL-C258: -1 -- IMPROVEMENT. bengali leaves this list: its ch2-ch5 lessons had no sequence at all, and migration assigned them in one global prerequisite order across the four chapters // HL-C251: -1 -- IMPROVEMENT, the migrated russian ch1 lessons close a continuity window that nothing could measure while they were v1 // HL11: -5. Hindi, Telugu, Kannada, Malayalam and Sanskrit now declare a reading order for every lesson, recovered from their own books
    */
    ...
    expect(report.summary.forwardPrerequisites).toBeLessThanOrEqual(35); // Urdu removes one false forward prerequisite; this debt ceiling may fall, never grow.
    /* Superseded merge-side ceilings:
    ...
    expect(report.summary.forwardPrerequisites).toBeLessThanOrEqual(0) // Urdu order recovery: -1 and closes the corpus prerequisite order // Persian order recovery: -2; Portuguese order recovery: -6; Italian order recovery: -6; Arabic order recovery: -12; Punjabi order recovery: -15; CEILING — this is debt; it may fall, never grow; // HL11: -82, and this is the payoff of the order fix rather than a side effect. A forward prerequisite is a lesson requiring something the reader has not reached; when five tracks had no declared order at all, their words sorted AFTER the script lessons that depended on them, so most of these were artifacts of an undeclared order rather than real gaps in the ramp
    */
    ...
    */
    expect(report.summary.missedByWindow.R2).toBe(3690); // Marwadi Chapter 2 retrieves its newly measurable atoms, and French's first spelling atom is revisited across every early window, so the wider-window total holds.
  });
