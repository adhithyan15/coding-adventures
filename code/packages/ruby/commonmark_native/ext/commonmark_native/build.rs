...
// Ruby interpreter at runtime. The C API symbols (rb_define_module,
// rb_define_module_function, etc.) are provided by the interpreter, not by
...
fn generate_msvc_lib(dll_name: &str, so_name: &str, out_dir: &str) -> bool {
    ...
    let symbols: &[(&str, bool)] = &[
        ...
        // These are always pulled in by the ruby-bridge rlib even if we
        // don't call the functions that reference them. /FORCE:UNRESOLVED
        // handles any that still can't be resolved.
        ("rb_cObject", true),
