...
type ZipWriter() =
    ...
    let addEntry (name: string) (data: byte[]) (compress: bool) (unixMode: uint) =
        ...
        // File_Name_Length is a 16-bit field (max 65535). Without this check,
        // `uint16 nameBytes.Length` on an oversized name would silently wrap
        // (e.g. a 70000-byte name encodes a length of 4464), writing a header
        // whose declared name length doesn't match the bytes actually appended
        // to the buffer — every entry after it becomes misaligned and unreadable.
        if nameBytes.Length > 0xFFFF then
            raise (ArgumentException(
                sprintf "zip: entry name '%s' encodes to %d UTF-8 bytes, exceeding the 65535-byte limit"
...
type ZipReader(data: byte[]) =
    ...
    // `offset` is validated with int64 arithmetic before the `+ N > d.Length`
    // comparison: every caller derives `offset` from attacker-controlled header
    // fields (uint32 sizes/offsets narrowed to `int`), and a narrowing cast can
    // itself have wrapped negative. Doing the bounds math in `int64` means a
    // negative or huge `offset` is rejected here instead of reaching
    // `ReadOnlySpan`, which would otherwise surface as an undocumented
    // `ArgumentOutOfRangeException` rather than the documented `InvalidDataException`.
    static let readU16 (d: byte[]) (offset: int) =
        if offset < 0 || int64 offset + 2L > int64 d.Length then
    ...
    static let readU32 (d: byte[]) (offset: int) =
        if offset < 0 || int64 offset + 4L > int64 d.Length then
    ...
    let meta : ZipEntryMeta list =
        ...
        // Read EOCD fields: cd_size at +12, cd_offset at +16. Both are attacker-
        // controlled uint32 values read straight from the archive bytes, so the
        // bounds check is done in int64 first — narrowing a value like
        // 0xFFFFFFFFu to `int` before comparing would wrap it negative and let
        // `cdOffset + cdSize > data.Length` (a false comparison against a small
        // positive length) pass a bogus Central Directory range through.
        let cdOffsetRaw = readU32 data (eocdOffset + 16)
        let cdSizeRaw   = readU32 data (eocdOffset + 12)
        let cdOffset64  = int64 cdOffsetRaw
        let cdSize64    = int64 cdSizeRaw

        if cdOffset64 + cdSize64 > int64 data.Length then
            raise (InvalidDataException(
                sprintf "zip: Central Directory [%d, %d) out of bounds (file size %d)"
                    cdOffset64 (cdOffset64 + cdSize64) data.Length))

        let cdOffset = int cdOffset64
        let cdSize   = int cdSize64
    ...
    let readEntry (m: ZipEntryMeta) : byte[] =
        if m.IsDirectory then [||]
            ...
            // `m.LocalOffset` and `m.CompressedSize` are uint32 values lifted
            // verbatim from the (attacker-controlled) Central Directory. All of
            // the offset arithmetic below is done in int64 and range-checked
            // against `data.Length` BEFORE narrowing to `int` for indexing —
            // narrowing an oversized uint32 (e.g. LocalOffset = 0x80000000u)
            // straight to `int` would wrap negative and could let a bogus
            // offset slip past a check written in `int` arithmetic, since
            // "negative > data.Length" is always false.
            if int64 m.LocalOffset > int64 data.Length then
                raise (InvalidDataException(
                    sprintf "zip: entry '%s' local header offset %d out of bounds (file size %d)"
                        m.Name m.LocalOffset data.Length))
            ...
            let dataStart64 = int64 lhOff + 30L + int64 lhNameLen + int64 lhExtraLen
            let dataEnd64   = dataStart64 + int64 m.CompressedSize

            if dataStart64 < 0L || dataEnd64 > int64 data.Length then
                raise (InvalidDataException(
                    sprintf "zip: entry '%s' data [%d, %d) out of bounds" m.Name dataStart64 dataEnd64))

            let dataStart = int dataStart64
            let dataEnd   = int dataEnd64
