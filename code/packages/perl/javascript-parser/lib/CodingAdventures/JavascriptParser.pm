package CodingAdventures::JavascriptParser;

# ============================================================================
# CodingAdventures::JavascriptParser — Hand-written recursive-descent JS parser
# ============================================================================
#
# This module parses a subset of JavaScript into an Abstract Syntax Tree (AST).
...
#
#   var x = 5;                          — variable declaration (var/let/const)
#   x = 10;                             — assignment
...
#
# # Token types from CodingAdventures::JavascriptLexer
# =====================================================
#
# Keywords emit their own specific type names (not a generic KEYWORD):
#
#   VAR, LET, CONST     — variable declaration keywords
...
#   postfix_expr     — expr ++/-- (synthesized as unary_expr here)
...
sub _expect {
    ...
    unless ($tok->{type} eq $type) {
        die sprintf(
            "CodingAdventures::JavascriptParser: parse error at line %d col %d: "
...
sub _leaf {
    my ($self, $tok) = @_;
    return CodingAdventures::JavascriptParser::ASTNode->new_leaf($tok);
}

# Create an inner ASTNode.
sub _node {
    my ($self, $rule_name, @children) = @_;
    return CodingAdventures::JavascriptParser::ASTNode->new($rule_name, \@children);
}
...
# program = { statement } ;
#
# The JavaScript grammar allows an empty program (zero statements).
# We keep parsing until we hit EOF.
sub _parse_program {
    my ($self) = @_;
    my @children;
...
# For the assignment vs. expression ambiguity, we look one token ahead.
sub _parse_statement {
    ...
    # Lookahead: NAME followed immediately by EQUALS (but not EQUALS_EQUALS etc.)
    if ($type eq 'NAME') {
...
#
# The for header uses semicolons as separators (not statement terminators).
# We parse the three parts without consuming extra SEMICOLON tokens.
#
...
sub _parse_primary {
    ...
    # --- Boolean literals --- (TRUE and FALSE are distinct token types)
    ...
    #
    # Heuristic: use _is_arrow_params() to look ahead and detect the => token
    # after the closing ).
    if ($type eq 'LPAREN') {
        ...
        # This handles: (x) => x + 1  where x is parsed as an expression
    ...
    die sprintf(
        "CodingAdventures::JavascriptParser: unexpected token '%s' (type %s) "
      . "at line %d col %d\n",
...
CodingAdventures::JavascriptParser - Hand-written recursive-descent JavaScript parser
...
=head1 SYNOPSIS
    ...
    my $ast = CodingAdventures::JavascriptParser->parse_js("let y = x + 1;");

=head1 DESCRIPTION

A hand-written recursive-descent parser for a practical subset of JavaScript.
Tokenizes input with C<CodingAdventures::JavascriptLexer> and builds an
Abstract Syntax Tree (AST) of C<CodingAdventures::JavascriptParser::ASTNode> nodes.
