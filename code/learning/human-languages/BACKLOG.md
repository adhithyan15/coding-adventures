...
oval-or-teardrop body, while the other positional forms remain distinct. The
next smallest starter entry is ی; HL-C09AE queues its source-backed audit.
...
| HL-C09 | Queued — 30 of 228 verified | Expand `DUCTUS` to cover the ten scripts with prose stroke-order entries. | Add cited, font-checked ductus and verified pen-lift metadata for the remaining 198 entries measured by HL-C19; each passes the on-ink, join-tolerance, coverage, citation, and source-agreement invariants. |
...
| HL-C09AE | Queued — next | Locate and verify independent Urdu ی in the same *Zer o Zabar* chapter. | Audit the independent chhoṭī ye body-and-dot animations, determine the exact body direction and lift order, then reconcile that motion with the canonical Noto Naskh fallback before authoring the smallest next path. |
