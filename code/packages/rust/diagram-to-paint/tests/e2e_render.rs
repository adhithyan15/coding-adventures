...
mod apple {
    ...
    use diagram_layout_temporal::{layout_event_model_diagram, layout_temporal_diagram};
    ...
    use mermaid_parser::{
        parse_architecture, parse_block, parse_c4_diagram, parse_er_diagram,
    ...
    fn render_mermaid_block_to_png() {
        ...
        assert!(layout.edges.iter().any(|edge| {
            edge.from_node_id == "L"
                && edge.to_node_id == "M"
                && edge.style.stroke_width == 3.5
                && edge.label.as_ref().is_some_and(|label| label.text == "thick route")
        }));
        assert!(layout.edges.iter().any(|edge| {
            edge.from_node_id == "N"
                && edge.to_node_id == "O"
                && edge.style.stroke_dash.as_deref() == Some(&[5.0, 4.0][..])
                && edge.label.as_ref().is_some_and(|label| label.text == "dotted route")
        }));
        let inline_source = layout
            ...
            .find(|node| node.id == "H")
            .unwrap();
        assert_eq!(inline_source.style.fill, "#e0f2fe");
        assert_eq!(inline_source.style.stroke, "#0369a1");
        assert!(layout.edges.iter().any(|edge| {
        ...
        assert!(scene.instructions.iter().any(|instruction| matches!(instruction,
            PaintInstruction::Rect(rect)
                if rect.x == inline_source.x && rect.y == inline_source.y
                    && rect.fill.as_deref() == Some("#e0f2fe")
...
line "Target" [35, 50, 68, 82]"##,
    ...
    fn render_mermaid_gantt_to_png() {
        let gantt = parse_gantt(
            "gantt\naccTitle: Native Gantt\naccDescr: Gantt rendered through Metal\ntitle #Release<br/>timeline\ndateFormat DD/MM/YYYY HH:mm Z\naxisFormat %m/%d\ntickInterval 1day\ninclusiveEndDates\ntopAxis\ntodayMarker off\nexcludes weekends\nincludes 2026-01-03\nsection ;Build<br/>Core\n#Parser<br/>Core :done, parser, 01/01/2026 06:00 +02:00, 04/01/2026 18:00 +02:00\nclick parser href \"https://example.com/parser\" call inspectTask(parser)\nWindow :window, 29/12/2025 06:00 Z, until parser docs\nPaint :crit, active, paint, after parser docs, 3d\nsection Ship\nDocs :docs, 02/01/2026 06:00 -05:00, 2d\nReview :after docs, 1d\nPackage :active, 1d\nQA :qa, after task2, 12h\nDeadline :vert, deadline, 06/01/2026 12:00 Z, 0d\nRelease :crit, done, milestone, release, after task2, 0d",
        )
        .expect("Mermaid Gantt parse failed");
        assert_eq!(gantt.date_format.source, "DD/MM/YYYY HH:mm Z");
        assert_eq!(gantt.title.as_deref(), Some("Release\ntimeline"));
    ...
    #[test]
    fn render_mermaid_timeline_to_png() {
        ...
        assert!(!scene.instructions.is_empty());
        let pixels = render(&scene);
        write_png(&pixels, "/tmp/mermaid_timeline_e2e.png").expect("PNG write failed");
        assert!(pixels.width > 0 && pixels.height > 0);
    }
    ...
    fn render_mermaid_event_modeling_to_png() {
        ...
        let metrics = CoreTextMetrics;
        let resolver = CoreTextResolver::new();
        let scene = diagram_to_paint_event_model(&layout, &DiagramToPaintOptions {
            background: layout_ir::Color { r: 255, g: 255, b: 255, a: 255 },
            device_pixel_ratio: 2.0,
            label_font: font_spec("Helvetica", 12.0),
            title_font: font_spec("Helvetica", 17.0),
    ...
    fn render_mermaid_swimlane_to_png() {
        ...
        let scene = diagram_to_paint_swimlane(&layout, &DiagramToPaintOptions {
            background: layout_ir::Color { r: 255, g: 255, b: 255, a: 255 }, device_pixel_ratio: 2.0,
            label_font: font_spec("Helvetica", 13.0), title_font: font_spec("Helvetica", 17.0),
            shaper: &shaper, metrics: &metrics, resolver: &resolver,
        });
    ...
    #[test]
    fn render_mermaid_railroad_to_png() {
        let diagram = parse_railroad("railroad-beta\ntitle Numeric literal\naccTitle: Numeric grammar\nsign = optional(terminal(\"-\"));\ndigit = choice(terminal(\"0\"), terminal(\"1\"));\nnumber = sequence(nonterminal(\"sign\"), oneOrMore(nonterminal(\"digit\")), special(\"integer\"));").expect("railroad parse failed");
        ...
        assert_eq!(scene.metadata.as_ref().and_then(|metadata| metadata.get("accessibility.title")), Some(&"Numeric grammar".to_string()));
        assert!(scene.instructions.iter().any(|instruction| matches!(instruction, PaintInstruction::Path(_))));
        assert!(scene.instructions.iter().any(|instruction| matches!(instruction, PaintInstruction::Rect(_))));
    ...
    fn render_mermaid_ishikawa_to_png() {
        ...
        let scene = diagram_to_paint_ishikawa(&layout, &DiagramToPaintOptions {
            background: layout_ir::Color { r: 255, g: 255, b: 255, a: 255 }, device_pixel_ratio: 2.0,
            label_font: font_spec("Helvetica", 13.0), title_font: font_spec("Helvetica", 15.0),
            shaper: &shaper, metrics: &metrics, resolver: &resolver,
        });
        assert!(scene.instructions.iter().any(|instruction| matches!(instruction, PaintInstruction::Path(_))));
        let pixels = render(&scene);
    ...
    fn render_mermaid_radar_to_png() {
        ...
        let metrics = CoreTextMetrics;
        let resolver = CoreTextResolver::new();
        let scene = diagram_to_paint_chart(&layout, &DiagramToPaintOptions {
            background: layout_ir::Color { r: 255, g: 255, b: 255, a: 255 },
            device_pixel_ratio: 2.0,
            label_font: font_spec("Helvetica", 12.0),
            title_font: font_spec("Helvetica", 17.0),
