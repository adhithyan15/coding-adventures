...
Output is verified interoperable with the real `zstd` CLI: two of the JUnit
tests (`tc9CliInterop`, `rtCliInteropHighSequenceCount`) shell out to a
`zstd` binary on `PATH`, compressing/decompressing across the
Java-implementation boundary in both directions. They're skipped gracefully
(not failed) if `zstd` isn't installed.
