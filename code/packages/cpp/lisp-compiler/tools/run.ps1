# Build and run the lisp-compiler tests under MSVC (cl.exe) — pure ISO C/C++ — via the
...
$env:ISO_INCLUDE = "include ..\lisp-parser\include ..\lisp-lexer\include $harness\include"
. (Join-Path $harness 'lib\iso-lib.ps1')
Iso-BuildAndRun -Lang cpp -Name lisp_compiler-tests -Sources @("tests\lisp_compiler_test.cpp")
