...
finite literal `+`, `-`, `*`, or `/` arithmetic and exact tracked integer
snapshots before feeding another pure built-in bounded exponent. The selector
still executes; uninitialized integers, bare tracked-real arithmetic,
non-finite results, effectful selectors, and overrides fail closed.
