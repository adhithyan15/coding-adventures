...
describe("the real corpus", () => {
  ...
  it("pins what nothing had measured", { timeout: 30_000 }, () => {
    ...
    expect(report.summary.lessonsWithoutSequence).toBeLessThanOrEqual(65); // Portuguese removes nine unordered lessons; this debt ceiling may fall, never grow.
    expect(report.summary.tracksWithUnorderedLessons).toBe(2); // Portuguese leaves the unordered-track list.
    /* Superseded merge-side ceilings:
    expect(report.summary.lessonsWithoutSequence).toBeLessThanOrEqual(74) // Italian order recovery: -9; CEILING — this is debt; it may fall, never grow;    // 19 -> 18: Tamil joins chinese, japanese and latin as a fully-ordered track — // HL11: -155. Hindi, Telugu, Kannada, Malayalam and Sanskrit each had ~30 lessons with no declared reading order, so the corpus believed their words came after their script -- the opposite of what their books print. The order was recovered from the books' own section labels, which is the only place it was ever written down
    // the fourth of 22, not the first.
    expect(report.summary.tracksWithUnorderedLessons).toBe(3); // Italian order recovery: -1 -- IMPROVEMENT. // Punjabi now also declares its complete authored reading order. // HL-C259: -1 -- IMPROVEMENT, gujarati leaves this list: migration gave it a complete sequence order // HL-C258: -1 -- IMPROVEMENT. bengali leaves this list: its ch2-ch5 lessons had no sequence at all, and migration assigned them in one global prerequisite order across the four chapters // HL-C251: -1 -- IMPROVEMENT, the migrated russian ch1 lessons close a continuity window that nothing could measure while they were v1 // HL11: -5. Hindi, Telugu, Kannada, Malayalam and Sanskrit now declare a reading order for every lesson, recovered from their own books
    expect(report.summary.lessonsWithoutSequence).toBeLessThanOrEqual(8) // Portuguese order recovery: -9; Italian order recovery: -9; Arabic order recovery: -26; Punjabi order recovery: -31; CEILING — this is debt; it may fall, never grow;    // 19 -> 18: Tamil joins chinese, japanese and latin as a fully-ordered track — // HL11: -155. Hindi, Telugu, Kannada, Malayalam and Sanskrit each had ~30 lessons with no declared reading order, so the corpus believed their words came after their script -- the opposite of what their books print. The order was recovered from the books' own section labels, which is the only place it was ever written down
    // the fourth of 22, not the first.
    expect(report.summary.tracksWithUnorderedLessons).toBe(2); // Portuguese order recovery: -1 -- IMPROVEMENT // Italian order recovery: -1 -- IMPROVEMENT // Arabic order recovery: -1 -- IMPROVEMENT // Punjabi order recovery: -1 -- IMPROVEMENT // HL-C259: -1 -- IMPROVEMENT, gujarati leaves this list: migration gave it a complete sequence order // HL-C258: -1 -- IMPROVEMENT. bengali leaves this list: its ch2-ch5 lessons had no sequence at all, and migration assigned them in one global prerequisite order across the four chapters // HL-C251: -1 -- IMPROVEMENT, the migrated russian ch1 lessons close a continuity window that nothing could measure while they were v1 // HL11: -5. Hindi, Telugu, Kannada, Malayalam and Sanskrit now declare a reading order for every lesson, recovered from their own books
    */
    ...
    expect(report.summary.forwardPrerequisites).toBeLessThanOrEqual(30); // Portuguese removes six false forward prerequisites; this debt ceiling may fall, never grow.
    /* Superseded merge-side ceilings:
    expect(report.summary.forwardPrerequisites).toBeLessThanOrEqual(36) // Italian order recovery: -6; CEILING — this is debt; it may fall, never grow; // HL11: -82, and this is the payoff of the order fix rather than a side effect. A forward prerequisite is a lesson requiring something the reader has not reached; when five tracks had no declared order at all, their words sorted AFTER the script lessons that depended on them, so most of these were artifacts of an undeclared order rather than real gaps in the ramp
    expect(report.summary.forwardPrerequisites).toBeLessThanOrEqual(3) // Portuguese order recovery: -6; Italian order recovery: -6; Arabic order recovery: -12; Punjabi order recovery: -15; CEILING — this is debt; it may fall, never grow; // HL11: -82, and this is the payoff of the order fix rather than a side effect. A forward prerequisite is a lesson requiring something the reader has not reached; when five tracks had no declared order at all, their words sorted AFTER the script lessons that depended on them, so most of these were artifacts of an undeclared order rather than real gaps in the ramp
    */
    ...
    expect(report.summary.forwardReviews).toBeLessThanOrEqual(39); // Portuguese removes six false or stray forward reviews; this debt ceiling may fall, never grow.
    /* Superseded merge-side ceilings:
    ...
    expect(report.summary.forwardReviews).toBeLessThanOrEqual(8) // Portuguese order recovery: -6, including one stray future-Chapter-4 review; Italian order recovery: -7; Arabic order recovery: -15; Punjabi order recovery: -16; CEILING — this is debt; it may fall, never grow; // HL11: -99, same cause as forwardPrerequisites above. With five tracks carrying no declared order, a lesson reviewing an earlier one looked like it reviewed a later one. Recovering the order from their books turned most of these back into what they always were: ordinary backward references
    */
    ...
    expect(report.summary.neverRevisitedPercent).toBe(13); // telugu pre-A1 round 4: 14 -> 13, an IMPROVEMENT and a rounding boundary crossed: atomsNeverRevisited held at 629 while atomsTaught grew 4636 -> 4671, so 13.57% became 13.47%. Not one of the 35 new atoms is unrevisited -- each chapter's second lesson practises the previous chapter's last atom and each payoff practises all five of its own // hindi pre-A1 round 2: +35 lessons, +7 chapters (chapters 52-58) // HL-C172: +4 -- the B2 argue rung (chapter 270) // HL-C173: +3 -- B2 closes (chapter 271) // hindi pre-A1 tranche: +35 lessons, +7 chapters (chapters 45-51)
