...
pub fn normalize_code128_b(data: &str) -> Result<String, String> {
    for ch in data.chars() {
        ...
        if !(32..=126).contains(&code) {
            return Err(
                "Code 128 Code Set B supports printable ASCII characters only".to_string(),
            );
        }
    }
    Ok(data.to_string())
...
fn build_symbols(encoded: &[EncodedCode128Symbol]) -> Vec<Barcode1DSymbolDescriptor> {
    encoded
        ...
        .map(|entry| Barcode1DSymbolDescriptor {
            ...
            modules: if entry.role == Barcode1DRunRole::Stop { 13 } else { 11 },
...
mod tests {
    ...
    fn computes_checksum_and_builds_scene() {
        ...
        assert_eq!(
            scene.metadata.as_ref().and_then(|metadata| metadata.get("codeSet")),
            Some(&"B".to_string())
        );
    }
}
