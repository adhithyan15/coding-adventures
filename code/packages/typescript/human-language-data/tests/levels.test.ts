...
describe("corpus snapshot", () => {
  ...
  it("pins where the corpus actually stands on the ladder", () => {
    ...
    expect(summary.byLevel["pre-A1"]).toBe(1030); // +3: HL-C97 adds the repair kit (no entiendo, mas despacio) at chapter 14 // +4: HL-C98 // +40: vocabulary wave 5 (persian 12, telugu 13, malayalam 15) // +54: vocabulary wave 6 (russian 14, persian 14, urdu 13, bengali 13) // HL12: +30 recognition segments (telugu/kannada/malayalam 8 each, sanskrit 6) -- all pre-A1, since every one sits on SPINE-MEET-GREET
    ...
    expect(summary.byLevel.A1).toBe(361); // +1: ES-C02-concordancia sits on SPINE-TIME-OF-DAY
    ...
    expect(summary.byLevel.A2).toBe(487); // +39: Spanish chapters 11-18 plus prerequisite closure // +3: HL-C88 slice 8 // +1: HL-C88 slice 9 (falsos amigos) // +3 — the plural rung sits on SPINE-TALK-ABOUT-PAST, which is A2: HL-C113 preterite plural // HL-C113 preterite close
  ...
  it("can already build a ramp-to-A1 edition from the canonical corpus", () => {
    ...
    expect(ramp).toHaveLength(1391); // +3: HL-C97 adds the repair kit (no entiendo, mas despacio) at chapter 14 // +40: vocabulary wave 5, all of it pre-A1 // +54: vocabulary wave 6, all of it pre-A1 // HL12: +30 recognition segments (telugu/kannada/malayalam 8 each, sanskrit 6) -- all of it below the A1 cut
