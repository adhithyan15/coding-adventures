...
#[test]
fn phase86_three_sqrt_k_log6_k_over_k2_closes() {
    // √k·√k·√k·log(k)^6 / k²: effective_x2=3; 2·2=4 > 3 → closes.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
...
#[test]
fn phase86_three_sqrt_k_log6_k_times_k_over_k3_closes() {
    // √k·√k·√k·log(k)^6·k / k³: effective_x2=3+2=5; 2·3=6 > 5 → closes.
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
...
#[test]
fn phase86_three_sqrt_k_log6_k_times_k_over_k2_refused() {
    // √k·√k·√k·log(k)^6·k / k²: effective_x2=5; 2·2=4 not > 5 → refused (diverges).
    let k = sym("k");
    let kp1 = apply(sym(ADD), vec![k.clone(), int(1)]);
    let sqrt_k = apply(sym("Sqrt"), vec![k.clone()]);
    let sqrt_kp1 = apply(sym("Sqrt"), vec![kp1.clone()]);
