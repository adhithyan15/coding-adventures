...
Urban Müller in 1993. It has only 8 commands — each a single character —
...
- ``opcodes.py`` — The 9 Brainfuck opcodes (8 commands + HALT)
- ``translator.py`` — Source code → CodeObject (no parser needed)
- ``handlers.py`` — Opcode handler functions registered with GenericVM
- ``vm.py`` — Factory function and convenience executor
"""
