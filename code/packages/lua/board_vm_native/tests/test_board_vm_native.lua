...
describe("coding_adventures.board_vm_native", function()
    ...
    it("exposes Rust-owned board target metadata", function()
        ...
        assert.is_true(capabilities["storage.size"])
        assert.is_true(capabilities["network.ipv4"])
        assert.is_true(capabilities["network.tcp"])
        assert.is_true(capabilities["network.tcp.connected"])
        assert.is_true(capabilities["network.tcp.available"])
        assert.is_true(capabilities["network.udp"])
        assert.is_true(capabilities["network.udp.write_bytes"])
        assert.is_true(capabilities["network.udp.read_bytes"])
        assert.is_true(capabilities["network.udp.available"])
        assert.is_true(capabilities["network.dns"])
        assert.is_true(capabilities["network.dns.resolve"])
        assert.is_true(capabilities["network.dns.set_server"])
        assert.is_true(capabilities["network.dns.query"])
        assert.is_true(capabilities["network.dns.response_ipv4"])
        assert.is_true(capabilities["network.dns.exchange_udp"])
        assert.is_true(capabilities["network.dns.exchange_udp_retry"])
        assert.is_true(capabilities["network.dns.exchange_udp_fallback"])
        assert.is_true(capabilities["network.wifi.associate"])
        assert.is_true(capabilities["network.wifi.disconnect"])
        assert.is_true(capabilities["network.wifi.status"])
        assert.are.equal("WiFiS3", uno.network_interfaces[1].name)
        assert.are.equal("wifi", uno.network_interfaces[1].transport)
        assert.are.equal("ESP32-S3 coprocessor", uno.network_interfaces[1].chip)
        assert.are.equal("ipv4", uno.network_interfaces[1].protocols[1])
        assert.are.equal("tcp", uno.network_interfaces[1].protocols[2])
        assert.are.equal("udp", uno.network_interfaces[1].protocols[3])
        assert.are.equal("dns", uno.network_interfaces[1].protocols[4])
        assert.are.equal(4, uno.network_interfaces[1].max_sockets)
