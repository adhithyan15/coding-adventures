...
- Multi-corner DC source sweeps across Python, TypeScript, and Rust SPICE
  engines: named corner specs apply core linear element-parameter overrides,
  then reuse the existing `.DC` source sweep to collect one trace per corner.
...
| **Multi-corner parallel sweep** | Run the same analysis at N PVT corners in parallel goroutines / subprocesses. Mostly an orchestration problem once the core engine is solid. DC operating-point corners shipped in PR #3495; DC source sweep corners are in flight. |
