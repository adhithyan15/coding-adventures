...
Instead of hand-writing tokenization rules, this crate loads the `excel.tokens` grammar file and feeds it to the generic `GrammarLexer`. The grammar file defines all of JavaScript's tokens — keywords, identifiers, numbers, strings, operators, and delimiters — in a declarative format.
...
lexer               (GrammarLexer: tokenizes source using TokenGrammar)
       |
       v
excel-lexer    (THIS CRATE: wires grammar + lexer together for JavaScript)
       |
       v
excel-parser   (consumes tokens to build AST)
```

## Usage

```rust
use coding_adventures_excel_lexer::{create_excel_lexer, tokenize_excel};

// Quick tokenization — returns a Vec<Token>
let tokens = tokenize_excel("var x = 42;");

// Or get the lexer object for more control
let mut lexer = create_excel_lexer("function add(a, b) { return a + b; }");
