...
- ◑ **BA4** — string literal `PRINT` runs on all 7 backends via **E4**.
  ...
  **String arrays ◑** (E4d-BA-arr, part 1 on [Llvm, Wasm, Vm, Jit]): `DIM A$(n)`
  lowers to an `array<str>` (the E5 aggregate carrying E4-dyn string handles);
  `A$(i)=s`/`A$(i)` are `str`-typed `array_set`/`array_get`. Part 2 adds
  NativeAot/JVM/CLR (native element-size allowance + managed reference-array
  lowering). Broader dynamic string expressions and string `READ`/`DATA` remain.
