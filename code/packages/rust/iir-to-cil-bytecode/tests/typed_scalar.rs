...
fn acyclic_control_flow_uses_void_shapes_and_short_branches() {
    ...
    assert_eq!(
        ...
        vec![
            0x17, 0x0a, 0x06, 0x2c, 0x04, 0x1d, 0x0b, 0x07, 0x2a, 0x1f, 0x09, 0x0c,
            0x08, 0x2a,
        ]
    );
}
...
fn control_flow_refuses_invalid_cfgs_and_path_dependent_values() {
    let conditional = |target: &str| {
        IIRInstr::new(
            "jmp_if_true",
            None,
            vec![v("condition"), v(target)],
            "void",
        )
    };
    let mut m = IIRModule::new("strict", "test");
    m.add_or_replace(IIRFunction::new(
        ...
        vec![
            conditional("join"),
            IIRInstr::new("const", Some("one_path".into()), vec![Operand::Int(1)], "i32"),
    ...
    m.functions[0].instructions.insert(
        1,
        IIRInstr::new("label", None, vec![v("top")], "void"),
    );
    m.functions[0].instructions.insert(
        2,
        IIRInstr::new("jmp", None, vec![v("top")], "void"),
    );
    refuses(m, "backward branches");
    ...
    for (instruction, diagnostic) in [
        ...
        (
            ...
            "invalid unconditional branch shape",
        ),
        (
        ...
        let mut m = base();
        if diagnostic != "undefined branch label" {
            m.functions[0].instructions.insert(
                1,
                IIRInstr::new("label", None, vec![v("target")], "void"),
            );
        }
