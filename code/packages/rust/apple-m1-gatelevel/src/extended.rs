use apple_m1_simulator::{AppleM1Error, AppleM1State, MEMORY_SIZE, XZR};
...
fn lane_bits(value: u128, width: usize) -> Vec<u8> {
    (0..width).map(|bit| ((value >> bit) & 1) as u8).collect()
}

fn bits_lane(bits: &[u8]) -> u128 {
    bits.iter()
        .enumerate()
        .fold(0, |value, (bit, q)| value | (u128::from(*q) << bit))
}

fn gate_add(a: u128, b: u128, width: usize, subtract: bool) -> u128 {
    let a = lane_bits(a, width);
    let mut b = lane_bits(b, width);
    if subtract {
        for bit in &mut b {
            *bit = not_gate(*bit);
        }
    }
    bits_lane(&ripple_carry_adder_with_carry(&a, &b, u8::from(subtract)).sum)
}

fn gate_multiply(a: u128, b: u128, width: usize) -> u128 {
    let a = lane_bits(a, width);
    let b = lane_bits(b, width);
    let mut product = vec![0; width];
    for (shift, multiplier) in b.iter().copied().enumerate() {
        let mut partial = vec![0; width];
        for bit in 0..(width - shift) {
            partial[bit + shift] = and_gate(a[bit], multiplier);
        }
        product = ripple_carry_adder_with_carry(&product, &partial, 0).sum;
    }
    bits_lane(&product)
}
    ...
    if opcode == 0x10 || opcode == 0x13 {
        ...
        for shift in (0..total).step_by(elem) {
            ...
            let lane = if opcode == 0x13 {
                gate_multiply(a, b, elem)
            } else if unsigned {
                gate_add(a, b, elem, true)
            } else {
                gate_add(a, b, elem, false)
            };
