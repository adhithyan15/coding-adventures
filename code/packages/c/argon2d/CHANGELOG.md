...
- Initial pure-ISO C17 port of the Rust `argon2d` crate (RFC 9106): the
  data-dependent memory-hard password-hashing function.
- `argon2d(...)` writing the tag into a caller-provided buffer with an
  `Argon2dStatus` result; full parameter validation matching the Rust variants.
- Argon2 G compression, permutation P, the H' variable-length BLAKE2b extender,
  the RFC 9106 §3.4 `index_alpha` reference mapping, and data-dependent segment
  fill.
