...
describe("the committed corpus", () => {
  ...
  it("pins the first ledger, and it is the reason the rule exists", () => {
    ...
    expect(l.summary.roots).toBe(3079); // +1: French's typed salut opener records its etymon atom
    expect(l.summary.underspent).toBe(2964); // French adds one after Latin's guided-copy bridge re-spends salvē
    ...
    expect(byNamespace).toEqual({ roots: 2194, "etymon-atom": 885 }); // +1 etymon-atom: French salut
  });
