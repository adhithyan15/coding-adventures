...
% canonical-source-hash: fnv1a64:ef6734ab26d6546e
% canonical-lessons: AR-C33-shay, AR-C33-halib, AR-C33-sukkar, AR-C33-qahwa, AR-R33-the-courtesies-again
...
\end{tcolorbox}
\section[min faḍlik · shukran · naʿam]{The courtesies, doing real work}

\label{lesson:AR-R33-the-courtesies-again}



\begin{quote}
Order tea politely, accept when it is offered, and thank them. Three words you have met and one you met in this chapter. No reading first.
\end{quote}
...
\begin{itemize}
\raggedright
  \item tea, please — \emph{shāy, min faḍlik}
  \item coffee with milk and sugar, please — \emph{qahwa maʿa ḥalīb wa-sukkar, min faḍlik}
  \item accept — \emph{naʿam}
  \item thank them, and hear the reply — \emph{shukran} … \emph{ʿafwan}
  \item a man apologises, then a woman — \emph{āsif} … \emph{āsifa}
\end{itemize}

The courtesy kit was four separate single-word lessons long ago. This is the first time it has had to carry a real transaction.

\subsection*{How to answer: the reply that erases the debt}
Say \textbf{\ar{شكرا}} and what comes back is \textbf{\ar{عفوا}} — \emph{it is nothing}, from a root about forgiving and waiving. English accepts thanks; Arabic \textbf{waves them away}. The exchange has two halves and you now own both.

\textbf{\ar{آسف}} is a \textbf{gendered adjective}, so it changes with the speaker. When you want a quick, ungendered \emph{excuse me} — squeezing past somebody in a café — the word is \textbf{\ar{معذرة}} \emph{maʿdhira}.

\subsection*{You'll want to know: the invisible bend}
\textbf{\ar{من} \ar{فضلك}} changes with whom you address and the page does not show it: only the unwritten short vowel moves, \emph{faḍlak} to a man and \emph{faḍlik} to a woman.

\textbf{\ar{كم} \ar{عمرك؟}} hides the same suffix, and hides a verb too — literally \textbf{how much is your lifetime}. Arabic neither \emph{has} its years nor \emph{is} them; it owns them, as a noun, with nothing in between.

\subsection*{Script: shapes inside the words above}
Write these out:

\begin{itemize}
\raggedright
  \item \textbf{\ar{شكرا}} — \emph{shīn} with its three dots, then \emph{kāf}, \emph{rāʾ}, \emph{alif}
  \item \textbf{\ar{شاي}} — the same \emph{shīn}, then \emph{alif}, then \emph{yāʾ}
\end{itemize}

\textbf{\ar{ش}} is \textbf{\ar{س}} carrying three dots, and \textbf{\ar{ك}} is \emph{kāf}. Both words start with the same letter, and in both the letter keeps its skeleton while the connectors at its edges change to suit its neighbours.

\begin{tcolorbox}[breakable,colback=teal!4,colframe=teal!35!black,title={Before you move on}]
Give the reply to thanks and its literal force. (\textbf{\ar{عفوا}} — it is nothing.) Say what a woman says for \emph{sorry}. (\textbf{\ar{آسفة}}.) Say why \textbf{\ar{من} \ar{فضلك}} looks identical for both genders. (\textbf{Only the unwritten vowel moves}.) And say what stays constant when a letter joins its neighbours. (\textbf{Its skeleton}.)
\end{tcolorbox}
