...
/// Caller checks `2·den_deg > effective_x2`.
fn four_sqrt_six_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 4 || log_count != 6 { return None; }
...
/// Caller checks `2·den_deg > effective_x2`.
fn five_sqrt_six_log_poly_effective_x2(node: &IRNode, k: &IRNode) -> Option<i64> {
    let apply_node = match node { IRNode::Apply(a) => a, _ => return None };
    if !head_is(&apply_node.head, MUL) { return None; }
    let mut sqrt_degs_x2: Vec<i64> = Vec::new();
    let mut log_count: usize = 0;
    ...
    if sqrt_degs_x2.len() != 5 || log_count != 6 { return None; }
    Some(sqrt_degs_x2.iter().sum::<i64>() + 2 * poly_deg)
}
...
fn g_vanishes_at_infinity(g: &IRNode, k: &IRNode) -> bool {
    ...
    // Phase 87: Mul(Sqrt(P1), Sqrt(P2), Sqrt(P3), Sqrt(P4), Log(diverging)×6, polynomial..., bounded...).
    // Four Sqrt + six Log; log⁶ sub-polynomial — effective_x2 = sum(sqrt_degs_x2) + 2·poly_deg.
    // Closes when 2 * den_deg > effective_x2 or non-polynomial diverging denom.
    if let Some(s4l6_x2) = four_sqrt_six_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s4l6) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s4l6 > s4l6_x2 {
                return true;
            }
        } else if h_diverges_at_infinity(den, k) {
            return true;
    ...
    // Phase 88: Mul(Sqrt(P1)×5, Log(diverging)×6, polynomial..., bounded...).
    // Five Sqrt + six Log; log⁶ sub-polynomial — effective_x2 = sum(sqrt_degs_x2) + 2·poly_deg.
    // Closes when 2 * den_deg > effective_x2 or non-polynomial diverging denom.
    if let Some(s5l6_x2) = five_sqrt_six_log_poly_effective_x2(num, k) {
        if let Some(den_deg_s5l6) = polynomial_degree_in_k(den, k) {
            if 2 * den_deg_s5l6 > s5l6_x2 {
                return true;
            }
        } else if h_diverges_at_infinity(den, k) {
            return true;
