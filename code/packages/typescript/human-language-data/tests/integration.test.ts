...
describe("real curriculum", () => {
  ...
  it("keeps the cross-script closure queue measured after Persian and Urdu qaf", () => {
    ...
    const teluguE = scripts.telugu!.independentVowels!.find((entry) => entry.glyph === "ఎ")!;
    ...
    expect(teluguE.strokeOrderNote).toMatch(/three numbered movements.*two pen-down runs.*1.?2.*movement 3/i);
    ...
    const malayalamChilluLL = scripts.malayalam!.finalConsonants!.find((entry) => entry.glyph === "ൾ")!;
    ...
    expect(malayalamChilluLL.strokeOrderNote).toMatch(/four visible movements.*one continuous pen-down run/i);
    ...
    const persianQaf = scripts["perso-arabic"]!.letters.find((letter) => letter.glyph === "ق")!;
    ...
    expect(persianQaf.strokeOrderSource?.citation).toMatch(/Persian Online.*ق.*02:14–02:18/i);
    ...
    const urduQaf = scripts["urdu-nastaliq"]!.letters.find((letter) => letter.glyph === "ق")!;
    ...
    expect(persianQaf.strokeOrderSource?.url).not.toBe(urduQaf.strokeOrderSource?.url);
    ...
    const teluguAa = scripts.telugu!.independentVowels!.find((entry) => entry.glyph === "ఆ")!;
    ...
    const kannadaU = scripts.kannada!.independentVowels!.find((entry) => entry.glyph === "ಉ")!;
    ...
    const kannadaE = scripts.kannada!.independentVowels!.find((entry) => entry.glyph === "ಎ")!;
    ...
    const japaneseYu = scripts.japanese!.letters.find((entry) => entry.glyph === "ゆ")!;
