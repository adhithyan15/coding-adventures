...
% canonical-source-hash: fnv1a64:6682465863b5269c
% canonical-lessons: ES-C38-luego, ES-C38-porque, ES-C38-repaso-los-tres-porques, ES-C38-imperfecto, ES-C38-contar
...
\end{tcolorbox}
\section[(review)]{Review — the accent is doing the work}

\label{lesson:ES-C38-repaso-los-tres-porques}



\begin{quote}
\textquotedblleft{}I get up because I run.\textquotedblright{} (\emph{Me levanto porque corro}.)
\end{quote}
...
\begin{tabularx}{\linewidth}{@{}>{\raggedright\arraybackslash}X>{\raggedright\arraybackslash}X@{}}
\toprule
\textbf{written} & \textbf{job} \\
\midrule
\emph{porque} & \textbf{because} — gives the reason \\
\emph{por qué} & \textbf{why} — asks for it \\
\emph{el porqué} & \textbf{the reason} — a thing you can own \\
\bottomrule
\end{tabularx}

\emph{¿\textbf{Por qué} te levantas?} — \emph{Me levanto \textbf{porque} corro.} — \emph{No entiendo \textbf{el porqué}.}

Ask, answer, and then name the answer.
\end{grammarlens}

\begin{culture}
The accent on \emph{qué} is not there to look nice, and it is not there at random. It is the same mark that sits on every question word: \emph{qué}, \emph{quién}, \emph{dónde}, \emph{cuándo}, \emph{cómo}. Spanish uses one mark, consistently, for one job — \textbf{this word is the one doing the asking}.

So \emph{por qué} is two words because it is literally \emph{for what}, with the accent marking \emph{what} as the question. And \emph{porque}, fused into one word with no accent, has nothing left to ask with.

\emph{El porqué} is the odd one, and it is odd in a way that makes sense: put \emph{el} in front of anything in Spanish and it becomes a thing. It keeps the accent because the question word is still inside it — the reason is, after all, the answer to a \emph{why}.
\end{culture}

\subsection*{Your turn}
\begin{itemize}
\raggedright
  \item \emph{Say it:} \emph{¿por qué?} · \emph{porque} · \emph{el porqué}
\end{itemize}

\emph{Twice through:}

\begin{tcolorbox}[breakable,colback=teal!4,colframe=teal!35!black,title={Before you move on}]
Which one carries an accent and a space? (\emph{\textbf{Por qué}} — the one that asks.) What job does the accent do everywhere in Spanish? (\textbf{Marks the word that does the asking}.) Which one takes \emph{el}? (\emph{\textbf{El porqué}}, the reason itself.)
\end{tcolorbox}
