...
describe("the real corpus", () => {
  ...
  it("pins what nothing had measured", { timeout: 30_000 }, () => {
    ...
    expect(report.summary.atomsTaught).toBe(2705); // +4: ES-LEX-VOS-01, ES-CULTURE-VOSEO-02 (ES-C03-vos) // +3: HL-C98 splits AR-PRESENT-SINGULAR into 1SG/2SG/3SG
    ...
    expect(report.summary.atomsNeverRevisited).toBe(461); // -17: payoff lessons plus si/no moving early // +2: HL-C98's per-cell atoms are revisited later than the R-windows reach
    ...
    // R1 stops at 3.
    expect(report.summary.missedByWindow.R1).toBe(866); // +2: HL-C98
  ...
  it("shows what a declared reading order was worth", () => {
    ...
    expect(spanish).toMatchObject({
      ...
      lessonCount: 249, // +4: HL-C98 // +3: HL-C97 adds the repair kit (no entiendo, mas despacio) at chapter 14 // +8 payoff lessons // +1 ES-C03-vos, +1 ES-C02-concordancia
      ...
      atomsTaught: 429, // +3: HL-C98's per-cell atoms
      atomsNeverRevisited: 52, // +2: HL-C98
    });
  });
