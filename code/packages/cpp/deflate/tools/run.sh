...
# Include this package's headers, the sibling header-only lzss package's
# (DEFLATE's LZSS tokenization pass is CMP02's `ca::lzss::encode`), and the
# harness's (for iso_test.h).
...
iso_build_and_run cpp deflate-tests tests/deflate_test.cpp
