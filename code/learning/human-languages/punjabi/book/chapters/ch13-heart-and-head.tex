...
% canonical-source-hash: fnv1a64:1883f5250700eb24
% canonical-lessons: PA-S13-pairin-bindi, PA-C13-dil, PA-C13-sir
...
\section[(borrowed) · sha]{\pa{਼} · \pa{ਸ਼} — two pieces met inside a word you already say}

\label{lesson:PA-S13-pairin-bindi}



\begin{quote}
One dot, and a whole layer of Punjabi's borrowed vocabulary becomes readable.

First, say the last things you learned once more, out loud and from memory:

\begin{itemize}
\raggedright
  \item \emph{Say it:} \emph{kann} · \emph{mūnh} · \emph{nakk}
\end{itemize}
\end{quote}

\subsection*{Script you'll notice: \pa{਼} · \pa{ਸ਼}}
\begin{quote}
\pa{਼}
\end{quote}

\textbf{pairī̃ bindī} — \emph{(borrowed)}. A single dot placed \textbf{under} a letter to mark a sound Punjabi borrowed — mostly from Persian and Arabic. The dot is the passport stamp.

\begin{quote}
\pa{ਸ਼}
\end{quote}

\textbf{sassā + pairī̃ bindī} — \emph{sha}. The dotted \emph{sassā}. Punjabi prints it as one character, and it is the \emph{sh} of English \emph{ship}.

Now put them where you have already heard them:

\begin{quote}
\textbf{\pa{ਸ}} + \textbf{\pa{਼}} $\to$ \textbf{\pa{ਸ਼}} (\emph{sha});  \textbf{\pa{ਗ}} + \textbf{\pa{਼}} $\to$ \textbf{\pa{ਗ਼}} (\emph{gha}, a Persian sound)
\end{quote}

\begin{quote}
\textbf{\pa{ਸ਼}} — \emph{sha} — \textbf{the borrowed \emph{sh}}
\end{quote}
...
Keep \pa{਼} · \pa{ਸ਼} in front of you and do not cover any of them. Follow each shape once with a finger, then copy it once with a pencil, larger than it is printed. Say its sound as your hand stops.
...
\begin{itemize}
\raggedright
  \item \emph{Look:} at \textbf{\pa{ਸ਼}} and point to each of today's pieces in turn
  \item \emph{Say it:} \emph{(borrowed)}, \emph{sha}, then the whole word — \emph{sha}
...
Name today's pieces: \emph{pairī̃ bindī}, \emph{sassā + pairī̃ bindī}. Which of them is a \textbf{mātrā} or a mark rather than a letter, and what does \textbf{\pa{ਸ਼}} say? (\emph{sha}, “the borrowed \emph{sh}”.)
