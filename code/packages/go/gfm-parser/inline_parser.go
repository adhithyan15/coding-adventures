...
// ParseInline is Phase 2 of GFM parsing: scan raw inline content
...
// delimiterToken represents a run of *, _, or ~~.
...
// scanDelimiterRun scans a run of *, _, or ~ and returns a delimiterToken with
...
func resolveEmphasis(tokens []inlineToken) []documentast.InlineNode {
	...
	for i < len(tokens) {
		...
		if closer.char == '~' {
			emphNode = &documentast.StrikethroughNode{Children: innerNodes}
		} else if isStrong {
...
// isValidEmailLocalPart checks the email local part per GFM spec.
