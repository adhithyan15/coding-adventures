...
describe("the committed corpus", () => {
  ...
  it("pins the first ledger, and it is the reason the rule exists", () => {
    ...
    expect(l.summary.roots).toBe(3080); // Italian's typed ciao opener records its etymon atom.
    expect(l.summary.underspent).toBe(2965); // The new Italian etymon begins below the reuse floor.
    ...
    expect(byNamespace).toEqual({ roots: 2194, "etymon-atom": 886 }); // +1: Italian's migrated opener etymon.
  });
