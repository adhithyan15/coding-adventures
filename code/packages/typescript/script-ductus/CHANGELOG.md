...
### Added — source-verified Tamil long uu

- Add independent Tamil **ஊ** from the UT Austin manual's compositional
  Module 17 instruction.
- Write familiar **உ** first, then overlay familiar **ள** in its three-run
  order.
- Fit the four-run result to the bundled Noto Sans Tamil outline.
...
- Add script-scoped Persian and Urdu **ز** ductus from independently cited
  lessons.
- Keep the re-series body joined before the separately lifted upper dot.
- Reuse fitted Noto Naskh geometry without conflating script provenance.
...
- Add script-scoped Persian and Urdu **ط** ductus from independently cited
  lessons.
- Keep the closed body and leftward finish joined before the lifted upright.
- Reuse fitted Noto Naskh geometry without conflating script provenance.

### Added — source-verified Persian and Urdu qaf

- Add script-scoped Persian and Urdu **ق** ductus from independently cited
  lessons.
- Keep the head and deep bowl joined before two separately lifted upper dots.
- Reuse fitted Noto Naskh geometry without conflating script provenance.

### Added — source-verified Japanese hiragana ta

- Add hiragana **た** as four source-backed runs: upper horizontal, crossing
  left-falling stem, short right horizontal, and lower-right bowl.
- Preserve Sirgazil's 31-frame CC0 animation's three lifts while fitting all
  four printed components to the bundled Noto Sans JP outline.

### Added — source-verified Japanese hiragana ku

- Add hiragana **く** as one source-backed run: sweep down-left from the upper
  right into the central turn, then continue down-right to the lower tip.
- Preserve Sirgazil's 20-frame CC0 animation's zero-lift order while fitting
  both visible movements to the bundled Noto Sans JP outline.
...
- Add Kannada **ಆ** as two source-backed pen-down runs: join the compact left
  loop to the broad lower bowl, lift once, then draw the rounded right loop and
  return left along the inner bar.
- Preserve the animation's one-lift order while fitting the medians to the
  bundled Noto Sans Kannada outline.

### Added — source-verified Kannada independent e

- Add Kannada **ಎ** as one source-backed four-movement run from the compact
  left loop through both lower curves and the tall outer arch.
- Preserve the animation's zero-lift order while fitting the medians to the
  bundled Noto Sans Kannada outline.

### Added — source-verified Japanese hiragana mo

- Add hiragana **も** as three source-backed pen-down runs: the descending stem
  and broad lower bowl, then the upper and lower bars from left to right.
- Preserve the animation's two lifts while fitting the medians to the bundled
  Noto Sans JP outline in a three-frame filmstrip.
...
- Add Telugu independent **ఇ** as three source-backed pen-down components: the
  broad outer bowl, compact upper-left lobe, and angled upper-right shoulder.
- Preserve the two demonstrated lifts while fitting the component proportions
  and joins to the bundled Noto Sans Telugu outline.

### Added — source-verified Telugu independent aa

- Add Telugu independent **ఆ** as two source-backed pen-down runs: the hooked
  left lobe and broad bowl, then the rounded right lobe after one lift.
- Fit the demonstrated component order to the bundled Noto Sans Telugu outline,
  including the printed inner-bar return in the second run.

### Added — source-verified Persian shin

- Add Persian **ش** as one continuous teeth-and-bowl body followed by three
  separately lifted dots in lower-left, lower-right, centered-upper order.
- Pin three lifts and fit the cited Persian freehand order to the bundled Noto
  Naskh outline without borrowing the Arabic or Urdu provenance.
...
- Preserve the rising left arch, clockwise right loop, inner return, and
  above-line hook in a zero-lift Noto Sans Malayalam fit.
...
- Preserve the descending left bowl, central rise, right loop, and finishing
  above-line hook in a zero-lift Noto Sans Malayalam fit.
...
- Preserve the middle-stem retrace before the second arch, outer curve, lower
  loop, and rightward exit in a zero-lift Noto Sans Kannada fit.
...
- Pin Arabic, Persian, and Urdu **ٓ** as an alef carrier plus a separately
  encoded wave-shaped mark using Unicode 17's canonical decomposition.
- Reuse each script's source-verified alif ductus and keep the carrier-first
  learner convention explicit rather than inventing a standalone mark path.

### Added — source-verified Persian and Urdu che

- Add Persian and Urdu **چ** as a continuous head and bowl followed by three
  separately lifted lower dots from independent sources.
- Share the Noto Naskh fallback geometry while pinning script-owned provenance
  and preserving jīm **ج**'s separate dot-first order.

### Added — source-verified Urdu do-chashmi he

- Add Urdu **ھ** as a continuous four-movement path through both eyes and its
  low leftward finish.
- Pin zero lifts, the independent animation order, and the Noto Naskh fallback
  fit separately from chhoṭī he **ہ**.

### Added — source-verified Urdu pe

- Add Urdu **پ** as one right-to-left be-series bowl followed by the lower-left,
  lower-right, and lower-center dots in three separately lifted strokes.
- Pin three lifts, the source's two-near/one-lower triangle, independent Urdu
  provenance, and the shared Noto Naskh fallback fit.
...
- Add script-owned Persian and Urdu **ف** paths with a joined clockwise head
  and leftward body followed by the separately placed upper dot.
- Pin one lift, distinct provenance, and the shared Noto Naskh fallback fit.
...
- Add Malayalam **എ** as two pen-down runs: the joined hook, bar, upright, and
  inner loop; then the broad outer arch.
- Pin its one lift, UT Austin handwriting clip, below-line movements, and Noto
  Sans Malayalam outline fit.
...
- Add Persian **ر** as one unbroken two-movement path: short descent, then the
  leftward lower curve.
- Pin its zero lifts, Persian Online timestamp, Noto Naskh fit, and script-scoped
  identity beside the distinct Arabic and Urdu records.

### Added — source-verified Devanagari lla (HL-C09I9)

- Add the three-run **ळ** learner path: joined figure-eight body, short upper
  stem, and left-to-right headline.
- Pin its two lifts, published point-array provenance, and Noto outline fit.
...
- Add the four-run **ञ** learner path: clockwise open-left bowl, rising
  rightward shoulder, short lower stem, and left-to-right headline.
...
- Add the three-run **ठ** learner path: descending short central stem,
  counterclockwise closed round body, and left-to-right headline.
- Pin its two lifts, frame-by-frame source provenance, and Noto outline fit.

### Added — source-verified Devanagari tha (HL-C09I1)

- Add the three-run **थ** learner path: one continuous upper spiral and broad
  lower bowl, followed by the descending right stem and left-to-right headline.
- Pin its two lifts, frame-by-frame source provenance, and Noto outline fit.
...
- Add the three-run **छ** learner path: one continuous nested body through both
  left loops, the broad lower bowl, the outer-right rise, and the inner loop,
  followed by the short upper stem and left-to-right headline.
- Pin its two lifts, frame-by-frame source provenance, and Noto outline fit.

### Added — source-verified Devanagari tta (HL-C09HY)

- Add the two-run **ट** learner path: joined descending central stem and
  counterclockwise open round body, followed by the left-to-right headline.
- Pin its one lift, frame-by-frame source provenance, and Noto outline fit.

### Added — source-backed Devanagari nukta composition (HL-C09HX)

- Model nukta as one below-base diacritic composed with the seven consonant
  carriers present in the current Hindi, Marathi, and Sanskrit corpus.
- Pin the Unicode 17 provenance, carrier-first learner convention, and explicit
  boundary between encoded composition and universal handwriting order.

### Added — source-verified Devanagari nna (HL-C09HW)

- Add the three-run **ण** learner path: joined left stem, lower bowl, and inner
  right stem; descending outer right stem; and left-to-right headline.
- Pin its two lifts, frame-by-frame source provenance, and Noto outline fit.
...
- Add the four-run **ख** learner path: joined left body and lower bowl,
  upper-right loop, descending right stem, and left-to-right headline.
- Pin its three lifts, source provenance, Noto outline fit, and the documented
  continuous-handwriting bridge across the printed font's upper-loop gap.
...
- Mark the 22 source-verified Hebrew letters and nine current-corpus niqqud
  complete after the real lesson corpus passes the script-closure gate.
- Keep less-common niqqud explicit as a separate inventory expansion.
...
- Reuse Yaa's verified body geometry without copying its dots or identity.
- Expose 31 learner rows while keeping 29 canonical base/standalone rows.

### Added — source-verified Arabic taa-marbuta ending (HL-C09HL)

- Add isolated **ة** as a word-final-only learner row with connected final **ـة**.
- Close its compact clockwise body before placing the two upper dots.
- Keep the canonical base/standalone inventory at 29 rows while exposing 30
  source-verified learner rows.
...
- Keep Qaf's closed head and deep bowl joined before its two upper dots.
- Keep Arabic incomplete while seated Hamza and ending variants remain unaudited.
...
- Preserve the complete **ع** body before lifting once for **غ**'s upper dot.
- Keep Arabic incomplete while two lesson-corpus base letters remain absent.

### Added — source-verified Arabic ظ ductus (HL-C09HG)

- Extend Arabic's canonical inventory to 26 unique source-backed rows.
- Preserve the complete **ط** body, then place **ظ**'s dot before its upright.
- Keep Arabic incomplete while three lesson-corpus base letters remain absent.
...
- Preserve t30apps.com's version-1.0 first-and-second-path order while fitting both medians to Noto Sans Gujarati.
- Add source, font-routing, on-ink, whole-glyph, and two-frame filmstrip coverage; the focused suite now passes 1,322 tests.
...
- Preserve t30apps.com's version-1.0 first-and-second-path order while fitting both medians to Noto Sans Gujarati.
- Add source, font-routing, on-ink, whole-glyph, and two-frame filmstrip coverage; the focused suite now passes 1,315 tests.

### Added — cited Gujarati ખ ductus (HL-C09FS)

- Render **ખ** as a joined left-lobe-and-curl run followed by its separate right spine and lower foot with one lift.
- Preserve t30apps.com's version-1.0 first-and-second-path order while fitting both medians to Noto Sans Gujarati.
- Add source, font-routing, on-ink, whole-glyph, and two-frame filmstrip coverage; the focused suite now passes 1,308 tests.
...
- Preserve t30apps.com's version-1.0 first-and-second-path order while fitting both medians to Noto Sans Gujarati.
- Add source, font-routing, on-ink, whole-glyph, and two-frame filmstrip coverage; the focused suite now passes 1,301 tests.
...
- Preserve RussianIrina's 10:34–10:38 looped entry and rounded shoulder while fitting Noto Sans Cyrillic's broad top flag, straight stem, and closed lower bowl.
- Add source, font-routing, on-ink, whole-glyph, and five-frame filmstrip coverage; the focused suite now passes 1,167 tests.

### Added — cited Cyrillic щ ductus (HL-C09EZ)

- Render **щ** as one joined left-to-middle-to-right-to-tail run with zero lifts.
- Preserve RussianIrina's 10:17–10:25 rounded school-hand joins and looped exit while fitting Noto Sans Cyrillic's three straight stems, baseline bars, and short right descender.
- Add source, font-routing, on-ink, whole-glyph, and six-frame filmstrip coverage; the focused suite now passes 1,160 tests.

### Added — cited Cyrillic ш ductus (HL-C09EY)

- Render **ш** as one joined left-to-middle-to-right stem run with zero lifts.
- Preserve RussianIrina's 09:49–09:57 rounded school-hand joins and rising exit while fitting Noto Sans Cyrillic's three straight stems and horizontal baseline bars.
- Add source, font-routing, on-ink, whole-glyph, and five-frame filmstrip coverage; the focused suite now passes 1,153 tests.

### Added — cited Cyrillic ч ductus (HL-C09EX)

- Render **ч** as one joined short-left-stem-to-bowl-to-full-right-stem run with zero lifts.
- Preserve RussianIrina's 09:24–09:28 narrow rounded school-hand bridge and rising exit while fitting Noto Sans Cyrillic's printed shallow bowl and straight stems.
- Add source, font-routing, on-ink, whole-glyph, and three-frame filmstrip coverage; the focused suite now passes 1,146 tests.

### Added — cited Cyrillic ц ductus (HL-C09EW)

- Render **ц** as one joined left-stem-to-base-to-right-stem-to-descender run with zero lifts.
- Preserve RussianIrina's 09:05–09:10 rounded, loop-tailed school-hand order while fitting Noto Sans Cyrillic's squared body and short right descender.
- Add source, font-routing, on-ink, whole-glyph, and four-frame filmstrip coverage; the focused suite now passes 1,139 tests.

### Added — cited Cyrillic х ductus (HL-C09EV)

- Render **х** as a top-to-bottom left run followed, after one lift, by a top-to-bottom right run through the same centre crossing.
- Preserve RussianIrina's 08:42–08:49 two-facing-curve school-hand order while fitting Noto Sans Cyrillic's four straight diagonal arms.
- Add source, font-routing, on-ink, whole-glyph, and four-frame filmstrip coverage; the focused suite now passes 1,132 tests.

### Added — cited Cyrillic ф ductus (HL-C09EU)

- Render **ф** as a long central stem followed, after one lift, by one joined left-bowl-to-right-bowl run.
- Preserve RussianIrina's 08:16–08:26 stem-first narrow-loop school-hand order while fitting Noto Sans Cyrillic's straight ascender-descender and wider upright bowls.
- Add source, font-routing, on-ink, whole-glyph, and five-frame filmstrip coverage; the focused suite now passes 1,125 tests.

### Added — cited Cyrillic у ductus (HL-C09ET)

- Render **у** as one joined left-arm-to-right-arm-to-descender-to-left-terminal run with zero lifts.
- Preserve RussianIrina's 07:50–07:55 loop-descender Latin-y-like school-hand order while fitting Noto Sans Cyrillic's printed straight upper arms and unlooped left-curving terminal.
- Add source, font-routing, on-ink, whole-glyph, and four-frame filmstrip coverage; the focused suite now passes 1,118 tests.
...
- Preserve RussianIrina's 07:04–07:08 tall, slightly slanted school-hand order while fitting Noto Sans Cyrillic's wider upright C-like outline.
- Add source, font-routing, on-ink, whole-glyph, and two-frame filmstrip coverage; the focused suite now passes 1,104 tests.

### Added — cited Cyrillic р ductus (HL-C09EQ)

- Render **р** as one joined descender-to-upper-shoulder-to-closed-bowl run with zero lifts.
- Preserve RussianIrina's 06:46–06:52 long-descender school-hand order while fitting Noto Sans Cyrillic's straight stem and closed rounded bowl.
- Add source, font-routing, on-ink, whole-glyph, and three-frame filmstrip coverage; the focused suite now passes 1,097 tests.
