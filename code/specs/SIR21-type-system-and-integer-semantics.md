...
| T3c-3 | backends | Have each backend's emitter build a `TypeEnv` while walking a function body and consult `resolve_binary` when lowering a `BuiltinCall` binary op (currently every operand is `Dynamic`, so all resolve to `RuntimeDispatch` — behaviour-preserving until a typed frontend or sized-int lowering supplies types). One backend at a time, mirroring the OOP arc's per-backend-PR discipline — threading a type environment through a live emitter's full call graph is real, backend-specific work, not a one-line change. |
