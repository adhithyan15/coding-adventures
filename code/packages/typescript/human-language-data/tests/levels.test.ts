...
describe("corpus snapshot", () => {
  ...
  it("pins where the corpus actually stands on the ladder", () => {
    ...
    expect(summary.byLevel.A1).toBe(356); // +1: ES-C02-concordancia sits on SPINE-TIME-OF-DAY
    ...
    expect(summary.byLevel.A2).toBe(446); // +39: Spanish chapters 11-18 plus prerequisite closure
  ...
  it("can already build a ramp-to-A1 edition from the canonical corpus", () => {
    ...
    expect(ramp).toHaveLength(1252); // +3: HL-C97 adds the repair kit (no entiendo, mas despacio) at chapter 14
