...
describe("corpus snapshot", () => {
  ...
  it("pins the ramp, and the size of its blind spot", () => {
    ...
    expect(report.summary.unmeasurableLessons).toBe(587); // +2: HL-C88 slice 8 review + synthesis, correctly atom-free // +1: B1 si-condition synthesis, same reason // +2: the preterite review and synthesis, both correctly atom-free -- a review introduces nothing by definition
