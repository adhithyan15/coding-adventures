...
- ◑ **AL4** — literal string `print`/`output` I/O runs on all 7 backends via
  **E4**. Undeclared statement-position `print('HI')`/`output('HI')` calls lower
  ...
  unroll cap; this nonconditional form no longer requires a standard-function
  result in the base. Uninitialized or nonlocal values, fractional and
  non-finite results, calls, and nested powers remain on `f64_pow`.
