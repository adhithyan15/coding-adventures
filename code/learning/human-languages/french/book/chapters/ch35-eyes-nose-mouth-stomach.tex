...
% canonical-source-hash: fnv1a64:86b38061acf180ce
% canonical-lessons: FR-C31-oeil, FR-C31-nez, FR-C31-bouche, FR-C31-ventre, FR-R35-second-pass-marks-from-cold
...
\end{tcolorbox}
\section[le ventre]{The three marks, from cold}

\label{lesson:FR-R35-second-pass-marks-from-cold}



\begin{quote}
The word that closed four chapters of everyday nouns — the stomach. Say it with its article.
\end{quote}

\subsection*{You'll want to know: le ventre, and the nasal it shares}
\textbf{le ventre} — masculine, and nasal like \textbf{la main}, but reaching that nasal through a different vowel. Two body words, two routes to the same kind of sound, and the gender is not predictable from either.

\subsection*{Script: the three marks, thirty chapters later}
\begin{quote}
ê  ç  ï
\end{quote}

Cover the page and say what each one does before checking. They were taught three pages apart and have not been asked for since, which makes this the first real test of them:

\begin{itemize}
\raggedright
  \item \textbf{ê} records a letter that is gone. English usually still has it.
  \item \textbf{ç} keeps a \textbf{c} soft where it would otherwise harden.
  \item \textbf{ï} splits two vowels that would otherwise be read as one.
\end{itemize}

One is history, one is sound, one is grouping. Sorting them that way is what stops them blurring into a single topic called \emph{accents}.

\begin{tcolorbox}[breakable,colback=teal!4,colframe=teal!35!black,title={Before you move on}]
Which mark is a piece of history rather than an instruction? (\textbf{The circumflex}.) Which one changes a consonant? (\textbf{The cédille}.) Which one changes how two letters group? (\textbf{The tréma}.) And what gender is \textbf{ventre}? (\textbf{Masculine} — and nothing about the word says so.)
\end{tcolorbox}
