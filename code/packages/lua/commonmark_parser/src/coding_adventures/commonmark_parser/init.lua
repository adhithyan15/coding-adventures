...
local function parse_list_marker(line)
  ...
  -- Note: per CommonMark spec, the space separator is either `+` spaces or
...
-- Implements the CommonMark delimiter stack algorithm for emphasis.
...
local function extract_plain_text(nodes)
  ...
  for _, node in ipairs(nodes) do
    ...
    elseif node.type == "soft_break" then
      result = result .. " "
    elseif node.type == "emphasis" or node.type == "strong" or node.type == "link" then
...
-- Implements CommonMark Appendix A.
local function resolve_emphasis(tokens)
  ...
  while i <= #tokens do
    ...
    local use_len = (opener.count >= 2 and closer.count >= 2) and 2 or 1
    local is_strong = use_len == 2
...
--- Scan a delimiter run of `*` or `_` and compute flanking properties.
...
local function parse_inline(raw, link_refs)
  ...
  while not sc:done() do
    ...
    if ch == "&" then
      ...
      -- Per CommonMark spec, numeric references are only valid with
...
local function convert_to_ast(mutable_doc, link_refs)
  ...
  local function convert_block(block)
    ...
    elseif block.kind == "heading" then
      local id = next_id
      next_id = next_id + 1
      raw_inline_content[id] = block.content
    ...
    elseif block.kind == "paragraph" then
      ...
      -- Strip leading whitespace from each line (per CommonMark)
      local lines = {}
    ...
    elseif block.kind == "list" then
      ...
      for _, item in ipairs(block.items) do
        ...
        children[#children + 1] = ast.list_item(item_children)
      end
...
local function resolve_inline_content(document, raw_inline_content, link_refs)
  local function walk(block)
    if (block.type == "heading" or block.type == "paragraph") and block._raw_id ~= nil then
