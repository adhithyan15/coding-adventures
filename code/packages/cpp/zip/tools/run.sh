#!/bin/sh
# Build and run the zip tests under EVERY available C/C++ compiler (pure
...
# Include this package's headers, the sibling header-only `deflate` package's
# (CMP09 ZIP is a DEFLATE container — `zip.hpp` calls `ca::deflate::compress`
# / `inflate` directly), `deflate`'s own transitive dependency on `lzss`
# (DEFLATE's LZSS tokenization pass), and the harness's (for iso_test.h).
...
iso_build_and_run cpp zip-tests tests/zip_test.cpp
