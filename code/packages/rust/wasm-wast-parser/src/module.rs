    ...
    if let Some(items) = expr.as_list() {
        if items.len() == 3 && items[0].as_atom() == Some("ref") && items[1].as_atom() == Some("null") {
            return match items[2].as_atom() {
                ...
                // `(ref null any)` (W39 slice 4 Correction 3: `code/specs/
                // W39-wasm-gc-ref-eq-cast-br-on-cast.md`) -- the NULLABLE
                // counterpart of the non-null `(ref any)` fix just below
                // this function (its own 2-item non-null branch), needed
                // for the identical reason: `br_on_cast.wast`'s own
                // `(func (param (ref null any)) (result (ref $t)) ...)`
                // function signature is a genuine param declaration, not
                // an instruction-local type immediate `ref.test`/`ref.
                // cast`/`br_on_cast`'s own LOCAL parsing could intercept.
                // Maps to the SAME `ValueType::Anyref` the bare `anyref`
                // keyword already produces (this is not a new variant,
                // unlike the non-null case) -- but widening ITS reach here
                // risked the identical regression class: `ref_eq.wast`'s
                // own `assert_invalid (param $r (ref null any)) ...`
                // case relied on this exact rejection. Kept correctly
                // rejected anyway by the SAME targeted `wasm-validator`
                // `0xD3` check the non-null case needed, widened to cover
                // plain `Anyref` too -- see that arm's own doc comment.
                Some("any") => Ok(ValueType::Anyref),
        ...
        // `(ref any)` -- non-null reference to the ABSTRACT top of the
        // `any` hierarchy (W39 slice 4 Correction 3: `code/specs/
        // W39-wasm-gc-ref-eq-cast-br-on-cast.md`), needed for a genuine
        // FUNCTION SIGNATURE / block-result declaration (`br_on_cast.
        // wast`'s own `(func (param (ref any)) (result (ref $t)) ...)`) --
        // unlike Correction 2's five abstract non-null forms below,
        // `ref.test`/`ref.cast`/`br_on_cast`'s own local, per-instruction
        // parsing can't help here: a function param type has no "parse it
        // locally inside one instruction's own encoder" escape hatch, so
        // THIS one form genuinely widens the shared parser. Must come
        // before the generic `(ref $t)` concrete-reference branch below,
        // same reasoning as `i31`/`array` just above (`any` is a bare
        // keyword here, not a type name to resolve). See `ValueType::
        // NonNullAnyref`'s own doc comment for the real regression this
        // risked (`ref_eq.wast`'s own `assert_invalid (ref any)` param
        // case) and how it's kept correctly rejected anyway (`wasm-
        // validator`'s `0xD3` arm now does one narrow, targeted check).
        if items.len() == 2 && items[0].as_atom() == Some("ref") && items[1].as_atom() == Some("any") {
            return Ok(ValueType::NonNullAnyref);
        }
        // W32 second slice: `(ref $t)` -- non-null concrete reference, no
        // `null` keyword. Must come AFTER the `(ref i31)`/`(ref array)`/
        // `(ref any)` checks above (all three stay their own dedicated
        // variant regardless of nullability spelling, see those branches'
        // own doc comments) so it only ever catches a real named/numeric
        // type reference here.
        //
        // Non-null `(ref eq)`/`(ref struct)`/`(ref func)`/`(ref extern)`
        // remain UNRECOGNIZED here, same as before W39 -- this function is
        // shared by every reference-type-typed declaration in the text
        // format (params, locals, globals, block types, ...), and
        // loosening it to accept these four (plus `any`, before W39 slice
        // 4's Correction 3 above) here was tried and reverted: `ref_eq.
        // wast`'s own `assert_invalid` cases rely on exactly this
        // rejection (a `(param $r (ref func))` declaration must be
        // rejected SOMEWHERE for `(ref.eq (local.get $r) ...)` to
        // correctly fail as "type mismatch" -- today that "somewhere" is
        // this parser, since `wasm-validator`'s own `ref.eq` arm doesn't do
        // real per-operand type checking for THESE four; loosening parsing
        // here without ALSO tightening that validator arm would silently
        // regress those corpus directives from "correctly rejected" to
        // "wrongly accepted"). `ref.test`/`ref.cast`'s own encoder (W39
        // slice 2b, below) needs all five forms too, but parses them
        // ITSELF, locally, rather than widening this shared function's
        // blast radius -- see that encoder's own doc comment.
    ...
    if let Some(simd_op) = wasm_opcodes::get_simd_op_by_name(name) {
        match simd_op.kind {
            ...
            | wasm_opcodes::SimdOpKind::Load32x2U => {
                ...
                // v128.loadN_splat family (SIMD PR40), the v128.loadN_zero
                // family (SIMD PR41), plus the v128.load_extend family
                // (SIMD PR42): a standard `memarg` immediate (align,
                // offset), same shape and parser every scalar `iNN.load`/
                // `iNN.store` uses -- this first slice doesn't support an
                // explicit leading memidx token (memory 0 only, same
                // scope note as the executor in wasm-execution). The
                // encoding is identical regardless of Load vs. Store vs.
            ...
            | wasm_opcodes::SimdOpKind::RelaxedDotI8x16I7x16AddS => {
                ...
                // likewise invisible here. `ExtendLow/HighI8x16S/_U`/
                // `ExtendLow/HighI16x8S/_U` (SIMD widen PR26) join too:
                // the "extend" family is UNARY (pop one v128, push one),
                ...
                // invisible at this encoding level. `NegF64x2`/
                // `SqrtF64x2`/`AddF64x2`/`SubF64x2`/`MulF64x2`/
                // `DivF64x2` (SIMD widen PR31) join too, a direct
                ...
                // `MaxF32x4`/`PminF32x4`/`PmaxF32x4` (SIMD widen PR34)
                // join too, closing the gap left by PR19/PR29 in f32x4's
                // arithmetic family -- all 3 BINARY, same no-immediate
                // encoding shape; `max`'s NaN-canonicalizing runtime
                // subtlety (mirroring `MinF32x4`) and `pmin`/`pmax`'s
...
/// Parse a `ref.test`/`ref.cast`/`br_on_cast`/`br_on_cast_fail` (W39 slices
/// 2 and 4: `code/specs/W39-wasm-gc-ref-eq-cast-br-on-cast.md`) reftype
/// immediate -- `(ref ht)` (non-null) / `(ref null ht)` (nullable) or a
/// bare abbreviation like `structref`/`i31ref`/`$t0` -- into its binary
/// heap-type-immediate bytes plus whether the reftype ITSELF is nullable.
/// Factored out of `ref.test`/`ref.cast`'s own encoder (W39 slice 2) so
/// `br_on_cast`/`br_on_cast_fail` (W39 slice 4) can reuse the EXACT same
/// logic for their own two reftype immediates (`ht1`/`ht2`) instead of a
/// second hand-copied match -- the spec's own explicit instruction ("reuse
/// slice 2's own heap-type-immediate encoder verbatim").
///
/// `(ref any)` / `(ref eq)` / `(ref struct)` / `(ref func)` / `(ref
/// extern)` -- the non-null spelling of five abstract-hierarchy heap
/// types, a real corpus need (`ref_test.wast`'s own `(ref.test (ref any)
/// ...)`/`(ref eq)`/`(ref struct)`/`(ref func)`/`(ref extern)`, confirmed
/// by direct read) -- are handled LOCALLY, right here, rather than by
/// widening `parse_value_type`'s own shared non-null `(ref X)` branch:
/// that was tried first and reverted -- `parse_value_type` is shared by
/// every reference-typed declaration in the text format, and loosening it
/// there let `ref_eq.wast`'s own `assert_invalid` param-type cases
/// (`(param $r (ref func))`, expected to be REJECTED as "type mismatch")
/// parse successfully instead, silently regressing three corpus
/// directives from "correctly rejected" to "wrongly accepted" (this
/// crate's `ref.eq` validator arm doesn't do real per-operand type
/// checking, so nothing downstream caught it). Matching this crate's own
/// "minimal, targeted fix" discipline, this local match keeps the widened
/// acceptance scoped to exactly the instructions that actually need it.
/// `(ref null any)` and the four bottom heap types' own `(ref null X)`
/// compound spelling get the same local-only treatment, one level over --
/// see the match arms below for the full accounting.
fn parse_gc_reftype_immediate(ty_expr: &SExpr, icx: &InstrCtx) -> Result<(Vec<u8>, bool), WastParseError> {
    let local_abstract = if let Some(items) = ty_expr.as_list() {
        if items.len() == 2 && items[0].as_atom() == Some("ref") {
            ...
            // `(ref null any)` / the four bottom heap types' own `(ref null
            // X)` compound spelling (`nofunc`/`noextern`/`none`/`noexn`) --
            // see this function's own doc comment for the regression this
            // avoids by staying local instead of widening the shared
            // `parse_value_type`. These map to the SAME `ValueType::
            // Null{Ref,Funcref,Externref,Exnref}`/`Anyref` variants
            // `parse_value_type`'s own bare-atom match already produces for
            // `nullref`/`nullfuncref`/`nullexternref`/`nullexnref`/`anyref`
            // (and this function's own `parsed_ty` match below already has
            // arms for all of them, unconditionally nullable) -- no new
            // untested surface, just a second parse route into the same
            // variants.
...
fn encode_flat_instr(
    ...
    if name == "ref.test" || name == "ref.cast" {
        let ty_expr = args.first().ok_or(WastParseError::UnexpectedEof)?;
        let (immediate, nullable) = parse_gc_reftype_immediate(ty_expr, icx)?;
        encode_instr_list(&args[1..], icx, out)?;
        out.push(0xFB);
    ...
    // `br_on_cast` / `br_on_cast_fail` (W39 slice 4: `code/specs/
    // W39-wasm-gc-ref-eq-cast-br-on-cast.md`) -- conditional branches that
    // dynamically test a value already on the stack against a reftype,
    // branching on success (`br_on_cast`) or failure (`br_on_cast_fail`).
    // Real grammar: `(br_on_cast $l rt1 rt2 <value>?)` -- a label, TWO full
    // reftype descriptors (`rt2 <: rt1`, checked at validation time, not
    // here), then an OPTIONAL folded value expression. The value operand is
    // genuinely optional in the real corpus (`br_on_cast.wast`'s own
    // `(br_on_cast $l2 structref (ref $st))`, no trailing expression at
    // all) -- the tested value is then whatever a PRIOR instruction already
    // left on the operand stack (typically an enclosing block's own
    // `param`), exactly the same "operand may already be on the stack, not
    // necessarily supplied inline" shape `encode_instr_list` already
    // handles generically for zero-or-more trailing args (an empty slice
    // emits no bytes at all, leaving whatever's already on the stack
    // untouched).
    //
    // `ht1`/`ht2` are parsed via the EXACT SAME `parse_gc_reftype_
    // immediate` helper `ref.test`/`ref.cast` just above use (the spec's
    // own explicit instruction: "reusing slice 2's own heap-type-immediate
    // encoder verbatim for `ht1`/`ht2`") -- not reimplemented. Binary
    // layout, per the real spec's own table (`code/specs/
    // W39-wasm-gc-ref-eq-cast-br-on-cast.md`'s "Real spec text" section):
    // `0xFB 0x18/0x19 <flags:u8> <labelidx:LEB> <ht1> <ht2>` -- `flags` bit
    // 0 = rt1 nullable, bit 1 = rt2 nullable, matching `wasm-execution`'s
    // decoder byte-for-byte (see that crate's own `GcOp::flags` doc
    // comment).
    if name == "br_on_cast" || name == "br_on_cast_fail" {
        let label_expr = args.first().ok_or(WastParseError::UnexpectedEof)?;
        let rt1_expr = args.get(1).ok_or(WastParseError::UnexpectedEof)?;
        let rt2_expr = args.get(2).ok_or(WastParseError::UnexpectedEof)?;
        let (ht1, null1) = parse_gc_reftype_immediate(rt1_expr, icx)?;
        let (ht2, null2) = parse_gc_reftype_immediate(rt2_expr, icx)?;
        let value_args = if args.len() > 3 { &args[3..] } else { &[] };
        encode_instr_list(value_args, icx, out)?;
        out.push(0xFB);
        out.push(if name == "br_on_cast" { 0x18 } else { 0x19 });
        let flags: u8 = (null1 as u8) | ((null2 as u8) << 1);
        out.push(flags);
    ...
    if let Some(simd_op) = wasm_opcodes::get_simd_op_by_name(name) {
        match simd_op.kind {
            ...
            | wasm_opcodes::SimdOpKind::Load32x2U => {
                ...
                // family (SIMD PR41), plus the v128.load_extend family
                // (SIMD PR42): a standard `memarg` immediate (align,
                // offset) -- this first slice doesn't support an explicit
                // leading memidx token (memory 0 only, same scope note as
            ...
            | wasm_opcodes::SimdOpKind::RelaxedDotI8x16I7x16AddS => {
                ...
                // `NegF64x2`/`SqrtF64x2`/`AddF64x2`/`SubF64x2`/
...
"#;
    ...
    #[test]
    fn v128_load_and_store_encode_the_real_sub_opcodes_with_a_memarg() {
        // SIMD widen PR15: v128.load/v128.store -- the FIRST SIMD ops
        // needing a memarg immediate (align + offset), both single-byte
        // sub-opcodes (0x00/0x0B). Covers both the folded form (with an
        // explicit `offset=` attribute) and the flat/stream form, mirroring
        // `memory_load_with_offset_attribute`'s scalar-op precedent above.
        let m = parse_module(
            r#"(module (memory 1)
                 (func (param i32) (result v128) (v128.load offset=8 (local.get 0)))
                 (func (param i32 v128) (v128.store (local.get 0) (local.get 1))))"#,
        )
        .unwrap();
        // local.get 0 ; v128.load align=0 offset=8 ; end
        assert_eq!(code_of(&m, 0), &[0x20, 0x00, 0xFD, 0x00, 0x00, 0x08, 0x0B]);
        // local.get 0 ; local.get 1 ; v128.store align=0 offset=0 ; end
        assert_eq!(code_of(&m, 1), &[0x20, 0x00, 0x20, 0x01, 0xFD, 0x0B, 0x00, 0x00, 0x0B]);
    ...
    #[test]
    fn v128_load_zero_family_encodes_the_real_sub_opcodes_with_a_memarg() {
        // SIMD PR41: v128.load32_zero/load64_zero -- reuses the SAME
        // memarg-parsing match arm `v128.load`/`v128.store`/the
        // `load_splat` family already own (see this file's own
        // `SimdOpKind::Load | ... | Load64Zero` arm), just with the new
        // sub-opcode values (0x5C/0x5D). Covers both the folded form
        // (with an explicit `offset=` attribute) and the flat/stream
        // form, mirroring
        ...
        let m = parse_module(
            r#"(module (memory 1)
                 (func (param i32) (result v128) (v128.load32_zero offset=8 (local.get 0)))
                 (func (param i32) (result v128) (v128.load64_zero (local.get 0))))"#,
        )
        .unwrap();
        // local.get 0 ; v128.load32_zero align=0 offset=8 ; end
        assert_eq!(code_of(&m, 0), &[0x20, 0x00, 0xFD, 0x5C, 0x00, 0x08, 0x0B]);
        assert_eq!(code_of(&m, 1), &[0x20, 0x00, 0xFD, 0x5D, 0x00, 0x00, 0x0B]);

        let flat = parse_module(
            "(module (memory 1) (func (param i32) (result v128) local.get 0 v128.load32_zero))",
        )
        .unwrap();
        assert_eq!(code_of(&flat, 0), &[0x20, 0x00, 0xFD, 0x5C, 0x00, 0x00, 0x0B]);
    }

    #[test]
    fn v128_load_extend_family_encodes_the_real_sub_opcodes_with_a_memarg() {
        // SIMD PR42: v128.load8x8_s/_u, v128.load16x4_s/_u,
        // v128.load32x2_s/_u -- reuses the SAME memarg-parsing match arm
        // `v128.load`/`v128.store`/the `load_splat`/`load_zero` families
        // already own (see this file's own `SimdOpKind::Load | ... |
        // Load32x2U` arm), just with the new sub-opcode values
        // (0x01-0x06). Covers both the folded form (with an explicit
        // `offset=` attribute) and the flat/stream form, mirroring
        // `v128_load_zero_family_encodes_the_real_sub_opcodes_with_a_memarg`
        // above.
        let m = parse_module(
            r#"(module (memory 1)
                 (func (param i32) (result v128) (v128.load8x8_s offset=8 (local.get 0)))
        ...
        .unwrap();
        // local.get 0 ; v128.load8x8_s align=0 offset=8 ; end
        assert_eq!(code_of(&m, 0), &[0x20, 0x00, 0xFD, 0x01, 0x00, 0x08, 0x0B]);
        assert_eq!(code_of(&m, 1), &[0x20, 0x00, 0xFD, 0x02, 0x00, 0x00, 0x0B]);
        assert_eq!(code_of(&m, 2), &[0x20, 0x00, 0xFD, 0x03, 0x00, 0x00, 0x0B]);
        assert_eq!(code_of(&m, 3), &[0x20, 0x00, 0xFD, 0x04, 0x00, 0x00, 0x0B]);
        assert_eq!(code_of(&m, 4), &[0x20, 0x00, 0xFD, 0x05, 0x00, 0x00, 0x0B]);
        assert_eq!(code_of(&m, 5), &[0x20, 0x00, 0xFD, 0x06, 0x00, 0x00, 0x0B]);

        let flat = parse_module(
            "(module (memory 1) (func (param i32) (result v128) local.get 0 v128.load8x8_s))",
        )
        .unwrap();
    ...
    #[test]
    fn v128_load16_lane_and_store16_lane_encode_the_real_sub_opcode_with_a_combined_memarg_and_lane_index() {
        // SIMD PR45: v128.load16_lane (0x55) / v128.store16_lane (0x59)
        // -- one width up from `v128.load8_lane`/`v128.store8_lane`
        ...
        // corpus: `(v128.load16_lane offset=8 5 (addr) (x))`. Binary
        // encoding order (verified against BinarySIMD.md: "m:memarg,
        // i:ImmLaneIdx8") is sub-opcode, align, offset, THEN the raw
        // lane-index byte -- same order as the 8-bit pair, just a
        // narrower valid lane range (0-7, not checked at this layer --
        // see `wasm-validator`'s own test for that).
        let m = parse_module(
            r#"(module (memory 1)
                 (func (param i32 v128) (result v128) (v128.load16_lane offset=8 5 (local.get 0) (local.get 1)))
                 (func (param i32 v128) (v128.store16_lane offset=8 5 (local.get 0) (local.get 1))))"#,
        )
        .unwrap();
        // local.get 0 ; local.get 1 ; v128.load16_lane align=0 offset=8 lane=5 ; end
        assert_eq!(code_of(&m, 0), &[0x20, 0x00, 0x20, 0x01, 0xFD, 0x55, 0x00, 0x08, 0x05, 0x0B]);
        // local.get 0 ; local.get 1 ; v128.store16_lane align=0 offset=8 lane=5 ; end
        assert_eq!(code_of(&m, 1), &[0x20, 0x00, 0x20, 0x01, 0xFD, 0x59, 0x00, 0x08, 0x05, 0x0B]);

        // Flat/stream form: operands already emitted, memarg tokens (none
        // here -- defaults to align=0/offset=0) and the lane-index
        // literal trail the mnemonic.
        let flat = parse_module(
            "(module (memory 1) (func (param i32 v128) (result v128) local.get 0 local.get 1 v128.load16_lane 5))",
        )
        .unwrap();
        assert_eq!(code_of(&flat, 0), &[0x20, 0x00, 0x20, 0x01, 0xFD, 0x55, 0x00, 0x00, 0x05, 0x0B]);
    }

    #[test]
    fn v128_load32_lane_and_store32_lane_encode_the_real_sub_opcode_with_a_combined_memarg_and_lane_index() {
        // SIMD PR46: v128.load32_lane (0x56) / v128.store32_lane (0x5A)
        // -- one width up from `v128.load16_lane`/`v128.store16_lane`
        // above, reusing the identical encoder arm (see that arm's own
        // updated doc comment) since the encoding shape is unchanged --
        // only `simd_op.sub_opcode` differs. Real WAT syntax confirmed
        // against the pinned simd_load32_lane.wast/simd_store32_lane.wast
        // corpus: `(v128.load32_lane offset=8 2 (addr) (x))`. Binary
        // encoding order (verified against BinarySIMD.md: "m:memarg,
        // i:ImmLaneIdx4") is sub-opcode, align, offset, THEN the raw
        // lane-index byte -- same order as the 8-bit and 16-bit pairs,
        // just a narrower valid lane range (0-3, not checked at this
        // layer -- see `wasm-validator`'s own test for that).
        let m = parse_module(
            r#"(module (memory 1)
                 (func (param i32 v128) (result v128) (v128.load32_lane offset=8 2 (local.get 0) (local.get 1)))
                 (func (param i32 v128) (v128.store32_lane offset=8 2 (local.get 0) (local.get 1))))"#,
        )
        .unwrap();
        // local.get 0 ; local.get 1 ; v128.load32_lane align=0 offset=8 lane=2 ; end
        assert_eq!(code_of(&m, 0), &[0x20, 0x00, 0x20, 0x01, 0xFD, 0x56, 0x00, 0x08, 0x02, 0x0B]);
        // local.get 0 ; local.get 1 ; v128.store32_lane align=0 offset=8 lane=2 ; end
        assert_eq!(code_of(&m, 1), &[0x20, 0x00, 0x20, 0x01, 0xFD, 0x5A, 0x00, 0x08, 0x02, 0x0B]);

        // Flat/stream form: operands already emitted, memarg tokens (none
        // here -- defaults to align=0/offset=0) and the lane-index
        // literal trail the mnemonic.
        let flat = parse_module(
            "(module (memory 1) (func (param i32 v128) (result v128) local.get 0 local.get 1 v128.load32_lane 2))",
        )
        .unwrap();
    ...
    #[test]
    fn float_splat_family_encodes_the_real_sub_opcodes() {
        // SIMD widen PR17: f32x4.splat/f64x2.splat -- the immediate
        // continuation of PR16's splat run, joining the same shared
        // "no immediate beyond the opcode byte itself" encoder arm.
        // Both sub-opcodes (0x13/0x14) are single-byte LEB128. The
        // mixed f32/f64 operand TYPES are invisible to this encoder (a
        // type-checker concern -- see wasm-validator), since operand
        // count/values are driven entirely by the S-expression
        // recursion that already ran.
        let m = parse_module(
            r#"(module
        ...
        .unwrap();
        assert!(code_of(&m, 0).windows(2).any(|w| w == [0xFD, 0x13]), "f32x4.splat: {:?}", code_of(&m, 0));
        assert!(code_of(&m, 1).windows(2).any(|w| w == [0xFD, 0x14]), "f64x2.splat: {:?}", code_of(&m, 1));

        let flat = parse_module(
            "(module (func (param f32) (result v128) local.get 0 f32x4.splat))",
        )
        .unwrap();
        assert!(code_of(&flat, 0).windows(2).any(|w| w == [0xFD, 0x13]), "flat f32x4.splat: {:?}", code_of(&flat, 0));
    }
    ...
    #[test]
    fn i8x16_swizzle_emits_the_prefix_and_no_immediate() {
        // SIMD widen PR18: i8x16.swizzle (sub-opcode 0x0E) -- a plain
        // BINARY v128,v128->v128 op, same "no immediate beyond the
        // opcode byte itself" shape as i8x16.add, just a different
        // sub-opcode.
        let folded = parse_module(
            r#"(module (func (param v128 v128) (result v128) (i8x16.swizzle (local.get 0) (local.get 1))))"#,
        )
        .unwrap();
        let flat = parse_module(
            r#"(module (func (param v128 v128) (result v128) local.get 0 local.get 1 i8x16.swizzle))"#,
        )
        .unwrap();
        for code in [code_of(&folded, 0), code_of(&flat, 0)] {
    ...
    #[test]
    fn i8x16_extract_lane_s_and_u_folded_and_flat_lane_index_leads_and_covers_0_and_15() {
        // SIMD widen PR18: i8x16.extract_lane_s (0x15) / _u (0x16) --
        // same "lane index leads in folded form, trails in flat/stream
        // form" shape as i32x4.extract_lane, just at i8x16's 0-15 lane
        // range instead of i32x4's 0-3. Covers both ends of the valid
        // range (0 and 15), not just an arbitrary middle value.
        for (name, sub_opcode) in [("i8x16.extract_lane_s", 0x15u8), ("i8x16.extract_lane_u", 0x16u8)] {
            for lane in [0u8, 15u8] {
                let folded = parse_module(&format!(
                    r#"(module (func (param v128) (result i32) ({name} {lane} (local.get 0))))"#
                ))
                .unwrap();
                let flat = parse_module(&format!(r#"(module (func (param v128) (result i32) local.get 0 {name} {lane}))"#)).unwrap();
                for code in [code_of(&folded, 0), code_of(&flat, 0)] {
                    assert!(
    ...
    #[test]
    fn i8x16_replace_lane_folded_and_flat_lane_index_leads_and_two_operands_encode_in_order() {
        // SIMD widen PR18: i8x16.replace_lane (0x17) -- the lane index
        // leads (folded)/trails (flat), same convention as extract_lane*,
        // but TWO operands (v128, then i32) must both encode, in order,
        // around it -- the genuinely new shape this op introduces.
        let folded = parse_module(
            r#"(module (func (param v128 i32) (result v128) (i8x16.replace_lane 7 (local.get 0) (local.get 1))))"#,
        )
        .unwrap();
        let flat = parse_module(
            r#"(module (func (param v128 i32) (result v128) local.get 0 local.get 1 i8x16.replace_lane 7))"#,
        )
        .unwrap();
        for code in [code_of(&folded, 0), code_of(&flat, 0)] {
            // local.get 0 (0x20 0x00), local.get 1 (0x20 0x01), then
            // [0xFD, 0x17, 0x07] -- both operands precede the SIMD op,
            // in source order, with the lane-index byte trailing the
            // sub-opcode.
            assert!(
                code.windows(2).any(|w| w == [0x20, 0x00]),
                "missing local.get 0 (the v128 operand): {code:?}"
            );
            assert!(
    ...
    #[test]
    fn f32x4_max_pmin_pmax_family_encodes_the_real_two_byte_leb128_sub_opcodes() {
        // SIMD widen PR34 (task #217-219): f32x4.max (0xE9), f32x4.pmin
        // (0xEA), f32x4.pmax (0xEB) -- the last 3 opcodes of f32x4's
        // arithmetic family, sitting immediately past f32x4.min's 0xE8
        // (PR19) with no gap. Same "real 2-byte LEB128" shape as the
        // f32x4_arith3 test above (all three sub-opcode values are >=
        // 128), and same "no immediate beyond the opcode bytes
        // themselves" BINARY shape -- `max`'s NaN-canonicalizing runtime
        // semantics and `pmin`/`pmax`'s deliberately simpler `<`-based
        // select are both entirely invisible at this encoding level.
        let folded = parse_module(
            r#"(module (func (param v128 v128) (result v128)
                 (f32x4.pmax (f32x4.pmin (f32x4.max (local.get 0) (local.get 1)) (local.get 1)) (local.get 1))))"#,
        )
        .unwrap();
        let flat = parse_module(
            r#"(module (func (param v128 v128) (result v128)
                 local.get 0 local.get 1 f32x4.max local.get 1 f32x4.pmin local.get 1 f32x4.pmax))"#,
        )
        .unwrap();
        for code in [code_of(&folded, 0), code_of(&flat, 0)] {
    ...
    #[test]
    fn f64x2_arith_family_encodes_the_real_two_byte_leb128_sub_opcodes() {
        // SIMD widen PR31 (task #208-210): f64x2.neg (0xED), f64x2.sqrt
        // (0xEF), f64x2.add (0xF0), f64x2.sub (0xF1), f64x2.mul (0xF2),
        // f64x2.div (0xF3) -- a direct structural mirror of PR29's
        // f32x4.neg/sqrt/add/sub/div above, at f64x2's 2-lane width, plus
        // `mul` on the same binary-op shape (f64x2.mul didn't exist
        // before this PR). All six sub-opcode values are >= 128, so each
        // encodes as a real 2-byte LEB128 sequence (`[byte, 0x01]`), same
        // as every f32x4 arith op since PR19. `neg`/`sqrt` are UNARY (one
        // v128 operand); `add`/`sub`/`mul`/`div` are BINARY (two) -- same
        // "no immediate beyond the opcode bytes themselves" shape as the
        // rest of this table.
        let folded = parse_module(
            r#"(module (func (param v128 v128) (result v128)
                 (f64x2.div
                   (f64x2.mul
                     (f64x2.sub
                       (f64x2.add (f64x2.sqrt (f64x2.neg (local.get 0))) (local.get 1))
                       (local.get 1))
                     (local.get 1))
                   (local.get 1))))"#,
        )
        ...
        let flat = parse_module(
            r#"(module (func (param v128 v128) (result v128)
                 local.get 0 f64x2.neg f64x2.sqrt
                 local.get 1 f64x2.add
                 local.get 1 f64x2.sub
                 local.get 1 f64x2.mul
                 local.get 1 f64x2.div))"#,
        )
        .unwrap();
        for code in [code_of(&folded, 0), code_of(&flat, 0)] {
            assert!(code.windows(3).any(|w| w == [0xFD, 0xED, 0x01]), "missing f64x2.neg: {code:?}");
            assert!(code.windows(3).any(|w| w == [0xFD, 0xEF, 0x01]), "missing f64x2.sqrt: {code:?}");
            assert!(code.windows(3).any(|w| w == [0xFD, 0xF0, 0x01]), "missing f64x2.add: {code:?}");
            assert!(code.windows(3).any(|w| w == [0xFD, 0xF1, 0x01]), "missing f64x2.sub: {code:?}");
            assert!(code.windows(3).any(|w| w == [0xFD, 0xF2, 0x01]), "missing f64x2.mul: {code:?}");
            assert!(code.windows(3).any(|w| w == [0xFD, 0xF3, 0x01]), "missing f64x2.div: {code:?}");
        }
    }
    ...
    #[test]
    fn simd_extend_low_high_family_encodes_the_real_two_byte_leb128_sub_opcodes() {
        // SIMD widen PR26 (task #193-195): i16x8.extend_low/high_i8x16_s/
        // _u (0x87/0x88/0x89/0x8A) and i32x4.extend_low/high_i16x8_s/_u
        // (0xA7/0xA8/0xA9/0xAA) -- all eight sub-opcode values are >= 128,
        // so each encodes as a real 2-byte LEB128 sequence (`[byte,
        // 0x01]`). All eight are UNARY (one v128 operand) -- same "no
        // immediate beyond the opcode bytes themselves" shape as
        // `i16x8.extmul_low_i8x16_s`/etc. above, exercised here in both
        // folded and flat/stream form (this crate's two independent
        // encoders).
        let folded = parse_module(
            r#"(module
        ...
        .unwrap();
        assert!(code_of(&folded, 0).windows(3).any(|w| w == [0xFD, 0x87, 0x01]), "i16x8.extend_low_i8x16_s: {:?}", code_of(&folded, 0));
        assert!(code_of(&folded, 1).windows(3).any(|w| w == [0xFD, 0x88, 0x01]), "i16x8.extend_high_i8x16_s: {:?}", code_of(&folded, 1));
        assert!(code_of(&folded, 2).windows(3).any(|w| w == [0xFD, 0x89, 0x01]), "i16x8.extend_low_i8x16_u: {:?}", code_of(&folded, 2));
        assert!(code_of(&folded, 3).windows(3).any(|w| w == [0xFD, 0x8A, 0x01]), "i16x8.extend_high_i8x16_u: {:?}", code_of(&folded, 3));
        assert!(code_of(&folded, 4).windows(3).any(|w| w == [0xFD, 0xA7, 0x01]), "i32x4.extend_low_i16x8_s: {:?}", code_of(&folded, 4));
        assert!(code_of(&folded, 5).windows(3).any(|w| w == [0xFD, 0xA8, 0x01]), "i32x4.extend_high_i16x8_s: {:?}", code_of(&folded, 5));
        assert!(code_of(&folded, 6).windows(3).any(|w| w == [0xFD, 0xA9, 0x01]), "i32x4.extend_low_i16x8_u: {:?}", code_of(&folded, 6));
        assert!(code_of(&folded, 7).windows(3).any(|w| w == [0xFD, 0xAA, 0x01]), "i32x4.extend_high_i16x8_u: {:?}", code_of(&folded, 7));
        ...
        .unwrap();
        let flat_code = code_of(&flat, 0);
        assert!(flat_code.windows(3).any(|w| w == [0xFD, 0x87, 0x01]), "missing i16x8.extend_low_i8x16_s: {flat_code:?}");
