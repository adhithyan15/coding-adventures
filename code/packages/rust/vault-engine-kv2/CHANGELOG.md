...
- `KvV2EngineSummary` and `KvV2PathSummary` plus `summary`,
  `path_summary`, and `list_path_summaries` for redacted inventory
  reporting: mount config, root generation, path counts, and
  live/destroyed/retained version counts without cloning secret bytes.
