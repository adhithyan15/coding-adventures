//! C# lexer backed by compiled versioned token grammars.
...
pub const DEFAULT_VERSION: &str = "12.0";
...
pub fn create_csharp_lexer<'src>(
    ...
    let grammar =
        _grammar::token_grammar(version).expect("compiled C# token grammar missing supported version");
...
mod tests {
    ...
    fn token_pairs(tokens: &[Token]) -> Vec<(TokenType, &str)> {
        tokens
            ...
            .map(|token| (token.type_, token.value.as_str()))
            .collect()
    }
    ...
    fn tokenizes_basic_class() {
        ...
        assert_eq!(pairs[0], (TokenType::Keyword, "class"));
        assert_eq!(pairs[1], (TokenType::Name, "Hello"));
    }

    #[test]
    fn all_supported_versions_load() {
        for version in SUPPORTED_VERSIONS {
            let tokens = tokenize_csharp("42;", version).unwrap();
    ...
    fn unknown_version_returns_error() {
        let error = tokenize_csharp("int x = 1;", "99.0").unwrap_err();
