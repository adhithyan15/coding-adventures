...
describe("the script ramp against the real corpus", () => {
  it("pins the script ramp, which no gate had ever measured", () => {
    ...
    expect(report.summary.lessonsWithForeignScript).toBe(219); // tamil pre-A1 tranche: +2, not +9. This counts FIRST sightings, and the tamil track had already shown nearly every cousin glyph these lessons cite. Exactly two are new: TA-C44-salt is the first tamil lesson to print ಉ (KANNADA LETTER U), and TA-C45-ear the first to print െ (MALAYALAM VOWEL SIGN E). The other seven citing lessons show only glyphs earlier tamil lessons had already put on the page // HL-C134: the hand-written prose carried back into the lessons is now visible to this measurement — the words were always on the page, only the markdown had not seen them // HL-C190: +16 -- see/say verbs, Kannada Telugu Malayalam Hindi // HL-C190 // HL-C200: +35 telugu pre-A1 lessons, +7 chapters (chapters 46-52) // kannada pre-A1 tranche: +35 lessons, +7 chapters (chapters 46-52) // malayalam pre-A1 tranche: +35 lessons, +7 chapters (chapters 46-52) // tamil pre-A1 round 3: +35 lessons, +7 chapters (chapters 58-64) // hindi pre-A1 round 3: +1, not +35. This counts FIRST sightings, and hindi had already printed nearly every Perso-Arabic letter these lessons cite. Exactly one is new: HI-C60-hen is the first hindi lesson to print ARABIC LETTER GHAIN, inside the Persian source of murgi. The other 34 new lessons show no foreign script at all
