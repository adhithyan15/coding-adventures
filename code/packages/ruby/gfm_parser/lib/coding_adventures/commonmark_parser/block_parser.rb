...
# Block-Level Parser
#
# Phase 1 of GFM parsing: split the input into block-level tokens
...
module CodingAdventures
  module CommonmarkParser
    ...
    def self.convert_to_ast(mutable_doc, link_refs)
      convert_block = lambda do |block|
        ...
        when MutableParagraph
          ...
          table = maybe_parse_table(content, link_refs)
        ...
        when MutableListItem
          children = block.children.filter_map { |b| convert_block.call(b) }
          task = maybe_convert_task_item(block, children, link_refs)
          task || DocumentAst::ListItemNode.new(children: children)
    ...
    def self.maybe_convert_task_item(block, converted_children, link_refs)
      return nil if converted_children.empty?
      return nil unless block.children.first.is_a?(MutableParagraph)
      return nil unless converted_children.first&.type == "paragraph"

      content = block.children.first.lines.map { |line| line.sub(/\A[ \t]+/, "") }.join("\n")
      match = /\A\[( |x|X)\](?:[ \t]+|$)/.match(content)
      return nil unless match

      stripped = content[match[0].length..] || ""
      children = converted_children.dup
      children[0] = DocumentAst::ParagraphNode.new(
        children: CommonmarkParser.parse_inline(stripped, link_refs)
      )

      DocumentAst::TaskItemNode.new(
        checked: match[1].downcase == "x",
        children: children
      )
    end

    def self.maybe_parse_table(content, link_refs)
      parsed = try_parse_table(content)
      return nil unless parsed

      rows = []
      rows << DocumentAst::TableRowNode.new(
        is_header: true,
        children: normalize_table_row(parsed[:header], parsed[:align].length).map { |cell|
          DocumentAst::TableCellNode.new(children: CommonmarkParser.parse_inline(cell, link_refs))
        }
      )
      parsed[:rows].each do |row|
        rows << DocumentAst::TableRowNode.new(
          is_header: false,
          children: normalize_table_row(row, parsed[:align].length).map { |cell|
            DocumentAst::TableCellNode.new(children: CommonmarkParser.parse_inline(cell, link_refs))
          }
        )
      end

      DocumentAst::TableNode.new(align: parsed[:align], children: rows)
    end

    def self.try_parse_table(content)
      lines = content.split("\n")
      return nil if lines.length < 2

      header_cells = split_table_row(lines[0])
      delimiter_cells = split_table_row(lines[1])
      return nil if header_cells.nil? || delimiter_cells.nil?
      return nil if header_cells.empty? || header_cells.length != delimiter_cells.length

      align = delimiter_cells.map do |cell|
        trimmed = cell.strip
        return nil unless trimmed.match?(/\A:?-{3,}:?\z/)
      ...
      rows = lines[2..].map do |line|
        return nil if line.strip.empty?
        cells = split_table_row(line)
        return nil if cells.nil?
        normalize_table_row(cells, header_cells.length)
      end

      {
        align: align,
        header: normalize_table_row(header_cells, delimiter_cells.length),
        rows: rows
      }
    end

    def self.split_table_row(line)
      return nil unless line.include?("|")

      stripped = line.strip
      had_outer_pipe = stripped.start_with?("|") || stripped.end_with?("|")

      start_idx = 0
      end_idx = line.length
      start_idx += 1 while start_idx < end_idx && whitespace?(line[start_idx])
      end_idx -= 1 while end_idx > start_idx && whitespace?(line[end_idx - 1])
      start_idx += 1 if start_idx < end_idx && line[start_idx] == "|"
      end_idx -= 1 if end_idx > start_idx && line[end_idx - 1] == "|"
      ...
      line[start_idx...end_idx].each_char do |ch|
        if escaped
          current << ch
          escaped = false
        elsif ch == "\\"
      ...
      cells << current.strip
      return nil if pipe_count.zero? && !had_outer_pipe

      cells
    end

    def self.normalize_table_row(cells, width)
      normalized = cells.first(width)
      normalized << "" while normalized.length < width
      normalized
    end
