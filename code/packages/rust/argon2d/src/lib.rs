...
//! reference block index is derived from the first 64 bits of the previously
//! computed block. This maximises GPU/ASIC resistance at the cost of leaking
//! a noisy channel through memory-access timing, so Argon2d is appropriate
//! only in contexts where side-channel attacks are not in the threat model
//! (e.g. proof-of-work). For password hashing prefer [`argon2id`](../argon2id).
//!
...
/// Argon2 G-round mixer — BLAKE2b round with an extra `2*trunc32(a)*trunc32(b)`
/// term and no SIGMA (message is zero). Operates on four 64-bit words.
fn g_b(v: &mut [u64], a: usize, b: usize, c: usize, d: usize) {
    let (mut va, mut vb, mut vc, mut vd) = (v[a], v[b], v[c], v[d]);

    va = va
        .wrapping_add(vb)
        .wrapping_add(2u64.wrapping_mul(va & MASK32).wrapping_mul(vb & MASK32));
    vd = rotr64(vd ^ va, 32);
    vc = vc
        .wrapping_add(vd)
        .wrapping_add(2u64.wrapping_mul(vc & MASK32).wrapping_mul(vd & MASK32));
    vb = rotr64(vb ^ vc, 24);
    va = va
        .wrapping_add(vb)
        .wrapping_add(2u64.wrapping_mul(va & MASK32).wrapping_mul(vb & MASK32));
...
/// Permutation P over 16 64-bit words (4 diagonal + 4 column G-rounds).
...
/// Compression G: R = X XOR Y; Q = P-row(R); Z = P-col(Q); out = R XOR Z.
...
/// Variable-length BLAKE2b extender H' (RFC 9106 §3.3).
...
/// RFC 9106 §3.4 reference index mapping from (J1, position) → block offset.
...
/// Fill one segment in Argon2d's data-dependent mode. J1/J2 always come from
/// the first u64 of the previously-computed block.
...
pub fn argon2d(
    ...
    h0_in.extend_from_slice(&le32(TYPE_D));
    ...
    for r in 0..t {
        for sl in 0..SYNC_POINTS {
            for lane in 0..p {
                fill_segment(&mut memory, r, lane, sl, q, sl_len, p);
            }
        }
    }
...
/// `argon2d` with lowercase-hex output.
pub fn argon2d_hex(
    password: &[u8],
    salt: &[u8],
    time_cost: u32,
    ...
    let tag = argon2d(password, salt, time_cost, memory_cost, parallelism, tag_length, opts)?;
...
mod tests {
    ...
    #[test]
    fn rfc9106_vector_5_1() {
        let password = filled(32, 0x01);
        let salt = filled(16, 0x02);
        let key = filled(8, 0x03);
        let ad = filled(12, 0x04);
        let expected = hex_decode("512b391b6f1162975371d30919734294f868e3be3984f3c1a13a4db9fabe4acb");
    ...
    fn hex_matches_bytes() {
        let tag = argon2d(b"password", b"saltsalt", 1, 8, 1, 32, &Options::default()).unwrap();
        let h = argon2d_hex(b"password", b"saltsalt", 1, 8, 1, 32, &Options::default()).unwrap();
        assert_eq!(h, tag.iter().map(|b| format!("{:02x}", b)).collect::<String>());
    }

    #[test]
    fn short_salt_rejected() {
        assert!(argon2d(b"pw", b"short", 1, 8, 1, 32, &Options::default()).is_err());
    }

    #[test]
    fn tag_length_too_small_rejected() {
        assert!(argon2d(b"pw", b"saltsalt", 1, 8, 1, 3, &Options::default()).is_err());
    }

    #[test]
    fn memory_below_minimum_rejected() {
        assert!(argon2d(b"pw", b"saltsalt", 1, 1, 1, 32, &Options::default()).is_err());
    }

    #[test]
    fn zero_time_cost_rejected() {
        assert!(argon2d(b"pw", b"saltsalt", 0, 8, 1, 32, &Options::default()).is_err());
    }

    #[test]
    fn zero_parallelism_rejected() {
        assert!(argon2d(b"pw", b"saltsalt", 1, 8, 0, 32, &Options::default()).is_err());
    }
    ...
    fn unsupported_version_rejected() {
        assert!(argon2d(
            ...
            1,
            32,
            &Options { key: None, associated_data: None, version: Some(0x10) }
        )
        .is_err());
    }
    ...
    #[test]
    fn deterministic() {
        let a = argon2d(b"password", b"somesalt", 1, 8, 1, 32, &Options::default()).unwrap();
        let b = argon2d(b"password", b"somesalt", 1, 8, 1, 32, &Options::default()).unwrap();
        assert_eq!(a, b);
        assert_eq!(a.len(), 32);
    }
    ...
    fn different_passwords_differ() {
        let a = argon2d(b"password1", b"somesalt", 1, 8, 1, 32, &Options::default()).unwrap();
        let b = argon2d(b"password2", b"somesalt", 1, 8, 1, 32, &Options::default()).unwrap();
        assert_ne!(a, b);
    }

    #[test]
    fn different_salts_differ() {
        let a = argon2d(b"password", b"saltsalt", 1, 8, 1, 32, &Options::default()).unwrap();
        let b = argon2d(b"password", b"saltsal2", 1, 8, 1, 32, &Options::default()).unwrap();
        assert_ne!(a, b);
    }
    ...
    fn key_binds() {
        ...
        let b = argon2d(
            ...
            1,
            32,
            &Options { key: Some(b"secret!!"), associated_data: None, version: None },
        )
        .unwrap();
        assert_ne!(a, b);
    }
    ...
    fn associated_data_binds() {
        ...
        let b = argon2d(
            ...
            1,
            32,
            &Options { key: None, associated_data: Some(b"ad"), version: None },
        )
        .unwrap();
        assert_ne!(a, b);
    }
    ...
    fn tag_length_variants() {
        for t in [4, 16, 32, 64, 65, 128] {
            let tag = argon2d(b"password", b"saltsalt", 1, 8, 1, t, &Options::default()).unwrap();
    ...
    fn multi_lane_parameters() {
        let tag = argon2d(&filled(32, 0x01), &filled(16, 0x02), 3, 32, 4, 32, &Options::default()).unwrap();
        assert_eq!(tag.len(), 32);
    }

    #[test]
    fn multiple_passes() {
        let t1 = argon2d(b"password", b"saltsalt", 1, 8, 1, 32, &Options::default()).unwrap();
        let t2 = argon2d(b"password", b"saltsalt", 2, 8, 1, 32, &Options::default()).unwrap();
        let t3 = argon2d(b"password", b"saltsalt", 3, 8, 1, 32, &Options::default()).unwrap();
        assert_ne!(t1, t2);
        assert_ne!(t2, t3);
        assert_ne!(t1, t3);
    }
