...
describe("the real corpus", () => {
  ...
  it("pins what nothing had measured", { timeout: 30_000 }, () => {
    ...
    expect(report.summary.atomsTaught).toBe(2890); // +4: ES-LEX-VOS-01, ES-CULTURE-VOSEO-02 (ES-C03-vos) // +3: HL-C98 splits AR-PRESENT-SINGULAR into 1SG/2SG/3SG // +83: vocabulary wave 5 (persian ch9-11, telugu ch35-40, malayalam ch35-40) // +7: HL-C88 slices 5-6 (Spanish)
    ...
    expect(report.summary.missedByWindow.R2).toBe(2009); // +1: ES-C02-concordancia (HL-C85). buenos dias/buenas tardes stop REQUIRING the agreement rule; it becomes a payoff lesson after the learner has used all three greetings. // +84: vocabulary wave 5, new pre-A1 nouns landing late in already-long chapters // +4: HL-C88 slices 5-6 // +1: HL-C112, same cause as R1 above
  });
  ...
  it("shows what a declared reading order was worth", () => {
    ...
    expect(spanish).toMatchObject({
      ...
      lessonCount: 337, // +4: HL-C98 // +3: HL-C97 adds the repair kit (no entiendo, mas despacio) at chapter 14 // +8 payoff lessons // +1 ES-C03-vos, +1 ES-C02-concordancia // +4: HL-C88 slices 5-6 // +1: HL-C88 slice 7 (ES-C09-ncia)
      ...
      atomsTaught: 531, // +3: HL-C98's per-cell atoms // +7: HL-C88 slices 5-6 // +2: HL-C88 slice 7 (ES-C09-ncia)
      atomsNeverRevisited: 72, // +2: HL-C98 // +4: HL-C88 slices 5-6 // +1: HL-C88 slice 7 (ES-C09-ncia)
    });
  });
