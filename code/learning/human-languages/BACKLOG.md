...
and continues below the baseline through the leftward bowl without a lift. The
next starter entry is م; HL-C09AB queues its source-backed audit from Chapter 3,
*Te, mīm, jīm, che, and more diacritics*.
...
| HL-C09 | Queued — 27 of 228 verified | Expand `DUCTUS` to cover the ten scripts with prose stroke-order entries. | Add cited, font-checked ductus and verified pen-lift metadata for the remaining 201 entries measured by HL-C19; each passes the on-ink, join-tolerance, coverage, citation, and source-agreement invariants. |
...
| HL-C09AB | Queued — next | Locate and verify independent Urdu م in *Zer o Zabar*'s source-backed Chapter 3, *Te, mīm, jīm, che, and more diacritics*. | Audit the independent animation's round head and descending tail, reconcile its pen-down motion with the Noto Naskh fallback, then author the smallest next path with source, lifts, font, and filmstrip agreement. |
