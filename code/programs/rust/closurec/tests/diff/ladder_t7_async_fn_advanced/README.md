# ladder_t7_async_fn_advanced

Rung 7.async_fn of the CCR-066 differential complexity ladder, at `ADVANCED`.

## What this rung is for

Exercises an async function declaration.
...
java -Duser.language=en -Duser.country=US -Duser.timezone=UTC -Dfile.encoding=UTF-8 \
  ...
  --compilation_level ADVANCED_OPTIMIZATIONS --js tests/diff/ladder_t7_async_fn_advanced/input/a.js
```
...
closurec refuses valid JavaScript that upstream compiles: an async function declaration fails at the typed-AST bridge. This is a front-end capability gap, not an optimizer one. Note it fails only on the typed path — WHITESPACE_ONLY passes, because that path never parses.
