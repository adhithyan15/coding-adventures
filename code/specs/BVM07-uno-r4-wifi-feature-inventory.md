...
| WiFi | WiFiS3 library through onboard network module | descriptor metadata, network capability strings, first TCP/UDP socket bytecode tranches, bounded TCP/UDP socket-control queries, bounded WiFi association control, bounded DNS resolution, resolver server policy, and first DNS query-message payload implemented | `transport.wifi`, `network.ipv4`, `network.tcp`, `network.udp`, `network.dns`, `network.tcp.open`, `network.tcp.write`, `network.tcp.read`, `network.tcp.close`, `network.tcp.connected`, `network.tcp.available`, `network.udp.open`, `network.udp.write`, `network.udp.read`, `network.udp.available`, `network.udp.close`, `network.wifi.associate`, `network.wifi.disconnect`, `network.wifi.status`, `network.dns.resolve`, `network.dns.set_server`, `network.dns.query` |
...
5. Add UART, CAN, RTC, watchdog, EEPROM/store, and WiFi/network
   ...
   constructing a bounded recursive A-record query in the Rust runtime, leaving
   UDP transport and response parsing for later follow-ups.
   ...
   same bounded storage substrate. Higher-level KV storage, DNS response
   payloads, DNS transport, and deeper socket controls remain later capability
   tranches.
