...
import { JvmBackendError, lowerIrToJvmClassFile, writeClassFile, } from "../src/index.js";
...
function programWithCallAndStaticData() {
    const program = new IrProgram("_start");
...
describe("ir-to-jvm-class-file", () => {
    ...
    it("rejects invalid class names", () => {
        expect(() => lowerIrToJvmClassFile(simpleProgram(), { className: ".Bad" })).toThrow(JvmBackendError);
    });
    ...
    it("rejects malformed operands that violate the backend contract", () => {
        ...
        expect(() => lowerIrToJvmClassFile(badImmediate, { className: "BadImmediate" })).toThrow(JvmBackendError);
        ...
        expect(() => lowerIrToJvmClassFile(badLabel, { className: "BadLabel" })).toThrow(JvmBackendError);
    });
    ...
    it("rejects output roots with symlinked ancestors", () => {
        ...
        try {
            ...
            const artifact = lowerIrToJvmClassFile(simpleProgram(), { className: "demo.Example" });
            expect(() => writeClassFile(artifact, join(tempdir, "linked-root", "nested"))).toThrow(JvmBackendError);
        }
        finally {
            rmSync(tempdir, { recursive: true, force: true });
            rmSync(sink, { recursive: true, force: true });
...
//# sourceMappingURL=ir-to-jvm-class-file.test.js.map
