//! Dev helper: lower a Ruby (or Twig) snippet to SIR and print the emitted Ruby.
//!
//! Usage:
//!   cargo run -p semantic-ir-to-ruby --example dump_rb -- ruby 'puts 2 + 3 * 4'
//!   cargo run -p semantic-ir-to-ruby --example dump_rb -- twig '(print (+ 2 3))'
...
fn main() {
    ...
    match semantic_ir_to_ruby::compile(&module) {
        Ok(artifact) => print!("{}", artifact.source),
        Err(e) => {
            eprintln!("Ruby backend rejected the module: {e}");
            std::process::exit(1);
        }
    }
}
