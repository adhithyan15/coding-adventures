...
//   TC-12: Empty archive
//
// This numbering (TC-10 = Unicode, TC-11 = Nested paths, TC-12 = Empty
// archive) matches every other language's port in this repo. The spec's own
// TC-10 ("Cross-compatibility with system ZIP tools") is explicitly called
// out there as "Manual or subprocess-based" and is not automated in any of
// the Python/Go/Ruby/TypeScript/Rust/Elixir/Lua/Perl/Swift ports either.
//
// Beyond the 12 spec cases, a handful of "Hardening" tests below cover
// attacker-controlled size/offset fields in the Central Directory (integer
// overflow on narrowing an oversized uint32 to `int`) and an oversized entry
// name on write — see the comment above `findCdEntryOffset` for detail.
...
// ── Hardening: attacker-controlled size/offset fields ─────────────────────
//
// The Central Directory's Compressed_Size, Uncompressed_Size and
// Relative_Offset_Of_Local_Header fields are 32-bit values taken straight
// from archive bytes. A naive `int` narrowing of an oversized uint32 (e.g.
// 0x80000000u or 0xFFFFFFFFu) wraps negative, which can defeat an
// `offset > data.Length` bounds check written in `int` arithmetic (a
// negative value always compares less than a small positive length). These
// tests corrupt each field in turn and assert the reader still fails
// closed with `InvalidDataException` rather than silently reading garbage,
// wrapping into an unrelated exception type, or crashing.

// Helper: scan archive bytes for the Central Directory entry for `entryName`
// and return the byte offset where its "PK\x01\x02" signature starts.
let private findCdEntryOffset (archive: byte[]) (entryName: string) : int =
    let cdMagic = [| 0x50uy; 0x4Buy; 0x01uy; 0x02uy |]
    let name = Encoding.UTF8.GetBytes(entryName)
...
// Overwrite a little-endian uint32 field in place.
let private pokeU32 (archive: byte[]) (offset: int) (value: uint32) =
    archive[offset]     <- byte (value &&& 0xFFu)
    archive[offset + 1] <- byte ((value >>> 8) &&& 0xFFu)
    archive[offset + 2] <- byte ((value >>> 16) &&& 0xFFu)
    archive[offset + 3] <- byte ((value >>> 24) &&& 0xFFu)

[<Fact>]
let ``Hardening CompressedSize overflow rejected`` () =
    let archive = ZipArchive.zip (seq { { Name = "f.txt"; Data = Encoding.UTF8.GetBytes("test data") } })
    let corrupted = Array.copy archive
    let cdOffset = findCdEntryOffset corrupted "f.txt"

    // Compressed_Size is at offset +20 within the Central Directory Header.
    // 0x80000000u wraps negative when naively narrowed to `int`.
    pokeU32 corrupted (cdOffset + 20) 0x80000000u

    let reader = ZipReader(corrupted)
    Assert.Throws<InvalidDataException>(fun () -> reader.Read("f.txt") |> ignore) |> ignore

[<Fact>]
let ``Hardening LocalOffset overflow rejected`` () =
    let archive = ZipArchive.zip (seq { { Name = "f.txt"; Data = Encoding.UTF8.GetBytes("test data") } })
    let corrupted = Array.copy archive
    let cdOffset = findCdEntryOffset corrupted "f.txt"

    // Relative_Offset_Of_Local_Header is at offset +42 within the CD Header.
    pokeU32 corrupted (cdOffset + 42) 0xFFFFFFFFu
...
let ``Hardening oversized entry name rejected`` () =
    // File_Name_Length is a 16-bit field (max 65535 bytes). A name that
    // encodes to more UTF-8 bytes than that must be rejected outright rather
    // than silently truncating the length field on write.
