...
describe("Punjabi task-shape inventories", () => {
  ...
  it("makes the project-defined A1 destination an exact four-paper contract", () => {
    const inventory = loadTaskShapeInventory("punjabi", "A1");

    expect(inventory.target).toEqual({
      name: "Coding Adventures Punjabi A1 Assessment — project-defined equivalent",
      basis: "project-defined",
    });
    expect(inventory.administration).toMatchObject({
      writtenMinutes: 58,
    ...
    expect(listening?.parts.every((part) =>
      part.promptModes.includes("recorded Punjabi at 90-110 words per minute") && part.replayCount === 2,
    )).toBe(true);
