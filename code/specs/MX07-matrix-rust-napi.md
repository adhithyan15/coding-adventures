...
| 3 | `package.json` (npm `build` + `smoke` scripts) + `.github/workflows/matrix-rust-napi.yml` (path-filtered Actions workflow). Builds `matrix_rust_napi.node` on `ubuntu-latest` (linux-x64-gnu) and `macos-latest` (darwin-arm64); confirms it loads and all four exports (`graphRoundTripJson`, `runGraphOnCpu`, `Graph`, `Runtime`) are present.  No publish step yet — that's a separately-coordinated follow-up. | shipped (#3546) |
| 4 | TypeScript wrapper package `code/packages/typescript/matrix-rust-napi/` — typed re-exports of the four addon entry points (lazy `createRequire` of the `.node` artifact, `Proxy`-shimmed `Graph` and `Runtime` classes).  `tests/smoke.test.ts` vitest suite drives end-to-end Add + 2×2 MatMul with `Buffer[]` I/O plus error paths (wrong arity, byte length, non-Buffer inputs).  CRITICAL fix in same PR: matrix-rust-napi's class constructors were stored as stale `napi_value` instead of persistent `napi_ref` — both `Graph.fromJson` and `Runtime.create` returned undefined because `napi_new_instance` got an invalidated local handle.  Fixed by switching to `napi_ref` + `napi_create_reference` / `napi_get_reference_value`; `lessons.md` updated. | **this PR** |
