...
% canonical-source-hash: fnv1a64:4ff3b93c801c68e5
% canonical-lessons: AR-C40-limadha, AR-C40-lianna, AR-C40-anna, AR-C40-hinama, AR-R40-why-that-when, AR-R40-the-plate-and-the-spoon
...
\end{tcolorbox}
\section[milʿaqa · ilā l-ghad · tuṣbiḥ ʿalā khayr]{Words that carry their own reason}

\label{lesson:AR-R40-the-plate-and-the-spoon}



\begin{quote}
Name the plate and the spoon in Arabic and spell each one aloud. Then use this chapter's \emph{why} and \emph{because} to explain where the spoon got its name.
\end{quote}
...
\toprule
\textbf{word, spelled out} & \textbf{root} & \textbf{what the root means} \\
\midrule
\textbf{\ar{ملعقة}} \emph{milʿaqa} — \emph{mīm · lām · ʿayn · qāf · \ar{ة}} & \textbf{\ar{ل}-\ar{ع}-\ar{ق}} & he licked, he lapped \\
\textbf{\ar{طبق}} \emph{ṭabaq} — \emph{ṭāʾ · bāʾ · qāf} & \textbf{\ar{ط}-\ar{ب}-\ar{ق}} & to cover, to fit over \\
\bottomrule
\end{tabularx}

So a spoon is \textbf{the licking-thing} and a plate is \textbf{the covering-thing}. Nothing in either word had to be learned as an arbitrary noise: the root says what the object is for, and the shape says it is an object.

\textbf{\ar{أطباق}} is the plural of \textbf{\ar{طبق}}, in the same broken shape as \textbf{\ar{أكواب}}. One broken plural is a surprise; two in the same shape is a \textbf{pattern}, and a pattern is something you can use on the next word.
\end{culture}

\subsection*{Your turn: because, and when}
Say these aloud:

\begin{itemize}
\raggedright
  \item ask why the spoon is called that, then answer — \emph{limādhā? … li-anna …}
  \item until tomorrow — \emph{ilā l-ghad}
  \item the other farewell built on the same word — \emph{ilā l-liqāʾ}
\end{itemize}

\textbf{\ar{إلى} \ar{الغد}} is the same \emph{ilā}, \textbf{until}, with a new word in the slot. And \textbf{\ar{الغد}} has its reason inside it too: the root \textbf{gh-d-w} means \textbf{to set out at dawn}, so \emph{tomorrow} in Arabic is named after the hour you begin it.

\begin{culture}
\textbf{\ar{تصبح} \ar{على} \ar{خير}} is the everyday \textbf{good night}, and it says nothing about the night. Literally: \textbf{may you wake up into goodness}.

English, French, German and Spanish all wish the sleeper a quiet \emph{night}. Arabic looks past the night to the far side of it — \emph{tuṣbiḥ} carries the morning root, and \textbf{\ar{خير}} is the goodness the wellbeing answer already uses. The phrase is built entirely from two things met long before bedtime came up.
\end{culture}

\subsection*{How to answer: the word that erases what was owed}
\textbf{\ar{عفوا}} \emph{ʿafwan} answers thanks as \textbf{you are welcome}, and the same word, on a different tone, means \textbf{excuse me} and \textbf{pardon}. The reason is the root \textbf{ʿ-f-w}, to \textbf{erase}: a debt, an intrusion or a mistake, all wiped away by one word.

\begin{tcolorbox}[breakable,colback=teal!4,colframe=teal!35!black,title={Before you move on}]
Say what the root under \textbf{\ar{ملعقة}} means. (\textbf{He licked}.) Say what the root under \textbf{\ar{الغد}} means. (\textbf{To set out at dawn}.) Give the literal wish inside \textbf{\ar{تصبح} \ar{على} \ar{خير}}. (\textbf{That you wake into goodness}.) And give the root under \textbf{\ar{عفوا}}. (\textbf{ʿ-f-w}, to erase.)
\end{tcolorbox}
