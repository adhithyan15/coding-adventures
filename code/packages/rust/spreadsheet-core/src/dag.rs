...
impl<'a> TarjanState<'a> {
    ...
    fn strongconnect(&mut self, v: Node) {
        ...
        // We treat out-edges as "v depends on dep", so we visit deps
        // from v. The resulting SCCs correctly include any cycles.
        let deps: Vec<Node> = self.graph.dependencies(v).to_vec();
        for w in deps {
