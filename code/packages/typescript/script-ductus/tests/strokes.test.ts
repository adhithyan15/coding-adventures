...
describe("handwriting ductus", () => {
  ...
  it("Japanese し descends and sweeps upward right without lifting", () => {
    ...
    expect(JAPANESE_SHI.strokes[0].segments.map((segment) => segment.label)).toEqual([
  ...
  it("Japanese small っ scales つ's one-run movement to its own glyph", () => {
    ...
    expect(JAPANESE_SMALL_TSU.strokes[0].segments.map((segment) => segment.label)).toEqual([
    ...
    expect(JAPANESE_SMALL_TSU.source.citation).toMatch(/Sirgazil.*つ.*24 frames.*Unicode Standard 17\.0.*U\+3063/i);
    expect(JAPANESE_SMALL_TSU.source.variation).toMatch(/one uninterrupted run.*small tsu.*scaling.*explicit/i);
  });
  ...
  it("Japanese も draws its bowl before two lifted left-to-right bars", () => {
    ...
    expect(JAPANESE_MO.strokes.map((stroke) => stroke.segments.map((segment) => segment.label))).toEqual([
  ...
  it("Telugu ఇ keeps its three source-verified components in separate pen-down runs", () => {
    ...
    expect(TELUGU_I.strokes.map((stroke) => stroke.segments.map((segment) => segment.label))).toEqual([
  ...
  it("Telugu ఎ groups three source-verified movements into two pen-down runs", () => {
    ...
    expect(TELUGU_E.strokes.map((stroke) => stroke.segments.map((segment) => segment.label))).toEqual([
      [
  ...
  it("Malayalam ഇ keeps all four animated movements in one run", () => {
    ...
    expect(MALAYALAM_I.strokes[0].segments.map((segment) => segment.label)).toEqual([
  ...
  it("Malayalam chillu ൾ keeps all four animated movements in one run", () => {
    ...
    expect(MALAYALAM_CHILLU_LL.strokes[0].segments.map((segment) => segment.label)).toEqual([
  ...
  it("Malayalam chillu ർ keeps all three animated movements in one run", () => {
    ...
    expect(MALAYALAM_CHILLU_RR.strokes[0].segments.map((segment) => segment.label)).toEqual([
  ...
  it("Persian and Urdu independent ز keep scoped body-first sources", () => {
    for (const zay of [PERSIAN_ZAY, URDU_ZE]) {
      ...
      expect(zay.strokes.map((stroke) => stroke.segments.length)).toEqual([2, 1]);
      expect(zay.strokes.map((stroke) => stroke.segments.map((segment) => segment.path))).toEqual(
        ARABIC_ZAY.strokes.map((stroke) => stroke.segments.map((segment) => segment.path)),
      );
    }
    expect(PERSIAN_ZAY.source.citation).toMatch(/Persian Online.*ز.*01:13–01:16/i);
    expect(URDU_ZE.source.citation).toMatch(/Zer o Zabar.*independent ز.*Ze instructions/i);
  ...
  it("Chinese 汉 preserves all five pinned source medians and their lift boundaries", () => {
    ...
    expect(src.citation).toMatch(/Hanzi Writer Data 汉\.json.*medians 1–5.*snapshot 68d10a4/i);
    expect(src.variation).toMatch(
    ...
    expect(CHINESE_HAN.strokes.map((stroke) => penPath(stroke).length)).toEqual([3, 3, 5, 13, 7]);
  ...
  it("Chinese 语 preserves the 讠-before-五-before-口 source sequence", () => {
    ...
    expect(src.citation).toMatch(/Hanzi Writer Data 语\.json.*medians 1–9.*snapshot 68d10a4/i);
    expect(src.variation).toMatch(
    ...
    expect(CHINESE_LANGUAGE.strokes.map((stroke) => penPath(stroke).length)).toEqual([
      3, 10, 5, 4, 7, 6, 3, 7, 5,
    ]);
  ...
  it("Chinese 文 preserves its four separately started source medians", () => {
    ...
    expect(src.citation).toMatch(/Hanzi Writer Data 文\.json.*medians 1–4.*snapshot 68d10a4/i);
    expect(src.variation).toMatch(
    ...
    expect(CHINESE_WRITING.strokes.map((stroke) => penPath(stroke).length)).toEqual([3, 6, 12, 7]);
  ...
  it("Chinese 国 closes the frame only after all five inner 玉 medians", () => {
    ...
    expect(src.citation).toMatch(/Hanzi Writer Data 国\.json.*medians 1–8.*snapshot 68d10a4/i);
    expect(src.variation).toMatch(
    ...
    expect(CHINESE_COUNTRY.strokes.map((stroke) => penPath(stroke).length)).toEqual([
      6, 12, 6, 4, 5, 5, 3, 5,
    ]);
