...
pub struct EmitOptions {
    ...
    /// inlining the component fragment) + `README.md` alongside
    /// the component `.html` file. Default `false`.
...
/// Build the two standalone-shell side files for a single
/// component. No validation surface beyond the upstream
...
fn build_html_project_files(component_name: &str, fragment: &str) -> ProjectFiles {
    ProjectFiles {
...
fn build_index_html(component_name: &str, fragment: &str) -> String {
    ...
    shell.push_str("</head>\n");
    shell.push_str("<body>\n");
    shell.push_str(&format!("  <section data-component=\"{component_name}\">\n"));
...
mod tests {
    ...
    fn host_table_with_dir_rtl_keyword_emits_dir_attr_on_table() {
        let table = node_with_props_and_children(
            "HostTable",
            vec![prop_keyword("dir", "rtl")],
            vec![],
        );
        let l = layout("T", table);
        let out = from_pipeline(&component("T", vec![]), &l, &empty_style("T"))
    ...
    fn host_table_body_with_dir_keyword_does_not_emit_dir_attr() {
        ...
        let out = from_pipeline(&component("T", vec![]), &layout("T", body), &empty_style("T"))
            .unwrap()
            .output;
    ...
    fn ui32_index_html_inlines_component_fragment() {
        ...
        assert!(
            proj.index_html.contains("<section data-component=\"Demo\">"),
    ...
    /// the two UI32 spec §2.2 HTML-row files. Tripwire forces a
    /// spec amendment if a field is added.
    #[test]
    fn ui32_project_files_struct_exposes_only_spec_22_html_files() {
        let m = component("X", vec![]);
        let l = layout("X", node("Box"));
        let s = empty_style("X");
        let mut opts = EmitOptions::default();
