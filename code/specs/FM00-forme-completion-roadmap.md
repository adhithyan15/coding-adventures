...
| 28 | FM-B039 | active | Implement bounded lazy stream fan-out | Depends on FM-B038. One upstream iterator feeds every statically known consumer through ordered 64-item windows; a slow branch backpressures the producer, consumer cancellation detaches safely, upstream errors reach every branch, and instrumentation tests bound both pulls and retained values. |
| 29 | FM-B040 | ready | Implement the pipeline-wide concurrent scheduler | Depends on FM-B039. A stable ready queue and shared permit pool enforce `settings.maxConcurrency` across stage and per-item invocations, suspend permits while stream consumers wait on upstream to avoid `maxConcurrency: 1` deadlocks, preserve declaration/input order in outputs and summaries, and deterministically cancel and dispose in-flight work. |
