...
use language_lexer::create_xml_lexer;
...
pub fn tokenize(source: &str) -> Result<String, JsValue> {
    let mut lexer = create_xml_lexer(source);
    let tokens = lexer
        .tokenize()
        .map_err(|e| to_js_error(format!("XML tokenization failed: {e}")))?;
...
mod tests {
    ...
    fn wrapper_tokenizes_to_json() {
        let json = tokenize("<p>hi</p>").unwrap();
