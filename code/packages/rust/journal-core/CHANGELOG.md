...
- **Hardening from the pre-push security review**: ids must be 1–64 bytes of
  printable ASCII, checked before every lookup so no error echoes an unchecked id (`InvalidId`); at most 1,000
  ...
  overflow; names and tags also refuse zero-width, bidi-override, soft-hyphen and
