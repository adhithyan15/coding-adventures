...
use language_parser::create_algol_parser;
...
pub fn parse(source: &str) -> Result<String, JsValue> {
    let mut parser = create_algol_parser(source);
    let ast = parser
        .parse()
        .map_err(|e| to_js_error(format!("ALGOL 60 parse failed: {e}")))?;
...
mod tests {
    ...
    fn wrapper_parses_to_json() {
        let json = parse("begin integer x; x := 42 end").unwrap();
