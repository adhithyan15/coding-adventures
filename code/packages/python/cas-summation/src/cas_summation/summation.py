...
def _g_vanishes_at_infinity(g: IRNode, k: IRSymbol) -> bool:
    ...
    if s4l6p_x2 is not None:
        ...
        if den_deg_s4l6 is not None:
            ...
            return True
    # Phase 88: ``Mul(Sqrt(P1), Sqrt(P2), Sqrt(P3), Sqrt(P4), Sqrt(P5), Log(h1), ..., Log(h6),
    #           polynomial..., bounded...)`` numerator.
    # Five Sqrt factors + six Log factors; log⁶ sub-polynomial → 0.
    # effective_x2 = sqrt1_x2 + sqrt2_x2 + sqrt3_x2 + sqrt4_x2 + sqrt5_x2 + 2·poly_deg.
    # Vanishes when ``2·den_deg > effective_x2`` (polynomial) or
    # non-polynomial diverging denominator.
    s5l6p_x2 = _five_sqrt_six_log_poly_effective_x2(num, k)
    if s5l6p_x2 is not None:
        den_deg_s5l6 = _polynomial_degree_in_k(den, k)
        if den_deg_s5l6 is not None:
            if 2 * den_deg_s5l6 > s5l6p_x2:
                return True
        elif _h_diverges_at_infinity(den, k):
            return True
...
def _five_sqrt_six_log_poly_effective_x2(node: IRNode, k: IRSymbol) -> int | None:
    """Return ``sqrt1_x2 + sqrt2_x2 + sqrt3_x2 + sqrt4_x2 + sqrt5_x2 + 2·poly_deg`` when
    ``node`` is a ``Mul`` with **exactly five** ``Sqrt(positive-leading polynomial)`` factors,
    **exactly six** ``Log(diverging-in-k)`` factors, any polynomial factors, and any
    bounded factors; ``None`` otherwise.
    ...
    Effective growth:
    ``sqrt(k^a)·sqrt(k^b)·sqrt(k^c)·sqrt(k^d)·sqrt(k^e)·log(k)⁶·k^m ≈ k^{(a+b+c+d+e)/2+m}``.
    ``log⁶(k)`` is sub-polynomial (``o(k^ε)``), contributing 0.
    ...
    | Input                                                             | Return              |
    +===================================================================+=====================+
    | ``Mul(Sqrt(k)×5, Log(k)×6)``                                     | ``5``               |
    | ``Mul(Sqrt(k²)×5, Log(k)×6)``                                    | ``10``              |
    | ``Mul(Sqrt(k)×5, Log(k)×6, k)``                                  | ``7``               |
    | ``Mul(Sqrt(k)×4, Log(k)×6)``                                     | None (4 Sqrts)      |
    | ``Mul(Sqrt(k)×6, Log(k)×6)``                                     | None (6 Sqrts)      |
    | ``Mul(Sqrt(k)×5, Log(k)×5)``                                     | None (5 Logs)       |
    +-------------------------------------------------------------------+---------------------+

    Algorithm:
      1. Require ``node = Mul(...)``.
      2. For each factor:
         - ``Sqrt(P)`` → record ×2 degree; bail after 5.
         - ``Log(diverging)`` → count; bail after 6.
         - Polynomial in ``k`` → accumulate degree.
         - Bounded → accept silently.
         - Anything else → return ``None``.
      3. Require exactly 5 Sqrt AND exactly 6 Log factors.
      4. Return ``sum(sqrt_degs_x2) + 2 * poly_deg_sum``.
    """
    if not isinstance(node, IRApply) or node.head != MUL:
        return None
    ...
    for arg in node.args:
        ...
        if _is_bounded_in_k(arg, k):
            continue
        return None
    if len(sqrt_degs_x2) != 5 or log_count != 6:
        return None
    return sum(sqrt_degs_x2) + 2 * poly_deg_sum
