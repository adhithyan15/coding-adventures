---
schema_version: 2
id: MW-C10-bajar
spine_node: SPINE-POLITE-REQUEST-REPAIR
sequence: 950
chapter: 14
type: word
headword: बाजार
romanization: bājār
gloss: market, written after ja is isolated
concept_tag: MW-PLACE-MARKET-WRITTEN
prerequisites: [MW-W10-ja]
sounds: [voiced-b, long-aa, voiced-j]
roots: []
etymology_hook: Familiar ब, ा, and र now frame the newly isolated ज.
duration:
  max_seconds: 160
requires:
  knowledge: [MW-LEX-BAJAR-01, MW-SCRIPT-BA-01, MW-SCRIPT-JA-01, MW-SCRIPT-AA-MATRA-01, MW-SCRIPT-RA-01]
introduces:
  knowledge: [MW-SCRIPT-BAJAR-01]
practises:
  knowledge: [MW-LEX-BAJAR-01, MW-SCRIPT-BAJAR-01, MW-SCRIPT-BA-01, MW-SCRIPT-JA-01, MW-SCRIPT-AA-MATRA-01, MW-SCRIPT-RA-01, MW-LEX-BACHCHA-01, MW-RESPONSE-HAAN-SAA-POLITE-01, MW-PERFORMANCE-FAMILY-SEVEN-FOUR-SKILL-01]
skills: [listening, speaking, reading, writing]
modes: [interpretive, presentational]
strands: [meaning-input, meaning-output, language-focus]
register: neutral
variety: central-marwari-devanagari
reviews_of: [MW-C10-hear-bajar, MW-W10-ja, MW-C09-bachcha]
---

# बाजार — market on the page

## Warm-up
<!-- hl-knowledge: introduces=[MW-SCRIPT-BAJAR-01]; assesses=[MW-LEX-BAJAR-01, MW-SCRIPT-BA-01, MW-SCRIPT-JA-01, MW-SCRIPT-AA-MATRA-01, MW-SCRIPT-RA-01, MW-LEX-BACHCHA-01, MW-RESPONSE-HAAN-SAA-POLITE-01, MW-PERFORMANCE-FAMILY-SEVEN-FOUR-SKILL-01] -->

[PAUSE 20s] Say market and child. Give polite yes, recall the seven-label
payoff, then write **ब**, **ज**, **ा**, and **र**.

## You'll want to know
<!-- hl-knowledge: introduces=[]; assesses=[MW-LEX-BAJAR-01, MW-SCRIPT-BAJAR-01] -->

> **बा** + **जार** → **बाजार** — *bājār* — market

## Guided Practice
<!-- hl-knowledge: introduces=[]; assesses=[MW-SCRIPT-BAJAR-01] -->
<!-- hl-writing-stage: delayed-copy -->

Look, cover, wait five seconds, and write the word. Check both long-vowel marks.

## Wrap-up Recall
<!-- hl-knowledge: introduces=[]; assesses=[MW-LEX-BAJAR-01, MW-SCRIPT-BAJAR-01] -->
<!-- hl-activity: {"id":"MW-C10-bajar-dictation","kind":"text","assesses":["MW-LEX-BAJAR-01","MW-SCRIPT-BAJAR-01"],"prompt":"From bājār, write the place word.","answer":"बाजार","accepted":["बाजार market"],"feedback":{"correct":"Right: बाजार is market.","incorrect":"Build two chunks: बा + जार."},"response_seconds":12} -->

Source: [Marwari Pathshala, Lesson 5](https://www.marwaripathshala.com/marwari-lesson-5-english).
