...
Argon2i generates its reference indices from a *data-independent*
pseudo-random stream (a block of (J1, J2) pairs produced by running
``G`` twice with a counter-seeded input block).  The memory access
pattern is therefore the same for any secret, which defeats
side-channel observers — at the cost of being the easiest variant for
GPUs and ASICs to parallelise.  For password hashing prefer Argon2id.
...
(:func:`argon2i`, :func:`argon2i_hex`), both one-shot.  There is no
...
# ---------------------------------------------------------------------------
# Argon2i fill — data-independent addressing via the address-block stream.
# ---------------------------------------------------------------------------
...
def _fill_segment(
    ...
    """Fill one (pass, slice, lane) segment using Argon2i addressing.

    Each address block contains 128 (J1, J2) pairs.  The stream is
    produced by applying ``G`` twice to a counter-seeded input block
    that encodes (pass, lane, slice, m', t, type, counter, 0, ...).
    """
    ...
    # Pass 0 slice 0 needs the address block primed before the loop
    # because the i==0 boundary check inside the loop won't fire.
    if starting_c != 0:
        _next_addresses()
    ...
    *,
    ...
    for r in range(t):
        for sl in range(SYNC_POINTS):
            for lane in range(p):
                _fill_segment(memory, r, lane, sl, q, SL, p, m_prime, t)
