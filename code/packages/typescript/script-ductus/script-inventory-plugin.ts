...
function safeInventoryPath(curriculumRoot: string, inventory: string): string {
  if (!INVENTORY_IDS.has(inventory)) {
    throw new Error(`script-ductus-inventories: unknown inventory ${JSON.stringify(inventory)}`);
  }
  ...
  if (inside === ".." || inside.startsWith("../") || isAbsolute(inside)) {
    throw new Error(`script-ductus-inventories: inventory parent escapes curriculum root: ${path}`);
  }
  return path;
}
...
export function scriptInventoryModuleIdsForShardPath(
  ...
  const match = /^data\/scripts\/([^/]+)\.d\/(?:[^/]+\/)?[^/]+\.json$/.exec(inside);
...
export function scriptInventoryPlugin(options: ScriptInventoryPluginOptions): Plugin {
  return {
    name: "script-ductus-inventories",
    configureServer(server) {
      const scriptsRoot = dirname(safeInventoryPath(options.curriculumRoot, INVENTORIES[0].id));
      server.watcher.add(scriptsRoot);
      server.watcher.on("all", (event, changedPath) => {
        const ids = scriptInventoryModuleIdsForShardPath(changedPath, options.curriculumRoot);
        for (const id of ids) {
    ...
    load(id) {
      return loadScriptInventoryModule(id, options.curriculumRoot, (path) => this.addWatchFile(path));
    },
  };
}
