# ladder_t6_class_simple

Rung 6.class of the CCR-066 differential complexity ladder, at `SIMPLE`.

## What this rung is for

Exercises a class with a constructor and a method after it.
...
java -Duser.language=en -Duser.country=US -Duser.timezone=UTC -Dfile.encoding=UTF-8 \
  ...
  --compilation_level SIMPLE_OPTIMIZATIONS --js tests/diff/ladder_t6_class_simple/input/a.js
```
...
closurec refuses valid JavaScript that upstream compiles: a class method following a constructor fails at the parse stage. This is a front-end capability gap, not an optimizer one. Note it fails only on the typed path — WHITESPACE_ONLY passes, because that path never parses.
