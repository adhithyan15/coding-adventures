namespace CodingAdventures.Argon2d.FSharp
...
/// Optional Argon2d inputs from RFC 9106.
type Argon2dOptions =
    {
        Key: byte array
...
/// Pure F# Argon2d password hashing as specified by RFC 9106.
...
module Argon2d =
    ...
    let private fillSegment
        ...
        for index = startingColumn to segmentLength - 1 do
            ...
            let pseudoRandom = memory.[lane].[previousColumn].[0]
            ...
            let newBlock = compress memory.[lane].[previousColumn] memory.[referenceLane].[referenceColumn]
    ...
    let derive
        ...
        for pass = 0 to timeCost - 1 do
            for slice = 0 to SyncPoints - 1 do
                for lane = 0 to parallelism - 1 do
                    fillSegment memory pass lane slice laneLength segmentLength parallelism
    ...
    /// Compute an Argon2d tag with empty key and associated data.
    let deriveDefault password salt timeCost memoryCost parallelism tagLength =
        derive password salt timeCost memoryCost parallelism tagLength Argon2dOptions.Default

    /// Compute an Argon2d tag and return lowercase hexadecimal.
    ...
    /// Compute a default-option Argon2d tag and return lowercase hexadecimal.
    let deriveHexDefault password salt timeCost memoryCost parallelism tagLength =
        deriveHex password salt timeCost memoryCost parallelism tagLength Argon2dOptions.Default
