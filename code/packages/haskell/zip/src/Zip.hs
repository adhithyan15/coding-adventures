...
--   2. Otherwise, run LZ77 match-finding via LZSS.encodeWith (window=32768,
...
deflateCompress bs
    ...
    | otherwise =
        ...
        -- These constants always satisfy 'LZSS.encodeWith's parameter bounds
        -- (window <= 65535, 0 < minMatch <= maxMatch <= 255), so the 'Left'
        -- branch is unreachable; we still handle it defensively rather than
        -- pattern-matching irrefutably.
        let tokens = case encodeWith 32768 255 3 bs of
                Right ts  -> ts
                Left err  -> error ("zip: internal LZSS parameter error: " ++ err)
            bw0    = newBitWriter
...
encodeToken bw (Match off len) =
    ...
    -- 'LZSS.Token's Match carries 'matchOffset :: Word16' and
    -- 'matchLength :: Word8'; the length/distance tables index by 'Int', so
    -- widen both before consulting them.
    let lenInt = fromIntegral len :: Int
        offInt = fromIntegral off :: Int
        (lSym, lBase, lExtra) = encodeLength lenInt
        (lCode, lBits)        = fixedLLEncode lSym
        bw1 = writeHuffman bw lCode lBits
        bw2 = if lExtra > 0
                  then writeLsb bw1 (fromIntegral (lenInt - lBase)) lExtra
                  else bw1
        (dCode, dBase, dExtra) = encodeDistance offInt
        -- Distance codes are 5-bit, written with bit reversal (Huffman style).
        bw3 = writeHuffman bw2 (fromIntegral dCode) 5
        bw4 = if dExtra > 0
                  then writeLsb bw3 (fromIntegral (offInt - dBase)) dExtra
                  else bw3
    in bw4
