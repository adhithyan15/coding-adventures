...
- **E4 — Strings.** ◑ *IR + reference VM slice landed; see
  **[`lang-full-e4-strings.md`](lang-full-e4-strings.md)** for the full backend plan.*
  ...
  has a VM/JIT/JVM/CLR matrix proof. JVM/CLR own the literal-output shape (`ldc`/`ldstr` +
  `PrintStream.print(String)`/`Console.Write(string)`); richer string ops and the other
  managed/static backend lowerings remain. Unlocks BASIC strings + string `PRINT` (BA4),
...
- ◑ **BA4** — string literal `PRINT` runs on VM/JIT/JVM/CLR via **E4**; string variables and most
