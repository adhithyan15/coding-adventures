 ...
 */
function sixtyThreeLogPolyEffectiveDeg(node: IRNode, k: IRNode): number | undefined {
  if (node.kind !== "apply" || !equals(node.head, MUL)) return undefined;
  let logCount = 0;
  let polyDeg = 0;
  for (const arg of node.args) {
    ...
    if (isLogOfDivergingInK(arg, k)) {
      logCount++;
      if (logCount > 63) return undefined;
      continue;
    }
    const d = polynomialDegreeInK(arg, k);
    if (d !== undefined) { polyDeg += d; continue; }
...
function oneSqrtSixtyThreeLogPolyEffectiveDeg(node: IRNode, k: IRNode): number | undefined {
  if (node.kind !== "apply" || !equals(node.head, MUL)) return undefined;
  let sqrtHalfDeg: number | undefined;
  let logCount = 0;
  let polyDeg = 0;
  ...
  if (sqrtHalfDeg === undefined || logCount !== 63) return undefined;
  return sqrtHalfDeg + polyDeg;
}

/** Phase 427 — Two-Sqrt × Sixty-Three-Log × polynomial numerator. */
function twoSqrtSixtyThreeLogPolyEffectiveDeg(node: IRNode, k: IRNode): number | undefined {
  if (node.kind !== "apply" || !equals(node.head, MUL)) return undefined;
  const sqrtHalfDegs: number[] = [];
  let logCount = 0;
  let polyDeg = 0;
  ...
  if (sqrtHalfDegs.length !== 2 || logCount !== 63) return undefined;
  return sqrtHalfDegs[0] + sqrtHalfDegs[1] + polyDeg;
}

/** Phase 428 — Three-Sqrt × Sixty-Three-Log × polynomial numerator. */
function threeSqrtSixtyThreeLogPolyEffectiveDeg(node: IRNode, k: IRNode): number | undefined {
  if (node.kind !== "apply" || !equals(node.head, MUL)) return undefined;
  const sqrtHalfDegs: number[] = [];
  let logCount = 0;
  let polyDeg = 0;
  ...
  if (sqrtHalfDegs.length !== 3 || logCount !== 63) return undefined;
  return sqrtHalfDegs[0] + sqrtHalfDegs[1] + sqrtHalfDegs[2] + polyDeg;
}

/** Phase 429 — Four-Sqrt × Sixty-Three-Log × polynomial numerator. */
function fourSqrtSixtyThreeLogPolyEffectiveDeg(node: IRNode, k: IRNode): number | undefined {
  if (node.kind !== "apply" || !equals(node.head, MUL)) return undefined;
  const sqrtHalfDegs: number[] = [];
  let logCount = 0;
  let polyDeg = 0;
  ...
  if (sqrtHalfDegs.length !== 4 || logCount !== 63) return undefined;
  return sqrtHalfDegs[0] + sqrtHalfDegs[1] + sqrtHalfDegs[2] + sqrtHalfDegs[3] + polyDeg;
}
 ...
 */
function fiveSqrtSixtyThreeLogPolyEffectiveDeg(node: IRNode, k: IRNode): number | undefined {
  if (node.kind !== "apply" || !equals(node.head, MUL)) return undefined;
  const sqrtHalfDegs: number[] = [];
  let logCount = 0;
  let polyDeg = 0;
  ...
  if (sqrtHalfDegs.length !== 5 || logCount !== 63) return undefined;
  return sqrtHalfDegs[0] + sqrtHalfDegs[1] + sqrtHalfDegs[2] + sqrtHalfDegs[3] + sqrtHalfDegs[4] + polyDeg;
}

/** Phase 419 — Zero-Sqrt × Sixty-Two-Log × polynomial numerator.
 * effectiveDeg = polyDeg (no sqrt factors).
 * Caller checks `denDeg > sixtyTwoLogPolyEffectiveDeg(num, k)`.
 */
function sixtyTwoLogPolyEffectiveDeg(node: IRNode, k: IRNode): number | undefined {
  if (node.kind !== "apply" || !equals(node.head, MUL)) return undefined;
  let logCount = 0;
  let polyDeg = 0;
  for (const arg of node.args) {
    ...
    if (isLogOfDivergingInK(arg, k)) {
      logCount++;
      if (logCount > 62) return undefined;
      continue;
    }
    const deg = polynomialDegreeInK(arg, k);
    if (deg !== undefined) { polyDeg += deg; continue; }
...
function oneSqrtSixtyTwoLogPolyEffectiveDeg(node: IRNode, k: IRNode): number | undefined {
  if (node.kind !== "apply" || !equals(node.head, MUL)) return undefined;
  let sqrtHalfDeg: number | undefined;
  let logCount = 0;
  let polyDeg = 0;
  ...
  if (sqrtHalfDeg === undefined || logCount !== 62) return undefined;
  return sqrtHalfDeg + polyDeg;
}

/** Phase 421 — Two-Sqrt × Sixty-Two-Log × polynomial numerator. */
function twoSqrtSixtyTwoLogPolyEffectiveDeg(node: IRNode, k: IRNode): number | undefined {
  if (node.kind !== "apply" || !equals(node.head, MUL)) return undefined;
  const sqrtHalfDegs: number[] = [];
  let logCount = 0;
  let polyDeg = 0;
  ...
  if (sqrtHalfDegs.length !== 2 || logCount !== 62) return undefined;
  return sqrtHalfDegs[0] + sqrtHalfDegs[1] + polyDeg;
}

/** Phase 422 — Three-Sqrt × Sixty-Two-Log × polynomial numerator. */
function threeSqrtSixtyTwoLogPolyEffectiveDeg(node: IRNode, k: IRNode): number | undefined {
  if (node.kind !== "apply" || !equals(node.head, MUL)) return undefined;
  const sqrtHalfDegs: number[] = [];
  let logCount = 0;
  let polyDeg = 0;
  ...
  if (sqrtHalfDegs.length !== 3 || logCount !== 62) return undefined;
  return sqrtHalfDegs[0] + sqrtHalfDegs[1] + sqrtHalfDegs[2] + polyDeg;
}

/** Phase 423 — Four-Sqrt × Sixty-Two-Log × polynomial numerator. */
function fourSqrtSixtyTwoLogPolyEffectiveDeg(node: IRNode, k: IRNode): number | undefined {
  if (node.kind !== "apply" || !equals(node.head, MUL)) return undefined;
  const sqrtHalfDegs: number[] = [];
  let logCount = 0;
  let polyDeg = 0;
  ...
  if (sqrtHalfDegs.length !== 4 || logCount !== 62) return undefined;
  return sqrtHalfDegs[0] + sqrtHalfDegs[1] + sqrtHalfDegs[2] + sqrtHalfDegs[3] + polyDeg;
}
 ...
 */
function fiveSqrtSixtyTwoLogPolyEffectiveDeg(node: IRNode, k: IRNode): number | undefined {
  if (node.kind !== "apply" || !equals(node.head, MUL)) return undefined;
  const sqrtHalfDegs: number[] = [];
  let logCount = 0;
  let polyDeg = 0;
  ...
  if (sqrtHalfDegs.length !== 5 || logCount !== 62) return undefined;
  return sqrtHalfDegs[0] + sqrtHalfDegs[1] + sqrtHalfDegs[2] + sqrtHalfDegs[3] + sqrtHalfDegs[4] + polyDeg;
}
 ...
 */
function sixtyLogPolyEffectiveDeg(node: IRNode, k: IRNode): number | undefined {
  if (node.kind !== "apply" || !equals(node.head, MUL)) return undefined;
  let logCount = 0;
  let polyDeg = 0;
  for (const arg of node.args) {
    ...
    if (isLogOfDivergingInK(arg, k)) {
      logCount++;
      if (logCount > 60) return undefined;
      continue;
    }
    const deg = polynomialDegreeInK(arg, k);
    if (deg !== undefined) { polyDeg += deg; continue; }
...
function oneSqrtSixtyLogPolyEffectiveDeg(node: IRNode, k: IRNode): number | undefined {
  if (node.kind !== "apply" || !equals(node.head, MUL)) return undefined;
  let sqrtHalfDeg: number | undefined;
  let logCount = 0;
  let polyDeg = 0;
  ...
  if (sqrtHalfDeg === undefined || logCount !== 60) return undefined;
  return sqrtHalfDeg + polyDeg;
}

/** Phase 409 — Two-Sqrt × Sixty-Log × polynomial numerator. */
function twoSqrtSixtyLogPolyEffectiveDeg(node: IRNode, k: IRNode): number | undefined {
  if (node.kind !== "apply" || !equals(node.head, MUL)) return undefined;
  const sqrtHalfDegs: number[] = [];
  let logCount = 0;
  let polyDeg = 0;
  ...
  if (sqrtHalfDegs.length !== 2 || logCount !== 60) return undefined;
  return sqrtHalfDegs[0] + sqrtHalfDegs[1] + polyDeg;
}

/** Phase 410 — Three-Sqrt × Sixty-Log × polynomial numerator. */
function threeSqrtSixtyLogPolyEffectiveDeg(node: IRNode, k: IRNode): number | undefined {
  if (node.kind !== "apply" || !equals(node.head, MUL)) return undefined;
  const sqrtHalfDegs: number[] = [];
  let logCount = 0;
  let polyDeg = 0;
  ...
  if (sqrtHalfDegs.length !== 3 || logCount !== 60) return undefined;
  return sqrtHalfDegs[0] + sqrtHalfDegs[1] + sqrtHalfDegs[2] + polyDeg;
}

/** Phase 411 — Four-Sqrt × Sixty-Log × polynomial numerator. */
function fourSqrtSixtyLogPolyEffectiveDeg(node: IRNode, k: IRNode): number | undefined {
  if (node.kind !== "apply" || !equals(node.head, MUL)) return undefined;
  const sqrtHalfDegs: number[] = [];
  let logCount = 0;
  let polyDeg = 0;
  ...
  if (sqrtHalfDegs.length !== 4 || logCount !== 60) return undefined;
  return sqrtHalfDegs[0] + sqrtHalfDegs[1] + sqrtHalfDegs[2] + sqrtHalfDegs[3] + polyDeg;
}
 ...
 */
function fiveSqrtSixtyLogPolyEffectiveDeg(node: IRNode, k: IRNode): number | undefined {
  if (node.kind !== "apply" || !equals(node.head, MUL)) return undefined;
  const sqrtHalfDegs: number[] = [];
  let logCount = 0;
  let polyDeg = 0;
  ...
  if (sqrtHalfDegs.length !== 5 || logCount !== 60) return undefined;
  return sqrtHalfDegs[0] + sqrtHalfDegs[1] + sqrtHalfDegs[2] + sqrtHalfDegs[3] + sqrtHalfDegs[4] + polyDeg;
}
...
function gVanishesAtInfinity(g: IRNode, k: IRNode): boolean {
  ...
  // Phase 430: five sqrt + 63 logs
  const s5l63Deg = fiveSqrtSixtyThreeLogPolyEffectiveDeg(num, k);
  if (s5l63Deg !== undefined) {
    const denDegS5l63 = polynomialDegreeInK(den, k);
    if (denDegS5l63 !== undefined) {
      if (denDegS5l63 > s5l63Deg) return true;
    } else if (hDivergesAtInfinity(den, k)) {
      return true;
    }
  }
  // Phase 429: four sqrt + 63 logs
  const s4l63Deg = fourSqrtSixtyThreeLogPolyEffectiveDeg(num, k);
  if (s4l63Deg !== undefined) {
    const denDegS4l63 = polynomialDegreeInK(den, k);
    if (denDegS4l63 !== undefined) {
      if (denDegS4l63 > s4l63Deg) return true;
    } else if (hDivergesAtInfinity(den, k)) {
      return true;
    }
  }
  // Phase 428: three sqrt + 63 logs
  const s3l63Deg = threeSqrtSixtyThreeLogPolyEffectiveDeg(num, k);
  if (s3l63Deg !== undefined) {
    const denDegS3l63 = polynomialDegreeInK(den, k);
    if (denDegS3l63 !== undefined) {
      if (denDegS3l63 > s3l63Deg) return true;
    } else if (hDivergesAtInfinity(den, k)) {
      return true;
    }
  }
  // Phase 427: two sqrt + 63 logs
  const s2l63Deg = twoSqrtSixtyThreeLogPolyEffectiveDeg(num, k);
  if (s2l63Deg !== undefined) {
    const denDegS2l63 = polynomialDegreeInK(den, k);
    if (denDegS2l63 !== undefined) {
      if (denDegS2l63 > s2l63Deg) return true;
    } else if (hDivergesAtInfinity(den, k)) {
      return true;
    }
  }
  // Phase 426: one sqrt + 63 logs
  const s1l63Deg = oneSqrtSixtyThreeLogPolyEffectiveDeg(num, k);
  if (s1l63Deg !== undefined) {
    const denDegS1l63 = polynomialDegreeInK(den, k);
    if (denDegS1l63 !== undefined) {
      if (denDegS1l63 > s1l63Deg) return true;
    } else if (hDivergesAtInfinity(den, k)) {
      return true;
    }
  }
  // Phase 425: zero sqrt + 63 logs
  const sl63Deg = sixtyThreeLogPolyEffectiveDeg(num, k);
  if (sl63Deg !== undefined) {
    const denDegSl63 = polynomialDegreeInK(den, k);
    if (denDegSl63 !== undefined) {
      if (denDegSl63 > sl63Deg) return true;
    } else if (hDivergesAtInfinity(den, k)) {
      return true;
    }
  }
  // Phase 424: five sqrt + 62 logs
  const s5l62Deg = fiveSqrtSixtyTwoLogPolyEffectiveDeg(num, k);
  if (s5l62Deg !== undefined) {
    const denDegS5l62 = polynomialDegreeInK(den, k);
    if (denDegS5l62 !== undefined) {
      if (denDegS5l62 > s5l62Deg) return true;
    } else if (hDivergesAtInfinity(den, k)) {
      return true;
    }
  }
  // Phase 423: four sqrt + 62 logs
  const s4l62Deg = fourSqrtSixtyTwoLogPolyEffectiveDeg(num, k);
  if (s4l62Deg !== undefined) {
    const denDegS4l62 = polynomialDegreeInK(den, k);
    if (denDegS4l62 !== undefined) {
      if (denDegS4l62 > s4l62Deg) return true;
    } else if (hDivergesAtInfinity(den, k)) {
      return true;
    }
  }
  // Phase 422: three sqrt + 62 logs
  const s3l62Deg = threeSqrtSixtyTwoLogPolyEffectiveDeg(num, k);
  if (s3l62Deg !== undefined) {
    const denDegS3l62 = polynomialDegreeInK(den, k);
    if (denDegS3l62 !== undefined) {
      if (denDegS3l62 > s3l62Deg) return true;
    } else if (hDivergesAtInfinity(den, k)) {
      return true;
    }
  }
  // Phase 421: two sqrt + 62 logs
  const s2l62Deg = twoSqrtSixtyTwoLogPolyEffectiveDeg(num, k);
  if (s2l62Deg !== undefined) {
    const denDegS2l62 = polynomialDegreeInK(den, k);
    if (denDegS2l62 !== undefined) {
      if (denDegS2l62 > s2l62Deg) return true;
    } else if (hDivergesAtInfinity(den, k)) {
      return true;
    }
  }
  // Phase 420: one sqrt + 62 logs
  const s1l62Deg = oneSqrtSixtyTwoLogPolyEffectiveDeg(num, k);
  if (s1l62Deg !== undefined) {
    const denDegS1l62 = polynomialDegreeInK(den, k);
    if (denDegS1l62 !== undefined) {
      if (denDegS1l62 > s1l62Deg) return true;
    } else if (hDivergesAtInfinity(den, k)) {
      return true;
    }
  }
  // Phase 419: zero sqrt + 62 logs
  const sl62Deg = sixtyTwoLogPolyEffectiveDeg(num, k);
  if (sl62Deg !== undefined) {
    const denDegSl62 = polynomialDegreeInK(den, k);
    if (denDegSl62 !== undefined) {
      if (denDegSl62 > sl62Deg) return true;
    } else if (hDivergesAtInfinity(den, k)) {
      return true;
    }
  }
  ...
  // Phase 412: five sqrt + 60 logs
  const s5l60Deg = fiveSqrtSixtyLogPolyEffectiveDeg(num, k);
  if (s5l60Deg !== undefined) {
    const denDegS5l60 = polynomialDegreeInK(den, k);
    if (denDegS5l60 !== undefined) {
      if (denDegS5l60 > s5l60Deg) return true;
    } else if (hDivergesAtInfinity(den, k)) {
      return true;
    }
  }
  // Phase 411: four sqrt + 60 logs
  const s4l60Deg = fourSqrtSixtyLogPolyEffectiveDeg(num, k);
  if (s4l60Deg !== undefined) {
    const denDegS4l60 = polynomialDegreeInK(den, k);
    if (denDegS4l60 !== undefined) {
      if (denDegS4l60 > s4l60Deg) return true;
    } else if (hDivergesAtInfinity(den, k)) {
      return true;
    }
  }
  // Phase 410: three sqrt + 60 logs
  const s3l60Deg = threeSqrtSixtyLogPolyEffectiveDeg(num, k);
  if (s3l60Deg !== undefined) {
    const denDegS3l60 = polynomialDegreeInK(den, k);
    if (denDegS3l60 !== undefined) {
      if (denDegS3l60 > s3l60Deg) return true;
    } else if (hDivergesAtInfinity(den, k)) {
      return true;
    }
  }
  // Phase 409: two sqrt + 60 logs
  const s2l60Deg = twoSqrtSixtyLogPolyEffectiveDeg(num, k);
  if (s2l60Deg !== undefined) {
    const denDegS2l60 = polynomialDegreeInK(den, k);
    if (denDegS2l60 !== undefined) {
      if (denDegS2l60 > s2l60Deg) return true;
    } else if (hDivergesAtInfinity(den, k)) {
      return true;
    }
  }
  // Phase 408: one sqrt + 60 logs
  const s1l60Deg = oneSqrtSixtyLogPolyEffectiveDeg(num, k);
  if (s1l60Deg !== undefined) {
    const denDegS1l60 = polynomialDegreeInK(den, k);
    if (denDegS1l60 !== undefined) {
      if (denDegS1l60 > s1l60Deg) return true;
    } else if (hDivergesAtInfinity(den, k)) {
      return true;
    }
  }
  // Phase 407: zero sqrt + 60 logs
  const sl60Deg = sixtyLogPolyEffectiveDeg(num, k);
  if (sl60Deg !== undefined) {
    const denDegSl60 = polynomialDegreeInK(den, k);
    if (denDegSl60 !== undefined) {
      if (denDegSl60 > sl60Deg) return true;
    } else if (hDivergesAtInfinity(den, k)) {
      return true;
    }
  }
