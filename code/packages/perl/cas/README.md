# CodingAdventures::Cas — Content-Addressable Storage (Perl)
...
| Package | Description |
|---|---|
| `CodingAdventures::Cas` | CAS wrapper: hashes, stores, verifies, prefix-resolves |
| `CodingAdventures::Cas::BlobStore` | Abstract base class for storage backends |
| `CodingAdventures::Cas::LocalDiskStore` | Filesystem backend (Git 2/38 fanout layout) |
| `CodingAdventures::Cas::Error` | Typed exception hierarchy |

## Quick start

```perl
use CodingAdventures::Cas;
use CodingAdventures::Cas::LocalDiskStore;

# Open (or create) a store rooted at /tmp/my-cas
my $store = CodingAdventures::Cas::LocalDiskStore->new('/tmp/my-cas');
...
```perl
use CodingAdventures::Cas::Error;

eval { $cas->get('0000000000000000000000000000000000000000') };
if (ref $@ && $@->isa('CodingAdventures::Cas::Error::CasNotFoundError')) {
    say "not found: ", $@->key;
} elsif (ref $@ && $@->isa('CodingAdventures::Cas::Error::CasCorruptedError')) {
    say "corrupted: ", $@->key;
} elsif ($@) {
...
Subclass `CodingAdventures::Cas::BlobStore` and override the four methods:
