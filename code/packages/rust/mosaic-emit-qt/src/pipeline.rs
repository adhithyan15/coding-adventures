...
pub struct EmitOptions {
    ...
    /// Require the standard Rust application runtime and emit no sample or
    /// nullable-host fallback. Used by the `native-complete` package profile.
...
pub struct ProjectFiles {
    ...
    /// REQUIRED COMPONENTS Quick QuickControls2 QmlImportScanner)`, executable
    /// target that links Qt6::Quick, Qt6::QuickControls2, and embeds the QML via
...
fn build_main_cpp(
    ...
    if require_runtime {
        ...
        out.push_str("#include <QStringList>\n");
        ...
        out.push_str("    mosaicHost.requireRuntime();\n");
        out.push_str("    const QVariantMap slotNames{\n");
        for slot in slots {
            writeln!(
                out,
        ...
        out.push_str("    };\n");
        out.push_str("    const QStringList requiredProps{\n");
        for slot in slots
            .iter()
            .filter(|slot| slot.required && slot.default.is_none())
        {
            writeln!(out, "      QStringLiteral(\"{}\"),", slot.name).unwrap();
        }
        out.push_str("    };\n");
        out.push_str("    mosaicHost.configureRequiredProps(slotNames, requiredProps);\n\n");
...
fn build_qt_readme(name: &str, module_name: &str, require_runtime: bool) -> String {
    let runtime_note = if require_runtime {
        "\n## Native-complete runtime contract\n\nThis shell requires Mosaic's standard `MosaicHost.h/.cpp` binding and a loadable Rust application library. It validates every required MIL slot before QML construction and exits with an explicit error instead of showing sample state when the runtime contract is not satisfied.\n"
    } else {
        ""
    };
    format!(
...
fn emit_qml_tree(
    ...
    let owners = qml_property_owners(&[
        &typography,
        &text_part_lines,
        &cell_lines,
        &builtin_owned,
    ...
    for line in qml_owned_lines(&builtin_owned, 3, &owners) {
        writeln!(out, "{pad}    {line}").unwrap();
    }
    ...
    if is_text {
        ...
        for line in qml_owned_lines(&cell_lines, 2, &owners) {
            writeln!(out, "{pad}    {line}").unwrap();
        }
        for line in qml_owned_lines(&text_part_lines, 1, &owners) {
            writeln!(out, "{pad}    {line}").unwrap();
        }
        for line in qml_owned_lines(&typography, 0, &owners) {
            writeln!(out, "{pad}    {line}").unwrap();
        }
...
fn emit_text_input_qml(
    ...
    let cell_lines = ctx
        ...
        .map(cell_text_style_lines)
        .unwrap_or_default();
    let owners = qml_property_owners(&[&typography, &part_lines, &cell_lines]);

    for line in qml_owned_lines(&part_lines, 1, &owners) {
        writeln!(out, "{inner_pad}{line}").unwrap();
    }
    for line in qml_owned_lines(&typography, 0, &owners) {
        writeln!(out, "{inner_pad}{line}").unwrap();
...
/// Decide, once, which source owns each top-level QML property.
///
/// #15155 -- a QML object may assign a property exactly once. "Property
/// value set multiple times" is a hard `qmlcachegen` error, not a warning,
/// so two writers landing on one object take the whole app's build down.
/// That is what happened when a part authored `color`: the part style and
/// the enclosing cell's text style both wrote `color:` onto one
/// `TextInput`, and the Trestle build failed three packages downstream of
/// the authored value.
///
/// `sources` is given HIGHEST PRECEDENCE FIRST; the first source naming a
/// property owns it. Callers then emit each source in its own positional
/// order, keeping only the lines it owns, so the generated file's shape is
/// unchanged apart from the duplicates that could never have compiled.
///
/// This replaces a one-property special case: the two text paths already
/// filtered `font.pixelSize:` by hand when a live typography binding
/// replaced it. That guarded the single overlap someone had hit, and left
/// `color` -- and any future overlap -- open.
fn qml_property_owners(sources: &[&[String]]) -> HashMap<String, usize> {
    let mut owner: HashMap<String, usize> = HashMap::new();
    for (idx, source) in sources.iter().enumerate() {
        for line in source.iter() {
            if let Some(property) = qml_top_level_property(line) {
                owner.entry(property.to_string()).or_insert(idx);
            }
        }
    }
    owner
...
/// The lines `source` (at precedence index `idx`) may emit.
///
/// A line whose property another source owns is dropped. When such a line
/// OPENS a block (`background: Rectangle {`), its children are dropped
/// with it -- otherwise the nested lines would be re-parented onto the
/// control and change its meaning, or fail to parse.
fn qml_owned_lines(source: &[String], idx: usize, owner: &HashMap<String, usize>) -> Vec<String> {
...
fn emit_host_button_qml(
    ...
    let button_owners = qml_property_owners(&[&typography, &button_part_lines]);
    for line in qml_owned_lines(&button_part_lines, 1, &button_owners) {
        writeln!(out, "{inner_pad}{line}").unwrap();
    }
    for line in qml_owned_lines(&typography, 0, &button_owners) {
        writeln!(out, "{inner_pad}{line}").unwrap();
    }
...
mod tests {
    ...
    fn ui32_main_cpp_loads_component_from_embedded_qml_module() {
        ...
        let style_setup = proj
            .main_cpp
            .find("QQuickStyle::setStyle(QStringLiteral(\"Basic\"));")
            .expect("main.cpp must choose a customization-capable default Controls style");
        let app_setup = proj
            .main_cpp
        ...
        assert!(
            proj.main_cpp
                .contains("qEnvironmentVariableIsEmpty(\"QT_QUICK_CONTROLS_STYLE\")"),
            "an explicit host Controls style must override Mosaic's default"
        );
    }
    ...
    fn ui32_project_shell_exposes_optional_mosaic_host_bridge() {
        ...
        assert!(
            proj.cmake_lists.contains(
                "target_link_libraries(Review PRIVATE Qt6::Quick Qt6::QuickControls2 Qt6::Widgets)"
            ),
            "CMake should link the Controls style and native file picker modules:\n{}",
            proj.cmake_lists
        );
        assert!(
    ...
    #[test]
    fn native_complete_project_requires_runtime_before_qml_construction() {
        let m = component(
            "Review",
            vec![slot("app-title", SlotType::Text, true)],
            vec![EmitDecl {
        ...
        let result = from_pipeline_with_options(&m, &l, &s, &opts).unwrap();
        let project = result.project.unwrap();

        assert!(result.output.contains("required property var mosaicHost"));
        assert!(result
            .output
            .contains("applyMosaicResponse(mosaicHost.handleRequiredEvent(event))"));
        assert!(!result.output.contains("mosaicHost ?"));
        ...
        assert!(project.main_cpp.contains("QStringLiteral(\"app-title\")"));
        let initial = project
            .main_cpp
            .find("view.setInitialProperties(initialProperties);")
            .unwrap();
        let source = project.main_cpp.find("view.setSource(url);").unwrap();
        ...
        assert!(!project
            .cmake_lists
            .contains("if(EXISTS \"${CMAKE_CURRENT_SOURCE_DIR}/MosaicHost.cpp\")"));
        assert!(project.readme.contains("Native-complete runtime contract"));
    }
...
mod qml_property_ownership_tests {
    ...
    fn the_more_specific_source_owns_a_contested_property() {
        ...
        let owners = qml_property_owners(&[&typography, &part, &cell]);
        assert_eq!(owners.get("color"), Some(&1), "the part style must own color");

        let part_out = qml_owned_lines(&part, 1, &owners);
        let cell_out = qml_owned_lines(&cell, 2, &owners);
    ...
    fn a_live_typography_binding_still_outranks_the_styles() {
        ...
        let owners = qml_property_owners(&[&typography, &part, &cell]);
        assert_eq!(owners.get("font.pixelSize"), Some(&0));
        let cell_out = qml_owned_lines(&cell, 2, &owners);
    ...
    fn dropping_a_block_opener_drops_its_children() {
        ...
        let owners = qml_property_owners(&[&winner, &loser]);
        let out = qml_owned_lines(&loser, 1, &owners);
        assert_eq!(
            out,
