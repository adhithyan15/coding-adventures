# Changelog — CodingAdventures::WasmOpcodes (Perl)

## [0.01] — 2026-03-29

### Added

- `%OPCODES` hash with 50+ opcode entries covering all MVP opcode categories:
...
- `is_valid_opcode($byte)` — hash-based predicate.
- `get_opcode_info($byte)` — returns full entry hashref or undef.
- Test suite (`t/00-load.t`, `t/01-basic.t`) with 100+ assertions covering all
  opcode categories, unknown byte fallbacks, and structural invariants.
